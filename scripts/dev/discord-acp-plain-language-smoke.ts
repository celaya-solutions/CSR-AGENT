#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports OpenAgent repository automation.
...
function usage(): string {
  return (
    ...
    "1) OpenAgent spawned an ACP thread binding\n" +
    ...
    "  --state-dir <p>              Override OpenAgent state dir for plugin-state polling\n" +
    "  --openclaw-bin <path>        OpenAgent CLI binary for driver=openclaw (default: openclaw)\n" +
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from OpenAgent.`,
        diagnostics: {
          bindingCandidates: [
            {
