...
/** Resolve package peer dependency metadata for the OpenAgent plugin API. */
...
export async function buildPluginNpmRuntime(params: PluginNpmRuntimeBuildParams) {
  ...
  if (missingHostExports.length > 0) {
    throw new Error(
      `${plan.pluginDir} runtime imports missing OpenAgent host exports: ${missingHostExports.join(", ")}`,
    );
  }
...
async function preparePluginNativeImport(params: PluginNpmRuntimeBuildParams) {
  ...
  const hostManifest = readRootJsonObjectSync({
    ...
    boundaryLabel: "OpenAgent source checkout",
  });
  if (
    !hostManifest.ok ||
    ...
    throw new Error("Native-import preparation must run from an OpenAgent source checkout root.");
  }
  ...
  if (
    ...
    throw new Error("Host SDK output is missing; build OpenAgent before preparing native imports.");
  }
