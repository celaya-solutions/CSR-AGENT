...
# Runs the OpenAgent first-run Docker smoke against the package-installed
...
echo "Running in-container OpenAgent first-run smoke..."
...
if [ "$status" -ne 0 ]; then
  echo "Docker OpenAgent first-run smoke failed"
