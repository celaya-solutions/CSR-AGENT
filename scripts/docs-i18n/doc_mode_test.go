...
func (docFrontmatterTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	replacer := strings.NewReplacer(
		"Step-by-step Fly.io deployment for OpenAgent with persistent storage and HTTPS", "在 Fly.io 上逐步部署 OpenAgent，包含持久化存储和 HTTPS",
		"Deploying OpenAgent on Fly.io", "在 Fly.io 上部署 OpenAgent",
...
func (docFrontmatterFallbackTranslator) Translate(_ context.Context, text, _, _ string) (string, error) {
	switch text {
	case "Step-by-step Fly.io deployment for OpenAgent with persistent storage and HTTPS":
		return strings.Join([]string{
			"<frontmatter>",
			"title: Fly.io",
			"summary: \"在 Fly.io 上部署 OpenAgent 的逐步指南，包含持久化存储和 HTTPS 设置\"",
			"read_when:",
			"  - 在 Fly.io 上部署 OpenAgent",
			"  - 设置 Fly 卷、机密和初始运行配置",
			"</frontmatter>",
			"",
			"<body>",
	...
	case "Deploying OpenAgent on Fly.io":
		return "在 Fly.io 上部署 OpenAgent", nil
...
func TestProcessFileDocUsesFieldLevelFrontmatterTranslation(t *testing.T) {
	...
	source := strings.Join([]string{
		"---",
		"title: Fly.io",
		"summary: \"Step-by-step Fly.io deployment for OpenAgent with persistent storage and HTTPS\"",
		"read_when:",
		"  - Deploying OpenAgent on Fly.io",
	...
	text := string(output)
	if !strings.Contains(text, "在 Fly.io 上逐步部署 OpenAgent，包含持久化存储和 HTTPS") {
		t.Fatalf("expected translated summary in output:\n%s", text)
	}
	if !strings.Contains(text, "在 Fly.io 上部署 OpenAgent") {
...
func TestProcessFileDocRejectsSuspiciousFrontmatterScalarExpansion(t *testing.T) {
	...
	if !strings.Contains(text, "summary: Step-by-step Fly.io deployment for OpenAgent with persistent storage and HTTPS") {
...
func TestValidateDocChunkTranslationRejectsMovedProtectedProductLinkLabel(t *testing.T) {
	...
	source := "Deploy OpenAgent on [Render](https://render.com) using the Blueprint.\n"
	translated := "Render पर Blueprint का उपयोग करके [OpenAgent](https://render.com) परिनियोजित करें।\n"
...
func TestValidateDocBodyRejectsMovedProtectedReferenceLinkLabel(t *testing.T) {
	...
	translated := "Auf Render mit [OpenAgent][provider] bereitstellen.\n\n[provider]: https://render.com\n"
	err := validateDocBodyFencedLiterals(source, translated)
	if err == nil || !strings.Contains(err.Error(), "protected link label mismatch") {
