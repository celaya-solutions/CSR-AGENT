...
# Checks both Claude Code and OpenAgent auth status
...
echo ""
echo "OpenAgent Auth (~/.openclaw/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if systemctl --user is-active openclaw >/dev/null 2>&1; then
    echo -e "OpenAgent service: ${GREEN}running${NC}"
else
    echo -e "OpenAgent service: ${RED}NOT running${NC}"
fi
