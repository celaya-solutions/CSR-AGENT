...
function reusedEvidenceChildren() {
  return [
    ...
    ["releaseChecks", "303", "OpenAgent Release Checks"],
    ["productPerformance", "505", "OpenAgent Performance"],
  ].map(([role, runId, name]) => ({
...
case "$run_id" in
  ...
  303) title="OpenAgent Release Checks full-release-validation-77-1-release-checks"; workflow="openclaw-release-checks.yml" ;;
  505) title="OpenAgent Performance full-release-validation-77-1"; workflow="openclaw-performance.yml" ;;
esac
status="completed"
