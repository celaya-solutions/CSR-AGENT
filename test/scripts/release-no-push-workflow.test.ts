...
describe("release validation no-push transport", () => {
  ...
  it.each([
    ...
    const validate = step(
      ...
      "Validate OpenAgent Docker E2E package",
    );
  ...
  it("records exact adopted child identity without monitoring or cancellation", () => {
    ...
    for (const [jobName, stepName] of [
      ...
      ["performance", "Dispatch OpenAgent Performance"],
    ] as const) {
  ...
  it("builds shared images locally, verifies artifacts, and cannot fall back to a registry", () => {
    ...
    const packageIdentity = step(dockerProducer, "Validate OpenAgent package artifact identity");
    ...
    for (const [name, condition] of [
      [
        "Download current-run OpenAgent Docker E2E package",
        "inputs.package_artifact_run_id == github.run_id",
      ],
      [
        "Download previous-run OpenAgent Docker E2E package",
        "inputs.package_artifact_run_id != github.run_id",
      ],
    ] as const) {
    ...
    const validatePackage = step(dockerProducer, "Validate OpenAgent Docker E2E package");
    ...
    for (const name of [
      ...
      expect(step(consumer, "Download OpenAgent Docker E2E package").with).toMatchObject({
