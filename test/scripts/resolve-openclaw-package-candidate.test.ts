...
describe("resolve-openclaw-package-candidate", () => {
  ...
  it("accepts only OpenAgent release package specs for npm candidates", () => {
    for (const spec of [
      "openclaw@beta",
      "openclaw@alpha",
...
`,
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    for (const filename of unsafeFilenames) {
      ...
      ).rejects.toThrow("npm pack reported unsafe OpenAgent tarball filename");
    }
  });
...
`,
    ...
    try {
      await expect(
        ...
        `package_ref ${sourceSha} resolved to ${sourceSha}, which is not reachable from an OpenAgent branch or release tag`,
      );
    } finally {
      process.env.PATH = previousPath;
      delete process.env.FAKE_GIT_LOG;
