...
describe("Parallels smoke model selection", () => {
  ...
  it("extracts the last OpenAgent version from a bounded log tail", async () => {
    const tempDir = makeTempDir(tempDirs, "openclaw-parallels-log-tail-");
    const logPath = join(tempDir, "phase.log");
    writeFileSync(logPath, ["OpenAgent 0.0.1", "x".repeat(4 * 1024 * 1024)].join("\n"));
  ...
  it("owns the reusable Windows VM and OpenAgent baseline lifecycle", () => {
...
if (commandArgs[0] === "list") {
  ...
  it("resolves Windows OpenAgent commands without assuming the npm shim path", () => {
