...
printf 'BBBBB22222\\t0\\tBeta Team\\r\\n'`,
  ...
  it("prefers the canonical OpenAgent iOS team when it is present", async () => {
...
if [[ "$1" == "-extract" && "$2" == "IDEProvisioningTeamByIdentifier" ]]; then
  cat <<'JSON'
{"account-id":[{"teamID":"FWJYW4S8P8","teamName":"Celaya Solutions","isFreeProvisioningTeam":false,"teamType":"Company"}]}
JSON
  exit 0
fi
echo '{}'`,
  ...
  it("fails canonical-only resolution when only fallback teams are available", () => {
    ...
    expect(result.stderr).toContain(
      `Canonical OpenAgent iOS Team ID '${CANONICAL_TEAM_ID}' is not available`,
    );
  });
  ...
  it("rejects explicit non-canonical teams in canonical-only mode", () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.stderr).toContain("is not the canonical OpenAgent iOS team");
  });
