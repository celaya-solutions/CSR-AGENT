...
function createCanonicalNodeMigrationHarness(mac: MacScriptFixture, nodeId = "fixture-node") {
  ...
  writeFileSync(
    envPath,
    [
      "# Generated by Zero to Agent. Do not edit while the gateway service is installed.",
...
function createArtifactVerificationHarness(mac: MacScriptFixture) {
  ...
  const binDir = path.join(tempRoot, "bin");
  const archivePath = path.join(tempRoot, "Zero to Agent-fixture-stable.zip");
  const installerPath = path.join(tempRoot, "Zero to Agent-fixture-stable-installer.sh");
  const receiptPath = path.join(tempRoot, "Zero to Agent-fixture-stable.json");
    ...
    [
      ...
      `builtin printf '%s\\n' ${quoteCliArg(
        [
          ...
          "  fi",
          '  if [ "${TEST_SYMLINK_DAMAGED_APP_BEFORE_CUSTODY:-0}" = "1" ] && echo "$3" | grep -q \'[.]failed-elevation-host-.*[/]Zero to Agent[.]app$\'; then',
          '    rm -rf "$2"',
          ...
          "  fi",
          '  if [ "${TEST_REPLACE_DAMAGED_APP_DIRECTORY_BEFORE_CUSTODY:-0}" = "1" ] && echo "$3" | grep -q \'[.]failed-elevation-host-.*[/]Zero to Agent[.]app$\'; then',
          '    /bin/mv "$2" "$2.race-displaced"',
          ...
          '  /bin/mv "$2" "$3" || exit $?',
          '  if [ "${TEST_REMOVE_CUA_DRIVER_AFTER_UNSAFE_ENTRY_MOVE:-0}" = "1" ] && echo "$3" | grep -q \'elevation-host[.]quarantined-app[.].*[/]Zero to Agent[.]app$\' && [ -L "$3" ]; then',
          ...
          "    printf '%s\\n' replacement-owner >\"$2\"",
          "  fi",
          '  if [ "${TEST_RECREATE_APP_DURING_DAMAGED_CUSTODY:-0}" = "1" ] && echo "$3" | grep -q \'[.]failed-elevation-host-.*[/]Zero to Agent[.]app$\'; then',
          '    mkdir -p "$2/Contents/MacOS"',
          "    printf '%s\\n' replacement >\"$2/Contents/replacement\"",
          "  fi",
          '  if [ "${TEST_SIGNAL_DURING_RECOVERY_APP_MOVE:-0}" = "1" ] && echo "$3" | grep -q \'[.]failed-elevation-host-.*[/]Zero to Agent[.]app$\'; then',
          '    kill -TERM "$PPID"',
          "  fi",
          "  exit 0",
          "fi",
...
function createInstallRollbackHarness(
  ...
  writeExecutable(path.join(appPath, "Contents", "MacOS", "Zero to Agent"), "#!/bin/sh\nexit 0\n");
  ...
  writeExecutable(
    ...
    [
      ...
      `printf '%s\\n' p777777 n${JSON.stringify(path.join(appPath, "Contents", "MacOS", "Zero to Agent"))}`,
      "",
    ].join("\n"),
  );
...
describe("mac elevation host command contract", () => {
  ...
  it.concurrent("documents package and transactional lifecycle commands without probing macOS", async ({
    ...
    mac.lifetime.run(async () => {
      ...
      expect(result.stdout).toContain("never rewrites ordinary Zero to Agent");
    }));
  ...
  it.concurrent("keeps the elevation service separate and fail-closed", async ({ mac }) =>
    mac.lifetime.run(async () => {
      ...
      expect(script).toContain("conflicting Zero to Agent launch agent is installed");
      expect(script).toContain("unsupervised or conflicting Zero to Agent process is running");
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        const oldBinary = readFileSync(
          path.join(harness.appPath, "Contents", "MacOS", "Zero to Agent"),
        );
        ...
        expect(
          readFileSync(path.join(harness.appPath, "Contents", "MacOS", "Zero to Agent")),
        ).toEqual(oldBinary);
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        expect(recovered.stderr).toContain(
          "recovery failed and the current Zero to Agent installation could not be restored completely",
        );
        expect(readFileSync(harness.launchStateFile, "utf8").trim()).toBe("elevation-absent");
        expect(existsSync(harness.elevationPlist)).toBe(false);
        expect(existsSync(path.join(harness.appPath, "Contents", "Resources", "cua-driver"))).toBe(
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        expect(result.status).toBe(1);
        expect(result.stderr).toContain(
          "installed Zero to Agent app does not pass strict signature and identity validation",
        );
        expect(readFileSync(harness.sourcePlist, "utf8")).toBe(harness.sourceContents);
        expect(readFileSync(harness.launchStateFile, "utf8").trim()).toBe("source-loaded");
        expect(existsSync(path.join(harness.stateDir, "elevation-host-install.json"))).toBe(false);
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        expect(result.status).toBe(1);
        expect(result.stderr).toContain("a Zero to Agent app process survived owner shutdown");
        expect(result.stderr).toContain("automatic elevation-host rollback was incomplete");
        expect(
          readFileSync(path.join(harness.appPath, "Contents", "MacOS", "Zero to Agent")),
        ).toEqual(oldBinary);
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    "never treats a live but uninspectable Zero to Agent PID as quiescent",
    async ({ mac }) =>
      mac.lifetime.run(async () => {
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        expect(recovered.status).toBe(1);
        expect(recovered.stderr).toContain(
          "current Zero to Agent app has an unsupported entry type",
        );
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        expect(recovered.status).toBe(1);
        expect(recovered.stderr).toContain(
          "could not restore the previous Zero to Agent installation completely",
        );
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        expect(recovered.status).toBe(1);
        expect(recovered.stderr).toContain(
          "current Zero to Agent installation could not be restored completely",
        );
        expect(readFileSync(installReceiptPath, "utf8")).toBe(currentReceipt);
  ...
  it.skipIf(process.platform !== "darwin").concurrent(
    ...
    async ({ mac }) =>
      mac.lifetime.run(async () => {
        ...
        expect(recovered.status).toBe(1);
        expect(recovered.stderr).toContain(
          "could not restore the previous Zero to Agent installation",
        );
        expect(readFileSync(harness.sourcePlist, "utf8")).toBe("replacement owner\n");
        expect(
          readFileSync(path.join(harness.appPath, "Contents", "MacOS", "Zero to Agent")),
        ).toEqual(installedBinary);
        expect(readFileSync(harness.launchStateFile, "utf8").trim()).toBe("elevation-loaded");
      }),
  );
  ...
  it.concurrent("relaunches an adopted app only after exact exit with its selected state and config", async ({
    ...
    mac.lifetime.run(async () => {
      ...
      expect(installBody.indexOf("CUTOVER_ADOPTION_TERMINATION_SENT=1")).toBeLessThan(
        installBody.indexOf("adopted Zero to Agent process did not exit"),
      );
