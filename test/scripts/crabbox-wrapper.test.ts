...
describe("scripts/crabbox-wrapper", () => {
  ...
  it("fails closed when provider readiness reports broker auth failure", () => {
    ...
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain(
      "provider=aws requires managed Crabbox broker authentication for OpenAgent proof",
    );
  ...
  it("fails closed when doctor does not classify an auth failure", () => {
    ...
    expect(result.status).toBe(2);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("provider=aws failed readiness for OpenAgent proof");
  ...
  it("does not treat an injected Azure Windows default as direct intent", () => {
    ...
    expect(result.status).toBe(2);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("provider=azure failed readiness for OpenAgent proof");
  });
  ...
  it.each(["azure", "daytona"])(
    ...
    (provider) => {
      ...
      expect(result.status).toBe(2);
      expect(result.stdout).toBe("");
      expect(result.stderr).toContain(`provider=${provider} failed readiness for OpenAgent proof`);
    },
  );
...
`,
  ...
  it("preflights Swift 6.3 for raw AWS macOS Swift app builds", () => {
    ...
    expect(remoteCommand).toContain("OpenAgent macOS app proof requires Swift tools 6.3+");
    expect(remoteCommand).toContain("xcodebuild -version");
    expect(remoteCommand).toContain("OpenAgent macOS app proof requires Xcode 26.4+");
    expect(remoteCommand).not.toContain("openclaw_crabbox_bootstrap_macos_js");
    expectGroupedShellCommand(
      remoteCommand,
      "swift build --package-path apps/macos --product OpenAgent",
    );
  });
  ...
  it("preflights Swift for AWS macOS script-stdin Swift builds", () => {
    ...
    expect(output.scriptContent).toContain("OpenAgent macOS app proof requires Swift tools 6.3+");
    expect(output.scriptContent).toContain("OpenAgent macOS app proof requires Xcode 26.4+");
    expect(output.scriptContent).toContain(`\n${script}`);
  });
