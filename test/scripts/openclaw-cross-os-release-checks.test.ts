...
describe("scripts/openclaw-cross-os-release-checks", () => {
  ...
  it("restarts an exited manual gateway once after the exact startup migration refusal", async () => {
    await withTempDirAsync("openclaw-cross-os-gateway-restart-", async (dir) => {
      const refusal =
        "OpenAgent plugin migration inputs changed during startup convergence; refusing to report the gateway ready.";
  ...
  it("gives the Windows packaged updater wrapper enough headroom for OpenAgent timeout output", () => {
  ...
  it("wraps installed Windows CLI cmd fallbacks without Node shell argv", () => {
    expect(
      resolveInstalledCliInvocation(
        win32.join(String.raw`C:\OpenAgent Prefix`, "openclaw.cmd"),
      ...
      args: [
        ...
        "/s",
        "/c",
        String.raw`""C:\OpenAgent Prefix\openclaw.cmd" gateway run --port 1234"`,
      ],
      shell: false,
      windowsVerbatimArguments: true,
    });
