...
describe("k8s manifests", () => {
  ...
  it("keeps config and persistence manifests aligned with the gateway", () => {
    ...
    const gateway = assertRecord(config.gateway, "openagent config gateway");
    const auth = assertRecord(gateway.auth, "openagent config auth");
    const agents = assertRecord(config.agents, "openagent config agents");
    const defaults = assertRecord(agents.defaults, "openagent config agent defaults");
    ...
    expect(data["AGENTS.md"]).toContain("OpenAgent Assistant");
