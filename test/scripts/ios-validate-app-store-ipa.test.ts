...
async function writeIpaFixture(root: string): Promise<string> {
  ...
  const ipaPath = path.join(root, "OpenAgent.ipa");
...
async function writeValidFixture(
  ...
  const infoBody = [
    ...
    plistString("CFBundleDisplayName", options.displayName ?? "OpenAgent"),
    ...
    plistString(
      ...
      "OpenAgent reads Health data for Health Summaries.",
    ),
    options.healthUpdateUsage === null
      ...
      : typeof options.healthUpdateUsage === "boolean"
        ...
        : plistString(
            ...
            options.healthUpdateUsage ?? "OpenAgent reads Health data for Health Summaries.",
          ),
  ...
  writeFileSync(
    ...
    plist(
      options.localizedDisplayName === undefined
        ? plistString("NSCameraUsageDescription", "OpenAgent verwendet die Kamera.")
  ...
  writeFileSync(
    ...
    plist(
      [
        plistString("Name", "OpenAgent App Store ai.openclawfoundation.app"),
...
describe("scripts/ios-validate-app-store-ipa.sh", () => {
  ...
  it("rejects an IPA with the wrong canonical display name", async () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "openclaw-ios-ipa-"));
    tempDirs.push(root);
    const fixture = await writeValidFixture(root, { displayName: "OpenAgent Debug" });
