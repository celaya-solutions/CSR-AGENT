...
describe("scripts/ios-release-signing.mjs", () => {
  ...
  it("emits manual App Store profile settings for every signed target", () => {
    ...
    expect(output).toContain(
      "OPENCLAW_APP_PROFILE = OpenAgent App Store ai.openclawfoundation.app",
    );
    expect(output).toContain(
      "OPENCLAW_SHARE_PROFILE = OpenAgent App Store ai.openclawfoundation.app.share",
    );
    expect(output).toContain(
      "OPENCLAW_ACTIVITY_WIDGET_PROFILE = OpenAgent App Store ai.openclawfoundation.app.activitywidget",
    );
    expect(output).toContain(
      "OPENCLAW_WATCH_APP_PROFILE = OpenAgent App Store ai.openclawfoundation.app.watchkitapp",
    );
