...
describe("scripts/k8s/deploy.sh", () => {
  ...
  it("keeps a custom namespace and unrelated workloads when the legacy delete mode is used", () => {
    ...
    expect(result.status, output).toBe(0);
    expect(output).toContain("Deleting OpenAgent resources from namespace 'my-namespace'");
  ...
  it("deletes OpenAgent resources without deleting the namespace", () => {
  ...
  it("surfaces resource delete failures instead of reporting teardown success", () => {
    ...
    expect(output).toContain("Deleting OpenAgent resources from namespace 'restricted-namespace'");
    expect(calls).toEqual([
      "cluster-info",
