...
export async function runCiGitStep(options: {
  ...
  job?: string;
  action?: "ensure-base-commit" | "git-owner" | "publish-generated-pr";
  ...
  const externalOwner = options.workflow || publisher;
  const clock = {
    ...options,
    realDrain:
  ...
  return withCiCheckoutFixture(
    ...
    (root) => {
      ...
      for (const action of ["git-owner", "ensure-base-commit"]) {
        ...
        if (action === "git-owner" && (publisher || pluginRelease || releaseAdmission)) {
