  ...
  it("keeps shared install helpers parsing and verifying installed CLI versions", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        'case "${1:-}" in',
        "  --version)",
        "    printf 'OpenAgent v2026.6.21-beta.1\\r\\n'",
        "    ;;",
        "  --help)",
        "    printf 'usage\\n'",
        "    ;;",
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          ...
          "printf 'parsed=%s\\n' \"$(extract_openclaw_semver 'OpenAgent v2026.6.21-beta.1+build.7')\"",
...
`,
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an OpenAgent branch or release tag");
  });
...
if (args[0] === "--version") {
  console.log("OpenAgent " + fs.readFileSync(process.env.FAKE_VERSION_FILE, "utf8"));
...
`,
  ...
  it("requires Bun to trust and execute OpenAgent lifecycle scripts", () => {
    ...
    expect(blocked.stderr).toContain("OpenAgent lifecycle scripts remain blocked by Bun");
    ...
    expect(skipped.stderr).toContain("OpenAgent package lifecycle did not complete");
  });
...
if [[ "\${1:-}" == */openclaw.mjs ]]; then
  if [ "$FAKE_BUN_RUNTIME" = "unsupported" ]; then
    echo 'openclaw: the Bun runtime is unsupported because OpenAgent requires node:sqlite.' >&2
    exit 1
  fi
...
const args = process.argv.slice(2);
if (args[0] === "--version") {
  console.log("OpenAgent 2026.6.17");
} else if (args[0] === "--help") {
