...
describe("real-behavior-proof-policy", () => {
    ...
    const evaluation = evaluatePullRequestContext({
      pullRequest: externalPr(
        [
          ...
          "",
          "- Behavior addressed: The gateway dropped the configured Discord channel during startup.",
          "- Real environment tested: macOS 15.4, Node 24, local OpenAgent gateway.",
  ...
  it("accepts out-of-scope follow-ups as not-tested proof detail", () => {
    const body = [
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: OpenAgent 2026.5.21`",
      "",
  ...
  it("accepts source PR proof when explicit gaps live in out-of-scope follow-ups", () => {
    const body = [
      ...
      "## Evidence",
      "",
      "- Real environment tested: Local macOS source checkout, Node v24.8.0, OpenAgent 2026.5.21 (c8a35c4), local `openclaw` shim pointed at the freshly built checkout. No channel credentials or provider API keys were used.",
      "- Exact steps or command run after this patch:",
      "  1. Built the local checkout with `node --import tsx scripts/build-all.mts`.",
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: OpenAgent 2026.5.21 (c8a35c4)`",
      "",
      "## Out-of-scope Follow-ups",
  ...
  for (const { name, login, userType, expectedPassed } of [
    ...
    {
      name: "accepts exact OpenAgent ClawSweeper bot pass verdict markers when GitHub omits the app source",
