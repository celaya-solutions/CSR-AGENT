...
describe("install.sh", () => {
  ...
  it("re-execs a streamed installer on Darwin Bash 5.3+ without leaving a temp file", (context) => {
    ...
    try {
      ...
      expect(result.stdout).toContain("OpenAgent installer (macOS + Linux)");
...
EOF
  ...
  it("uses a blobless partial clone for new git installs", () => {
    const result = runInstallShell(`
      ...
      run_quiet_step() {
        printf 'step:%s|%s\\n' "$1" "\${*:2}"
        if [[ "$1" == "Cloning OpenAgent" ]]; then
          target="\${*: -1}"
          mkdir -p "$target/.git"
          printf 'complete\\n' > "$target/checkout.marker"
        elif [[ "$1" == "Building OpenAgent" ]]; then
          mkdir -p "$repo/dist"
          printf '%s\\n' 'process.stdout.write("fixture-version\\n");' > "$repo/dist/entry.js"
        fi
        return 0
    ...
    expect(result.status, JSON.stringify(result)).toBe(0);
    expect(result.stdout).toContain(
      "step:Cloning OpenAgent|git clone --filter=blob:none https://github.com/openclaw/openclaw.git",
    );
  ...
  it.each([
    ...
    ({ configured, upgrade, verify, doctorExit, verifyExit, onboard, expectedStatus }) => {
      ...
      const output = result.stdout;
      const successMatches = output.match(/OpenAgent installed successfully/g) ?? [];
  ...
  it("links the executable package launcher when dist/entry.js is not executable", () => {
    ...
    writeFileSync(
      ...
      '#!/usr/bin/env node\nprocess.stdout.write("OpenAgent fixture\\n");\n',
    );
    ...
    try {
      ...
      expect(result.status, result.stderr || result.stdout).toBe(0);
      expect(result.stdout).toContain("OpenAgent fixture");
    } finally {
      rmSync(tmp, { force: true, recursive: true });
    }
  });
  ...
  it.each([
    ...
    `,
      ...
      ui_stage() {
        if [[ "$1" == "Installing OpenAgent" ]]; then
          printf 'selected=%s\\n' "$(command -v node)"
          exit 0
        fi
      }
...
describe("install.sh duplicate OpenAgent install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple OpenAgent global installs detected");
  ...
  it("stays quiet when only one OpenAgent npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple OpenAgent global installs detected");
  });
