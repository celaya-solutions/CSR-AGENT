...
describe("check-package-dist-imports", () => {
  ...
  it.each([
    ...
    ({ leading, tail, accepted }) => {
      ...
      if (accepted) {
        expect(result.stdout).toContain("OpenAgent package dist import closure passed.");
      } else {
        expect(result.stderr).toContain("Unexpected package dist import check argument");
        expect(result.stdout).not.toContain("OpenAgent package dist import closure passed.");
      }
    },
  );
