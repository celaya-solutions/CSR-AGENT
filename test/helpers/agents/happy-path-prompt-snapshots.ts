...
const CODEX_WORKSPACE_BOOTSTRAP_PROMPT_CONTEXT = [
  "OpenAgent loaded these user-editable workspace files for the current turn. Codex loads AGENTS.md natively. SOUL.md, IDENTITY.md, and USER.md are prepared separately from user input and are not repeated here.",
  "",
  "# Project Context",
  "",
...
const CODEX_WORKSPACE_TURN_SCOPED_DEVELOPER_INSTRUCTIONS = [
  "## OpenAgent Agent Soul",
  "",
  "OpenAgent loaded these workspace instruction files from the active agent workspace. They are the canonical definitions of who you are, how you think and work, and the human you work alongside. Internalize and follow them accordingly.",
  "",
...
async function createScenarios(codexApi: CodexPromptSnapshotApi): Promise<PromptScenario[]> {
  ...
  const discordGroupCtx: TemplateContext = {
    ...
    SenderName: "Pash",
    SenderUsername: "pash",
    GroupSubject: "OpenAgent maintainers",
    GroupChannel: "#agent-sandbox",
    GroupSpace: "OpenAgent",
    ConversationLabel: "OpenAgent/#agent-sandbox",
    WasMentioned: true,
    InboundHistory: [
      {
        sender: "Peter",
      ...
      {
        sender: "Pash",
        body: "@OpenAgent please verify the Codex happy path too.",
      },
    ],
    Body: "@OpenAgent can you audit whether this prompt path has conflicting silence instructions?",
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to OpenAgent/Codex.",
      ],
      trigger: "user",
      ctx: telegramDirectCtx,
      prompt: createPrompt(
...
function renderModelBoundPromptLayers(params: {
  ...
  const additionalContextLayers = Object.entries(
    ...
    .map(([key, entry]) => {
      ...
      return {
        heading: `### ${role}: OpenAgent Additional Context (${key})`,
  ...
  return [
      ...
      stableJson({
        ...
        openClawRuntime: {
          ...
          workspaceBootstrapContextFrom:
            "extensions/codex app-server turn/start input OpenAgent runtime context",
    ...
    "",
    "### Request Instructions: OpenAgent Parent-Local Context",
    "",
    ...
    "",
    "### Developer: OpenAgent Runtime Instructions",
    "",
...
function buildCodexOpenClawRuntimeContext(): string {
  return [
    "OpenAgent runtime context for this turn:",
    "Treat this OpenAgent-provided context as supporting project/user reference for the current request.",
    "",
    "## OpenAgent Workspace Context",
    "",
...
function renderScenarioSnapshot(
  ...
  return [
    ...
    "- This captures the OpenAgent-owned Codex app-server inputs and reconstructs the stable Codex model/permission layers from committed Codex prompt fixtures.",
...
function renderReadme(scenarios: PromptScenario[]): string {
  return [
    ...
    "",
    "The materialized Markdown snapshots show selected app-server thread/turn params plus a reconstructed model-bound prompt layer stack: Codex `gpt-5.5` model instructions from a pinned Codex model catalog fixture, Codex permission developer instructions for the happy-path yolo profile, OpenAgent developer instructions, turn input with simulated OpenAgent workspace bootstrap runtime context, and references to the complete dynamic tool catalog.",
    "",
    "The workspace bootstrap simulation includes dummy workspace contents so prompt reviewers can see how managed OpenAgent inference adds profile files to parent-only request instructions and keeps `MEMORY.md` in turn input. `AGENTS.md` is intentionally not repeated here because Codex loads it natively.",
    "",
    "The tool catalog is pinned to the canonical happy-path OpenAgent tools so optional locally installed plugin tools do not create fixture churn.",
    "",
    ...
    "",
    "These snapshots are still not a byte-for-byte raw OpenAI request capture. Codex-owned native `AGENTS.md`, environment context, memories, app/plugin instructions, and built-in collaboration-mode instructions can be added inside the Codex runtime after OpenAgent sends thread and turn params.",
    "",
    "Regenerate with:",
    "",
