...
```json
{
  ...
  "openClawRuntime": {
    ...
    "workspaceBootstrapContextFrom": "extensions/codex app-server turn/start input OpenAgent runtime context"
  }
}
```
...
Deferred searchable OpenAgent dynamic tools available: automations, gateway, nodes, session_status, sessions_history, sessions_list, sessions_search, sessions_send, subagents, tts, web_fetch, web_search.
...
Use Codex native `spawn_agent` for Codex subagents. `spawn_agent` and the other native collaboration tools may be deferred. Use OpenAgent `sessions_spawn` only for OpenAgent or ACP delegation, never as a substitute for `spawn_agent` on internal legwork.
...
### User: OpenAgent Additional Context (openclaw_current_sender)

```text
<external_openclaw_current_sender>{"sender":{"id":"1000001","name":"Pash","username":"pash"}}</external_openclaw_current_sender>
```

### Developer: OpenAgent Additional Context (openclaw_source_delivery)
...
### Developer: OpenAgent Additional Context (openclaw_temporal_context)
