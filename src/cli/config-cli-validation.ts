...
function formatInvalidConfigRepairHint(
  ...
  return isPluginPackagingRuntimeOutputInvalidConfigSnapshot(snapshot)
    ...
    : `Run \`${formatCliCommand("openagent doctor --fix")}\` ${doctorMessage}`;
}
...
export function ensureValidConfigSnapshotForCli(
  ...
  if (options.json) {
    writeRuntimeJson(runtime, {
      ...formatCliJsonFailure(`OpenAgent config is invalid: ${shortenHomePath(snapshot.path)}`),
      issues: normalizeConfigIssues(snapshot.issues),
    });
    exitCliAfterOutput(runtime, 1);
  }
  runtime.error(`OpenAgent config is invalid: ${shortenHomePath(snapshot.path)}`);
