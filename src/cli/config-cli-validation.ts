...
export function ensureValidConfigSnapshotForCli(
  ...
  if (options.json) {
    writeRuntimeJson(runtime, {
      ...formatCliJsonFailure(`Zero to Agent config is invalid: ${shortenHomePath(snapshot.path)}`),
      issues: normalizeConfigIssues(snapshot.issues),
    });
    exitCliAfterOutput(runtime, 1);
  }
  runtime.error(`Zero to Agent config is invalid: ${shortenHomePath(snapshot.path)}`);
