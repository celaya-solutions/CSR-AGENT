...
describe("mcp cli", () => {
  ...
  it("labels listed MCP servers as OpenAgent-managed", async () => {
    await withTempHome("openclaw-cli-mcp-home-", async () => {
      const workspaceDir = await createWorkspace();
      vi.spyOn(process, "cwd").mockReturnValue(workspaceDir);
      ...
      expect(output).toContain("OpenAgent-managed MCP servers (");
      expect(output).toContain("- context7");
      expect(output).toContain("OpenAgent-managed mcp.servers entries");
  ...
  it("tells the operator how to add a server when probing with none configured", async () => {
    await withTempHome("openclaw-cli-mcp-home-", async () => {
      ...
      expect(output).toContain("openagent mcp add <name> --command <command>");
  ...
  it("removes pure disabled tombstones when enabling MCP servers", async () => {
    await withTempHome("openclaw-cli-mcp-home-", async () => {
      ...
      await runMcpCommand(["mcp", "list"]);
      const output = mockLog.mock.calls.map((call) => String(call[0])).join("\n");
      expect(output).toContain("No OpenAgent-managed MCP servers configured in ");
      expect(output).toContain("does not include mcporter servers from config/mcporter.json");
    });
  });
  ...
  it("fails named probes for disabled MCP servers", async () => {
    await withTempHome("openclaw-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `MCP server "docs" is disabled in ${configPath}. Run openagent mcp configure docs --enable before probing it.`,
      );
    });
  });
  ...
  it("fails when removing an unknown MCP server", async () => {
    await withTempHome("openclaw-cli-mcp-home-", async (home) => {
      ...
      expect(lastErrorLine()).toBe(
        `No MCP server named "missing" in ${configPath}. Run openagent mcp list to see configured servers.`,
      );
    });
  });
  ...
  it("points serve startup failures at the deep Gateway health diagnostic", async () => {
    await withTempHome("openclaw-cli-mcp-home-", async () => {
      ...
      expect(lastErrorLine()).toBe(
        "MCP server failed to start: gateway unavailable. Run openagent gateway status --deep --require-rpc to inspect Gateway health.",
      );
    });
  });
});
