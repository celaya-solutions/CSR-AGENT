...
const RESUME_INTERACTIVE_TERMINAL_GUIDANCE =
  "Attaching to a session requires an interactive terminal. Re-run `openagent resume [query]` from an interactive terminal.";
...
async function promptResumeSession(
  ...
  if (choices.length === 0) {
    throw new Error(
      "No recent sessions found. Run `openagent sessions` to inspect sessions or `openagent tui` to start one.",
    );
  }
...
function reportResumeFailure(
  ...
  defaultRuntime.error(
    "Run `openagent resume` to choose from recent sessions or `openagent sessions` to inspect all sessions.",
  );
}
