...
describe("CLI help process exit", () => {
  ...
  it("exits promptly after a lazy group --help", async () => {
    ...
    expect(result.stderr).toBe("");
    expect(result.stdout).toContain("Usage: openagent backup [options] [command]");
  });
  ...
  it.concurrent.each([
    { args: ["acp", "--help"], usage: "Usage: openagent acp [options] [command]" },
    { args: ["acp", "client", "--help"], usage: "Usage: openagent acp client [options]" },
