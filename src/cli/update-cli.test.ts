...
describe("update-cli", () => {
      ...
      [
        {
          ...
          status: "skipped",
          message:
            'Disabled "demo" after plugin update failure; OpenAgent will continue without it. Failed to update demo: registry timeout',
        },
      ],
      true,
    );
  ...
  it("does not clean handoffs before rejecting an unknown package owner", async () => {
    ...
    resolveGlobalManager.mockRejectedValueOnce(
      new Error(
        "Update refused: package manager owner is unknown; no changes were made. Run this OpenAgent install through its active npm, pnpm, or Bun global shim, or reinstall it with that package manager, then retry.",
      ),
    );
  ...
  it.each([
    {
      ...
      assert: () => {
        expect(getLogOutput()).toContain("OpenAgent update status");
  ...
  it("never starts the candidate after plugin post-update invalidates config", async () => {
    ...
    expect(getLogOutput()).toContain("OpenAgent update failed: post-update-plugins.");
    expect(getErrorOutput()).not.toContain("Update failed during plugin post-update sync.");
  });
  ...
  it("keeps the core stopped for plugin Doctor and never restarts after Doctor fails", async () => {
    ...
    expect(defaultRuntime.exit).not.toHaveBeenCalled();
    expect(getLogOutput()).toContain("OpenAgent update failed: post-update-plugins.");
    expect(getLogOutput()).not.toContain("OpenAgent updated");
  });
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    const logs = getLogOutput();
    expect(logs).toContain("OpenAgent update skipped: dirty.");
    expect(logs).toContain(
  ...
  it("leaves a same-version service untouched when its package root is foreign", async () => {
    ...
    expect(getErrorOutput()).toContain("service belongs to a different OpenAgent installation");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
  });
    ...
    async ({ previous, mutatesCore }) => {
      await withEnvAsync({ OPENCLAW_UPDATE_IN_PROGRESS: previous }, async () => {
        ...
        const successIndex = vi
          .mocked(defaultRuntime.log)
          .mock.calls.findIndex((call) => String(call[0]).includes("OpenAgent updated"));
