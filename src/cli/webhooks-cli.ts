...
export function registerWebhooksCli(program: Command) {
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Zero to Agent hooks")
    ...
    .option("--hook-url <url>", "Zero to Agent hook URL")
    .option("--hook-token <token>", "Zero to Agent hook token")
