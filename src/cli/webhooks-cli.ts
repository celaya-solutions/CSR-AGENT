...
export function registerWebhooksCli(program: Command) {
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + OpenAgent hooks")
    ...
    .option("--hook-url <url>", "OpenAgent hook URL")
    .option("--hook-token <token>", "OpenAgent hook token")
...
function parseGmailSetupOptions(raw: Record<string, unknown>): GmailSetupOptions {
  ...
  if (!account) {
    throw new Error(
      `--account is required. Example: ${formatCliCommand("openagent webhooks gmail setup --account default")}.`,
    );
  }
