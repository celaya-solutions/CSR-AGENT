...
export const GATEWAY_SERVICE_PATHS_UNVERIFIED =
  "Installed Gateway service state and config paths could not be verified. Inspect the service environment with `openagent gateway status --deep` before repairing plugin state.";
...
export function compareCliGatewayStateDirs(params: {
  ...
  return {
    ...
    message: [
      ...
      `Fix: run OPENCLAW_STATE_DIR=${quoteCliArg(gatewayStateDir)} OPENCLAW_CONFIG_PATH=${quoteCliArg(gatewayConfigPath)} ${params.command ?? "openagent configure"}.`,
