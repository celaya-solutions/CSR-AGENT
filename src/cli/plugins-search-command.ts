...
/** Options accepted by `openagent plugins search`. */
...
function formatPackageSearchLine(entry: ClawHubPackageSearchResult): string {
  ...
  return `${pkg.name}  ${theme.muted(flags.join(" | "))}${summary}\n  ${theme.muted(`Install: ${formatCliCommand(`openagent plugins install clawhub:${pkg.name}`)}`)}`;
}
...
export async function runPluginsSearchCommand(
  ...
  if (!query) {
    const message = "Usage: openagent plugins search <query>";
