    ...
    "package.json": JSON.stringify(
      {
        ...
        private: true,
        scripts: {
          build: "tsc -p tsconfig.json && openagent plugins build",
          validate: "openagent plugins validate --json",
          pack: "openagent plugins pack --json",
        },
...
This OpenAgent feature plugin includes a typed draft-analysis operation, a model tool, a native page, and a composer replacement. The browser entry owns its DOM and uses the host's canonical draft and send operations.
...
After browser-only changes, run the build again and use **Plugins > Customize UI > Reload plugin UI** as an administrator. After editing installed backend source, run \`openagent plugins reload ${params.id}\`. Rebuild compiled code before reloading; for a copied installation, reinstall the rebuilt package. Plugin install and update commands apply changes through the running Gateway. Native plugins run trusted code in the Gateway and browser; install only code you trust.
