  ...
  const committed = await transformConfigFile({
    transform: (config) =>
      withHooksReport(config, resolveHooksReportTarget(config, agentId), (report) => {
        ...
        if (!hook) {
          throw new Error(
            `Hook "${hookName}" not found. Run \`${formatCliCommand("openagent hooks list")}\` to see available hooks.`,
          );
        }
        ...
        if (enabled && !hook.requirementsSatisfied) {
          ...
          const installHint = hook.install.length
            ...
            : "";
          throw new Error(
            `Hook "${hookName}" is not eligible; missing ${missing}.${installHint} Run \`${formatCliCommand(`openagent hooks info ${hookName}`)}\` for details.`,
          );
        }
...
export function registerHooksCli(program: Command): void {
  ...
  hooks.hook("preAction", (_thisCommand, actionCommand) => {
    ...
    if (
      ...
      throw new Error(
        `openagent hooks ${actionCommand.name()} does not support --agent; the option only selects an owner for read-only hook reports.`,
      );
    }
  });
  ...
  hooks
    .command("install")
    .description("Deprecated: install a hook pack via `openagent plugins install`")
    ...
    .action(async (raw: string, opts: HooksInstallOptions) => {
      defaultRuntime.log(
        theme.warn("`openagent hooks install` is deprecated; use `openagent plugins install`."),
      );
  ...
  hooks
    .command("update")
    .description("Deprecated: update hook packs via `openagent plugins update`")
    ...
    .action(async (id: string | undefined, opts: HooksUpdateOptions) => {
      defaultRuntime.log(
        theme.warn("`openagent hooks update` is deprecated; use `openagent plugins update`."),
      );
