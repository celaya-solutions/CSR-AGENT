// `openagent plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `openagent plugins inspect`. */
...
export async function runPluginsInspectCommand(
  ...
  if (!targetPlugin) {
    ...
    if (id === "skill-workshop") {
      ...
      const lines = [
        "Skill Workshop is built into OpenAgent, not a plugin; configure it under skills.workshop.",
      ];
  ...
  if (output === undefined) {
    failPluginInspect(
      formatMissingPluginMessage({ id, listCommand: "openagent plugins list --json" }),
      opts.json,
    );
  } else if (opts.json) {
    defaultRuntime.writeStdout(output);
