...
const bootLifecycle = vi.hoisted(() => ({
  manualChannelStartHint: `Start a channel manually with: openagent gateway call channels.start --params '{"channel":"<id>"}'`,
  decisions: [] as Array<{
    tripped: boolean;
    uncleanBoots: number;
    windowMs: number;
...
vi.mock("../terminal-interactivity.js", () => ({
  ...
  NON_INTERACTIVE_GATEWAY_RUN_FORCE_MESSAGE:
    "Refusing to kill the operator's running gateway service from a non-interactive shell. Use an isolated dev gateway (openagent gateway run --dev, or --profile <name> with a free port) for testing.",
}));
...
describe("gateway run option collisions", () => {
  ...
  it("refuses non-interactive --force when a verified gateway appears before signaling", async () => {
    ...
    expect(startGatewayServer).not.toHaveBeenCalled();
    expect(runtimeErrors.join("\n")).toContain("openagent gateway run --dev");
  ...
  it("reports forced port cleanup failures before startup", async () => {
    ...
    expect(runtimeErrors.join("\n")).toContain("openagent gateway status --deep");
  });
  ...
  it.each([
    ...
    async ({ platform, managed, warns }) => {
      const warning = "Gateway restart storm: inspect launchd jobs with openagent gateway status.";
  ...
  it.each(["initial", "restart", "cause", "aggregate"] as const)(
    ...
    async (kind) => {
      ...
      try {
        ...
        expect(message).toBe(
          `Legacy session store requires migration: ${storePath}. Run "openagent doctor --fix" against the same state/config before starting OpenAgent.`,
        );
        const failure =
          kind === "cause"
  ...
  it("does not park launchd for a nonrepairable shared-state schema", async () => {
    startGatewayServer.mockRejectedValueOnce(
      new Error(
        "OpenAgent state database /tmp/openclaw.sqlite has a noncanonical agent database registry schema that cannot be repaired automatically.",
      ),
    );
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `openagent onboard --mode local` or `openagent setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
