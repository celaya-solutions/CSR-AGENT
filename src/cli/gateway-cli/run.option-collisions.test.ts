...
describe("gateway run option collisions", () => {
  ...
  it.each(["initial", "restart", "cause", "aggregate"] as const)(
    ...
    async (kind) => {
      ...
      try {
        ...
        expect(message).toBe(
          `Legacy session store requires migration: ${storePath}. Run "openclaw doctor --fix" against the same state/config before starting Zero to Agent.`,
        );
        const failure =
          kind === "cause"
  ...
  it("does not park launchd for a nonrepairable shared-state schema", async () => {
    startGatewayServer.mockRejectedValueOnce(
      new Error(
        "Zero to Agent state database /tmp/openclaw.sqlite has a noncanonical agent database registry schema that cannot be repaired automatically.",
      ),
    );
