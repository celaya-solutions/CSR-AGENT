...
async function runGatewayLoopWithSupervisedLockRecovery(params: {
  ...
  for (;;) {
    try {
      ...
      params.log.warn(
        `${lifecycleContention ? "gateway-lifecycle ownership held by another OpenAgent process" : "gateway already running"} under ${supervisor}; waiting ${waitMs}ms before retrying startup`,
      );
      await sleep(waitMs);
    }
  }
...
async function runGatewayCommandOnce(opts: GatewayRunOpts, hooks: GatewayRunRuntimeHooks = {}) {
  ...
  if (opts.force) {
    ...
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("openagent gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("openagent gateway stop")}`,
      );
      try {
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("openagent gateway status --deep")} for diagnostics.`,
    );
