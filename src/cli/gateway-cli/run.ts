...
async function runGatewayLoopWithSupervisedLockRecovery(params: {
  ...
  for (;;) {
    try {
      ...
      params.log.warn(
        `${lifecycleContention ? "gateway-lifecycle ownership held by another Zero to Agent process" : "gateway already running"} under ${supervisor}; waiting ${waitMs}ms before retrying startup`,
      );
      await sleep(waitMs);
    }
  }
