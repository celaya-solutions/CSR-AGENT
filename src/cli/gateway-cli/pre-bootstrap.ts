...
export function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("openagent setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  return [
    params.mode === undefined
      ? [
          ...
          `Re-run \`${formatCliCommand("openagent onboard --mode local")}\` or \`${formatCliCommand("openagent setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
        ].join(" ")
