  ...
  it("preserves the typed schema refusal when a runner migration fails before Doctor starts", () => {
    ...
    expect(formatCliJsonFailure(error, { env: {} })).toMatchObject({
      ...
      error: {
        ...
        databases,
        commands: expect.arrayContaining(["openagent gateway stop", "openagent doctor --fix"]),
      },
    });
  });
  ...
  it.each([
    ...
    const error = Object.assign(
      new ExpectedCliError({
        message: 'OpenAgent sessions has no command "lst".',
        humanOutput:
...
describe("formatCliFailureLines", () => {
  ...
  it("shows a concise reason and recovery commands by default", () => {
    ...
    expect(lines).toEqual([
      ...
      "[openclaw] Try: openagent doctor",
      "[openclaw] Help: openagent --help",
    ]);
  });
  ...
  it.each([
    ...
    ({ createError }) => {
      ...
      expect(output).not.toContain("openagent doctor");
    },
  );
