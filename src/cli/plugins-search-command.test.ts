...
describe("plugins search command", () => {
  ...
  it.each([
    {
      ...
      profile: undefined,
      container: undefined,
      command: "openagent plugins install clawhub:openclaw-calendar",
    },
    {
      context: "profile",
      profile: "work",
      container: undefined,
      command: "openagent --profile work plugins install clawhub:openclaw-calendar",
    },
    {
      context: "container",
      profile: undefined,
      container: "staging",
      command: "openagent --container staging plugins install clawhub:openclaw-calendar",
    },
    {
  ...
  it("leaves missing-query JSON failures to the root renderer", async () => {
    await expect(runPluginsSearchCommand([], { json: true }, mocks.runtime)).rejects.toThrow(
      "Usage: openagent plugins search <query>",
    );
