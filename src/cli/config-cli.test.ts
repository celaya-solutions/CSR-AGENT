...
/**
 * Test for issue #6070:
 * `openagent config set/unset` must update snapshot.resolved (user config after $include/${ENV},
  ...
  describe("config set - issue #6070", () => {
    ...
    it("rejects an unresolved primary model before writing config", async () => {
      ...
      mockCheckTouchedTextModelRefs.mockResolvedValueOnce({
        ...
        refsTotal: 1,
        errors: [
          'Cannot set model reference "missing/nope" at agents.defaults.model.primary: Unknown model: missing/nope. Run openagent models list to list available models.',
        ],
      });
      ...
      expectErrorIncludes("openagent models list");
    });
    ...
    it("rejects plugin install record config updates", async () => {
      ...
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
      expectErrorIncludes("openagent plugins install <spec>");
      expectErrorIncludes("openagent plugins update <plugin-id>");
    });
  ...
  describe("config get", () => {
    ...
    it.each([
      {
        ...
        path: "gateway.bind",
        message:
          "Config path is valid but unset: gateway.bind. The runtime default applies until you set an authored value with openagent config set gateway.bind <value>.",
      },
      {
        name: "valid but unset array path",
        path: "models.providers.example.models[0].id",
        message:
          "Config path is valid but unset: models.providers.example.models[0].id. The runtime default applies until you set an authored value with openagent config set 'models.providers.example.models[0].id' <value>.",
      },
      {
        name: "unknown path",
        path: "nonexistent.path",
        message:
          "Unknown config path: nonexistent.path. Run openagent config schema to inspect valid paths.",
      },
    ...
    it("returns invalid configuration as JSON without observing persistent state", async () => {
      ...
      expect(parseLastLogPayload()).toMatchObject({
        ...
        error: {
          type: "cli_error",
          message: expect.stringContaining("OpenAgent config is invalid"),
        },
        issues: [{ path: "gateway.bind", message: "Invalid enum value" }],
      });
    });
  ...
  describe("config unset - issue #6070", () => {
    ...
    it("rejects an unset that makes a dependent model reference unresolved", async () => {
      ...
      mockCheckTouchedTextModelRefs.mockResolvedValueOnce({
        ...
        refsTotal: 1,
        errors: [
          'Cannot set model reference "backup" at agents.defaults.model.fallbacks.0: Unknown model: openai/backup. Run openagent models list to list available models.',
        ],
      });
    ...
    it("fails when unsetting a runtime-only default shown by config get", async () => {
      ...
      expectLogExcludes("No change");
      expectErrorIncludes(
        `Config path not found in authored config: ${aliasPath}. It only exists after runtime defaults are applied, so there is nothing for config unset to remove. Use openagent config set <path> <value> to override the inherited value.`,
      );
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
