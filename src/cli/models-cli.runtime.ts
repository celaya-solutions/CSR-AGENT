...
export function rejectAgentScopedModelCommand(
  ...
  throw new Error(
    `openagent models ${commandName} does not support --agent; it is global and never agent-scoped. Remove --agent, or run ${formatCliCommand("openagent agents list")} and set the per-agent model in agent config.`,
  );
}
