...
it.each([
  { kind: "package-post-install-doctor", name: "openagent doctor", exitCode: 0 },
  { kind: "package-post-install-doctor", name: "openagent doctor", exitCode: 86 },
  ...
  const message =
    kind === "recoverable-maintenance"
      ...
      : "Skipped derived cache cleanup: permission denied. Run openagent doctor --fix.";
  const otherWarning =
    kind === "recoverable-maintenance"
      ...
      : "Skipped legacy cache cleanup: read-only directory. Run openagent doctor --fix.";
  ...
  if (scenario === "owned" || scenario === "foreign" || scenario === "absent") {
    ...
    if (scenario === "owned-pending") {
      ...
      if (scenario !== "unresolved-root") {
        expect(failure.message).toContain("openagent gateway status --deep");
