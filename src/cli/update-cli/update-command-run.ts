      ...
      : await service
          ...
          .catch((cause: unknown) => {
            throw new GatewayServiceUpdateOwnershipError(
              "Gateway service inspection is unavailable before update admission. Run `openagent gateway status --deep` from the service's owning account and retry when service access is restored.",
              cause,
            );
          });
    if (command) {
