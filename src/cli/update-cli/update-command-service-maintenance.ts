...
const GATEWAY_SERVICE_INSPECTION_BLOCK_MESSAGE =
  "Gateway service inspection is unavailable. Refusing to mutate code because managed service ownership cannot be verified. Run `openagent gateway status --deep` and retry when service access is restored.";
  ...
  if (serviceUpdateVerdict.kind === "foreign") {
    return {
      ...
      serviceMutationSkipMessage:
        "Gateway service management skipped: the service belongs to a different OpenAgent installation and was left untouched.",
    };
  }
