...
function readinessWarning(
  ...
  return {
    ...
    guidance: [
      finding.fixHint ??
        `Resolve this finding, then rerun \`openagent doctor --lint --only ${finding.checkId}\`.`,
    ],
  ...
  return {
    ...
    warnings: [
      ...(pluginUpdate.warnings ?? []),
      {
        ...
        guidance: ["Run `openagent update repair` to retry post-update readiness checks."],
      },
    ],
  };
}
...
export async function applyPostPluginUpdateReadiness(params: {
  ...
  if (!entryPath) {
    return createPostPluginReadinessExecutionFailure(
      params.pluginUpdate,
      "Updated OpenAgent entrypoint not found for post-plugin readiness checks",
    );
  }
