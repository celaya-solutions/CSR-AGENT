    ...
    try {
      ...
      if (retainedDifferentRuntime && !pluginsUpdatedInFreshProcess) {
        return {
          ...
          detail:
            "The installed target could not resume plugin convergence. Run openagent update using the installed target executable.",
        };
      }
      ...
      if (failureFacts.length) {
        resultWithPostUpdate.steps.push({
          ...
          command: "openagent plugins update",
          cwd: postUpdateRoot,
          durationMs: 0,
          exitCode: 1,
          failureFacts,
