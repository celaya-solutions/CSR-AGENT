...
const UPDATE_CHAT_HANDOFF_GUIDANCE =
  "From chat, the OpenAgent owner can start the update with the gateway update action or /update, which hands it to a managed helper.";
...
export async function handoffUpdateFromGateway(params: {
  ...
  if (!argv1) {
    throw new UpdatePreMutationError(
      ...
      "Cannot locate the installed updater; run `openagent doctor` before retrying.",
    );
  }
  ...
  if (started.status === "joined") {
    throw new UpdatePreMutationError(
      ...
      "Another managed update is already running. Inspect `openagent status --all` before retrying.",
    );
  }
  const identity = {
