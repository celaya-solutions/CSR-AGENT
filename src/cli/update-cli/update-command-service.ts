...
export async function tryInstallShellCompletion(opts: {
  ...
  try {
    ...
    if (!opts.jsonMode) {
      const completionCacheRefreshCommand = formatCliCommand("openagent completion --write-state");
      defaultRuntime.log(
        theme.warn(
  ...
  try {
    ...
    if (!status.profileInstalled && !opts.skipPrompt) {
      ...
      if (isCancel(shouldInstall) || !shouldInstall) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${formatCliCommand("openagent completion --install")}\` later to enable.`,
          ),
        );
        return;
      }
    ...
    defaultRuntime.log(
      theme.warn(
        `Shell completion refresh failed: ${message}. Update will continue. Resolve the reported error before retrying: ${formatCliCommand("openagent completion --write-state --install")}`,
      ),
    );
  }
}
...
export async function maybeRestartService(params: {
    ...
    try {
      ...
      defaultRuntime.error(
        `Gateway: restart failed: ${String(err)}. Code update remains installed; a service stopped for update may still be stopped. ` +
          "Run `openagent gateway status --deep` and ask its service owner to restart it manually.",
      );
      return await failed();
    }
    return "ok";
  ...
  if (!activation.opts.json) {
    ...
    if (activation.result.mode === "npm" || activation.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("openagent doctor")}\`, then \`${formatCliCommand("openagent gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${formatCliCommand("openagent gateway restart")}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
