...
export function resolveUpdateResultNextAction(params: {
  ...
  if (result.status === "error") {
    ...
    const deployment = containerPermissionFailure
      ? "Detected package update permission failure (EACCES) inside a container. Pull or build a Zero to Agent image with the target version, then recreate or redeploy the container with the same state/config mounts. In-container package changes are not durable. "
      : "";
  ...
  if (result.reason === "not-git-install") {
    return `This Zero to Agent install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${command("openclaw doctor")}\` and \`${command("openclaw gateway restart")}\`. Examples: \`npm i -g openclaw@latest\` or \`pnpm add -g openclaw@latest\`.`;
  }
  if (result.status === "ok") {
