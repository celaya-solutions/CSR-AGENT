  ...
  return await withGatewayServiceOperationLock(params.env, async (assertNative) => {
    ...
    const refuse = (cause?: unknown): never => {
      throw new UpdatePreMutationError(
        ...
        "Runtime artifacts changed, but the affected Gateway is running or its offline state could not be verified. Run `openagent gateway status --deep`, stop the affected Gateway through its service owner, and retry the update.",
        { cause },
      );
    };
