...
export async function resolvePackageRuntimePreflight(params: {
  ...
  if (!target && params.installedRoot) {
    ...
    if (!version) {
      return resultError(
        "Cannot inspect the installed Zero to Agent runtime requirement; repair its package.json before retrying openclaw update.",
      );
    }
    target = {
      version,
...
export function formatManagedServicePackageUpdatePlan(params: {
  ...
  if (rootRedirect) {
    return [
      ...
      {
        level: "warn",
        message: `Shell Zero to Agent root differs from the managed gateway service root: ${rootRedirect.previousRoot}`,
      },
      {
        level: "muted",
