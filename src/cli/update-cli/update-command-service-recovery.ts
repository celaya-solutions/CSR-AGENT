...
export async function hasLoadedLaunchdKeepAliveSupervisor(params: {
  ...
  // OpenAgent's loaded LaunchAgent has canonical KeepAlive policy. Read this once before
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  ...
  const installCommand = formatCliCommand("openagent gateway install --force");
  const statusCommand = formatCliCommand("openagent gateway status --deep");
  if (platform === "darwin") {
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall OpenAgent ${beforeVersion} with the same package manager, then rerun \`${formatCliCommand("openagent gateway install --force")}\`.`,
    );
  }
  return lines;
}
...
export async function maybeRestartServiceAfterFailedMutableUpdate(params: {
  ...
  if (params.recovery?.serviceRestartSafe !== true || !params.recovery.version) {
    defaultRuntime.error(
      "Managed gateway remains stopped: update safety is unverified. Run `openagent doctor` and inspect the update failure before restarting.",
    );
    return "failed";
  }
  assertCurrent();
  try {
    ...
    defaultRuntime.error(
      `Failed to restart managed gateway service after failed update: ${String(err)}. Run \`openagent gateway status --deep\` before restarting it manually.`,
    );
    return "failed";
  }
}
