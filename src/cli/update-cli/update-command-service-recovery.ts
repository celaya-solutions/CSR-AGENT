...
export async function hasLoadedLaunchdKeepAliveSupervisor(params: {
  ...
  // Zero to Agent's loaded LaunchAgent has canonical KeepAlive policy. Read this once before
...
export function formatPostUpdateGatewayRecoveryInstructions(
  ...
  if (isPackageManagerUpdateMode(result.mode) && beforeVersion) {
    lines.push(
      `Rollback: reinstall Zero to Agent ${beforeVersion} with the same package manager, then rerun \`${formatCliCommand("openclaw gateway install --force")}\`.`,
    );
  }
  return lines;
}
