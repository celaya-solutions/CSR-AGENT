...
`,
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'OpenAgent Gateway'");
    ...
    it("uses OPENCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'OpenAgent Gateway (custom)'");
    ...
    it.each([
      ...
      ["win32", "production", "$taskName = 'OpenAgent Gateway (production)'"],
