...
`,
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Zero to Agent Gateway'");
    ...
    it("uses OPENCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Zero to Agent Gateway (custom)'");
    ...
    it.each([
      ...
      ["win32", "production", "$taskName = 'Zero to Agent Gateway (production)'"],
