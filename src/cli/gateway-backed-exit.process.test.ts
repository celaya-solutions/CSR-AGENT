...
describe("gateway-backed CLI process exit", () => {
  ...
  it.runIf(process.platform !== "win32")(
    "runs gateway status through one OpenAgent entry process",
    async () => {
  ...
  it.each([
    ...
    async ({ label, args, machineOutput }) => {
      ...
      expect(result.stderr).toContain(
        "Start it with `openagent gateway run` or check `openagent gateway status`.",
      );
  ...
  it("rejects invalid remote config before a node pairing mutation without opening state", async () => {
    ...
    expect(result.stderr).toContain("OpenAgent config is invalid");
...
`,
  ...
  it.each([
    ...
    async ({ label, args, machineOutput }) => {
      ...
      const message =
        "Automation not found: missing-job. Run `openagent cron list` to see recent automation ids.";
      expect(result).toMatchObject({ code: 1, signal: null });
      if (machineOutput) {
        expect(JSON.parse(result.stdout)).toEqual({
          ok: false,
