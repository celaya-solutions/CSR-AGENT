...
describe("state-dir-gateway-check", () => {
  ...
  it("refuses an installed service mismatch from its recorded environment", async () => {
    ...
    await expect(
      checkCliGatewayStateDir({ command: "openagent channels add", config: {} }),
  ...
  it("allows a matching installed service without probing", async () => {
    ...
    await expect(
      checkCliGatewayStateDir({ command: "openagent models auth", config: {} }),
  ...
  it.each([false, true])(
    ...
    async (serviceSetsHome) => {
      ...
      await expect(
        checkCliGatewayStateDir({ command: "openagent configure", config: {} }),
  ...
  it("warns when service inspection is unavailable without exposing its error", async () => {
    ...
    const result = await checkCliGatewayStateDir({ command: "openagent configure", config: {} });
