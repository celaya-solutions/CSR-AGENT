...
describe("skills cli commands", () => {
  ...
  it.each(["install", "verify"])(
    ...
    (commandName) => {
      ...
      expect(help).toContain(`openagent skills ${commandName} @owner/weather`);
      expect(help).not.toContain(`openagent skills ${commandName} weather`);
    },
  );
  ...
  it.each([
    {
      ...
      expected:
        'Skill "missing-skill" not found. Run `openagent skills list` to see available skills.\n\nTip: use `openagent skills search`, `openagent skills install`, and `openagent skills update` for ClawHub-backed skills.',
    },
    {
      label: "JSON",
  ...
  it("keeps non-JSON skills list output on stdout with human-readable formatting", async () => {
    ...
    expect(runtimeStdout.at(-1)).toContain("openagent skills search");
  });
});
