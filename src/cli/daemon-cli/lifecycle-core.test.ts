...
const SERVICE_REPAIR_COMMAND_CASES = [
  ["Gateway", "", "", "openagent gateway", "restart"],
  ["Node", "", "", "openagent node", "install --force"],
  ["Node", "work", "", "openagent --profile work node", "install --force"],
  ["Node", "work", "demo", "openagent --container demo node", "install --force"],
] as const;
...
describe("runServiceRestart token drift", () => {
    ...
    await withUnsupportedGatewayService(async (unsupportedService) => {
      await expect(
        runServiceStart({
          ...
          renderStartHints: () => ["openagent gateway install"],
          opts: { json: true },
          onNotLoaded,
        }),
  ...
  it("restarts a disabled installed service through its native manager", async () => {
    ...
    const renderStartHints = vi.fn(() => ["openagent gateway install"]);
  ...
  it("fails start with install hints when no service is installed", async () => {
    ...
    expect(payload.hints?.includes("openagent gateway install")).toBe(true);
    expect(
      payload.hintItems?.some(
        (item) => item.kind === "install" && item.text === "openagent gateway install",
      ),
    ).toBe(true);
    expect(service.start).not.toHaveBeenCalled();
  });
