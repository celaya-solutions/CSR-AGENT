...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (gatewayVersion) {
    ...
    if (status.cli?.version && status.cli.version !== gatewayVersion) {
      defaultRuntime.error(
        warnText(
          `Warning: this Zero to Agent command is version ${status.cli.version}, but the running Gateway is version ${gatewayVersion}.`,
        ),
      );
      defaultRuntime.error(
        warnText(
          "Check `openclaw --version`, `which openclaw`, and `openclaw gateway status --deep`; if this mismatch is unexpected, update PATH so `openclaw` points to the version you want, or reinstall the Gateway service from that same Zero to Agent install.",
        ),
      );
    }
    spacer();
  ...
  if (status.connections?.established.length) {
    ...
    defaultRuntime.log(
      warnText(
        "If logs show protocol mismatch after rollback, stop stale Zero to Agent client processes listed here and re-run gateway status.",
      ),
    );
    spacer();
  }
  ...
  if (service.foreignLaunchdJobs?.length) {
    ...
    if (shouldWarn) {
      ...
      defaultRuntime.log(infoText("Other Zero to Agent launchd jobs (macOS)"));
  ...
  if (staleUpdateLaunchdJobs?.length) {
    defaultRuntime.error(errorText("Stale Zero to Agent updater launchd job(s) detected."));
