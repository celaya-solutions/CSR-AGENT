    ...
    async (action) => {
      ...
      await expect(repair).rejects.toThrow(
        [
          ...
          `Run \`openagent gateway ${action}\` with the installed state directory, config path, and port (or unset conflicting environment overrides). To retarget intentionally, run \`openagent gateway install --force\`.`,
        ].join("\n"),
      );
