          ...
          if (hasOfficialPluginVersionCandidates({ installRecords, config: daemonCfg })) {
            if (!targetServiceCommand) {
              pluginVersionRestartReadiness = {
                status: "unresolved",
                reason:
                  "Gateway service command is unavailable, so the post-restart Zero to Agent version is unknown.",
              ...
              if (!layout?.packageVersion) {
                pluginVersionRestartReadiness = {
                  status: "unresolved",
                  reason:
                    "Gateway service package version is unavailable, so the post-restart Zero to Agent version is unknown.",
                  ...(runningGatewayVersion ? { runningGatewayVersion } : {}),
                };
              } else {
