...
export type DaemonStatus = {
   ...
   * without a corresponding `openagent plugins update`.
   */
          ...
          if (hasOfficialPluginVersionCandidates({ installRecords, config: daemonCfg })) {
            if (!targetServiceCommand) {
              pluginVersionRestartReadiness = {
                status: "unresolved",
                reason:
                  "Gateway service command is unavailable, so the post-restart OpenAgent version is unknown.",
              ...
              if (!layout?.packageVersion) {
                pluginVersionRestartReadiness = {
                  status: "unresolved",
                  reason:
                    "Gateway service package version is unavailable, so the post-restart OpenAgent version is unknown.",
                  ...(runningGatewayVersion ? { runningGatewayVersion } : {}),
                };
              } else {
  ...
  return {
    ...
    service: {
      ...
      runtime: runtime?.inspectionFailure
        ? {
            ...runtime,
            detail: `${runtime.detail}; retry with openagent gateway status --deep`,
          }
        : runtime,
      configAudit,
      ...(command
