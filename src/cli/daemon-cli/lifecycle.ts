...
async function stopGatewayWithoutServiceManager(
  ...
  if (pids.length === 0) {
    ...
    if (portUsage !== "free") {
      throw new Error(
        portUsage === "busy"
          ? `Port ${port} is in use but the owning process could not be identified. Run ${formatCliCommand("openagent gateway status --deep")} to diagnose.`
          : `Could not determine whether port ${port} is still in use, so the gateway cannot be confirmed stopped. Run ${formatCliCommand("openagent gateway status --deep")} to diagnose.`,
      );
    }
    return null;
  }
...
async function signalGatewayRestart(
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "openagent gateway status --deep" before retrying restart`,
    );
  }
...
export async function runDaemonRestart(opts: DaemonLifecycleOptions = {}): Promise<boolean> {
  ...
  return await runServiceRestart({
    ...
    postRestartCheck: async ({ warnings, fail, stdout, warn, activationAccepted: accepted }) => {
      ...
      fail(
        failure.failMessage,
        [formatCliCommand("openagent gateway status --deep"), formatCliCommand("openagent doctor")],
        activationAccepted ? "restart-health-failed" : undefined,
      );
