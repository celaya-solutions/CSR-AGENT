...
function renderNodeServiceStartHints(): string[] {
  return buildPlatformServiceStartHints({
    installHint: formatCliCommand("openagent node install"),
    startCommand: formatCliCommand("openagent node start"),
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("openagent node install --force")}`);
    }
    return;
  }
