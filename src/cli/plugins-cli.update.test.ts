...
describe("plugins cli update", () => {
  ...
  it.each([
    ["missing", "missing-plugin", [], undefined, undefined, "openagent"],
    ["preview", "missing-plugin", ["--dry-run"], undefined, undefined, "openagent"],
    ["object-name", "constructor", [], undefined, undefined, "openagent"],
    ["npm-spec", "@acme/missing-plugin@beta", [], undefined, undefined, "openagent"],
  ...
  it("exits non-zero when a ClawHub update is skipped because security data is unavailable", async () => {
    await expectSkippedClawHubPluginUpdate({
      ...
      message:
        'Skipped demo ClawHub update: ClawHub security data for "@openclaw/plugin-demo@1.1.0" is unavailable, so OpenAgent left the existing installed plugin unchanged. Try again later or choose a different version.',
