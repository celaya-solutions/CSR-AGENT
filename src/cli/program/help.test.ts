...
describe("configureProgramHelp", () => {
  ...
  it("keeps valid root, group, subcommand, short, and help-command output successful", async () => {
    ...
    expect(groupHelp.stdout).toContain("Usage: openagent plugins [options] [command]");
    expect(subcommandHelp.stdout).toContain("Usage: openagent plugins list [options]");
  ...
  it("formats parse errors from the exact Commander command path", async () => {
    ...
    expect(stderr.match(/Try: openagent plugins list --help/g)).toHaveLength(2);
    expect(stderr).not.toContain("openagent plugins list list --help");
  ...
  it("prints version and exits immediately when version flags are present", () => {
    ...
    expectVersionExit({ expectedVersion: "OpenAgent 9.9.9-test (abc1234)" });
  });

  it("prints version and exits immediately without commit metadata", () => {
    process.argv = ["node", "openclaw", "--version"];
    resolveCommitHashMock.mockReturnValue(null);
    expectVersionExit({ expectedVersion: "OpenAgent 9.9.9-test" });
  });
