...
const EXAMPLES = [
  ["openagent onboard", "Run guided setup for a local Gateway, workspace, auth, and channels."],
  ["openagent setup", "Create the baseline config, workspace, and session folders."],
  ["openagent configure", "Change models, Gateway, channels, plugins, skills, and health checks."],
  ["openagent status", "Check Gateway, channel, model, and recent-session status."],
  ["openagent doctor --fix", "Repair common config, service, plugin, and channel problems."],
  ["openagent channels add", "Add or update a chat channel account with guided prompts."],
  ["openagent channels status", "See connected messaging accounts and login state."],
  ["openagent --dev gateway", "Run a dev Gateway (isolated state/config) on ws://127.0.0.1:19001."],
  ["openagent gateway run --force", "Start the Gateway and replace anything bound to its port."],
  ["openagent models status", "Show model/provider auth health before running agents."],
  ["openagent plugins list", "Inspect enabled, disabled, and installed plugins."],
  [
    'openagent agent --to +15555550123 --message "Run summary" --deliver',
    "Run one agent turn through the Gateway and optionally deliver the reply.",
  ],
  [
    'openagent message send --channel telegram --target @mychat --message "Hi"',
