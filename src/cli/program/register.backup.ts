...
function registerBackupGitCommands(backup: Command): void {
  ...
  git
    .command("create")
    .description("Dump selected OpenAgent databases and commit one Git revision")
    .requiredOption("--repository <path>", "Git backup repository directory")
    .option("--all", "Back up the shared database and every registered agent database", false)
    .option("--global", "Back up the shared OpenAgent state database", false)
...
function registerBackupSqliteCommands(backup: Command): void {
  ...
  sqlite
    .command("create")
    .description("Create a compact, verified snapshot of an OpenAgent SQLite database")
    .option("--global", "Snapshot the shared OpenAgent state database", false)
    .option("--agent <id>", "Snapshot one per-agent OpenAgent database")
