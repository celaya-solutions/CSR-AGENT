...
describe("formatCliParseErrorOutput", () => {
  it.each<{
    ...
    },
    {
      ...
      message: 'Zero to Agent does not recognize option "--unknown".',
  ...
  it("uses the same structured root diagnostic as the human renderer", () => {
    ...
    expect(error.message).toBe('Zero to Agent does not know the command "pairng".');
    expect(error.humanOutput).toBe(
      'Zero to Agent does not know the command "pairng".\nDid you mean this?\n  openclaw pairing\nTry: openclaw --help\nPlugin command? openclaw plugins list\nDocs: https://docs.openclaw.ai/cli\n',
    );
  });
  ...
  it("strips Commander framing from structured nested diagnostics", () => {
    ...
    expect(error.message).toBe('Zero to Agent sessions has no command "lst".');
  ...
  it("explains unknown commands with root help and plugin hints", () => {
    ...
    expect(output).toBe(
      'Zero to Agent does not know the command "wat".\nTry: openclaw --help\nPlugin command? openclaw plugins list\nDocs: https://docs.openclaw.ai/cli\n',
    );
  });
  ...
  it("explains unknown subcommands within the active command tree", () => {
    ...
    expect(output).toBe(
      'Zero to Agent webhooks has no command "list".\nTry: openclaw webhooks --help\nDocs: https://docs.openclaw.ai/cli\n',
    );
  });
  ...
  it("suggests sibling subcommands within the active command tree", () => {
    ...
    expect(output).toBe(
      'Zero to Agent webhooks has no command "gmial".\nDid you mean this?\n  openclaw webhooks gmail\nTry: openclaw webhooks --help\nDocs: https://docs.openclaw.ai/cli\n',
    );
  });
  ...
  it("suggests a live child command when later arguments follow the typo", async () => {
    ...
    expect(error.code).toBe("commander.unknownCommand");
    expect(output).toBe(
      'Zero to Agent config has no command "gett".\nDid you mean this?\n  openclaw config get\nTry: openclaw config --help\nDocs: https://docs.openclaw.ai/cli\n',
    );
  });
  ...
  it("suggests close known commands for unknown commands", () => {
    ...
    expect(output).toBe(
      'Zero to Agent does not know the command "upate".\nDid you mean this?\n  openclaw update\nTry: openclaw --help\nPlugin command? openclaw plugins list\nDocs: https://docs.openclaw.ai/cli\n',
    );
  });
  ...
  it("points unknown options at the active command help", () => {
    ...
    expect(output).toBe(
      'Zero to Agent does not recognize option "--wat".\nTry: openclaw channels status --help\n',
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      message:
        "Zero to Agent could not parse this command: option '--timeout <ms>' argument missing",
    },
