...
export function registerMigrateCommand(program: Command) {
  const migrate = addVerifyPluginAppsOption(
    program
      ...
      .option("--no-backup", "Skip the pre-migration OpenAgent backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openagent migrate list", "Show available migration providers."],
          ["openagent migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["openagent migrate hermes --dry-run", "Preview Hermes migration only."],
          [
            "openagent migrate apply hermes --yes",
            "Apply Hermes migration non-interactively after writing a verified backup.",
          ],
          [
            "openagent migrate hermes --no-auth-credentials",
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing OpenAgent state"),
