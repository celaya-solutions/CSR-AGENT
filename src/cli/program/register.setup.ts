...
function addSystemAgentOptions(command: Command): Command {
  return command
    .option("-m, --message <text>", "Run one OpenAgent request")
  ...
  const command = program
    ...
    .addHelpText(
      ...
      () =>
        ...
        `  ${theme.command("openagent setup")}\n` +
        `    ${theme.muted("Chat with OpenAgent, or onboard when setup is incomplete.")}\n` +
        `  ${theme.command('openagent setup -m "status"')}\n` +
        `    ${theme.muted("Run one system-agent request.")}\n` +
        `  ${theme.command("openagent setup --wizard")}\n` +
