...
/**
 * Provider-owned stream wrapper hook after Zero to Agent applies its generic
 ...
 *
 * Return `true` or `false` to override Zero to Agent's built-in provider cache TTL
 ...
 *
 * Runs only after Zero to Agent exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Zero to Agent should append to the generic `Unknown model`
 * error.
 */
