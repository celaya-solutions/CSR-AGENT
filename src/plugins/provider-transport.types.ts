...
/**
 * Provider-owned stream wrapper hook after OpenAgent applies its generic
 ...
 *
 * Return `true` or `false` to override OpenAgent's built-in provider cache TTL
 ...
 *
 * Runs only after OpenAgent exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that OpenAgent should append to the generic `Unknown model`
 * error.
 */
