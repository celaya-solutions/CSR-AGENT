      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'openagent plugins inspect <id> --runtime --json' for runtime diagnostics and 'openagent plugins list' for registry state. After fixing plugin code or load paths, run 'openagent plugins reload <id>' to retry.`,
        );
      }
    }
    if (context.shouldActivate) {
