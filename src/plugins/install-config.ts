...
async function recoverPluginInstallConfig(
  ...
  if (
    ...
    throw new PluginInstallConfigError(
      `Config invalid outside the plugin recovery path for ${pluginLabel}; run \`openagent doctor --fix\` before reinstalling it.`,
    );
  }
  if (
    ...
    throw new PluginInstallConfigError(
      "Config plugin recovery uses an unsupported $include shape; use a single-file top-level plugins include or run `openagent doctor --fix` before reinstalling it.",
    );
  }
