...
async function promptEnvSecretRefForSetup(params: {
  ...
  await params.prompter.note(
    params.copy?.envValidatedMessage?.(envVar) ??
      `Validated environment variable ${envVar}. Zero to Agent will store a reference, not the key value.`,
    "Reference validated",
  );
  ...
  try {
    ...
    await params.prompter.note(
      params.copy?.providerValidatedMessage?.(selectedProvider, id, providerEntry.source) ??
        `Validated ${providerEntry.source} reference ${selectedProvider}:${id}. Zero to Agent will store a reference, not the key value.`,
      "Reference validated",
    );
    return { ref, resolvedValue };
  } catch (error) {
  ...
  while (true) {
    const sourceRaw: SecretRefChoice = await params.prompter.select<SecretRefChoice>({
      ...
      options: [
        ...
        {
          value: "store",
          label: "Zero to Agent secret store",
    ...
    if (source === "store") {
      ...
      await params.prompter.note(
        `Validated store reference ${ref.provider}:${id}. Zero to Agent will store a reference, not the value.`,
        "Reference validated",
      );
      return { ref, resolvedValue };
    }
