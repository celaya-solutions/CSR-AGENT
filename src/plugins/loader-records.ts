...
function resolvePluginImportHint(
  ...
  try {
    ...
    if (seam) {
      ...
      return {
        hint: `Plugin ${record.id} cannot import ${sdkCompatibility.seam} (built with Zero to Agent ${record.builtWithOpenClawVersion ?? "unknown"}; running core ${VERSION}); ${repair}`,
        sdkCompatibility,
      };
    }
...
export function recordBundleDiagnostics(params: {
  ...
  for (const capability of unsupportedCapabilities) {
    params.registry.diagnostics.push({
      ...
      pluginId: params.record.id,
      source: params.record.source,
      message: `bundle capability detected but not wired into Zero to Agent yet: ${capability}`,
    });
  }
  if (
