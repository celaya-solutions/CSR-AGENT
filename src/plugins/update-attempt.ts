...
export function formatNewerExactPinnedNpmDefaultLineMessage(params: {
  ...
  return (
    ...
    `Pass \`openagent plugins update ${params.newer.packageName}@${params.newer.registryLine}\` to replace this version pin.`
  );
}
...
function formatNewerExactPinnedClawHubDefaultLineMessage(params: {
  ...
  return (
    ...
    `Pass \`openagent plugins install clawhub:${params.newer.packageName}${selector} --force\` to replace this version pin.`
  );
}
