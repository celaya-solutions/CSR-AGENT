...
function throwManagedPluginCapabilityConsentRequired(
  review: PluginCapabilityConsentReview,
  recovery = "Rerun the openagent plugins install, enable, update, or reload command with --accept-capabilities after reviewing the plugin.",
): never {
...
async function resolvePluginArtifactCapabilityConsent(params: {
  ...
  if (
    ...
    return throwManagedPluginCapabilityConsentRequired(
      finalReview,
      `The plugin was not ${outcome}. Re-run the same "openagent plugins install" or "openagent plugins update" command with --accept-capabilities, keeping its source and other options. For Doctor or setup, complete the plugin command first, then retry Doctor or setup.`,
    );
  }
