...
export type CliBackendConfig = {
  ...
  modelArg?: string;
  /** Model aliases mapping (OpenAgent model id → CLI model id). */
...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before OpenAgent consumes it. */
...
export type CliBackendPrepareExecutionContext = {
  ...
  /** Effective OpenAgent context budget selected for this run. */
  contextTokenBudget?: number;
  /** Effective OpenAgent thinking level selected for this run. */
...
/** Exact backend-native plus canonical OpenAgent tool surface for one CLI run. */
export type CliBackendToolAvailability = {
  native: readonly string[];
  /** Canonical OpenAgent tool names served through the host-isolated transport. */
...
type CliBackendPluginBase = {
  ...
  /**
   * Whether OpenAgent should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume OpenAgent's bundle
   ...
   *
   * Use this when the backend should consume a canonical OpenAgent auth profile
   ...
   * `profile-only` treats the selected OpenAgent auth profile as the sole auth
   ...
   * hooks that do not consume OpenAgent auth profiles.
   */
   ...
   * as static config, such as mapping OpenAgent thinking levels to a backend's
   * native effort flag.
   */
   ...
   *
   * Tool events report execution already performed by the backend. OpenAgent
  ...
  /**
   * Whether this CLI backend can expose native tools outside OpenAgent's tool
