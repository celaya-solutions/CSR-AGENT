...
export type CliBackendConfig = {
  ...
  modelArg?: string;
  /** Model aliases mapping (Zero to Agent model id → CLI model id). */
...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Zero to Agent consumes it. */
...
export type CliBackendPrepareExecutionContext = {
  ...
  /** Effective Zero to Agent context budget selected for this run. */
  contextTokenBudget?: number;
  /** Effective Zero to Agent thinking level selected for this run. */
...
/** Exact backend-native plus canonical Zero to Agent tool surface for one CLI run. */
export type CliBackendToolAvailability = {
  native: readonly string[];
  /** Canonical Zero to Agent tool names served through the host-isolated transport. */
...
type CliBackendPluginBase = {
  ...
  /**
   * Whether Zero to Agent should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Zero to Agent's bundle
   ...
   *
   * Use this when the backend should consume a canonical Zero to Agent auth profile
   ...
   * `profile-only` treats the selected Zero to Agent auth profile as the sole auth
   ...
   * hooks that do not consume Zero to Agent auth profiles.
   */
   ...
   * as static config, such as mapping Zero to Agent thinking levels to a backend's
   * native effort flag.
   */
   ...
   *
   * Tool events report execution already performed by the backend. Zero to Agent
  ...
  /**
   * Whether this CLI backend can expose native tools outside Zero to Agent's tool
