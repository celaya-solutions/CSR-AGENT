  ...
  for (const candidate of candidates) {
    ...
    if (isManifestlessConfiguredFile && isCoreReservedPluginId(candidate.idHint)) {
      diagnostics.push({
        ...
        message: `plugin manifest id "${candidate.idHint}" is reserved by OpenAgent core`,
      });
      continue;
    }
    const manifestRes:
    ...
    if (candidate.origin !== "bundled") {
          ...
          message:
            minHostVersionCheck.kind === "invalid"
              ...
              : minHostVersionCheck.kind === "unknown_host_version"
                ? `plugin requires OpenAgent >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined; skipping load`
                : `plugin requires OpenAgent >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}; skipping load`,
        });
        continue;
      }
      ...
      if (
        ...
        diagnostics.push({
          ...
          pluginId: effectivePluginId,
          source: packageManifestSource,
          message: `plugin requires plugin API ${packagePluginApiRange}, but this host is ${currentHostVersion}; skipping load (check "openagent --version", OPENCLAW_COMPATIBILITY_HOST_VERSION, or run "openagent doctor")`,
        });
        continue;
      }
    }
