...
function assertWritableInstalledPluginIndexStoreOptions(
  ...
  if (options.filePath?.endsWith(".json")) {
    throw new Error(
      "Explicit JSON installed plugin index paths are retired. Use the shared SQLite state DB or run openagent doctor --fix to migrate legacy plugins/installs.json.",
    );
  }
}
...
function resolveRefreshedPersistedInstalledPluginIndex(
  ...
  if (params.reason === "manual" && !params.installRecords) {
    ...
    if (foreignPluginIds.length > 0) {
      throw new Error(
        `Plugin registry refresh cannot verify npm install ownership outside the selected state directory: ${foreignPluginIds.join(", ")}. Reinstall copied plugins in this state directory, then run \`openagent plugins registry --refresh\` again.`,
      );
    }
  }
