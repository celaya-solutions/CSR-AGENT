/** Loads and normalizes Zero to Agent plugin manifests, including contracts and config schemas. */
...
export function loadPluginManifest(
  ...
  if (isCoreReservedPluginId(id)) {
    return cacheResult({
      ok: false,
      error: `plugin manifest id "${id}" is reserved by Zero to Agent core`,
      manifestPath,
    });
  }
