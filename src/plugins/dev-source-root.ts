...
/** Env var that points bundled-plugin lookup at a Zero to Agent source checkout. */
...
/** Resolves and validates the configured Zero to Agent development source root. */
...
export function formatSourceBundledPluginNotice(pluginId: string): string {
  return `Kept bundled plugin "${pluginId}" from the Zero to Agent source build; the registry artifact has no matching host SDK build identity. Matching version strings do not establish SDK compatibility.`;
}
