...
/** Env var that points bundled-plugin lookup at an OpenAgent source checkout. */
...
/** Resolves and validates the configured OpenAgent development source root. */
...
export function formatSourceBundledPluginNotice(pluginId: string): string {
  return `Kept bundled plugin "${pluginId}" from the OpenAgent source build; the registry artifact has no matching host SDK build identity. Matching version strings do not establish SDK compatibility.`;
}
