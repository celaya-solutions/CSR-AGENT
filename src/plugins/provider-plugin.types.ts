...
export type ProviderPlugin = {
   ...
   *
   * OpenAgent calls this only from async model resolution paths. Return the
   ...
   *
   * Runs after OpenAgent performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before OpenAgent registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic OpenAgent wrappers.
   *
   ...
   *
   * Called after OpenAgent resolves the raw configured credential but before the
   ...
   * hint than OpenAgent's generic auth-store guidance.
   */
   ...
   * OpenAgent's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when OpenAgent should surface a provider-specific hint for
   ...
   * merging. OpenAgent deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. OpenAgent stores the
   ...
   * without replacing the full OpenAgent-owned system prompt.
   */
   ...
   *
   * Runs after OpenAgent's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep OpenAgent's default fallback policy.
   */
   ...
   * rewrites after OpenAgent has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before OpenAgent handles
   ...
   *
   * OpenAgent uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that generic auth must ignore and `openagent doctor` should remove.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `openagent doctor` should offer.
   *
   ...
   *
   * OpenAgent calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep OpenAgent's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Keep process/network I/O here; OpenAgent publishes the completed result for this generation.
   */
   ...
   * OpenAgent falls back to that stored profile.
   */
