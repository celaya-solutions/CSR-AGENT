...
export type ProviderPlugin = {
   ...
   *
   * Zero to Agent calls this only from async model resolution paths. Return the
   ...
   *
   * Runs after Zero to Agent performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Zero to Agent registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Zero to Agent wrappers.
   *
   ...
   *
   * Called after Zero to Agent resolves the raw configured credential but before the
   ...
   * hint than Zero to Agent's generic auth-store guidance.
   */
   ...
   * Zero to Agent's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Zero to Agent should surface a provider-specific hint for
   ...
   * merging. Zero to Agent deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Zero to Agent stores the
   ...
   * without replacing the full Zero to Agent-owned system prompt.
   */
   ...
   *
   * Runs after Zero to Agent's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Zero to Agent's default fallback policy.
   */
   ...
   * rewrites after Zero to Agent has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Zero to Agent handles
   ...
   *
   * Zero to Agent uses this when a stored auth profile is already valid and needs to
   ...
   *
   * Zero to Agent calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep Zero to Agent's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Keep process/network I/O here; Zero to Agent publishes the completed result for this generation.
   */
   ...
   * Zero to Agent falls back to that stored profile.
   */
