...
function addLegacyNpmDeclarationDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `legacy npm plugin declaration ignored for "${declaration.pluginId}"; run "openagent doctor --fix" to install ${declaration.npmSpec} into the managed plugin root`,
  });
  return true;
}
...
function shouldSkipIncompatiblePackagePluginApi(params: {
  ...
  params.diagnostics.push({
    level: "warn",
    source: path.join(params.packageDir, "package.json"),
    message: `plugin requires plugin API ${packagePluginApiRange}, but this host is ${compatibilityHostVersion}; skipping discovery (check "openagent --version", OPENCLAW_COMPATIBILITY_HOST_VERSION, or run "openagent doctor")`,
    pluginId: params.pluginId,
  });
  return true;
}
    ...
    tracePluginLifecyclePhase(
      ...
      () => {
        ...
        if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
          // Keep workspace auto-discovery constrained to the OpenAgent extensions root.
