// Covers supervisor marker files used to identify managed OpenAgent processes.
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd from OpenAgent's explicit marker or current gateway launchd job", () => {
    expect(
  ...
  it("detects Linux OpenAgent gateway service markers only for opt-in callers", () => {
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ OPENCLAW_WINDOWS_TASK_NAME: "OpenAgent Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
