// Covers supervisor marker files used to identify managed Zero to Agent processes.
...
describe("detectRespawnSupervisor", () => {
  it("detects launchd from Zero to Agent's explicit marker or current gateway launchd job", () => {
    expect(
  ...
  it("detects Linux Zero to Agent gateway service markers only for opt-in callers", () => {
  ...
  it("detects scheduled-task supervision on Windows from either hint family", () => {
    expect(
      detectRespawnSupervisor({ OPENCLAW_WINDOWS_TASK_NAME: "Zero to Agent Gateway" }, "win32"),
    ).toBe("schtasks");
    expect(
      detectRespawnSupervisor(
        {
