...
// update.
//
// `runGatewayUpdate` (git mode) runs `openagent doctor --fix` with
...
// `shouldDeferConfiguredPluginInstallRepair`). The `openagent update` CLI resumes
...
// binary's hidden `openagent update finalize` entrypoint — the designed
...
// re-enters the full `openagent update` CLI. Re-run convergence on no-op retries:
  ...
  return {
    ...
    steps: [
      ...result.steps,
      {
        ...
        command: "openagent update finalize",
