// Validates the current runtime before Zero to Agent startup.
...
/** Returns whether a detected runtime meets Zero to Agent's minimum runtime contract. */
...
/** Returns whether the current process runtime satisfies Zero to Agent's engine contract. */
export function isCurrentRuntimeSupported(): boolean {
  return runtimeSatisfies(detectRuntime());
}

/** Checks a Node version label against Zero to Agent's supported Node version range. */
export function isSupportedNodeVersion(version: string | null): boolean {
  return isSupportedOpenClawNodeVersion(version);
}

/** Checks a Bun version label against Zero to Agent's minimum supported release. */
...
export async function assertSupportedRuntime(
  ...
  const retryHint =
    details.kind === "bun"
      ? "Upgrade Bun or run Zero to Agent with a supported Node release."
