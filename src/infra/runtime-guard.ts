// Validates the current runtime before OpenAgent startup.
...
/** Returns whether a detected runtime meets OpenAgent's minimum runtime contract. */
...
/** Returns whether the current process runtime satisfies OpenAgent's engine contract. */
export function isCurrentRuntimeSupported(): boolean {
  return runtimeSatisfies(detectRuntime());
}

/** Checks a Node version label against OpenAgent's supported Node version range. */
export function isSupportedNodeVersion(version: string | null): boolean {
  return isSupportedOpenClawNodeVersion(version);
}

/** Checks a Bun version label against OpenAgent's minimum supported release. */
