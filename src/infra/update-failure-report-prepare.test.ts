...
describe("update report diagnostic command boundary", () => {
  ...
  it.each([
    ...
    { service: "healthy", outcome: "verified safe to restart" },
    {
      service: "failed",
      outcome:
        "runtime files verified; Gateway restart failed. Run `openagent gateway status --deep` before restarting manually.",
    },
  ] as const)(
  ...
  it("uses the failure code when a phase label is executable text", async () => {
    ...
    expect(report.body).not.toContain("openagent doctor");
  });
