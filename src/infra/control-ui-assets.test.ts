...
`,
  ...
  it.each(["ready", "stale", "incomplete"])(
    ...
    async (kind) => {
      ...
      const execPath = path.join(root, "OpenClaw.app", "Contents", "MacOS", "OpenAgent");
      ...
      if (result.ok) {
        ...
        expect(result.message).toContain("Reinstall OpenAgent");
      }
  ...
  it("tells packaged installs to reinstall when their bundled assets are missing", async () => {
    ...
    await expect(
      ...
      ok: false,
      built: false,
      message: `Missing Control UI assets at ${indexPath}. Reinstall OpenAgent to restore bundled Control UI assets.`,
    });
  });
  ...
  it("rejects a packaged index whose first-party startup asset is missing", async () => {
    ...
    await expect(ensureControlUiAssetsBuilt(undefined, { root })).resolves.toEqual({
      ok: false,
      built: false,
      message: `Incomplete Control UI assets at ${indexPath} (missing assets/startup.js). Reinstall OpenAgent to restore bundled Control UI assets.`,
    });
    expect(state.runCommandWithTimeout).not.toHaveBeenCalled();
  });
