  ...
  it("kills a stuck scan at its deadline before releasing ownership", async () => {
    ...
    try {
      await expect(
        ...
        `SQLite integrity check timed out after 31 seconds (budget for 15 B) for ${source}. Stop the Gateway service and other OpenAgent processes using this database, then retry; if already stopped, check storage performance. (lastObservedPhase=checking)`,
      );
