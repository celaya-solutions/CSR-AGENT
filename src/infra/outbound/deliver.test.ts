...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes OpenAgent tmp root in plugin mediaLocalRoots", async () => {
  ...
  it("queues a spool copy while the live send keeps the producer's path", async () => {
    ...
    // OpenAgent temp root that TTS actually writes to. Staging must resolve the
  ...
  it("stages agent-workspace media that only the agent-scoped capability can read", async () => {
    ...
    // Deliberately outside the OpenAgent temp root: that root is itself a default
