...
describe("update run ledger", () => {
  ...
  it.each([
    ...
    (status, stepStatus) => {
      ...
      expect(persisted?.steps.find((step) => step.step === "openagent doctor")).toEqual({
        step: "openagent doctor",
        status: stepStatus,
        startedAtMs: 1_000,
        endedAtMs: 2_000,
    ...
    const run = createUpdateRun(
      {
        trigger: "cli",
        origin: {
          nextAction: "Run openagent update cleanup --dry-run for state /home/operator/.openclaw.",
        },
      },
      options,
    );
