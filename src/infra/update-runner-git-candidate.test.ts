...
describe("Git candidate activation", () => {
  ...
  beforeEach(async () => {
    ...
    await git(remote, "config", "user.name", "OpenAgent Test");
    ...
    await git(root, "config", "user.name", "OpenAgent Test");
  ...
  it.each([
    ...
    async ({ layout, localCommit, inspection }) => {
      ...
      if (localCommit) {
        ...
        expect.soft(committer === "OpenAgent Test <openclaw@example.com>").toBe(true);
      }
