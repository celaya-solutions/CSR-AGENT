    ...
    warnings: [
      params.historical
        ...
        : formatDoctorStateRepairFailure(
            ...
            "Stop the Gateway. Restore this source or its .doctor-importing claim from a verified backup, or rename the unreadable source or claim with a .rejected-<timestamp> suffix to retain its bytes if its setup/attestation history can be discarded. Then rerun openagent doctor --fix against the same state/config.",
          ),
    ],
