...
function targetsLiveStateSqliteDatabase(
  ...
  // External SQLite clients bypass Zero to Agent's runtime/version guard and can join the live WAL.
...
export async function rejectUnsafeExecControlShellCommand(command: string): Promise<void> {
  ...
  if (unsafeKind === "channel-login") {
    throw new Error(
      [
        "exec cannot run interactive Zero to Agent channel login commands.",
...
export async function rejectUnsafeExecLiveStateSqliteShellCommand(
  ...
  throw new Error(
    [
      "external sqlite3 cannot open databases under the active Zero to Agent state directory.",
      "Use Zero to Agent commands for live state, or inspect a private backup copy outside `OPENCLAW_STATE_DIR`.",
    ].join(" "),
  );
}
