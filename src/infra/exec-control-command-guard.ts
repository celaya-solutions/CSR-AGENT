...
function targetsLiveStateSqliteDatabase(
  ...
  // External SQLite clients bypass OpenAgent's runtime/version guard and can join the live WAL.
...
export async function rejectUnsafeExecControlShellCommand(command: string): Promise<void> {
  ...
  if (unsafeKind === "channel-login") {
    throw new Error(
      [
        "exec cannot run interactive OpenAgent channel login commands.",
        "Run `openagent channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
      ].join(" "),
    );
  }
}
...
export async function rejectUnsafeExecLiveStateSqliteShellCommand(
  ...
  throw new Error(
    [
      "external sqlite3 cannot open databases under the active OpenAgent state directory.",
      "Use OpenAgent commands for live state, or inspect a private backup copy outside `OPENCLAW_STATE_DIR`.",
    ].join(" "),
  );
}
