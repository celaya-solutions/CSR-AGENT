...
describe("update run report", () => {
  it.each(["status", "failure"])(
    ...
    async (surface) => {
      ...
      expect(text).toContain(
        "A 2026.9.2-era update never progressed past admission; treated as abandoned after 24 h; run `openagent update` to retry.",
      );
    },
  );
  ...
  it.each([
    ...
    ["repair-requires-config-change", "run openagent doctor --fix under your own authority"],
  ...
  it("keeps recent failures and next action within the chat budget without truncating CLI guidance", () => {
    ...
    expect(
      report.markdown.endsWith("Run openagent triage to diagnose and repair the failed update."),
    ).toBe(true);
  ...
  it.each([
    ...
    const originAction = "Run `openagent --profile work triage` to repair this installation.";
    const nextAction =
      source === "options"
        ? "Run `openagent --profile team triage` to repair this installation."
        : originAction;
    const report = renderUpdateRunReport(
    ...
    expect(report.markdown).not.toContain("Run openagent triage");
    expect(report.markdown).not.toContain("run openagent doctor --fix");
    expect(report.markdown).not.toContain("operator can run openagent triage locally");
    if (source === "options") {
  ...
  it("reports pending work, verification, and repair facts without inferring success", () => {
    const report = renderUpdateRunReport(
      run({
        ...
        phase: "verifying",
        after: {},
        origin: { doctorHint: "Run openagent doctor", nextAction: "Run the update manually" },
        verification: {
          booted: true,
          versionMatch: false,
          channelsReady: false,
    ...
    expect(report.markdown).not.toContain("openagent doctor");
