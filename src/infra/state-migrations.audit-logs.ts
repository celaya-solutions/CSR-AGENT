  ...
  try {
    ...
    if (params.source.storage === "raw-archive") {
      ...
      if (!scrubbedRecords.ok) {
        ...
        warnings.push(
          `Retained uncheckpointed ${params.source.label} recovery archive; rerun openagent doctor --fix`,
        );
        return result("refused");
      }
      if (scrubbedRecords.records.length !== 0) {
        warnings.push(
          `A legacy ${params.source.label} writer appended during recovery; rerun openagent doctor --fix to import the retained rows`,
        );
        return result("refused");
      }
    ...
    if (scrubbedRecords.records.length !== 0) {
      warnings.push(
        `A legacy ${params.source.label} writer appended during migration; rerun openagent doctor --fix to import the retained rows`,
      );
      return result("refused");
    }
    ...
    if ((await root.exists(sourceRelativePath)) && !params.recreatedSourceScheduled) {
      warnings.push(
        `An old writer recreated ${params.source.label} at ${params.source.logicalSourcePath}; rerun openagent doctor --fix to import the retained rows`,
      );
    }
    return result(checkpointed ? "completed" : "refused");
  } finally {
