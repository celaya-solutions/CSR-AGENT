...
const {
  ...
  >(async () => ({
    ...
    pid: 12345,
    command: "openagent update --yes --channel beta --timeout 2700",
...
describe("update-startup", () => {
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("openagent update")}`,
    );
  ...
  it("discovers and deduplicates an exact extended-stable update without auto-applying", async () => {
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (extended-stable): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("openagent update")}`,
    );
        ...
        () =>
          new Promise((resolve) => {
            releaseHandoff = () => {
              const handoff = {
                pid: 12345,
                command: "openagent update --yes --channel beta",
  ...
  it("keeps a foreground Gateway serving when automatic update has no restart owner", async () => {
    ...
    expect((await readRestartSentinel())?.payload).toMatchObject({
      ...
      message: expect.stringMatching(
        /Stop the foreground Gateway.*`openagent --profile work update --yes --channel beta --tag 2\.0\.0-beta\.1 --timeout 2700`.*then launch the Gateway again/s,
      ),
  ...
  it("transfers supervised auto-updates to validation while the gateway keeps serving", async () => {
    ...
    startManagedServiceUpdateHandoffMock.mockResolvedValueOnce({
      status: "started",
      pid: 12345,
      command: "openagent update --yes --channel beta --tag 2.0.0-beta.1 --timeout 2700",
      logPath: "/tmp/openclaw-handoff.log",
    ...
    async (triageFails) => {
      ...
      if (triageFails) {
        runUpdateFailureTriageMock.mockResolvedValueOnce({
          status: "failed",
          hint: "Triage could not complete: collector failed. Run openagent triage.",
        });
      }
      const log = { info: vi.fn() };
      const terminalSentinels: Array<ReturnType<typeof readRestartSentinel>> = [];
      ...
      expect(log.info).toHaveBeenCalledWith("auto-update attempt failed", {
        ...
        triage: expect.stringContaining(triageFails ? "openagent triage" : triageResult.hint),
      });
      expect(log.info).toHaveBeenCalledWith(
      ...
      expect((await terminalSentinels.at(-1))?.payload).toMatchObject({
        kind: "update",
        status: "error",
        doctorHint: expect.stringContaining(triageFails ? "openagent triage" : triageResult.hint),
