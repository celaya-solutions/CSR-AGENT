// Resolves the OpenAgent package root from runtime and package metadata.
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another OpenAgent checkout keeps its own package root.
  try {
...
// Every distinct OpenAgent package root among the runtime hints, in candidate order (symlinked
