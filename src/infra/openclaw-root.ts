// Resolves the Zero to Agent package root from runtime and package metadata.
...
function candidateDirsFromArgv1(argv1: string): string[] {
  ...
  // the target so a launcher nested under another Zero to Agent checkout keeps its own package root.
  try {
...
// Every distinct Zero to Agent package root among the runtime hints, in candidate order (symlinked
