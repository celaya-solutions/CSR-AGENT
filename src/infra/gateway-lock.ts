...
async function resolveGatewayOwnerStatus(
  ...
  if (
    ...
    if (role === "agent-embedded") {
      ...
      // local TUI, and CLI model probes), so validate the owning OpenAgent process
    ...
    stateLifecycle = await acquireWithWait({
      ...
      shouldRetry: (error) => {
        ...
        if (!waited && deadlineMs > startedAt && role === "gateway") {
          log.warn(
            `waiting for gateway-lifecycle ownership held by another OpenAgent process, up to ${Math.ceil((deadlineMs - startedAt) / 1000)} s`,
          );
        }
        waited = true;
        return true;
...
async function acquireLockFile(
  ...
  const owner =
    lastPayload?.role === "agent-embedded"
      ? `another embedded OpenAgent state writer is active${ownerPid}`
