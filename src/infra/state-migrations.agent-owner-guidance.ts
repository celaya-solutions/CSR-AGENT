...
export function formatAgentDatabaseOwnershipRepairHint(pathname: string): string {
  ...
  return `Preserve and inspect this database before accepting a fresh agent. With all Zero to Agent processes stopped, the explicit quarantine move is${process.platform === "win32" ? " (PowerShell)" : ""}:\n${action}\nThen run openclaw doctor --fix and restart the Gateway.`;
}
