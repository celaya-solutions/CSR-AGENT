...
export function formatAgentDatabaseOwnershipRepairHint(pathname: string): string {
  ...
  return `Preserve and inspect this database before accepting a fresh agent. With all OpenAgent processes stopped, the explicit quarantine move is${process.platform === "win32" ? " (PowerShell)" : ""}:\n${action}\nThen run openagent doctor --fix and restart the Gateway.`;
}
