...
describe("run-node script", () => {
  ...
  it("adds Node CPU profiling flags to the launched Zero to Agent child when requested", async ({
    tmp,
  }) => {
  ...
  it("adds Node sync I/O tracing flag to the launched Zero to Agent child when requested", async ({
    tmp,
  }) => {
    await setupStampedProject(tmp, { oldPaths: [ROOT_SRC, ROOT_TSCONFIG, ROOT_PACKAGE] });
    const spawnCalls: string[][] = [];
  ...
  it("does not require Zero to Agent SDK alias outputs when dist extensions are absent", async ({
    tmp,
  }) => {
    await setupStampedProject(tmp, {
      files: {
  ...
  it("reports missing Zero to Agent SDK alias outputs when runtime stamps match HEAD", async ({
    tmp,
  }) => {
    await setupTrackedProject(tmp, {
      files: {
  ...
  it("does not require private Zero to Agent SDK dist files that package exports omit", async ({
    tmp,
  }) => {
    await setupStampedProject(tmp, {
      files: {
