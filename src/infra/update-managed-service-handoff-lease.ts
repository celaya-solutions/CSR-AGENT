...
export function createManagedHandoffLeaseStore(
  ...
  function bootIdentity() {
    ...
    if (!parsed.success) {
      throw new Error("OS boot identity unavailable; run openagent triage manually");
    }
    return parsed.data;
  }
  ...
  function handle(root: string, value: LeaseRow): ManagedHandoffLease {
    ...
    if (!payload || !text.safeParse(value.owner).success) {
      throw new Error(
        "existing managed handoff lease is incompatible; retain diagnostics and run openagent triage manually",
      );
    }
    return {
      key: root,
