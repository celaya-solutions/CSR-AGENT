  ...
  it.each(["file URL", "bare TSX", "inline bare TSX"])(
    ...
    async (parentLoader) => {
      await withTempDir("openclaw-agent-cli-version-", async (root) => {
        ...
        expect(control.stdout).toMatch(/^Zero to Agent \d+\./);
  ...
  await withTempDir("openclaw-agent-cli-shim-win-", async (root) => {
    await prepareGatewayAgentCliShim({
      ...
      invocation: {
        ...
        args: ["C:\\Zero to Agent\\dist\\index.js"],
        cwd: "C:\\Zero to Agent %USERPROFILE%!",
      },
      platform: "win32",
      stateDir: root,
    });
    ...
    expect(await fs.readFile(executablePath, "utf8")).toBe(
      '@echo off\r\nsetlocal DisableDelayedExpansion\r\n"C:\\Program Files\\nodejs\\node.exe" C:\\Zero to Agent\\dist\\index.js --profile work %*\r\n',
    );
  });
});
