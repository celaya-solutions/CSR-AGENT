...
/** Supported package managers for OpenAgent global install and update flows. */
...
async function resolvePortableGitPathPrepend(): Promise<string[]> {
  ...
  const portableGitRoot = path.join(localAppData, "OpenAgent", "deps", "portable-git");
  const candidates = [
 ...
 * Used for staged installs where OpenAgent creates the prefix itself.
 */
...
/**
 * Detects an installed global OpenAgent package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global OpenAgent install.
 * npm receives quiet/freshness-bypass flags; pnpm and Bun approve OpenAgent's lifecycle.
 */
