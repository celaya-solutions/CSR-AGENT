  ...
  for (const target of params.targets) {
    ...
    try {
      ...
      results.set(target.path, {
        recovered: true,
        warning: `Recovered agent ${target.agentId}: ${target.path} was a byte-identical copy of agent ${ownerId}'s database. Preserved the copy at ${recovery.movedFiles.join(", ")}. Agent ${target.agentId} can start with a fresh database. Run openagent doctor --fix to verify repairs.`,
      });
    } catch (error) {
      results.set(target.path, {
        recovered: false,
