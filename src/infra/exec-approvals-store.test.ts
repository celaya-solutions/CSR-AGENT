...
describe("exec approvals SQLite store", () => {
  ...
  it.each([
    ...
    (_, suffix) => {
      ...
      expect(caught).toMatchObject({
        message: `Legacy exec approvals exist at ${sourcePath}. Run \`openagent doctor --fix\` with OPENCLAW_STATE_DIR set to ${stateDir} before using exec approvals.`,
      });
  ...
  it("scopes the doctor command to the blocked state directory", () => {
    // A bare `openagent doctor --fix` repairs the default root, leaving a scoped
    ...
    expect(error.message).toContain(
      `Run \`openagent doctor --fix\` with OPENCLAW_STATE_DIR set to ${stateDir}`,
    );
  });
