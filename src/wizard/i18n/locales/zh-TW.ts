...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 OpenAgent 設定",
      ...
      tokenStoreProvisioned:
        "已產生 Gateway 權杖並以 {name} 存入 OpenAgent 祕密儲存。設定中只保留參照；可用 `openagent secrets store list` 檢視。",
    ...
    migration: {
      ...
      appliedTitle: "遷移已套用",
      cancelled: "遷移已取消",
      complete: "遷移完成。下一步執行 `openagent doctor`。",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 OpenAgent 工作區？",
    ...
    remote: {
      ...
      plaintextTokenHint: "將密鑰直接儲存在 OpenAgent 設定中",
    ...
    guided: {
      laneQuestion: "你想如何開始？",
      laneSecurityLine: "OpenAgent 執行的 AI 代理程式擁有這台機器的實際存取權限。",
      ...
      quickstartBackground: "如需日後讓閘道在背景持續執行，請執行：openagent gateway install",
      quickstartReopen: "執行 openclaw 開啟 TUI，或執行 openagent dashboard 重新開啟網頁儀表板。",
      quickstartBrowserUnavailable: "瀏覽器交接未完成。閘道仍在執行；準備好後請開啟儀表板。",
      quickstartGatewayPending:
        "閘道尚無法連線。請保持此終端機開啟，並執行 openagent gateway status 檢查啟動狀態。",
      ...
      custodianIntro: "你好 — 我是 OpenAgent，負責維護這套系統。我們開始設定吧。",
      findMeLater: "之後隨時找得到我 — 在終端機執行 `openagent`，或在儀表板中開啟設定。",
      ...
      setupDone: "一切就緒。",
      appliedTitle: "推理已就緒",
      complete: "OpenAgent 已準備就緒。",
      completeWithoutAi: "OpenAgent 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      escapeHatches: "如需完整的分步精靈，請執行 `openagent onboard --classic`。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "OpenAgent 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
        "未進行任何變更。請執行 {fixCommand} 修復，再執行 {inspectCommand} 檢查，然後重試 onboarding。",
      invalidConfigUnknown: "- 無法解析設定。",
      manualChoice: "你想如何連接 AI？",
      nextSteps:
        "工作區：{workspace}\n新增頻道：`openagent channels add`\n偏好聊天？執行 `openagent setup`，然後說 `connect telegram`（或 `connect slack`）。\n開啟 dashboard：`openagent dashboard`\n稍後聊天：`openagent`",
      nextStepsWithoutAi:
        "工作區：{workspace}\n稍後新增 AI：重新執行 `openagent onboard`\n連接 AI 後新增頻道：`openagent channels add`\n開啟 dashboard：`openagent dashboard`",
      nextStepsTitle: "下一步",
      noInferenceOptions:
      ...
      ttyRequired:
        "Onboarding 需要互動式 TTY。自動化請使用 `openagent onboard --non-interactive --accept-risk ...`。",
      welcomeTitle: "設定選項",
    ...
    security: {
      ...
      attribution:
        "OpenAgent 是由 OpenAgent 基金會（非營利組織）開發的開源助手，會與你一同學習成長。",
      baselineDmSessions:
      ...
      confirm: "我理解 OpenAgent 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉存取控制和安全加固，請在獲得協助前不要執行 OpenAgent。",
      notMultitenant: "預設情況下，OpenAgent 並非旨在安全地隔離多個使用者。",
      personalAgent: "預設情況下，OpenAgent 是供一位操作者使用的個人 agent。",
    ...
    telemetry: {
      ...
      decline: "不用，謝謝",
      description:
        "在每日更新檢查中分享你使用的功能（頻道、提供者、插件數量）。絕不包含訊息或識別碼。查看實際傳送的內容：`openagent telemetry show`。隨時變更：`openagent telemetry on|off`。",
      title: "幫助 OpenAgent 變得更好？",
    },
    skills: {
      configure: "現在設定技能？（建議）",
      containerBrewHidden:
      ...
      manualPrereqsDoctorHint: "請先安裝這些工具，然後執行 `openagent doctor` 檢查技能和需求。",
    ...
    channels: {
      ...
      commandOwnerHelp:
        "聊天存取權限允許使用者與你的代理交談。命令擁有者還可以更新 OpenAgent、重新啟動 Gateway、變更設定並核准命令。請僅選擇你自己信任的管理員帳號。伺服器和群組無需 DM 配對也可使用此功能；它不會授予聊天存取權限或變更頻道存取規則。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → OpenAgent → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "OpenAgent" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 OpenAgent 將你加入允許清單。",
      ...
      separatePhoneLabel: "OpenAgent 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 OpenAgent 用來向 Chat 傳送回覆的 URL。",
      ...
      publicWebhookUrlScope: "僅公開此 webhook 路由；OpenAgent 使用它提供短期附件下載。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "OpenAgent"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 OpenAgent，並啟用完整 agent 能力。",
    ...
    search: {
      ...
      configured: "已設定",
      configureLaterHint: "稍後可用 openagent configure --section web 設定",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 OpenAgent。",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 OpenAgent。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 OpenAgent。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 OpenAgent。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。OpenAgent 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
        "在你的電腦上執行 agent 存在風險。請執行 openagent security audit --deep 檢查你的設定。",
