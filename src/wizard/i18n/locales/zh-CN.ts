...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 Zero to Agent 配置",
      ...
      tokenStoreProvisioned:
        "已生成 Gateway 令牌并以 {name} 存入 Zero to Agent 密钥存储。配置中只保留引用；可用 `openclaw secrets store list` 查看。",
    ...
    memoryImport: {
      ...
      confirm: "将这些 memory 导入你的 Zero to Agent 工作区？",
    ...
    remote: {
      ...
      plaintextTokenHint: "将密钥直接存储在 Zero to Agent 配置中",
    ...
    guided: {
      laneQuestion: "你想如何开始？",
      laneSecurityLine:
        "Zero to Agent 运行的 AI 智能体拥有这台机器的实际访问权限。安全指南：https://docs.openclaw.ai/gateway/security",
      ...
      custodianIntro: "你好 — 我是 Zero to Agent，负责维护这套系统。我们开始设置吧。",
      ...
      setupDone: "一切就绪。",
      appliedTitle: "推理已就绪",
      complete: "Zero to Agent 已准备就绪。",
      completeWithoutAi: "Zero to Agent 设置已保存。连接 AI 后再打开聊天。",
      detected: "AI 检测完成。",
      ...
      intro: "连接你的 AI",
      invalidConfigDetails: "Zero to Agent 配置 {path} 无效：\n{issues}",
      invalidConfigRepair:
    ...
    security: {
      ...
      attribution:
        "Zero to Agent 是由 Zero to Agent 基金会（非营利组织）开发的开源助手，会与你一同学习成长。",
      baselineDmSessions:
      ...
      confirm: "我理解 Zero to Agent 默认面向个人使用；共享/多用户使用需要加固。继续？",
      hardeningRequired: "如果你不熟悉访问控制和安全加固，请在获得帮助前不要运行 Zero to Agent。",
      learnMore: "了解更多请访问",
      notMultitenant: "默认情况下，Zero to Agent 并非旨在安全地隔离多个用户。",
      personalAgent: "默认情况下，Zero to Agent 是供一位操作员使用的个人 agent。",
    ...
    channels: {
      ...
      commandOwnerHelp:
        "聊天访问权限允许用户与你的代理交谈。命令所有者还可以更新 Zero to Agent、重启 Gateway、更改配置并批准命令。请仅选择你自己信任的管理员账号。服务器和群组无需 DM 配对也可使用此功能；它不会授予聊天访问权限或更改频道访问规则。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作区设置 → Integrations → Zero to Agent → 创建 bot → 复制 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "Zero to Agent" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 Zero to Agent 将你加入允许列表。",
      ...
      separatePhoneLabel: "Zero to Agent 专用独立手机",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 Zero to Agent 用来向 Chat 发送回复的 URL。",
      ...
      publicWebhookUrlScope: "仅公开此 webhook 路由；Zero to Agent 使用它提供短期附件下载。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "Zero to Agent"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 Zero to Agent，并启用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本机浏览器中复制/粘贴这个 URL 来控制 Zero to Agent。",
      dashboardOpened: "已在浏览器中打开。保留该标签页以控制 Zero to Agent。",
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 Zero to Agent。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 Zero to Agent。",
      ...
      containerSystemdUnavailable:
        "此容器内没有 systemd 用户服务。Zero to Agent 只会跳过后台服务安装；请以前台方式运行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
