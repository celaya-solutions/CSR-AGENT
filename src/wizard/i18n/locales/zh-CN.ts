...
export const zh_CN = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密码存入 OpenAgent 配置",
    ...
    memoryImport: {
      ...
      confirm: "将这些 memory 导入你的 OpenAgent 工作区？",
    ...
    remote: {
      ...
      plaintextTokenHint: "将密钥直接存储在 OpenAgent 配置中",
    ...
    guided: {
      ...
      setupDone: "一切就绪。",
      appliedTitle: "推理已就绪",
      complete: "OpenAgent 已准备就绪。",
      completeWithoutAi: "OpenAgent 设置已保存。连接 AI 后再打开聊天。",
      detected: "AI 检测完成。",
      ...
      intro: "连接你的 AI",
      invalidConfigDetails: "OpenAgent 配置 {path} 无效：\n{issues}",
      invalidConfigRepair:
    ...
    security: {
      ...
      attribution:
        "OpenAgent 是由 OpenAgent 基金会（非营利组织）开发的开源助手，会与你一同学习成长。",
      baselineDmSessions:
    ...
    channels: {
      ...
      commandOwnerHelp:
        "聊天访问权限允许用户与你的代理交谈。命令所有者还可以更新 OpenAgent、重启 Gateway、更改配置并批准命令。请仅选择你自己信任的管理员账号。服务器和群组无需 DM 配对也可使用此功能；它不会授予聊天访问权限或更改频道访问规则。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作区设置 → Integrations → OpenAgent → 创建 bot → 复制 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 运行：./occ talk:bot:install "OpenAgent" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要发送者/所有者号码，以便 OpenAgent 将你加入允许列表。",
      ...
      separatePhoneLabel: "OpenAgent 专用独立手机",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "这是 OpenAgent 用来向 Chat 发送回复的 URL。",
      ...
      publicWebhookUrlScope: "仅公开此 webhook 路由；OpenAgent 使用它提供短期附件下载。",
    ...
    signal: {
      ...
      multipleEntries: "多个条目请用逗号分隔。",
      nextLinkDevice: '绑定设备：signal-cli link -n "OpenAgent"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "将 Synology NAS Chat 连接到 OpenAgent，并启用完整 agent 能力。",
    ...
    finalize: {
      ...
      optionalApps: "可选应用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 链接控制 OpenAgent。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已打开；保留该标签页以控制 OpenAgent。",
