...
describe("setupWizardShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("openagent", {
      generationMode: "full",
    });
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "openagent");
  ...
  it.each([
    ...
    async ({ profileInstalled, usesSlowPattern }) => {
      ...
      expect(prompter.note).toHaveBeenCalledWith(
        "Failed to generate completion cache. Run `openagent completion --write-state --install` later.",
        "Shell completion",
      );
      expect(deps.installCompletion).not.toHaveBeenCalled();
    ...
    await withEnvAsync({ OPENCLAW_LOCALE: "zh-CN" }, async () => {
      ...
      expect(prompter.confirm).toHaveBeenCalledWith(
        expect.objectContaining({
          message: "为 openagent 启用 zsh shell completion？",
        }),
      );
      expect(prompter.note).toHaveBeenCalledWith(
  ...
  it.each([
    ...
    await withEnvAsync(
      ...
      async () => {
        const cachePath = resolveCompletionCachePath(testCase.shell, "openagent");
  ...
  it("shows a concrete PowerShell profile reload command after setup", async () => {
    await withEnvAsync({ HOME: "/Users/ada" }, async () => {
      ...
      expect(deps.installCompletion).toHaveBeenCalledWith("powershell", true, "openagent");
      expect(prompter.note).toHaveBeenCalledWith(
