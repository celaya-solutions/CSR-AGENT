...
export async function withConfigWriteLock<T>(
  ...
  return await configMutationQueue
    ...
    .catch(async (error: unknown) => {
      ...
      throw new Error(
        `Zero to Agent cannot write to the config directory ${configDir}. Fix its ownership or permissions, then try again. Underlying error: ${formatErrorMessage(error)}`,
        { cause: error },
      );
    });
}
