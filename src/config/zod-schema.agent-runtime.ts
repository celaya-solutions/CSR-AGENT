...
function addExecPolicyModeConflictIssue(
  ...
  ctx.addIssue({
    code: z.ZodIssueCode.custom,
    path: ["mode"],
    message: `mode cannot be combined with security or ask in the same exec object. Update the deploy script, template, or patch at this scope. ${repair} Doctor migrates supported legacy policies to mode; run "openagent doctor --fix" only when the saved file still needs migration.`,
  });
}
...
const CodeModeSchema = z
  .union([
    ...
    z
      .object({
        /** OpenAgent Code Mode default, overridden by per-model codeMode. Default: false; "auto" engages catalog-preferred models. */
...
export const ToolsSchema = z
  .object({
    ...
    sessions: z
      ...
      .optional(),
    loopDetection: ToolLoopDetectionSchema,
    /** Compact large OpenAgent, MCP, and client tool catalogs behind search/call tools. */
