...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the OpenAgent agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus OpenAgent, or the agent id plus OpenAgent. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the OpenAgent agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which OpenAgent conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing OpenAgent Discord conversation. */
