...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the Zero to Agent agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus Zero to Agent, or the agent id plus Zero to Agent. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the Zero to Agent agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which Zero to Agent conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing Zero to Agent Discord conversation. */
