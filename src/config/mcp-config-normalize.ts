...
/** Maps CLI-native MCP type aliases to OpenAgent HTTP transport names. */
...
/** Checks whether a raw MCP `type` value is a legacy CLI alias OpenAgent can rewrite. */
...
export function canonicalizeConfiguredMcpServer(
  ...
  // `transport` is OpenAgent's canonical field; legacy `type` only fills a gap.
