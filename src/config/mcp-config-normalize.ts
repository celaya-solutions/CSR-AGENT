...
/** Maps CLI-native MCP type aliases to Zero to Agent HTTP transport names. */
...
/** Checks whether a raw MCP `type` value is a legacy CLI alias Zero to Agent can rewrite. */
...
export function canonicalizeConfiguredMcpServer(
  ...
  // `transport` is Zero to Agent's canonical field; legacy `type` only fills a gap.
