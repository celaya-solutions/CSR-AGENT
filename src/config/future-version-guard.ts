...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this Zero to Agent binary (${currentVersion}) is older than the config last written by Zero to Agent ${touchedVersion}.`,
    hints: [
