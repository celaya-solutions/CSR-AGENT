...
export class ConfigReadOnlyError extends Error {
  ...
  constructor(params: { configPath?: string } = {}) {
    super(
      [
        "Config is externally managed (`OPENCLAW_CONFIG_READONLY=1`), so OpenAgent treats openclaw.json as immutable.",
        ...(params.configPath ? [`Config path: ${params.configPath}`] : []),
        "Edit the config in your external deployment source, then redeploy or restart OpenAgent as needed.",
      ].join("\n"),
    );
