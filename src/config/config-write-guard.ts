...
/** Public Zero to Agent Nix overview shown with immutable-config errors. */
...
export class ConfigReadOnlyError extends Error {
  ...
  constructor(params: { configPath?: string } = {}) {
    super(
      [
        "Config is externally managed (`OPENCLAW_CONFIG_READONLY=1`), so Zero to Agent treats openclaw.json as immutable.",
        ...(params.configPath ? [`Config path: ${params.configPath}`] : []),
        "Edit the config in your external deployment source, then redeploy or restart Zero to Agent as needed.",
      ].join("\n"),
    );
...
function formatNixModeConfigMutationMessage(params: { configPath?: string } = {}): string {
  return [
    ...
    `Zero to Agent Nix overview: ${NIX_OVERVIEW_URL}`,
  ].join("\n");
}
