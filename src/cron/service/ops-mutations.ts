...
export async function remove(
  ...
  const cleanup = async () => {
    try {
      ...
      const message = `Cron job ${id} was removed, but session cleanup failed: ${String(error)}. Use openagent sessions list --json, then openagent sessions delete to retry.`;
