...
function formatHooksTokenReuseRemediation(reuse: GatewayAuthSharedSecretReuse): string {
  ...
  return `Run ${formatCliCommand("openagent doctor --fix")} to rotate a persisted hooks.token, then update external hook senders to use the new hook token.`;
}
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep OPENCLAW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("openagent security audit --fix")}".`,
    });
  }
  return findings;
}
  ...
  if (controlPlaneContexts.length > 0) {
    findings.push({
      ...
      detail:
        ...
        "Prompt injection in open conversations can trigger OpenAgent updates, plugin changes, or scheduled automation.",
...
export function collectLikelyMultiUserSetupFindings(cfg: OpenClawConfig): SecurityAuditFinding[] {
  ...
  if (mainGroupScopes.length > 0) {
    findings.push({
      ...
      remediation:
        'Use session.groupScope="per-group" globally and remove binding overrides, or reserve "main" for rooms whose members you trust.',
    });
  }
  ...
  findings.push({
    ...
    detail:
      ...
      `\n${impactLine}\n${riskyContextsDetail}\n` +
      "OpenAgent's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway. For multiple users or organizations, run one isolated Gateway cell per tenant.",
    remediation:
