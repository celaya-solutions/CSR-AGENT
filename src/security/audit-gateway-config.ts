...
export function collectGatewayConfigFindings(
  ...
  if (cfg.mcp?.apps?.enabled === true) {
    findings.push({
      ...
      remediation:
        "Keep this enabled only for MCP servers you trust. Disable with `openagent config set mcp.apps.enabled false --strict-json` when it is not needed.",
    });
  }
  ...
  for (const credential of ["token", "password"] as const) {
    ...
    if (isInvalidGatewaySecret(input)) {
      findings.push({
        ...
        remediation:
          credential === "token"
            ? "Run `openagent doctor --fix --generate-gateway-token` for an inline token; otherwise rotate its external secret source. Restart the Gateway afterward."
