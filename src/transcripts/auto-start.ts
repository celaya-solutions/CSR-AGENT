  ...
  const stopCapture = async (capture: OwnedCapture, requireProviderStop = false) => {
    ...
    try {
      ...
      if (typeof details.summaryExportError === "string") {
        warnings.push(
          `summary saved; export failed intendedSummaryPath=${formatAutoStopDiagnostic(details.intendedSummaryPath)}: ${formatAutoStopDiagnostic(details.summaryExportError)}. Correct the export destination, then run openagent transcripts path <session> or openagent transcripts show <session>.`,
        );
      }
