...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[openagent proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[openagent proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`openagent proxy coverage\` for details.`,
  );
}
