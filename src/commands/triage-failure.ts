...
export async function triageAfterFailure(
  ...
  try {
    await withConsoleLogsRoutedToStderr(async () => {
      ...
      if (!resolvedRoot) {
        throw new Error("installed CLI root is unavailable; run openagent triage manually");
      }
    ...
    runtime.error(
      `Automatic triage could not complete: ${reason}. Run \`openagent triage\` manually.`,
    );
    if (managedStartup && !cancellation.aborted) {
      try {
        ...
        runtime.error(
          "Managed triage diagnostics could not complete; retain the original failure and run openagent triage manually.",
        );
      }
    }
    if (failure.kind === "update" && failure.installationRoot && !cancellation.aborted) {
      ...
      try {
        ...
        runtime.error(
          `Saved failure diagnostics: ${promptPath}. Run openagent triage manually after repairing the installed CLI.`,
        );
      } catch {
        runtime.error(
          "Failure diagnostics could not be saved; retain the original update error and run openagent triage manually.",
        );
      }
    }
  } finally {
