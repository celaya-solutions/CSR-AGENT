...
async function resolveSystemdServiceRewriteBlock(
  ...
  return `Gateway service ${unitName} is running; skipped command/entrypoint rewrites and leaving supervisor metadata unchanged. Stop the service first or use \`openagent gateway install --force\` when you want to replace the active launcher.`;
}
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (
    ...
    note(
      "Update-mode doctor detected gateway service drift but left the live systemd unit unchanged. Review the service file and run `openagent gateway install --force` when you want OpenAgent to rewrite its managed unit; operator-owned drop-ins remain unchanged.",
      "Gateway service config",
    );
    return cfg;
  }
  ...
  if (!repair) {
    if (!emittedSourceCheckoutWarning) {
      note(
        "Run `openagent gateway install --force` when you want to replace the gateway service definition.",
        "Gateway service config",
      );
    }
    return cfg;
...
export async function maybeResolveDuelingSystemdGatewayScopes(
  ...
  note(
    [
      "Both a user-scope and a system-scope OpenAgent gateway unit are installed:",
      `- user:   ${user.unitPath}`,
