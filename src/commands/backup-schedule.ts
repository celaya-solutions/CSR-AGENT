...
const LOCAL_GATEWAY_REQUIRED_ERROR =
  "backup enable manages backups on the Gateway host and currently requires a local Gateway. Create the cron job manually with openagent cron add for remote Gateways.";
...
export async function backupEnableCommand(
  ...
  if (options.push) {
    ...
    if (origin.code !== 0) {
      throw new Error(
        `--push requires an origin remote. Run: openagent backup git init --repository ${shortenHomePath(repositoryPath)} --remote <url>`,
      );
    }
    if (!redactSecrets) {
