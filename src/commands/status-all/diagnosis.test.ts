...
describe("status-all diagnosis port checks", () => {
  ...
  it("treats a single wildcard Gateway listener as healthy", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("✓ Port 18789");
    expect(output).toContain("Detected OpenAgent Gateway listener on the configured port.");
    expect(output).not.toContain("Port 18789 is already in use.");
  });
  ...
  it("keeps warning for multi-process listener conflicts", async () => {
    ...
    const output = params.lines.join("\n");
    expect(output).toContain("! Port 18789");
    expect(output).toContain("2 OpenAgent gateway processes appear to be listening on port 18789");
  ...
  it("does not let attributed listeners override indeterminate availability", async () => {
    ...
    expect(output).toContain("! Port 18789");
    expect(output).toContain("Port 18789 availability could not be determined.");
    expect(output).not.toContain("Detected OpenAgent Gateway listener");
  });
  ...
  it.each([
    {
      ...
      headline: "⚠️ OpenAgent update failed: managed-service-handoff-failed.",
      hint: "Run openagent triage to diagnose and repair the failed update.",
    },
    {
      status: "skipped",
      reason: "restart-health-pending",
      headline: "⬆️ OpenAgent update in progress: restarting.",
      hint: "Check progress with openagent update status.",
    },
  ] as const)(
    ...
    async ({ status, reason, headline, hint }) => {
      ...
      expect(output).not.toContain("run openagent gateway restart");
    },
  );
  ...
  it("renders failed diagnostics as unavailable instead of empty", async () => {
    ...
    expect(output).toContain("Retry: openagent gateway stability --type telemetry.exporter");
    ...
    expect(output).toContain("Retry: openagent gateway stability");
  ...
  it("avoids unreachable gateway diagnosis in node-only mode", async () => {
    ...
    expect(output).not.toContain("Retry: openagent gateway stability");
  });
