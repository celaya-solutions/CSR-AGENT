...
describe("promptDefaultModel", () => {
  it("adds runtime-route hints for canonical OpenAI models", async () => {
    ...
    expect(canonical.hint).not.toContain("OpenAgent runtime route");
  });
  ...
  it.each([
    ...
    "labels official OpenAI with %s as an OpenAgent runtime route",
    async (_label, defaults) => {
      ...
      expect(option.hint).toContain("OpenAgent runtime route");
  ...
  ] as const)("labels an OpenAI %s as an OpenAgent runtime route", async (_label, api, baseUrl) => {
    loadModelCatalog.mockResolvedValue([
