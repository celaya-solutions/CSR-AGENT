...
export async function runNonInteractiveLocalSetup(params: {
  ...
  if (workspaceConflict) {
    runtime.error(
      [
        ...
        `Run \`${formatCliCommand("openagent onboard --classic")}\` to confirm moving the existing agent fleet.`,
      ].join("\n"),
    );
  }
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `openagent gateway run` or `openagent agent --local`.",
              ]
            : [`Run \`${formatCliCommand("openagent gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
    ...
    if (!probe.ok) {
      ...
      if (!explicitlySkippedAbsentGateway || !opts.json) {
        logNonInteractiveOnboardingFailure({
          ...
          hints: !opts.installDaemon
            ? [
                "Non-interactive local setup only waits for an already-running gateway unless you pass `--install-daemon` to `openagent onboard`.",
                `Fix: start \`${formatCliCommand("openagent gateway run")}\`, re-run \`${formatCliCommand("openagent onboard --install-daemon")}\`, or use \`${formatCliCommand("openagent onboard --skip-health")}\`.`,
      ...
      try {
        ...
        logNonInteractiveOnboardingFailure({
          ...
          daemonInstall: daemonInstallStatus,
          daemonRuntime: opts.installDaemon ? daemonRuntimeRaw : undefined,
          hints: [`Run \`${formatCliCommand("openagent health")}\` for full diagnostics.`],
        });
        runtime.exit(1);
        return;
      }
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("openagent configure --section web")}\` to store your Brave API key for web_search.`,
    );
  }
}
