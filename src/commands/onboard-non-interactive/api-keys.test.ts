...
describe("resolveNonInteractiveApiKey", () => {
  ...
  it.each([
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      testCase.resolvedEnv
        ? "Paste the API key value, not an OpenAgent onboarding command. Check ZAI_API_KEY."
  ...
  it("rejects a command-shaped explicit env key before a secret-ref flag", async () => {
    ...
    process.env.ZAI_API_KEY = "openagent onboard --non-interactive --auth-choice zai-api-key"; // pragma: allowlist secret
    resolveEnvApiKey.mockImplementation(() => {
  ...
  it.each([
    ...
    },
    {
      ...
      flagValue: "openagent onboard --non-interactive --auth-choice fixture-api-key",
