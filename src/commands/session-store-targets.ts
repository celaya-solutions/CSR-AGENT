...
export function resolveExplicitSessionStorePath(params: {
  ...
  if (databaseFailure) {
    throw new Error(
      `Session store target is not a session store: ${displayTarget}. ${formatErrorMessage(databaseFailure.error)}. Pass a legacy store selector or SQLite target reported by openagent sessions or openagent status.`,
    );
  }
  return storePath;
}
