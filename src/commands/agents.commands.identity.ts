...
export async function agentsSetIdentityCommand(
  ...
  if (normalizedAgent && !normalizedAgent.ok) {
    failAgentIdentity(`Agent "${opts.agent}" not found. Create it with \`openagent agents add\`.`);
  }
  ...
  if (!resolvedAgentIds.includes(resolvedAgentId)) {
    failAgentIdentity(
      `Agent "${resolvedAgentId}" not found. Create it with \`openagent agents add\`.`,
    );
  }
  ...
  if (locatorDiffers && workspaceLocatorDir) {
    ...
    runtime.log(
      `Stored workspace unchanged. Relocate with ${formatCliCommand(`openagent config set agents.entries.${resolvedAgentId}.workspace ${quoteCliArg(workspaceLocatorDir)}`)}.`,
    );
