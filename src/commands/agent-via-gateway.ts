...
function missingAgentMessageError(): Error {
  return new Error(
    `Missing message. Use ${formatCliCommand('openagent agent --message "..." --agent <id>')} or ${formatCliCommand("openagent agent --message-file <path> --agent <id>")}.`,
  );
}
...
function formatGatewayAgentTransportLossHint(err: unknown): string | undefined {
  ...
  return (
    ...
    "Check `openagent gateway status` and the session transcript before retrying or rerunning with --local, so the turn does not execute twice."
  );
}
...
async function agentViaGatewayCommand(
  ...
  if (
    ...
    throw new Error(
      `No target session selected. Use --agent <id>, --session-key <key>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("openagent agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (knownAgents && !knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("openagent agents list")}" to see configured agents.`,
      );
    }
  }
...
export async function agentCliCommand(
  ...
  if (isCompactControlCommand(messageOpts.message)) {
    runtime.error?.(
      "Slash commands cannot be executed via --message from the CLI. Use: openagent sessions compact <key>",
    );
    runtime.exit(1);
    return undefined;
  }
