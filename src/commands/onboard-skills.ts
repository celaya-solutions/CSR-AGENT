...
export async function setupSkills(
  ...
  if (candidateInstallable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        ...
        `To inspect available skills, run: ${formatCliCommand("openagent skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("openagent skills check")}`,
      ].join("\n"),
  ...
  if (candidateInstallable.length > 0) {
    ...
    for (const target of selectedReadySkills) {
      ...
      // visible through `openagent skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("openagent doctor")}\` to review skills + requirements.`,
      );
    }
