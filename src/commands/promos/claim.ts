...
function resolveAuthChoice(
  ...
  if (!entry) {
    throw new Error(
      `Promotion "${promotion.slug}" requires auth choice "${authChoiceId}", which this Zero to Agent version does not know. Update Zero to Agent and retry.`,
    );
  }
...
function requirePromotionPlugins(
  ...
  throw new Error(
    `Promotion "${promotion.slug}" requires plugin package "${unsupported[0]}", but ${authChoiceLabel} does not provide it in this Zero to Agent version. Update Zero to Agent and retry.`,
  );
}
