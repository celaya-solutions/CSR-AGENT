...
describe("command owner health", () => {
  ...
  it("explains missing command owners in plain language", () => {
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "A command owner is your trusted human operator account, allowed to update OpenAgent with /update, restart the Gateway, change configuration, and approve commands. Chat allowlists do not grant this authority.",
        "Run openagent channels add and complete a channel's setup to choose your operator account, including servers and groups without DM pairing.",
        "CLI pairing approval records the first command owner. Control UI pairing approval has a separate owner checkbox.",
        "Fix: set commands.ownerAllowFrom to your channel user id, for example openagent config set commands.ownerAllowFrom '[\"telegram:123456789\"]'",
  ...
  it.each([
    ...
    expect(
      ...
      `Ask the operator to run \`openagent config set commands.ownerAllowFrom '${JSON.stringify(expected)}'\` in a terminal to make this sender a command owner.`,
    );
  });
