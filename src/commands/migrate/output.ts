...
const REASON_CODE_MESSAGES: Record<string, string> = {
  ...
  already_active: "Plugin is already active in Zero to Agent",
  installed: "Plugin is already installed in Zero to Agent",
...
function formatItemMessage(item: MigrationItem, mode: FormatMode): string | undefined {
  if (mode === "preview") {
    ...
    if (item.kind === "skill" && item.action === "copy") {
      return "Copy Codex skill into Zero to Agent";
    }
    if (item.kind === "plugin" && item.action === "install") {
      return "Install Codex plugin into Zero to Agent";
    }
...
export function writeApplyResult(
  ...
  if (result.backupPath) {
    ...
    runtime.log("Backup: skipped (no existing Zero to Agent state found)");
  }
  if (result.reportDir) {
