...
export async function prepareDoctorConfigRecovery(params: {
  ...
  if (params.enabled && snapshot.exists && !snapshot.valid) {
    ...
    if (!snapshot.valid && typeof snapshot.raw === "string" && !parseConfigJson5(snapshot.raw).ok) {
      throw new Error(
        `Config at ${snapshot.path} is not parseable and cannot be repaired automatically. The file remains unchanged. Inspect the exact parse error with ${formatCliCommand("openagent config validate")}, then hand-edit the file; or move it aside and run ${formatCliCommand("openagent onboard")} to generate a fresh config.`,
      );
    }
  }
