...
describe("status.command-sections", () => {
  ...
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:openagent security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:openagent security audit --deep)");
  });
  ...
  it("marks activated plugin service failures as warnings in deep health rows", () => {
    ...
    expect(rows).toContainEqual({
      ...
      Status: "warn(WARN)",
      Detail: "failed - service scheduler: address already in use; run openagent doctor",
    });
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:openagent status --all",
      "  Need to debug live? cmd:openagent logs --follow",
      "  Fix reachability first: cmd:openagent gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:openagent devices approve req-123)",
      "muted(Fallback: cmd:openagent devices approve --latest)",
      "muted(Inspect: cmd:openagent devices list)",
    ]);
  });
