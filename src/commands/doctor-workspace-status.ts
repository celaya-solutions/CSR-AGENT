...
function pluginVersionDriftToHealthFindings(
  ...
  return drift.drifts.map((entry) => {
    ...
    return {
      checkId: WORKSPACE_STATUS_CHECK_ID,
      severity: "warning",
      message: `Plugin ${entry.pluginId} is ${entry.installedVersion}, but a Gateway restart will load OpenAgent ${drift.gatewayVersion}.${runningGatewayVersion ? ` The running Gateway is ${runningGatewayVersion}.` : ""}${updateCommand ? "" : ` Repair target resolution failed: ${targetError}.`}`,
...
function notePluginVersionReadiness(readiness: PluginVersionRestartReadiness | undefined) {
  ...
  const lines = [
    ...
    } not on post-restart OpenAgent ${drift.gatewayVersion}`,
