...
export type GuidedOnboardingDeps = {
   ...
   * "chat" preserves the legacy handoff into the OpenAgent system-agent chat —
...
async function runGuidedOnboardingFlow(
  ...
  // Inference is the only prerequisite for OpenAgent. Use the caller's or
  // current default workspace as isolated probe context; OpenAgent owns any
  ...
  if (alreadyConfigured) {
    ...
    // would, then hand off to the hatch instead of parking in the OpenAgent chat.
    ...
    try {
      ...
      if (!appliedSnapshot.valid) {
        throw new Error("Setup wrote an invalid OpenAgent config.");
      }
