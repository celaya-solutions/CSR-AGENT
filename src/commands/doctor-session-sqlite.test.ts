...
describe("runDoctorSessionSqlite", () => {
  ...
  it("keeps mismatched older agent schema versions blocking during all-agent import", async () => {
    ...
    expect(failureReport).toContain("openagent doctor --session-sqlite recover --github-issue");
  ...
  it("aborts import when the legacy transcript changes between passes", () => {
    ...
    try {
      ...
      ).toThrow(/stop active session writers and rerun `openagent doctor --fix`/);
  ...
  it("aborts a batch when a prepared transcript changes before import", async () => {
    ...
    try {
      await expect(importLegacyStore(store)).rejects.toThrow(
        /stop active session writers and rerun `openagent doctor --fix`/,
      );
      expect(fs.existsSync(store.transcriptPath)).toBe(true);
    } finally {
      statSpy.mockRestore();
  ...
  it.each([
    ...
    },
    {
      ...
      message: /run openagent doctor --fix before compacting/iu,
    },
  ...
  it.each([1, 2, 3] as const)(
    ...
    (manifestVersion) => {
      ...
      expect(receiptJson).not.toContain("openagent doctor");
