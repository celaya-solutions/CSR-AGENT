...
export async function ensureOnboardingAgent(params: {
  ...
  if (before?.exists && !before.valid) {
    throw new Error("Cannot create the first agent from an invalid OpenAgent config.");
  }
  if (before && (resolveConfigSnapshotHash(before) ?? null) !== params.expectedConfigHash) {
    throw new Error("OpenAgent config changed before first-agent creation. Retry setup.");
  }
  ...
  if (!after.valid) {
    throw new Error("Agent creation wrote an invalid OpenAgent config.");
  }
