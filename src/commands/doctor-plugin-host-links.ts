...
/** Relinks npm-owned plugin packages to the current Zero to Agent host package. */
export async function maybeRepairPluginOpenClawHostLinks(
  ...
  if (!params.prompter.shouldRepair) {
    ...
    if (audit.peerLinkIssues.length > 0) {
      note(
        [
          "Managed npm Zero to Agent host peer links need repair:",
    ...
    if (audit.registeredPeerLinkIssues.length > 0) {
      note(
        [
          "Registered npm plugin Zero to Agent host links need repair:",
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm Zero to Agent host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
