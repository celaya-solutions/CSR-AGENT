...
/** Relinks npm-owned plugin packages to the current OpenAgent host package. */
export async function maybeRepairPluginOpenClawHostLinks(
  ...
  if (!params.prompter.shouldRepair) {
    ...
    if (audit.registeredPeerLinkIssues.length > 0) {
      note(
        [
          "Registered npm plugin OpenAgent host links need repair:",
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm OpenAgent host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
