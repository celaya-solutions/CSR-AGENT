...
export async function noteMacDisabledGatewayLaunchAgent(env: NodeJS.ProcessEnv = process.env) {
  ...
  note(
    [
      ...
      `After verifying the installation is safe to run, use ${labelEnv}${formatCliCommand("openagent gateway start", env)} to re-enable and start it. Keep the same state/config overrides.`,
      `If an update was interrupted or installation safety is uncertain, run ${formatCliCommand("openagent update", env)} or ${formatCliCommand("openagent doctor", env)} and ${formatCliCommand("openagent triage", env)} before starting it.`,
    ].join("\n"),
    "Gateway (macOS)",
  );
}

/** Returns a warning for stale OpenAgent updater launchd jobs left after interrupted updates. */
async function collectMacStaleOpenClawUpdateLaunchdJobsWarning(): Promise<string | null> {
  ...
  return [
    "- Stale OpenAgent updater launchd job(s) detected.",
    ...jobs.map((job) => {
      const exitStatus =
    ...
    `  ${formatCliCommand("openagent gateway restart")}`,
  ].join("\n");
}
    ...
    return audit.issues
      ...
      .map((issue) =>
        [
          ...
          `Run ${formatCliCommand("openagent gateway install --force")} only after verification; inspect drop-ins separately.`,
        ].join("\n"),
      );
  }
