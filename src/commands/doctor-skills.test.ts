...
describe("doctor skills", () => {
  ...
  it.each([
    ...
    expect(output.includes("Disable unused skills: openagent doctor --fix")).toBe(
      update && !available,
    );
  });
  ...
  it("formats unavailable skill names compactly and alphabetically", async () => {
    ...
    expect(typeof body === "string" ? body.split("\n") : []).toEqual([
      ...
      "- calendar, places",
      "Disable unused skills: openagent doctor --fix",
      "Inspect details: openagent skills check --agent <id> or openagent skills info <name> --agent <id>",
    ]);
  });
