...
export function noteCopilotAmbientToken(cfg: OpenClawConfig, env = process.env): void {
  ...
  if (claimed) {
    note(
      "GitHub Copilot is no longer enabled by GH_TOKEN/GITHUB_TOKEN. To use Copilot, run `openagent models auth login --provider github-copilot` or set COPILOT_GITHUB_TOKEN.",
      "GitHub Copilot",
    );
  }
}
...
export function noteSharedAuthStoreStatus(env: NodeJS.ProcessEnv = process.env): void {
  ...
  note(
    "Shared auth profiles still live in the main agent database. Run `openagent doctor --fix` to move them into shared SQLite state and make the main agent deletable.",
    "Shared auth store",
  );
}
...
async function resolveAuthIssueHint(
  ...
  if (issue.reasonCode === "malformed_api_key") {
    return "Paste the API key value, not an OpenAgent onboarding command.";
  }
  ...
  return {
    ...
    fixHint:
      ...
      (params.issue.status === "expiring"
        ? "Run `openagent doctor --fix` to refresh expiring OAuth profiles, or re-authenticate static tokens."
        : "Run `openagent doctor --fix` to refresh OAuth profiles, or re-authenticate this provider."),
  };
}
