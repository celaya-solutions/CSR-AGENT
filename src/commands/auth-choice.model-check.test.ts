...
describe("warnIfModelConfigLooksOff", () => {
  ...
  it("checks auth readiness without publishing a model catalog", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      'No auth configured for provider "openai". The agent may fail until credentials are added. Run `openagent models auth login --provider openai`, `openagent configure`, or set an API key env var.',
      "Model check",
    );
  });
  ...
  it("does not use pending auth profiles from a different provider", async () => {
    ...
    expect(note).toHaveBeenCalledWith(
      'No auth configured for provider "anthropic". The agent may fail until credentials are added. Run `openagent models auth login --provider anthropic`, `openagent configure`, or set an API key env var.',
      "Model check",
    );
  });
  ...
  it("accepts subscription auth but not key sources for gpt-5.3-codex-spark", async () => {
    ...
    expect(warning).toContain("openagent models auth login --provider openai");
