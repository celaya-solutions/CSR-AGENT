...
vi.mock("../status/summary.runtime.js", () => ({
  statusSummaryRuntime: {
    ...
    resolveSessionRuntime: vi.fn(() => ({ id: "openclaw", label: "OpenAgent Default" })),
...
describe("getStatusSummary", () => {
  ...
  it("reports task schema migration state without failing status", async () => {
    ...
    expect(summary.tasks.warning).toBe(
      "Task history is unavailable until Gateway startup or openagent doctor --fix repairs the state database.",
    );
  });
