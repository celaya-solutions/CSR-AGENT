...
export async function triageCommand(
  ...
  // Packaged Zero to Agent/Bun hosts cannot interpret npm shim entrypoints. Reuse the
  ...
  if (!installRoot) {
    throw new Error(
      "Cannot locate the Zero to Agent installation; use a suggested handoff command.",
    );
  }
  const failedResult =
