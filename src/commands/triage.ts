...
export async function triageCommand(
  ...
  // Packaged OpenAgent/Bun hosts cannot interpret npm shim entrypoints. Reuse the
  ...
  const agentLabel = runEmbedded
    ? "the embedded OpenAgent agent using your configured model"
    : handoff?.agent;
  ...
  if (!installRoot) {
    throw new Error("Cannot locate the OpenAgent installation; use a suggested handoff command.");
  }
  const failedResult =
  ...
  if (result.status === "unavailable") {
    ...
    throw new Error(
      `Embedded agent unavailable: ${result.reason}. Run \`openagent onboard\` or use a suggested handoff command.`,
    );
  }
