// Implements `openagent channels capabilities` account capability/probe reporting.
import {
...
export async function channelsCapabilitiesCommand(
  ...
  if ((!rawChannel || rawChannel === "all") && (opts.account || rawTarget)) {
    ...
    const message = `${option} requires a specific --channel. Run ${formatCliCommand("openagent channels list")} to choose one.`;
