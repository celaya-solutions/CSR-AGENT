...
describe("sessions lifecycle commands", () => {
    ...
    async ({ key, canonicalKey, agentId }) => {
      ...
      expect(runtime.log).toHaveBeenCalledWith(
        expect.stringContaining(
          `openagent memory forget --agent ${agentId} --session ${canonicalKey} on the Gateway host or container using its state and configuration`,
        ),
      );
    },
  );
  ...
  it("keeps separate owners when delete responses share a canonical key", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      expect.stringContaining("openagent memory forget --agent work --session global"),
    );
    expect(runtime.log).toHaveBeenCalledWith(
      expect.stringContaining("openagent memory forget --agent peer --session global"),
    );
  });
  ...
  it.each([undefined, "ws://gateway.test"])(
    ...
    async (url) => {
      ...
      expect(runtime.log).toHaveBeenCalledWith(
        expect.stringContaining(
          "openagent memory forget --agent work --session 'agent:work:notes;echo unsafe' on the Gateway host or container using its state and configuration",
        ),
      );
  ...
  it("does not print memory forget guidance when no delete archive is retained", async () => {
    ...
    expect(runtime.log).not.toHaveBeenCalledWith(
      expect.stringContaining("openagent memory forget"),
    );
  });
    ...
    expect(runtime.writeJson).toHaveBeenCalledWith(
      {
        ...
        results: [
          ...
          },
          {
            ...
            ok: false,
            status: "not_found",
            error: expect.stringContaining("openagent sessions list --json"),
          },
          {
            key: "agent:main:last",
            ok: false,
