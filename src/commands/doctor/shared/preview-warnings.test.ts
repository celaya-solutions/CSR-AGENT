...
describe("doctor preview warnings", () => {
  ...
  it("reports merged gateway owner profiles without applying the repair", async () => {
    ...
    const warning = 'Gateway owner profile is merged. Run "openagent doctor --fix" to repair it.';
  ...
  it("includes stale plugin config warnings", async () => {
    ...
    expect(warning).toContain('Run "openagent doctor --fix"');
  ...
  it("includes stale OAuth profile shadow warnings", async () => {
    staleOAuthShadowState.warnings = [
      '- ~/.openclaw/agents/telegram/agent/auth-profiles.json has stale OAuth auth profile openai-codex:default. Run "openagent doctor --fix".',
    ];
  ...
  it("includes stale configured auth-order warnings", async () => {
    staleAuthOrderState.warnings = [
      "- auth.order.anthropic references only missing profiles while compatible stored credentials exist; run openagent doctor --fix to remove the stale override and restore automatic selection.",
    ];
  ...
  it("warns but skips auto-removal when plugin discovery has errors", async () => {
    ...
    expect(warning).toContain('rerun "openagent doctor --fix"');
  });
