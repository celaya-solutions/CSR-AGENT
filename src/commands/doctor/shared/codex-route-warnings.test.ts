...
function legacyRouteWarning(...routes: string[]): string {
  return [
    ...
    ...routes,
    "- Run `openagent doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
  ].join("\n");
}
...
function disabledCodexPluginWarning(...routes: string[]): string {
  return [
    ...
    ...routes,
    "- Enable plugins.entries.codex and plugin loading, and remove `codex` from plugins.deny; or set the affected OpenAI models to an OpenAgent runtime policy.",
  ].join("\n");
}

function codexCompactionWarning(...details: string[]): string {
  return [
    "- Codex runtime uses native server-side compaction and ignores OpenAgent compaction summarizer overrides.",
    ...details,
  ].join("\n");
}
...
describe("collectCodexRouteWarnings", () => {
  ...
  it("warns without executing a custom Codex app-server command", () => {
    ...
    expect(warnings).toStrictEqual([
      ...
      [
        "- Custom Codex app-server command bypasses OpenAgent's managed exact-version binary.",
        "- plugins.entries.codex.config.appServer.command: Doctor did not execute, inspect, or rewrite this command.",
        "- Remove the override to use managed Codex startup, or verify the custom binary matches the Codex version bundled with this OpenAgent release.",
      ].join("\n"),
    ]);
  });
  ...
  it("warns when Codex runtime has OpenAgent compaction summarizer overrides", () => {
    const warnings = collectCodexRouteWarnings({
      agents: {
        defaults: {
          model: "openai/gpt-5.5",
    ...
    expect(warnings).toStrictEqual([
      codexCompactionWarning(
        ...
        "- Run `openagent doctor --fix`: it removes unsupported Codex compaction overrides.",
      ),
    ]);
  });
  ...
  it("keeps the Codex plugin disabled when OpenAI routes explicitly use the OpenAgent runtime", () => {
    const result = maybeRepairCodexRoutes({
      plugins: DISABLED_CODEX_PLUGIN_CONFIG,
      models: {
        providers: {
  ...
  itKeepsCodexPluginDisabled(
    "keeps the Codex plugin disabled when an auth-profiled OpenAI route explicitly uses the OpenAgent runtime",
    {
      agents: {
        defaults: {
  ...
  it("keeps repaired OpenAI refs on Codex runtime even when the OpenAI provider is otherwise OpenAgent/API-key routed", () => {
    const result = maybeRepairCodexRoutes({
      models: {
        providers: {
          openai: {
  ...
  it("preserves explicit OpenAgent runtime pins while repairing legacy session routes", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves Codex runtime intent alongside explicit OpenAgent harness pins", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves canonical OpenAI sessions that are explicitly pinned to OpenAgent", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
