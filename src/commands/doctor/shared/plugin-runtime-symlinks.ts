...
function stalePluginRuntimeSymlinkToHealthFinding(item: StalePluginRuntimeSymlink): HealthFinding {
  return {
    ...
    fixHint: "Run `openagent doctor --fix` to remove stale plugin-runtime symlinks.",
  };
}
...
export async function noteStalePluginRuntimeSymlinks(
  ...
  const lines = [
    ...
    `  ${PLUGIN_RUNTIME_DEPS_MARKER} directories from a previous OpenAgent install.`,
  ...
  lines.push("- Repair: run `openagent doctor --fix` to remove the dangling symlinks.");
