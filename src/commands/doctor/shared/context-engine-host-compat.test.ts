...
describe("doctor context-engine host compatibility", () => {
  ...
  it.each([true, false])(
    ...
    async (discovered) => {
      ...
      const params = { cfg, doctorFixCommand: "openagent doctor --fix" };
  ...
  it("evaluates native Codex and OpenAgent agent-run hosts", async () => {
    ...
    expect(warnings.join("\n")).toContain("OpenAgent embedded runner");
