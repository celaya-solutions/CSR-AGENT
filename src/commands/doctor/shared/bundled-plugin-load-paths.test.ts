...
describe("bundled plugin load path repair", () => {
  ...
  it("removes stale bundled paths from old versioned OpenAgent package roots", () => {
  ...
  it("removes stale legacy bundled paths from old versioned OpenAgent package roots", () => {
    const currentPackageRoot = path.resolve("node_modules", "openclaw");
    const stalePackageRoot = path.resolve(
      "pnpm-global",
      ".pnpm",
