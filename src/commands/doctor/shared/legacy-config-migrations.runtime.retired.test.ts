  ...
  it.each([true, false])(
    ...
    (suppressToolErrors) => {
      ...
      expect(findLegacyConfigIssues(raw)).toContainEqual({
        ...
        message:
          'messages.suppressToolErrors is retired; tool failure warnings now appear only when a run ends without a reply. Run "openagent doctor --fix".',
      });
  ...
  it.each([
    ...
    if (checkRule) {
      ...
      expect(retiredRule?.message).toBe(
        'agents.defaults.compaction.truncateAfterCompaction is retired; byte-triggered compaction now opts in via maxActiveTranscriptBytes alone. Run "openagent doctor --fix".',
      );
    }
  });
