...
describe("repairMissingConfiguredPluginInstalls", () => {
  ...
  it("maps a missing beta-channel plugin to a structured finding and dry-run effect offline", async () => {
    ...
    expect(
      ...
      severity: "warning",
      target: "matrix",
      fixHint: "Run `openagent doctor --fix` to install @openclaw/plugin-matrix.",
    });
    expect(
  ...
  it("installs a missing configured OpenAgent channel plugin from npm by default", async () => {
    const cfg = {
  ...
  it("honors npm-first catalog metadata for missing OpenAgent channel plugins", async () => {
    mocks.installPluginFromNpmSpec.mockResolvedValueOnce(
      successfulInstall({
        pluginId: "twitch",
