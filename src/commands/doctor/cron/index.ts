...
function legacyCronStoreFinding(params: {
  ...
  return {
    ...
    fixHint:
      params.fixHint ??
      `Run ${formatCliCommand("openagent doctor --fix")} to normalize legacy cron storage.`,
  };
}
  ...
  try {
    ...
    return [
      legacyCronStoreFinding({
        ...
        fixHint: [
          `Fix the file's permissions or contents and re-run ${formatCliCommand("openagent doctor")}.`,
    ...
    if (names.length > 0) {
      findings.push(
        legacyCronStoreFinding({
          ...
          path: sqliteStorePath,
          requirement,
          fixHint: `Review with ${formatCliCommand("openagent automations list --all")} and reauthorize with ${formatCliCommand("openagent automations edit <id> --tools <tool,...>")}.`,
        }),
      );
    }
  }
  ...
  if (normalized.legacyGatewayExecJobs.length > 0) {
    findings.push(
      legacyCronStoreFinding({
        ...
        fixHint:
          "Review the affected jobs with `openagent automations list --all`, then recreate each one from a fresh authenticated creator turn or explicitly reauthorize its complete tool cap from a trusted operator shell.",
      }),
    );
  }
...
export async function maybeRepairLegacyCronStore(params: {
  ...
  try {
    ...
    note(
      [
        ...
        `- ${reason}`,
        `Fix the file's permissions or contents and re-run ${formatCliCommand("openagent doctor")}; later health checks will continue.`,
      ].join("\n"),
      "Cron",
    );
    return;
  ...
  if (rawJobs.length === 0) {
    ...
    note(
      [
        noteHeading,
        ...previewLines,
        `Repair with ${formatCliCommand("openagent doctor --fix")} to finish the migration.`,
      ].join("\n"),
      "Cron",
    );
  ...
  if (inFlightCount > 0) {
    ...
    note(
      [
        ...
        `- Review with ${formatCliCommand("openagent automations list --all")} or ${formatCliCommand("openagent automations show <id>")}.`,
      ].join("\n"),
      "Cron",
    );
  }
  ...
  if (chronicFailureCount > 0) {
    note(
      [
        ...
        `- Review with ${formatCliCommand("openagent automations list")} or ${formatCliCommand("openagent automations show <id>")}.`,
      ].join("\n"),
      "Cron",
    );
  }
    ...
    note(
      [
        ...
        ...autoDisabledJobs.map(
          (job) =>
            `- ${job.name} (${job.id}): recorded reason \`${job.reason}\` after ${job.consecutiveErrors} consecutive errors. Fix the cause, then re-enable with ${formatCliCommand(`openagent automations enable ${job.id}`)}.`,
        ),
      ].join("\n"),
      "Cron",
    );
  ...
  note(
    [
      noteHeading,
      ...previewLines,
      `Repair with ${formatCliCommand("openagent doctor --fix")} to normalize the store before the next scheduler run.`,
    ].join("\n"),
    "Cron",
  );
