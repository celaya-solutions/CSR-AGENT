...
describe("agents add command", () => {
  ...
  it.each([
    {
      ...
      message: `Non-interactive agent creation requires --workspace. Re-run ${formatCliCommand("openagent agents add <id> --workspace <path>")} or omit flags to use the wizard.`,
    },
    {
    ...
    },
    {
      ...
      flags: { hasAutomationFlags: true },
      message: `Agent name is required in non-interactive mode. Run ${formatCliCommand("openagent agents add <id> --workspace <path>")}.`,
    },
    {
    ...
    ...RESERVED_SYSTEM_AGENT_IDS_FOR_TEST.map((agentId) => ({
      ...
      flags: { hasAutomationFlags: true },
      message: `"${agentId}" is reserved. Choose another name, or run ${formatCliCommand("openagent agents list")} to inspect configured agents.`,
    })),
  ...
  it("uses the explicit agent target for auth checks and creation", async () => {
    ...
    expect(prompter.intro).toHaveBeenCalledWith("Add OpenAgent agent");
  ...
  it("surfaces the canonical main gate before guided auth or workspace side effects", async () => {
    ...
    checkAgentCreationGateMock.mockResolvedValueOnce({
      ...
      agentId: "main",
      message: "Run openagent doctor --fix, then retry.",
    });

    await agentsAddCommand({ name: "main" }, runtime);

    expect(checkAgentCreationGateMock).toHaveBeenCalledWith("main");
    expect(prompter.outro).toHaveBeenCalledWith("Run openagent doctor --fix, then retry.");
    expect(prompter.text).not.toHaveBeenCalled();
