...
describe("runGuidedOnboarding", () => {
  ...
  it("uses the configured workspace only as inference and OpenAgent context", async () => {
  ...
  it("keeps OpenAgent unavailable until a manual key passes", async () => {
  ...
  it("shows copyable repair commands without opening AI when config is invalid", async () => {
    ...
    expect(prompter.outro).toHaveBeenCalledWith(expect.stringContaining("openagent doctor --fix"));
    expect(prompter.outro).toHaveBeenCalledWith(
      expect.stringContaining("openagent config validate"),
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
    expect(deps.runSystemAgentChat).not.toHaveBeenCalled();
    expect(deps.detect).not.toHaveBeenCalled();
