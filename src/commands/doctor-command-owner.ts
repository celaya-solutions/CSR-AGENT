...
export function formatCommandOwnerHint(params: {
  ...
  const command = formatCliCommand("openagent config set commands.ownerAllowFrom");
...
export function noteCommandOwnerHealth(cfg: OpenClawConfig): void {
  ...
  note(
    [
      ...
      `Run ${formatCliCommand("openagent channels add")} and complete a channel's setup to choose your operator account, including servers and groups without DM pairing.`,
      "CLI pairing approval records the first command owner. Control UI pairing approval has a separate owner checkbox.",
      `Fix: set commands.ownerAllowFrom to your channel user id, for example ${formatCliCommand("openagent config set commands.ownerAllowFrom '[\"telegram:123456789\"]'")}`,
