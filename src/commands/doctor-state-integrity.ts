  ...
  switch (issue.kind) {
    ...
    case "missing-state-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `openagent doctor --fix` to create the state directory.",
      };
    case "state-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `openagent doctor --fix` to repair state directory permissions.",
      };
    case "state-dir-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `openagent doctor --fix` to tighten state directory permissions.",
      };
    case "config-file-too-open":
      return {
        ...
        path: issue.path,
        fixHint: "Run `openagent doctor --fix` to tighten config file permissions.",
      };
    case "missing-runtime-dir":
      return {
        ...
        path: issue.path,
        fixHint: "Run `openagent doctor --fix` to create missing runtime state directories.",
      };
    case "runtime-dir-not-writable":
      return {
        ...
        path: issue.path,
        fixHint: "Run `openagent doctor --fix` to repair runtime state directory permissions.",
      };
  }
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        `  Set locally: OPENCLAW_STATE_DIR=~/.openclaw ${formatCliCommand("openagent doctor")}`,
      ].join("\n"),
    );
  }
  if (linuxSdBackedStateDir) {
      ...
      if (wedgedSubagentSessions.length > 0) {
        ...
        warnings.push(
          [
            ...
            "  OpenAgent will not auto-resume these child sessions on restart; reconcile their task records instead.",
            `  Examples: ${wedgedSubagentSessions
              .slice(0, 3)
              .map(({ key }) => key)
              .join(", ")}`,
            `  Fix: ${formatCliCommand("openagent tasks maintenance --apply")}`,
          ].join("\n"),
        );
