...
describe("setupWizardCommand", () => {
  ...
  it("rejects --reset-scope without --reset", async () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      "--reset-scope requires --reset. Re-run with openagent onboard --reset --reset-scope full.",
    );
    expect(runtime.exit).toHaveBeenCalledWith(1);
    expect(mocks.handleReset).not.toHaveBeenCalled();
    expect(mocks.runInteractiveSetup).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    const runtime = makeRuntime();
    const message = `Invalid --mode "${mode}". Use "local" or "remote", or run ${formatCliCommand("openagent onboard")} for interactive setup.`;
    ...
    ...([false, true] as const).map(
      (json) =>
        [
          ...
          formatCliCommand(
            "openagent onboard --non-interactive --accept-risk --mode remote --remote-url ws://127.0.0.1:3000",
          ),
        ] as const,
    ),
    [
  ...
  it("validates a provider-specific API key before reset", async () => {
    ...
    await expectAuthPreflightError(
      ...
      () =>
        `Missing --anthropic-api-key (or ANTHROPIC_API_KEY in env). Export ANTHROPIC_API_KEY, pass --anthropic-api-key, or run ${formatCliCommand("openagent onboard")} for interactive setup.`,
    );
  });
