...
describe("SQLite backup commands", () => {
  ...
  it("reports missing snapshot paths for verify and restore", async () => {
    ...
    const runtime = createRuntimeCapture();
    const missingRepositoryMessage = `SQLite snapshot repository does not exist: ${repositoryPath}. Check the snapshot path or create a snapshot with \`openagent backup sqlite create\`.`;
    ...
    const missingSnapshotMessage = `SQLite snapshot does not exist: ${snapshotPath}. Run \`openagent backup sqlite list --repository ${repositoryPath}\` to inspect available snapshots.`;
    await expect(backupSqliteVerifyCommand(runtime, snapshotPath, {})).rejects.toThrow(
  ...
  it("requires exactly one named OpenAgent database source", async () => {
    const runtime = createRuntimeCapture();
