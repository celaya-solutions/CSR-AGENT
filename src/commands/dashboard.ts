// Implements `openagent dashboard` URL resolution, readiness check, clipboard, and browser launch.
...
async function resolveDashboardTarget() {
  ...
  if (snapshot.exists && !snapshot.valid) {
    throw new Error(
      `OpenAgent config is invalid: ${snapshot.path}. Run \`openagent doctor --fix\` or \`openagent config validate\`.`,
    );
  }
...
export async function dashboardCommand(
  ...
  if (!(await hasVerifiedControlUiLoopbackAlias(target))) {
    ...
    runtime.log("Restart the Gateway, then run `openagent gateway status --deep` for details.");
    return;
  }
  const document = await waitForControlUiDocument({
    url: target.documentUrl,
  ...
  if (!document.ready) {
    ...
    runtime.log("Run `openagent gateway status --deep` for details.");
    runtime.exit(1);
    return;
  }
  ...
  try {
    ...
    runtime.log("Run `openagent doctor`, then retry `openagent dashboard`.");
    return;
  }
  ...
  if (fallbackToManualAuth) {
    ...
    runtime.log(
      "One-time pairing URL not delivered. Run `openagent dashboard --json` and open its `browserUrl` within ten minutes.",
    );
  }
}
