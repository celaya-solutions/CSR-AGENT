...
export async function modelsAuthPasteTokenCommand(
  ...
  if (provider === "anthropic") {
    runtime.log("Anthropic setup-token auth is supported in OpenAgent.");
    runtime.log("OpenAgent prefers Claude CLI reuse when it is available on the host.");
    runtime.log("Anthropic staff told us this OpenAgent path is allowed again.");
  }
}
...
export async function runModelsAuthLoginFlowCore(
  ...
  if (useProviderPicker) {
    await prompter.note(
      `Provider "${requestedProviderId}" uses its own CLI login. Select a provider with an OpenAgent auth flow.`,
      "Provider auth",
    );
  ...
  if (opts.force) {
    ...
    try {
      ...
      if (!clearedStore) {
        throw new Error(
          "auth store is busy; close other OpenAgent commands using this state directory and retry",
        );
      }
      opts.runtime.log(
