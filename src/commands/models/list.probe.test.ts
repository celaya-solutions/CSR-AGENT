...
describe("runAuthProbes", () => {
    ...
    await withTempState(async (stateDir) => {
      ...
      try {
        await expect(
          ...
          `A Gateway is running for this state directory (pid ${process.pid}, port 28789). Stop the Gateway first (openagent gateway stop), then rerun models status --probe.`,
        );
      } finally {
  ...
  it("runs Codex-pinned auth probes through raw OpenAgent model-run mode", async () => {
