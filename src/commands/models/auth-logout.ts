...
export async function removeModelAuthCredentials(params: {
  ...
  if (configChanged && !(application.claimed && (await application.result) === "applied")) {
    return "Credentials were removed, but the Gateway has not confirmed applying the change. Run `openagent gateway restart` to apply it.";
  }
  return undefined;
}
...
export async function modelsAuthLogoutCommand(
  ...
  if (!profileId) {
    throw new Error(
      `Missing profile id. Run ${formatCliCommand("openagent models auth list")} to see saved profile ids.`,
    );
  }
  ...
  if (!credential) {
    throw new Error(
      `Auth profile "${profileId}" not found for agent "${agentId}". Run ${formatCliCommand(`openagent models auth list --agent ${agentId}`)} to see saved profile ids.`,
    );
  }
  ...
  if (remaining.length === 0) {
    runtime.log(
      `No auth profiles remain for ${credential.provider}. Run ${formatCliCommand(`openagent models auth login --provider ${credential.provider}`)} to sign in again.`,
    );
  }
}
