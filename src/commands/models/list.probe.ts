...
function formatActiveGatewayModelsProbeRefusal(identity: GatewayLockIdentity): string {
  return `A Gateway is running for this state directory (pid ${identity.pid}, port ${identity.port}). Stop the Gateway first (${formatCliCommand("openagent gateway stop")}), then rerun models status --probe.`;
}
