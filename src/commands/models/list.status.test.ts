    ...
    try {
      ...
      expect(parseFirstJsonLog(jsonRuntime).auth.unusableProfiles).toEqual([
        expect.objectContaining({
          ...
          recoveryHint:
            "Re-authenticate with `openagent models auth login --provider anthropic --profile-id 'anthropic:default'`.",
        }),
      ]);
      ...
      expect(output).toContain("openagent models auth login --provider anthropic");
    } finally {
