...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    ...
    hints.push(`Then reinstall: ${formatCliCommand("openagent gateway install", env)}`);
  }
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("openagent gateway install", env)}`);
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
    return hints;
  ...
  if (missingGuiSession || runtime.status === "stopped") {
    if (!missingGuiSession && platform === "linux" && isSystemdStartLimitHit(runtime)) {
      ...
      hints.push(
        ...
        `Recover with: ${formatCliCommand("openagent gateway restart", env)}, then inspect logs if it keeps crashing.`,
      );
  ...
  if (platform === "linux" && isSystemdCgroupHygieneRisk(runtime.systemd)) {
    ...
    if (summary) {
      hints.push(
        ...
        `After reviewing service settings, run: ${formatCliCommand("openagent gateway restart", env)}`,
      );
    }
  }
  return hints;
