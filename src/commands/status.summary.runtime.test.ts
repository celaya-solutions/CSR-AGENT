...
describe("statusSummaryRuntime.resolveSessionRuntime", () => {
  ...
  it("does not treat an unlocked producing harness as the current runtime", () => {
    ...
    ).toEqual({ id: "codex", label: "OpenAI Codex (previous runtime: OpenAgent Default)" });
  });

  it("reports the owning Codex harness for a locked session with stale OpenAgent metadata", () => {
    expect(
      resolveSessionRuntime({
        cfg: {
          agents: {
