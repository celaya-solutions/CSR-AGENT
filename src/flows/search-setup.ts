...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Zero to Agent will enable the plugin and use it as your web_search provider.",
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Zero to Agent will store a reference instead of the API key.",
