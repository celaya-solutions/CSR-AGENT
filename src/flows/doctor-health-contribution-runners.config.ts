    ...
    try {
      ...
      if (confirmedConfigSource && error instanceof ConfigMutationConflictError) {
        ...
        note(
          [
            ...
            'These config fixes were not written. Rerun "openagent doctor" to review repairs for the current config.',
          ].join("\n"),
          "Doctor warnings",
        );
      ...
      if (isConfigIncludeOwnershipError(error)) {
        ...
        note(
          [
            ...
            `${unpersistedLine} Repair ${error.ownedConfigPath} in ${includedFile} by hand, then rerun "openagent doctor --fix" for the remaining changes.`,
          ].join("\n"),
          "Doctor warnings",
        );
      ...
      if (isConfigValidationFailedError(error)) {
        ...
        note(
          [
            ...
            ...issueLines,
            `${unpersistedLine} Fix the value(s) above in ${shortenHomePath(ctx.configPath)} by hand, then rerun "openagent doctor --fix".`,
          ].join("\n"),
          "Doctor warnings",
        );
      ...
      note(
        [
          ...
          'Resolve the reported Gateway or cron-store condition, then rerun "openagent doctor --fix".',
        ].join("\n"),
        "Doctor warnings",
      );
...
export async function collectWriteConfigHealthFindings(
  ...
  if (resolveIsConfigReadOnly(process.env)) {
    findings.push({
      ...
      severity: "warning",
      message: isNixMode
        ? "Doctor config writes are disabled because OpenAgent is running in Nix mode."
