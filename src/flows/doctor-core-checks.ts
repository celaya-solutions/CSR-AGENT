  ...
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `openagent configure` and set Gateway mode (local/remote), or `openagent config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `openagent config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `openagent config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};
    ...
    if (
      ...
      fixHints.push(
        ctx.mode === "doctor"
          ...
          : "Run `openagent doctor --fix` to process eligible Workshop relocations and legacy collection backups.",
      );
    }
...
function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `openagent doctor --allow-exec` to verify exec SecretRefs during doctor, or `openagent secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
  ...
  return [
    {
      ...
      path: "gateway.auth.token",
      fixHint:
        "Run `openagent doctor --fix --generate-gateway-token` to generate a token, then restart the Gateway.",
    },
  ];
}
  ...
  async detect(ctx) {
    ...
    return [
      ...detected.preview.map((line): HealthFinding => ({
        ...
        fixHint: "Run `openagent doctor --fix` to migrate legacy state.",
      })),
      ...detected.warnings.map((warning): HealthFinding => ({
        ...
        message: warning,
        path: detected.stateDir,
        fixHint: "Resolve the warning, then rerun `openagent doctor --fix`.",
      })),
    ];
  },
};
      ...
      return inspectConfiguredModelReferences({
        ...
        const migrationFinding = migrationTarget
          ? {
              ...
              fixHint: `Run \`openagent doctor --fix\` to migrate this model reference to "${migrationTarget}".`,
            }
          : undefined;
      ...
      (issue): HealthFinding => ({
        ...
        fixHint: issue.repairBlocked
          ? [
              ...
              "or set the affected OpenAI models to an OpenAgent runtime policy.",
            ].join(" ")
          : [
              "Run `openagent doctor --fix`: it enables plugins.entries.codex,",
              "or set the affected OpenAI models to an OpenAgent runtime policy.",
            ].join(" "),
      }),
    );
...
const telegramGeneralTopicConversationsCheck: HealthCheck = {
  ...
  async detect(ctx) {
    ...
    return repairs.map((repair) => ({
      ...
      fixHint: "Run `openagent doctor --fix` to merge the stale topic-qualified identity.",
    }));
  },
  async repair(ctx) {
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `openagent doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyClawdBrowserProfileResidue): HealthFinding {
  return {
    ...
    fixHint:
      "Run `openagent doctor --fix` to archive the stale clawd profile safely instead of deleting it in place.",
  };
}
...
const browserClawdProfileResidueCheck: HealthCheck = {
  ...
  kind: "core",
  description:
    "Legacy clawd managed browser profile residue has been archived after the OpenAgent rename.",
  source: "doctor",
  async detect(ctx, scope) {
