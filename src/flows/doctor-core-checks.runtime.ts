    ...
    if (
      ...
      return [
        warning(
          ...
          "Rerun `openagent doctor --lint --only core/doctor/gateway-health --allow-exec` to permit configured secret execution.",
        ),
      ];
    }
    ...
    if (!probeDetails) {
      return [
        warning(
          ...
          "Fix Gateway connection configuration, then rerun `openagent doctor --lint --only core/doctor/gateway-health`.",
        ),
      ];
    }
        ...
        : {
            ...
            fixHint:
              mode === "remote"
                ...
                : "Inspect the service with `openagent gateway status --deep`, or run `openagent doctor` for guided checks.",
          };
...
export async function collectGatewayDaemonFindings(
  ...
  if (state.loadState.status === "unknown") {
    findings.push({
      ...
      fixHint: "Run `openagent gateway status --deep`, restore service-manager access, and retry.",
    });
    return findings;
  }
  if (!state.installed) {
    findings.push({
      ...
      path: "gateway.mode",
      target: service.label,
      fixHint: "Run `openagent gateway install` to install the service.",
    });
    return findings;
  }
          ...
          ? {
              fixHint: [
                ...(runtime.status === "unsupported"
                  ...
                  : []),
                "Repair the Node runtime, then run `openagent gateway install`.",
              ].join("\n"),
            }
          : {}),
      });
  ...
  if (state.loadState.status === "not-loaded") {
    findings.push({
      ...
      path: state.command?.sourcePath,
      target: service.label,
      fixHint: "Start the installed service with `openagent gateway start`.",
    });
  }
  ...
  if (state.loadState.status === "loaded" && !state.running) {
    findings.push({
      ...
      target: service.label,
      fixHint:
        "Run `openagent gateway status --deep` to inspect the service before choosing a recovery action.",
    });
  }
