/**
 * Shared realtime voice controls for active Zero to Agent agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_FAILURE_MESSAGE =
  "Zero to Agent could not process that voice control. Please try again.";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal Zero to Agent voice control result.",
    "Do not delegate this message or call any tools.",
    "Speak this exact Zero to Agent status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "Zero to Agent is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "Zero to Agent has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "Zero to Agent could not queue that follow-up."
    : "Zero to Agent could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "Zero to Agent finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `Zero to Agent is starting ${name}.` : "Zero to Agent is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `Zero to Agent finished ${name} and is continuing.`
        : "Zero to Agent finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `Zero to Agent is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "Zero to Agent is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `Zero to Agent is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "Zero to Agent is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "Zero to Agent is working on the current voice request.";
  }
