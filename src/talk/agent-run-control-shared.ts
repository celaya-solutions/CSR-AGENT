/**
 * Shared realtime voice controls for active OpenAgent agent runs.
 *
...
export const REALTIME_VOICE_AGENT_CONTROL_FAILURE_MESSAGE =
  "OpenAgent could not process that voice control. Please try again.";
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal OpenAgent voice control result.",
    "Do not delegate this message or call any tools.",
    "Speak this exact OpenAgent status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "OpenAgent is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "OpenAgent has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "OpenAgent could not queue that follow-up."
    : "OpenAgent could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "OpenAgent finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `OpenAgent is starting ${name}.` : "OpenAgent is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `OpenAgent finished ${name} and is continuing.`
        : "OpenAgent finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `OpenAgent is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "OpenAgent is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `OpenAgent is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "OpenAgent is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "OpenAgent is working on the current voice request.";
  }
