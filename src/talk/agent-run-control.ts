/**
 * Runtime adapter for realtime voice control of active Zero to Agent agent runs.
 *
...
export async function controlRealtimeVoiceAgentRun(
  ...
  const noActiveRun = (): RealtimeVoiceAgentControlResult => ({
    ...
    message: `There is no active Zero to Agent run to ${mode === "cancel" ? "cancel" : "steer"}.`,
    ...controlResultPresentation,
  });
  ...
  if (mode === "cancel") {
    ...
    const message = aborted
      ...
      : "Zero to Agent could not cancel the active run.";
    return {
      ok: aborted,
      ...controlResultContext,
      sessionId,
  ...
  const message = unconfirmed
    ? "Zero to Agent could not confirm that input. It was not sent again; check the conversation before retrying."
    : mode === "followup"
      ? "Queued that follow-up for the active Zero to Agent run."
