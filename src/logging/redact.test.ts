...
describe("redactSensitiveText", () => {
  ...
  it("masks named Gateway security headers", () => {
    ...
    const input = [
      `X-OpenAgent-Token: ${openClawToken}`,
      `x-pomerium-jwt-assertion: ${pomeriumJwt}`,
      `X-Api-Key=${apiKey}`,
    ].join("\n");
    const output = redactSensitiveText(input, { mode: "tools" });

    expect(output).toContain("X-OpenAgent-Token: supers…7890");
  ...
  it("masks URL punctuation inside named Gateway header values", () => {
    ...
    expect(
      redactSensitiveText("X-OpenAgent-Token=prefix&actual-secret#tail", { mode: "tools" }),
    ).toBe("X-OpenAgent-Token=prefix…tail");
  ...
  it("keeps equals-assignment bitmap masking aligned with form parsing", () => {
    ...
    const header = "X-OpenAgent-Token=prefix&actual-secret#tail";
