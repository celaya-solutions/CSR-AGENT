...
/** Stable finding codes emitted by `openagent secrets audit`. */
  ...
  for (const target of targets) {
    ...
    for (const source of listLegacyAuthProfileSources({ agentDir, env: params.env })) {
      ...
      addFinding(params.collector, {
        ...
        file: source.path,
        jsonPath: "<root>",
        message: `Retired auth source ${source.kind} is present; run openagent doctor --fix to migrate and archive it.`,
      });
    }
  }
