...
export async function applyPluginNodeInvokePolicy(params: {
  ...
  if (!nodeCommandDispatched && !nodeGateDecisionRecorded && isCallerRuntimeAuthorityActive()) {
    recordNodeDecision({
      ...
      summary: result.ok
        ? "The plugin policy returned without invoking the expected Zero to Agent node callback."
        ...
        ? [
            {
              ...
              text: "Route the native action through the provided Zero to Agent node callback.",
            },
          ]
        : [],
    });
