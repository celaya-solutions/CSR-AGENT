...
export async function applyPluginNodeInvokePolicy(params: {
  ...
  if (!nodeCommandDispatched && !nodeGateDecisionRecorded && isCallerRuntimeAuthorityActive()) {
    recordNodeDecision({
      ...
      summary: result.ok
        ? "The plugin policy returned without invoking the expected OpenAgent node callback."
        ...
        ? [
            {
              ...
              text: "Route the native action through the provided OpenAgent node callback.",
            },
          ]
        : [],
    });
