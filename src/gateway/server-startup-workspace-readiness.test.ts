  ...
  it.each(["managed write", "file watcher"])(
    ...
    async (ingress) => {
      ...
      if (ingress === "managed write") {
        ...
        await expect(writeConfigFile(nextConfig)).rejects.toThrow("openagent doctor --fix");
        ...
        await expect
          ...
          .toBe(true);
        expect(reloadError).toHaveBeenCalledWith(expect.stringContaining("openagent doctor --fix"));
