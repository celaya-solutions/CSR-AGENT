...
async function verifyCodexCodeModeOnlyDynamicToolProbe(params: {
  ...
  const { text, events } = await requestAgentTextWithEvents({
    ...
    message: [
      ...
      "Before replying, call the Zero to Agent sessions_list tool exactly once.",
...
async function verifyCodexGuardianProbe(params: {
  ...
  if (allowStatus === "denied") {
    ...
    // The Zero to Agent contract is that the review completes and the agent receives
  ...
  // Zero to Agent's strict projection contract is covered by the allow probe above.
...
async function verifyCodexSessionDeletion(params: {
  ...
  await requestCodexCommandText({
    ...
    expectedText: "owned by another Zero to Agent session or conversation",
  });
  ...
  // Session deletion releases Zero to Agent ownership, not the native Codex history.
  ...
  const attached = await requestCodexCommandText({
    ...
    expectedText: "Attached this Zero to Agent session",
  });
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
          try {
            ...
            try {
              ...
              // `/new` deliberately retains the physical Zero to Agent session id. Prove the
