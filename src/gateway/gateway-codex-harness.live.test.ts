...
async function verifyCodexCodeModeOnlyDynamicToolProbe(params: {
  ...
  const { text, events } = await requestAgentTextWithEvents({
    ...
    message: [
      ...
      "Before replying, call the OpenAgent sessions_list tool exactly once.",
...
async function verifyCodexGuardianProbe(params: {
  ...
  if (allowStatus === "denied") {
    ...
    // The OpenAgent contract is that the review completes and the agent receives
  ...
  // OpenAgent's strict projection contract is covered by the allow probe above.
...
async function verifyCodexSessionDeletion(params: {
  ...
  await requestCodexCommandText({
    ...
    expectedText: "owned by another OpenAgent session or conversation",
  });
  ...
  // Session deletion releases OpenAgent ownership, not the native Codex history.
  ...
  const attached = await requestCodexCommandText({
    ...
    expectedText: "Attached this OpenAgent session",
  });
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
          try {
            ...
            try {
              ...
              // `/new` deliberately retains the physical OpenAgent session id. Prove the
