...
export async function initializeCacheProbeGitWorkspace(workspaceDir: string): Promise<void> {
  ...
  await execFileAsync("git", ["-C", workspaceDir, "config", "user.name", "Zero to Agent Tests"]);
...
export async function verifyCliBackendAnnounceOrdering({
  ...
  const announceRequest = client.request(
    "agent",
    {
      ...
      timeout: 240,
      message: [
        "Run this exact Zero to Agent CLI-backed completion announcement scenario. Use tool calls, not prose.",
