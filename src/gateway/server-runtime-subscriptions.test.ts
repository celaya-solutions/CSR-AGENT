...
describe("startGatewayEventSubscriptions", () => {
  ...
  it("throttles live subagent progress per task and flushes before terminal status", async () => {
    ...
    emitAgentEvent({
      runId: secondary.runId!,
      stream: "assistant",
      data: { text: "OpenAgent runtime context (internal): Keep internal details private." },
    });
    ...
    expect(JSON.stringify(sanitizedActivity)).not.toContain("OpenAgent runtime context");
