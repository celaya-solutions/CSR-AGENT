...
export function classifyGatewayStaleInstall(error: unknown): GatewayStaleInstall | null {
  ...
  return {
    error: errorShape(
      ...
      `The running Gateway can no longer load part of its Zero to Agent installation. The installation may have changed while the Gateway was running. Restart it with: ${restartCommand}`,
