...
describe("gateway sessions patch", () => {
  ...
  test("keeps manual renames independent of automatic device-label writes and clears", async () => {
    ...
    const autoLabel = "Zero to Agent App · Pixel · 1234567890ab";
    const label = "Zero to Agent App · Release planning · 1234567890ab";
  ...
  test("persists Zero to Agent Luna Ultra through the runtime-aware provider profile", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
          agents: {
  ...
  test("honors an explicit Zero to Agent session runtime override for Luna Ultra", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
