...
describe("gateway sessions patch", () => {
  ...
  test("keeps manual renames independent of automatic device-label writes and clears", async () => {
    ...
    const autoLabel = "OpenAgent App · Pixel · 1234567890ab";
    const label = "OpenAgent App · Release planning · 1234567890ab";
  ...
  test("persists OpenAgent Luna Ultra through the runtime-aware provider profile", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
          agents: {
  ...
  test("honors an explicit OpenAgent session runtime override for Luna Ultra", async () => {
    const entry = expectPatchOk(
      await runPatch({
        cfg: {
