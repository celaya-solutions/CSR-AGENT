...
function resolvePackageRoot(packageRoot: string | undefined): string {
  ...
  if (!resolved) {
    throw new Error("Unable to locate the running OpenAgent package root for worker bundling");
  }
  return resolved;
}
...
async function verifyPublishedNpmRelease(params: {
  ...
  try {
    const published = parseNpmPackageIdentity(
      unwrapNpmJsonEntry(
        await runNpmProofCommand({
          ...
          cwd: temporaryRoot,
          failureMessage: `OpenAgent ${params.version} is not published; use the worker bundle install`,
          runCommand,
        }),
      ),
    );
    ...
    const packedValue = await runNpmProofCommand({
      ...
      cwd: temporaryRoot,
      failureMessage:
        "Unable to verify the installed OpenAgent package; use the worker bundle install",
      runCommand,
    });
    ...
    if (
      ...
      throw new Error(
        `Installed OpenAgent ${params.version} does not match the published package; use the worker bundle install`,
      );
    }
    ...
    if (packedBundle.bundleHash !== params.bundleHash) {
      throw new Error(
        `Published OpenAgent ${params.version} does not match the prepared worker bundle; use the worker bundle install`,
      );
    }
...
async function prepareWorkerBundle(
  ...
  if (!openclawVersion) {
    throw new Error("Worker bundle requires a non-empty OpenAgent version");
  }
