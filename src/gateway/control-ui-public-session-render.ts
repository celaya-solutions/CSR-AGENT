...
export function renderPublicSessionDocument(params: {
  ...
  for (const value of params.messages.slice(-MAX_MESSAGES).toReversed()) {
    ...
    rows.push(
      `<article class="message ${message.role}" aria-label="${message.role === "user" ? "User" : "Assistant"} message"><h2>${message.role === "user" ? "User" : "OpenAgent"}</h2><div class="content">${markdown.render(text)}${clipped ? '<p class="omitted">Message shortened for this public view.</p>' : ""}</div></article>`,
    );
  }
  const description = "A public, read-only OpenAgent conversation. No login required.";
...
<title>${title} · OpenAgent</title>${canonicalMetadata}
<meta property="og:type" content="website"><meta property="og:site_name" content="OpenAgent">
...
<header><div class="masthead"><div class="brand"><span aria-hidden="true">✳</span>OpenAgent</div><span class="badge">Public · Read-only</span></div>
