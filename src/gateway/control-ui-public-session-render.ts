...
export function renderPublicSessionDocument(params: {
  ...
  for (const value of params.messages.slice(-MAX_MESSAGES).toReversed()) {
    ...
    rows.push(
      `<article class="message ${message.role}" aria-label="${message.role === "user" ? "User" : "Assistant"} message"><h2>${message.role === "user" ? "User" : "Zero to Agent"}</h2><div class="content">${markdown.render(text)}${clipped ? '<p class="omitted">Message shortened for this public view.</p>' : ""}</div></article>`,
    );
  }
  const description = "A public, read-only Zero to Agent conversation. No login required.";
...
<title>${title} · Zero to Agent</title>${canonicalMetadata}
<meta property="og:type" content="website"><meta property="og:site_name" content="Zero to Agent">
...
<header><div class="masthead"><div class="brand"><span aria-hidden="true">✳</span>Zero to Agent</div><span class="badge">Public · Read-only</span></div>
