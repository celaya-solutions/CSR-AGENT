...
export async function runSystemAgentGatewayTask<T>(task: () => Promise<T>): Promise<T> {
  ...
  // active ids, so a queued Zero to Agent request could otherwise outlive its socket.
  ...
  return await enqueueCommandInLane(CommandLane.SystemAgent, () =>
    ...
    // setup writes atomic with respect to other Zero to Agent gateway requests.
