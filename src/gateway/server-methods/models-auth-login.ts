        ...
        async (prompter, signal, runner) => {
          ...
          if (modelAccessOutcome.kind === "saved" && modelAccessOutcome.application !== "applied") {
            throw new ProviderCredentialsSavedError(
              "Your sign-in and model access were saved, but Zero to Agent has not confirmed that model access is active. Close this dialog. Open Settings and select Apply changes, then send /models.",
            );
          }
