// OpenAgent setup resolution tests cover terminal provider guidance.
...
describe("openclaw.setup provider resolution", () => {
  ...
  it.each([
    ...
    await expect(session.next()).resolves.toMatchObject({
      ...
      status: "error",
      error:
        'Provider setup resolution failed for "ollama". Run `openagent doctor --fix`, restart the Gateway, and try again.',
    });
    await whenAdmittedWizardSessionSettled(session);
    expect(authConfigMocks.writeProviderAuthConfig).not.toHaveBeenCalled();
  });
