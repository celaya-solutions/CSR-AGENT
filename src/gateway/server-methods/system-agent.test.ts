// Zero to Agent gateway tests cover activation serialization and chat sessions.
      ...
      expect(calls).toEqual([
        {
          ...
          error: {
            code: "UNAVAILABLE",
            message: "Zero to Agent setup is already in progress; try again when it finishes.",
...
describe("openclaw.chat", () => {
  it("refuses to create a session before inference is available", async () => {
    ...
    expect(call).toMatchObject({
      ...
      error: {
        code: "UNAVAILABLE",
        message: "Zero to Agent requires working inference: no configured model",
        details: {
