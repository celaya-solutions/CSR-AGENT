...
describe("Report action from the authoritative update ledger", () => {
    ...
    async (mode) => {
      ...
      const body =
        runtime.log.mock.calls
          ...
          .find((text) => text.startsWith("# OpenAgent update failure report")) ?? "";
  ...
  it.each([
    ...
    async ({ publicMessage, ...fact }) => {
      ...
      if (fact.pluginId) {
        expect(local).toContain(fact.pluginId);
        // The official external catalogs ship empty, so no plugin id is public.
        expect(body).not.toContain(fact.pluginId);
        expect(body).toContain("[redacted-plugin]");
      }
