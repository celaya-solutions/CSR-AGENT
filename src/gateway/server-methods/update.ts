    ...
    const refuseUnauthorizedChatUpdate = () => {
      ...
      const message =
        reason === "owner_required"
          ? `Only the Zero to Agent owner can start an update from chat. ${formatCommandOwnerHint({ cfg: currentConfig, channel: requester.channel, id: requester.senderId })}`
          : "Updates from chat are disabled (commands.restart=false). Use the Control UI or ask the Gateway operator to update Zero to Agent.";
