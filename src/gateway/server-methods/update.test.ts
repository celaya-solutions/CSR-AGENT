...
describe("update.run acknowledgement", () => {
    ...
    async (managed) => {
      ...
      expect(sendGatewayLifecycleNoticeMock).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          message: `⬆️ Updating Zero to Agent 1.0.0 → ${managed ? "2.0.0" : "the latest release"}. The gateway stays available while the update is validated; you'll get a message here when it finishes.`,
  ...
  it("merges explicit route fields and reports a synchronous failure after acknowledgement", async () => {
    ...
    expect(sendGatewayLifecycleNoticeMock).toHaveBeenLastCalledWith(
      expect.objectContaining({
        to: "C0456DEF",
        message: expect.stringContaining("⚠️ Zero to Agent update failed: build-failed."),
      }),
    );
  });
      ...
      expect(sendGatewayLifecycleNoticeMock).toHaveBeenLastCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            "Zero to Agent update failed: managed-service-handoff-failed",
          ),
        }),
      );
    },
