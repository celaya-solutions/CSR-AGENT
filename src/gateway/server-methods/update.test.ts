...
describe("update.run acknowledgement", () => {
    ...
    async (managed) => {
      ...
      expect(sendGatewayLifecycleNoticeMock).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          message: `⬆️ Updating OpenAgent 1.0.0 → ${managed ? "2.0.0" : "the latest release"}. The gateway stays available while the update is validated; you'll get a message here when it finishes.`,
  ...
  it("merges explicit route fields and reports a synchronous failure after acknowledgement", async () => {
    ...
    expect(sendGatewayLifecycleNoticeMock).toHaveBeenLastCalledWith(
      expect.objectContaining({
        to: "C0456DEF",
        message: expect.stringContaining("⚠️ OpenAgent update failed: build-failed."),
      }),
    );
  });
...
describe("update.run restart scheduling", () => {
  ...
  it("persists managed update continuation before transferring validation while serving", async () => {
    ...
    expect(payload?.handoff).toEqual({
      status: "started",
      pid: 12345,
      command: "openagent update --yes --timeout 1800",
    });
  ...
  it("keeps unmanaged restart ownership and records failed notice persistence", async () => {
    ...
    expect(summarizeUpdateRunResponse(payload).next).toContain(
      "Run openagent update status after the gateway restarts.",
    );
  });
      ...
      expect(sendGatewayLifecycleNoticeMock).toHaveBeenLastCalledWith(
        expect.objectContaining({
          message: expect.stringContaining(
            "OpenAgent update failed: managed-service-handoff-failed",
          ),
        }),
      );
    },
      ...
      steps: [
        {
          ...
          command: "openagent config validate --json",
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        ...
        "Stop the foreground Gateway, run `openagent update --yes --timeout 1800` from a shell, then launch the Gateway again. For a managed deployment, use its host's stop, update, and restart workflow.",
    });
  });
