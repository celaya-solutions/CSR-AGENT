...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `openagent doctor` for diagnostics.";
  }
  return message;
}
    ...
    if (!storedAuth?.token && deviceAuthScope) {
      throw new GatewayStoredDeviceAuthUnavailableError(
        [
          ...
          `Run \`openagent tui --url ${deviceAuthScope}\` to send a pairing request, approve it in that gateway's Control UI (Settings -> Devices) or run \`openagent devices approve --latest\` on the gateway host, then retry.`,
        ].join("\n"),
      );
    }
  }
