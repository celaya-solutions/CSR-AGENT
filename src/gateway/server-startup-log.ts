...
function formatSuppressedAmbientChannelsStartupWarning(channelIds: readonly string[]): string {
  ...
  return (
    ...
    "Configure channels.<id> (openagent channels add <id>) to enable the channel, or pass " +
...
function formatConfiguredChannelMissingOwnerStartupWarning(entry: {
  ...
  return (
    ...
    `is installed or loadable (${reasons}). Run \`openagent doctor --fix\` or install the ` +
