...
const failure: RestartSentinelPayload = {
  ...
  stats: {
    ...
    steps: [
      { name: "doctor", command: "openagent doctor --fix", durationMs: 10, log: { exitCode: 1 } },
    ],
    durationMs: 20,
  ...
  beforeEach(async () => {
    ...
    mocks.submitGithubIssue.mockImplementation(
      async (_issue: unknown, _runGh: unknown, hooks: GithubIssueSubmitHooks) => {
        ...
        return {
          status: "created",
          url: "https://github.com/celaya-solutions/CSR-AGENT/issues/999999",
        };
      },
    );
  });
    ...
    async ({ authority, retire, boundary }) => {
      ...
      const runGh = vi.fn<RunGithubCli>(async (args) => {
        ...
        return {
          ...
          status: 0,
          stdout: Buffer.from(
            "HTTP/2.0 201 Created\nhttps://github.com/celaya-solutions/CSR-AGENT/issues/999999\n",
          ),
        };
      });
