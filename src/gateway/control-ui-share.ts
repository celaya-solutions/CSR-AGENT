...
export function serveControlUiShareDocument(
  ...
  const title =
    session.namespace === "dashboard" ? "Zero to Agent dashboard" : "Zero to Agent session";
  const description = "Open this shared link in Zero to Agent. Access to the session is required.";
...
<meta property="og:image:alt" content="Zero to Agent logo and lobster mascot">
...
<img src="${image}" width="1200" height="630" alt="Zero to Agent logo and lobster mascot">
<h1>${title}</h1><p>${description}</p><a href="${target}">Open ${title === "Zero to Agent dashboard" ? "dashboard" : "session"}</a>
</main></body></html>`;
  res.statusCode = 200;
