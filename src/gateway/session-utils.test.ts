...
describe("gateway session utils", () => {
  ...
  test.each([
    {
      name: "a locked Codex session under OpenAgent config",
  ...
  test.each([true, false])(
    "does not reuse stale transcript context after an OpenAgent to Codex change (lightweight=%s)",
    async (lightweightListRow) => {
  ...
  test("buildGatewaySessionRow prefers generated titles over Android node stamps", () => {
    ...
    const stamp = "OpenAgent App · Pixel · 1234567890ab";
    const entry = {
      sessionId: "node-1",
      updatedAt: 1,
      autoLabel: stamp,
    ...
    const manualPrefix = {
      ...entry,
      label: "OpenAgent App · Release planning · 1234567890ab",
    } as SessionEntry;
    ...
    expect(manualRow.displayName).toBe("OpenAgent App · Release planning · 1234567890ab");
  });
