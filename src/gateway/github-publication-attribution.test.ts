...
describe("Gateway GitHub publication attribution", () => {
  ...
  it("publishes canonical ordered credit and exactly one final team-session backlink", async () => {
    ...
    const result = await coordinator.requestForSession({
      ...
      body: "Detailed proof\n\n## Worked on by\n\n- @untrusted\n\n### Verification notes\n\nKeep this paragraph.\n\n---\n[View the OpenAgent team session](https://untrusted.example/session)",
    });
    ...
    expect(JSON.parse(post?.input ?? "null")).toEqual({
      title: "fix: publish the reconciled fix",
      body: `Detailed proof\n\n### Verification notes\n\nKeep this paragraph.\n\n## Worked on by\n\n- @alice\n- @grace\n\n<!-- openclaw-publication:${result.requestId} -->\n\n---\n[View the OpenAgent team session](https://team.example/control/chat/main/dashboard/publication)`,
