...
function buildForcedConsultCheckingPrompt(): string {
  return [
    "Briefly tell the person that you are checking with Zero to Agent.",
    "Do not answer the request yet. Wait for the Zero to Agent result before giving the actual answer.",
  ].join(" ");
}

function buildForcedConsultSpeechPrompt(text: string): string {
  return [
    "Zero to Agent finished checking. Speak this result naturally and concisely.",
...
export function buildAlreadyDeliveredToolResult(): Record<string, string> {
  return {
    ...
    message: "Zero to Agent already delivered this consult result internally. Do not repeat it.",
  };
}
  ...
  session.harness.forcedConsults.schedule(handle, FORCED_CONSULT_FALLBACK_DELAY_MS, () => {
    ...
    broadcastToOwner(session.context, session.connId, {
      ...
      args: {
        ...
        context:
          "The realtime provider produced a final user transcript without invoking openclaw_agent_consult, so Zero to Agent is forcing the consult for realtime Talk.",
