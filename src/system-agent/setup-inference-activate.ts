  ...
  if (params.surface === "gateway" && params.recordSetupAudit !== false) {
    ...
    try {
      await appendSystemAgentAuditEntry({
        ...
        summary: "Verified and configured AI access through OpenAgent setup",
      ...
      const warning = `Inference setup completed, but OpenAgent could not record its audit entry: ${formatErrorMessage(error)}`;
