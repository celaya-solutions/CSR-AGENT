  ...
  if (params.surface === "gateway" && params.recordSetupAudit !== false) {
    ...
    try {
      await appendSystemAgentAuditEntry({
        ...
        summary: "Verified and configured AI access through Zero to Agent setup",
      ...
      const warning = `Inference setup completed, but Zero to Agent could not record its audit entry: ${formatErrorMessage(error)}`;
