...
describe("SystemAgentChatEngine runtime", () => {
  ...
  it("keeps remote Gateway mode guidance-only", async () => {
    ...
    expect(reply.text).toContain("`openagent onboard` for fresh setup");
    expect(reply.text).toContain("`openagent configure` for the mode question");
  ...
  it("hands CLI Gateway credentials to the masked terminal wizard", async () => {
    ...
    expect(stopped.text).toContain("openagent configure --section gateway");
  ...
  it("does not promise Doctor will repair every invalid channel setup config", async () => {
    ...
    expect(reply.text).toContain("openagent doctor --fix");
