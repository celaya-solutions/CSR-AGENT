// Zero to Agent operation grammar, approval descriptions, and public types.
...
/**
 * Parse one user command into Zero to Agent's closed operation union. Anything
...
export function parseSystemAgentOperation(input: string): SystemAgentOperation {
  ...
  switch (lower) {
    ...
    case "quit":
    case "exit":
      return { kind: "none", message: "Zero to Agent retracts into shell. Bye." };
    default:
      break;
  }
...
export function describeSystemAgentPersistentOperation(operation: SystemAgentOperation): string {
  switch (operation.kind) {
    ...
    case "doctor-fix":
      return "run openclaw doctor --fix on the machine running Zero to Agent, with Zero to Agent stopped";
    case "plugin-install":
...
export const SYSTEM_AGENT_OPERATOR_NAVIGATION_HANDOFF =
  "Channel, model, and setup flows need a human operator in the Zero to Agent app; they cannot run from a delegated agent request. Open `openclaw dashboard` or run `openclaw setup` on the Gateway host.";
...
function formatSetupPlanDescription(
  ...
  return `bootstrap Zero to Agent setup for workspace ${workspace}`;
}
