// OpenAgent operation grammar, approval descriptions, and public types.
...
/**
 * Parse one user command into OpenAgent's closed operation union. Anything
...
export function parseSystemAgentOperation(input: string): SystemAgentOperation {
  ...
  switch (lower) {
    ...
    case "quit":
    case "exit":
      return { kind: "none", message: "OpenAgent retracts into shell. Bye." };
    default:
      break;
  }
...
function formatSetupPlanDescription(
  ...
  return `bootstrap OpenAgent setup for workspace ${workspace}`;
}
