// OpenAgent rescue messages expose approved setup-helper commands over message channels.
...
/**
 * Message-channel rescue command handling for OpenAgent.
 *
...
function createCaptureRuntime(): { runtime: RuntimeEnv; read: () => string } {
  ...
  return {
    runtime: {
      ...
      error: push,
      exit: (code) => {
        throw new Error(`OpenAgent operation exited with code ${code}`);
      },
    },
    read: () => lines.join("\n").trim(),
  };
...
function formatUnsupportedRemoteOperation(operation: SystemAgentOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      "OpenAgent rescue cannot open the local TUI from a message channel.",
...
export async function runSystemAgentRescueMessage(
  ...
  if (approvalIntent === "approve") {
    ...
    if (!operation) {
      return "No pending OpenAgent rescue change is waiting for approval.";
    }
    ...
    return capture.read() || "OpenAgent rescue change applied.";
  }
  ...
  if (isPersistentSystemAgentOperation(operation)) {
    ...
    if (nowMs === undefined || expiresAtMs === undefined) {
      return "OpenAgent rescue could not create a pending approval because the expiry clock is invalid.";
    }
  ...
  return capture.read() || "OpenAgent listened, clicked a claw, and found nothing to change.";
}
