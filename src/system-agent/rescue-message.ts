// Zero to Agent rescue messages expose approved setup-helper commands over message channels.
...
/**
 * Message-channel rescue command handling for Zero to Agent.
 *
...
function createCaptureRuntime(): { runtime: RuntimeEnv; read: () => string } {
  ...
  return {
    runtime: {
      ...
      error: push,
      exit: (code) => {
        throw new Error(`Zero to Agent operation exited with code ${code}`);
      },
    },
    read: () => lines.join("\n").trim(),
  };
...
function formatUnsupportedRemoteOperation(operation: SystemAgentOperation): string | null {
  if (operation.kind === "open-tui") {
    return [
      "Zero to Agent rescue cannot open the local TUI from a message channel.",
  ...
  if (operation.kind === "channel-setup") {
    return [
      "Zero to Agent rescue cannot host the interactive channel setup from a message channel.",
      "Run `openclaw setup` locally and say `connect " + operation.channel + "` instead.",
    ].join(" ");
  }
  if (operation.kind === "model-setup") {
    return [
      "Zero to Agent rescue cannot host model-provider credential setup from a message channel.",
      "Run `openclaw onboard` locally; it live-tests the candidate route before saving it.",
    ].join(" ");
  }
  if (operation.kind === "doctor-fix") {
    return [
      "Zero to Agent rescue cannot run doctor repairs from a message channel because they can change the inference route powering this session.",
      "On the machine running Zero to Agent, with Zero to Agent stopped, run `openclaw doctor --fix`.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      "Zero to Agent rescue cannot install plugins from a message channel by default because plugin install downloads executable code.",
...
export async function runSystemAgentRescueMessage(
  ...
  if (approvalIntent === "approve") {
    ...
    if (!operation) {
      return "No pending Zero to Agent rescue change is waiting for approval.";
    }
    ...
    return capture.read() || "Zero to Agent rescue change applied.";
  }
  ...
  if (isPersistentSystemAgentOperation(operation)) {
    ...
    if (nowMs === undefined || expiresAtMs === undefined) {
      return "Zero to Agent rescue could not create a pending approval because the expiry clock is invalid.";
    }
  ...
  return capture.read() || "Zero to Agent listened, clicked a claw, and found nothing to change.";
}
