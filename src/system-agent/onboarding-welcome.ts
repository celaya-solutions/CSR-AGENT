...
export async function buildOnboardingWelcome(params: {
  ...
  if (!defaultModel) {
    throw new Error(
      "Zero to Agent onboarding requires working inference first. Run `openclaw onboard` on the machine running Zero to Agent to configure and verify a default model.",
    );
  }
  ...
  const welcome = [
    "## Hi, I'm Zero to Agent — let's hatch your agent.",
    "",
