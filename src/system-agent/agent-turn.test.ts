...
describe("runSystemAgentTurn", () => {
  ...
  it("uses the default agent CLI route while keeping OpenAgent session identity", async () => {
    ...
    expect(requireValue(call.systemAgentTool, "missing CLI OpenAgent tool").proposalRef).toBe(
      session.proposalRef,
    );
  });

  it("rejects an always-on CLI backend before launching OpenAgent", async () => {
    useTempStateDir();
          ...
          list: [
            {
              id: "ops",
              default: true,
              // Keep the model owner's policy stable. OpenAgent executes with
  ...
  it("uses the default agent embedded model, auth directory, profile, and runtime", async () => {
    ...
    expect(requireValue(call.systemAgentTool, "missing embedded OpenAgent tool").proposalRef).toBe(
      session.proposalRef,
    );
  });
