...
export function formatSystemAgentOverview(overview: SystemAgentOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "OpenAgent online. Little claws, typed tools.",
    "",
    `Config: ${configStatus}`,
    ...
    `AI: ${
      overview.defaultModel
        ...
        : "inference unavailable; run openagent onboard before starting OpenAgent"
    }`,
...
function recommendSystemAgentNextStep(overview: SystemAgentOverview): string {
  if (!overview.config.exists) {
    return 'run "openagent onboard" to establish inference';
  }
...
function formatStartupAction(overview: SystemAgentOverview): string | undefined {
  ...
  if (!overview.defaultModel) {
    return "Inference is unavailable. Run `openagent onboard` and complete a live model check.";
  }
  return undefined;
}

/**
 * Welcome shown right after inference activation. OpenAgent owns the
...
export function formatSystemAgentStartupMessage(overview: SystemAgentOverview): string {
  ...
  return [
    "Hi, I'm OpenAgent — caretaker of this gateway, config, channels, and agents.",
