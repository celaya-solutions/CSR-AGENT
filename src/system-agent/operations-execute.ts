...
/** Execute a parsed OpenAgent operation after applying approval gates and audit logging. */
export async function executeSystemAgentOperation(
  ...
  switch (operation.kind) {
    ...
    case "channel-setup":
      // Channel setup is a multi-step wizard; only interactive OpenAgent (TUI
    ...
    case "create-agent": {
      ...
      if (operation.model?.trim()) {
        throw new Error(
          "OpenAgent cannot save an explicit per-agent model until that new route can be live-tested. Retry without `model`; the new agent inherits the verified default, then use `set_default_model` with agentId to live-test and save its own model.",
        );
      }
      return await applyPersistentOperation({
