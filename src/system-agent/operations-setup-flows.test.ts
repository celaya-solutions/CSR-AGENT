...
describe("system agent setup-flow operations", () => {
  ...
  it("prints one-shot setup pointers", async () => {
    ...
    expect(output).toContain("openagent onboard`");
    expect(output).toContain("openagent onboard --classic");
    expect(output).toContain("openagent channels add --channel slack");
    expect(output).toContain("openagent configure --section web");
    expect(output).toContain("openagent configure --section gateway");
  ...
  it("prints one-shot pointers for hosted skills, search, and Gateway setup", async () => {
    ...
    expect(lines.join("\n")).toContain("openagent configure --section skills");
    expect(lines.join("\n")).toContain("openagent configure --section web");
    expect(lines.join("\n")).toContain("openagent configure --section gateway");
  });
  ...
  it("routes one-shot model setup through the verified OpenAgent flow", async () => {
    ...
    expect(result.applied).toBe(false);
    expect(lines.join("\n")).toContain("Run `openagent onboard` on the machine running OpenAgent");
    expect(lines.join("\n")).toContain("Stop the OpenAgent host");
    expect(lines.join("\n")).toContain("restart the host");
    expect(lines.join("\n")).not.toContain("openagent configure --section model");
  });
  ...
  it("prints discovered channel metadata and sorted unknown-channel choices", async () => {
    ...
    expect(knownOutput).not.toContain("Docs:");
