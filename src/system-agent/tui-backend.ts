// OpenAgent TUI backend runs setup-helper dialogue inside the shared local TUI shell.
...
class SystemAgentTuiBackend implements TuiBackend {
  ...
  stop(): void {
    // The enclosing TUI owns terminal shutdown; OpenAgent has no transport to close.
  }
  ...
  async listAgents(): Promise<TuiAgentsList> {
    return {
      ...
      mainKey: "main",
      scope: "per-sender",
      agents: [{ id: SYSTEM_AGENT_ID, kind: "system", name: "OpenAgent" }],
    };
  }
  ...
  private emitFinal(runId: string, sessionKey: string, text: string): void {
    const assistant = message(
      "assistant",
      text || "OpenAgent listened and found nothing to change.",
    );
  ...
  private async respond(runId: string, sessionKey: string, text: string): Promise<void> {
    ...
    try {
      ...
      if ((reply.action === "open-tui" || reply.action === "open-setup") && reply.handoff) {
        // The outer loop owns interactive handoffs after the OpenAgent TUI exits.
...
export async function runSystemAgentTui(
  ...
  for (;;) {
    ...
    // replay it when OpenAgent re-enters the chat shell.
