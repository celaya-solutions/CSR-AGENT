// OpenAgent chat engine: stable transport-agnostic facade over turn and wizard owners.
import type {
...
/**
 * One conversation with OpenAgent, independent of transport. The facade owns
  ...
  async resolveOperatorApproval(
    ...
    const turn = this.turnQueue.then(async () => {
      ...
      if (reply && terminalStatus && !reply.applied) {
        reply.text = `OpenAgent change ${terminalStatus}. No change. Retry the request if it is still needed.`;
      }
      if (reply && decision === "allow-once" && !reply.applied) {
        reply.text += " Check the current settings and OpenAgent status before retrying.";
      }
      if (reply?.text) {
