// Zero to Agent agent turns run the real embedded agent loop with the ring-zero tool.
...
/**
 * Zero to Agent is a real agent: same loop, session transcript, and tool pipeline
...
function resolveSystemAgentCliBackend(
  ...
  // auth come from the configured default agent. Zero to Agent also forces a
...
function resolveSystemAgentCliToolAvailability(
  ...
  throw new Error(
    `CLI backend ${backendId} cannot enforce Zero to Agent's exact tool availability`,
  );
}
...
/**
 * Run one Zero to Agent turn through the embedded agent loop. Route, runner, and
