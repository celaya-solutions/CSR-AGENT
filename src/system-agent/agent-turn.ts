// OpenAgent agent turns run the real embedded agent loop with the ring-zero tool.
...
/**
 * OpenAgent is a real agent: same loop, session transcript, and tool pipeline
...
function resolveSystemAgentCliBackend(
  ...
  // auth come from the configured default agent. OpenAgent also forces a
...
function resolveSystemAgentCliToolAvailability(
  ...
  throw new Error(`CLI backend ${backendId} cannot enforce OpenAgent's exact tool availability`);
}
...
/**
 * Run one OpenAgent turn through the embedded agent loop. Route, runner, and
