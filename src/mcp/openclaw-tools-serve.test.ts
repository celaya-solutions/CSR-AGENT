// OpenAgent MCP tools tests cover core tool server startup and registration.
...
describe("OpenAgent tools MCP server", () => {
  ...
  it("reconstructs delegated proposal staging from env on the native CLI MCP tool", async () => {
    ...
    expect(text).not.toContain("OpenAgent operator UI");
