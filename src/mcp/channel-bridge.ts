...
/**
 * Runtime bridge between MCP tools and the OpenAgent Gateway channel APIs.
 *
...
export class OpenClawChannelBridge {
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClientCtor({
      ...
      clientDisplayName: "OpenAgent MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`openagent mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(
    ...
    try {
      ...
      process.stderr.write(`openagent mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `openagent mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
      return "failed";
    }
  ...
  private async dispatchGatewayEvent(event: EventFrame): Promise<void> {
    try {
      ...
      process.stderr.write(`openagent mcp: gateway event ${event.event} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `openagent mcp: gateway event ${event.event} error: ${String(error)}\n`,
        );
      }
    }
  }
