...
export function resolveToolsMcpAgentId(
  ...
  if (!value || value.startsWith("--") || argv.includes("--openclaw-agent-id", index + 1)) {
    throw new Error("--openclaw-agent-id requires one OpenAgent agent owner");
  }
...
export function resolveToolsMcpSessionContext(params: {
  ...
  if (
    ...
    throw new Error(
      `${OPENCLAW_TOOLS_MCP_AGENT_SESSION_KEY_ENV} must be a canonical agent session key or have a matching explicit OpenAgent owner`,
    );
  }
