...
const COMPACT_OMITTED_NOTICE =
  "⚠️ Skills catalog using compact format (descriptions omitted). Run `openagent skills check` to audit.";
const COMPACT_SHORTENED_NOTICE =
  "⚠️ Skills catalog using compact format (descriptions shortened). Run `openagent skills check` to audit.";
...
describe("applySkillsPromptLimits (via buildWorkspaceSkillsPrompt)", () => {
  ...
  it("count truncation + compact: shows included X of Y with compact note", async () => {
    ...
    const truncatedNotice =
      "⚠️ Skills truncated: included 10 of 30 (compact format, descriptions shortened). Run `openagent skills check` to audit.";
      ...
      const notice =
        format === "compact"
          ...
          : format === "count-limited"
            ? "⚠️ Skills truncated: included 1 of 2. Run `openagent skills check` to audit."
            : "";
      const catalog =
