...
export function resolveSkillWorkshopToolConstructionBlock(
  ...
  if (context.sandboxed && !context.libraryAuthoring) {
    return {
      ...
      fix: "Use a non-sandboxed session for this agent, or a human turn with host-granted library-authoring authority. The openagent skills workshop CLI is also available.",
    };
  }
  return undefined;
}
