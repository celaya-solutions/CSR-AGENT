...
describe("skills-clawhub", () => {
  ...
  it.each([
    {
      ...
      expected:
        'Skill "missing-skill" not found on ClawHub. Run `openagent skills search missing-skill` to find the right skill reference.',
    },
    {
  ...
  it("formats ambiguous ClawHub slug responses with owner-qualified guidance", async () => {
    ...
    expect(result.error).toContain("openagent skills install @owner/weather");
    ...
    expect(refused).toEqual([
      {
        ...
        code: "force_required",
        error:
          'Skill "weather" was installed before OpenAgent recorded file fingerprints, so local changes cannot be detected. Updating replaces the installed skill directory.',
      },
    ]);
