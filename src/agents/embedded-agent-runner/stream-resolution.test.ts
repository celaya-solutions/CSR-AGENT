...
describe("prepared embedded stream strategy", () => {
  ...
  it("describes default Codex fallback as Zero to Agent native", () => {
    expect(
      resolveEmbeddedAgentStream({
        sessionId: "session-1",
        currentStreamFn: undefined,
...
describe("resolveEmbeddedAgentStream", () => {
  ...
  it("routes Codex responses fallbacks through Zero to Agent native transport", async () => {
    // Codex OAuth models use the Zero to Agent native transport, with prompt-cache
  ...
  it("routes Zero to Agent native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the Zero to Agent native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for Zero to Agent native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the Zero to Agent native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
  ...
  it.each(["run", "caller"] as const)(
    "cancels the authenticated Zero to Agent native fallback when the %s signal aborts",
    async (signalOwner) => {
      const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
      const runController = new AbortController();
      const callerController = new AbortController();
  ...
  it("forwards the run signal on the sync Zero to Agent native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    useNativeStreamFn(nativeStreamFn as never);
    const { streamFn } = resolveEmbeddedAgentStream({
  ...
  it("strips cache boundary markers on the Zero to Agent native fallback path", async () => {
