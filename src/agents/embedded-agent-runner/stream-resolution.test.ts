...
describe("prepared embedded stream strategy", () => {
  ...
  it("describes default Codex fallback as OpenAgent native", () => {
    expect(
      resolveEmbeddedAgentStream({
        sessionId: "session-1",
        currentStreamFn: undefined,
...
describe("resolveEmbeddedAgentStream", () => {
  ...
  it("routes Codex responses fallbacks through OpenAgent native transport", async () => {
    // Codex OAuth models use the OpenAgent native transport, with prompt-cache
  ...
  it("routes OpenAgent native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the OpenAgent native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for OpenAgent native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the OpenAgent native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
  ...
  it.each(["run", "caller"] as const)(
    "cancels the authenticated OpenAgent native fallback when the %s signal aborts",
    async (signalOwner) => {
      const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
      const runController = new AbortController();
      const callerController = new AbortController();
  ...
  it("forwards the run signal on the sync OpenAgent native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    useNativeStreamFn(nativeStreamFn as never);
    const { streamFn } = resolveEmbeddedAgentStream({
  ...
  it("strips cache boundary markers on the OpenAgent native fallback path", async () => {
