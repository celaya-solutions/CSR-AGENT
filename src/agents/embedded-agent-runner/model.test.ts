  ...
  return {
    ...
    buildSuppressedBuiltInModelError: ({
      ...
      if (
        ...
        return `Unknown model: ${provider}/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run \`openagent models auth login --provider openai\` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.`;
      }
      if (isUnsupportedXaiMultiAgentModel(provider, id)) {
        return "Unknown model: xai/grok-4.20-multi-agent-0309. OpenAgent does not currently support xAI multi-agent models; choose another xAI model. See https://docs.openclaw.ai/providers/xai.";
      }
      return undefined;
    },
  };
...
describe("resolveModel", () => {
  ...
  it("skips OpenAgent auth and model discovery during dynamic model resolution", async () => {
  ...
  it("defaults missing model cost before handing models to OpenAgent", async () => {
  ...
  it.each([
    ...
    expect(result.error).toBe(
      'Unknown model: openai-codex/gpt-5.4. "openai-codex" is a legacy provider ID. Run `openagent doctor --fix` to migrate legacy model and provider config to the current OpenAI format. If the provider has no authenticated profile, run `openagent models status` to check provider auth and re-authenticate if needed.',
    );
  });
  ...
  it("does not build an openai fallback for removed gpt-5.3-codex-spark", async () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: openai/gpt-5.3-codex-spark. gpt-5.3-codex-spark is available only through ChatGPT/Codex OAuth. Run `openagent models auth login --provider openai` and use openai/gpt-5.3-codex-spark with that OAuth profile; OpenAI API-key auth cannot use this model.",
    );
  });
  ...
  it("does not build a configured fallback for unsupported xAI multi-agent models", async () => {
    ...
    expect(result.model).toBeUndefined();
    expect(result.error).toBe(
      "Unknown model: xai/grok-4.20-multi-agent-0309. OpenAgent does not currently support xAI multi-agent models; choose another xAI model. See https://docs.openclaw.ai/providers/xai.",
    );
  });
