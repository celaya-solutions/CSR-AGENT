...
export function buildCredentialSafetyPrompt(
  ...
  return [
    ...
    ...(typeof input !== "string" && input?.controlToolsAvailable === false
      ? [
          "Channel, provider, and credential setup: use terminal `openagent channels add <channel>` or `openagent configure`; prompts mask secrets. Never collect tokens, API keys, or passwords in chat.",
        ]
      : []),
  ].join("\n");
}
