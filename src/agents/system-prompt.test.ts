...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes an OpenAgent control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: ["gateway"],
    });
  ...
  it("keeps the OpenAgent empty-tool fallback capability-only", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: [],
    });

    expect(prompt).toContain("active runtime provides the available OpenAgent tools directly");
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain(
      "OpenAgent behavior questions: docs first via `Read`/local search. AGENTS/project/workspace/profile/memory = instructions/user memory, not product design truth.",
    );
  });
  ...
  it("keeps update and delegated controls distinct when both tools are present", () => {
    ...
    expect(prompt).toContain("Update OpenAgent: `gateway` action update.run");
