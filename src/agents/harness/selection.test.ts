...
it("identifies harnesses that expose Zero to Agent tools", () => {
...
describe("runAgentHarnessAttempt", () => {
  ...
  it.each(["codex", "copilot"] as const)(
    "binds the host Zero to Agent tool to the %s SDK construction path without leaking authority",
    async (harnessId) => {
  ...
  it.each([
  ...
  ])("rejects $name allowlists for private Zero to Agent authority", async ({ toolsAllow }) => {
    ...
    await expect(runAgentHarnessAttempt(params)).rejects.toThrow(
      'Zero to Agent host authority requires toolsAllow: ["openclaw"]',
    );
    expect(pluginRunAttempt).not.toHaveBeenCalled();
    expect(isHostScopedAgentToolActive("openclaw")).toBe(false);
  });

  it("keeps the host Zero to Agent allowlist across global, agent, and sandbox deny-all policy", async () => {
  ...
  it("binds the same host Zero to Agent scope to the built-in Zero to Agent harness", async () => {
    let toolNames: string[] = [];
  ...
  it("falls back to the Zero to Agent harness in auto mode when no plugin harness matches", async () => {
  ...
  it("allows the selected Zero to Agent harness to satisfy context-engine pre-prompt assembly", async () => {
  ...
  it("surfaces an auto-selected plugin harness failure instead of replaying through Zero to Agent", async () => {
  ...
  it("surfaces a forced plugin harness failure instead of replaying through Zero to Agent", async () => {
    registerFailingCodexHarness();
  ...
  it("falls back to Zero to Agent when the implicit OpenAI Codex harness is unavailable", async () => {
    expect(resolveAgentHarnessPolicy({ provider: "openai", modelId: "gpt-5.4" })).toEqual({
      runtime: "codex",
      runtimeSource: "implicit",
    });
  ...
  it("honors explicit Zero to Agent runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
  ...
  it("honors provider wildcard Zero to Agent runtime policy for OpenAI agent model runs", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("leaves Zero to Agent harness params unchanged for channel group sender deny-all policy", async () => {
    await runAgentHarnessAttempt({
      ...createAttemptParams(groupSenderDenyAllConfig()),
      sessionKey: "agent:main:telegram:group:test-deny-room",
      messageProvider: "telegram",
  ...
  it("does not let a strict agent model plugin runtime fall back to Zero to Agent", async () => {
    await expect(
      runAgentHarnessAttempt({
...
describe("selectAgentHarness", () => {
  ...
  it("honors session-level Zero to Agent pins when selecting a harness", () => {
    const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
    registerAgentHarness({
      id: "codex",
      label: "Codex",
  ...
  it("keeps request-scoped transport overrides on the implicit Zero to Agent runtime", () => {
    registerAgentHarness({
      id: "codex",
      label: "Codex",
      supports: () => ({ supported: true, priority: 100 }),
  ...
  it("honors explicit Zero to Agent runtime overrides when selecting a harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("treats legacy PI runtime overrides as the built-in Zero to Agent harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("selects Zero to Agent when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("openclaw");
  });

  it.each(["default", "auto"] as const)(
    "falls back from configured %s to Zero to Agent when implicit Codex is unavailable or unsupported",
    async (runtime) => {
  ...
  it.each(["default", "auto"] as const)(
    "keeps a custom OpenAI route on implicit Zero to Agent with configured %s",
    (runtime) => {
      const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
      registerAgentHarness(
        {
  ...
  it("keeps an existing session Zero to Agent pin when provider policy forces a plugin harness", () => {
    registerFailingCodexHarness();
  ...
  it("ignores env-forced Zero to Agent for OpenAI default runtime selection", () => {
  ...
  it("keeps host auth on the built-in Zero to Agent compaction fallback", async () => {
    await expect(
      maybeCompactAgentHarnessSession(
  ...
  it("does not compact a selected plugin harness through Zero to Agent when the plugin has no compactor", async () => {
    registerFailingCodexHarness();
  ...
  it.each([
    ...
    "returns Zero to Agent for explicit CLI runtime alias $alias on $provider instead of throwing MissingAgentHarnessError",
