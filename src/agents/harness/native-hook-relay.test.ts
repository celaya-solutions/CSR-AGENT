...
describe("native hook relay registry", () => {
  ...
  it("uses the Codex no-op output when no OpenAgent hook decides", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      sessionId: "session-1",
      runId: "run-1",
  ...
  it("maps Codex PreToolUse to OpenAgent before_tool_call and blocks before execution", async () => {
  ...
  it("normalizes Codex exec_command cmd input before running OpenAgent policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("normalizes Codex exec_command argv cmd input before running OpenAgent policy", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
  ...
  it("maps Codex PostToolUse to OpenAgent after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
  ...
  it("maps Codex MCP PreToolUse to OpenAgent before_tool_call and can block", async () => {
    const beforeToolCall = vi.fn(async () => ({
      block: true,
    ...
    const response = await invokeNativeHookRelay({
      ...
      rawPayload: {
        ...
        tool_input: {
          entities: [{ name: "OpenAgent", entityType: "project", observations: ["test"] }],
        },
      },
    });
  ...
  it("maps Codex MCP PostToolUse to OpenAgent after_tool_call observation", async () => {
    const afterToolCall = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "after_tool_call", handler: afterToolCall }]),
    );
  ...
  it("routes Codex MCP PermissionRequest payloads through OpenAgent approval policy", async () => {
    const relay = registerNativeHookRelay({
      provider: "codex",
      agentId: "agent-1",
      sessionId: "session-1",
  ...
  it("defers PermissionRequest when OpenAgent approval does not decide", async () => {
...
describe("native hook relay command builder", () => {
  it("uses the Codex hook relay command shape", () => {
    expect(
      ...
      `${NATIVE_HOOK_RELAY_EXEC_PREFIX}openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event permission_request --timeout 5000`,
    );
  });
  ...
  it("execs niced relays so the Codex timeout owns the relay process", () => {
    ...
    expect(command).toBe(
      process.platform === "win32"
        ? "openagent hooks relay --provider codex --relay-id relay-1 --event post_tool_use --timeout 5000"
        : "exec nice -n 10 openagent hooks relay --provider codex --relay-id relay-1 --event post_tool_use --timeout 5000",
    );
  });
  ...
  it("includes explicit unavailable noop mode only for PreToolUse", () => {
    expect(
      ...
      `${NATIVE_HOOK_RELAY_EXEC_PREFIX}openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event pre_tool_use --pre-tool-use-unavailable noop --timeout 5000`,
    );
  });
});
