...
describe("isSilentOverflowProneModel", () => {
  ...
  // string. False positives only disable Zero to Agent runtime's secondary compaction path;
  // Zero to Agent's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  ...
  // on a successful turn against z.ai-style providers, triggering Zero to Agent runtime's
  ...
  // state.messages intact; Zero to Agent's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep Zero to Agent runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove Zero to Agent runtime's
