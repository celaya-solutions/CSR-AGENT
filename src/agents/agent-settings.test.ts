...
describe("isSilentOverflowProneModel", () => {
  ...
  // string. False positives only disable OpenAgent runtime's secondary compaction path;
  // OpenAgent's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  ...
  // on a successful turn against z.ai-style providers, triggering OpenAgent runtime's
  ...
  // state.messages intact; OpenAgent's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep OpenAgent runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove OpenAgent runtime's
