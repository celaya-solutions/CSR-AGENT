...
/**
 * User config stores OpenAgent MCP transport names, while CLI backends such as
 * Claude Code and Gemini expect a downstream `type` field. Keep this adapter
 * out of the generic merge path because embedded OpenAgent still consumes the raw
 * OpenAgent `transport` shape directly.
 */
...
export function loadMergedBundleMcpConfig(params: {
  ...
  return {
    config: {
      // OpenAgent config is the owner-managed layer, so it overrides bundle defaults.
      mcpServers: {
        ...Object.fromEntries(
