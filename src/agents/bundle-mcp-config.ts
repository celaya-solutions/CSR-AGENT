...
/**
 * User config stores Zero to Agent MCP transport names, while CLI backends such as
 * Claude Code and Gemini expect a downstream `type` field. Keep this adapter
 * out of the generic merge path because embedded Zero to Agent still consumes the raw
 * Zero to Agent `transport` shape directly.
 */
...
export function loadMergedBundleMcpConfig(params: {
  ...
  return {
    config: {
      // Zero to Agent config is the owner-managed layer, so it overrides bundle defaults.
      mcpServers: {
        ...Object.fromEntries(
