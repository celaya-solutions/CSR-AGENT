      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `openagent sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `openagent sandbox recreate --browser --agent ${agentId}`;
          }
          return "openagent sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
