...
class AuthStorageLegacyPathMigrationRequiredError extends Error {
  ...
  constructor() {
    super(
      "Deprecated AuthStorage path contains unmigrated credentials; run openagent doctor --fix for standard agent auth.json or migrate plugin storage to AuthStorage.forAgent(agentDir).",
    );
