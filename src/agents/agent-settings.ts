...
export function applyAgentCompactionSettingsFromConfig(params: {
  ...
  // Omission preserves embedded/project settings. OpenAgent config reloads create a new
 ...
 * fires on a successful turn and triggers OpenAgent runtime's `_runAutoCompaction` from
...
/**
 * Disable OpenAgent runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when OpenAgent or a plugin owns compaction:
 ...
 * Default-mode runs against ordinary providers keep OpenAgent runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores OpenAgent runtime's
