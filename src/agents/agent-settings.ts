...
export function applyAgentCompactionSettingsFromConfig(params: {
  ...
  // Omission preserves embedded/project settings. Zero to Agent config reloads create a new
 ...
 * fires on a successful turn and triggers Zero to Agent runtime's `_runAutoCompaction` from
...
/**
 * Disable Zero to Agent runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when Zero to Agent or a plugin owns compaction:
 ...
 * Default-mode runs against ordinary providers keep Zero to Agent runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores Zero to Agent runtime's
