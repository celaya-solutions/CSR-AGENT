...
describe("spawnAcpDirect", () => {
  ...
  it("strips an inherited OpenAgent auth profile before ACP initialization", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        defaults: {
  ...
  it("rejects an explicit OpenAgent auth profile for ACP runtimes", async () => {
    const result = await spawnAcpDirect(
      {
        task: "Investigate flaky tests",
        agentId: "codex",
    ...
    expect(result).toHaveProperty(
      "error",
      "ACP model overrides cannot select OpenAgent auth profiles; configure credentials in the ACP runtime instead.",
    );
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects OpenAgent config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      acp: {
        enabled: true,
    ...
    expect(result).toHaveProperty(
      "error",
      'agentId "pleres" is an OpenAgent config agent, not an ACP harness. Use runtime="subagent" or omit runtime for OpenAgent config agents. Use runtime="acp" only with external ACP harness ids such as codex, claude, droid, gemini, or opencode, or configure agents.entries.*.runtime.type="acp" with runtime.acp.agent.',
    );
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps OpenAgent ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
