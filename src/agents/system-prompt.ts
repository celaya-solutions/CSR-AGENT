/**
 * OpenAgent system prompt renderer.
 *
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Provider messaging: never exec/curl; OpenAgent routes.",
    messageToolAvailable
      ? [
          "",
          "### message tool",
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to OpenAgent main. */
  ...
  if (promptMode === "none") {
    return ["You are a personal assistant running inside OpenAgent.", modelIdentityLine]
      .filter(Boolean)
      .join("\n");
  }
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    agents_list: acpSpawnRuntimeEnabled
      ? "List allowed OpenAgent subagent ids; not ACP ids"
  ...
  // CLI backends own native file tools outside OpenAgent's projected tool list.
  ...
  const stablePrefix = cacheStablePromptPrefix(stablePrefixCacheKey, () => {
    const lines = [
      ...
      "User-editable; OpenAgent loads below as Project Context.",
      "",
    ];
