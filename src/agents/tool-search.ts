/** Tool Search catalog compaction for large Zero to Agent, MCP, and client tool inventories. */
...
export function createToolSearchTools(ctx: ToolSearchToolContext): AnyAgentTool[] {
  ...
  return [
    ...
    },
    {
      name: TOOL_CALL_RAW_TOOL_NAME,
      label: "Tool Call",
      description: "Call an exact Tool Search result id or name through Zero to Agent.",
      parameters: Type.Object({
        id: Type.String({ description: "Tool search result id or tool name." }),
        args: Type.Optional(
