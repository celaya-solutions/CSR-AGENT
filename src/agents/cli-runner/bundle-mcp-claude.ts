/**
 * Claude CLI argument helpers for Zero to Agent-managed bundle MCP config.
 */
...
export function findClaudeMcpConfigPaths(args?: string[]): string[] {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--mcp-config") {
      ...
      // as positional prompts after Zero to Agent injects its strict overlay.
...
/** Return Claude args with Zero to Agent's strict MCP config path injected. */
...
/** Writes the active per-attempt capture token into Zero to Agent's generated Claude MCP config. */
