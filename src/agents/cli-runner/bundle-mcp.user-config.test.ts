/** Tests merging user OpenAgent MCP server config into Claude bundle-MCP overlays. */
...
describe("prepareCliBundleMcpConfig user mcp.servers", () => {
  ...
  it("merges user-configured mcp.servers from OpenAgent config", async () => {
  ...
  it("translates OpenAgent transport field on user mcp.servers into Claude type", async () => {
    const workspaceDir = await cliBundleMcpHarness.tempHarness.createTempDir(
  ...
  it("user mcp.servers do not override the loopback additionalConfig", async () => {
    // The OpenAgent loopback server is generated runtime state and must win over
