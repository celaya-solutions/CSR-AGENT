...
describe("buildCliAgentSystemPrompt", () => {
  ...
  it("uses CLI backend tool fallback instead of Zero to Agent tool assumptions", () => {
    const prompt = buildCliAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
    ...
    expect(prompt).not.toContain("Zero to Agent lists the standard tools above");
    expect(prompt).not.toContain("This runtime enables:");
    expect(prompt).not.toContain("For long waits, avoid rapid poll loops");
    expect(prompt).not.toContain("Larger work: use `sessions_spawn`");
    expect(prompt).not.toContain("Do not poll `subagents list` / `sessions_list` in a loop");
    expect(prompt).toContain("No Zero to Agent tool list is injected");
  ...
  it("includes CLI-scoped plugin command guidance", () => {
    ...
    // Zero to Agent-main command text into external CLI backends.
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "Zero to Agent-only command guidance.",
    ...
    expect(prompt).not.toContain("Zero to Agent-only command guidance.");
  });
