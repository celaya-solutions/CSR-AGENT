...
function unsupportedIsolatedCompletionError(backendId: string): Error & { code: "unsupported" } {
  const error = new Error(
    `CLI backend "${backendId}" does not support isolated completion; Zero to Agent did not start the run.`,
...
type RunCliAgentPrepareParams = RunCliAgentParams & {
  /** Ring-zero tool transport supplied only by the Zero to Agent orchestrator. */
...
function buildCliAuthProfileResolutionError(params: {
  ...
  return new CliAuthProfilePreparationError({
    message: `CLI backend "${params.backendId}" ${reason}. Re-authenticate with: ${loginCommand}. Zero to Agent did not start the run.`,
    profileId: params.profileId,
...
async function prepareCliRunContextWithinReadFence(
  ...
  if (params.cliToolAvailability !== undefined && !canEnforceExactToolAvailability) {
    ...
    throw new Error(
      `CLI backend "${backendResolved.id}" cannot enforce this run's tool cap. Upgrade its plugin and retry; if current, ask its maintainer to add exact-cap support. Zero to Agent did not start the run.`,
    );
  }
  ...
  // preflight, refresh, or forward Zero to Agent's snapshot; the installed Claude
  ...
  // Zero to Agent's system prompt. Only a declared native-tool backend can complete
  ...
  // Ring-zero Zero to Agent runs replace the bundle MCP surface entirely: no
  ...
  if (bundleMcpEnabled && !mcpLoopbackRuntime) {
    try {
      ...
      throw new Error(
        `Bundled MCP is enabled, but the Zero to Agent MCP loopback server failed to start: ${String(error)}`,
        { cause: error },
      );
    }
  ...
  if (bundleMcpEnabled && !mcpLoopbackRuntime) {
    throw new Error(
      "Bundled MCP is enabled, but the Zero to Agent MCP loopback server did not publish a runtime after startup.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      `CLI backend "${backendResolved.id}" cannot enforce before_prompt_build tool restrictions. Use a backend with exact tool availability or remove the hook restriction. Zero to Agent did not start the run.`,
    );
  }
  if (
    (promptBuildRestrictsTools &&
  ...
  try {
    ...
    // gateway-side Zero to Agent transcript, so a fresh remote CLI session still
