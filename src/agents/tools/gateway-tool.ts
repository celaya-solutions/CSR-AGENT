...
export function createGatewayTool(options?: {
  ...
  return {
    ...
    description: allowConfigReads
      ...
      : "Update Zero to Agent with update.run, only on an explicit owner request. Restart and completion notice are automatic. Never via shell.",
      ...
      if (action === "update.run") {
        ...
        if (options?.senderIsOwner !== true) {
          ...
          return jsonResult({
            ok: false,
            code: "owner_required",
            message: `Only the Zero to Agent owner can start an update from chat. ${hint}`,
          });
        }
