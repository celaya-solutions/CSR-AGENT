/**
 * openclaw built-in tool: ring-zero setup/repair actions for the Zero to Agent
 * agent. Never exposed to normal agents — construction is bound to a host-owned
 * per-run scope, and every action funnels through Zero to Agent's typed operation
...
export type SystemAgentToolOptions = {
  /** Verified inference owner, distinct from the internal Zero to Agent execution agent. */
  agentId?: string;
...
const SystemAgentToolSchema = Type.Object({
  ...
  target: Type.Optional(
    stringEnum(["guided", "classic", "channels", "search", "gateway"], {
      description:
        "Setup target for open_setup. channels/search/gateway open masked terminal flows; guided/classic require exiting Zero to Agent and running openclaw onboard.",
    }),
  ),
                ...
                : directive.kind === "gateway-config-setup"
                  ...
                  : directive.kind === "memory-import"
                    ...
                    : directive.kind === "model-setup"
                      ? `${SYSTEM_AGENT_DIRECTIVE_PREFIX} the active inference route cannot be changed inside Zero to Agent. Tell the user to exit Zero to Agent and run \`openclaw onboard\`; do not ask for provider credentials here.`
                        ...
                        : directive.target === "channels"
                          ...
                          : directive.target === "search"
                            ...
                            : directive.target === "gateway"
                              ...
                              : `${SYSTEM_AGENT_DIRECTIVE_PREFIX} ${directive.target} setup cannot run inside Zero to Agent because it may change the active inference route. Tell the user to exit Zero to Agent and run \`openclaw onboard\`.`,
          {},
        );
      }
