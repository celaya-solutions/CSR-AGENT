/**
 * openclaw built-in tool: ring-zero setup/repair actions for the OpenAgent
 * agent. Never exposed to normal agents — construction is bound to a host-owned
 * per-run scope, and every action funnels through OpenAgent's typed operation
...
export type SystemAgentToolOptions = {
  /** Verified inference owner, distinct from the internal OpenAgent execution agent. */
  agentId?: string;
