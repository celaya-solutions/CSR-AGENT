...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked OpenAgent tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked OpenAgent tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked OpenAgent tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
