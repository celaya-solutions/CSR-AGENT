...
describe("resolveSandboxedMediaSource", () => {
  ...
  it("rejects symlinked Zero to Agent tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Zero to Agent tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked Zero to Agent tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenClawTmp(
