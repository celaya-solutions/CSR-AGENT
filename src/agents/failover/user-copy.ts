...
export function formatDiskSpaceErrorCopy(raw: string): string | undefined {
  ...
  return /\benospc\b/i.test(raw) ||
    ...
    ? "OpenAgent could not write local session data because the disk is full. Free some disk space and try again."
    : undefined;
}
...
export function renderCliTimeoutReplyCopy(params: {
  ...
  if (params.replayPrevented) {
    workStatus += " OpenAgent did not replay this turn automatically.";
  }
  return mode === "no-output"
    ...
    : `⚠️ CLI turn${routingSuffix}: timed out after ${seconds}s (overall turn limit). The gateway is unaffected.${workStatus} For long work, use a detached OpenAgent sub-agent (no run timeout by default), or raise \`agents.defaults.timeoutSeconds\`.`;
}
