...
describe("lockWorktreeForProcess", () => {
  ...
  it("reclaims a lock left behind by a dead Zero to Agent process", async () => {
  ...
  it("keeps a lock held by a live Zero to Agent process", async () => {
    const record = await setupWorktree();
  ...
  it("keeps a foreign lock that Zero to Agent does not own", async () => {
    const record = await setupWorktree();
