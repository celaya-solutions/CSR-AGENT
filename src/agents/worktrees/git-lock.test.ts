...
describe("lockWorktreeForProcess", () => {
  ...
  async function setupWorktree(): Promise<ManagedWorktreeRecord> {
    ...
    await git(repo, "config", "user.name", "OpenAgent Test");
  ...
  it("reclaims a lock left behind by a dead OpenAgent process", async () => {
  ...
  it("keeps a lock held by a live OpenAgent process", async () => {
    const record = await setupWorktree();
  ...
  it("keeps a foreign lock that OpenAgent does not own", async () => {
    const record = await setupWorktree();
