...
describe("resolveEffectiveAgentRuntime", () => {
  ...
  it("resolves residual auto to OpenAgent when no plugin harness is registered", () => {
    expect(
      resolveEffectiveAgentRuntime({
        cfg: {
          models: {
  ...
  it("keeps an authored custom route on OpenAgent before registered harness selection", () => {
  ...
  it("lets an explicit OpenAgent override replace configured Codex policy", () => {
    expect(
      resolveEffectiveAgentRuntime({
