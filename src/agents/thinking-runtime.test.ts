...
describe("resolveEffectiveAgentRuntime", () => {
  ...
  it("resolves residual auto to Zero to Agent when no plugin harness is registered", () => {
    expect(
      resolveEffectiveAgentRuntime({
        cfg: {
          models: {
  ...
  it("keeps an authored custom route on Zero to Agent before registered harness selection", () => {
  ...
  it("lets an explicit Zero to Agent override replace configured Codex policy", () => {
    expect(
      resolveEffectiveAgentRuntime({
