/**
 * Prompt-surface helpers for OpenAgent tool guidance.
 *
...
export function buildOpenClawToolFallbackText(params: { surface: AgentPromptSurfaceKind }): string {
  if (isOpenClawMainPromptSurface(params.surface)) {
    return "The active runtime provides the available OpenAgent tools directly. Use only exposed tools; names are case-sensitive.";
  }

  return "No OpenAgent tool list is injected for this runtime prompt surface. Use only tools exposed directly by the active backend.";
}

/** Returns whether the main OpenAgent prompt should include workflow hints around the tool list. */
