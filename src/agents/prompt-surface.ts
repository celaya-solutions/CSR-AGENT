/**
 * Prompt-surface helpers for Zero to Agent tool guidance.
 *
...
export function buildOpenClawToolFallbackText(params: { surface: AgentPromptSurfaceKind }): string {
  if (isOpenClawMainPromptSurface(params.surface)) {
    return "The active runtime provides the available Zero to Agent tools directly. Use only exposed tools; names are case-sensitive.";
  }

  return "No Zero to Agent tool list is injected for this runtime prompt surface. Use only tools exposed directly by the active backend.";
}

/** Returns whether the main Zero to Agent prompt should include workflow hints around the tool list. */
