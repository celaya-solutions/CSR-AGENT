...
const WARNING_PREFIX = "openagent gateway restart-storm warning ";
  ...
  try {
    ...
    for (const line of tail.split("\n")) {
      ...
      if (body.startsWith(WARNING_PREFIX)) {
        ...
        /^openagent gateway lifecycle source=cli action=restart mode=(?:kickstart|bootout)(?: |$)/.test(
          body,
        )
      ) {
...
export async function warnAboutGatewayRestartStorm(
  ...
  const message = [
    ...
    likelyJobs.length > 0
      ...
      : "Check for a stray keepalive launchd job invoking openagent gateway restart.",
    "Run openagent gateway status to inspect jobs and openagent doctor --fix to remove eligible stray jobs.",
  ].join(" ");
  try {
    fs.appendFileSync(
      resolveGatewayRestartLogPath(env),
