...
// an absolute OpenAgent path; inspectJob also excludes shell-altering job environments.
...
async function inspectJob(
  ...
  if (
    hasServiceMarker ||
    generatedWrapper ||
    /<key>Comment<\/key>\s*<string>OpenAgent (Gateway|Node)\b/.test(plist ?? "")
  ) {
    return null;
  }
...
export async function findForeignLaunchdJobs(
  ...
  if (labels.length > MAX_JOBS) {
    throw new Error(
      `Too many OpenAgent launchd jobs to inspect safely (${labels.length}; limit ${MAX_JOBS}).`,
    );
  }
  ...
  for (const label of labels) {
    if (Date.now() >= deadline) {
      throw new Error("OpenAgent launchd job inspection exceeded its 10-second budget.");
    }
