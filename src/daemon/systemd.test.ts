    ...
    async (comment) => {
      await withStageFixture(async ({ env, unitPath }) => {
        ...
        await fs.writeFile(
          ...
          [
            "[Unit]",
            "Description=OpenAgent Gateway (v2026.7.1-2)",
            "",
            "[Service]",
            comment,
            "ExecStart=/usr/bin/openclaw gateway run",
        ...
        expect(unit).toContain("Description=OpenAgent Gateway\n");
  ...
  it("clears stale inline-managed keys from env file on re-stage (#76860)", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      ...
      await stageSystemdService({
        ...
        // marks it as an OpenAgent-managed key so the stale env-file copy is cleared.
        environment: {
...
describe("systemd service install and uninstall", () => {
  ...
  it.each([
    ...
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      ...
      await expect(fs.readFile(unitPath, "utf8")).resolves.toContain("OpenAgent Node");
