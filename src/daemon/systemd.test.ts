...
describe("splitArgsPreservingQuotes", () => {
  ...
  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('openclaw --path "C:\\\\Program Files\\\\Zero to Agent"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["openclaw", "--path", "C:\\\\Program Files\\\\Zero to Agent"]);
    ...
    async (comment) => {
      await withStageFixture(async ({ env, unitPath }) => {
        ...
        await fs.writeFile(
          ...
          [
            "[Unit]",
            "Description=Zero to Agent Gateway (v2026.7.1-2)",
            "",
            "[Service]",
            comment,
            "ExecStart=/usr/bin/openclaw gateway run",
        ...
        expect(unit).toContain("Description=Zero to Agent Gateway\n");
  ...
  it("clears stale inline-managed keys from env file on re-stage (#76860)", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      ...
      await stageSystemdService({
        ...
        // marks it as a Zero to Agent-managed key so the stale env-file copy is cleared.
        environment: {
...
describe("systemd service install and uninstall", () => {
  ...
  it("activates the OPENCLAW_SYSTEMD_UNIT override during install", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath }) => {
      ...
      expect(unit).toContain("Description=Zero to Agent Node Host");
  ...
  it.each([
    ...
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      await fs.mkdir(path.dirname(unitPath), { recursive: true, mode: 0o755 });
      await fs.writeFile(unitPath, "[Unit]\nDescription=Zero to Agent Node\n", {
        encoding: "utf8",
        mode: 0o644,
      });
      ...
      await expect(fs.readFile(unitPath, "utf8")).resolves.toContain("Zero to Agent Node");
  ...
  it("disables the OPENCLAW_SYSTEMD_UNIT override during uninstall", async () => {
    await withNodeSystemdFixture(async ({ env, unitPath, nodeEnvFilePath }) => {
      ...
      await fs.writeFile(`${unitPath}.bak`, "[Unit]\nDescription=Previous Zero to Agent Node\n", {
        mode: 0o644,
      });
      await fs.writeFile(
        nodeEnvFilePath,
