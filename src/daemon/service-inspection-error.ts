...
const SERVICE_INSPECTION_MESSAGES = {
  ...
  "launchd-system-domain-unavailable":
    "The launchd system domain cannot be queried by this account. Ask root to inspect it with sudo launchctl print system/<label>. OpenAgent manages user LaunchAgents, not custom system LaunchDaemons.",
  "launchd-system-owned":
    "The Gateway label belongs to a system LaunchDaemon. OpenAgent manages user LaunchAgents; the custom system daemon belongs to its deployment owner.",
} as const;
