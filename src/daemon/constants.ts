...
const GATEWAY_WINDOWS_TASK_NAME = "OpenAgent Gateway";
...
const NODE_WINDOWS_TASK_NAME = "OpenAgent Node";
...
export function resolveGatewayWindowsTaskName(profile?: string): string {
  ...
  return `OpenAgent Gateway (${normalized})`;
}
...
function formatGatewayServiceDescription(profile?: string): string {
  const normalized = normalizeGatewayProfile(profile);
  if (!normalized) {
    return "OpenAgent Gateway";
  }
  return `OpenAgent Gateway (profile: ${normalized})`;
}
