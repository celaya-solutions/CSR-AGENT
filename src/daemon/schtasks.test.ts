...
describe("scheduled task runtime derivation", () => {
  ...
  it.each([
    ...
    const runtime = await readRuntimeFromQueryOutput(
      [
        "Aufgabenname: \\Zero to Agent Gateway",
        `Status: ${task.label}`,
  ...
  it.each([
    ...
    expect(probeScheduledTaskExists("Zero to Agent Gateway")).toBe(true);
    expect(isScheduledTaskDefinitelyNotRunning("Zero to Agent Gateway")).toBe(
  ...
  it.each([
    ...
    expect(probeScheduledTaskExists("Zero to Agent Gateway")).toBe(true);
    expect(isScheduledTaskDefinitelyNotRunning("Zero to Agent Gateway")).toBe(true);
  });
  ...
  it.each([null, "3", 5])(
    ...
    async (state) => {
      ...
      expect(probeScheduledTaskExists("Zero to Agent Gateway")).toBe(true);
      expect(isScheduledTaskDefinitelyNotRunning("Zero to Agent Gateway")).toBe(false);
    },
  );
  ...
  it.each(["-2147024894", "-2147024893"])(
    ...
    async (stdout) => {
      ...
      expect(probeScheduledTaskExists("Zero to Agent Gateway")).toBe(false);
      expect(isScheduledTaskDefinitelyNotRunning("Zero to Agent Gateway")).toBe(false);
    },
  );
  ...
  it.each([
    ...
    expect(probeScheduledTaskExists("Zero to Agent Gateway")).toBeNull();
  });
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Zero to Agent Share\\node.exe" "\\\\fileserver\\Zero to Agent Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Zero to Agent Share\\node.exe",
            "\\\\fileserver\\Zero to Agent Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
...
it.each([false, true])("retains observed Task Scheduler enable policy %s", (enabled) => {
  ...
  expect(probeScheduledTaskState("Zero to Agent Gateway")).toMatchObject({
    status: "found",
    state: 3,
    enabled,
  });
