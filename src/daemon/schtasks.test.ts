...
describe("scheduled task runtime derivation", () => {
  ...
  it.each([
    ...
    const runtime = await readRuntimeFromQueryOutput(
      [
        "Aufgabenname: \\OpenAgent Gateway",
        `Status: ${task.label}`,
  ...
  it.each([
    ...
    expect(probeScheduledTaskExists("OpenAgent Gateway")).toBe(true);
    expect(isScheduledTaskDefinitelyNotRunning("OpenAgent Gateway")).toBe(
  ...
  it.each([
    ...
    expect(probeScheduledTaskExists("OpenAgent Gateway")).toBe(true);
    expect(isScheduledTaskDefinitelyNotRunning("OpenAgent Gateway")).toBe(true);
  });
  ...
  it.each([null, "3", 5])(
    ...
    async (state) => {
      ...
      expect(probeScheduledTaskExists("OpenAgent Gateway")).toBe(true);
      expect(isScheduledTaskDefinitelyNotRunning("OpenAgent Gateway")).toBe(false);
    },
  );
  ...
  it.each(["-2147024894", "-2147024893"])(
    ...
    async (stdout) => {
      ...
      expect(probeScheduledTaskExists("OpenAgent Gateway")).toBe(false);
      expect(isScheduledTaskDefinitelyNotRunning("OpenAgent Gateway")).toBe(false);
    },
  );
  ...
  it.each([
    ...
    expect(probeScheduledTaskExists("OpenAgent Gateway")).toBeNull();
  });
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\OpenAgent Share\\node.exe" "\\\\fileserver\\OpenAgent Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\OpenAgent Share\\node.exe",
            "\\\\fileserver\\OpenAgent Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
...
it.each([false, true])("retains observed Task Scheduler enable policy %s", (enabled) => {
  ...
  expect(probeScheduledTaskState("OpenAgent Gateway")).toMatchObject({
    status: "found",
    state: 3,
    enabled,
  });
