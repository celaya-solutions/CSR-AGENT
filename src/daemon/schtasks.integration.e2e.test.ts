  ...
  it("isolates and completes the native Scheduled Task lifecycle", async () => {
    ...
    try {
      ...
      pendingProof = await withEnvAsync(env, async () => {
        ...
        const defaultTaskBefore = await readTaskDefinitionSnapshot("OpenAgent Gateway");
        ...
        await service.install({
          ...
          description: `OpenAgent CI Scheduled Task integration ${id}`,
        });
        ...
        expect(await canBindLoopbackPort(gatewayPort)).toBe(true);
        expect(await readTaskDefinitionSnapshot("OpenAgent Gateway")).toEqual(defaultTaskBefore);
