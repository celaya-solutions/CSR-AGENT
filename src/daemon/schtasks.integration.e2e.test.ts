  ...
  it("isolates and completes the native Scheduled Task lifecycle", async () => {
    ...
    try {
      ...
      pendingProof = await withEnvAsync(env, async () => {
        ...
        const defaultTaskBefore = await readTaskDefinitionSnapshot("Zero to Agent Gateway");
        ...
        await service.install({
          ...
          description: `Zero to Agent CI Scheduled Task integration ${id}`,
        });
        ...
        expect(await canBindLoopbackPort(gatewayPort)).toBe(true);
        expect(await readTaskDefinitionSnapshot("Zero to Agent Gateway")).toEqual(
          defaultTaskBefore,
        );
