...
export function resolveAcpInstallCommandHint(cfg: OpenClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `openagent plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^openagent plugins install /u, "");
    ...
    return "openagent plugins install acpx";
  }
