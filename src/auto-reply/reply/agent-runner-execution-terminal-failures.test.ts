  ...
  it.each([
    {
      ...
      recoveryText:
        ...
        "Zero to Agent run: run-max-turns. Zero to Agent session: session-1. Claude session: claude-session-1. " +
    ...
    },
    {
      ...
      recoveryText:
        ...
        "Zero to Agent run: run-hook-stopped. Zero to Agent session: session-1. Claude session: claude-session-1. " +
        "Tool actions may already have run; verify their effects before retrying. " +
  ...
  it.each([
    ...
    async ({ rejection, mode, routingSubstring }) => {
      ...
      if (mode === "overall") {
        ...
        expect(result.payload.text).toContain("detached Zero to Agent sub-agent");
