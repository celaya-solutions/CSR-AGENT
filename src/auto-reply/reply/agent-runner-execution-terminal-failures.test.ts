  ...
  it.each([
    {
      ...
      recoveryText:
        ...
        "OpenAgent run: run-max-turns. OpenAgent session: session-1. Claude session: claude-session-1. " +
    ...
    },
    {
      ...
      recoveryText:
        ...
        "OpenAgent run: run-hook-stopped. OpenAgent session: session-1. Claude session: claude-session-1. " +
        "Tool actions may already have run; verify their effects before retrying. " +
  ...
  it.each([
    ...
    async ({ rejection, mode, routingSubstring }) => {
      ...
      if (mode === "overall") {
        ...
        expect(result.payload.text).toContain("detached OpenAgent sub-agent");
