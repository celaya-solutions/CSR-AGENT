      ...
      return (
        options.result ?? {
          details: {
            ...
            command: "openagent sessions export-trajectory --session-key agent:target:session",
            cwd: "/tmp",
          },
        }
      );
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(command).not.toContain("openagent sessions export-trajectory");
  ...
  it.each([
    ...
    async ({ outcome, acknowledgement }) => {
      ...
      expect(privateReplies[0]?.text).toContain("openagent sessions export-trajectory");
