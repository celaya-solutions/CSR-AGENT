        ...
        try {
          ...
          expect(result.reply?.text).toContain(
            status === "applied"
              ...
              : "Model access was saved, but Zero to Agent has not confirmed it is active. Open Settings and select Apply changes, then send /models.",
          );
  ...
  it.each([
    ...
    async (label, allow, outcome, revocation) => {
      await withOpenClawTestState({ label: "login-command-consent" }, async (state) => {
        ...
        expect(denied?.reply?.text).toContain("Only a Zero to Agent owner can sign in here.");
