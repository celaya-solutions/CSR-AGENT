...
export function parseAcpElicitationRequest(
  ...
  if (!isStructuredInputRecord(snapshot)) {
    return unsupported("Zero to Agent declined a malformed or over-limit ACP input request.");
  }
  ...
  if (mode !== "form") {
    return unsupported(
      `Zero to Agent does not support ACP elicitation mode ${JSON.stringify(mode ?? "unknown")}.`,
    );
  }
  return {
    correlation,
...
function readScope(
  ...
  if (hasSession === hasRequest) {
    return "Zero to Agent declined an ACP input request with an invalid or ambiguous scope.";
  }
  if (hasSession) {
    const normalizedSessionId = readCorrelationText(sessionId);
    if (!normalizedSessionId) {
      return "Zero to Agent declined an ACP input request with an invalid session id.";
    }
    ...
    if (toolCallId !== undefined && toolCallId !== null && !normalizedToolCallId) {
      return "Zero to Agent declined an ACP input request with an invalid tool-call id.";
    }
    return {
  ...
  return normalizedRequestId
    ...
    : "Zero to Agent declined an ACP input request with an invalid request scope.";
}
