...
describe("executeAgentTurn: authentication failures", () => {
  it("surfaces gateway reauth guidance without a profile id", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Your model provider needs a new login. Send `/login openai` from a private chat or Control UI session. Where shown, you can also select **Sign in**. You can also re-auth with `openagent models auth login --provider openai` on the gateway.",
      );
  ...
  it("surfaces gateway reauth guidance from typed OAuth refresh failures", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Your model provider needs a new login. Send `/login openai` from a private chat or Control UI session. Where shown, you can also select **Sign in**. You can also re-auth with `openagent models auth login --provider openai --profile-id 'openai:user@example.com'` on the gateway.",
      );
      expect(result.payload.presentation).toEqual(PROVIDER_LOGIN_PRESENTATION);
    }
  });
  ...
  it("omits OAuth profile ids from group reauth guidance", async () => {
    ...
    expect(result.kind).toBe("final");
    if (result.kind === "final") {
      expect(result.payload.text).toContain("openagent models auth login --provider openai");
  ...
  it("adds provider login while retaining non-OpenAI terminal guidance", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Your model provider needs a new login. Send `/login anthropic` from a private chat or Control UI session. Where shown, you can also select **Sign in**. You can also re-auth with `openagent models auth login --provider anthropic` on the gateway.",
      );
  ...
  it("surfaces Agent SDK OAuth session expiry in Discord channels", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Your model provider needs a new login. Send `/login` from a private chat or Control UI session. Where shown, you can also select **Sign in**. You can also re-auth with `claude auth login && openagent models auth login --provider anthropic --method cli` on the gateway.",
      );
  ...
  it("surfaces typed missing API-key auth guidance without parsing the message", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        '⚠️ Missing API key for provider "openai". Run `openagent doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `openagent models auth login --provider openai` or run `openagent configure`.',
      );
    }
  });
  ...
  it("formats auth-profile failover copy from typed FailoverError metadata", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).toContain("openagent configure");
  ...
  it("renders bounded recovery when the selected auth profile is unavailable", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "The selected auth profile is unavailable in this agent's OpenAgent credential store. Import or migrate that credential into the agent, select another configured profile, or run `openagent configure`, then retry.",
      );
  ...
  it("does not suggest re-authentication for typed format failures", async () => {
    ...
    if (result.kind === "final") {
      ...
      expect(result.payload.text).not.toContain("openagent configure");
    }
  });
  ...
  it("falls back to a generic reauth command when the provider in the OAuth error is unsafe", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "⚠️ Your model provider needs a new login. Send `/login` from a private chat or Control UI session. Where shown, you can also select **Sign in**. You can also re-auth with `openagent models auth login` on the gateway.",
      );
      expect(result.payload.presentation).toEqual(providerLoginPresentation("/login"));
    }
  });
