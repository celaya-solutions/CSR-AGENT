...
describe("executeAgentTurn: authentication failures", () => {
  ...
  it("renders bounded recovery when the selected auth profile is unavailable", async () => {
    ...
    if (result.kind === "final") {
      expect(result.payload.text).toBe(
        "The selected auth profile is unavailable in this agent's OpenAgent credential store. Import or migrate that credential into the agent, select another configured profile, or run `openclaw configure`, then retry.",
      );
