        ...
        operation: async (checkpoint) => {
          ...
          if (result.started) {
            try {
              ...
              throw new Error(
                `Fleet cell ${tenantId} was created but did not become healthy within 60s; inspect it with \`openagent fleet status ${tenantId}\` or \`openagent fleet logs ${tenantId}\`, or remove it with \`openagent fleet rm ${tenantId} --force\`.`,
                { cause: error },
              );
            }
          }
