...
function splitSessionSharingSchema(sql: string): { sharing: string; withoutSharing: string } {
  ...
  if (start === -1 || end === -1) {
    throw new Error("OpenAgent agent session-sharing schema markers are missing.");
  }
  return {
...
if (sessionSuggestionsStart === -1) {
  throw new Error("OpenAgent agent session-suggestions schema marker is missing.");
}
