...
export function createOpenClawStateDatabaseAsyncLifecycle() {
  ...
  const assertOpen = (record: IdentityRecord) => {
    if (isSealed(record)) {
      throw new Error("OpenAgent state database read admission is closed");
    }
  };
      ...
      return {
        ...
        assertCurrent() {
          ...
          if (records.get(record.identity.key) !== record || record.generation !== generation) {
            throw new Error("OpenAgent state database read admission changed");
          }
        },
      };
    },
      ...
      const pending = tail.then(async () => {
        ...
        if (errors.length > 1) {
          throw createSqliteLifecycleAggregateError(
            errors,
            "OpenAgent state resource drainage failed",
            errors[0],
          );
        }
