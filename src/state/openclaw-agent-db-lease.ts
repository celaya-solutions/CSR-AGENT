...
export function assertAgentDatabaseMaintenanceAuthority(
  ...
  if (!authority || (expected && authority !== expected)) {
    throw new Error(
      "Agent identity migration requires stopped-writer maintenance; stop active agents and run openagent doctor --fix.",
    );
  }
    ...
    (database) => {
      ...
      if (maintenance || authority) {
        ...
        if (
          ...
          throw new Error(
            "Agent database maintenance is in progress; retry after openagent doctor --fix completes.",
          );
        }
...
export function assertNoOpenClawAgentDatabaseLeases(
  ...
  for (const row of rows) {
    ...
    if (leaseStillExists && (!agentId || row.agent_id === agentId)) {
      const remediation = agentId ? "." : "; stop that process and rerun openagent doctor --fix.";
      throw new OpenClawAgentDatabaseLeaseActiveError(
