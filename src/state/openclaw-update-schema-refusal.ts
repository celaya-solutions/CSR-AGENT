...
export class UpdateSchemaRefusalError extends Error {
  ...
  constructor(
    ...
    super(
      `Doctor refused update-time schema repair driven by OpenAgent ${updaterVersion}: this updater reopens the ledger with old code after migration, and version publication could not be deferred safely. ` +
        databases
          .map(
            (database) =>
