// OpenAgent state database manages shared persisted state and migrations.
...
export { OPENCLAW_SQLITE_BUSY_TIMEOUT_MS };
export type {
  ...
  try {
    ...
    if (errors.length > 0) {
      throw createSqliteLifecycleAggregateError(
        [error, ...errors],
        `Fresh OpenAgent state database open failed releasing access and closing its unpublished handle for ${pathname}.`,
        error,
      );
    }
    throw error;
