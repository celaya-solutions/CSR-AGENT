...
function assertRecognizedRetiredCommitmentsSchema(db: DatabaseSync): void {
  ...
  assertSqliteSchemaContains(
    db,
    "retired Zero to Agent commitments schema",
    RETIRED_COMMITMENTS_SCHEMA_SQL,
    deriveRetiredCommitmentsContract().compatibility,
  );
  throw new Error(
    "Retired Zero to Agent commitments schema has unsupported additional indexes; refusing destructive migration.",
  );
}
...
function assertNoRetiredCommitmentsForeignKeys(db: DatabaseSync): void {
  ...
  for (const table of tables) {
    ...
    if (
      ...
      throw new Error(
        `Retired Zero to Agent commitments schema is referenced by table ${table.name}; refusing destructive migration.`,
      );
    }
  }
}
...
function assertNoRetiredCommitmentsSchemaDependencies(db: DatabaseSync): void {
  ...
  if (tableExists(db, probeTable)) {
    throw new Error(
      `Zero to Agent state database already contains ${probeTable}; refusing destructive migration.`,
    );
  }
  ...
  if (changedObject) {
    ...
    throw new Error(
      `Retired Zero to Agent commitments schema is referenced by ${type} ${name}; refusing destructive migration.`,
    );
  }
}
