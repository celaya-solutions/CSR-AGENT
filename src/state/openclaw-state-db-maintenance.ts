...
export function assertOpenClawStateDatabaseOwner(
  ...
  if (metadata?.role !== "global") {
    ...
    throw new Error(
      `Zero to Agent state database ${options.pathname} has schema role ${role}; expected global.`,
    );
  }
}
...
export function assertOpenClawStateDatabaseForMaintenance(
  ...
  if (readStateSchemaContentVersion(database) !== OPENCLAW_STATE_SCHEMA_VERSION) {
    throw new Error(
      `Zero to Agent state database ${options.pathname} uses schema version ${userVersion}; run openclaw doctor --fix before compacting it.`,
    );
  }
  ...
  if (metadata?.schema_version !== userVersion) {
    ...
    throw new Error(
      `Zero to Agent state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${userVersion}; run openclaw doctor --fix before compacting it.`,
    );
  }
  assertSqliteSchemaContains(
    database,
...
function assertOpenClawStateDatabaseVersionForMigration(
  ...
  if (readStateSchemaMigrationVersion(database) !== options.version) {
    throw new Error(
      `Zero to Agent state database ${options.pathname} uses schema version ${userVersion}; expected ${options.version} before migrating it.`,
    );
  }
  assertOpenClawStateDatabaseOwner(database, options);
  const metadata = database
  ...
  if (metadata?.schema_version !== userVersion) {
    const schemaVersion =
      typeof metadata?.schema_version === "number" ? metadata.schema_version : "invalid";
    throw new Error(
      `Zero to Agent state database ${options.pathname} metadata schema version ${schemaVersion} does not match ${userVersion}; repair the ownership metadata before migrating it.`,
    );
  }
...
function migratePreparedWorkerOwnership(db: DatabaseSync, previousVersion: number): boolean {
  ...
  if (start < 0 || end < start) {
    throw new Error("Zero to Agent worker environment schema marker is missing.");
  }
