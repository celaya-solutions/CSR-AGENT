...
export function formatIncompatibleDatabaseSchemas(
  ...
  const doctorGuidance =
    operation === "doctor"
      ? ` ${incompatibleDatabases.map(formatDoctorIncompatibleDatabase).join(" ")} Run Doctor with the Zero to Agent install that wrote this state (typically the active Gateway install), or another build that supports these schemas.`
      : "";
  return (
    `${prefix} because ${incompatibleDatabases.length} Zero to Agent database schema(s) are newer than this build. ` +
  ...
  const action =
    operation === "doctor"
      ...
      : operation === "gateway-startup"
        ? "Gateway refused startup"
        : "Gateway refused restart";
  return `${action} because persisted database readiness could not be verified: ${shown.join("; ")}${omitted > 0 ? `; +${omitted} more` : ""}. ${operation === "doctor" ? "Stop Zero to Agent processes, then restore the affected database from a verified backup." : "Stop the Gateway and other Zero to Agent processes, run openclaw doctor --fix, then retry."}`;
}
