// Creates isolated Zero to Agent state directories for integration-style tests.
    ...
    const state: OpenClawTestState = {
      ...
      applyEnv: () => {
        if (releasePromise || cleanupPromise) {
          throw new Error("Cannot apply a released Zero to Agent test state");
        }
        resetConfigRuntimeStateForTest();
        // A later write can throw after earlier keys changed; restoration still owns them.
        envApplied = true;
        for (const [key, value] of Object.entries(envVars)) {
          // Test fixtures apply a fixed Zero to Agent env set, not plugin-provided host env.
          if (value === undefined) {
