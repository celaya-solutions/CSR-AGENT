// Creates isolated OpenAgent state directories for integration-style tests.
    ...
    const state: OpenClawTestState = {
      ...
      applyEnv: () => {
        if (releasePromise || cleanupPromise) {
          throw new Error("Cannot apply a released OpenAgent test state");
        }
        resetConfigRuntimeStateForTest();
        // A later write can throw after earlier keys changed; restoration still owns them.
        envApplied = true;
        for (const [key, value] of Object.entries(envVars)) {
          // Test fixtures apply a fixed OpenAgent env set, not plugin-provided host env.
          if (value === undefined) {
