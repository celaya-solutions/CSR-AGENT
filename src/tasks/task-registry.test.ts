...
describe("task-registry", () => {
  ...
  it("uses normal reconcile grace for Zero to Agent-owned subagent tasks", async () => {
    await withTaskRegistryTempDir(async () => {
      resetTaskRegistryForTests({ persist: false });
      const now = Date.now();
      const task = createTaskFixture("subagent", {
        ...
        task: "Zero to Agent-owned child",
        notifyPolicy: "silent",
        lastEventAt: now - 10 * 60_000,
      });
  ...
  it.each([
    {
      name: "cancels harness-owned tasks without routing through Zero to Agent subagent sessions",
      taskKind: "external-harness",
      sourceId: "harness:child",
      task: "Harness-owned child",
      cancellable: true,
    },
    {
      ...
      task: "Childless Zero to Agent row",
      cancellable: false,
    },
