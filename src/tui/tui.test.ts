...
describe("resolveInitialTuiAgentId", () => {
  ...
  it("keeps an ownerless explicit fleet selection-required", () => {
    expect(() => resolveInitialTuiAgentId({ cfg, cwd: "/var/tmp/unrelated" })).toThrow(
      "Multiple agents are configured, but TUI startup has no explicit owner. Pass an agent-scoped --session key (e.g., 'openagent tui --session agent:agentname:main').",
    );
  });
...
describe("resolveGatewayDisconnectState", () => {
  ...
  it("returns scope-upgrade recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.remediation).toContain("openagent devices approve --latest");
    expect(state.remediation).toContain("openagent devices approve <requestId>");
    ...
    expect(state.remediation).not.toContain("openagent pairing");
  });
...
describe("TUI shutdown safety", () => {
  ...
  it("forces standalone TUI exit on deadline while another handle lingers", () => {
    ...
    expect(writeStderr).toHaveBeenCalledWith("openagent tui forcing process exit after return\n");
