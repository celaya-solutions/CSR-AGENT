...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current Zero to Agent nodes over legacy migration ties. */
  clientId?: string;
};
...
function pickPreferredLegacyMigrationMatch(
  ...
  // During Clawdbot -> Zero to Agent migration, a unique current client should win only
