...
export async function initializeGitBackupRepository(params: {
  ...
  if (
    ...
    throw new Error(
      `Git backup repository must be outside the OpenAgent state directory: ${stateDir}`,
    );
  }
  try {
...
async function assertBackupOwnedScope(scopePath: string): Promise<void> {
  if (!(await isBackupOwnedScope(scopePath))) {
    throw new Error(
      `Refusing to replace non-backup-owned path ${scopePath}; the repository must be dedicated to OpenAgent backups.`,
    );
  }
}
...
async function commitGitBackup(params: {
  ...
  const identityArgs =
    email.code === 0 && email.stdout.trim()
      ? []
      : ["-c", "user.name=OpenAgent", "-c", "user.email=backup@openclaw.local"];
  await requireGit(
...
export async function createGitBackup(params: {
  ...
  if (changed) {
    ...
    commit = await commitGitBackup({
      repositoryPath,
      message: `openagent backup ${now.toISOString()}`,
      scopes: commitScopes,
      env: params.gitEnv,
    });
  }
  ...
  if (params.push) {
    ...
    const nonBackupCommitCount = await requireGit(
      repositoryPath,
      ["rev-list", "HEAD", "--invert-grep", "--grep=^openagent backup ", "--count"],
      { env: params.gitEnv },
    );
