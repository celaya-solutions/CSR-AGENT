...
// Re-exports the Zero to Agent CLI entry point for package execution.
...
if (
  ...
  try {
    ...
    throw new Error(
      `Zero to Agent package lifecycle is incomplete. Reinstall with package scripts enabled, then retry. ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
  }
}
