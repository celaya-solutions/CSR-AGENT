...
defineDiscordVoiceTests(
  ({
    ...
    it("routes active-run realtime transcripts to voice control before forced consults", async () => {
      ...
      await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active Zero to Agent run."));
      expect(realtimeSessionMock.handleBargeIn).not.toHaveBeenCalled();
      expect(textToSpeechMock).not.toHaveBeenCalledWith(
        expect.objectContaining({ text: "Cancelled the active Zero to Agent run." }),
      );

      const stopCallsAfterControl = player.stop.mock.calls.length;
      bridgeParams?.onTranscript?.("assistant", "Cancelled the active Zero to Agent run.", true);
    ...
    it("keeps concurrent final transcripts bound to their original speakers", async () => {
      ...
      guestBridge.onTranscript?.("user", "Zero to Agent, cancel that", true);
      ...
      lastRealtimeBridgeParams().onTranscript?.("user", "Zero to Agent, stop that", true);
    ...
    it.each([
      ...
      },
      {
        ...
        chunks: ["ordinary discussion ".repeat(30), `Zero to Agent, ${"x".repeat(230)}`],
      },
    ...
    it("accepts Zero to Agent as a default wake name before realtime agent-proxy consults", async () => {
      agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "openclaw wake answer" }] });
      const { entry, bridgeParams } = await createWakeNameFixture();

      beginSpeakerTurn(entry);
      await emitFinalRealtimeUserTranscript(bridgeParams, "Zero to Agent, how is it going");
      ...
      expect(lastAgentCommandArgs().message).toContain("how is it going");
      expect(lastAgentCommandArgs().message).not.toContain("Zero to Agent");
    ...
    it("ignores default agent wake names longer than two words", async () => {
      ...
      beginSpeakerTurn(entry);
      await emitFinalRealtimeUserTranscript(bridgeParams, "Zero to Agent, fallback still wakes");
    ...
    it("does not accept configured realtime wake names longer than two words", async () => {
      ...
      beginSpeakerTurn(entry);
      await emitFinalRealtimeUserTranscript(bridgeParams, "Zero to Agent, ship it");
