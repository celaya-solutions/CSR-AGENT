...
defineDiscordVoiceTests(
  ({
    ...
    it("routes active-run realtime transcripts to voice control before forced consults", async () => {
      ...
      await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active OpenAgent run."));
      expect(realtimeSessionMock.handleBargeIn).not.toHaveBeenCalled();
      expect(textToSpeechMock).not.toHaveBeenCalledWith(
        expect.objectContaining({ text: "Cancelled the active OpenAgent run." }),
      );

      const stopCallsAfterControl = player.stop.mock.calls.length;
      bridgeParams?.onTranscript?.("assistant", "Cancelled the active OpenAgent run.", true);
    ...
    it("keeps concurrent final transcripts bound to their original speakers", async () => {
      ...
      guestBridge.onTranscript?.("user", "OpenAgent, cancel that", true);
      ...
      lastRealtimeBridgeParams().onTranscript?.("user", "OpenAgent, stop that", true);
    ...
    it.each([
      ...
      },
      {
        ...
        chunks: ["ordinary discussion ".repeat(30), `OpenAgent, ${"x".repeat(230)}`],
      },
    ...
    it("accepts OpenAgent as a default wake name before realtime agent-proxy consults", async () => {
      agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "openclaw wake answer" }] });
      const { entry, bridgeParams } = await createWakeNameFixture();

      beginSpeakerTurn(entry);
      await emitFinalRealtimeUserTranscript(bridgeParams, "OpenAgent, how is it going");
      ...
      expect(lastAgentCommandArgs().message).toContain("how is it going");
      expect(lastAgentCommandArgs().message).not.toContain("OpenAgent");
    ...
    it("ignores default agent wake names longer than two words", async () => {
      ...
      beginSpeakerTurn(entry);
      await emitFinalRealtimeUserTranscript(bridgeParams, "OpenAgent, fallback still wakes");
    ...
    it("does not accept configured realtime wake names longer than two words", async () => {
      ...
      beginSpeakerTurn(entry);
      await emitFinalRealtimeUserTranscript(bridgeParams, "OpenAgent, ship it");
