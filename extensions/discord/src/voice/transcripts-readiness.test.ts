    ...
    it.each([
      ...
      async ({ occupied, recovery, stage, overlap }) => {
        ...
        try {
          ...
          vi.useFakeTimers();
          // Native speaking state exists before OpenAgent installs its receive listeners.
    ...
    it.each([
      ...
      async ({ mode, origin }) => {
        ...
        try {
          ...
          // A native restart inside OpenAgent's grace retains this same owned receive stream.
