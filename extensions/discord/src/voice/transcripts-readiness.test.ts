    ...
    it.each([
      ...
      async ({ occupied, recovery, stage, overlap }) => {
        ...
        try {
          ...
          vi.useFakeTimers();
          // Native speaking state exists before Zero to Agent installs its receive listeners.
    ...
    it.each([
      ...
      async ({ mode, origin }) => {
        ...
        try {
          ...
          // A native restart inside Zero to Agent's grace retains this same owned receive stream.
