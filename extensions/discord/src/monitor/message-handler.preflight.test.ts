...
describe("preflightDiscordMessage", () => {
      ...
      ...(includeParent
        ? {
            referencedMessage: createDiscordMessage({
              ...
              author: { id: "openclaw-bot", bot: true, username: "Zero to Agent" },
            }),
          }
        : {}),
    });
  ...
  it.each([
    ...
    const mentionedBotUser = { id: botUserId, username: "Zero to Agent" };
    const message = createDiscordMessage({
  ...
  it("hydrates mention metadata from REST when bot mention syntax is present but mentions are missing", async () => {
    ...
    client.rest = {
      get: vi.fn(async () => ({
        ...
        mentions: [{ id: botId, username: "Zero to Agent", bot: true }],
        mention_roles: [],
        mention_everyone: false,
      })),
