...
Official MXC sandbox execution plugin for OpenAgent.

This plugin lets OpenAgent run tool execution through MXC on Windows hosts with
...
- Minimum OpenAgent host: `2026.6.11`
...
- `workspaceAccess`:
  ...
  - `rw`: the active agent workspace is mounted read-write. If protected
    OpenAgent skill roots (`skills`, `.agents/skills`, or the materialized
...
- `scope` workspace selection:
  - `session`, `agent`, and `shared` choose the OpenAgent workspace directory
    passed to MXC.
...
- OpenAgent passes per-run command, environment, and filesystem config to the
...
Use an already configured OpenAgent installation with MXC installed and enabled
...
```json
{
  "filesystem": {
    ...
    "additionalReadonlyPaths": ["C:\\Tools\\OpenAgent\\shared-readonly"],
...
When multiple configured policy files exist, OpenAgent layers them
...
The filesystem bridge keeps protected OpenAgent skill overlays read-only. For
