...
Official MXC sandbox execution plugin for Zero to Agent.

This plugin lets Zero to Agent run tool execution through MXC on Windows hosts with
...
- Minimum Zero to Agent host: `2026.6.11`
...
- `workspaceAccess`:
  ...
  - `rw`: the active agent workspace is mounted read-write. If protected
    Zero to Agent skill roots (`skills`, `.agents/skills`, or the materialized
...
- `scope` workspace selection:
  - `session`, `agent`, and `shared` choose the Zero to Agent workspace directory
    passed to MXC.
...
- Zero to Agent passes per-run command, environment, and filesystem config to the
...
Use an already configured Zero to Agent installation with MXC installed and enabled
...
```json
{
  "filesystem": {
    ...
    "additionalReadonlyPaths": ["C:\\Tools\\Zero to Agent\\shared-readonly"],
...
When multiple configured policy files exist, Zero to Agent layers them
...
The filesystem bridge keeps protected Zero to Agent skill overlays read-only. For
