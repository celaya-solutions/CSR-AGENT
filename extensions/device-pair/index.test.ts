...
describe("device-pair /pair qr", () => {
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Zero to Agent iOS app:");
    ...
    expect(text).not.toContain("![Zero to Agent pairing QR]");
  });
  ...
  `("%s", async ({ channel, context, target, opts }) => {
    ...
    expect(caption).toContain("Scan this QR code with the Zero to Agent iOS app:");
