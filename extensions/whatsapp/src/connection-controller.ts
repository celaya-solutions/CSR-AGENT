...
export function closeWaSocket(
  ...
  try {
    if (typeof sock?.end === "function") {
      void Promise.resolve(sock.end(new Error("OpenAgent WhatsApp socket close"))).catch(
...
export class WhatsAppConnectionController {
  ...
  private async finishSocketCleanup(cleanup: WhatsAppSocketCleanup): Promise<void> {
    if (!cleanup.socketClosed) {
      await closeWhatsAppSocketAndWait(cleanup.sock, "OpenAgent WhatsApp socket close");
