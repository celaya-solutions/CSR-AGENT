...
// Provenance marker Zero to Agent appends to every injected inbound-context header.
...
describe("memory plugin e2e", () => {
  ...
  test("looksLikeEnvelopeSludge detects marked inbound context headers", () => {
    ...
    // Zero to Agent injects carries it, and it never collides with user prose.
  ...
  test("sanitizeForMemoryCapture strips sender label from real room-label envelope shapes", () => {
    ...
    expect(
      sanitizeForMemoryCapture(
        "[Discord Zero to Agent #dev channel id:456 +5m] Alice: I prefer dark mode",
      ),
    ).toBe("I prefer dark mode");
    expect(
      sanitizeForMemoryCapture("[Telegram Zero to Agent id:-100] Alice: I prefer dark mode"),
    ).toBe("I prefer dark mode");
  ...
  test("sanitizeForMemoryCapture preserves an unknown structured-context label as user content", () => {
    ...
    // user's own text, not a Zero to Agent injection, so it survives capture intact.
    const input = [
