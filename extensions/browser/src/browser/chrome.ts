...
export async function launchOpenClawChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`openagent browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`openagent browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`openagent browser network-prediction prefs failed: ${String(err)}`);
  }

  try {
    ensureProfileCleanExit(userDataDir);
  } catch (err) {
    log.warn(`openagent browser clean-exit prefs failed: ${String(err)}`);
  }
  signal?.throwIfAborted();
  ...
  const launchOnceAndWait = async (allowSingletonRecovery: boolean): Promise<RunningChrome> => {
    ...
    try {
      ...
      log.info(
        `openagent browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
      );
