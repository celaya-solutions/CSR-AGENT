...
export function buildBrowserDoctorReport(params: {
  ...
  if (transport === "chrome-mcp") {
    ...
    checks.push({
      ...
      status: status.running ? "pass" : "fail",
      summary: status.running
        ? "OpenAgent Chrome extension is connected"
        : "OpenAgent Chrome extension is not connected",
      ...(status.running
        ? {}
        : {
            fixHint:
    ...
    checks.push({
      ...
      ...(mismatch
        ? {
            fixHint:
              "Reload the OpenAgent extension from chrome://extensions. If the versions still differ, fully quit and reopen Chrome.",
          }
        : {}),
    });
  } else {
