...
describe("browser server-context ensureBrowserAvailable", () => {
  ...
  it("explains attachOnly for externally managed loopback CDP services", async () => {
    ...
    await expect(promise).rejects.toThrow(
      "set browser.profiles.openclaw.attachOnly=true so Zero to Agent attaches without trying to manage the local process",
    );
    await expect(promise).rejects.toThrow(
      "For Browserless Docker, set EXTERNAL to the same WebSocket endpoint Zero to Agent can reach via browser.profiles.<name>.cdpUrl.",
    );
