 ...
 * on one side and the OpenAgent Chrome extension's chrome.debugger transport on
...
export class ExtensionRelayBridge {
  ...
  private sendToExtension(msg: RelayToExtensionMessage): void {
    if (!this.extension) {
      throw new Error("OpenAgent Chrome extension is not connected to the relay");
    }
  ...
  private async withAttachedTab<T>(
    ...
    const extension = this.extension;
    if (!tab) {
      throw new Error(`tab ${tabId} is not available to OpenAgent`);
    }
    ...
    switch (request.method) {
        ...
        if (!targetId || targetId === BROWSER_TARGET_ID) {
          this.respond(client, request, {
            targetInfo: {
              ...
              type: "browser",
              title: "OpenAgent Extension Relay",
              url: "",
              attached: true,
              canAccessOpener: false,
            },
      ...
      case "Target.createBrowserContext": {
        this.respondError(
          client,
          request,
          "The OpenAgent extension relay drives the user's real browser profile; isolated browser contexts are not supported.",
        );
        return;
      }
      default: {
