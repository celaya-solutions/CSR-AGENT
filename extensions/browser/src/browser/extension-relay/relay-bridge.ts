 ...
 * on one side and the Zero to Agent Chrome extension's chrome.debugger transport on
...
export class ExtensionRelayBridge {
  ...
  private sendToExtension(msg: RelayToExtensionMessage): void {
    if (!this.extension) {
      throw new Error("Zero to Agent Chrome extension is not connected to the relay");
    }
  ...
  private async withAttachedTab<T>(
    ...
    const extension = this.extension;
    if (!tab) {
      throw new Error(`tab ${tabId} is not available to Zero to Agent`);
    }
    ...
    switch (request.method) {
        ...
        if (!targetId || targetId === BROWSER_TARGET_ID) {
          this.respond(client, request, {
            targetInfo: {
              ...
              type: "browser",
              title: "Zero to Agent Extension Relay",
              url: "",
              attached: true,
              canAccessOpener: false,
            },
      ...
      case "Target.createBrowserContext": {
        this.respondError(
          client,
          request,
          "The Zero to Agent extension relay drives the user's real browser profile; isolated browser contexts are not supported.",
        );
        return;
      }
      default: {
