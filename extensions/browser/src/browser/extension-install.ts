...
const OWNED_LAUNCHER_MARKER = "# OpenAgent native messaging bootstrap v1";
...
const NATIVE_HOST_DESCRIPTION = "OpenAgent browser extension bootstrap";
export {
  FOUNDATION_CHROME_WEB_STORE_URL,
...
function expectedExtensionIds(extensionIds: string[]): string[] {
  ...
  // it never proves that an arbitrary extension path is OpenAgent-owned.
...
async function inspectRegistration(
  ...
  try {
    ...
    try {
      ...
      issue =
        "registered native host runtime or entry is unavailable or unsafe; run openagent browser extension install";
    }
    return {
      product: root.product,
      browser: root.label,
...
export async function installChromeExtensionBootstrap(params: {
  ...
  for (const root of chromeProductRoots(deps)) {
    ...
    try {
      ...
      if (request) {
        params.onProgress?.(
          `Requested the OpenAgent Store extension for ${root.label}. Restart Chrome if needed, then approve OpenAgent in chrome://extensions.`,
        );
      }
    } catch (error) {
      preRegistrationIssues.push(
        `${root.label}: Store installation request refused (${error instanceof Error ? error.message : String(error)}). Add OpenAgent directly: ${FOUNDATION_CHROME_WEB_STORE_URL}`,
      );
    }
  }
  if (preRegisteredRoots > 0) {
    params.onProgress?.(
      `Native bootstrap is ready. Add OpenAgent from the Chrome Web Store: ${FOUNDATION_CHROME_WEB_STORE_URL}. For development, load unpacked from ${installed}.`,
    );
  } else {
    preRegistrationIssues.push(
  ...
  while (
    ...
    if (!announcedWait) {
      params.onProgress?.("Waiting for Chrome to verify the OpenAgent extension…");
      announcedWait = true;
    }
...
export async function browserExtensionStatus(params: {
  ...
  return {
    ...
    issues: [
      ...(installedCopy.present && !installedCopy.owned
        ? [`Chrome extension copy is not OpenAgent-owned: ${installedPath}`]
        : []),
      ...discovery.issues,
...
/** Remove only registrations and launchers that carry OpenAgent ownership. */
