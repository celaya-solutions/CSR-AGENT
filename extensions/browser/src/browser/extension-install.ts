...
const OWNED_LAUNCHER_MARKER = "# Zero to Agent native messaging bootstrap v1";
...
const NATIVE_HOST_DESCRIPTION = "Zero to Agent browser extension bootstrap";
export {
  FOUNDATION_CHROME_WEB_STORE_URL,
...
function expectedExtensionIds(extensionIds: string[]): string[] {
  ...
  // it never proves that an arbitrary extension path is Zero to Agent-owned.
...
export async function installChromeExtensionBootstrap(params: {
  ...
  for (const root of chromeProductRoots(deps)) {
    ...
    try {
      ...
      if (request) {
        params.onProgress?.(
          `Requested the Zero to Agent Store extension for ${root.label}. Restart Chrome if needed, then approve Zero to Agent in chrome://extensions.`,
        );
      }
    } catch (error) {
      preRegistrationIssues.push(
        `${root.label}: Store installation request refused (${error instanceof Error ? error.message : String(error)}). Add Zero to Agent directly: ${FOUNDATION_CHROME_WEB_STORE_URL}`,
      );
    }
  }
  if (preRegisteredRoots > 0) {
    params.onProgress?.(
      `Native bootstrap is ready. Add Zero to Agent from the Chrome Web Store: ${FOUNDATION_CHROME_WEB_STORE_URL}. For development, load unpacked from ${installed}.`,
    );
  } else {
    preRegistrationIssues.push(
  ...
  while (
    ...
    if (!announcedWait) {
      params.onProgress?.("Waiting for Chrome to verify the Zero to Agent extension…");
      announcedWait = true;
    }
...
export async function browserExtensionStatus(params: {
  ...
  return {
    ...
    issues: [
      ...(installedCopy.present && !installedCopy.owned
        ? [`Chrome extension copy is not Zero to Agent-owned: ${installedPath}`]
        : []),
      ...discovery.issues,
...
/** Remove only registrations and launchers that carry Zero to Agent ownership. */
