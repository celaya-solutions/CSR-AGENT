...
/** Import decrypted system-profile cookies into one managed Zero to Agent profile. */
export async function importSystemProfileCookies(
  ...
  if (
    ...
    throw new Error(
      `profile "${into}" is not a locally managed Zero to Agent profile; import into a fresh profile name`,
    );
  }
      ...
      return await runProfileContextOperation(
        ...
        async (signal, profileRuntime) => {
          ...
          if (
            ...
            throw new Error(
              `managed profile "${into}" is not owned by this Zero to Agent browser runtime; stop it and import into a fresh profile name`,
            );
          }
          if (!usesOpenClawMockKeychain(userDataDir)) {
            throw new Error(
              `managed profile "${into}" does not use the Zero to Agent mock keychain; import into a fresh profile name`,
            );
          }
