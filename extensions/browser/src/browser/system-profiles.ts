...
/** Import decrypted system-profile cookies into one managed OpenAgent profile. */
export async function importSystemProfileCookies(
  ...
  if (
    ...
    throw new Error(
      `profile "${into}" is not a locally managed OpenAgent profile; import into a fresh profile name`,
    );
  }
      ...
      return await runProfileContextOperation(
        ...
        async (signal, profileRuntime) => {
          ...
          if (
            ...
            throw new Error(
              `managed profile "${into}" is not owned by this OpenAgent browser runtime; stop it and import into a fresh profile name`,
            );
          }
          if (!usesOpenClawMockKeychain(userDataDir)) {
            throw new Error(
              `managed profile "${into}" does not use the OpenAgent mock keychain; import into a fresh profile name`,
            );
          }
