...
export function registerBrowserExtensionCommands(
  ...
  const extension = browser
    .command("extension")
    .description("Install and inspect the Zero to Agent Chrome extension bootstrap");
    ...
    .action(async (opts, command) => {
      await runCommandWithRuntime(
        ...
        async () => {
          ...
          if (!json) {
            defaultRuntime.log(info("Preparing the Zero to Agent Chrome extension…"));
          }
                ...
                ? theme.warn(
                    status.platformSupport === "manual_required"
                      ...
                      : status.storeInstallRequests.some((entry) => entry.state === "requested")
                        ? `Store installation requested. Enable Zero to Agent in chrome://extensions and approve Chrome's prompt. If it has not appeared, restart Chrome when convenient or add it from ${FOUNDATION_CHROME_WEB_STORE_URL}. Run extension status to check setup again.`
                        : `Setup needs attention. Add Zero to Agent from ${FOUNDATION_CHROME_WEB_STORE_URL} after native registration succeeds. For development, load the printed unpacked path. If the extension attempted setup before the native host existed, restart Chrome once.`,
                  )
                : info(
  ...
  extension
    ...
    .description(
      "Remove Zero to Agent-owned Store install requests; Chrome may remove the extension on restart",
    )
  ...
  extension
    ...
    .description("Remove only Zero to Agent-owned Chrome native-host registrations")
    .option("--json", "Print a machine-readable removal report")
    .action(async (opts, command) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        const json = opts.json === true || parentOpts(command).json === true;
        ...
        async () => {
          ...
          defaultRuntime.log(
            [
              ...
              info("2. Open the Zero to Agent popup and paste this pairing string:"),
              "",
