...
export function describeBrowserTool(opts: {
  ...
  const lines = [
    `Control the browser via Zero to Agent's browser control server. Available actions: ${opts.capabilities.actions.join(", ")}.`,
    ...(actions.has("profiles")
      ? [
          "Browser choice: omit profile to use the configured default (normally the isolated Zero to Agent-managed `openclaw` browser).",
