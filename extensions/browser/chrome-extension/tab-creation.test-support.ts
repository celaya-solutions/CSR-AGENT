...
export async function assertRelayTabCreation(params: {
  ...
  try {
    ...
    expect(await created.page.title()).toBe("OpenAgent selected tab");
    ...
    expect(snapshot.body).toMatchObject({
      ...
      snapshot: expect.stringContaining("OpenAgent created destination"),
    });
    await fs.writeFile(
      artifact,
