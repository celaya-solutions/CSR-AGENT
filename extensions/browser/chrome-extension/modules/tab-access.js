...
export function createTabAccessPolicy({ chromeApi = chrome, isSelectedTab, getGroupColor }) {
  ...
  async function requireTab(tabId, epoch = capture(tabId), afterNavigation = false) {
    ...
    if (state.reason === "paused") {
      throw new Error(`tab ${tabId} is paused for OpenAgent`);
    }
    if (state.reason === "not-selected") {
      throw new Error(`tab ${tabId} is not in the OpenAgent tab group`);
    }
    if (state.reason === "incognito") {
      throw new Error(`tab ${tabId} is incognito and unavailable to OpenAgent`);
    }
    throw new Error(`tab ${tabId} is restricted or unavailable to OpenAgent`);
  }
