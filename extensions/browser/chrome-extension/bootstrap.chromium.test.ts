...
async function waitForExtensionId(context: BrowserContext, extensionPath: string): Promise<string> {
  ...
  throw new Error("Chromium did not report the loaded OpenAgent extension");
}
    ...
    try {
      ...
      if (
        ...
        !(await fs.readFile(manifest.path, "utf8")).includes(
          "# OpenAgent native messaging bootstrap v1",
        )
      ) {
        return false;
      }
      ...
      async () => {
        ...
        const gatewayServer = http.createServer((req, res) => {
          if (req.url === "/browser-owner-proof") {
            ...
            res.end("<title>OpenAgent selected tab</title><h1>OpenAgent created destination</h1>");
            return;
          }
          res.writeHead(426);
          res.end();
        ...
        await controlled.goto(
          `data:text/html,${encodeURIComponent(
            '<title>OpenAgent E2E</title><style>body{margin:0}#spacer{height:2200px}#target{display:block;width:240px;height:96px;background:#1457d9;color:white;border:0;font:20px sans-serif}</style><div id="spacer"></div><button id="target">Offscreen target</button>',
          )}`,
        );
