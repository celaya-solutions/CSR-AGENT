...
// OpenAgent extension service worker.
//
// Thin transport between the OpenAgent extension relay (loopback WebSocket) and
...
// The OpenAgent tab group is the ACL in selected mode and an ownership marker
// in all-tabs mode.
import {
...
function failRelayAuthentication(ws, error) {
  ...
  relayStatusHint =
    "Relay authentication v2 failed. Update OpenAgent, or re-pair after a relay key rotation.";
  try {
    closeRelaySocket(
      4001,
...
function handleRelayOpeningDeadline() {
  ...
  setBadge("error");
  relayStatusHint = "Relay authentication v2 timed out. Make sure OpenAgent is up to date.";
  scheduleReconnect();
}
