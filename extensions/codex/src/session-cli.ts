...
async function continueCodexSession(
  ...
  writeLine(`Zero to Agent session: ${singleLineTerminalText(result.sessionKey)}`);
}
...
export function registerCodexSessionCli(program: Command): void {
  ...
  addGatewayClientOptions(
    codex
      ...
      .description("Continue a Gateway-local Codex thread as a Zero to Agent branch")
  ...
  addGatewayClientOptions(
    codex
      ...
      .option(
        ...
        "Confirm no other Codex client or Zero to Agent runner is using this thread",
        false,
      )
      .option("--json", "Print the structured response", false),
