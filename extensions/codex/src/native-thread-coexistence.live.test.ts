    ...
    await withTempDir("openclaw-codex-coexistence-", async (root) => {
      try {
        ...
        const started = await withClient(clientOptions, async (first) => {
          ...
          await first.request(
            "thread/name/set",
            { threadId: response.thread.id, name: "OpenAgent coexistence source" },
            { timeoutMs: 60_000 },
          );
          return response;
        });
        ...
        const forked = await withClient(clientOptions, async (second) => {
          ...
          await second.request(
            "thread/name/set",
            { threadId: response.thread.id, name: "OpenAgent coexistence fork" },
            { timeoutMs: 60_000 },
          );
          await second.request(
            "thread/archive",
        ...
        await withClient(clientOptions, async (third) => {
          ...
          expect(read.thread).toMatchObject({
            ...
            name: "OpenAgent coexistence fork",
          });
