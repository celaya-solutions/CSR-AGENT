        ...
        try {
          ...
          return [
            managedCodexFinding({
              ...
              fixHint:
                "Reinstall the staged Zero to Agent package with its @openai/codex platform dependency, then rerun the candidate check.",
            }),
          ];
        }
      ...
      if (!nativeCommand) {
        return [
          managedCodexFinding({
            ...
            fixHint:
              "Reinstall the staged Zero to Agent package with the matching @openai/codex platform package, then rerun the candidate check.",
          }),
        ];
      }
      ...
      try {
        ...
        return [
          managedCodexFinding({
            ...
            fixHint:
              versionFailureHint ??
              "Repair or reinstall the staged Zero to Agent package, then rerun the candidate check before cutover.",
          }),
        ];
      }
      ...
      if (detectedVersion !== CODEX_APP_SERVER_VERSION) {
        return [
          managedCodexFinding({
            ...
            path: nativeCommand,
            requirement: `the exact Zero to Agent-pinned Codex version ${CODEX_APP_SERVER_VERSION}`,
            fixHint:
              versionFailureHint ??
              "Reinstall the staged Zero to Agent package so its managed @openai/codex dependency matches the pinned version, then rerun the candidate check.",
          }),
        ];
      }
      return [];
