...
const ForkParamsSchema = Type.Object(
  {
    ...
    attach: Type.Optional(
      Type.Boolean({
        default: true,
        description: "Attach the fork to this Zero to Agent session for its next turn.",
      }),
    ),
  },
  { additionalProperties: false },
      ...
      const run = async (archiveAdmission?: {
        ...
        if (archiveAdmission) {
          ...
          if (await options.bindingStore.hasOtherThreadOwner(threadId, identity)) {
            throw new Error(
              "cannot archive a native Codex thread owned by another Zero to Agent session",
            );
          }
        ...
        if (attach && !session) {
          throw new Error("cannot attach a Codex fork without an active Zero to Agent session");
        }
