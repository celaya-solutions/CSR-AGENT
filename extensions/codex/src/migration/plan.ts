  ...
  for (const memory of params.memoryFiles) {
    ...
    if (!targetNotRegular) {
      ...
      if (isPathInside(source, destination) || isPathInside(destination, source)) {
        throw new Error(
          "Codex memory source and OpenAgent import destination must be separate paths.",
        );
      }
    }
      ...
      createMigrationItem({
        ...
        reason: targetNotRegular
          ...
          : targetConflict
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: "Copy consolidated Codex memory into the OpenAgent memory index.",
        details: {
      ...
      return createMigrationItem({
        ...
        reason: collision
          ...
          : conflict
            ? MIGRATION_REASON_TARGET_EXISTS
            : undefined,
        message: `Copy ${item.skill.sourceLabel} into this OpenAgent agent workspace.`,
    ...
    if (
      ...
      items.push(
        createMigrationItem({
          ...
          message: `Install Codex plugin "${plugin.pluginName}" in the OpenAgent-managed Codex app-server runtime.`,
          details: {
            configKey,
            marketplaceName: CODEX_PLUGINS_MARKETPLACE_NAME,
            pluginName: plugin.pluginName,
...
function buildPluginConfigItem(
  ...
  return createMigrationItem({
    ...
    applyPhase: "after-promotion",
    message:
      "Enable OpenAgent's Codex plugin integration and record migrated source-installed curated plugins.",
    details: {
