...
describe("codex conversation turn input", () => {
  ...
  it.each(localFileCases)(
    ...
    ({ scheme, field }) => {
      const imagePath = path.resolve("Zero to Agent QA", "photo #1?.png");
      expect(
        buildCodexConversationTurnInput({
          prompt: "look",
          event: {
  ...
  it("treats local media URLs as Codex local image input", () => {
    const secondImagePath = path.resolve("Zero to Agent QA", "second.jpg");
    expect(
      buildCodexConversationTurnInput({
        prompt: "look",
        event: {
    ...
    expect(
      buildCodexConversationTurnInput({
        ...
        event: {
          ...
          isGroup: false,
          metadata: {
            mediaUrl: "C:\\Zero to Agent QA\\photo.png",
            mediaType: "image/png",
          },
        },
      }),
      ...
      { type: "text", text: "look", text_elements: [] },
      { type: "localImage", path: "C:\\Zero to Agent QA\\photo.png" },
    ]);
  });
});
