...
export async function handleNativeGoal(
  ...
  if (!binding?.threadId) {
    return "No Codex thread is attached to this Zero to Agent session yet.";
  }
...
export async function startThreadAction(
  ...
  if (kind === "compact") {
    ...
    if (
      ...
      return "Codex compaction is unavailable because the current Zero to Agent session is not using the Codex runtime.";
    }
