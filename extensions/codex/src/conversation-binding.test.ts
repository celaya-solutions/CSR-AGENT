...
describe("codex conversation binding", () => {
  ...
  it("rejects binding when configured exec auto mode may need unrouted human approvals", async () => {
    ...
    await expect(
      ...
      "Zero to Agent native Codex conversation binding cannot route interactive approvals yet",
    );
  ...
  it("rejects attaching a conversation to another session's owned thread", async () => {
    ...
    ).rejects.toThrow("owned by another Zero to Agent session");
    expect(request).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the current Zero to Agent session is sandboxed", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    await writeTestConversationBinding(sessionFile, { threadId: "thread-1", cwd: tempDir });
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because Zero to Agent sandboxing is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when exec host=node is active", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because Zero to Agent exec host=node is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the binding agent uses node exec without a session key", async () => {
    ...
    expect(result?.reply?.text).toContain("Zero to Agent exec host=node is active");
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  });
  ...
  it("blocks bound Codex CLI node turns when the current Zero to Agent session is sandboxed", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex CLI node conversation binding is unavailable because Zero to Agent sandboxing is active for this session.",
        ),
      },
    });
  ...
  it("moves session history while clamping rootless inherited cwd to the agent workspace", async () => {
    ...
    expect(requests[0]?.params).toMatchObject({
      ...
      approvalPolicy: "on-request",
      approvalsReviewer: "auto_review",
      developerInstructions: expect.stringContaining("bound to a Zero to Agent conversation"),
