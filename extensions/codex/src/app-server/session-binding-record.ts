...
/** Resolves the same agent scope Zero to Agent uses for transcript/session ownership. */
...
const threadBindingSchema = z
  .object({
    ...
    // marker; public Zero to Agent session metadata must never authorize user-home access.
    ...
    // Freeze Zero to Agent-carried AGENTS.md at thread creation; bootstrap refreshes
    ...
    // this marker across resumes so Zero to Agent never substitutes a default or fallback.
    preserveNativeModel: z.literal(true).optional().catch(undefined),
    // Continue creates the Zero to Agent Chat before native execution. This closed
