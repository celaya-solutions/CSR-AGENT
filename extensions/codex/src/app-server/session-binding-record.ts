...
/** Resolves the same agent scope OpenAgent uses for transcript/session ownership. */
...
const threadBindingSchema = z
  .object({
    ...
    // marker; public OpenAgent session metadata must never authorize user-home access.
    ...
    // Freeze OpenAgent-carried AGENTS.md at thread creation; bootstrap refreshes
    ...
    // this marker across resumes so OpenAgent never substitutes a default or fallback.
    preserveNativeModel: z.literal(true).optional().catch(undefined),
    // Continue creates the OpenAgent Chat before native execution. This closed
