...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Zero to Agent dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Zero to Agent dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
export async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    const message = "Zero to Agent dynamic tool call aborted before execution.";
  ...
  const abortFromRun = () => {
    const message = "Zero to Agent dynamic tool call aborted.";
    const terminalReason = resolveCodexToolAbortTerminalReason(params.signal);
    params.onFallbackSelected?.();
  ...
  try {
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Zero to Agent dynamic tool call finished."));
    }
  }
}
...
function readDynamicToolResponseText(response: CodexDynamicToolCallResponse): string {
  const text = response.contentItems
    ...
    .join("\n")
    .trim();
  return text || "Zero to Agent dynamic tool call failed.";
}

/** Strips Zero to Agent-only metadata before sending a dynamic tool response to Codex. */
...
export function resolveDynamicToolCallTimeoutMs(params: {
  ...
  if (
    ...
    try {
      ...
      // Zero to Agent delegation uses that default for its ten-minute approval plus
