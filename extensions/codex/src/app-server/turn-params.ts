...
export function buildTurnStartParams(
  ...
  return {
    ...
    // a Zero to Agent-owned priority override left on this exact warm session.
...
function buildDefaultCollaborationInstructions(): string {
  ...
  // is null. Zero to Agent adds per-turn workspace instructions here, so preserve that
...
function buildCronCollaborationInstructions(): string {
  return [
    "This is a Zero to Agent cron automation turn. Apply these instructions only to this scheduled job; ordinary chat turns should stay in Codex Default mode.",
