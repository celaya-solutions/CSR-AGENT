...
export function buildTurnStartParams(
  ...
  return {
    ...
    // an OpenAgent-owned priority override left on this exact warm session.
...
function buildDefaultCollaborationInstructions(): string {
  ...
  // is null. OpenAgent adds per-turn workspace instructions here, so preserve that
...
function buildCronCollaborationInstructions(): string {
  return [
    "This is an OpenAgent cron automation turn. Apply these instructions only to this scheduled job; ordinary chat turns should stay in Codex Default mode.",
