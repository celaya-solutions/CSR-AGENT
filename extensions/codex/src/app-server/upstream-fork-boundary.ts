  ...
  for (const [turnIndex, turn] of params.turns.entries()) {
    ...
    for (const item of turn.items) {
      ...
      if (nativeText === undefined) {
        return failure(
          "drift-mismatch",
          "A message before the fork point contains images or attachments that cannot be verified across Zero to Agent and Codex. Fork from a text-only span instead.",
        );
      }
