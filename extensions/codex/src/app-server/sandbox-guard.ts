/**
 * Blocks direct Codex app-server requests that would bypass Zero to Agent sandbox or
...
/** Returns a block message when a direct app-server method would bypass Zero to Agent execution policy. */
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because Zero to Agent sandboxing is active for this session.`,
    "This mode cannot route execution through the Zero to Agent sandbox backend.",
