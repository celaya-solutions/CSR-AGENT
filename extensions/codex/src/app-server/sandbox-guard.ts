/**
 * Blocks direct Codex app-server requests that would bypass OpenAgent sandbox or
...
/** Returns a block message when a direct app-server method would bypass OpenAgent execution policy. */
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because OpenAgent sandboxing is active for this session.`,
    "This mode cannot route execution through the OpenAgent sandbox backend.",
