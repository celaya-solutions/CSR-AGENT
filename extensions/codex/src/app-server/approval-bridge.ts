/**
 * Bridges Codex app-server approval requests into Zero to Agent policy hooks and
 * plugin approval UX.
 */
import {
...
/** Converts a Zero to Agent approval outcome into the app-server method response. */
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `Zero to Agent native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        coerceErrorMessage(error),
      )}`,
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "Zero to Agent native hook relay failed for Codex app-server approval.",
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "Zero to Agent native hook policy denied Codex app-server approval.",
