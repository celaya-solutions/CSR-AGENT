/**
 * Bridges Codex app-server approval requests into OpenAgent policy hooks and
 * plugin approval UX.
 */
import {
...
/** Converts an OpenAgent approval outcome into the app-server method response. */
...
> {
  ...
  try {
    ...
    return {
      handled: true,
      blocked: true,
      reason: `OpenAgent native hook relay unavailable for Codex app-server approval: ${formatCodexDisplayText(
        coerceErrorMessage(error),
      )}`,
  ...
  if (!response || response.exitCode !== 0) {
    return {
      ...
      reason:
        ...
        "OpenAgent native hook relay failed for Codex app-server approval.",
  ...
  if (output?.permissionDecision === "deny") {
    return {
      ...
      reason:
        ...
        "OpenAgent native hook policy denied Codex app-server approval.",
