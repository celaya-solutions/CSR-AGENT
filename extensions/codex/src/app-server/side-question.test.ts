...
describe("runCodexAppServerSideQuestion", () => {
  ...
  it("returns an explicit unsupported decline for ordinary MCP input", async () => {
    ...
    await expect(
      ...
      content: null,
      _meta: {
        message: "OpenAgent Codex side questions do not support interactive MCP input.",
      },
    });
  ...
  it("applies native search restrictions to side forks and suppresses managed search", async () => {
    ...
    expect(toolResponse).toEqual({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown OpenAgent tool: web_search" }],
    });
    expect(toolExecuteMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects /btw before forking when the current OpenAgent session is sandboxed", async () => {
    await expect(
      runCodexAppServerSideQuestion(
        sideParams({
      ...
      "Codex-native /btw side-question mode is unavailable because OpenAgent sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because OpenAgent exec host=node is active for this session.",
    );
  ...
  it("omits computer control from side threads without a compaction owner", async () => {
    ...
    expect(toolResponse).toEqual({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown OpenAgent tool: computer" }],
    });
  });
