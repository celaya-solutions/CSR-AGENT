...
export class CodexAppServerClient {
  ...
  /** Registers a handler for app-server requests sent back to OpenAgent. */
  ...
  addNotificationHandler(handler: CodexServerNotificationHandler): () => void {
    ...
    // OpenAgent can reserve the first thread or install its shared turn router.
...
function defaultServerRequestResponse(
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "OpenAgent did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
  ...
  if (request.method === "mcpServer/elicitation/request") {
    return createCodexElicitationResponse("decline", null, {
      message: "OpenAgent has no interactive handler for this elicitation.",
    });
  }
  return {};
}
...
function timeoutServerRequestResponse(timeoutMs: number): JsonValue {
  return {
    contentItems: [
      {
        type: "inputText",
        text: `OpenAgent dynamic tool call timed out after ${timeoutMs}ms before sending a response to Codex.`,
      },
    ],
    success: false,
  };
...
class CodexAppServerVersionError extends Error {
  ...
  constructor(detectedVersion: string | undefined) {
    const detected = detectedVersion
      ...
      : "OpenAgent could not determine the running Codex version";
    super(
...
function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // OpenAgent, Codex Desktop, or an env override, so only the slash-delimited
...
/** Returns true for app-server approval request methods OpenAgent can answer. */
