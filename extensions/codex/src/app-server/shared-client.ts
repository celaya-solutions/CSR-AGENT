...
async function resolveCodexAppServerClientStartContext(
  ...
  if (preparedAuth?.kind === "profile" && !preparedAuth.store.profiles[preparedAuth.profileId]) {
    throw new Error(
      `Prepared Codex auth profile "${preparedAuth.profileId}" was not found. Select an existing OpenAI profile or sign in again with Zero to Agent, then retry.`,
    );
  }
...
export function captureCodexAppServerClientLifetime(
  ...
  if (
    ...
    throw new AgentHarnessPreflightError(
      "Codex ordinary configuration refresh requires a Zero to Agent-managed local stdio process, not an external socket or app-server proxy. No turn was sent; reconnect through managed local stdio before continuing.",
    );
  }
