...
function resolveUnimportedAgentCodexAuthMessage(params: {
  ...
  // Zero to Agent-owned starts force ephemeral Codex auth, so this file would otherwise be
...
export async function resolveCodexAppServerPreparedAuthHandoff(params: {
  ...
  // token logins, so a prepared Zero to Agent handoff here would rewrite the account that
...
async function reconcileCodexComputerUseStartArtifactsOnce(params: {
  ...
  if (shouldProvisionComputerUse && !provisioningAgentDir) {
    throw new Error("Managed Codex Computer Use requires a Zero to Agent agent directory");
  }
...
async function assertNativeCodexAccountMatchesRoute(
  ...
  if (authRequirement === "subscription") {
    if (accountType !== "chatgpt") {
      throw createCodexAppServerAuthError(
        'Codex subscription route requires ChatGPT auth in the native Codex home. Run `codex login` for that home, or use appServer.homeScope="agent" with a Zero to Agent OAuth profile, then retry.',
      );
    }
    return;
  }
  if (accountType === "chatgpt") {
    throw createCodexAppServerAuthError(
      'Codex Platform route requires an API-key account, but the native Codex home is signed in with a ChatGPT subscription. Sign that home in with `codex login --with-api-key`, or set appServer.homeScope="agent" so Zero to Agent can inject its own key.',
    );
  }
}
...
async function resolveCodexAppServerAuthProfileLoginParams(params: {
  ...
  if (profileId && !profile) {
    throw new CodexAppServerAuthProfileUnavailableError(
      `Codex app-server auth profile "${profileId}" was not found. Select an existing OpenAI profile or sign in again with Zero to Agent, then retry.`,
    );
  }
...
async function resolveOAuthCredentialForCodexAppServer(
  ...
  if (
    ...
    throw new CodexAppServerAuthProfileUnavailableError(
      `Codex app-server auth profile "${profileId}" is no longer available. Sign in again with Zero to Agent, then retry.`,
    );
  }
  ...
  if (params.forceRefresh && !persistedOAuthCredential && overlaidOAuthCredential) {
    ...
    if (!refreshedRuntimeCredential?.access?.trim()) {
      throw new Error(
        `Codex app-server auth profile "${profileId}" could not refresh. Sign in again with Zero to Agent, then retry.`,
      );
    }
...
async function resolveScopedOAuthCredential(params: {
  ...
  const refresh = (async () => {
    ...
    if (!refreshed?.access?.trim()) {
      throw new Error(
        `Codex app-server auth profile "${params.profileId}" could not refresh. Sign in again with Zero to Agent, then retry.`,
      );
    }
    assertCodexOAuthRefreshWorkspace(params.profileId, refreshed, params.expectedAccountId);
