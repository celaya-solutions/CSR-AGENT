...
describe("runCodexAppServerAttempt", () => {
  ...
  it("starts active Zero to Agent sandbox threads with Codex native execution disabled", async () => {
  ...
  it("routes native Codex execution through a Zero to Agent sandbox exec-server when opted in", async () => {
    const appServer = {
      ...createThreadLifecycleAppServerOptions(),
      sandbox: "danger-full-access" as const,
    };
  ...
  it("starts Codex threads without duplicate Zero to Agent workspace tools by default", async () => {
    const { sessionFile, workspaceDir } = createRunPaths();
  ...
  it("includes Codex app-server scoped plugin command guidance in developer instructions", () => {
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "Zero to Agent main command guidance.",
    ...
    expect(instructions).not.toContain("Zero to Agent main command guidance.");
  });
  it("passes Zero to Agent skills as turn collaboration developer instructions", async () => {
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).toContain("## Zero to Agent Skills");
    expect(collaborationInstructions).toContain("<available_skills>");
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).not.toContain("## Zero to Agent Skills");
    ...
    expect(compiledContext?.data?.systemPrompt).toContain("## Zero to Agent Skills");
  ...
  it("emits TUI-compatible tool events for Codex dynamic tool calls", async () => {
    ...
    expect(firstResponse).toMatchObject({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Zero to Agent tool: python" }],
    });
    ...
    expect(onRunAgentEvent).toHaveBeenCalledWith({
      ...
      data: {
        ...
        isError: true,
        result: {
          content: [{ type: "text", text: "Unknown Zero to Agent tool: python" }],
        },
      },
    });
  ...
  it("keeps leading delivery hints out of the Codex current user request", async () => {
    for (const [index, deliveryHint] of MESSAGE_TOOL_DELIVERY_HINTS.entries()) {
      ...
      const inputText = turnStartParams.input?.[0]?.text ?? "";
      expect(inputText).toContain("Zero to Agent delivery metadata:");
  ...
  it("keeps Zero to Agent control-path tools direct when code-mode-only is enabled", () => {
    const tools = [
      createRuntimeDynamicTool("message"),
  ...
  it("keeps the heartbeat schema deferred and stable across normal and heartbeat turns", async () => {
    ...
    expect(normalInstructions).not.toContain(
      "Deferred searchable Zero to Agent dynamic tools available: heartbeat_respond",
    );
  ...
  it("keeps message in the registered schema when disabled for an internal turn", async () => {
    ...
    await expect(
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Zero to Agent tool is not available for this turn: message",
        },
      ],
    });
  });
  ...
  it("projects bounded continuity when starting Codex without a native thread binding", async () => {
    ...
    const inputText =
      ...
      "";
    expect(inputText).toContain("Zero to Agent assembled context for this turn:");
  ...
  it("does not replay mirrored history already covered by an existing Codex binding", async () => {
    ...
    const inputText =
      (turnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]?.text ??
      "";
    expect(inputText).not.toContain("Zero to Agent assembled context for this turn:");
  ...
  it("does not project mirrored messages on consecutive resumes", async () => {
    ...
    const firstInputText =
      (firstTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(firstInputText).toContain("Zero to Agent assembled context for this turn:");
    ...
    const secondInputText =
      (secondTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(secondInputText).not.toContain("Zero to Agent assembled context for this turn:");
  ...
  it("routes AGENTS.md natively and MEMORY.md through tools", async () => {
    ...
    expect(collaborationInstructions).toContain("Zero to Agent Agent Soul");
    ...
    expect(collaborationInstructions).toContain("Zero to Agent Workspace Memory");
    ...
    expect(inputText).not.toContain("Zero to Agent runtime context for this turn:");
    ...
    expect(inputText).not.toContain("Zero to Agent Workspace Memory");
  ...
  it("adds memory recall guidance when dated memory notes exist without root MEMORY.md", async () => {
    ...
    expect(collaborationInstructions).toContain("memory_search");
    expect(collaborationInstructions).toContain("memory_get");
    expect(collaborationInstructions).not.toContain("Zero to Agent Workspace Memory");
  ...
  it("inherits external-cwd agent instructions through Codex thread creation", async () => {
    ...
    expect(threadInstructions).toContain("Zero to Agent Agent Workspace Instructions");
  ...
  it.each([
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).not.toContain("This is a Zero to Agent heartbeat turn");
  ...
  it("keeps lightweight cron Codex turns out of Zero to Agent bootstrap context", async () => {
    const { sessionFile, workspaceDir } = createRunPaths();
    const exactCommand =
  ...
  it("keeps lightweight cron delivery hints byte-for-byte without Zero to Agent prompt context", async () => {
  ...
  it("keeps effective default Codex yolo when Zero to Agent tool policy exists", async () => {
    initializeGlobalHookRunner(
  ...
  it("keeps explicit Codex yolo mode unpromoted when Zero to Agent tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const { sessionFile, workspaceDir } = createRunPaths();
  ...
  it("keeps normalized full exec mode unpromoted when Zero to Agent tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const { sessionFile, workspaceDir } = createRunPaths();
