...
describe("bridgeCodexAppServerStartOptions", () => {
  ...
  it("does not inject native CLI OAuth without an OpenAgent profile", async () => {
  ...
  it("does not borrow OS-home native OAuth for an isolated OpenAgent home", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-codex-app-server-"));
  ...
  it("requires an OpenAgent profile instead of refreshing the native CLI store", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-codex-app-server-"));
    const agentDir = path.join(root, "agent");
    const codexHome = path.join(root, "codex-cli");
