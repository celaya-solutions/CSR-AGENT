...
export function ensureCodexAppServerClientRuntime(
  ...
  client.addRequestHandler(async (request) => {
    ...
    if (!runtime.context.agentDir) {
      throw new Error("ChatGPT token refresh requires an OpenAgent-owned auth profile.");
    }
    ...
    try {
      const tokens = await withTimeout(
        ...
        "Codex app-server ChatGPT token refresh timed out before its external-auth deadline. Retry the request; if it persists, sign in again with OpenAgent.",
      );
