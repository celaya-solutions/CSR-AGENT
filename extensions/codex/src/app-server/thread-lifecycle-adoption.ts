...
async function preparePendingCodexThreadResume(
  ...
  const fail = (reason: string) =>
    new Error(
      `Cannot configure resumed Codex thread ${binding.threadId}: ${reason}. ` +
        "The thread is preserved; continue it in native Codex or use /new for the current Zero to Agent tools.",
    );
  ...
  if (
    ...
    throw fail("configuration adoption requires a Zero to Agent-owned local Codex home");
  }
  ...
  try {
    ...
    if (
      ...
      throw fail(
        "its immutable native tool catalog does not match the current Zero to Agent tools",
      );
    }
    assertCurrent();
    return {
...
function observeCodexThreadConfiguration(
  ...
  return {
    ...
    assertConfigured: () => {
      ...
      // reservation against native-internal reloads outside Zero to Agent's thread queue.
      if (!unloaded) {
        throw new Error(
