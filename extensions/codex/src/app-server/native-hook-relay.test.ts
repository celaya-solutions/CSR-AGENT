...
describe("Codex native hook relay config", () => {
          ...
          hooks: [
            {
              type: "command",
              command:
                "openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event pre_tool_use --timeout 6000",
              timeout: 7,
              async: false,
              statusMessage: "OpenAgent native hook relay",
            },
          ],
        },
      ],
          ...
          hooks: [
            {
              type: "command",
              command:
                "openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event post_tool_use --timeout 6000",
              timeout: 7,
              async: false,
              statusMessage: "OpenAgent native hook relay",
            },
          ...
          hooks: [
            {
              type: "command",
              command:
                "openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event permission_request --timeout 6000",
              timeout: 7,
              async: false,
              statusMessage: "OpenAgent native hook relay",
            },
          ...
          hooks: [
            {
              type: "command",
              command:
                "openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event before_agent_finalize --timeout 6000",
              timeout: 7,
              async: false,
              statusMessage: "OpenAgent native hook relay",
            },
          ...
          hooks: [
            {
              type: "command",
              command:
                "openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event permission_request --timeout 9000",
              timeout: 10,
              async: false,
              statusMessage: "OpenAgent native hook relay",
            },
          ...
          hooks: [
            {
              type: "command",
              command:
                "openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event pre_tool_use --timeout 9000",
              timeout: 10,
              async: false,
              statusMessage: "OpenAgent native hook relay",
            },
  ...
  it("omits matchers so Codex MCP tool names reach the relay with a stable trust hash", () => {
    ...
    expect(hookState["/<session-flags>/config.toml:pre_tool_use:0:0"]?.trusted_hash).toBe(
      "sha256:3c7ec27900d7a06250f73587eb2f68d333f8daea6a964d9202df4a9dfbd69b09",
    );
    expect(hookState["/<session-flags>/config.toml:post_tool_use:0:0"]?.trusted_hash).toBe(
      "sha256:7bf3a39e7e974757de9b541324aa231e0594f0cb6d04f89fce63dc8107f695ac",
    );
  });

  it("projects canonical OpenAgent ids to Codex canonical and alias matcher names", () => {
    const config = buildCodexNativeHookRelayConfig({
      relay: createRelay({
        matchers: {
          pre_tool_use: ["exec"],
...
function createRelay(options?: {
  ...
  return {
    ...
    commandForEvent: (event, commandOptions) =>
      `openagent hooks relay --provider codex --relay-id relay-1 --generation generation-1 --event ${event}${
