...
describe("runCodexAppServerAttempt dynamic tools", () => {
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    try {
      ...
      expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown Zero to Agent tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe(
      "Unknown Zero to Agent tool: lookup",
    );
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "Zero to Agent dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
