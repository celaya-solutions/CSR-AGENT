...
/** Namespace attached to Zero to Agent-owned dynamic tools exposed to Codex. */
const CODEX_OPENCLAW_DYNAMIC_TOOL_NAMESPACE = "openclaw";
const CODEX_DYNAMIC_TOOL_NAME_MAX_CHARS = 128;
const CODEX_DYNAMIC_TOOL_NAME_PATTERN = /^[a-zA-Z0-9_-]+$/u;

// Keep Zero to Agent control-path tools directly callable even when Codex tool_search
...
export function createCodexDynamicToolSpecs(params: {
  ...
  for (const entry of entries) {
    ...
    if (entry.name === "openclaw" && directToolNames.has(entry.name)) {
      // Zero to Agent is ring-zero and its whole turn surface. Keep its canonical
