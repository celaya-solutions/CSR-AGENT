...
describe("Codex usage attribution", () => {
  it("labels new Zero to Agent threads without rewriting resumed thread attribution", () => {
...
describe("Codex app-server native code mode config", () => {
  it("keeps Codex-native subagents primary while limiting Zero to Agent spawn to Zero to Agent delegation", () => {
    const instructions = buildDeveloperInstructions(createAttemptParams({ provider: "openai" }), {
      dynamicTools: [
        {
          type: "function",
          name: "sessions_spawn",
          description: "Start a Zero to Agent session",
    ...
    expect(instructions).toContain(
      "Use Zero to Agent `sessions_spawn` only for Zero to Agent or ACP delegation, never as a substitute for `spawn_agent` on internal legwork.",
    );
  });
  ...
  it("summarizes deferred dynamic tool names in developer instructions", () => {
    ...
    expect(instructions).toContain(
      "Deferred searchable Zero to Agent dynamic tools available: image_generate, music_generate.",
    );
    expect(instructions).toContain("Use `tool_search` when directly callable");
    expect(instructions).toContain(
      "On code-mode-only models, use `exec` instead: filter `ALL_TOOLS` by name and description",
  ...
  it("materializes namespaced prompt tools once while preserving all guidance", () => {
    ...
    expect(namespaceReads).toBe(1);
    expect(instructions).toContain(
      "Deferred searchable Zero to Agent dynamic tools available: alpha_tool, skill_workshop, zeta_tool.",
    );
  ...
  it("keeps developer instructions compact when no dynamic tools are deferred", () => {
    ...
    expect(instructions).not.toContain("Deferred searchable Zero to Agent dynamic tools available");
  });
  ...
  it("keeps Zero to Agent skill catalogs out of developer instructions", () => {
    const params = createAttemptParams({ provider: "openai" });
  ...
  it("omits Zero to Agent model selection when adopting a native Codex thread", () => {
    const request = buildThreadResumeParams(createAttemptParams({ provider: "codex" }), {
  ...
  it("preserves the Zero to Agent default for Codex's materialized unauthored default", () => {
...
describe("Codex app-server turn params", () => {
  it("builds resume and turn params from the currently selected Zero to Agent model", () => {
    const params = createAttemptParams({ provider: "codex" });
  ...
  it("keeps heartbeat Codex turns in normal Default collaboration mode", () => {
    ...
    expect(workspaceInstructions).not.toContain("This is a Zero to Agent heartbeat turn");
  ...
  it("uses turn-scoped collaboration instructions for cron Codex turns", () => {
    ...
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
      "This is a Zero to Agent cron automation turn",
    );
    expect(cronCollaborationMode.settings.developer_instructions).toContain(
...
describe("Codex app-server adopted thread lifecycle", () => {
  ...
  it("keeps Zero to Agent from overriding App Server model selection across resumes", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createThreadLifecycleParams(sessionFile, workspaceDir);
    const { identity, threadId } = await seedAdoptedThreadBinding(params, workspaceDir);
