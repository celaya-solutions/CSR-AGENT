...
describe("Codex app-server dynamic tool build", () => {
  ...
  it("keeps policy-filterable OpenAgent coding replacements when native tools are disabled", () => {
    const tools = [
      "read",
      "write",
      "edit",
  ...
  it.each([
    ...
    const gatewayExec = expectDefined(
      ...
      `${testCase.mode} OpenAgent shell replacement`,
    );
  ...
  it("preserves the host-provided OpenAgent tool through the Codex allowlist", async () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
  ...
  it("exposes OpenAgent sandbox shell tools under distinct names for non-Docker sandbox backends", async () => {
  ...
  it("exposes Docker sandbox shell tools when OpenAgent sandboxing disables native Code Mode", async () => {
    setOpenClawCodingToolsFactoryForTests(() => [
      createRuntimeDynamicTool("exec"),
      createRuntimeDynamicTool("process"),
      createRuntimeDynamicTool("message"),
  ...
  it("keeps a pinned Gateway shell path beside Codex native shell", async () => {
    ...
    expect(gatewayExec?.description).toContain("OpenAgent-managed Gateway environment access");
  ...
  it("does not expose Gateway process sessions as remote-node control in auto host runs", async () => {
    ...
    expect(nodeList.contentItems).toEqual([
      { type: "inputText", text: "Unknown OpenAgent tool: node_process" },
    ]);
    const nodeExec = tools.find((tool) => tool.name === "node_exec");
  ...
  it("restores the policy-filtered OpenAgent shell when a finite allowlist disables native Code Mode", async () => {
  ...
  it.each(["ultra", "off"] as const)(
    "passes active %s thinking into shared OpenAgent tool construction",
    async (thinkLevel) => {
      const sessionFile = path.join(tempDir, "session.jsonl");
      const workspaceDir = path.join(tempDir, "workspace");
      const params = createParams(sessionFile, workspaceDir);
  ...
  it("disables Codex native tool surfaces whenever an OpenAgent sandbox is active", () => {
    const workspaceDir = path.join(tempDir, "workspace");
    const params = createParams(path.join(tempDir, "session.jsonl"), workspaceDir);
    params.disableTools = false;
