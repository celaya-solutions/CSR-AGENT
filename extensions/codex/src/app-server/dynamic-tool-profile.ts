...
/** Tool names owned by Codex app-server and normally excluded from Zero to Agent dynamic tools. */
...
/** Normalizes Zero to Agent/Codex tool names before filtering and allowlist checks. */
export function normalizeCodexDynamicToolName(name: string): string {
  const normalized = name.trim().toLowerCase();
  return DYNAMIC_TOOL_NAME_ALIASES[normalized] ?? normalized;
}

/** True only for the host-scoped Zero to Agent run's exact tool contract. */
...
/** Filters Zero to Agent tools that Codex owns natively or config explicitly excludes. */
...
/** Keeps Zero to Agent coding tools that replace a disabled Codex native surface. */
