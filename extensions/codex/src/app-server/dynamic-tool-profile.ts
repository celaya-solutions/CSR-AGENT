...
/** Tool names owned by Codex app-server and normally excluded from OpenAgent dynamic tools. */
...
/** Normalizes OpenAgent/Codex tool names before filtering and allowlist checks. */
export function normalizeCodexDynamicToolName(name: string): string {
  const normalized = name.trim().toLowerCase();
  return DYNAMIC_TOOL_NAME_ALIASES[normalized] ?? normalized;
}

/** True only for the host-scoped OpenAgent run's exact tool contract. */
...
/** Filters OpenAgent tools that Codex owns natively or config explicitly excludes. */
...
/** Keeps OpenAgent coding tools that replace a disabled Codex native surface. */
