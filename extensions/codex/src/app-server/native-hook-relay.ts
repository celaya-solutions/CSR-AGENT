/**
 * Bridges Codex native hook callbacks into Zero to Agent's native hook relay so
 * app-server tool events can still run Zero to Agent policy and diagnostics.
 */
...
/** Codex hook events that can be registered through Zero to Agent's native relay. */
...
/** Enterprise managed-only policy silently drops the session-layer hooks that enforce Zero to Agent. */
  ...
  if (!attestation) {
    attestation = client
      ...
      .then((response) => {
        ...
        if (managedOnly === true) {
          throw new Error(
            "Codex managed-only hooks disable the Zero to Agent native hook relay; refusing unenforced execution",
          );
        }
      });
...
/** Registers a Zero to Agent native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian Zero to Agent plugin approval prompt.
        ...
        hooks: [
          {
            ...
            timeout,
            async: false,
            statusMessage: "Zero to Agent native hook relay",
          },
        ],
      },
    ] satisfies JsonValue;
...
function buildCodexNativeToolMatcher(toolNames: readonly string[] | undefined): string | undefined {
  ...
  for (const toolName of toolNames) {
    ...
    if (!canonicalToolName || canonicalToolName === "*") {
      throw new TypeError("Codex native hook matcher requires canonical Zero to Agent tool ids");
    }
