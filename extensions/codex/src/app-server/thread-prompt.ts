...
export function buildDeveloperInstructions(
  ...
  const sections = [
    ...
    deferredToolNames.size > 0
      ? `Deferred searchable Zero to Agent dynamic tools available: ${[...deferredToolNames]
    ...
    nativeDelegationAvailable
      ? `Use Codex native \`spawn_agent\` for Codex subagents. \`spawn_agent\` and the other native collaboration tools may be deferred.${hasSessionsSpawn ? " Use Zero to Agent `sessions_spawn` only for Zero to Agent or ACP delegation, never as a substitute for `spawn_agent` on internal legwork." : ""}`
      : undefined,
