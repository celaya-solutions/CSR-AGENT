...
describe("Codex plugin status command", () => {
  ...
  it("keeps hosted management separate from local permission and callable tools", async () => {
    ...
    expect(result.text).toContain("Zero to Agent app access: disabled");
  ...
  it.each([
    {
      ...
      expected: "Zero to Agent app access: disabled",
...
describe("Codex hosted app refresh", () => {
  ...
  it.each([
    { options: { disabled: true }, expected: "Zero to Agent app access: disabled" },
