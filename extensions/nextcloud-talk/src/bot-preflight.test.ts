...
describe("probeNextcloudTalkBotResponseFeature", () => {
  ...
  it.each([1 | 2 | 8, "+011"])(
    ...
    async (features) => {
      ...
      await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
        ...
        features: 11,
        message: 'Nextcloud Talk bot "Zero to Agent" has the response feature.',
      });
    },
  );
  ...
  it("reports missing response feature for the matching webhook bot", async () => {
    ...
    await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
      ...
      features: 9,
      message:
        'Nextcloud Talk bot "Zero to Agent" (7) is missing the response feature (features=9); outbound replies will fail. Run ./occ talk:bot:state --feature webhook --feature response --feature reaction 7 1 or reinstall the bot with --feature response.',
    });
  });
  ...
  it.each(["2response", -1])(
    ...
    async (features) => {
      ...
      await expect(probeNextcloudTalkBotResponseFeature({ account: account() })).resolves.toEqual({
        ...
        botName: "Zero to Agent",
        message:
          'Nextcloud Talk bot "Zero to Agent" (7) is missing the response feature; outbound replies will fail. Run ./occ talk:bot:state --feature webhook --feature response --feature reaction 7 1 or reinstall the bot with --feature response.',
      });
    },
  );
