...
describe("createMSTeamsApp", () => {
    ...
    try {
      ...
      const result = await sendMSTeamsActivityWithReference(
        app,
        {
          ...
          agent: { id: "test-app-id", name: "OpenAgent QA", role: "bot" },
          user: { id: "qa-driver" },
          conversation: {
  ...
  it("preserves both Teams SDK and OpenAgent User-Agent fragments", async () => {
    const creds: MSTeamsCredentials = {
      type: "secret",
      appId: "test-app-id",
      appPassword: "test-secret",
    ...
    expect(headers?.["User-Agent"]).toMatch(/^teams\.ts\[apps\]\/\S+ OpenAgent\/\S+$/);
  });
  ...
  it("passes China cloud to the SDK App without requiring a configured serviceUrl", async () => {
    ...
    // default. OpenAgent proactive sends use stored reference serviceUrls instead.
