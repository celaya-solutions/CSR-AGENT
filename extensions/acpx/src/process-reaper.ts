 ...
 * OpenAgent-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const OWNED_ACP_PACKAGE_NAMES = [
  ...
  // recognition until their OpenAgent-owned wrapper/process tree is gone.
...
/** Check whether a command references an OpenAgent-generated ACPX wrapper path. */
...
/** Check whether a command is owned by OpenAgent ACPX runtime packages or wrappers. */
...
/** Terminate one validated OpenAgent-owned ACPX wrapper process tree. */
export async function cleanupOpenClawOwnedAcpxProcessTree(params: {
  ...
  // that this PID still belongs to an OpenAgent-owned wrapper, fail closed to
...
/** Reap orphaned OpenAgent-owned ACPX wrapper trees during runtime startup. */
