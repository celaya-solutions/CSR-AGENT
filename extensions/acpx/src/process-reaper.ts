 ...
 * Zero to Agent-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const OWNED_ACP_PACKAGE_NAMES = [
  ...
  // recognition until their Zero to Agent-owned wrapper/process tree is gone.
...
/** Check whether a command references a Zero to Agent-generated ACPX wrapper path. */
...
/** Check whether a command is owned by Zero to Agent ACPX runtime packages or wrappers. */
...
/** Terminate one validated Zero to Agent-owned ACPX wrapper process tree. */
export async function cleanupOpenClawOwnedAcpxProcessTree(params: {
  ...
  // that this PID still belongs to a Zero to Agent-owned wrapper, fail closed to
...
/** Reap orphaned Zero to Agent-owned ACPX wrapper trees during runtime startup. */
