...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Zero to Agent's isolated CODEX_HOME wrapper, but launch the plugin-local
  ...
  for (const openClawCodexConfig of openClawCodexConfigs) {
    try {
      ...
      if (
        ...
        throw new Error("invalid Zero to Agent Codex config");
      }
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Zero to Agent; fallback must not float to an unpatched newer release.
