...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep OpenAgent's isolated CODEX_HOME wrapper, but launch the plugin-local
  ...
  for (const openClawCodexConfig of openClawCodexConfigs) {
    try {
      ...
      if (
        ...
        throw new Error("invalid OpenAgent Codex config");
      }
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in OpenAgent; fallback must not float to an unpatched newer release.
