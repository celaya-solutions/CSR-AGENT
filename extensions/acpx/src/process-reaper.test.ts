...
describe("process reaper", () => {
  ...
  it("does not kill a reused pid when the live command is not Zero to Agent-owned", async () => {
  ...
  it("reaps stale Zero to Agent-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
