...
describe("process reaper", () => {
  ...
  it("does not kill a reused pid when the live command is not OpenAgent-owned", async () => {
  ...
  it("reaps stale OpenAgent-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
