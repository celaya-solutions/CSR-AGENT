...
export function meetStatusScript(params: {
  ...
  const routeMeetAudioInput = async () => {
    ...
    try {
      ...
      audioInputDeviceLabel = input.label || 'OpenAgent meeting audio';
  ...
  const routeMeetAudioOutput = async () => {
    ...
    try {
      ...
      audioOutputDeviceLabel = output.label || "OpenAgent meeting audio";
  ...
  const captionState = (() => {
    ...
    // A reused tab starts a fresh logical transcript for each OpenAgent session.
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    manualAction = manualActionFor("google-login-required", "Sign in to Google in the OpenAgent browser profile, then retry the Meet join.");
    ...
    manualAction = manualActionFor("meet-admission-required", "Admit the OpenAgent browser participant in Google Meet, then retry speech.");
  } else if (permissionNeeded) {
    manualAction = manualActionFor("meet-permission-required", allowMicrophone ? "Allow microphone/camera/speaker permissions for Meet in the OpenAgent browser profile, then retry." : "Join without microphone/camera permissions in the OpenAgent browser profile, then retry.");
    ...
    manualAction = manualActionFor(
      ...
      "Select BlackHole 2ch or OpenAgent Meeting Audio as both the Meet microphone and speaker, then retry."
    );
    ...
    manualAction = manualActionFor("meet-audio-choice-required", allowMicrophone ? "Meet is showing the microphone choice. Click Use microphone in the OpenAgent browser profile, then retry." : "Meet is showing the microphone choice. Choose the no-microphone option in the OpenAgent browser profile, then retry.");
  }
