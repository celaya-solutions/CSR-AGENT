...
export function meetStatusScript(params: {
  ...
  const routeMeetAudioInput = async () => {
    ...
    try {
      ...
      audioInputDeviceLabel = input.label || 'Zero to Agent meeting audio';
  ...
  const routeMeetAudioOutput = async () => {
    ...
    try {
      ...
      audioOutputDeviceLabel = output.label || "Zero to Agent meeting audio";
  ...
  const captionState = (() => {
    ...
    // A reused tab starts a fresh logical transcript for each Zero to Agent session.
  ...
  if (!inCall && (host === "accounts.google.com" || /use your google account|to continue to google meet|choose an account|sign in to (join|continue)/i.test(pageText))) {
    manualAction = manualActionFor("google-login-required", "Sign in to Google in the Zero to Agent browser profile, then retry the Meet join.");
    ...
    manualAction = manualActionFor("meet-admission-required", "Admit the Zero to Agent browser participant in Google Meet, then retry speech.");
  } else if (permissionNeeded) {
    manualAction = manualActionFor("meet-permission-required", allowMicrophone ? "Allow microphone/camera/speaker permissions for Meet in the Zero to Agent browser profile, then retry." : "Join without microphone/camera permissions in the Zero to Agent browser profile, then retry.");
    ...
    manualAction = manualActionFor(
      ...
      "Select BlackHole 2ch or Zero to Agent Meeting Audio as both the Meet microphone and speaker, then retry."
    );
    ...
    manualAction = manualActionFor("meet-audio-choice-required", allowMicrophone ? "Meet is showing the microphone choice. Click Use microphone in the Zero to Agent browser profile, then retry." : "Meet is showing the microphone choice. Choose the no-microphone option in the Zero to Agent browser profile, then retry.");
  }
