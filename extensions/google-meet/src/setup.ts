...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the Zero to Agent browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the Zero to Agent browser profile; configure browser.defaultProfile to choose another profile",
  });
