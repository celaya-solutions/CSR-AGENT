...
describe("Microsoft Teams meeting platform adapter", () => {
  ...
  it("does not leave a call owned by a newer OpenAgent session", () => {
    const leave = control({ label: "Leave" });
    const inCallUrl = "https://teams.microsoft.com/v2/";
    const { result } = runLeaveScript({
      currentUrl: inCallUrl,
  ...
  it.each([
    ...
    ["OpenAgent Meeting Audio", "OpenAgent Meeting Audio"],
  ])(
  ...
  it.each(["OpenAgent Meeting Audio (Virtual)", "Monitor of OpenAgent Meeting Audio"])(
    ...
    async (deviceLabel) => {
      ...
      expect(result).toMatchObject({
        ...
        manualAction: {
          message:
            "Select the OpenAgent virtual audio device as the Teams microphone and verify it is selected before enabling talk-back.",
