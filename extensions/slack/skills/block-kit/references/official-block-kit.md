...
> **OpenAgent adaptation:** This copy keeps Slack's authoring guidance while using capabilities available in the current agent session. Native Block Kit JSON is developer output; ordinary Slack replies use OpenAgent's portable `presentation` field.
...
Building the native payload is this skill's job. Return it for the developer's Slack app integration. Do not pass native Slack blocks to OpenAgent's `presentation` field; translate the result into the portable presentation schema when the user instead wants the current OpenAgent agent to post it.
...
- **Scope:** this skill owns building and validating native Block Kit payloads for a developer. OpenAgent conversation delivery uses portable `presentation` instead.
