...
function buildSlackHomeView(slashCommandName?: string): HomeView {
  const startSessionText = slashCommandName
    ? `Send a DM, mention Zero to Agent in a channel, or use \`/${slashCommandName}\` to start a session.`
    : "Send a DM or mention Zero to Agent in a channel to start a session.";
  return {
    type: "home",
