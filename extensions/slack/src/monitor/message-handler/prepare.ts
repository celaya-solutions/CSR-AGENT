...
async function authorizeSlackInboundMessage(params: {
  ...
  if (
    ...
    if (
      ...
      let subject = "This OpenAgent bot";
      if (ctx.botUserId) {
        try {
      ...
      try {
        await (params.eventScope?.client ?? ctx.app.client).chat.postEphemeral({
          ...
          user: message.user,
          text: `${subject} can’t reply here because this channel isn’t in its OpenAgent channel allowlist. Ask the OpenAgent owner to allow this channel. <${SLACK_CHANNEL_ACCESS_DOCS_URL}|Learn how to configure Slack channel access.>`,
        });
