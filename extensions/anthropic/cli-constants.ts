...
/** Retired Zero to Agent auth profile replaced by Claude CLI's native login. */
...
/** Non-secret marker telling Zero to Agent that the installed Claude CLI owns auth. */
...
// steer Zero to Agent-managed Claude CLI runs toward a different provider,
...
/** Environment variables removed before launching Zero to Agent-managed Claude CLI runs. */
export const CLAUDE_CLI_CLEAR_ENV = [
  ...
  // Re-injected per run from Zero to Agent's canonical context budget.
  ...
  // Re-injected per run from Zero to Agent's effective thinking level.
  "MAX_THINKING_TOKENS",
