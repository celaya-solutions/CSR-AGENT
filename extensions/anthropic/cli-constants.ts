...
/** Retired OpenAgent auth profile replaced by Claude CLI's native login. */
...
/** Non-secret marker telling OpenAgent that the installed Claude CLI owns auth. */
...
// steer OpenAgent-managed Claude CLI runs toward a different provider,
...
/** Environment variables removed before launching OpenAgent-managed Claude CLI runs. */
export const CLAUDE_CLI_CLEAR_ENV = [
  ...
  // Re-injected per run from OpenAgent's canonical context budget.
  ...
  // Re-injected per run from OpenAgent's effective thinking level.
  "MAX_THINKING_TOKENS",
