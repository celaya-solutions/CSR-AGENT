...
describe("Claude session catalog", () => {
  ...
  it("adopts a local CLI row with a locked one-shot fork binding", async () => {
    ...
    expect(createSessionEntry).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        // because Zero to Agent labels are unique and duplicate CLI titles must adopt.
  ...
  ])("links a catalog row to an existing Zero to Agent session via $label", async ({ entry }) => {
    const home = await createHome();
    process.env.HOME = home;
