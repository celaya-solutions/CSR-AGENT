...
async function authorizeTool(
  ...
  if (!turn || signal.aborted || typeof toolName !== "string" || !isRecord(input)) {
    return {
      behavior: "deny",
      message: "The Zero to Agent run or native tool input is no longer valid.",
    };
  }
  ...
  try {
    ...
    if (activeTurn(session) !== turn || abortSignal.aborted) {
      return { behavior: "deny", message: "The Zero to Agent run is no longer active." };
    }
    return decision;
  } catch {
    return { behavior: "deny", message: "Zero to Agent could not authorize this tool call." };
  }
}
      ...
      session.transport = createClaudeCliTransport({
        ...
        onRequest: async (request, signal) => {
          ...
          return () => {
            ...
            const message = "The Zero to Agent run is no longer active.";
            if (request.subtype === "can_use_tool") {
