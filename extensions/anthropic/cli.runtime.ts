...
async function authorizeTool(
  ...
  if (!turn || signal.aborted || typeof toolName !== "string" || !isRecord(input)) {
    return {
      behavior: "deny",
      message: "The OpenAgent run or native tool input is no longer valid.",
    };
  }
  ...
  try {
    ...
    if (activeTurn(session) !== turn || abortSignal.aborted) {
      return { behavior: "deny", message: "The OpenAgent run is no longer active." };
    }
    return decision;
  } catch {
    return { behavior: "deny", message: "OpenAgent could not authorize this tool call." };
  }
}
      ...
      session.transport = createClaudeCliTransport({
        ...
        onRequest: async (request, signal) => {
          ...
          return () => {
            ...
            const message = "The OpenAgent run is no longer active.";
            if (request.subtype === "can_use_tool") {
