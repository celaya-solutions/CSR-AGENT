...
describe("Claude CLI user input adapter", () => {
  ...
  it("guides a malformed question retry without replaying the rejected call", async () => {
    ...
    const rejected = {
      behavior: "deny",
      message:
        "OpenAgent rejected malformed Claude user questions: questions[0].header must be at most 12 characters. Correct the invalid field and retry AskUserQuestion.",
    };
  ...
  it.each([
    ...
    async (_case, malformedInput, expectedDetail) => {
      ...
      expect(result).toEqual({
        behavior: "deny",
        message: `OpenAgent rejected malformed Claude user questions: ${expectedDetail}. Correct the invalid field and retry AskUserQuestion.`,
      });
      expect(requestUserInput).not.toHaveBeenCalled();
    },
  );
