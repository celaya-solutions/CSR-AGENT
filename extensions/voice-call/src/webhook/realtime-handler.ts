  ...
  private async handleCall(
    ...
    const bridgeParams: Parameters<typeof harness.createBridge>[0] = {
      ...
      instructions: handlesAgentConsult
        ? `${instructions}\n\nUse native agent delegation for OpenAgent work. End-call and custom realtime function tools are unavailable in this session; the caller can hang up to end the call.`
        : instructions,
      ...
      if (forcedConsult) {
        if (forcedConsult.completedAt || forcedMatch.kind === "already_delivered") {
          return await submitFinalToolResult({
            ...
            message:
              "OpenAgent already delivered this consult result internally. Do not repeat it.",
          });
        }
        forcedConsult.sendSpeechPrompt = false;
