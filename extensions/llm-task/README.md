...
`openclaw.invoke --each`) without adding new Zero to Agent code per workflow.
...
This extension depends on Zero to Agent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Zero to Agent extension (like `lobster`) and
