...
`openclaw.invoke --each`) without adding new OpenAgent code per workflow.
...
This extension depends on OpenAgent internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenAgent extension (like `lobster`) and
