...
> {
  ...
  // The live probe may run while an older local OpenAgent profile awaits Doctor.
  ...
  it(
    ...
    async ({ skip }) => {
      ...
      const server = createServer((req, res) => {
        if (req.url === "/") {
          res.statusCode = 200;
          res.end("<!doctype html><title>OpenAgent GA sideband proof</title>");
          return;
        }
      ...
      try {
              ...
              try {
                void Promise.resolve(
                  controlBridge?.submitToolResult(event.callId, {
                    result: "OpenAgent GA sideband live proof passed.",
                  }),
                ).catch((error: unknown) =>
        ...
        controlBridge?.sendUserMessage?.("Run the requested OpenAgent verification.", {
...
describeLive("OpenAI OAuth WebRTC", () => {
  it(
    ...
    async ({ skip }) => {
      ...
      if (!auth) {
        skip("No OpenAgent ChatGPT OAuth profile is available");
        return;
      }
      ...
      const server = createServer((req, res) => {
        if (req.url === "/") {
          ...
          res.end("<!doctype html><title>OpenAgent realtime live proof</title>");
          return;
        }
        if (req.url === OPENAI_QUICKSILVER_OFFER_PATH) {
          void realtime.handler(req, res);
