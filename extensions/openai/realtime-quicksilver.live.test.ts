...
> {
  ...
  // The live probe may run while an older local Zero to Agent profile awaits Doctor.
  ...
  it(
    ...
    async ({ skip }) => {
      ...
      const server = createServer((req, res) => {
        if (req.url === "/") {
          res.statusCode = 200;
          res.end("<!doctype html><title>Zero to Agent GA sideband proof</title>");
          return;
        }
      ...
      try {
              ...
              try {
                void Promise.resolve(
                  controlBridge?.submitToolResult(event.callId, {
                    result: "Zero to Agent GA sideband live proof passed.",
                  }),
                ).catch((error: unknown) =>
        ...
        controlBridge?.sendUserMessage?.("Run the requested Zero to Agent verification.", {
...
describeLive("OpenAI OAuth WebRTC", () => {
  it(
    ...
    async ({ skip }) => {
      ...
      if (!auth) {
        skip("No Zero to Agent ChatGPT OAuth profile is available");
        return;
      }
      ...
      const server = createServer((req, res) => {
        if (req.url === "/") {
          ...
          res.end("<!doctype html><title>Zero to Agent realtime live proof</title>");
          return;
        }
        if (req.url === OPENAI_QUICKSILVER_OFFER_PATH) {
          void realtime.handler(req, res);
