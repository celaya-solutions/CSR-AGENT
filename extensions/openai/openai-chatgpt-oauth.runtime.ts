  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the Zero to Agent process and then retry `openclaw models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this Zero to Agent process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [
