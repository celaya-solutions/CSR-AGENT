  ...
  if (/unsupported_country_region_territory/i.test(message)) {
    return createOpenAICodexOAuthError(
      ...
      [
        ...
        "If you normally use a proxy, verify HTTPS_PROXY, HTTP_PROXY, or ALL_PROXY is set for the OpenAgent process and then retry `openagent models auth login --provider openai`.",
      ].join(" "),
      error,
    );
  }
  ...
  await prompter.note(
    isRemote
      ? [
          ...
          "If this OpenAgent process can receive the browser callback, sign-in may finish automatically before you paste.",
        ].join("\n")
      : [
