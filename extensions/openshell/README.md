...
Official NVIDIA OpenShell sandbox backend for OpenAgent.

This plugin lets OpenAgent use OpenShell-managed local or remote sandboxes with
...
`plugins.entries.openshell.config.workspace`; this is separate from OpenAgent's
...
operating system user that runs the OpenAgent Gateway, verify:
...
`plugins.entries.openshell`, and restart the OpenAgent Gateway. OpenShell
...
- Minimum OpenAgent host: `2026.5.12-beta.1`
