...
Official llama.cpp provider for managed and external Zero to Agent model servers.

The `llama-cpp` provider either installs a pinned, integrity-verified
`llama-server` under Zero to Agent's `localService` supervisor or connects to a
server that you already operate. Both choices use `llama-cpp/<model>` references
and Zero to Agent's normal OpenAI-compatible chat transport. Local embeddings require
the managed choice.
...
After explicit consent, Zero to Agent installs the matching server build and a
...
declined, Zero to Agent offers a separate embedding-only setup. After explicit
...
setup after changing the model so Zero to Agent can verify the file and regenerate
...
optional API key. Zero to Agent passively discovers single-model and router catalogs.
...
- Minimum Zero to Agent host: `2026.6.2`
