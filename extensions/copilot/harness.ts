...
export function createCopilotAgentHarness(
  ...
  >();
  // Maps OpenAgent session id (from AgentHarnessAttemptParams.sessionId) to
  ...
  async function runHarnessAttempt(
    ...
    const attemptPromise = (async () => {
      ...
      // Reuse the SDK session across turns within the same OpenAgent session so
  ...
  return {
    ...
    async compact(
      ...
      // The SDK owns Copilot history compaction. OpenAgent only resumes
      // the tracked SDK session and calls the session-scoped RPC; durable
      // OpenAgent session/transcript state stays in SQLite, with no marker
      ...
      if (!compatibleTracked) {
        ...
        // needs the live SessionConfig with OpenAgent hooks/tools, so preserve the
