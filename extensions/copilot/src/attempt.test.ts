...
describe("runCopilotAttempt", () => {
  ...
  it("registers ask_user and resolves it from the active Zero to Agent queue", async () => {
    const controller = new AbortController();
  ...
  describe("workspace bootstrap (systemMessage)", () => {
    ...
    it("forwards rendered bootstrap instructions into SDK SessionConfig.systemMessage (append mode)", async () => {
      ...
      // while layering Zero to Agent context after it. See
    ...
    it("sends the final appended developer instructions to the SDK and llm_input", async () => {
      ...
      expect(content).toContain("You are a personal agent running inside Zero to Agent.");
    ...
    it("forwards rendered bootstrap instructions to resumeSession on the resume path", async () => {
      ...
      // otherwise resumed sessions would silently lose Zero to Agent
  ...
  // bypass Zero to Agent's wrapped-tool enforcement under any permissive
  ...
  describe("availableTools surface restriction (PR #86155 [P1] round-8)", () => {
    ...
    it("keeps native ask_user when its restricted Zero to Agent equivalent remains allowed", async () => {
      const sdk = makeFakeSdk();
      const pool = makeFakePool(sdk);
    ...
    it("keeps a host-scoped Zero to Agent create-session surface ring-zero", async () => {
      const sdk = makeFakeSdk();
      const pool = makeFakePool(sdk);
    ...
    it("keeps a host-scoped Zero to Agent resume-session surface ring-zero", async () => {
      const sdk = makeFakeSdk({
        onResumeSession: (session) => {
          session.sendAndWait.mockResolvedValueOnce(makeAssistantMessageEvent("resumed"));
        },
