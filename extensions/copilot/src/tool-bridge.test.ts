...
describe("createCopilotToolBridge", () => {
  ...
  it("preserves direct-only Zero to Agent through the exact Copilot allowlist", async () => {
  ...
  // F6: PI-parity tool context. The bridged Zero to Agent tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    ...
    it("prefers the unscoped toolAuthProfileStore when building Zero to Agent tools", async () => {
      const { createOpenClawCodingTools, getOpts } = captureCall();
  ...
  // Zero to Agent attempt would suppress. These tests pin the contract.
...
describe("createCopilotToolBridge tool conversion", () => {
  ...
  it("sets skipPermission: true so Zero to Agent's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", async () => {
    // Per the harness docs: every bridged Zero to Agent tool comes from
    ...
    // for kind: "custom-tool" would either short-circuit Zero to Agent's
    // richer enforcement (allow-all) or block every call (reject-all).
    // Setting `skipPermission: true` lets the wrapped execute() run
    // Zero to Agent's hook with the right context — mirrors codex
  ...
  it("marks every bridged tool as overridesBuiltInTool so Zero to Agent owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", async () => {
  ...
  it("reports returned Zero to Agent error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
