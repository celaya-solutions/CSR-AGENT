...
const COPILOT_BYOK_TRANSPORT_POLICY_ERROR =
  "[copilot-attempt] BYOK does not support OpenAgent provider request proxy, TLS, or private-network policy overrides";
const COPILOT_BYOK_ENDPOINT_POLICY_ERROR =
  "[copilot-attempt] BYOK endpoint is blocked by OpenAgent SSRF policy";
...
/**
 * Maps OpenAgent's prepared model facts into the Copilot SDK's session-level
 * provider contract. The SDK owns the wire request; OpenAgent only supplies
