...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **Zero to Agent**.
...
In agent scope, `vault.path` is a parent directory. Zero to Agent appends the
normalized agent id:
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also persists a machine-readable snapshot in Zero to Agent plugin state so agent/runtime consumers do not have to scrape markdown pages.
...
Unmanaged raw Markdown can live under `sources/` without Zero to Agent page frontmatter. Add `<!-- openclaw:wiki:raw-source -->` near the top of the page body to opt it out of wiki page metadata and freshness lint; generated or source-sync tracked imports still require their structured metadata.
