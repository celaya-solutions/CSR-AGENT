  ...
  for (const workspaceDir of await resolveConfiguredWorkspaces(config, env)) {
    ...
    try {
      ...
      sources.push({
        ...
        workspaceDir: canonicalWorkspaceDir,
        filePath,
        reason: `Skipped unsafe Memory Core host event source ${filePath}: ${String(error)}. Check permissions and use regular files and directories inside the workspace, then rerun openagent doctor --fix. For shared notes, use canonical paths in memory.search.extraPaths; this does not migrate legacy events.`,
      });
    }
  }
  return sources;
