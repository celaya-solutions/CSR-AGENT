...
describe("diagnostics-otel service", () => {
  ...
  test.each([
    ...
    "replaces the default OpenAgent metric prefix with $metricNamePrefix",
  ...
  test.each([" TRUE ", "TrUe"])(
    "disables every OpenAgent-owned telemetry route for OTEL_SDK_DISABLED=%j",
    async (value) => {
      const events: TelemetryExporterEvent[] = [];
      const unsubscribe = onInternalDiagnosticEvent((event) => {
        if (event.type === "telemetry.exporter") {
  ...
  test("keeps explicit HTTP exporters canonical when ambient protocol is gRPC", async () => {
    ...
    emitEvent("log.record", {
      message: "OpenAgent-owned OTLP log",
    });
    await flushDiagnosticEvents();
  ...
  test("lets explicit OpenAgent sampling override the inherited sampler environment", async () => {
  ...
  test("nests exec spans under the run when the trace context is OpenAgent-owned", async () => {
    await startServiceFixture(["traces", "metrics"]);
    ...
    emitTrustedModelCallCompletedWithContent(
      {
        ...
        systemPrompt: "OpenAgent appended instructions",
      },
      {
