...
export const imessagePlugin: ChannelPlugin<ResolvedIMessageAccount, IMessageProbe> =
  createChatChannelPlugin<ResolvedIMessageAccount, IMessageProbe>({
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          "- iMessage current conversation: omit target, to, chatId, chatGuid, and chatIdentifier. OpenAgent resolves the trusted current chat server-side; never copy a redacted display value such as `***` into message actions.",
        ],
      },
