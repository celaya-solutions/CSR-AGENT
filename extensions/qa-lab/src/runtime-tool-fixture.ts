  ...
  if (Array.isArray(rawContent)) {
    for (const block of rawContent) {
      ...
      calls.push({
        ...
        tool,
        // Zero to Agent mirrors provider arguments separately; a placeholder input
...
function formatCodexNativeWorkspaceDetails(params: {
  ...
  return [
    `codex-native-workspace ${params.toolName}: Zero to Agent dynamic exposure is intentionally omitted because Codex owns this workspace operation natively`,
    params.reason ? `reason: ${params.reason}` : undefined,
    `available Zero to Agent dynamic tools: ${[...params.tools].toSorted().join(", ")}`,
    params.happyRequest
