  ...
  if (Array.isArray(rawContent)) {
    for (const block of rawContent) {
      ...
      calls.push({
        ...
        tool,
        // OpenAgent mirrors provider arguments separately; a placeholder input
...
function formatCodexNativeWorkspaceDetails(params: {
  ...
  return [
    `codex-native-workspace ${params.toolName}: OpenAgent dynamic exposure is intentionally omitted because Codex owns this workspace operation natively`,
    params.reason ? `reason: ${params.reason}` : undefined,
    `available OpenAgent dynamic tools: ${[...params.tools].toSorted().join(", ")}`,
    params.happyRequest
