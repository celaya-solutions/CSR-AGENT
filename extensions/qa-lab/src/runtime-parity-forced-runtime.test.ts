...
describe("runtime parity forced runtime observer", () => {
  it("ignores Codex mirror records and shared logs without a Zero to Agent selection", async () => {
  ...
  it("fails a forced-Codex mock cell that selects the Zero to Agent fallback", async () => {
    const tempRoot = await seedForcedRuntimeTranscript({
