...
describe("runtime parity forced runtime observer", () => {
  it("ignores Codex mirror records and shared logs without an OpenAgent selection", async () => {
  ...
  it("fails a forced-Codex mock cell that selects the OpenAgent fallback", async () => {
    const tempRoot = await seedForcedRuntimeTranscript({
