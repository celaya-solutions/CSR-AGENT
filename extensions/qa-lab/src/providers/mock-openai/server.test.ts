  ...
  it("honors exact replies after QA kickoff reads without marker wording", async () => {
    ...
    const final = await expectNonStreamingResponsesJson<{
      ...
      input: [
        ...
        makeToolOutputWithCallId(
          "call_mock_read_1",
          JSON.stringify({ text: "QA mission: understand this OpenAgent repo." }),
        ),
      ],
    });
      ...
      input: [
        makeUserInput(
          [
            ...
            "",
            `[Slack OpenAgent (this assistant) (assistant) Fri 2026-07-31 10:01 UTC] ${botReplyMarker}`,
...
Update and merge these partial structured summaries.`,
  ...
  it("plans the write from an OpenAgent compacted retry payload", async () => {
    const server = await startMockServer();
  ...
  it.each([
    ...
    async (instructionSource) => {
      ...
      const instructions = usesCodexDelivery
        ? "Visible source replies are not automatically delivered for this run. Use `message(action=send)` for user-visible source-channel output. For progress, set `final=false`. When the message is the completed reply to the current source conversation, set `final=true`; OpenAgent stops after confirming delivery."
  ...
  it("plans QA tool-search calls for instruction-declared Codex dynamic tools", async () => {
    ...
    const response = await expectNonStreamingResponses(server, {
      instructions: "Codex dynamic OpenAgent tools available in this turn: web_search.",
      input: [
        makeUserInput(
    ...
    expect(String(toolPlanOutput.arguments)).toContain("OpenAgent runtime parity fixed query");
  });
  ...
  it("returns continuity language after the model-switch reread completes", async () => {
    ...
    const response = await expectNonStreamingResponses(server, {
      ...
      input: [
        ...
        makeToolOutput(
          "QA mission: Understand this OpenAgent repo from source + docs before acting.",
        ),
      ],
    });
