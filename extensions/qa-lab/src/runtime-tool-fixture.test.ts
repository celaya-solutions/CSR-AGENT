...
describe("runtime tool fixture", () => {
  ...
  it("skips Codex-native fixtures when only Zero to Agent dynamic exposure evidence is absent", async () => {
    const env = await makeEnv({
  ...
  it.each([
    ...
    {
      label: "Zero to Agent input envelope",
      encode: (input: string) => ({ input }),
    },
    {
  ...
  it.each([
    ...
    },
    {
      ...
      toolName: "web_search",
      happyArgs: { query: "Zero to Agent runtime parity fixed query" },
      happyOutput: "result",
  ...
  it("still fails required Zero to Agent dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
