...
describe("runtime tool fixture", () => {
  ...
  it("skips Codex-native fixtures when only OpenAgent dynamic exposure evidence is absent", async () => {
    const env = await makeEnv({
  ...
  it.each([
    ...
    {
      label: "OpenAgent input envelope",
      encode: (input: string) => ({ input }),
    },
    {
  ...
  it.each([
    ...
    },
    {
      ...
      toolName: "web_search",
      happyArgs: { query: "OpenAgent runtime parity fixed query" },
      happyOutput: "result",
  ...
  it("still fails required OpenAgent dynamic fixtures when the tool is absent", async () => {
    const env = await makeEnv();
