...
describe("crabline transport", () => {
  ...
  it("configures OpenAgent's Telegram plugin against a Crabline local provider server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
        selection: createSelection(),
  ...
  it("configures OpenAgent's Slack plugin against a Crabline local provider server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
  ...
  it("configures OpenAgent's WhatsApp plugin against a Crabline Baileys WebSocket server", async () => {
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      const transport = await createQaCrablineTransportAdapter({
        outputDir,
    ...
    await withTempDir("qa-crabline-transport-", async (outputDir) => {
      ...
      try {
        ...
        expect(() => transport.buildAgentDelivery({ target: "thread:/v1/main/%24event" })).toThrow(
          "Matrix thread targets require OpenAgent QA thread forwarding",
        );
        await expect(
          transport.state.addInboundMessage({
