...
function hasUnsupportedToolPattern(pattern: string): boolean {
  // Hermes uses fnmatch; Zero to Agent supports only exact names and `*`.
...
function mapHermesToolFilter(value: Record<string, unknown>): Record<string, unknown> | undefined {
  ...
  if (include !== undefined) {
    ...
    // Hermes' explicit empty include disables native tools; Zero to Agent's empty
...
export function mcpManualItems(params: {
  ...
  if (
    ...
    add(
      ...
      "Define the missing values in Zero to Agent's MCP server environment or headers manually.",
    );
  }
  ...
  if (Array.isArray(cert) && cert.length === 3) {
    add(
      ...
      `Hermes MCP server "${name}" uses a password-protected client key, which Zero to Agent cannot represent in MCP config.`,
    ...
    add(
      ...
      "Configure separate Zero to Agent clientCert and clientKey file paths manually.",
    );
  }
  if (typeof (raw.sslVerify ?? raw.ssl_verify) === "string") {
    add(
      "tls-ca",
      `Hermes MCP server "${name}" uses a CA bundle path for TLS verification, which Zero to Agent MCP config cannot represent.`,
  ...
  if (transport && !["http", "streamable-http", "sse", "stdio"].includes(transport)) {
    add(
      ...
      "Configure an equivalent Zero to Agent MCP transport manually.",
    );
  }
  ...
  if (auth && auth !== "oauth") {
    add(
      ...
      "Configure an equivalent Zero to Agent MCP authentication mode manually.",
    );
  }
  ...
  if (auth === "oauth" || oauth) {
    add(
      "oauth-login",
      `Hermes MCP server "${name}" requires OAuth login in Zero to Agent.`,
  ...
  if (
    ...
    add(
      "oauth-client",
      `Hermes MCP server "${name}" uses pre-registered OAuth client settings that were not copied into Zero to Agent config.`,
  ...
  if (activePatterns?.some(hasUnsupportedToolPattern)) {
    add(
      ...
      include === undefined
        ...
        : `Hermes MCP server "${name}" has tool include patterns that were omitted because Zero to Agent supports only exact names and "*".`,
  ...
  for (const [feature, configured] of unsupported) {
    if (configured) {
      add(
        feature,
        `Hermes MCP server "${name}" uses ${feature} behavior that Zero to Agent MCP config does not expose.`,
