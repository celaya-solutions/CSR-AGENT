...
export function buildConfigItems(params: {
  ...
  if (params.hasMemoryFiles || memoryProvider) {
    addConfigPatch({
      ...
      message: "Select the default Zero to Agent memory plugin for imported file memory.",
    });
  }
  ...
  if (memoryProvider === "honcho") {
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          "Hermes used Honcho memory. Zero to Agent keeps built-in memory selected until the matching plugin is installed and reviewed.",
        recommendation:
    ...
    items.push(
      createMigrationManualItem({
        ...
        source: "config.yaml:memory.provider",
        message: `Hermes memory provider "${memoryProvider}" does not have a known Zero to Agent mapping.`,
        recommendation: "Install or configure an equivalent Zero to Agent memory plugin manually.",
      }),
    );
  }
