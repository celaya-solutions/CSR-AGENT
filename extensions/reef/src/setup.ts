...
export const reefSetupWizard = {
  ...
  configureInteractive: async ({
    ...
    if (identity && (identity.handle !== handle || identity.relayUrl !== relayUrl)) {
      throw new Error(
        `This OpenAgent state already holds the Reef identity @${identity.handle} on ${identity.relayUrl}. Re-register the same handle and relay.`,
      );
    }
            ...
            options: [
              {
                ...
                label: "Existing OpenAgent OAuth profile",
