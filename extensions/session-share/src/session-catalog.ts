    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          message:
            "Cannot list Zero to Agent sessions. Check the paired node's session-share configuration and connection.",
        },
      };
    }
  }
  ...
  return {
    ...
    async read(request) {
      if (!request.hostId.startsWith("node:") || !request.hostId.slice(5)) {
        throw new Error("Select a paired node host to read a Zero to Agent session");
      }
      ...
      if (!node) {
        throw new Error(
          "Zero to Agent session node is unavailable. Reconnect it and refresh the catalog.",
        );
      }
