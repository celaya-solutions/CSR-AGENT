    ...
    try {
      ...
      return {
        ...
        error: {
          ...
          message:
            "Cannot list OpenAgent sessions. Check the paired node's session-share configuration and connection.",
        },
      };
    }
  }
  ...
  return {
    ...
    async read(request) {
      if (!request.hostId.startsWith("node:") || !request.hostId.slice(5)) {
        throw new Error("Select a paired node host to read an OpenAgent session");
      }
      ...
      if (!node) {
        throw new Error(
          "OpenAgent session node is unavailable. Reconnect it and refresh the catalog.",
        );
      }
