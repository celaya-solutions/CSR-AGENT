...
describeLive("google plugin live", () => {
      ...
      const response = await completeSimple(
        ...
        } as Model<"google-generative-ai">,
        {
          messages: [
            {
              role: "user",
              content: "Reply with exactly: OpenAgent live catalog OK",
              timestamp: Date.now(),
            },
          ],
        },
  ...
  it("synthesizes speech through the registered provider", async () => {
    ...
    const audioFile = await provider.synthesize({
      text: "OpenAgent Google text to speech integration test OK.",
  ...
  it("transcodes speech to Opus for voice-note targets", async () => {
    ...
    const audioFile = await provider.synthesize({
      text: "OpenAgent Google voice note integration test OK.",
      cfg: { plugins: { enabled: true } } as never,
      providerConfig: { apiKey: GOOGLE_API_KEY },
      target: "voice-note",
      timeoutMs: 90_000,
  ...
  it("talks back when ready owns the realtime session", async () => {
    ...
    const bridge: RealtimeVoiceBridge = provider.createBridge({
      ...
      onReady: () => {
        readyCount += 1;
        bridge.triggerGreeting?.("Reply with exactly: OpenAgent Google realtime ready.");
      },
  ...
  it("delivers a queued prompt through the registered lazy realtime bridge", async () => {
    ...
    bridge.sendUserMessage?.("Reply with exactly: OpenAgent lazy bridge ready.");
    try {
      await bridge.connect();
      // Gemini 3.1 can omit transcription.finished. Wait for output to go idle
      // before close terminalizes the buffered transcript.
  ...
  it.each([undefined, "gemini-3.5-flash"])(
    ...
    async (model) => {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          result = await tool?.execute({ query: "OpenAgent GitHub", count: 1 });
          lastError = undefined;
          break;
        } catch (error) {
          lastError = error;
  ...
  it("runs Gemini web search through the Google model provider config fallback", async () => {
    await withGoogleApiEnvUnset(async () => {
      ...
      const result = await tool?.execute({ query: "OpenAgent GitHub", count: 1 });
