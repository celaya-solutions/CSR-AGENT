...
### Changes
- Version alignment with core OpenAgent release numbers.
...
- Matrix/mentions: keep room mention gating strict while accepting visible `@displayName` Matrix URI labels, so `requireMention` works for non-OpenAgent Matrix clients again. (#64796) Thanks @hclsys.
