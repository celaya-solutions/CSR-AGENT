...
describe("createTelegramBot", () => {
  ...
  it("unauthorized native group stop leaves the sender's pending fragment intact", async () => {
    ...
    const topic = {
      chat: { id: chatId, type: "supergroup", title: "OpenAgent Ops", is_forum: true },
  ...
  it.each([
    ...
    installTelegramTopicStateForTest();
    const chatId = nextForumCacheChatId();
    const chat = { id: chatId, type: "supergroup", title: "OpenAgent Ops", is_forum: true };
    loadConfig.mockReturnValue({
      commands: { native: true, allowFrom: { telegram: ["42"] } },
      messages: { inbound: { byChannel: { telegram: 3000 } } },
      channels: {
      ...
      try {
        ...
        await runTelegramMiddlewareChain({
          ctx: {
            ...
            message: {
              chat: { id: chatId, type: "supergroup", title: "OpenAgent Ops" },
              text,
              date: 1736380804,
              message_id: 104,
  ...
  it("keeps the login action when the callback sender is not an owner", async () => {
    ...
    expect(sendMessageSpy).toHaveBeenCalledWith(
      1234,
      "Only an OpenAgent owner can sign in here. Ask the owner to connect this provider or grant you owner access.",
      {},
    );
  });
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("openagent pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "OpenAgent", username: "openclaw_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
      ...
      me: { id: 7, is_bot: true, first_name: "OpenAgent", username: "openclaw_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
