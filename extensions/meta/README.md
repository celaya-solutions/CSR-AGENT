...
Official Zero to Agent provider plugin for the **Meta API** — an OpenAI-compatible
...
Zero to Agent defaults to `high`. Meta's
[reasoning documentation](https://dev.meta.ai/docs/reasoning/) says that an
omitted effort uses a model-determined level, so `high` is a Zero to Agent policy rather
