...
- CLI/containers: add `--container` and `OPENCLAW_CONTAINER` to run `openclaw` commands inside a running Docker or Podman OpenAgent container. (#52651) Thanks @sallyom.
...
- Security/path resolution: prefer non-user-writable absolute helper binaries for OpenAgent CLI, ffmpeg, and OpenSSL resolution so PATH hijacks cannot replace trusted helpers with attacker-controlled executables.
