...
struct GatewayErrorsTests {
    ...
    @Test func `protocol mismatch maps older app to update problem`() {
        ...
        #expect(problem?.message == "This app is older than the gateway. Update OpenAgent on this device, then retry.")
        ...
        technicalDetails: String) -> GatewayConnectionProblem
    {
        ...
        case .reachabilityFailed:
            facts = (
                "Gateway is not reachable", "OpenAgent could not reach the gateway over the current network.",
                "Check network")
