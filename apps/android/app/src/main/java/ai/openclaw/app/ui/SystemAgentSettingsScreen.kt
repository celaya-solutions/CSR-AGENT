...
private fun SystemAgentAccessGate(state: SystemAgentChatState) {
  ...
  val detail =
    when (state.access) {
      SystemAgentChatAccess.Disconnected -> nativeString("Connect this phone to a Gateway before opening OpenAgent.")
      ...
      SystemAgentChatAccess.CheckingGateway -> nativeString("Checking whether this Gateway supports the OpenAgent settings assistant.")
      SystemAgentChatAccess.GatewayUpdateRequired -> nativeString("Update this Gateway to use the OpenAgent settings assistant.")
      SystemAgentChatAccess.Ready -> ""
    }
      ...
      items(rows, key = { it.key }) { row ->
        when (row) {
          ...
          SystemAgentConversationRow.Working -> {
            Text(nativeString("OpenAgent is working…"), style = ClawTheme.type.caption, color = ClawTheme.colors.textMuted)
          }
    ...
    state.handoff?.let {
      ClawPanel(modifier = Modifier.fillMaxWidth()) {
        Column(modifier = Modifier.fillMaxWidth().padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
          Text(nativeString("OpenAgent is ready to continue in your ordinary chat."), style = ClawTheme.type.body, color = ClawTheme.colors.textMuted)
    ...
    ClawTextField(
      ...
      placeholder =
        if (state.expectsSensitiveReply) {
          ...
          nativeString("Reply to OpenAgent…")
        },
