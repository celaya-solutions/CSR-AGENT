...
enum IOSDeviceSettingsConsent: Equatable {
    ...
    var title: String {
        ...
        case .notificationEnrollment: String(localized: "Enable OpenAgent Hosted Push Relay?")
        }
    }
    ...
    var detail: String {
        ...
        case .healthSummary:
            String(localized: """
            OpenAgent reads steps, sleep, resting heart rate, and workouts from Apple Health only when a summary is \
            ...
            """)
        case .locationAlways:
            String(localized: "The Gateway can request this device's location even when OpenAgent is not in use.")
        case .notificationEnrollment:
            String(localized: "Enabling this sends delivery data through OpenAgent's hosted push relay.")
        }
    }
}
