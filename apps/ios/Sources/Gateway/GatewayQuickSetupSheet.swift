...
private struct GatewayQuickSetupHeader: View {
    ...
    private var subtitle: LocalizedStringKey {
        if self.hasCandidate {
            return "OpenAgent found a gateway on this network. Review it, then pair this iPhone as a secure node."
        }
        return "OpenAgent is searching the local network and tailnet for a Gateway you can trust."
    }
}
