// Control UI tests cover the global Ask OpenAgent panel toggle and persisted session identity.
...
function custodianGatewayScenario(): ControlUiMockGatewayScenario {
  return {
    ...
    methodResponses: {
      "sessions.list": {
        cases: [
          // The work session does not match the Ask OpenAgent palette query.
          {
            match: { search: "Ask OpenAgent" },
...
describeControlUiE2e("Control UI Ask OpenAgent panel toggle mocked Gateway E2E", () => {
  beforeAll(async () => {
  ...
  it("keeps Home available without an OpenAgent tab when openclaw.chat is not advertised", async () => {
    ...
    try {
      ...
      expect(await panel.getByRole("button", { name: "Ask OpenAgent", exact: true }).count()).toBe(
        0,
      );
  ...
  it("opens OpenAgent from Home and the palette and reuses the persisted session id", async () => {
    const context = await browser.newContext({
      colorScheme: "dark",
      locale: "en-US",
    ...
    try {
      ...
      await page.locator(".sidebar-footer-bar__home").click();
      const panel = page.locator("openclaw-assistant-panel");
      const openClawTab = panel.getByRole("button", { name: "Ask OpenAgent", exact: true });
      ...
      await page.getByPlaceholder("Search chats and commands…").fill("Ask OpenAgent");
      const paletteItem = page.getByRole("option", { name: "Ask OpenAgent", exact: true });
