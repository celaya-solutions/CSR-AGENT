// Control UI tests cover the global Ask Zero to Agent panel toggle and persisted session identity.
...
function custodianGatewayScenario(): ControlUiMockGatewayScenario {
  return {
    ...
    methodResponses: {
      "sessions.list": {
        cases: [
          // The work session does not match the Ask Zero to Agent palette query.
          {
            match: { search: "Ask Zero to Agent" },
...
describeControlUiE2e("Control UI Ask Zero to Agent panel toggle mocked Gateway E2E", () => {
  beforeAll(async () => {
  ...
  it("keeps Home available without a Zero to Agent tab when openclaw.chat is not advertised", async () => {
    ...
    try {
      ...
      expect(
        await panel.getByRole("button", { name: "Ask Zero to Agent", exact: true }).count(),
      ).toBe(0);
  ...
  it("opens Zero to Agent from Home and the palette and reuses the persisted session id", async () => {
    const context = await browser.newContext({
      colorScheme: "dark",
      locale: "en-US",
    ...
    try {
      ...
      await page.locator(".sidebar-footer-bar__home").click();
      const panel = page.locator("openclaw-assistant-panel");
      const openClawTab = panel.getByRole("button", { name: "Ask Zero to Agent", exact: true });
      ...
      await page.getByPlaceholder("Search chats and commands…").fill("Ask Zero to Agent");
      const paletteItem = page.getByRole("option", { name: "Ask Zero to Agent", exact: true });
