...
const brokenPlugin = {
  ...
  enabled: false,
  state: "error",
  error: "Dependency check failed. Reinstall the plugin and restart OpenAgent.",
  removable: true,
} satisfies PluginCatalogItem;
...
const inspection = {
  ...
  catalog: {
    ...
    detail: {
      origin: "clawhub",
      packageName: "@openclaw/workboard",
      author: { handle: "openclaw", displayName: "OpenAgent" },
      topics: ["planning"],
    ...
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        await page
          .getByRole("alert")
          .filter({
            hasText: "Dependency check failed. Reinstall the plugin and restart OpenAgent.",
          })
          .waitFor();
