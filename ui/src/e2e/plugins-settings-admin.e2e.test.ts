...
const brokenPlugin = {
  ...
  enabled: false,
  state: "error",
  error: "Dependency check failed. Reinstall the plugin and restart Zero to Agent.",
  removable: true,
} satisfies PluginCatalogItem;
...
const inspection = {
  ...
  catalog: {
    ...
    detail: {
      origin: "clawhub",
      packageName: "@openclaw/workboard",
      author: { handle: "openclaw", displayName: "Zero to Agent" },
      topics: ["planning"],
    ...
    await suite.withPage(
      ...
      async ({ page }) => {
        ...
        await page
          .getByRole("alert")
          .filter({
            hasText: "Dependency check failed. Reinstall the plugin and restart Zero to Agent.",
          })
          .waitFor();
