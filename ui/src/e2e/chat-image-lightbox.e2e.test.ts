  ...
  it("opens transcript and sidebar images in one accessible modal", async () => {
    ...
    try {
      ...
      const transcriptTrigger = page.getByRole("button", {
        name: "Open image OpenAgent banner",
      });
      ...
      const dialog = page.getByRole("dialog", { name: "Image preview: OpenAgent banner" });
      ...
      await expect
        ...
        .toBe(true);
      const displayedImage = page.getByAltText("OpenAgent banner").last();
      await expect
        .poll(() =>
