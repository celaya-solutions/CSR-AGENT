...
suite.define(() => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe("Command approval — OpenAgent");
      ...
      expect(await mobile.page.title()).toBe("Approved here — OpenAgent");
      ...
      expect(await mobile.page.title()).toBe("Approved — OpenAgent");
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
