...
suite.define(() => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe("Command approval — Zero to Agent");
      ...
      expect(await mobile.page.title()).toBe("Approved here — Zero to Agent");
      ...
      expect(await mobile.page.title()).toBe("Approved — Zero to Agent");
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
