...
suite.define(() => {
  it("marks exactly one target when the session is assigned to self", async () => {
    await suite.withPage(createControlUiE2eContextOptions(), async ({ page }) => {
      ...
      ).toHaveText(["Me", "Zero to Agent", "Bob", "Carol"]);
    });
  });
  ...
  it("assigns named and self owners through one keyboard-accessible submenu", async () => {
    await suite.withPage(createControlUiE2eContextOptions(), async ({ page }) => {
      ...
      await expectBrowser(ownerItems).toHaveText(["Me", "Zero to Agent", "Bob", "Carol"]);
    ...
    async ({ surface, width }) => {
      await suite.withPage(
        ...
        async ({ page }) => {
          ...
          ).toHaveText(["Me", "Zero to Agent", "Bob", "Carol", ...extraNames]);
