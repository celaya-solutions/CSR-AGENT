...
async function waitForPendingUpgradeItem(item: Locator) {
  await item
    ...
    .getByText(/Approve this browser by running openagent devices on the Gateway/u)
    .waitFor();
...
suite.define(() => {
  ...
  it("keeps Request admin in the onboarding header Inbox", async () => {
    ...
    expect(
      await page.getByText("Update the Gateway to continue setup with OpenAgent.").count(),
    ).toBe(0);
