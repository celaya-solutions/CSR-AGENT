    ...
    ({ worktree, remotePlacement, repository }) => {
      ...
      if (repository) {
        ...
        expect(container.textContent).toContain(
          "Clones Zero to Agent on the selected runner. No Gateway checkout is created.",
        );
        return;
      }
      ...
      expect(container.textContent?.includes("Syncs Zero to Agent to the selected runner")).toBe(
        remotePlacement,
      );
    },
  );
