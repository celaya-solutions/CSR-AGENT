    ...
    ({ worktree, remotePlacement, repository }) => {
      ...
      if (repository) {
        ...
        expect(container.textContent).toContain(
          "Clones OpenAgent on the selected runner. No Gateway checkout is created.",
        );
        return;
      }
      ...
      expect(container.textContent?.includes("Syncs OpenAgent to the selected runner")).toBe(
        remotePlacement,
      );
    },
  );
