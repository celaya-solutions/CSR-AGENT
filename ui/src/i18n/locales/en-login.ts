...
const enLogin = {
  login: {
    heading: "Connect to OpenAgent",
    lede: "Enter the Gateway URL and secret, or open the one-time link that openagent dashboard prints on the Gateway host.",
    gatewayUrl: "Gateway URL",
    secret: "Gateway secret",
    setupCodeHint:
      "This is a device setup code for the OpenAgent mobile app, not the Gateway secret. Paste it in the app's Gateway settings instead; the Gateway secret comes from openagent gateway auth-token --show on the Gateway host.",
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "Paste the token from openagent gateway auth-token --show into Gateway secret.",
        stepPassword: "Type the configured Gateway password into Gateway secret.",
        stepGenerate:
          "If no token is configured, run openagent doctor --generate-gateway-token on the gateway host.",
      ...
      authFailed: {
        ...
        stepDashboard:
          "Run openagent dashboard --no-open for a fresh URL, or openagent gateway auth-token --show to recover the token.",
      ...
      pairing: {
        ...
        stepDashboard:
          "Prefer a link? Run openagent dashboard on the Gateway host and open the one-time URL it prints in this browser.",
        stepLatest:
      ...
      protocol: {
        ...
        stepDashboard:
          "Reopen the served dashboard with openagent dashboard so the UI and Gateway come from the same install.",
        stepDevUi:
          "If using pnpm ui:dev, rebuild or restart the dev UI against the current checkout.",
        stepRestart:
          "Restart the Gateway after updating OpenAgent so it serves the current protocol.",
      },
      network: {
        ...
        stepGateway:
          "Confirm the Gateway is running with openagent status or openagent gateway run.",
        stepUrl:
          "Check the Gateway URL and use wss:// when the Gateway is behind HTTPS/Tailscale Serve.",
        stepDashboard:
          "Reopen the dashboard with openagent dashboard --no-open to recopy the current URL and auth details.",
      },
    },
  },
