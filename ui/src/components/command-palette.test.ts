...
describe("CommandPalette lifecycle", () => {
  ...
  it.each([
    ...
    "shows Ask OpenAgent only when availability is $available",
    async ({ available, expectedCount }) => {
      const { gateway } = createGateway(true);
      const { palette } = await mountPalette(
        createContext(
      ...
      expect(findPaletteOption(palette, "Ask OpenAgent", true) ? 1 : 0).toBe(expectedCount);
    },
  );

  it("opens Ask OpenAgent from its palette action", async () => {
    const { gateway } = createGateway(true);
    const { palette } = await mountPalette(
      createContext(
        gateway,
    ...
    try {
      findPaletteOption(palette, "Ask OpenAgent", true)?.click();
    } finally {
