...
describe("CommandPalette lifecycle", () => {
  ...
  it.each([
    ...
    "shows Ask Zero to Agent only when availability is $available",
    async ({ available, expectedCount }) => {
      const { gateway } = createGateway(true);
      const { palette } = await mountPalette(
        createContext(
      ...
      expect(findPaletteOption(palette, "Ask Zero to Agent", true) ? 1 : 0).toBe(expectedCount);
    },
  );

  it("opens Ask Zero to Agent from its palette action", async () => {
    const { gateway } = createGateway(true);
    const { palette } = await mountPalette(
      createContext(
        gateway,
    ...
    try {
      findPaletteOption(palette, "Ask Zero to Agent", true)?.click();
    } finally {
