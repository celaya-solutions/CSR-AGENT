 ...
 *
 * Routes models that need Zero to Agent-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
...
/** Returns whether Zero to Agent has a managed transport implementation for this API. */
...
/** Creates a managed Zero to Agent transport stream for explicit fallback/runtime callers. */
export function createOpenClawTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need Zero to Agent's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same Zero to Agent request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
