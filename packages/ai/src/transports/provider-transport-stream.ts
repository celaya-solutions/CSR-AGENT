 ...
 *
 * Routes models that need OpenAgent-managed proxy/TLS/local-service semantics onto built-in transport implementations.
 */
...
/** Returns whether OpenAgent has a managed transport implementation for this API. */
...
/** Creates a managed OpenAgent transport stream for explicit fallback/runtime callers. */
export function createOpenClawTransportStreamFnForModel(
  model: Model,
  ctx?: ProviderTransportStreamContext,
): StreamFn | undefined {
  // Explicit fallback callers use this when they need OpenAgent's HTTP
...
export function createBoundaryAwareStreamFnForModel(
  ...
  // HTTP streams preserve the same OpenAgent request contract.
  if (!isTransportAwareApiSupported(model.api)) {
    return undefined;
  }
  return createSupportedTransportStreamFn(model, ctx);
