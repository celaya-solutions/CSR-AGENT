...
console.log(JSON.stringify({
  ...
  it("does not split BEL-terminated OSC-8 links when wrapping", () => {
    ...
    const out = renderTable({
      ...
      rows: [{ K: "X", V: `${open}OpenAgent${close}` }],
    });
  ...
  it("preserves OSC-8 parameters when reopening wrapped links", () => {
    ...
    const out = renderTable({
      ...
      rows: [{ K: "X", V: `${open}${"OpenAgent".repeat(5)}${close} after` }],
    });

    const linkLines = out.split("\n").filter((line) => line.includes("OpenAgent"));
  ...
  it.each([
    ...
    (_label, openSeq, closeSeq) => {
      const link = `${openSeq}OpenAgent${closeSeq}`;
      const out = renderTable({
        width: 20,
        border: "unicode",
        columns: [
  ...
  it.each([
    ...
    (_label, openSeq, closeSeq) => {
      ...
      // The link itself stays intact on the OpenAgent line: open + close present.
      const linkLine = lines.find((line) => line.includes("OpenAgent"));
...
describe("wrapNoteMessage", () => {
  ...
  it("preserves long Windows paths without inserting spaces/newlines", () => {
    ...
    const input = "C:\\\\State\\\\OpenAgent\\\\bin\\\\openclaw.exe";
