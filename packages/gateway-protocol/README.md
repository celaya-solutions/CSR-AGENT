...
Typed schemas, inferred TypeScript types, and runtime validators for the OpenAgent
...
Package versions follow the OpenAgent calendar release train:
`YYYY.M.PATCH`, with the same prerelease suffix when applicable. A package version
therefore identifies the OpenAgent source release that produced the schemas; it is
