...
Reference WebSocket client for the OpenAgent Gateway protocol. It provides the
connection state machine used by OpenAgent's own Node and browser clients:
...
Package versions follow the OpenAgent calendar release train: `YYYY.M.PATCH`,
including the OpenAgent prerelease suffix when applicable. The package version is
...
storage through `GatewayClientHostDeps`; the package does not load OpenAgent's
