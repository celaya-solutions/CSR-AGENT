...
- The actual impact, including which OpenAgent trust boundary is crossed.
- Any remediation advice or focused patch you can provide.

Reports without reproduction steps, demonstrated impact, and remediation advice are deprioritized. We receive a high volume of AI-generated scanner findings, so we prioritize vetted reports from researchers who can show how the issue crosses an OpenAgent security boundary.
...
- A report whose only primitive is changing the process or child-process environment before running OpenAgent or an executable OpenAgent invokes.
...
- Scanner-only, dependency-only, or stale-path reports without a working repro and demonstrated OpenAgent impact.
...
- Tested version details (OpenAgent version and/or commit SHA).
...
- Reports that only show untrusted media bytes reaching a maintained native decoder dependency (for example image codec libraries such as libheif) without proving the shipped dependency version is vulnerable and demonstrating crash, memory corruption, data exposure, or a boundary bypass through OpenAgent. JavaScript header sniffing and image dimension fast-paths are preflight/UX checks, not the security boundary for native decoder correctness.
- Reports whose only impact is transient extra memory, CPU, or allocation work from decoding, base64 expansion, media transcoding, serialization, or other format conversion after the input was already accepted under OpenAgent's configured size/trust limits, including base64 decode-before-size-estimate findings. These are performance issues, not vulnerabilities, unless the report demonstrates unauthenticated amplification, bypass of configured limits, crash/process termination, persistent resource exhaustion, data exposure, or another documented boundary bypass.
...
- Reports that depend on attacker-controlled environment variables changing executable behavior, including variables that redirect lookup paths, preload code, select wrappers/interpreters, alter package-manager or runtime hooks, or make one executable call another executable. Control of the process or child-process environment is trusted host/operator control in OpenAgent's model; these reports need a separate OpenAgent boundary bypass that lets untrusted input set or mutate that environment.
...
OpenAgent does **not** model one gateway as a multi-tenant, adversarial user boundary.
...
- OpenAgent can technically run multiple gateway instances on one machine, but recommended operations are clean separation by trust boundary.
- Recommended mode: one user per machine/host (or VPS), one gateway for that user, and one or more agents inside that gateway.
- If multiple users need OpenAgent, use one VPS (or host/OS user boundary) per user.
...
- This is expected in OpenAgent's one-user trusted-operator model. If you need isolation, enable sandbox mode (`non-main`/`all`) and keep strict tool policy.

### Trusted Plugins

Plugins/extensions are part of OpenAgent's trusted computing base for a gateway.
...
- Using OpenAgent in ways that the docs recommend not to
- Test-only code and maintainer harnesses, including QA Lab, QE Lab, E2E fixtures, benchmark rigs, smoke-test containers, and local debugging proxies, unless the report demonstrates that the same vulnerable behavior is reachable from shipped OpenAgent production code or a published package artifact intended for users.
...
OpenAgent security guidance assumes:

- The host where OpenAgent runs is within a trusted OS/admin boundary.
- Anyone who can set or mutate the OpenAgent process environment, launcher environment, or child-process environment is inside that trusted host/operator boundary.
...
OpenAgent separates routing from execution, but both remain inside the same operator trust boundary:
...
- Exec approvals bind exact command/cwd/env context and, when OpenAgent can identify one concrete local script/file operand, that file snapshot too. This is best-effort integrity hardening, not a complete semantic model of every interpreter/runtime loader path.
...
- Plugins can execute with the same OS privileges as the OpenAgent process.
- Runtime helpers (for example `runtime.system.runCommandWithTimeout`) are convenience APIs, not a sandbox boundary.
- Only install plugins you trust, and prefer `plugins.allow` to pin explicit trusted plugin ids.

### Temp Folder Boundary

OpenAgent uses a dedicated temp root for local media handoff and sandbox-adjacent temp artifacts:
...
OpenAgent's web interface (Gateway Control UI + HTTP endpoints) is intended for **local use only**.
...
- The retired `gateway.controlUi.dangerouslyDisableDeviceAuth` break-glass key is not a
  ...
  - OpenAgent keeps deployment flexibility by design and does not hard-forbid non-local setups.
...
OpenAgent requires **Node.js 24.16+ or Node.js 26.1+**. Node 26 is recommended; Node 24 is the supported LTS line. These minimum versions include the upstream SQLite WAL-reset corruption fix and preserve embedded NUL characters in SQLite TEXT reads. Node 22, 23, and 25 are unsupported.
...
When running OpenAgent in Docker:
