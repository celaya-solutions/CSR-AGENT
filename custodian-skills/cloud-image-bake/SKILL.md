...
Record warmup total and compare against the pre-bake timing. Then confirm the OpenAgent path end to end: dispatch one session to the profile from a client (Cloud destination) and verify the placement reaches active. If any step fails, roll back the image selection and report the exact blocker.
