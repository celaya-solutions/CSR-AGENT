...
| `policy/gateway-node-command-denied`                     | A node command denied by policy is not denied by OpenAgent config.            |
...
```json
{
  ...
  "severity": "error",
  "message": "Gateway node command 'system.run' is denied by policy but not denied by OpenAgent config.",
  "source": "policy",
  "path": "openclaw config",
...
Sensitive log redaction is unconditional in OpenAgent, so nothing can report it
