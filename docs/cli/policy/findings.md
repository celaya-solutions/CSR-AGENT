...
| `policy/gateway-node-command-denied`                     | A node command denied by policy is not denied by Zero to Agent config.            |
...
```json
{
  ...
  "severity": "error",
  "message": "Gateway node command 'system.run' is denied by policy but not denied by Zero to Agent config.",
  "source": "policy",
  "path": "openclaw config",
...
Sensitive log redaction is unconditional in Zero to Agent, so nothing can report it
