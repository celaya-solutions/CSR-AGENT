---
summary: "Every supported `policy.jsonc` rule, the Zero to Agent state it observes, and when to use it"
read_when:
...
observed state is existing Zero to Agent config or workspace metadata.
...
| `routing.probes[].route`            | The public Zero to Agent route resolver             | Describe a representative inbound route without sending a message.     |
...
| `gateway.nodes.denyCommands`            | `gateway.nodes.commands.deny`                 | Require exact node command ids such as `system.run` to be denied in Zero to Agent config. |
...
denied by Zero to Agent config. A deployment that intentionally allows a privileged
...
| Policy field                                        | Observed state                                                                                                   | Use when                                                                    |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| `dataHandling.sensitiveLogging.requireRedaction`    | Runtime invariant `oc://openclaw.invariant/logging/redaction`                                                    | Set to `true` to record the requirement; Zero to Agent always satisfies it. |
