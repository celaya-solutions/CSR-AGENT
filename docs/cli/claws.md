...
A Claw is a versioned setup for one new Zero to Agent agent. It can describe the
...
For human-readable `claws add`, Zero to Agent prints the experimental warning before
...
portable agent prompt. Zero to Agent applies it as the Claw-managed `SOUL.md` for
the new agent:
...
Zero to Agent automatically discovers the optional `profiles/openclaw.yml` file.
...
This profile exists only inside the Claw package. Zero to Agent validates and uses it
while inspecting, adding, updating, and exporting that Claw; it is not copied
to the user's normal Zero to Agent configuration path. Other harnesses consume the
...
second copy of the Zero to Agent settings. The remaining schema fragments on this
page use JSON, with equivalent keys available in `CLAW.md` frontmatter.

The Zero to Agent package profile may use an explicit `tools.allow` list or select
any built-in tool profile registered by the running Zero to Agent version. The
...
A Zero to Agent profile may also declare harness-specific extension requirements:
...
`format` asserts the artifact format that Zero to Agent must detect (`openclaw`,
`claude`, `codex`, or `cursor`). The canonical plugin preflight resolves the
exact artifact and reports which components the current Zero to Agent adapter maps
...
Zero to Agent ignores foreign harness profiles during apply. Package integrity still
covers every published package byte, while a development snapshot binds the
portable manifest, bootstrap and workspace sources, and the selected Zero to Agent
...
instructions. Zero to Agent seeds it into the new agent workspace and records
...
plan consent approves missing installs; Zero to Agent completes those canonical
...
Plugins remain process-wide Zero to Agent capabilities rather than per-agent
installations.
...
Zero to Agent add plan offline, and build an immutable package artifact:
...
messages, or modify Zero to Agent state. Dependencies that require online preflight
...
Only package metadata, `CLAW.md`, optional `BOOTSTRAP.md`, the Zero to Agent profile,
...
Validate the source without planning local changes. For Zero to Agent profile
...
`--yes` alone is insufficient. Zero to Agent rebuilds the plan and rejects consent
...
Zero to Agent rebuilds the plan and compare-and-swaps owned state before each
...
mutation, Zero to Agent reports error code `update_partial` with structured
