...
A Claw is a versioned setup for one new OpenAgent agent. It can describe the
...
For human-readable `claws add`, OpenAgent prints the experimental warning before
...
portable agent prompt. OpenAgent applies it as the Claw-managed `SOUL.md` for
the new agent:
...
OpenAgent automatically discovers the optional `profiles/openclaw.yml` file.
...
This profile exists only inside the Claw package. OpenAgent validates and uses it
while inspecting, adding, updating, and exporting that Claw; it is not copied
to the user's normal OpenAgent configuration path. Other harnesses consume the
...
second copy of the OpenAgent settings. The remaining schema fragments on this
page use JSON, with equivalent keys available in `CLAW.md` frontmatter.

The OpenAgent package profile may use an explicit `tools.allow` list or select
any built-in tool profile registered by the running OpenAgent version. The
...
An OpenAgent profile may also declare harness-specific extension requirements:
...
`format` asserts the artifact format that OpenAgent must detect (`openclaw`,
`claude`, `codex`, or `cursor`). The canonical plugin preflight resolves the
exact artifact and reports which components the current OpenAgent adapter maps
...
OpenAgent ignores foreign harness profiles during apply. Package integrity still
covers every published package byte, while a development snapshot binds the
portable manifest, bootstrap and workspace sources, and the selected OpenAgent
...
instructions. OpenAgent seeds it into the new agent workspace and records
...
plan consent approves missing installs; OpenAgent completes those canonical
...
Plugins remain process-wide OpenAgent capabilities rather than per-agent
installations.
...
OpenAgent add plan offline, and build an immutable package artifact:
...
messages, or modify OpenAgent state. Dependencies that require online preflight
...
Only package metadata, `CLAW.md`, optional `BOOTSTRAP.md`, the OpenAgent profile,
...
Validate the source without planning local changes. For OpenAgent profile
...
`--yes` alone is insufficient. OpenAgent rebuilds the plan and rejects consent
...
OpenAgent rebuilds the plan and compare-and-swaps owned state before each
