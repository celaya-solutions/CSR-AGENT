...
read_when:
  - Zero to Agent is misbehaving and you want an agent-ready debugging prompt
...
Collect sanitized diagnostics and open a coding agent on this machine to diagnose, repair, and verify this Zero to Agent installation.
...
Before an automatic interactive launch, Zero to Agent shows what it will run. It shows the selected binary, or the embedded Zero to Agent agent using your configured model. It also shows the saved prompt path when available, and a notice that it uses your own account or tokens. Zero to Agent then asks for confirmation. Enter or `y` proceeds. `n` prints one manual next step for the selected agent, and preserves diagnostics and the failed update's exit status. After 30 seconds without an answer, Zero to Agent announces that it is continuing and proceeds as Yes.
...
Embedded Zero to Agent repair keeps local non-PTY commands and stdio LSP/MCP servers under a process-group owner on macOS and Linux. Cleanup must confirm that the owned process group has disappeared. Closed pipes, process listings, or a completed root command are insufficient. Commands that detach from that group and close inherited descriptors remain outside this cleanup guarantee. Zero to Agent `exec` requests with `pty: true` fall back to a non-PTY child without starting a native PTY and report a warning. Commands that require a terminal may fail. Windows stdio tools still lack cleanup confirmation, so their use leaves automatic cleanup uncertain.
