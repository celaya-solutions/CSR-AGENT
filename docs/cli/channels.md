---
summary: "CLI reference for `openagent channels` (accounts, status, dead letters, capabilities, resolve, logs, login/logout)"
read_when:
...
```bash
openagent channels list
openagent channels list --all
openagent channels status
openagent channels status --probe
openagent channels capabilities
openagent channels capabilities --channel discord --target channel:123
openagent channels resolve --channel discord "My Server/#general" "@someone"
openagent channels logs --channel all
openagent channels dead-letters list --channel telegram --account default
```

`channels status` keeps configured channels visible when their plugin fails to load or register. Affected accounts report `running: false`, `lifecycle: "blocked"`, and the plugin error instead of stale probe success. Run `openagent doctor`, repair or update the plugin, and restart the Gateway before checking again.

`channels list` shows chat channels only: configured accounts by default, with `installed`, `configured`, and `enabled` status tags per account (`--json` for machine output). Pass `--all` to also surface bundled channels that have no configured account yet and installable catalog channels that are not yet on disk. Provider auth and model usage live elsewhere: `openagent models auth list` for provider auth profiles, `openagent status` or `openagent models list` for usage/quota.
...
`channels status` does not support `--deep`; use `openagent channels status --probe` for channel checks. The separate top-level `openagent status --deep` command provides a broader status probe.
...
```bash
openagent channels dead-letters list --channel telegram --account default
openagent channels dead-letters list --channel telegram --account default --json
```
...
```bash
openagent channels dead-letters resubmit <event-id> --channel telegram --account default
```
...
`openagent health` reports dead-letter counts and oldest failure age per channel account. `openagent doctor` names affected accounts and points back to the inspection command.

Do not use `openagent sessions`, Gateway `sessions.list`, or the agent
...
openagent channels add --channel discord --use-env
openagent channels remove --channel telegram --delete
```
...
openagent onboard --non-interactive --accept-risk --skip-health \
  ...
  --skip-channels \
  --no-install-daemon
openagent channels add --channel telegram --use-env
```
...
<Tip>
`openagent channels add telegram --help` or `openagent channels add --channel telegram --help` shows only Telegram's setup flags. `openagent channels add --help` shows only the shared command envelope.
</Tip>
...
When you run `openagent channels add` with no direct account, credential, or channel-config flags, the interactive wizard can prompt. A positional channel id and `--channel <id>` both open that channel's guided setup immediately. Back returns to the full channel picker:

```bash
openagent channels add telegram
openagent channels add --channel telegram
```

Guided setup requires an interactive terminal. In a non-TTY shell, OpenAgent exits immediately instead of waiting for input; use `openagent channels add --channel <id> --use-env` or pass the selected plugin's credential flags.
...
If your config was already in a mixed state (named accounts present and top-level single-account values still set), run `openagent doctor --fix` to move account-scoped values into the promoted account chosen for that channel.
...
```bash
openagent channels login --channel <id>
openagent channels logout --channel <id>
```
...
When one account needs to reconnect while keeping its pairing and credentials, call the `channels.stop` and `channels.start` Gateway RPCs. Both require `operator.admin`. Invoke them through `openagent gateway call`:

```bash
# Stop one Telegram account without clearing its credentials.
openagent gateway call channels.stop --params '{"channel":"telegram","accountId":"<accountId>"}'
# Start the same account again.
openagent gateway call channels.start --params '{"channel":"telegram","accountId":"<accountId>"}'
openagent channels status --channel telegram --probe
```
...
Unlike this recovery path, `openagent channels logout` clears the account's credentials and requires login again; `openagent gateway restart` restarts the whole Gateway. See [Restart recovery](/gateway/restart-recovery) for the crash-loop breaker and its manual `channels.start` override.

## Troubleshooting

- Run `openagent status --deep` for a broad probe.
- Use `openagent doctor` for guided fixes.
- `openagent channels status` falls back to config-only summaries when the gateway is unreachable. If a supported channel credential is configured via SecretRef but unavailable in the current command path, it reports that account as configured with degraded notes instead of showing it as not configured.
...
```bash
openagent channels resolve --channel discord "My Server/#support" "@someone"
openagent channels --agent ops resolve --channel discord "My Server/#general"
openagent channels resolve --agent ops --channel discord "My Server/#general"
```
