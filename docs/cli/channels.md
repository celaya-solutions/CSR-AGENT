...
Guided setup requires an interactive terminal. In a non-TTY shell, OpenAgent exits immediately instead of waiting for input; use `openclaw channels add --channel <id> --use-env` or pass the selected plugin's credential flags.
