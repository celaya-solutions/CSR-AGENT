...
days. With a query, an exact session key wins; otherwise OpenAgent requires a
...
OpenAgent does not borrow credentials or a TLS pin from a different configured
...
fails closed under the normal explicit-target policy. OpenAgent never scans
...
not query-aware, so OpenAgent never strips or copies that query into a
...
Run the command in an already configured OpenAgent terminal. The terminal
...
If OpenAgent reports an invalid `--handoff` payload, return to the session's
