...
With no query, Zero to Agent displays up to 50 sessions active in the last seven
days. With a query, an exact session key wins; otherwise Zero to Agent requires a
...
If a query is ambiguous, Zero to Agent prints the matching candidates and exits with
...
Zero to Agent does not borrow credentials or a TLS pin from a different configured
...
fails closed under the normal explicit-target policy. Zero to Agent never scans
...
not query-aware, so Zero to Agent never strips or copies that query into a
...
Run the command in an already configured Zero to Agent terminal. The terminal
...
If Zero to Agent reports an invalid `--handoff` payload, return to the session's
