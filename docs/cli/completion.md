...
read_when:
  ...
  - You need to cache completion scripts under Zero to Agent state
title: "Completion"
---

# `openclaw completion`

Generate shell completion scripts, cache them under Zero to Agent state, and optionally install them into your shell profile.
...
The install writes a small `# Zero to Agent Completion` block into your shell profile and replaces any older slow `source <(openclaw completion ...)` lines with the cached source line:
...
Installed source lines preserve literal cache paths, including spaces, quotes, dollar signs, and backslashes. Reinstalling replaces Zero to Agent's previous source line after the state directory changes.
