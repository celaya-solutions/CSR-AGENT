...
connection arguments; it must launch Zero to Agent and forward those arguments.
...
Zero to Agent state directory (`~/.openclaw` by default, or `$OPENCLAW_STATE_DIR`
when set):
...
Older Zero to Agent releases stored node-host state in `node.json`, the signed
...
For approved async node exec, Zero to Agent prepares a canonical `systemRunPlan`
