---
summary: "CLI reference for `openagent automations` (schedule and run background jobs)"
read_when:
...
# `openagent automations`

Manage automation jobs for the Gateway scheduler. The command registers as `openagent cron` with `openagent automations` as an alias. Every subcommand below works with either spelling.

<Tip>
Run `openagent automations --help` for the full command surface. See [Automations](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`openagent automations create` is an alias for `openagent automations add`. For new jobs, put the schedule first and the prompt second:
...
Use `--command` for deterministic shell-style jobs that run inside the OpenAgent scheduler without starting an isolated agent/model run:
...
If session cleanup fails, the error is logged. A removal with no active run also returns the cleanup error to the caller. Use `openagent sessions list --json` to find the remaining session, then `openagent sessions delete <key> --yes` to retry cleanup after the Gateway or worker recovers.

## Delivery

`openagent automations add`, `openagent automations list`, and `openagent automations show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
2. `delivery.failureDestination` on the job, layered over the global destination fields on `cron.failureAlert` (`mode`, `channel`, `to`, `accountId`). The `cron.failureDestination` block, retired in 2026.8.1, is merged into them by `openagent doctor --fix`.
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `openagent automations edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
Automation jobs, pending runtime state, and run history live in the shared SQLite state database. The file store it replaced in 2026.6.1 is still imported once: legacy `jobs.json`, `<name>-state.json`, and `runs/*.jsonl` files are read and then renamed with a `.migrated` suffix. After import, edit schedules with `openagent automations add|edit|remove` instead of editing JSON files.

### Manual runs

Manually running a disabled job does not enable its schedule or create automatic retries. Use `openagent automations enable <job-id>` to resume scheduled runs.

`openagent automations run <job-id>` force-runs by default and returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use the returned `runId` to inspect the later result:

```bash
openagent automations run <job-id>
openagent automations runs <job-id> --run-id <run-id>
```

Add `--wait` when a script should block until that exact queued run records a terminal status:

```bash
openagent automations run <job-id> --wait --wait-timeout 10m --poll-interval 2s
```
...
- When a job has `--model` but no fallback list is configured, OpenAgent passes an explicit empty fallback override. The agent primary is therefore not appended as a hidden retry target.
- Local-provider preflight checks walk configured fallbacks before marking a run `skipped`.

`openagent doctor` reports jobs that already have `payload.model` set, including provider namespace counts and mismatches against `agents.defaults.model`. Use that check when auth, provider, or billing behavior looks different between live chat and scheduled jobs.
...
```bash
openagent automations edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
openagent automations edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
openagent automations edit <job-id> --light-context
```

Announce to a specific channel:

```bash
openagent automations edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
openagent automations edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
```bash
openagent automations create "*/30 * * * *" \
...
openagent automations list --agent ops
openagent automations get <job-id>
openagent automations get <job-id> --json
openagent automations show <job-id>
openagent automations run <job-id>
openagent automations run <job-id> --due
openagent automations run <job-id> --wait --wait-timeout 10m
openagent automations run <job-id> --wait --wait-timeout 10m --poll-interval 2s
openagent automations runs <job-id> --limit 50
openagent automations runs <job-id> --limit 50 --json
openagent automations runs <job-id> --run-id <run-id>
openagent automations runs <job-id> --status error --query timeout
openagent automations runs <job-id> --delivery-status not-delivered
openagent automations runs <job-id> --sort asc --offset 200 --limit 50
```
...
An unresolved owner does not stop the scheduler: that job is skipped with an explanation in `state.lastError`, while other jobs continue. Run `openagent doctor --fix` to repair unresolved multi-agent legacy ownership, set `agents.defaults.systemAgent.agentId`, or use `openagent cron edit <job-id> --agent <id>` to repair the job. Sole-agent rosters and legacy default markers honored by the runtime already resolve an owner and need no owner migration. The explanation stays on the job. No agent run-history entry is created, because no agent run starts.

`--json` always requests JSON output. Commands whose product is already a machine-readable result emit JSON results by default: `add`/`create`, `status`, `enable`, `disable`, `rm`/`remove`/`delete`, `run`, `edit`, `get`, and `runs`. They accept `--json` as the explicit machine-output spelling. `openagent automations get <job-id>` returns the stored job JSON directly. Use `automations show <job-id>` when you want the human-readable view with delivery-route preview.
...
```bash
openagent automations scratch <job-id>                  # print current scratch content
openagent automations scratch <job-id> --json           # scratch plus revision metadata
openagent automations scratch <job-id> --set "text"     # replace scratch with exact text
openagent automations scratch <job-id> --file notes.md  # replace scratch from a file (- for stdin)
openagent automations scratch <job-id> --unset          # remove the scratch row
```
...
```bash
openagent automations edit <job-id> --agent ops
openagent automations edit <job-id> --clear-agent
openagent automations edit <job-id> --session current
openagent automations edit <job-id> --session "session:daily-brief"
```

`openagent automations add` warns when `--agent` is omitted on agent-turn jobs and falls back to the default agent (`main`). Pass `--agent <id>` at create time to pin a specific agent.

Delivery tweaks:

```bash
openagent automations edit <job-id> --announce --channel slack --to "channel:C1234567890"
openagent automations edit <job-id> --webhook "https://example.invalid/openclaw/cron"
openagent automations edit <job-id> --best-effort-deliver
openagent automations edit <job-id> --no-best-effort-deliver
openagent automations edit <job-id> --no-deliver
```
