...
Use `--command` for deterministic shell-style jobs that run inside the Zero to Agent scheduler without starting an isolated agent/model run:
...
- When a job has `--model` but no fallback list is configured, Zero to Agent passes an explicit empty fallback override. The agent primary is therefore not appended as a hidden retry target.
