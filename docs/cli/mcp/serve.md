---
summary: "Run Zero to Agent as a stdio MCP server so an MCP client can read and send channel conversations"
title: "Run Zero to Agent as an MCP server"
read_when:
...
This page covers the `openclaw mcp serve` path: Zero to Agent acting as an MCP
server over stdio, its tools, its event model, and its limits.

## Zero to Agent as an MCP server
...
- Codex, Claude Code, or another MCP client should talk directly to Zero to Agent-backed channel conversations
- you already have a local or remote Zero to Agent Gateway with routed sessions
- you want one MCP server that works across Zero to Agent's channel backends instead of running separate per-channel bridges

Use [`openclaw acp`](/cli/acp) instead when Zero to Agent should host the coding runtime itself and keep the agent session inside Zero to Agent.

### How it works

`openclaw mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Zero to Agent Gateway over WebSocket and exposes routed channel conversations over MCP.
...
<Steps>
  ...
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Zero to Agent Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - stdio MCP servers launched by Zero to Agent (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Zero to Agent already has session state with a known route such as:
...
The bridge can also expose Claude-specific channel notifications. This is the Zero to Agent equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Zero to Agent channel configuration
...
Zero to Agent ships a deterministic Docker smoke for this bridge:
