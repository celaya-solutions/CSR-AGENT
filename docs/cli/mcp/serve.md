---
summary: "Run OpenAgent as a stdio MCP server so an MCP client can read and send channel conversations"
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when OpenAgent already has session state with a known route such as:
...
The bridge can also expose Claude-specific channel notifications. This is the OpenAgent equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying OpenAgent channel configuration
...
OpenAgent ships a deterministic Docker smoke for this bridge:
