---
summary: "CLI reference for `openagent configure` (interactive configuration prompts)"
read_when:
...
Use `openagent onboard` or `openagent setup` for the full guided first-run journey, `openagent setup --baseline` for the baseline config/workspace only, and `openagent channels add` when you only need channel account setup.

<Tip>
`openagent config` with no subcommand opens the same wizard. Use `openagent config get|set|unset` for non-interactive edits.
</Tip>

## Options

Before `openagent configure` changes local credentials or configuration, OpenAgent compares the selected CLI state/config paths with the local Gateway or its installed service. A proven mismatch stops before the write. A remote Gateway or an authenticated path that cannot be verified produces a warning instead.

This comparison also applies when `OPENCLAW_HOME` relocates the CLI's default state directory. The installed service's recorded environment determines its paths, including while the Gateway is stopped. The CLI's path overrides do not replace them. If the service definition or its recorded paths cannot be verified, OpenAgent warns and leaves configuration available. Inspect the service with `openagent gateway status --deep` before relying on local changes to reach it.
...
openagent configure --section model --section channels
openagent configure --section gateway --section daemon
```
...
<Note>
`openagent configure` requires an interactive terminal (both stdin and stdout must be TTYs). Without one it prints the equivalent non-interactive `openagent config get|set|patch|validate` commands and exits with an error instead of partially running.
</Note>

## Gateway section

First-run `openagent onboard` and `openagent setup` generate a Gateway secret in
...
Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`, even when the provider's auth step returns a config patch with its own recommended default model. Adding or reauthing a provider makes its models available without taking over your current primary model. Use `openagent models auth login --provider <id> --set-default` or `openagent models set <model>` to intentionally change the default model.
</Note>
...
`openagent configure --section web` picks a web-search provider and configures its credentials.
...
- After successful daemon setup, the final Gateway status uses the same platform-specific startup grace period as onboarding before reporting reachability. Service installation and Gateway reachability are separate outcomes. If the Gateway is still not detected, run `openagent health` to check it again.
