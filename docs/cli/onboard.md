...
read_when:
  - You want to establish inference, then finish setup with Zero to Agent
title: "Onboard"
---
...
Zero to Agent to configure the rest. `openclaw setup` reaches this flow on fresh
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Zero to Agent onboarding fits together.
  </Card>
...
- `--modern` is a compatibility alias for the Zero to Agent conversational setup
...
API-key or token methods use the same live completion path. Zero to Agent persists
...
In guided mode, `--workspace <dir>` supplies Zero to Agent's proposed workspace
and the isolated inference context. It is not persisted until you approve the
Zero to Agent setup proposal. Classic and noninteractive onboarding persist their
...
If applying setup fails, onboarding falls back to the conversational Zero to Agent
chat to finish interactively. Channels, agents,
plugins, and other optional features remain Zero to Agent chat territory: run
...
provider or its authentication, exit Zero to Agent and run `openclaw onboard`;
Zero to Agent does not open the guided or classic provider flows.
...
Zero to Agent will not start until its own live inference check passes.
...
- If the config file exists but fails validation, it starts the classic
  onboarding path with `openclaw doctor` guidance. Zero to Agent needs working
...
- If the config file is valid, it opens the normal agent TUI. A reachable
  ...
  onboarding or Zero to Agent. On a configured install, reach Zero to Agent with
...
Zero to Agent's official plugin catalog. If the required plugin is already installed
...
With `--modern`, JSON is a one-shot Zero to Agent overview and exits after that
