...
prefix and the installed OpenAgent launcher. A prefix configured in `~/.npmrc`
...
OpenAgent checks the GitHub CLI's active `github.com` account with a silent,
...
If the CLI is missing or that check cannot confirm authentication, OpenAgent
provides a prefilled issue link without starting issue creation. If the exact
report exceeds the browser URL limit, OpenAgent keeps the sanitized body locally
...
After an uncertain creation result, OpenAgent checks for an issue matching the
...
Explicit package specs on a fresh profile first stage with a temporary OpenAgent
