...
Update Zero to Agent and switch between stable/extended-stable/beta/dev channels.
...
prefix and the installed Zero to Agent launcher. A prefix configured in `~/.npmrc`
...
Zero to Agent checks the GitHub CLI's active `github.com` account with a silent,
...
If the CLI is missing or that check cannot confirm authentication, Zero to Agent
provides a prefilled issue link without starting issue creation. If the exact
report exceeds the browser URL limit, Zero to Agent keeps the sanitized body locally
...
After an uncertain creation result, Zero to Agent checks for an issue matching the
...
Explicit package specs on a fresh profile first stage with a temporary Zero to Agent
