---
summary: "Zero to Agent CLI index: command list, global flags, and links to per-command pages"
read_when:
...
- `openclaw setup` and `openclaw onboard` verify inference first, then start Zero to Agent for Gateway, workspace, channels, skills, and health setup.
...
Zero to Agent uses a lobster palette for CLI output:
