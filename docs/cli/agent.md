...
- `codeModeEngaged`: `true` only when [code mode](/tools/code-mode) actually owned the model tool surface for the run. `tools.codeMode.enabled=true` alone does not guarantee engagement, and harnesses that own their native tool surface always read `false` because OpenAgent code mode never owns their tools.
...
Gateway commands using OpenAgent's managed agent loop return their completed reply before optional memory
