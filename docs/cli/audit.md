...
agent, run, and action. Missing message provenance renders as `-`; Zero to Agent
...
Zero to Agent never silently selects the first or latest execution. The exact text
...
- An action performed wholly inside an ACP or other external native runtime
  without a Zero to Agent pre-action callback produces an ACP-owner `unsupported`
