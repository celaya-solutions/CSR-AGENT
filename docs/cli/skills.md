...
Zero to Agent does not download from skills.sh. These entries are shown as
...
Zero to Agent prints the reason and findings, then asks `type: '<skill>' to install
anyway` (or `update anyway`). If the fully rendered review exceeds 4,000
characters, Zero to Agent fails closed before prompting; reduce or coalesce the
...
| Skill Card fingerprint           | Installed ClawHub bundles can include a generated `skill-card.md`. Zero to Agent treats verification as a ClawHub server decision and does not reject an installed skill just because that generated card changes the bundle fingerprint.                                                                                         |
...
Official ClawHub skill publishers and bundled Zero to Agent skill sources bypass
...
agent-specific skill, or the Zero to Agent state directory for a shared skill
