---
summary: "Connect a machine to a Zero to Agent Gateway with one pasted command"
read_when:
...
Connect the current machine to a Zero to Agent Gateway as a headless node. The
...
Zero to Agent completes the first authenticated connection before installing the
...
succeeds does Zero to Agent persist `nodeHost.workerRuns.enabled=true`, preserving
...
unreadable, or not a regular file, Zero to Agent leaves it in place. A symlink is
allowed; Zero to Agent reads its target, removes the symlink after a successful read,
