---
summary: "Connect a machine to an OpenAgent Gateway with one pasted command"
read_when:
...
OpenAgent completes the first authenticated connection before installing the
...
succeeds does OpenAgent persist `nodeHost.workerRuns.enabled=true`, preserving
...
unreadable, or not a regular file, OpenAgent leaves it in place. A symlink is
allowed; OpenAgent reads its target, removes the symlink after a successful read,
