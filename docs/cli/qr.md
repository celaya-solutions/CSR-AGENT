...
Official Zero to Agent iOS and Android apps connect automatically when their
...
Plaintext LAN `ws://` setup remains available, but Zero to Agent automatically uses
...
The QR command advertises Tailscale URLs only when Zero to Agent owns the route through `gateway.tailscale.mode=serve|funnel`. Legacy external Serve routes that target the ordinary Gateway listener are not advertised because that listener rejects Tailscale-shaped proxy ingress.
