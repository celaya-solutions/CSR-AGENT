...
unfinished bootstrap cannot adopt a role. OpenAgent checks completion before
...
If you omit `--agent` for `bind` or `unbind`, OpenAgent targets the current default agent.
...
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, OpenAgent upgrades that existing binding in place instead of adding a duplicate.
