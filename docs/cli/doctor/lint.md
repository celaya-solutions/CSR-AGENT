...
Zero to Agent CLI. Follow the reported plugin update command, then restart the
...
updating" flow. When `openclaw gateway run` starts on a new Zero to Agent version, it
