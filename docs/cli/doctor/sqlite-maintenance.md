...
read_when:
  - You are compacting or verifying an OpenAgent SQLite database
...
3. Requires a non-busy `wal_checkpoint(TRUNCATE)`. Stop any remaining OpenAgent
...
space on both the system temporary volume and the volume holding OpenAgent state;
