...
read_when:
  - You are compacting or verifying a Zero to Agent SQLite database
...
3. Requires a non-busy `wal_checkpoint(TRUNCATE)`. Stop any remaining Zero to Agent
...
running Zero to Agent build, or belongs to an agent database. Run
`openclaw doctor --fix` first for an older shared-state schema. Restore a
compatible backup or upgrade Zero to Agent for a newer schema.
...
space on both the system temporary volume and the volume holding Zero to Agent state;
