---
summary: "CLI reference for `openagent pairing` (approve/list pairing requests)"
read_when:
...
Approve or inspect DM pairing requests for channels that support pairing (chat DMs only - node/device pairing uses [`openagent devices`](/cli/devices)).
...
openagent pairing list --channel telegram --account work
openagent pairing list telegram --json

openagent pairing approve <code>
openagent pairing approve telegram <code>
openagent pairing approve --channel telegram --account work <code> --notify
```
...
- `openagent pairing approve --channel <channel> <code>`
- `openagent pairing approve <code>` when exactly one pairing-capable channel is configured
...
If you approved a sender before the first-owner bootstrap shipped in 2026.4.29, run [`openagent doctor`](/cli/doctor). It warns when no command owner is configured. It also shows the exact `openagent config set commands.ownerAllowFrom ...` command to fix it.

## Related

- [CLI reference](/cli)
- [Channel pairing](/channels/pairing)
- [`openagent qr`](/cli/qr) — mobile/device bootstrap QR and setup code, not a channel DM pairing code
