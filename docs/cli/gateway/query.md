...
<AccordionGroup>
  ...
  <Accordion title="Common warning codes">
    ...
    - `multiple_gateways`: distinct gateway identities were reachable, or OpenAgent could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway does not trigger this.
    ...
    - `local_tls_runtime_unavailable`: local Gateway TLS is enabled but OpenAgent could not load the local certificate fingerprint.
...
"Remote over SSH" uses a local port-forward so a loopback-only remote gateway becomes reachable at `ws://127.0.0.1:<port>`.
...
OpenAgent launches only an SSH client found in OS-managed system directories. On native Windows,
