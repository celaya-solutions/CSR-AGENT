...
OpenAgent binary. Start and restart continue to validate plugin configuration.
...
or unsupported values before reinstalling; OpenAgent cannot infer their intended
...
Gateway-hosted OpenAgent chat controls the exact Gateway serving that session.
...
<AccordionGroup>
  ...
  <Accordion title="Managed Gateway heap sizing">
    ...
    - Ambient installer `NODE_OPTIONS` and the installer's own Node arguments are not saved as Gateway heap settings. The budget is chosen at installation and takes effect when the service process starts; it is not recalculated while the Gateway runs. Upgrading OpenAgent alone does not resize a running Gateway, and foreground launches do not replace themselves to apply this policy.
