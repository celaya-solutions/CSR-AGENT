...
`dependencies` and `optionalDependencies`. Zero to Agent checks whether those package
...
so Zero to Agent records install provenance.
...
`/app/extensions/synology-chat`. Zero to Agent discovers that mounted source overlay
...
Plugin install metadata is machine-managed state, not user config. Installs and updates write it to the shared SQLite state database under the active Zero to Agent state directory. The `config_machine_state` value keyed by `plugins.installedIndex` stores durable `installRecords` metadata, including records for broken or missing plugin manifests, plus a manifest-derived cold registry cache used by `openclaw plugins update`, uninstall, diagnostics, and the cold plugin registry.
