...
`dependencies` and `optionalDependencies`. OpenAgent checks whether those package
...
so OpenAgent records install provenance.
...
`/app/extensions/synology-chat`. OpenAgent discovers that mounted source overlay
