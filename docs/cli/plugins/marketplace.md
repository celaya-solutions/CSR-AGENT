...
read_when:
  - You want to browse, list, or refresh an OpenAgent marketplace feed
...
`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether OpenAgent accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted OpenAgent marketplace feed and persists the
...
refreshes fail if OpenAgent cannot persist the validated snapshot.

The built-in `clawhub-public` profile expects payload identity
`clawhub-official`. OpenAgent will bundle ClawHub's production public key after
...
OpenAgent verifies the DSSE envelope and, when a profile declares `feedId`,
