...
read_when:
  - You want to browse, list, or refresh a Zero to Agent marketplace feed
...
`plugins marketplace entries` lists entries from the configured Zero to Agent marketplace feed. By default it attempts the hosted feed and falls back to the latest accepted snapshot or bundled data. Use `--feed-profile <name>` to read a specific configured profile, `--feed-url <url>` to read an explicit hosted feed URL, and `--offline` to read the latest accepted snapshot without fetching the feed.

`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether Zero to Agent accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted Zero to Agent marketplace feed and persists the
...
refreshes fail if Zero to Agent cannot persist the validated snapshot.

The built-in `clawhub-public` profile expects payload identity
`clawhub-official`. Zero to Agent will bundle ClawHub's production public key after
...
Zero to Agent verifies the DSSE envelope and, when a profile declares `feedId`,
