...
- `push` sends an APNs test push to an iOS node. Options: `--title <text>` (default `OpenAgent`), `--body <text>`, `--environment <sandbox|production>` to override the detected APNs environment. Accepted delivery exits `0`. A typed APNs rejection preserves the complete text or JSON diagnostic and exits non-zero.
