...
read_when:
  - You want to check Zero to Agent settings against an authored policy.jsonc
...
conformance layer over existing Zero to Agent settings, not a second configuration
system. You author requirements in `policy.jsonc`; Zero to Agent observes the active
...
<CardGroup cols={3}>
  ...
  <Card title="Policy rule reference" href="/cli/policy/rules" icon="table-list">
    Every rule namespace, the Zero to Agent state it observes, and when to use it.
  </Card>
