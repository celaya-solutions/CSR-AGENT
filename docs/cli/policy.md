...
read_when:
  - You want to check OpenAgent settings against an authored policy.jsonc
...
conformance layer over existing OpenAgent settings, not a second configuration
system. You author requirements in `policy.jsonc`; OpenAgent observes the active
...
<CardGroup cols={3}>
  ...
  <Card title="Policy rule reference" href="/cli/policy/rules" icon="table-list">
    Every rule namespace, the OpenAgent state it observes, and when to use it.
  </Card>
