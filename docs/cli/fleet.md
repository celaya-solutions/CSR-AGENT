---
summary: "CLI reference for provisioning and managing isolated per-tenant OpenAgent cells"
read_when:
...
Cell state and the legacy auth-profile encryption key use separate per-tenant host paths under the active OpenAgent state directory:
