---
summary: "CLI reference for provisioning and managing isolated per-tenant Zero to Agent cells"
read_when:
...
`openclaw fleet` manages complete Zero to Agent instances called **cells**. Each cell has its own Gateway, state, credentials, channel accounts, container, and loopback-only host port. Use one cell for each tenant trust boundary; do not use one shared Gateway as a hostile multi-tenant boundary.
...
Cell state and the legacy auth-profile encryption key use separate per-tenant host paths under the active Zero to Agent state directory:
