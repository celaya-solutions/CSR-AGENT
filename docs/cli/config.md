...
OpenAgent CLI processes when a deployment system manages your config:
...
or current value. OpenAgent's config snapshot guard still rejects a later race between the
...
The runtime pin makes this an embedded OpenAgent recipe. A valid `fastMode`
value is a portable typed runtime control and does not choose OpenAgent by
itself.
...
OpenAgent-owned writes that change config reserialize JSON5 as standard JSON. When the source contains comments, the writer warns immediately before removing them; use a direct editor when preserving comments matters.
