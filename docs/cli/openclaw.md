---
summary: "CLI reference and security model for the inference-backed Zero to Agent setup and repair helper"
read_when:
  - You finished inference setup and want Zero to Agent to configure the rest
  - You need to inspect or repair Zero to Agent with the local setup agent
  - You are designing or enabling message-channel rescue mode
title: "Zero to Agent setup agent"
---

# `openclaw setup`

Zero to Agent ships with a built-in system agent — it speaks as "Zero to Agent" — for
...
- Config exists and is valid: opens the normal agent TUI. A reachable
  ...
  without onboarding or Zero to Agent. Use `/openclaw` inside the TUI, or run
  `openclaw setup` directly, to reach Zero to Agent later.

Running `openclaw setup` first live-tests the configured default model. A passing turn starts Zero to Agent. An interactive failure opens guided inference setup and hands off to Zero to Agent after a candidate passes. One-shot, JSON, and other noninteractive requests fail with instructions to run [`openclaw onboard`](/cli/onboard) when inference is unavailable. `openclaw --help` and `openclaw --version` keep their normal fast paths.

If inference plugin loading or owner verification fails, the error includes the underlying cause after applying Zero to Agent's error redaction. One-shot text and JSON output retain that detail alongside onboarding guidance.
...
`openclaw onboard --modern` remains a compatibility alias for Zero to Agent, but uses the same inference gate: working inference opens the chat, interactive failures start guided inference setup, and noninteractive failures exit with onboarding guidance. `openclaw onboard --classic` opens the full step-by-step wizard.

## What Zero to Agent shows

Interactive Zero to Agent opens the same TUI shell as `openclaw tui`, with a Zero to Agent chat backend. The startup greeting covers:

- config validity and the default agent
- the verified model Zero to Agent is using
...
Zero to Agent uses the same reference discovery as regular agents: in a Git checkout it points at local `docs/` and the source tree; in an npm install it uses bundled docs and links to [https://github.com/openclaw/openclaw](https://github.com/openclaw/openclaw), with guidance to check source when docs are not enough.
...
Zero to Agent uses typed operations instead of editing config ad hoc.
...
In direct Zero to Agent chat, persistent operations require conversational approval (or `--yes` for a one-shot command): write config, `config set`, `config set-ref`, setup/onboarding bootstrap, change the default model, start/stop/restart the Gateway, create agents, and install plugins.
...
mode. Restricted runs still require approval in the Zero to Agent operator UI. Replying
...
Configured agents can ask Zero to Agent to create another agent through their
...
Zero to Agent remains the executor, and authorized creation records that requesting
agent as the new agent's creator.

Delegated creation remains tied to the requesting run. If that run ends or loses
authority during preparation, Zero to Agent stops before starting the next persistent
...
Doctor repairs are unavailable inside Zero to Agent because they can rewrite the provider, authentication, or default-agent inference route powering the session. Exit Zero to Agent and run `openclaw doctor --fix` in a terminal. Read-only `doctor` remains available inside Zero to Agent.
...
configure or repair provider/auth access, exit Zero to Agent and run
`openclaw onboard`.
...
refused for the same reason; exit Zero to Agent and run
...
The Ask Zero to Agent page can show recent applied system-agent operations, Doctor
...
is watching, during a Zero to Agent-owned write, or at the next startup after an
offline edit.
...
until they reach a secret. The local Zero to Agent TUI does not accept sensitive wizard answers
...
Zero to Agent never changes provider/auth access from inside its own session: the
...
starting a wizard or writing config. Exit Zero to Agent and run `openclaw
onboard`; onboarding stages the credentials and saves only a route that
completes a real live turn. Start Zero to Agent again after onboarding succeeds.
...
Zero to Agent stops before starting the next persistent effect. Earlier completed
...
If inference is missing or its live check fails, leave Zero to Agent and run `openclaw onboard`. Guided onboarding tries the configured model first, then authenticated subscription CLIs, API keys, and remaining supported CLIs; it asks each candidate for a real reply and persists only a passing route. Zero to Agent starts immediately after that boundary and can then configure the workspace, Gateway, channels, agents, plugins, and other optional features.
...
credential, and provider/runtime state needed for that route after the test passes. Workspace and Gateway defaults remain for Zero to Agent. A failing candidate
...
working and Zero to Agent has started; existing plugin policy and explicit
supervision opt-outs remain untouched during inference setup.

## AI conversation

Interactive Zero to Agent's free-form conversation runs through the same agent loop as regular Zero to Agent agents, restricted to one ring-zero Zero to Agent authority tool, `openclaw`, that wraps the typed operations. Read actions run freely, mutations require your conversational approval for that exact operation (see Operations and approval), and every applied write is audited and re-validated. The agent session persists, so Zero to Agent has real multi-turn memory. If the verified inference route later stops working, return to `openclaw onboard` and repair it before continuing.
...
restriction directly: the run carries a Zero to Agent tool allow-list with only
the `openclaw` tool. For Codex, Zero to Agent also disables environments, native
...
harnesses do not consume Zero to Agent's allow-list,
so Zero to Agent admits only backends whose own tool-selection contract can prove
...
- Backends that declare no native tools receive the same dedicated Zero to Agent
  MCP server.
- Always-on or unknown native-tool backends fail closed before inference; they
  cannot host a Zero to Agent session.

Only Zero to Agent sessions get the openclaw MCP server; normal agent runs
...
a single Zero to Agent authority tool plus the inert native planning utility. In all
three cases, setup writes remain confined to Zero to Agent's audited approval
contract.
...
Use a natural-language selector to leave Zero to Agent and open the normal TUI:
...
`openclaw tui`, `openclaw chat`, and `openclaw terminal` open the normal agent TUI directly; they do not start Zero to Agent. After switching into the normal TUI, `/openclaw` returns to Zero to Agent, optionally with a follow-up request:
...
Message rescue mode is the message-channel entrypoint for Zero to Agent: use it when your normal agent is dead but a trusted channel (for example WhatsApp) still receives commands.

This is a deterministic emergency command handler, not the conversational
Zero to Agent agent. It does not bootstrap a fresh setup or relax the inference
gate for Zero to Agent chat.
...
Zero to Agent: Zero to Agent rescue mode. Gateway reachable: no. Config valid: no.
You: /openclaw restart gateway
Zero to Agent: Plan: restart the Gateway. Reply /openclaw yes to apply.
You: /openclaw yes
Zero to Agent: Applied. Audit entry written.
```
...
- Disabled when sandboxing is active for the agent/session; Zero to Agent refuses remote rescue and points to local CLI repair.
...
- Plugin search and list are read-only. Plugin install is always local-only (blocked in rescue, even when otherwise enabled) because it downloads executable code. Plugin uninstall is refused in both local Zero to Agent and rescue; run `openclaw plugins uninstall <id>` from a terminal.
...
That packaged-CLI lane starts with an empty state dir and proves Zero to Agent
...
Zero to Agent agent/tool/approval conversation. The QA Lab scenario below redirects
