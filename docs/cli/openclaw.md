---
summary: "CLI reference and security model for the inference-backed OpenAgent setup and repair helper"
read_when:
  - You finished inference setup and want OpenAgent to configure the rest
  - You need to inspect or repair OpenAgent with the local setup agent
...
OpenAgent uses typed operations instead of editing config ad hoc.
...
In direct OpenAgent chat, persistent operations require conversational approval (or `--yes` for a one-shot command): write config, `config set`, `config set-ref`, setup/onboarding bootstrap, change the default model, start/stop/restart the Gateway, create agents, and install plugins.
...
Configured agents can ask OpenAgent to create another agent through their
...
OpenAgent remains the executor, and authorized creation records that requesting
...
The Ask OpenAgent page can show recent applied system-agent operations, Doctor
...
is watching, during an OpenAgent-owned write, or at the next startup after an
offline edit.
...
until they reach a secret. The local OpenAgent TUI does not accept sensitive wizard answers
...
starting a wizard or writing config. Exit OpenAgent and run `openclaw
onboard`; onboarding stages the credentials and saves only a route that
completes a real live turn. Start OpenAgent again after onboarding succeeds.
...
working and OpenAgent has started; existing plugin policy and explicit
...
restriction directly: the run carries an OpenAgent tool allow-list with only
the `openclaw` tool. For Codex, OpenAgent also disables environments, native
...
harnesses do not consume OpenAgent's allow-list,
so OpenAgent admits only backends whose own tool-selection contract can prove
...
- Backends that declare no native tools receive the same dedicated OpenAgent
  MCP server.
- Always-on or unknown native-tool backends fail closed before inference; they
  cannot host an OpenAgent session.

Only OpenAgent sessions get the openclaw MCP server; normal agent runs
...
a single OpenAgent authority tool plus the inert native planning utility. In all
three cases, setup writes remain confined to OpenAgent's audited approval
contract.
...
Use a natural-language selector to leave OpenAgent and open the normal TUI:
...
OpenAgent: OpenAgent rescue mode. Gateway reachable: no. Config valid: no.
You: /openclaw restart gateway
OpenAgent: Plan: restart the Gateway. Reply /openclaw yes to apply.
You: /openclaw yes
OpenAgent: Applied. Audit entry written.
```
...
- Disabled when sandboxing is active for the agent/session; OpenAgent refuses remote rescue and points to local CLI repair.
