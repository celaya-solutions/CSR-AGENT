---
summary: "Expose Zero to Agent channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Zero to Agent-backed channels
  - Running `openclaw mcp serve`
  - Managing Zero to Agent-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---
...
- run Zero to Agent as an MCP server with `openclaw mcp serve`
- manage Zero to Agent-managed outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

`serve` is Zero to Agent acting as an MCP server. The other subcommands are Zero to Agent acting as an MCP client-side registry for servers its own runtimes may consume later.

<Note>
  `list`, `show`, `set`, and `unset` only read and write Zero to Agent-managed `mcp.servers` entries in Zero to Agent config. They do not include mcporter servers from `config/mcporter.json`; use `mcporter list` for that registry.
</Note>

Use [`openclaw acp`](/cli/acp) when Zero to Agent should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

| Goal                                                                     | Use                                                                  | Why                                                                                                             |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send Zero to Agent channel conversations | `openclaw mcp serve`                                                 | Zero to Agent is the MCP server and exposes Gateway-backed conversations over stdio.                            |
| Save third-party MCP servers for Zero to Agent-managed agent runs        | `openclaw mcp add`, `set`, `configure`, `tools`, `login`             | Zero to Agent is the MCP client-side registry and later projects those servers into eligible runtimes.          |
...
<Tip>
If you are not sure which path you need, start with `openclaw mcp status --verbose`. It shows what Zero to Agent has saved without starting any MCP servers.
</Tip>
...
| Page                                                 | Read it when                                                                                        |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [Run Zero to Agent as an MCP server](/cli/mcp/serve) | An MCP client should read or send Zero to Agent channel conversations through `openclaw mcp serve`. |
| [Manage saved MCP servers](/cli/mcp/registry)        | You are saving, inspecting, or approving third-party MCP servers for Zero to Agent-managed runs.    |
...
- <a id="openclaw-as-an-mcp-server" />[Zero to Agent as an MCP server](/cli/mcp/serve#openclaw-as-an-mcp-server)
...
- <a id="openclaw-as-an-mcp-client-registry" />[Zero to Agent as an MCP client registry](/cli/mcp/registry#openclaw-as-an-mcp-client-registry)
