...
read_when:
  - You want Claude Code to use Zero to Agent Gateway MCP tools
...
The bearer token is passed through environment variables, not argv. Zero to Agent launches Claude Code with `--strict-mcp-config --mcp-config <path>` so ambient Claude MCP servers do not join the attached session. Normal launches (without `--print-config`) revoke the grant when the Claude Code process exits.
...
session but does not identify an active Zero to Agent run or its original caller
