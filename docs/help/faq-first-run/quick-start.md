...
<AccordionGroup>
  <Accordion title="Recommended way to install and set up Zero to Agent">
    ```bash
...
<a id="i-am-stuck" />
  ...
  <Accordion title="Can I migrate my setup to a new machine without redoing onboarding?">
    ...
    1. Install Zero to Agent on the new machine.
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    Still reproducing this on latest Zero to Agent? Track/report it: [Issue #30640](https://github.com/openclaw/openclaw/issues/30640).
  ...
  <Accordion title="How do I install Zero to Agent on Linux?">
  ...
  <Accordion title="How do I install Zero to Agent on a VPS?">
  ...
  <Accordion title="Can I ask Zero to Agent to update itself?">
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. Run Zero to Agent with **API keys** (Anthropic/OpenAI/others) or **local-only models**
    ...
    Zero to Agent also supports hosted subscription-style options including **Qwen Cloud
  ...
  <Accordion title="Can I use Claude Max subscription without an API key?">
    Yes. Zero to Agent supports Claude CLI reuse for Pro/Max/Team/Enterprise plans. Anthropic
    currently treats the `claude -p` path Zero to Agent uses as subscription-plan usage subject
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    Anthropic setup-token auth is also still a supported token path, but Zero to Agent prefers
