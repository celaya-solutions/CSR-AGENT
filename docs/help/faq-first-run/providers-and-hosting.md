...
<AccordionGroup>
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Set a **fallback model** so Zero to Agent keeps replying while a provider is rate-limited.
  ...
  <Accordion title="Is AWS Bedrock supported?">
    Yes. Zero to Agent has a bundled **Amazon Bedrock (Converse)** provider. With AWS env
    markers present (`AWS_ACCESS_KEY_ID`, `AWS_PROFILE`, `AWS_BEARER_TOKEN_BEDROCK`),
    Zero to Agent auto-enables the implicit Bedrock provider for model discovery; otherwise
  ...
  <Accordion title="How does Codex auth work?">
    Zero to Agent supports **OpenAI Codex** via OAuth (ChatGPT sign-in). A fresh
    ...
    `openai/gpt-5.5` explicitly; Zero to Agent does not silently downgrade. Legacy
  ...
  <Accordion title="Why does Zero to Agent still mention legacy OpenAI Codex prefix?">
  ...
  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    ...
    tools/workflows like Zero to Agent. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="Can I use Gemini CLI or Antigravity OAuth?">
    Zero to Agent does not offer new Gemini CLI OAuth or Antigravity OAuth setup.
    ...
    Gemini CLI OAuth profiles remain executable for compatibility, but Zero to Agent
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Zero to Agent needs large context + strong safety; small cards truncate context
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Zero to Agent runs on macOS or Linux (Windows via WSL2). A Mac mini is a popular
  ...
  <Accordion title="If I buy a Mac mini to run Zero to Agent, can I connect it to my MacBook Pro?">
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Zero to Agent instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    Running Zero to Agent via systemd: make sure the service PATH includes
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    delete your data** - only the Zero to Agent code install changes. State (`~/.openclaw`) and
  ...
  <Accordion title="How important is it to run Zero to Agent on a dedicated machine?">
  ...
  <Accordion title="Can I run Zero to Agent in a VM and what are the requirements?">
