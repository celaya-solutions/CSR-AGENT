...
The direct `OpenAgent Release Checks` child alone may use `rerun_group=qa` as a
...
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. Verifies hidden OpenAgent runtime context persists as a
...
- `pnpm test:docker:npm-telegram-live`
  - Installs an OpenAgent package candidate in Docker, runs installed-package
...
- `pnpm test:docker:plugins`
  - Packs and installs the current OpenAgent build in Docker, starts the
