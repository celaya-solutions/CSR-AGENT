...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Zero to Agent reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.openclaw/openclaw.json`). If the file is missing, it uses safe-ish defaults, including a default workspace of `~/.openclaw/workspace`.
  </Accordion>
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Zero to Agent enforces gateway auth by default, including loopback. If no explicit auth path is configured, startup resolves to token mode and generates a runtime-only token for that startup, so local WS clients must authenticate. This blocks other local processes from calling the Gateway.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended**: `openclaw configure --section web` and pick a provider.
    ...
    - If `tools.web.fetch.provider` is omitted, Zero to Agent auto-detects the first ready fetch fallback provider from available credentials; the official Firecrawl plugin provides that fallback.
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    Current Zero to Agent protects most accidental clobbers:

    - Zero to Agent-owned config writes validate the full post-change config before writing.
    - Invalid or destructive Zero to Agent-owned writes are rejected and saved as `openclaw.json.rejected.*`.
  ...
  <Accordion title="Can the Zero to Agent browser run headless?">
    Yes:
