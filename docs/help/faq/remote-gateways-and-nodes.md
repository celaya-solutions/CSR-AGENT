...
<AccordionGroup>
  ...
  <Accordion title="Can two Zero to Agent instances talk to each other (local + VPS)?">
  ...
  <Accordion title="Do nodes run a gateway service?">
    ...
    [Config hot reload](/gateway/configuration/hot-reload) for the per-setting rules.
