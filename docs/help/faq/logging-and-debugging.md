...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - Windows: `schtasks /Query /TN "OpenAgent Gateway (<profile>)" /V /FO LIST`.
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. OpenAgent already trims to the Telegram limit and retries with fewer commands, but some menu entries may still be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
