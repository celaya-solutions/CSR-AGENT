...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Zero to Agent instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Zero to Agent but keep it installed?">
    ```bash
    openclaw reset
    ```
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Zero to Agent runs on **your own account** - if you are in the group, Zero to Agent can see it. By default, group replies are blocked until you allow senders (`groupPolicy: "allowlist"`).
  ...
  <Accordion title="Why does Zero to Agent not reply in a group?">
