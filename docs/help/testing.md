---
summary: "Index of the OpenAgent testing kit, one page per reader job"
read_when:
...
OpenAgent has three Vitest suites (unit/integration, e2e, live) plus Docker
...
This page is an index. The testing kit is documented on these pages, one per
...
here, so an existing link such as `/help/testing#quick-start` still
