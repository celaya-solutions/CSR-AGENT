---
summary: "Symptom first troubleshooting hub for Zero to Agent"
read_when:
  - Zero to Agent is not working and you need the fastest path to a fix
...
## Local OpenAI-compatible backend works directly but fails in Zero to Agent
...
2. Still fails only on Zero to Agent agent turns: set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. Tiny direct calls work but larger Zero to Agent prompts crash the backend: that
   is an upstream model/server limit, not a Zero to Agent bug. Continue in
...
shape Zero to Agent no longer accepts.
...
versions normally move with the Zero to Agent release, so a Zero to Agent update can
...
- Freezing Zero to Agent-owned plugins to one exact old version (for example, only
  `@openclaw/*@2026.5.3`).
...
Zero to Agent as the user that owns the state directory.
...
If you intentionally run Zero to Agent as root, repair the managed plugin root
instead:
...
```mermaid
flowchart TD
  A[Zero to Agent is not working] --> B{What breaks first}
  B --> C[No replies]
