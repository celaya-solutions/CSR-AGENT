---
summary: "Symptom first troubleshooting hub for OpenAgent"
read_when:
  - OpenAgent is not working and you need the fastest path to a fix
...
2. Still fails only on OpenAgent agent turns: set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. Tiny direct calls work but larger OpenAgent prompts crash the backend: that
   is an upstream model/server limit, not an OpenAgent bug. Continue in
...
shape OpenAgent no longer accepts.
...
versions normally move with the OpenAgent release, so an OpenAgent update can
...
- Freezing OpenAgent-owned plugins to one exact old version (for example, only
  `@openclaw/*@2026.5.3`).
...
OpenAgent as the user that owns the state directory.
...
```mermaid
flowchart TD
  A[OpenAgent is not working] --> B{What breaks first}
  B --> C[No replies]
