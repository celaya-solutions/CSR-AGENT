---
summary: "Connect OpenAgent to Radius with browser sign-in or an organization API key"
read_when:
  - You want to use Radius models in OpenAgent
...
clients. The `radius` plugin connects OpenAgent's agent loop to Radius, including
...
sign in from OpenAgent:

```bash
openclaw models auth login --provider radius --method oauth --set-default
```

OpenAgent displays a pairing code and opens the Radius verification page. Enter
...
OpenAgent stores the OAuth credentials in its auth profiles and refreshes them
automatically. Signing in to Pi separately does not sign OpenAgent in.
...
OpenAgent discovers the catalog from Radius using your credential. This includes
