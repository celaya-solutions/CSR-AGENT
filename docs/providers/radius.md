---
summary: "Connect Zero to Agent to Radius with browser sign-in or an organization API key"
read_when:
  - You want to use Radius models in Zero to Agent
...
clients. The `radius` plugin connects Zero to Agent's agent loop to Radius, including
...
sign in from Zero to Agent:

```bash
openclaw models auth login --provider radius --method oauth --set-default
```

Zero to Agent displays a pairing code and opens the Radius verification page. Enter
...
Zero to Agent stores the OAuth credentials in its auth profiles and refreshes them
automatically. Signing in to Pi separately does not sign Zero to Agent in.
...
Zero to Agent discovers the catalog from Radius using your credential. This includes
