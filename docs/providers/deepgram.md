...
Deepgram is a speech-to-text API. Zero to Agent uses it for inbound audio/voice-note
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Zero to Agent transcribes it
...
Use `flux-general-en` or `flux-general-multi` for Deepgram Flux. Zero to Agent
...
`redact`, and `tag` in `providerOptions.deepgram`. Zero to Agent ignores batch-only
options such as `detect_language`, `punctuate`, and `smart_format` on Flux.
Language hints apply only to `flux-general-multi`: Zero to Agent maps the model entry's
