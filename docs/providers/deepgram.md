...
Deepgram is a speech-to-text API. OpenAgent uses it for inbound audio/voice-note
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. OpenAgent transcribes it
...
Use `flux-general-en` or `flux-general-multi` for Deepgram Flux. OpenAgent
...
`redact`, and `tag` in `providerOptions.deepgram`. OpenAgent ignores batch-only
options such as `detect_language`, `punctuate`, and `smart_format` on Flux.
Language hints apply only to `flux-general-multi`: OpenAgent maps the model entry's
