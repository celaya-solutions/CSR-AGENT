...
SenseAudio transcribes inbound audio and voice-note attachments through Zero to Agent's shared `tools.media.audio` pipeline. Zero to Agent posts multipart audio to the OpenAI-compatible transcription endpoint and injects the returned text as `{{Transcript}}` plus an `[Audio]` block.
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. Zero to Agent uploads the
...
<Note>
SenseAudio is batch STT only in Zero to Agent. [Voice Call](/plugins/voice-call)
