...
title: "LongCat"
read_when:
  - You want to use LongCat-2.0 with OpenAgent
  - You need the LongCat API key or model limits
---

[LongCat](https://longcat.ai) provides a hosted API for LongCat-2.0, a
reasoning model built for coding and agentic workloads. OpenAgent provides the
...
LongCat exposes binary thinking control. OpenAgent maps enabled thinking levels
to `thinking: { type: "enabled" }` and `/think off` to
`thinking: { type: "disabled" }`. OpenAgent removes `reasoning_effort`
from LongCat requests.

LongCat returns reasoning in `reasoning_content`. OpenAgent preserves that field
...
model through an OpenAI-compatible runtime and use OpenAgent's existing
