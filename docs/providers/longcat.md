...
title: "LongCat"
read_when:
  - You want to use LongCat-2.0 with Zero to Agent
  - You need the LongCat API key or model limits
---

[LongCat](https://longcat.ai) provides a hosted API for LongCat-2.0, a
reasoning model built for coding and agentic workloads. Zero to Agent provides the
...
LongCat exposes binary thinking control. Zero to Agent maps enabled thinking levels
to `thinking: { type: "enabled" }` and `/think off` to
`thinking: { type: "disabled" }`. Zero to Agent removes `reasoning_effort`
from LongCat requests.

LongCat returns reasoning in `reasoning_content`. Zero to Agent preserves that field
...
model through an OpenAI-compatible runtime and use Zero to Agent's existing
