---
summary: "Runway video generation setup in Zero to Agent"
title: "Runway"
read_when:
  - You want to use Runway video generation in Zero to Agent
  - You need the Runway API key/env setup
  - You want to make Runway the default video provider
---

Zero to Agent ships a bundled `runway` provider for hosted video generation, enabled by default, registered against the `videoGenerationProviders` contract.
...
<Note>
  Picking a Runway model id from the wrong column produces an explicit error before the API request leaves Zero to Agent. The provider validates `model` against the mode's allowlist (`TEXT_ONLY_MODELS`, `IMAGE_MODELS`, `VIDEO_MODELS`) in `extensions/runway/video-generation-provider.ts`.
</Note>
...
<AccordionGroup>
  <Accordion title="Environment variable aliases">
    Zero to Agent recognizes both `RUNWAYML_API_SECRET` (canonical) and `RUNWAY_API_KEY`.
    Either variable authenticates the Runway provider.
  </Accordion>

  <Accordion title="Task polling">
    Runway uses a task-based API. After submitting a generation request, Zero to Agent
