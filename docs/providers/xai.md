---
summary: "Use xAI Grok models in OpenAgent"
read_when:
  - You want to use Grok models in OpenAgent
  - You are configuring xAI auth or model ids
title: "xAI"
---

OpenAgent ships a bundled `xai` provider plugin for Grok models. The
...
show Grok Build on the consent screen because OpenAgent uses xAI's shared
OAuth client.
...
<Note>
OpenAgent uses the xAI Responses API as the bundled xAI transport. The same
...
SuperGrok quota when the xAI provider is signed in with OAuth. OpenAgent fetches
...
<Tip>
Use `xai-oauth` when signing in from SSH, Docker, or a VPS. OpenAgent prints a
...
OpenAgent's flat catalog cost fields record the short-context rates. The current
[Grok Build](https://docs.x.ai/build/overview) coding agent uses Grok 4.6. The
historical OpenAgent `grok-build-latest` compatibility alias remains pinned to
Grok 4.5.
...
The bundled plugin maps supported xAI APIs onto OpenAgent's shared provider and
tool contracts. Capabilities that do not fit the shared contract are listed
below or under known limits.

| xAI capability             | OpenAgent surface                   | Status                                               |
| -------------------------- | --------------------------------------- | ---------------------------------------------------- |
...
| Files / batches            | Generic model API compatibility only    | Not a first-class OpenAgent tool                 |

<Note>
OpenAgent uses xAI's REST image/video/TTS/STT APIs for media generation and
...
The dated 0309 ids are the selectable catalog entries. OpenAgent sends all other
...
<Warning>
  ...
  `enabled` setting omitted, OpenAgent exposes it only for an active xAI model.
...
<AccordionGroup>
  ...
  <Accordion title="Video generation">
    ...
    OpenAgent encodes those as data URLs for xAI.
    </Warning>

    Video 1.5 also recognizes xAI's `grok-imagine-video-1.5-preview` and
    `grok-imagine-video-1.5-2026-05-30` identifiers. OpenAgent forwards the
  ...
  <Accordion title="Image generation">
    ...
    OpenAgent asks xAI for `b64_json` image responses so generated media can be
    ...
    OpenAgent forwards only the shared cross-provider image controls today;
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    OpenAgent uses xAI's batch `/v1/tts` endpoint for buffered synthesis,
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through OpenAgent's
    ...
    transcription request. Prompt hints are accepted by the shared OpenAgent
    ...
    Valid empty transcripts are skipped, and OpenAgent tries any configured
  ...
  <Accordion title="Realtime voice (Talk)">
    ...
    - Barge-in: xAI server VAD interrupts the response; OpenAgent clears queued playback
    ...
    <Note>
    `sessionResumption` defaults to `false`. When set to `true`, OpenAgent asks
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as an OpenAgent tool for
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as an OpenAgent tool for
  ...
  <Accordion title="Context compaction">
    ...
    compaction. The session keeps its OpenAgent transcript unchanged and stores
    ...
    Endpoint failures fall back to OpenAgent's client-side summarization.
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config
      ...
      even though OpenAgent does not require the Grok Build app.
    - OpenAgent does not currently expose the xAI multi-agent model family. xAI
      serves these models through the Responses API, but they do not accept
      the client-side or custom tools used by OpenAgent's shared agent loop.
      See the
  ...
  <Accordion title="Advanced notes">
    - OpenAgent applies xAI-specific tool-schema and tool-call compatibility
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as OpenAgent
      tools. OpenAgent attaches only the specific xAI built-in each tool needs
...
The shared image live file verifies the same xAI provider through OpenAgent's
