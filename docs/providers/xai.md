---
summary: "Use xAI Grok models in Zero to Agent"
read_when:
  - You want to use Grok models in Zero to Agent
  - You are configuring xAI auth or model ids
title: "xAI"
---

Zero to Agent ships a bundled `xai` provider plugin for Grok models. The
...
show Grok Build on the consent screen because Zero to Agent uses xAI's shared
OAuth client.
...
<Note>
Zero to Agent uses the xAI Responses API as the bundled xAI transport. The same
...
SuperGrok quota when the xAI provider is signed in with OAuth. Zero to Agent fetches
...
<Tip>
Use `xai-oauth` when signing in from SSH, Docker, or a VPS. Zero to Agent prints a
...
Zero to Agent's flat catalog cost fields record the short-context rates. The current
[Grok Build](https://docs.x.ai/build/overview) coding agent uses Grok 4.6. The
historical Zero to Agent `grok-build-latest` compatibility alias remains pinned to
Grok 4.5.
...
The bundled plugin maps supported xAI APIs onto Zero to Agent's shared provider and
tool contracts. Capabilities that do not fit the shared contract are listed
below or under known limits.

| xAI capability             | Zero to Agent surface                   | Status                                               |
| -------------------------- | --------------------------------------- | ---------------------------------------------------- |
...
| Files / batches            | Generic model API compatibility only    | Not a first-class Zero to Agent tool                 |

<Note>
Zero to Agent uses xAI's REST image/video/TTS/STT APIs for media generation and
...
The dated 0309 ids are the selectable catalog entries. Zero to Agent sends all other
...
<Warning>
  ...
  `enabled` setting omitted, Zero to Agent exposes it only for an active xAI model.
...
<AccordionGroup>
  ...
  <Accordion title="Video generation">
    ...
    Zero to Agent encodes those as data URLs for xAI.
    </Warning>

    Video 1.5 also recognizes xAI's `grok-imagine-video-1.5-preview` and
    `grok-imagine-video-1.5-2026-05-30` identifiers. Zero to Agent forwards the
  ...
  <Accordion title="Image generation">
    ...
    Zero to Agent asks xAI for `b64_json` image responses so generated media can be
    ...
    Zero to Agent forwards only the shared cross-provider image controls today;
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    Zero to Agent uses xAI's batch `/v1/tts` endpoint for buffered synthesis,
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through Zero to Agent's
    ...
    transcription request. Prompt hints are accepted by the shared Zero to Agent
    ...
    Valid empty transcripts are skipped, and Zero to Agent tries any configured
  ...
  <Accordion title="Realtime voice (Talk)">
    ...
    - Barge-in: xAI server VAD interrupts the response; Zero to Agent clears queued playback
    ...
    <Note>
    `sessionResumption` defaults to `false`. When set to `true`, Zero to Agent asks
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as a Zero to Agent tool for
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as a Zero to Agent tool for
  ...
  <Accordion title="Context compaction">
    ...
    compaction. The session keeps its Zero to Agent transcript unchanged and stores
    ...
    Endpoint failures fall back to Zero to Agent's client-side summarization.
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config
      ...
      even though Zero to Agent does not require the Grok Build app.
    - Zero to Agent does not currently expose the xAI multi-agent model family. xAI
      serves these models through the Responses API, but they do not accept
      the client-side or custom tools used by Zero to Agent's shared agent loop.
      See the
  ...
  <Accordion title="Advanced notes">
    - Zero to Agent applies xAI-specific tool-schema and tool-call compatibility
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as Zero to Agent
      tools. Zero to Agent attaches only the specific xAI built-in each tool needs
...
The shared image live file verifies the same xAI provider through Zero to Agent's
