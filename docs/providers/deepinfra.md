---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in OpenAgent"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in OpenAgent
title: "DeepInfra"
---
...
| Surface                  | Default model                                                                  | OpenAgent config/tool                             |
| ------------------------ | ------------------------------------------------------------------------------ | ----------------------------------------------------- |
...
native model types. OpenAgent has no provider contract for those categories
yet, so this plugin does not register them.

## Available models

OpenAgent discovers DeepInfra models dynamically once a key is configured. Use
...
discount expiry remain unknown; OpenAgent does not guess context tiers or parse
...
OpenAgent's registered catalog hook explicitly selects `discoveryMode: "strict"`
