---
summary: "Use Z.AI (GLM models) with Zero to Agent"
read_when:
  - You want Z.AI / GLM models in Zero to Agent
...
Zero to Agent uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In Zero to Agent, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Zero to Agent probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.
...
`https://api.z.ai/api/anthropic`. Zero to Agent's Z.AI choices use the documented
...
- [General-purpose agent tools](https://docs.z.ai/devpack/tool/others),
  including Zero to Agent, are served on a best-effort basis. During high inference
...
<Tip>
`zai-api-key` lets Zero to Agent detect the matching Z.AI endpoint from the key and
...
| `zai/glm-5-turbo`   | Zero to Agent-optimized text model; 200K context   |
...
<Tabs>
  <Tab title="GLM-5.3 and Flash">
    Levels: `low`, `high`, and `max` (default `max`). Zero to Agent maps these to
  ...
  <Tab title="GLM-5.2">
    Full range: `off`, `low`, `high`, `max` (default `off`). Zero to Agent maps
...
<AccordionGroup>
  ...
  <Accordion title="Preserved thinking">
    ...
    When enabled and thinking is on, Zero to Agent sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; Zero to Agent copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Zero to Agent config schema, including provider and model settings.
  </Card>
</CardGroup>
