---
summary: "Sign in to GitHub Copilot from OpenAgent using the device flow or non-interactive token import"
read_when:
...
models for your GitHub account and plan. OpenAgent can use Copilot as a model
provider or agent runtime in three different ways.

## Three ways to use Copilot in OpenAgent

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token. By default,
    OpenAgent puts the token in its protected local secret store and saves only a
    `tokenRef` in the auth profile. When OpenAgent runs, it validates Copilot access
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. OpenAgent talks to
    the proxy's `/v1` endpoint (default `http://localhost:3000/v1`) and uses the
    model list you configure.

    The `copilot-proxy` plugin ships with OpenAgent and is enabled by default.
...
endpoints rather than public `github.com`. OpenAgent exposes this as a
...
<Steps>
  <Step title="Pick the Enterprise auth choice">
    ...
    **GitHub Copilot (Enterprise / data residency)**. You will be prompted for
    ...
    OpenAgent derives those endpoints from the tenant root automatically.
...
tenant, or one tenant to another), OpenAgent will not reuse the existing token —
...
OpenAgent uses the `copilot-developer-cli` request identity by default, including
...
must choose a default, it also checks the live Copilot model catalog. OpenAgent
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    OpenAgent refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    Newly published Copilot models become visible without an OpenAgent upgrade,
  ...
  <Accordion title="Thinking levels">
    ...
    account, and OpenAgent preserves those efforts in Responses requests.
  ...
  <Accordion title="Request compatibility">
    OpenAgent sends Copilot-compatible request headers with a Copilot CLI request
  ...
  <Accordion title="Token storage">
    By default, device login stores the GitHub token in OpenAgent's protected local
    ...
    external secret provider. If OpenAgent cannot write the store, login stops
    ...
    The protected store is write-only through OpenAgent's user-facing secret APIs,
    but it is not encrypted at rest; its SQLite file relies on state-directory
    permissions. At runtime, OpenAgent resolves the reference, validates Copilot
...
have logged in, OpenAgent can use it for embeddings without a separate API key.

### Config

Set `memory.search.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, OpenAgent discovers available embedding models from
...
1. OpenAgent resolves your GitHub token (from env vars or auth profile).
...
embedding models are available, OpenAgent uses `memory.search.fallback` only
