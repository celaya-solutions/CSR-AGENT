---
summary: "Sign in to GitHub Copilot from Zero to Agent using the device flow or non-interactive token import"
read_when:
...
models for your GitHub account and plan. Zero to Agent can use Copilot as a model
provider or agent runtime in three different ways.

## Three ways to use Copilot in Zero to Agent

<Tabs>
  <Tab title="Built-in provider (github-copilot)">
    Use the native device-login flow to obtain a GitHub token. By default,
    Zero to Agent puts the token in its protected local secret store and saves only a
    `tokenRef` in the auth profile. When Zero to Agent runs, it validates Copilot access
  ...
  <Tab title="Copilot Proxy plugin (copilot-proxy)">
    Use the **Copilot Proxy** VS Code extension as a local bridge. Zero to Agent talks to
    the proxy's `/v1` endpoint (default `http://localhost:3000/v1`) and uses the
    model list you configure.

    The `copilot-proxy` plugin ships with Zero to Agent and is enabled by default.
...
endpoints rather than public `github.com`. Zero to Agent exposes this as a
...
<Steps>
  <Step title="Pick the Enterprise auth choice">
    ...
    **GitHub Copilot (Enterprise / data residency)**. You will be prompted for
    ...
    Zero to Agent derives those endpoints from the tenant root automatically.
...
tenant, or one tenant to another), Zero to Agent will not reuse the existing token —
...
Zero to Agent uses the `copilot-developer-cli` request identity by default, including
...
must choose a default, it also checks the live Copilot model catalog. Zero to Agent
...
<AccordionGroup>
  ...
  <Accordion title="Live catalog refresh from the Copilot API">
    ...
    Zero to Agent refreshes the model catalog on demand from `${baseUrl}/models`
    ...
    Newly published Copilot models become visible without a Zero to Agent upgrade,
  ...
  <Accordion title="Thinking levels">
    ...
    account, and Zero to Agent preserves those efforts in Responses requests.
  ...
  <Accordion title="Request compatibility">
    Zero to Agent sends Copilot-compatible request headers with a Copilot CLI request
  ...
  <Accordion title="Token storage">
    By default, device login stores the GitHub token in Zero to Agent's protected local
    ...
    external secret provider. If Zero to Agent cannot write the store, login stops
    ...
    The protected store is write-only through Zero to Agent's user-facing secret APIs,
    but it is not encrypted at rest; its SQLite file relies on state-directory
    permissions. At runtime, Zero to Agent resolves the reference, validates Copilot
...
have logged in, Zero to Agent can use it for embeddings without a separate API key.

### Config

Set `memory.search.provider` explicitly to use GitHub Copilot embeddings. If a
GitHub token is available, Zero to Agent discovers available embedding models from
...
1. Zero to Agent resolves your GitHub token (from env vars or auth profile).
...
embedding models are available, Zero to Agent uses `memory.search.fallback` only
