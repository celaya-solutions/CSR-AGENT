---
summary: "Use OpenAI via API keys or Codex subscription in OpenAgent"
read_when:
  - You want to use OpenAI models in OpenAgent
...
OpenAgent uses one provider id, `openai`, for both direct API-key auth and
...
facts decide whether OpenAgent may select the bundled Codex app-server runtime
...
- <a id="embedded-openclaw-translation" />[Embedded OpenAgent translation](/providers/openai/setup#embedded-openclaw-translation)
...
- <a id="openclaw-feature-coverage" />[OpenAgent feature coverage](/providers/openai/coverage-and-cost#openclaw-feature-coverage)
