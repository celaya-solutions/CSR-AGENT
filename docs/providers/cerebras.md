...
title: "Cerebras"
read_when:
  - You want to use Cerebras with OpenAgent
...
`https://api.cerebras.ai/v1`, OpenAgent reads
...
per-token strings; OpenAgent converts them to USD per million tokens. The public
feed does not provide cache tariffs. Zero cache fields in OpenAgent's runtime
...
empty catalog, or has no usable model rows, OpenAgent uses the bundled offline
...
marks `zai-glm-4.7` deprecated without naming a replacement. OpenAgent keeps the
