...
title: "Cerebras"
read_when:
  - You want to use Cerebras with Zero to Agent
...
`https://api.cerebras.ai/v1`, Zero to Agent reads
...
per-token strings; Zero to Agent converts them to USD per million tokens. The public
feed does not provide cache tariffs. Zero cache fields in Zero to Agent's runtime
...
empty catalog, or has no usable model rows, Zero to Agent uses the bundled offline
...
marks `zai-glm-4.7` deprecated without naming a replacement. Zero to Agent keeps the
