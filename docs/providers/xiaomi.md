---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with Zero to Agent"
read_when:
  - You want Xiaomi MiMo models in Zero to Agent
...
Zero to Agent's [`/think` directive](/tools/thinking) with levels `off`,
...
Zero to Agent sends `speakerVoice` for that model.
...
the desired voice description; Zero to Agent sends it as the `user` message, sends
...
Matrix, Telegram, and WhatsApp), Zero to Agent transcodes Xiaomi output to 48kHz
