---
summary: "Use Xiaomi MiMo pay-as-you-go and Token Plan models with OpenAgent"
read_when:
  - You want Xiaomi MiMo models in OpenAgent
...
OpenAgent's [`/think` directive](/tools/thinking) with levels `off`,
...
OpenAgent sends `speakerVoice` for that model.
...
the desired voice description; OpenAgent sends it as the `user` message, sends
...
Matrix, Telegram, and WhatsApp), OpenAgent transcodes Xiaomi output to 48kHz
