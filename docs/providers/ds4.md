---
summary: "Run Zero to Agent through ds4, a local DeepSeek V4 Flash OpenAI-compatible server"
read_when:
  - You want to run Zero to Agent against antirez/ds4
  - You want a local DeepSeek V4 Flash backend with tool calls
  - You need the Zero to Agent config for ds4-server
title: "ds4"
---

[ds4](https://github.com/antirez/ds4) serves DeepSeek V4 Flash from a local
Metal backend with an OpenAI-compatible `/v1` API. Zero to Agent connects to ds4
through the generic `openai-completions` provider family.

ds4 is not a bundled Zero to Agent provider plugin. Configure it under
...
<Warning>
Zero to Agent agent turns include tool schemas and workspace context. A tiny context
...
<Steps>
  ...
  <Step title="Add the Zero to Agent provider config">
...
with `--tokens` unless you intentionally want Zero to Agent to request less output
than the server default.

## On-demand startup

Zero to Agent can start ds4 only when a `ds4/...` model is selected. Add
...
If you run that large context, update both the server flags and Zero to Agent model
metadata:
...
Direct HTTP check, bypassing Zero to Agent:
...
Zero to Agent model routing (same as the Quickstart check):
...
<AccordionGroup>
  ...
  <Accordion title="500 prompt exceeds context">
    The configured `--ctx` is too small for the Zero to Agent turn. Raise
  ...
  <Accordion title="The first request is slow">
    ...
    `localService.readyTimeoutMs: 300000` when Zero to Agent starts the server on
    demand.
  </Accordion>
</AccordionGroup>
