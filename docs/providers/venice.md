---
summary: "Use Venice AI privacy-focused models in Zero to Agent"
read_when:
  - You want privacy-focused inference in Zero to Agent
...
Zero to Agent refreshes it from the Venice `/models` API and falls back to the seed
...
Zero to Agent catalog advertises only the canonical model IDs returned by `/models`.

## DeepSeek V4 replay behavior

If Venice exposes DeepSeek V4 models such as `deepseek-v4-pro` or
`deepseek-v4-flash`, Zero to Agent fills the required `reasoning_content` replay
...
Zero to Agent reads live prices from Venice's public
