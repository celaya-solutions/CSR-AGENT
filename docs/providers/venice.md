---
summary: "Use Venice AI privacy-focused models in OpenAgent"
read_when:
  - You want privacy-focused inference in OpenAgent
...
OpenAgent refreshes it from the Venice `/models` API and falls back to the seed
...
OpenAgent catalog advertises only the canonical model IDs returned by `/models`.

## DeepSeek V4 replay behavior

If Venice exposes DeepSeek V4 models such as `deepseek-v4-pro` or
`deepseek-v4-flash`, OpenAgent fills the required `reasoning_content` replay
...
OpenAgent reads live prices from Venice's public
