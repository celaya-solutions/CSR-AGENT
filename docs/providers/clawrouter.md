...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in OpenAgent
---

ClawRouter gives OpenAgent one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on OpenAgent usage surfaces.
...
OpenAgent host. The plugin ships bundled with OpenAgent (`enabledByDefault: true`);
you only need an issued ClawRouter key.

| Property      | Value                                    |
| ------------- | ---------------------------------------- |
| Provider      | `clawrouter`                             |
| Plugin        | bundled (included in OpenAgent)      |
...
# OpenAgent gateway startup readiness only; no model call is made.
...
`X-Request-ID`, so an OpenAgent model-call event can be joined to ClawRouter's
...
supported request routes. OpenAgent does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as an OpenAgent model when:
...
Adding a model to a supported ClawRouter provider needs no OpenAgent release:
...
A model's optional `displayName` is its picker label; without it, OpenAgent uses
...
ClawRouter owns upstream credentials; its catalog tells OpenAgent which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | OpenAgent transport |
| -------------------------------------------------------- | ----------------------- |
...
unsupported request format is intentionally not advertised as an OpenAgent
...
ClawRouter's `/v1/usage` response feeds the normal OpenAgent provider-usage
...
The same provider snapshot is available to `/status` in chat and OpenAgent's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter key; OpenAgent does not fall back to upstream provider keys.                                                |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    OpenAgent usage and status surfaces.
  </Card>
</CardGroup>
