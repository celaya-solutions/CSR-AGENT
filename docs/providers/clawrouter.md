...
read_when:
  ...
  - You need ClawRouter model discovery or quota reporting in Zero to Agent
---

ClawRouter gives Zero to Agent one policy-scoped key for multiple upstream model
...
the key's budget and aggregate usage on Zero to Agent usage surfaces.
...
Zero to Agent host. The plugin ships bundled with Zero to Agent (`enabledByDefault: true`);
you only need an issued ClawRouter key.

| Property      | Value                                    |
| ------------- | ---------------------------------------- |
| Provider      | `clawrouter`                             |
| Plugin        | bundled (included in Zero to Agent)      |
...
# Zero to Agent gateway startup readiness only; no model call is made.
...
`X-Request-ID`, so a Zero to Agent model-call event can be joined to ClawRouter's
...
supported request routes. Zero to Agent does not ship a second, fixed list of
ClawRouter models. A catalog model is advertised as a Zero to Agent model when:
...
Adding a model to a supported ClawRouter provider needs no Zero to Agent release:
...
A model's optional `displayName` is its picker label; without it, Zero to Agent uses
...
ClawRouter owns upstream credentials; its catalog tells Zero to Agent which
transport to use, so you never install every upstream company's auth plugin.

| Catalog capability / route                               | Zero to Agent transport |
| -------------------------------------------------------- | ----------------------- |
...
unsupported request format is intentionally not advertised as a Zero to Agent
...
ClawRouter's `/v1/usage` response feeds the normal Zero to Agent provider-usage
...
The same provider snapshot is available to `/status` in chat and Zero to Agent's
...
| `401` or `403` from catalog or usage     | Reissue or re-scope the ClawRouter key; Zero to Agent does not fall back to upstream provider keys.                                                |
...
<CardGroup cols={2}>
  ...
  <Card title="Usage tracking" href="/concepts/usage-tracking" icon="chart-line">
    Zero to Agent usage and status surfaces.
  </Card>
</CardGroup>
