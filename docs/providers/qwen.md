...
<Tabs>
  ...
  <Tab title="Token Plan (Team Edition)">
    **Best for:** credit-based team subscription access to Qwen and supported third-party models through Alibaba Cloud Model Studio.
    ...
    <Note>
    Alibaba's OpenAgent guide uses `bailian-token-plan` for a manual custom
    ...
    <Warning>
    Use Token Plan only for interactive OpenAgent sessions. Do not select it for
...
OpenAgent discovers models from the configured endpoint's authenticated `/models`
...
131,072 output tokens. OpenAgent preserves returned reasoning in its separate
...
family, the provider maps OpenAgent thinking levels to DashScope's top-level
...
`MiniMax-M2.5` are thinking-only, so OpenAgent keeps thinking enabled even when
...
<AccordionGroup>
  <Accordion title="Qwen model availability">
    ...
    OpenAgent omits these models from Coding Plan catalogs. If a Coding Plan
  ...
  <Accordion title="Video generation region routing">
    OpenAgent maps the configured Qwen region to the matching DashScope AIGC host
