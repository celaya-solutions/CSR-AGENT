...
<Tabs>
  ...
  <Tab title="Token Plan (Team Edition)">
    **Best for:** credit-based team subscription access to Qwen and supported third-party models through Alibaba Cloud Model Studio.
    ...
    <Note>
    Alibaba's Zero to Agent guide uses `bailian-token-plan` for a manual custom
    ...
    <Warning>
    Use Token Plan only for interactive Zero to Agent sessions. Do not select it for
...
Zero to Agent discovers models from the configured endpoint's authenticated `/models`
...
131,072 output tokens. Zero to Agent preserves returned reasoning in its separate
...
family, the provider maps Zero to Agent thinking levels to DashScope's top-level
...
`MiniMax-M2.5` are thinking-only, so Zero to Agent keeps thinking enabled even when
...
<AccordionGroup>
  <Accordion title="Qwen model availability">
    ...
    Zero to Agent omits these models from Coding Plan catalogs. If a Coding Plan
  ...
  <Accordion title="Video generation region routing">
    Zero to Agent maps the configured Qwen region to the matching DashScope AIGC host
