---
summary: "Use Anthropic Claude via API keys or Claude CLI in Zero to Agent"
read_when:
  - You want to use Anthropic models in Zero to Agent
  - You want to browse Claude CLI or Claude Desktop sessions across paired computers
title: "Anthropic"
---

Anthropic builds the **Claude** model family. Zero to Agent supports two auth routes:
...
Zero to Agent detects the available Anthropic credential and selects the matching usage surface:

- Zero to Agent-managed subscription/setup credentials show quota windows and optional extra-usage budget.
- Native Claude CLI logins stay under Claude's exclusive refresh control, so Zero to Agent does not poll their quota endpoint.
...
Admin API cost history comes from Anthropic's [Usage and Cost API](https://platform.claude.com/docs/en/manage-claude/usage-cost-api). It is actual provider billing, separate from Zero to Agent's session-derived estimated cost.

<Warning>
Claude Code owns its existing login and subscription; Zero to Agent does not persist
...
<Tabs>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      <Step title="Ensure Claude CLI is installed and logged in">
        Zero to Agent communicates directly with the installed Claude Code executable.
        ...
        Zero to Agent so the gateway launches the new binary:
      ...
      <Step title="Run onboarding">
        ...
        through Zero to Agent's direct CLI transport. Zero to Agent uses a non-secret route
        ...
        or remove its saved profile. Native-tool approvals remain under Zero to Agent
        control. Schema-valid native calls pass through Zero to Agent's canonical
    ...
    <Warning>
    Claude CLI reuse expects the Zero to Agent process to run on the same host as the
    ...
    a provider with Zero to Agent-managed OAuth such as
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    Zero to Agent release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. Zero to Agent also supports subscription-style options from
...
128,000-token output limit. Fable 5.1 always uses adaptive thinking. Zero to Agent defaults to
...
`fable-5` and `anthropic/claude-fable-5` selections still use Fable 5; Zero to Agent
...
Zero to Agent's Anthropic adapter converts a forced tool choice to `auto`
...
blocks. Claude Code manages this history for the CLI runtime. Zero to Agent's
...
invalidated replayed thinking server-side, and Zero to Agent logs a warning with the
...
On other eligible routes, a client-side prune is a one-time prefix edit. Zero to Agent
...
Anthropic drops those blocks unbilled, and Zero to Agent's embedded runtime omits
...
direct API-key routes Anthropic drops those blocks server-side and Zero to Agent
...
No additional Zero to Agent config is required for discovery. The Anthropic plugin
...
30 seconds while visible, so Claude sessions created outside Zero to Agent appear
...
`sessions.catalog.continue`. Zero to Agent re-resolves the local catalog record,
...
The new Zero to Agent session starts with the catalog title and color. Continuing an
already adopted session preserves any title or color changes made in Zero to Agent.
...
and its local `claude` executable resolves. Zero to Agent re-resolves the catalog
...
advertises `agent.cli.claude.run.v1`. Zero to Agent never modifies Claude Desktop
...
With an Anthropic API key configured, Zero to Agent refreshes the Claude catalog from
Anthropic's models endpoint, so newly published snapshots of supported model
families appear without a Zero to Agent release. Models the shipped catalog already
...
capabilities match the request shaping Zero to Agent would apply to it. A brand-new
model generation therefore stays hidden until Zero to Agent adds support for it,
...
`sonnet` tracks the current Sonnet. Upgrading Zero to Agent can therefore move a
...
higher native effort levels. Zero to Agent omits manual thinking budgets, custom
...
thinking. Zero to Agent defaults both versions to `medium` effort. Anthropic does not allow thinking to be
disabled for these models, so stored `off` and `minimal` settings map to `low`
effort instead. Zero to Agent also omits caller-selected sampling parameters for
both Fable versions.
...
stored `adaptive` selections resolve to Zero to Agent's `medium` default. Custom
...
adaptive-thinking and five-effort contract. Zero to Agent defaults to `high`, maps
...
Claude Opus 4.8 keeps thinking off by default in Zero to Agent. When you explicitly
enable adaptive thinking with `/think high|xhigh|max`, Zero to Agent sends
...
another Claude model. Zero to Agent opts into Anthropic's recommended per-category
...
1. For every direct API-key request to `anthropic/claude-opus-5`,
   `anthropic/claude-fable-5-1`, or `anthropic/claude-fable-5`, Zero to Agent sends the
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   Zero to Agent's normal [model failover](/concepts/model-failover).
...
4. If the recommended model declines as well, the turn surfaces the refusal
   as an error. Zero to Agent does not retry a final refusal or advance to another
   configured model.

The fallback happens at the Anthropic API level, so the serving model does not
need to be in your configured Zero to Agent fallback chain.
...
- Anthropic bills the fallback attempt at the serving model's rates. Zero to Agent
...
- A mid-stream decline additionally bills the already-streamed primary-model partial
  ...
  usage but not folded into Zero to Agent's per-turn estimate.
...
Zero to Agent supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  ...
  <Accordion title="Bedrock Claude notes">
    ...
    - Supported Nova models offer opt-in explicit caching: set `cacheRetention` explicitly to `short` or `long` for system/message checkpoints with a five-minute TTL. Unset retention adds no checkpoints. Nova explicit caching has not been live-verified against AWS by Zero to Agent maintainers yet. Other non-Claude models remain at `cacheRetention: "none"`; see [Bedrock prompt caching](/reference/prompt-caching#amazon-bedrock) for model IDs, AWS limits, and the live acceptance proof gap.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    For Claude Opus 5 and Opus 4.8, Zero to Agent's shared `/fast` toggle uses
    ...
    <Note>
    - Native fast mode is a research preview for Claude Opus 5 and Opus 4.8. It can deliver up to 2.5x higher output-token throughput and is billed at `$10/$50` per million input/output tokens. Zero to Agent applies the same 2x multiplier to cache pricing in its cost estimate.
    ...
    - Claude Sonnet 5 supports neither native fast mode nor Priority Tier, so Zero to Agent omits both fields.
  ...
  <Accordion title="Server-side compaction">
    ...
    Zero to Agent adds the `compact-2026-01-12` beta header and sends an Anthropic
    `context_management` compaction edit. When compaction occurs, Zero to Agent
    ...
    When `anthropicCompactThreshold` is omitted, Zero to Agent uses
    ...
    <Warning>
    Anthropic server-side compaction is a beta feature and Zero to Agent never
    ...
    proxies, Bedrock, Vertex, and Foundry are excluded. Zero to Agent does not send
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Zero to Agent
    ...
    When an image or PDF is attached to a conversation, Zero to Agent automatically
  ...
  <Accordion title="1M context window">
    ...
    thinking: Opus 4.8,
    Opus 4.7, Opus 4.6, and Sonnet 4.6. Zero to Agent sizes these models
    ...
    these models and Zero to Agent no longer sends the retired
    ...
    extended context. Zero to Agent uses CLI-owned metadata and configured limits;
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but Zero to Agent strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  <Accordion title="Claude CLI OAuth session expired or could not be refreshed">
    ...
    Claude Code owns its login and refresh lifecycle; do not copy an OAuth token into Zero to Agent.
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    ...
    Native Claude CLI logins remain owned by Claude Code, not the shared Zero to Agent auth store. For that route, use the [Claude CLI setup](/providers/anthropic#getting-started); do not copy native OAuth tokens into Zero to Agent.
