---
summary: "Use Anthropic Claude via API keys or Claude CLI in OpenAgent"
read_when:
  - You want to use Anthropic models in OpenAgent
  - You want to browse Claude CLI or Claude Desktop sessions across paired computers
title: "Anthropic"
---

Anthropic builds the **Claude** model family. OpenAgent supports two auth routes:
...
OpenAgent detects the available Anthropic credential and selects the matching usage surface:

- OpenAgent-managed subscription/setup credentials show quota windows and optional extra-usage budget.
- Native Claude CLI logins stay under Claude's exclusive refresh control, so OpenAgent does not poll their quota endpoint.
...
Admin API cost history comes from Anthropic's [Usage and Cost API](https://platform.claude.com/docs/en/manage-claude/usage-cost-api). It is actual provider billing, separate from OpenAgent's session-derived estimated cost.

<Warning>
Claude Code owns its existing login and subscription; OpenAgent does not persist
...
<Tabs>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      <Step title="Ensure Claude CLI is installed and logged in">
        OpenAgent communicates directly with the installed Claude Code executable.
        ...
        OpenAgent so the gateway launches the new binary:
      ...
      <Step title="Run onboarding">
        ...
        through OpenAgent's direct CLI transport. OpenAgent uses a non-secret route
        ...
        control. Schema-valid native calls pass through OpenAgent's canonical
    ...
    <Warning>
    Claude CLI reuse expects the OpenAgent process to run on the same host as the
    ...
    a provider with OpenAgent-managed OAuth such as
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    OpenAgent release. Check `claude auth status`, `/status`, and
...
128,000-token output limit. Fable 5.1 always uses adaptive thinking. OpenAgent defaults to
...
`fable-5` and `anthropic/claude-fable-5` selections still use Fable 5; OpenAgent
...
OpenAgent's Anthropic adapter converts a forced tool choice to `auto`
...
blocks. Claude Code manages this history for the CLI runtime. OpenAgent's
...
invalidated replayed thinking server-side, and OpenAgent logs a warning with the
...
On other eligible routes, a client-side prune is a one-time prefix edit. OpenAgent
...
Anthropic drops those blocks unbilled, and OpenAgent's embedded runtime omits
...
direct API-key routes Anthropic drops those blocks server-side and OpenAgent
...
No additional OpenAgent config is required for discovery. The Anthropic plugin
...
30 seconds while visible, so Claude sessions created outside OpenAgent appear
...
`sessions.catalog.continue`. OpenAgent re-resolves the local catalog record,
...
The new OpenAgent session starts with the catalog title and color. Continuing an
already adopted session preserves any title or color changes made in OpenAgent.
...
and its local `claude` executable resolves. OpenAgent re-resolves the catalog
...
advertises `agent.cli.claude.run.v1`. OpenAgent never modifies Claude Desktop
...
With an Anthropic API key configured, OpenAgent refreshes the Claude catalog from
Anthropic's models endpoint, so newly published snapshots of supported model
families appear without an OpenAgent release. Models the shipped catalog already
...
capabilities match the request shaping OpenAgent would apply to it. A brand-new
model generation therefore stays hidden until OpenAgent adds support for it,
...
`sonnet` tracks the current Sonnet. Upgrading OpenAgent can therefore move a
...
higher native effort levels. OpenAgent omits manual thinking budgets, custom
...
thinking. OpenAgent defaults both versions to `medium` effort. Anthropic does not allow thinking to be
disabled for these models, so stored `off` and `minimal` settings map to `low`
effort instead. OpenAgent also omits caller-selected sampling parameters for
both Fable versions.
...
stored `adaptive` selections resolve to OpenAgent's `medium` default. Custom
...
adaptive-thinking and five-effort contract. OpenAgent defaults to `high`, maps
...
Claude Opus 4.8 keeps thinking off by default in OpenAgent. When you explicitly
enable adaptive thinking with `/think high|xhigh|max`, OpenAgent sends
...
another Claude model. OpenAgent opts into Anthropic's recommended per-category
...
1. For every direct API-key request to `anthropic/claude-opus-5`,
   `anthropic/claude-fable-5-1`, or `anthropic/claude-fable-5`, OpenAgent sends the
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   OpenAgent's normal [model failover](/concepts/model-failover).
...
4. If the recommended model declines as well, the turn surfaces the refusal
   as an error. OpenAgent does not retry a final refusal or advance to another
   configured model.

The fallback happens at the Anthropic API level, so the serving model does not
need to be in your configured OpenAgent fallback chain.
...
- Anthropic bills the fallback attempt at the serving model's rates. OpenAgent
...
- A mid-stream decline additionally bills the already-streamed primary-model partial
  ...
  usage but not folded into OpenAgent's per-turn estimate.
...
OpenAgent supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    For Claude Opus 5 and Opus 4.8, OpenAgent's shared `/fast` toggle uses
    ...
    <Note>
    - Native fast mode is a research preview for Claude Opus 5 and Opus 4.8. It can deliver up to 2.5x higher output-token throughput and is billed at `$10/$50` per million input/output tokens. OpenAgent applies the same 2x multiplier to cache pricing in its cost estimate.
    ...
    - Claude Sonnet 5 supports neither native fast mode nor Priority Tier, so OpenAgent omits both fields.
  ...
  <Accordion title="Server-side compaction">
    ...
    OpenAgent adds the `compact-2026-01-12` beta header and sends an Anthropic
    `context_management` compaction edit. When compaction occurs, OpenAgent
    ...
    When `anthropicCompactThreshold` is omitted, OpenAgent uses
    ...
    <Warning>
    Anthropic server-side compaction is a beta feature and OpenAgent never
    ...
    proxies, Bedrock, Vertex, and Foundry are excluded. OpenAgent does not send
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. OpenAgent
    ...
    When an image or PDF is attached to a conversation, OpenAgent automatically
  ...
  <Accordion title="1M context window">
    ...
    thinking: Opus 4.8,
    Opus 4.7, Opus 4.6, and Sonnet 4.6. OpenAgent sizes these models
    ...
    these models and OpenAgent no longer sends the retired
    ...
    extended context. OpenAgent uses CLI-owned metadata and configured limits;
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but OpenAgent strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    ...
    Native Claude CLI logins remain owned by Claude Code, not the shared OpenAgent auth store. For that route, use the [Claude CLI setup](/providers/anthropic#getting-started); do not copy native OAuth tokens into OpenAgent.
