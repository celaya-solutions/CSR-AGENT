...
title: "Chutes"
read_when:
  - You want to use Chutes with OpenAgent
...
OpenAI-compatible API. OpenAgent supports both browser OAuth and API-key auth.
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run the OAuth onboarding flow">
        ...
        ```
        OpenAgent launches the browser flow locally, or shows a URL + redirect-paste
        flow on remote/headless hosts. OAuth tokens auto-refresh through OpenAgent auth
        profiles.
      </Step>
    </Steps>
  </Tab>
...
When Chutes auth is available, OpenAgent queries `GET /v1/models` with that
...
OpenAgent does not retry anonymously. Other request failures produce an
...
OpenAgent registers two convenience aliases for the Chutes catalog:
