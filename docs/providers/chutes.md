...
title: "Chutes"
read_when:
  - You want to use Chutes with Zero to Agent
...
OpenAI-compatible API. Zero to Agent supports both browser OAuth and API-key auth.
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run the OAuth onboarding flow">
        ...
        ```
        Zero to Agent launches the browser flow locally, or shows a URL + redirect-paste
        flow on remote/headless hosts. OAuth tokens auto-refresh through Zero to Agent auth
        profiles.
      </Step>
    </Steps>
  </Tab>
...
When Chutes auth is available, Zero to Agent queries `GET /v1/models` with that
...
Zero to Agent does not retry anonymously. Other request failures produce an
...
Zero to Agent registers two convenience aliases for the Chutes catalog:
