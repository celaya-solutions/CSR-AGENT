---
summary: "Run OpenAgent with Ollama (cloud and local models)"
read_when:
  - You want to run OpenAgent with cloud or local models via Ollama
  - You need Ollama setup and configuration guidance
  - You want Ollama vision models for image understanding
title: "Ollama"
---

OpenAgent talks to Ollama's native API (`/api/chat`), not the OpenAI-compatible
...
- <a id="remote-host-works-with-curl-but-not-openclaw" />[Remote host works with curl but not OpenAgent](/providers/ollama/troubleshooting#remote-host-works-with-curl-but-not-openclaw)
