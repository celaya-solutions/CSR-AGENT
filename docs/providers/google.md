...
title: "Google (Gemini)"
read_when:
  - You want to use Google Gemini models with Zero to Agent
...
<Tabs>
  <Tab title="AI Studio API key">
    **Recommended for:** standard Gemini API access.
    ...
    With a configured API key, Zero to Agent refreshes Google AI Studio's text-model
    catalog from the Gemini `models.list` API. Newly released Gemini 3 Pro, Flash,
    and Flash-Lite variants therefore appear in
    `openclaw models list --provider google` without waiting for a Zero to Agent
    ...
    **Advanced use only:** run a canonical `google/*` model through an installed
    ...
    <Steps>
      <Step title="Configure Google AI Studio">
        Complete the API-key setup in the first tab. Zero to Agent must have a usable
      ...
      <Step title="Install Gemini CLI">
        ...
        Zero to Agent supports both Homebrew installs and global npm installs, including
    ...
    but Zero to Agent cannot create or repair them. If one breaks, replace it with a
...
<Tip>
Gemini 3 models use `thinkingLevel` rather than `thinkingBudget`. Zero to Agent maps
...
a fixed Zero to Agent level. Gemini 3 and Gemini 3.1 omit a fixed `thinkingLevel` so
Google can choose the level; Gemini 2.5 sends Google's dynamic sentinel
`thinkingBudget: -1`.

Gemma 4 models (for example `gemma-4-26b-a4b-it`) support thinking mode. Zero to Agent
...
`thinkingBudget: 0`; Zero to Agent strips that value for Gemini 2.5 Pro requests
...
Zero to Agent adapts telephony/Meet bridge audio to Gemini's PCM Live API stream and
keeps tool calls on the shared realtime voice contract. Leave `temperature`
unset unless you need sampling changes; Zero to Agent omits non-positive values
...
sequential function calling. Zero to Agent omits the older `NON_BLOCKING`, function
...
<AccordionGroup>
  <Accordion title="Direct Gemini cache reuse">
    For direct Gemini API runs (`api: "google-generative-ai"`), Zero to Agent
    ...
    - Gemini cache-hit usage is normalized into Zero to Agent `cacheRead` from
  ...
  <Accordion title="Gemini CLI usage notes">
    ...
    - `stats.cached` is normalized into Zero to Agent `cacheRead`.
    - If `stats.input` is missing, Zero to Agent derives input tokens from
