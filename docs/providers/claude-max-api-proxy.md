...
**claude-max-api-proxy** is a community npm package (not a Zero to Agent plugin) that
...
plan articles, and [Anthropic provider](/providers/anthropic) for Zero to Agent's
...
<Steps>
  ...
  <Step title="Configure Zero to Agent">
    Point Zero to Agent at the proxy as a custom OpenAI-compatible endpoint:
...
<Note>
The model ids below are the proxy's own catalog, not Zero to Agent's Anthropic
...
<AccordionGroup>
  <Accordion title="Proxy-style OpenAI-compatible notes">
    This uses Zero to Agent's generic custom `/v1` OpenAI-compatible route, the same
    ...
    - No Responses `store`, prompt-cache hints, or OpenAI reasoning-compat
      payload shaping.
    - Zero to Agent's OpenAI/Codex attribution headers (`originator`, `version`,
...
<CardGroup cols={2}>
  <Card title="Anthropic provider" href="/providers/anthropic" icon="bolt">
    Native Zero to Agent integration with Claude CLI or API keys.
  </Card>
