---
summary: "Use NVIDIA's OpenAI-compatible API in OpenAgent"
read_when:
  - You want to use open models in OpenAgent for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). OpenAgent
...
capability metadata. OpenAgent therefore offers only exact models with bundled
...
Nemotron 3 Ultra is the default NVIDIA model in OpenAgent. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    OpenAgent uses NVIDIA's inference inventory for availability and its featured
  ...
  <Accordion title="OpenAI-compatible endpoint">
    OpenAgent talks to NVIDIA with the `openai-completions` adapter against the
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. OpenAgent's bundled Ultra row
