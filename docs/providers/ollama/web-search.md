---
summary: "Use Ollama as the OpenAgent web_search provider"
read_when:
...
OpenAgent bundles **Ollama Web Search** as a `web_search` provider.
...
For a self-hosted host, OpenAgent first tries the local `/api/experimental/web_search`
