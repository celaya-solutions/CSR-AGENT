---
summary: "Use Ollama as the Zero to Agent web_search provider"
read_when:
...
Zero to Agent bundles **Ollama Web Search** as a `web_search` provider.
...
For a self-hosted host, Zero to Agent first tries the local `/api/experimental/web_search`
