...
<AccordionGroup>
  ...
  <Accordion title="LAN Ollama host with manual models">
    ...
    `contextTokens` caps OpenAgent's active-input budget; `params.num_ctx` sets
  ...
  <Accordion title="Multiple Ollama hosts">
    ...
    OpenAgent strips the active provider prefix (falling back to a bare
  ...
  <Accordion title="Small local model profile">
    ...
    Large file reads use OpenAgent's context-based paging. The native adapter
...
prefix, such as `ollama-spark/qwen3:32b`, OpenAgent strips that prefix before
...
works but OpenAgent does not, check whether the Gateway runs on a different
