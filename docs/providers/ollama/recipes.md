...
<AccordionGroup>
  ...
  <Accordion title="LAN Ollama host with manual models">
    ...
    `contextTokens` caps Zero to Agent's active-input budget; `params.num_ctx` sets
  ...
  <Accordion title="Multiple Ollama hosts">
    ...
    Zero to Agent strips the active provider prefix (falling back to a bare
  ...
  <Accordion title="Small local model profile">
    ...
    Large file reads use Zero to Agent's context-based paging. The native adapter
...
prefix, such as `ollama-spark/qwen3:32b`, Zero to Agent strips that prefix before
...
# Zero to Agent catalog and selected model
openclaw models list --provider ollama
...
works but Zero to Agent does not, check whether the Gateway runs on a different
