---
summary: "How Zero to Agent discovers Ollama models implicitly, plus narrow smoke tests"
read_when:
  - You want to know which models Zero to Agent discovers and how
...
defined, Zero to Agent discovers models from `http://127.0.0.1:11434`:
...
| Token limits         | `maxTokens` defaults to Zero to Agent's Ollama max-token cap                                                                                                                                                                                                                                  |
...
hand-written `models.json` entry; Zero to Agent resolves it live. For signed-in
...
provider and `/api/tags` is unreachable, Zero to Agent records that run as
