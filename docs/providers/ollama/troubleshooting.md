---
summary: "Fixes for common Ollama failures in Zero to Agent"
read_when:
...
<AccordionGroup>
  <Accordion title="WSL2 crash loop (repeated reboots)">
    ...
    Zero to Agent logs a startup warning when it detects WSL2, `ollama.service`
  ...
  <Accordion title="Remote host works with curl but not Zero to Agent">
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    ...
    `params.num_ctx` overrides it. Cap both Zero to Agent's budget and Ollama's request
    ...
    Lower the model entry's `contextTokens` if Zero to Agent sends too much prompt. Lower
