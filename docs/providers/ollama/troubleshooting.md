---
summary: "Fixes for common Ollama failures in OpenAgent"
read_when:
...
<AccordionGroup>
  <Accordion title="WSL2 crash loop (repeated reboots)">
    ...
    OpenAgent logs a startup warning when it detects WSL2, `ollama.service`
  ...
  <Accordion title="Remote host works with curl but not OpenAgent">
  ...
  <Accordion title="Large-context model is too slow or runs out of memory">
    ...
    `params.num_ctx` overrides it. Cap both OpenAgent's budget and Ollama's request
    ...
    Lower the model entry's `contextTokens` if OpenAgent sends too much prompt. Lower
