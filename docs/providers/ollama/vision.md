...
media-understanding provider, so OpenAgent can route explicit image-description
...
that already support native vision. If the call fails, OpenAgent can continue
through `agents.defaults.imageModel.fallbacks`; file/URL preparation errors
fail before fallback is attempted. Use `infer image describe` for OpenAgent's
...
OpenAgent rejects image-description requests for models not marked
