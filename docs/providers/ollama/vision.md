...
media-understanding provider, so Zero to Agent can route explicit image-description
...
that already support native vision. If the call fails, Zero to Agent can continue
through `agents.defaults.imageModel.fallbacks`; file/URL preparation errors
fail before fallback is attempted. Use `infer image describe` for Zero to Agent's
...
Zero to Agent rejects image-description requests for models not marked
