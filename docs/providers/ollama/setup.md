---
summary: "Connect Zero to Agent to Ollama: auth rules, onboarding, and cloud models through a local host"
read_when:
  - You are connecting Zero to Agent to Ollama for the first time
...
<AccordionGroup>
  <Accordion title="Local and LAN hosts">
    Loopback, private-network, `.local`, and bare-hostname Ollama URLs do not need a real bearer token. Zero to Agent uses the `ollama-local` marker for these.
  </Accordion>
  ...
  <Tab title="Onboarding (recommended)">
    <Steps>
      <Step title="Run onboarding">
        ...
        On a fresh guided setup, Zero to Agent first checks the default or configured
        ...
        a real completion before Zero to Agent saves it; discovery never pulls or
        loads an idle model.
...
Zero to Agent prompts for the base URL, discovers local models, and checks
...
models at setup time, Zero to Agent falls back to its hardcoded suggested list so
