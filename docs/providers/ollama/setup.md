---
summary: "Connect OpenAgent to Ollama: auth rules, onboarding, and cloud models through a local host"
read_when:
  - You are connecting OpenAgent to Ollama for the first time
...
<AccordionGroup>
  <Accordion title="Local and LAN hosts">
    Loopback, private-network, `.local`, and bare-hostname Ollama URLs do not need a real bearer token. OpenAgent uses the `ollama-local` marker for these.
  </Accordion>
  ...
  <Tab title="Onboarding (recommended)">
    <Steps>
      <Step title="Run onboarding">
        ...
        On a fresh guided setup, OpenAgent first checks the default or configured
        ...
        a real completion before OpenAgent saves it; discovery never pulls or
        loads an idle model.
...
OpenAgent prompts for the base URL, discovers local models, and checks
