...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in this mode.** Use it only when a proxy needs OpenAI format and you do not depend on native tool calling.
    ...
    OpenAgent injects `options.num_ctx` by default in this mode so Ollama does
  ...
  <Accordion title="Context windows">
    For auto-discovered models, OpenAgent uses the context window `/api/show`
    reports, including larger `PARAMETER num_ctx` values from custom
    Modelfiles; otherwise it falls back to OpenAgent's default Ollama context
    window.
    ...
    native window), so OpenAgent can override a smaller Modelfile context even
    ...
    With `temperature: 0`, OpenAgent still normalizes `top_p` to `1` for greedy
    ...
    OpenAgent then attempts compaction and retries, or reports the failure.
  ...
  <Accordion title="Thinking control">
    ...
    OpenAgent forwards thinking as Ollama expects it: top-level `think`, not
    ...
    thinking for a specific model. OpenAgent preserves that explicit config
  ...
  <Accordion title="Memory embeddings">
    ...
    loopback origin derived from the configured `baseUrl` use OpenAgent's
