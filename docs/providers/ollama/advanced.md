...
<AccordionGroup>
  <Accordion title="Legacy OpenAI-compatible mode">
    ...
    **Tool calling is not reliable in this mode.** Use it only when a proxy needs OpenAI format and you do not depend on native tool calling.
    ...
    Zero to Agent injects `options.num_ctx` by default in this mode so Ollama does
  ...
  <Accordion title="Context windows">
    For auto-discovered models, Zero to Agent uses the context window `/api/show`
    reports, including larger `PARAMETER num_ctx` values from custom
    Modelfiles; otherwise it falls back to Zero to Agent's default Ollama context
    window.
    ...
    native window), so Zero to Agent can override a smaller Modelfile context even
    ...
    With `temperature: 0`, Zero to Agent still normalizes `top_p` to `1` for greedy
    ...
    Zero to Agent then attempts compaction and retries, or reports the failure.
  ...
  <Accordion title="Thinking control">
    ...
    Zero to Agent forwards thinking as Ollama expects it: top-level `think`, not
    ...
    thinking for a specific model. Zero to Agent preserves that explicit config
  ...
  <Accordion title="Memory embeddings">
    ...
    loopback origin derived from the configured `baseUrl` use Zero to Agent's
