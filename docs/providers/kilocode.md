---
summary: "Use Kilo Gateway's unified API to access many models in Zero to Agent"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Zero to Agent
---
...
Zero to Agent does not publish a task-to-upstream-model mapping for it; routing behind
`kilo-auto/balanced` is owned by Kilo Gateway.

At startup Zero to Agent queries `GET https://api.kilo.ai/api/gateway/models` and combines a nonempty public list
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path: Zero to Agent sanitizes Gemini thought
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails, Zero to Agent reports an unavailable catalog refresh. It does not replace the failed request with static rows or turn an empty response into `kilocode/kilo-auto/balanced`.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Zero to Agent configuration reference.
  </Card>
