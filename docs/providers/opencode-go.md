...
OpenCode Go is bundled in the OpenAgent package, so onboarding
...
OpenCode Go is already included with OpenAgent. Continue with
...
OpenAgent combines Go's advertised model IDs with authoritative metadata from
`https://models.opencode.ai/api.json`, so new upstream models appear without an
OpenAgent update when they use a supported transport on the trusted OpenCode
...
these examples as an inventory. OpenAgent excludes deprecated rows from active
...
model, because provider policy can change independently of OpenAgent.

## Advanced configuration

<AccordionGroup>
  <Accordion title="Routing behavior">
    OpenAgent routes any `opencode-go/...` model ref automatically. No extra
