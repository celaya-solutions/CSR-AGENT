...
OpenCode Go is bundled in the Zero to Agent package, so onboarding
...
OpenCode Go is already included with Zero to Agent. Continue with
...
Zero to Agent combines Go's advertised model IDs with authoritative metadata from
`https://models.opencode.ai/api.json`, so new upstream models appear without an
Zero to Agent update when they use a supported transport on the trusted OpenCode
...
these examples as an inventory. Zero to Agent excludes deprecated rows from active
...
model, because provider policy can change independently of Zero to Agent.

## Advanced configuration

<AccordionGroup>
  <Accordion title="Routing behavior">
    Zero to Agent routes any `opencode-go/...` model ref automatically. No extra
