---
summary: "Which OpenAI capabilities OpenAgent exposes, embeddings, and spend reporting"
read_when:
  - You want to know which OpenAI capability maps to which OpenAgent surface
...
OpenAgent keeps subscription quota and Platform API billing distinct:
...
- OpenAgent never sends `OPENAI_API_KEY` or an `openai` inference profile to organization APIs; those credentials may belong to custom, Azure, or agent-local endpoints.

An explicit Admin key takes precedence over OAuth. Provider-reported history is not merged with OpenAgent's session-derived estimated cost; it can include API activity from other clients and provider-side billing adjustments.
...
the realtime data channel. OpenAgent does not attach a continuous camera track
to the OpenAI session.
</Note>

## Memory embeddings

OpenAgent can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memory.search`. OpenAgent
