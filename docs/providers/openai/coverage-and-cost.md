---
summary: "Which OpenAI capabilities Zero to Agent exposes, embeddings, and spend reporting"
read_when:
  - You want to know which OpenAI capability maps to which Zero to Agent surface
...
Zero to Agent keeps subscription quota and Platform API billing distinct:
...
- Zero to Agent never sends `OPENAI_API_KEY` or an `openai` inference profile to organization APIs; those credentials may belong to custom, Azure, or agent-local endpoints.

An explicit Admin key takes precedence over OAuth. Provider-reported history is not merged with Zero to Agent's session-derived estimated cost; it can include API activity from other clients and provider-side billing adjustments.
...
## Zero to Agent feature coverage

| OpenAI capability         | Zero to Agent surface                                                                         | Status                                                             |
| ------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
...
<Note>
Released GPT-Live browser and Gateway-relay WebRTC try a Zero to Agent ChatGPT OAuth
...
the realtime data channel. Zero to Agent does not attach a continuous camera track
to the OpenAI session.
</Note>

## Memory embeddings

Zero to Agent can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memory.search`. Zero to Agent
