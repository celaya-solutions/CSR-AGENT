...
OpenAgent retains its ordinary 272,000-token active input budget by default.
The supported reasoning efforts are `low`, `medium`, `high`, `xhigh`, and `max`.
OpenAgent defaults Astra to `medium` on both the OpenAgent and Codex runtimes
...
`/think ultra` is also available on the OpenAgent and Codex runtimes. Ultra enables
proactive sub-agent orchestration; it is not a raw Responses API effort. OpenAgent
...
Use an OpenAI Platform API-key profile and the built-in OpenAgent runtime for
...
- **Async function calls:** Astra can continue reasoning while OpenAgent runs a
  direct function tool. OpenAgent sends the completed result in the next model
...
- **Reasoning changes without rebuilding the cached prefix:** Change the
  ...
  the next user turn. OpenAgent preserves the original request-level effort
...
OpenAgent recognizes the exact `openai/gpt-5.6-sol`,
