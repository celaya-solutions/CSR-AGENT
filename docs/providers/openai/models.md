...
| Account without GPT-5.6 access                         | `openai/gpt-5.5`                                                   | Explicit recovery choice; Zero to Agent does not silently downgrade. |
| Direct API-key billing, explicit Zero to Agent runtime | `openai/gpt-5.6` plus provider/model `agentRuntime.id: "openclaw"` | Select a normal `openai` API-key profile.                            |
...
Zero to Agent retains its ordinary 272,000-token active input budget by default.
The supported reasoning efforts are `low`, `medium`, `high`, `xhigh`, and `max`.
Zero to Agent defaults Astra to `medium` on both the Zero to Agent and Codex runtimes
...
`/think ultra` is also available on the Zero to Agent and Codex runtimes. Ultra enables
proactive sub-agent orchestration; it is not a raw Responses API effort. Zero to Agent
...
Use an OpenAI Platform API-key profile and the built-in Zero to Agent runtime for
...
- **Async function calls:** Astra can continue reasoning while Zero to Agent runs a
  direct function tool. Zero to Agent sends the completed result in the next model
...
- **Reasoning changes without rebuilding the cached prefix:** Change the
  ...
  the next user turn. Zero to Agent preserves the original request-level effort
...
Zero to Agent recognizes the exact `openai/gpt-5.6-sol`,
...
Zero to Agent surfaces the upstream access error and does not silently replace a
...
custom endpoints, and request-transport overrides remain on Zero to Agent. Plaintext
