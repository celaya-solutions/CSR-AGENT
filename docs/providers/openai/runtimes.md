...
read_when:
  - You need to know whether a turn runs on Zero to Agent or the native Codex harness
...
| Authored `openai-completions` adapter                                                                                                                                           | Zero to Agent         |
| Custom endpoint                                                                                                                                                                 | Zero to Agent         |
| Explicit exact official endpoint using HTTP                                                                                                                                     | Rejected              |
| Route with an authored provider/model request override                                                                                                                          | Zero to Agent         |
...
Zero to Agent. Explicit `agentRuntime.id: "codex"` requires a registered Codex harness;
unsupported routes/auth fail closed, except that authored request overrides may
use Codex's declared exact-request Zero to Agent fallback before execution. Inspect
...
and explicit auth order still take precedence. An authored Zero to Agent runtime choice
...
account-based. Zero to Agent selects auth in this order:
...
2. The native Codex account, when no host credential or account selection owns
   ...
   user login. Prepared Zero to Agent credentials stay in the agent home; Zero to Agent
...
credentials into a Zero to Agent profile. A fresh auth refresh observes native login
...
remains available when you want a Zero to Agent-owned profile.
...
Zero to Agent auth store before starting a native Codex turn. Replace `<agent-id>`
...
subscription-style Codex profile is selected, Zero to Agent also keeps
...
When that subscription profile is blocked by a Codex usage limit, Zero to Agent
...
prepared account. A native login stays distinct from a Zero to Agent profile; it does
