...
read_when:
  - You need to know whether a turn runs on OpenAgent or the native Codex harness
...
OpenAgent. Explicit `agentRuntime.id: "codex"` requires a registered Codex harness;
unsupported routes/auth fail closed, except that authored request overrides may
use Codex's declared exact-request OpenAgent fallback before execution. Inspect
...
and explicit auth order still take precedence. An authored OpenAgent runtime choice
...
2. The native Codex account, when no host credential or account selection owns
   ...
   user login. Prepared OpenAgent credentials stay in the agent home; OpenAgent
...
credentials into an OpenAgent profile. A fresh auth refresh observes native login
...
remains available when you want an OpenAgent-owned profile.
...
OpenAgent auth store before starting a native Codex turn. Replace `<agent-id>`
...
subscription-style Codex profile is selected, OpenAgent also keeps
...
When that subscription profile is blocked by a Codex usage limit, OpenAgent
...
prepared account. A native login stays distinct from an OpenAgent profile; it does
