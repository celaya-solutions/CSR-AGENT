...
read_when:
  - You are connecting OpenAI to Zero to Agent for the first time
...
<Tabs>
  <Tab title="API key (OpenAI Platform)">
    **Best for:** direct API access and usage-based billing.
    ...
    | `openai/gpt-5.6` | provider/model `agentRuntime.id: "openclaw"`                  | Zero to Agent embedded runtime | Selected `openai` API-key profile |
    | `openai/gpt-5.5` | explicit provider/model `agentRuntime.id`                     | Selected agent runtime    | Selected OpenAI API-key profile   |
    | `openai/*`       | authored Completions, custom, or request override | Zero to Agent embedded runtime | Credential type remains unchanged |
    ...
    `chat-latest` alias only accepts `medium` text verbosity; Zero to Agent forces
    any other requested verbosity to `medium` for this model.

    <Warning>
    Zero to Agent does **not** expose `gpt-5.3-codex-spark` on the direct OpenAI
    ...
    **Best for:** using your ChatGPT/Codex subscription with native Codex
    ...
    <Steps>
      ...
      <Step title="Use the canonical OpenAI model route">
        ...
        Zero to Agent installs or repairs the bundled Codex plugin when that runtime
        is chosen.
      </Step>
    ...
    | `openai/gpt-6-astra`     | provider/model `agentRuntime.id: "openclaw"`                  | Zero to Agent embedded runtime, internal Codex-auth transport | Selected `openai` OAuth profile                    |
    | `openai/gpt-5.5`         | explicit provider/model `agentRuntime.id`                     | Selected agent runtime                                   | Selected OpenAI auth profile                       |
    | `openai/*`               | authored Completions, custom, or request override | Zero to Agent embedded runtime                                | Credential requirement remains route-specific      |
    | `openai/*`               | plaintext official HTTP endpoint                  | Rejected                                                 | Credential is not sent                              |
    ...
    Codex GPT refs are legacy Zero to Agent routes, not the native Codex runtime
    ...
    the auth order under `openai`. Zero to Agent tries the subscription first, then
    ...
    browser OAuth (default) or the device-code flow above; Zero to Agent manages the
    ...
    the Codex runtime unless Zero to Agent is explicitly configured.

    ### Context window defaults and long-context opt-in

    Zero to Agent treats native model capacity and the active runtime budget as
    separate values:

    - `contextWindow` declares the model's native window.
    - `contextTokens` caps how much of that window Zero to Agent uses for active input.
    ...
    Zero to Agent sends Responses compaction controls, while native Codex owns its
    ...
    #### Embedded Zero to Agent translation

    This example pins the exact Sol model to the embedded Zero to Agent runtime,
    ...
    request and may drop every earlier input item. Zero to Agent persists that item
    ...
    A process-owned isolated-Gateway run on Zero to Agent 2026.8.1 verified this exact
    ...
    Keep the same Zero to Agent model selection, but make Codex the explicit runtime
    ...
    settings must change together. Zero to Agent can retain both choices only when
    ...
    Zero to Agent uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `gpt-5.5` row while the account
    is authenticated, Zero to Agent synthesizes that OAuth model row so cron,
