...
still accepted. Zero to Agent also protects the public OpenAI and OpenAI Codex OAuth
...
an `openai` OAuth profile is configured, Zero to Agent resolves that stored OAuth
...
also set `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; Zero to Agent
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for Zero to Agent on macOS" size=3840x2160 count=1
```
...
`supportsResolution`. Zero to Agent's shared normalization layer converts a
