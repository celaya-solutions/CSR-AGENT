...
still accepted. OpenAgent also protects the public OpenAI and OpenAI Codex OAuth
...
an `openai` OAuth profile is configured, OpenAgent resolves that stored OAuth
...
also set `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; OpenAgent
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for OpenAgent on macOS" size=3840x2160 count=1
```
...
`supportsResolution`. OpenAgent's shared normalization layer converts a
