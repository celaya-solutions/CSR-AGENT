...
<AccordionGroup>
  <Accordion title="Speech synthesis (TTS)">
    ...
    `extraBody` is merged into `/audio/speech` request JSON after OpenAgent's
  ...
  <Accordion title="Speech-to-text">
    ...
    OpenAgent's media-understanding transcription surface.
  ...
  <Accordion title="Realtime voice">
    ...
    falls back to the OpenAgent ChatGPT OAuth subscription profile. The
    ...
    configured OpenAgent agent. It can listen while speaking; interrupting speech
    ...
    subscription route. Its browser and Gateway-relay WebRTC try the OpenAgent ChatGPT
    ...
    Current Platform-key sessions accept `marin` and `cedar`. OpenAgent defaults
    ...
    and delegation mode at startup. OpenAgent exposes the
