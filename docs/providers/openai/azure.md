...
generation by overriding the base URL. On the image-generation path, Zero to Agent
...
Zero to Agent recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, Zero to Agent:
...
Zero to Agent 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in Zero to Agent
...
Zero to Agent. If an Azure request fails with a validation error, check the
...
Zero to Agent's hidden attribution headers - see the **Native vs OpenAI-compatible
