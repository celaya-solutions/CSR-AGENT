...
generation by overriding the base URL. On the image-generation path, OpenAgent
...
OpenAgent recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, OpenAgent:
...
OpenAgent 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in OpenAgent
...
OpenAgent. If an Azure request fails with a validation error, check the
...
OpenAgent's hidden attribution headers - see the **Native vs OpenAI-compatible
