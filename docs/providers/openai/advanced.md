...
OpenAgent adds a shared GPT-5 prompt contribution to matching GPT-5-family
OpenAgent-assembled prompts. The OpenAI plugin setting below controls the
...
project-doc behavior, and OpenAgent disables Codex's built-in personality for
native threads so agent workspace personality files stay authoritative.
OpenAgent contributes only runtime context to native Codex threads: channel
delivery, OpenAgent dynamic tools, ACP delegation, workspace context, and
OpenAgent skills. The heartbeat-guidance text from this same contribution is the
...
checks, and verification on matching OpenAgent-assembled prompts. Channel-
specific reply and silent-message behavior stays in the shared OpenAgent system
...
request settings, so an otherwise eligible `auto` route stays on OpenAgent
...
Authored embedded-provider settings can therefore select the declared OpenAgent
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    ...
    request and response still match the current history, OpenAgent sends only
    ...
    `websocket_connection_limit_reached` are safe exceptions: OpenAgent closes
  ...
  <Accordion title="Fast mode">
    OpenAgent exposes a shared fast-mode toggle for `openai/*`:
    ...
    params and do not select OpenAgent or Codex. The example below pins embedded
    OpenAgent because it describes a direct provider request.

    When enabled on the embedded runtime, OpenAgent maps fast mode to OpenAI API
    ...
    clears only the session layer. `/status` reports the resolved OpenAgent
  ...
  <Accordion title="OpenAI API Fast mode with service_tier">
    ...
    processing. OpenAgent sends the wire value
    `service_tier = "priority"`. Set an explicit tier per
    model on the embedded OpenAgent runtime:
    ...
    a proxy, OpenAgent leaves `service_tier` untouched. Configure the native
  ...
  <Accordion title="Server-side compaction (Responses API)">
    ...
    `api.openai.com`), the OpenAI plugin's OpenAgent stream wrapper auto-enables
    ...
    so OpenAgent does not delay local compaction unless the transport will inject
    ...
    these server-compaction controls. This applies to the built-in OpenAgent
    ...
    forward and drop the earlier input prefix it replaces. OpenAgent does this
  ...
  <Accordion title="Strict-agentic GPT mode">
    For `openai` provider GPT-5-family models run through OpenAgent's embedded
    runtime, OpenAgent already defaults to a stricter execution contract called
    ...
    With `strict-agentic` active, OpenAgent:
    ...
    OpenAgent does not classify assistant prose to decide whether a turn is a
    plan, progress update, or final answer.

    <Note>
    This contract lives entirely in OpenAgent's embedded agent runner. It does
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    OpenAgent treats direct OpenAI, Codex, and Azure OpenAI endpoints
