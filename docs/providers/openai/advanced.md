...
Zero to Agent adds a shared GPT-5 prompt contribution to matching GPT-5-family
Zero to Agent-assembled prompts. The OpenAI plugin setting below controls the
...
project-doc behavior, and Zero to Agent disables Codex's built-in personality for
native threads so agent workspace personality files stay authoritative.
Zero to Agent contributes only runtime context to native Codex threads: channel
delivery, Zero to Agent dynamic tools, ACP delegation, workspace context, and
Zero to Agent skills. The heartbeat-guidance text from this same contribution is the
...
checks, and verification on matching Zero to Agent-assembled prompts. Channel-
specific reply and silent-message behavior stays in the shared Zero to Agent system
...
request settings, so an otherwise eligible `auto` route stays on Zero to Agent
...
Authored embedded-provider settings can therefore select the declared Zero to Agent
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    ...
    request and response still match the current history, Zero to Agent sends only
    ...
    `websocket_connection_limit_reached` are safe exceptions: Zero to Agent closes
  ...
  <Accordion title="Fast mode">
    Zero to Agent exposes a shared fast-mode toggle for `openai/*`:
    ...
    params and do not select Zero to Agent or Codex. The example below pins embedded
    Zero to Agent because it describes a direct provider request.

    When enabled on the embedded runtime, Zero to Agent maps fast mode to OpenAI API
    ...
    clears only the session layer. `/status` reports the resolved Zero to Agent
  ...
  <Accordion title="OpenAI API Fast mode with service_tier">
    ...
    processing. Zero to Agent sends the wire value
    `service_tier = "priority"`. Set an explicit tier per
    model on the embedded Zero to Agent runtime:
    ...
    a proxy, Zero to Agent leaves `service_tier` untouched. Configure the native
  ...
  <Accordion title="Server-side compaction (Responses API)">
    ...
    `api.openai.com`), the OpenAI plugin's Zero to Agent stream wrapper auto-enables
    ...
    so Zero to Agent does not delay local compaction unless the transport will inject
    ...
    these server-compaction controls. This applies to the built-in Zero to Agent
    ...
    forward and drop the earlier input prefix it replaces. Zero to Agent does this
  ...
  <Accordion title="Strict-agentic GPT mode">
    For `openai` provider GPT-5-family models run through Zero to Agent's embedded
    runtime, Zero to Agent already defaults to a stricter execution contract called
    ...
    With `strict-agentic` active, Zero to Agent:
    ...
    Zero to Agent does not classify assistant prose to decide whether a turn is a
    plan, progress update, or final answer.

    <Note>
    This contract lives entirely in Zero to Agent's embedded agent runner. It does
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    Zero to Agent treats direct OpenAI, Codex, and Azure OpenAI endpoints
