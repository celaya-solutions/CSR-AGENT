---
summary: "Run Zero to Agent with LM Studio"
read_when:
  - You want to run Zero to Agent with open source models via LM Studio
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    On a fresh guided setup, Zero to Agent first queries `/api/v1/models` on the
...
LM Studio model keys use an `author/model-name` format (e.g. `qwen/qwen3.5-9b`); Zero to Agent model refs
...
servers; omit it for unauthenticated servers and Zero to Agent stores a local non-secret marker instead.
...
LM Studio doesn't always emit an OpenAI-shaped `usage` object on streamed responses. Zero to Agent
...
When LM Studio's `/api/v1/models` discovery reports model-specific reasoning options, Zero to Agent
...
"on"]`) while rejecting those literal values on `/v1/chat/completions`; Zero to Agent normalizes that
...
With preload enabled, Zero to Agent routes chat requests to a loaded instance with
...
LM Studio supports just-in-time (JIT) model loading, loading models on first request. Zero to Agent
...
disable Zero to Agent's preload step:
...
When a configured server cannot list models, Zero to Agent reports an unavailable catalog or a
