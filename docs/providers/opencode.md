---
summary: "Use OpenCode Zen and Go catalogs with Zero to Agent"
read_when:
...
OpenCode exposes two hosted catalogs in Zero to Agent:
...
having a Zen key does not by itself grant Go access. Zero to Agent keeps the runtime
provider ids split so upstream per-model routing stays correct.

Zero to Agent sends a stable `x-opencode-session` conversation header on requests to
    ...
    **Best for:** the separately subscribed Go lineup across DeepSeek, GLM, GPT,
    ...
    <Steps>
      <Step title="Use the bundled Go catalog">
        OpenCode Go is included with Zero to Agent, so no separate
...
Model availability and promotional routes can change independently of Zero to Agent.
...
Zero to Agent fetches and caches that catalog only when OpenCode Zen or Go is
...
without a Zero to Agent update when their metadata describes a supported transport
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so Zero to Agent keeps
