---
summary: "Use OpenCode Zen and Go catalogs with OpenAgent"
read_when:
...
OpenCode exposes two hosted catalogs in OpenAgent:
...
having a Zen key does not by itself grant Go access. OpenAgent keeps the runtime
provider ids split so upstream per-model routing stays correct.

OpenAgent sends a stable `x-opencode-session` conversation header on requests to
    ...
    **Best for:** the separately subscribed Go lineup across DeepSeek, GLM, GPT,
    ...
    <Steps>
      <Step title="Use the bundled Go catalog">
        OpenCode Go is included with OpenAgent, so no separate
...
Model availability and promotional routes can change independently of OpenAgent.
...
OpenAgent fetches and caches that catalog only when OpenCode Zen or Go is
...
without an OpenAgent update when their metadata describes a supported transport
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so OpenAgent keeps
