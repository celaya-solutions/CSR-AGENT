---
summary: "Use GMI Cloud's OpenAI-compatible API with OpenAgent"
read_when:
  - You want to run OpenAgent with GMI Cloud models
  - You need the GMI provider id, key, or endpoint
title: "GMI Cloud"
---

GMI Cloud is a hosted inference platform for frontier and open-weight models
behind an OpenAI-compatible API. In OpenAgent it is an official external provider
...
primary provider does. OpenAgent owns the provider id, auth profile, aliases,
...
- `401` or `403`: check that `GMI_API_KEY` is set for the process running
  OpenAgent, or re-run onboarding to store the key in the provider auth profile.
