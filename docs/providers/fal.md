---
summary: "fal image, video, and music generation setup in OpenAgent"
title: "Fal"
read_when:
  - You want to use fal image generation in OpenAgent
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate, video_generate, or music_generate
---

OpenAgent ships a bundled `fal` provider for hosted image, video, and music
generation.
...
OpenAgent converts aspect-ratio hints to valid dimensions.
For example, `aspectRatio: "3:2"` produces `1536x1024`.
Use `size` to choose exact dimensions. OpenAgent rejects invalid explicit sizes.
...
Krea 2 models use fal's native Krea payload schema. OpenAgent sends
...
`aspectRatio`; OpenAgent maps `size` to the closest supported Krea aspect ratio
...
transparent-background control in OpenAgent. They report `background` as an
ignored override.
Krea 2 endpoints do not expose an `output_format` request field through fal, so
OpenAgent rejects `outputFormat` overrides for Krea requests.
