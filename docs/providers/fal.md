---
summary: "fal image, video, and music generation setup in Zero to Agent"
title: "Fal"
read_when:
  - You want to use fal image generation in Zero to Agent
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate, video_generate, or music_generate
---

Zero to Agent ships a bundled `fal` provider for hosted image, video, and music
generation.
...
Zero to Agent converts aspect-ratio hints to valid dimensions.
For example, `aspectRatio: "3:2"` produces `1536x1024`.
Use `size` to choose exact dimensions. Zero to Agent rejects invalid explicit sizes.
...
Krea 2 models use fal's native Krea payload schema. Zero to Agent sends
...
`aspectRatio`; Zero to Agent maps `size` to the closest supported Krea aspect ratio
...
transparent-background control in Zero to Agent. They report `background` as an
ignored override.
Krea 2 endpoints do not expose an `output_format` request field through fal, so
Zero to Agent rejects `outputFormat` overrides for Krea requests.
