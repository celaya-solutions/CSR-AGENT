...
older Zero to Agent version may request a one-time command-surface upgrade after the
...
Install skills under the node machine's active Zero to Agent skills directory,
...
its active Zero to Agent state directory, so relative paths resolve on the node rather
