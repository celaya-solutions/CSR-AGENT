...
Install skills under the node machine's active OpenAgent skills directory,
...
its active OpenAgent state directory, so relative paths resolve on the node rather
