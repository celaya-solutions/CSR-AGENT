...
read_when:
  - You want Zero to Agent to identify the active Mac
...
the most recent physical mouse or keyboard input. Zero to Agent uses that signal to
...
- The Zero to Agent macOS app is paired and connected in node mode.
- **Settings -> Permissions -> Active computer detection** is enabled. It is off by default.
- **Accessibility** permission is granted to the signed Zero to Agent app.
...
used for UI automation. Zero to Agent sends idle duration, not input content. It does not send key values,
...
Zero to Agent waits 750 milliseconds so the connecting Mac can submit its first
...
- If no active Mac is available or primary delivery fails, Zero to Agent waits five
