...
read_when:
  - You want OpenAgent to identify the active Mac
...
the most recent physical mouse or keyboard input. OpenAgent uses that signal to
...
- The OpenAgent macOS app is paired and connected in node mode.
- **Settings -> Permissions -> Active computer detection** is enabled. It is off by default.
- **Accessibility** permission is granted to the signed OpenAgent app.
...
used for UI automation. OpenAgent sends idle duration, not input content. It does not send key values,
...
OpenAgent waits 750 milliseconds so the connecting Mac can submit its first
...
- If no active Mac is available or primary delivery fails, OpenAgent waits five
