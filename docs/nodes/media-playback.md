...
read_when:
  ...
  - Comparing media format support across Zero to Agent clients
  - Debugging playback metadata, transcoding, or codec availability
title: "Media playback"
---

Zero to Agent chat clients play assistant audio and video attachments inline. The
...
This page covers playback in Zero to Agent clients. Channel delivery, inbound media
...
table above is the cross-client contract Zero to Agent targets.
...
Zero to Agent also uses `ffprobe`, when available, to fill audio duration and video
...
rendition. If either step cannot safely handle the source, Zero to Agent serves the
