...
This is a takeover of the native session; unlike OpenAgent adoption, it does not
...
OpenAgent imports bounded visible history, resumes with `--fork-session` on the
...
row on that node becomes continuable: OpenAgent imports bounded history, binds
...
The receiver shows read-only rows under the source node in **OpenAgent sessions**.
