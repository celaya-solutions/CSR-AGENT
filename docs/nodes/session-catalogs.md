...
This is a takeover of the native session; unlike Zero to Agent adoption, it does not
...
Zero to Agent imports bounded visible history, resumes with `--fork-session` on the
...
row on that node becomes continuable: Zero to Agent imports bounded history, binds
...
## Zero to Agent sessions and transcripts

The bundled [Session Share plugin](/plugins/session-share) publishes selected
native Zero to Agent sessions from a source Gateway to a paired receiver Gateway.
...
The receiver shows read-only rows under the source node in **Zero to Agent sessions**.
