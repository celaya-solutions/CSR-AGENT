...
When `tools.media.<capability>.enabled` is not `false` and no models are configured, OpenAgent tries these in order and stops at the first working option:
...
- If a PDF falls back to rendered page images, OpenAgent forwards those images to vision-capable reply models and keeps the placeholder `[PDF content rendered to images]` in the file block.
