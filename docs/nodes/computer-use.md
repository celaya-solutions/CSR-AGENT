...
- **macOS fulfiller:** **Accessibility** and **Screen Recording** granted to OpenAgent. The native Peekaboo path also requires Event Posting access for its CoreGraphics input primitives.
...
Window input coordinates follow the observation's `details.coordinateSpace`. CUA reports `image-pixels`: use pixels in the delivered image, including when OpenAgent resized it. Peekaboo reports `global-logical-points`: use desktop logical points. Accessibility element bounds retain their native screen coordinates; prefer `elementRef` when targeting those elements. Browser coordinate inputs use viewport CSS pixels.
...
The managed endpoint is not part of the model contract. The CUA plugin registers no model tool, CLI command, service, or raw node-MCP descriptor, and its action schema accepts neither helper binaries, sockets, native sessions, driver arguments, nor provider tool names. On macOS only the app-owned worker receives the endpoint, while node shell execution is routed through the app host without that worker-only value. These boundaries prevent an OpenAgent model action from selecting an alternate route to the managed daemon.

CUA creates the Unix socket with mode `0600`, and OpenAgent places it in a random owner-only `0700` directory. This excludes remote clients and other local users. It does not authenticate or sandbox processes running as the same logged-in user: those processes are inside this boundary and may be able to discover and use same-user resources. Unrestricted CUA mode does not contain a compromised user account. Stronger same-user isolation would require inherited connected IPC or an OS-enforced process boundary.
...
scripts/dev/computer-use-macos-live-rig.sh prepare \
  cu-live-proof 29431 "$PWD/dist/OpenAgent.app" "$scratch" peekaboo
```
...
```bash
scripts/dev/computer-use-macos-live-rig.sh proof \
  "$scratch" cua "OpenAgent CUA X11 Target" "W3-LINUX CONFIRMED"
```
...
2. Verify the node-local SDK package before starting the node:
   ...
   OpenAgent checks the SDK package version, the selected OS/CPU package version, regular-file identity, and the pinned SHA-256 digest of the native library and Node runtime. A clean check prints `no findings`. If it reports a `COMPUTER_DRIVER_*` error, reinstall or update OpenAgent on this node host and run the check again. Do not download a standalone `cua-driver` executable or add one to `PATH`; Windows and Linux use the npm-installed in-process SDK.
...
| Code                                                 | Cause                                                                                                                                                         | Fix                                                                                                                                                                                                           |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `COMPUTER_DRIVER_UNAVAILABLE`                        | The CUA runtime cannot initialize, the macOS app-owned endpoint is absent, or the desktop permissions/session are unavailable.                                | On macOS, verify CUA is selected and the bundled driver is ready; on Windows/Linux, run `openclaw node run` inside the interactive desktop session. Reinstall OpenAgent if the pinned runtime is missing. |
| `COMPUTER_DRIVER_PACKAGE_MISSING`                    | The pinned SDK package, OS/CPU native package, native library, or Node runtime is absent or unreadable.                                                       | Reinstall OpenAgent on the node host, rerun `openclaw doctor --lint --only cua-computer/driver-artifacts`, then restart the node.                                                                         |
| `COMPUTER_DRIVER_VERSION_MISMATCH`                   | The SDK package or selected native package does not match the pinned version.                                                                                 | Update or reinstall OpenAgent so both packages come from the same release; rerun the focused doctor check.                                                                                                |
| `COMPUTER_DRIVER_DIGEST_MISMATCH`                    | A native SDK library or Node runtime is not a regular package file or does not match its pinned SHA-256 digest.                                               | Do not run or replace the file manually. Reinstall OpenAgent, rerun the focused doctor check, then restart the node.                                                                                      |
...
OpenAgent reports native session state independently of optional activity sharing;
...
viewer, use a supported local or remote login path. OpenAgent's availability
...
If the status says **Accessibility grant may be stale**, OpenAgent may already appear enabled under **System Settings → Privacy & Security → Accessibility** even though macOS rejects it. This happens when the Accessibility entry is pinned to an older app build. Select OpenAgent in that list, remove it with **−**, then re-add `/Applications/OpenAgent.app`. Quit and reopen OpenAgent after changing the grant because macOS can cache Accessibility trust for the lifetime of the process.
