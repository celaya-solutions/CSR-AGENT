...
- OpenAgent English docs are authored in `openclaw/openclaw` under `docs/`.
- ClawHub English docs are authored in `openclaw/clawhub` under `docs/` and mirrored into the publish repo's `docs/clawhub/` tree. Do not keep competing ClawHub pages in `openclaw/openclaw`; OpenAgent-specific integration guidance stays in the owning OpenAgent docs.
