...
read_when:
  - Hosting PeekabooBridge in Zero to Agent.app
...
Zero to Agent can host **PeekabooBridge** as a local, permission-aware UI automation broker (`PeekabooBridgeHostCoordinator`, backed by the `openclaw/Peekaboo` Swift package). This lets the `peekaboo` CLI drive UI automation while reusing the macOS app's TCC permissions.

## What this is (and is not)

- **Host**: Zero to Agent.app can act as a PeekabooBridge host.
- **Client**: the `peekaboo` CLI, installed from [peekaboo.sh](https://peekaboo.sh/) (there is no separate `openclaw ui ...` surface).
- **UI**: visual overlays stay in Peekaboo.app. Zero to Agent is a thin broker host.

## Relationship to other desktop-control paths

Zero to Agent has four desktop-control paths that intentionally stay separate:

- **PeekabooBridge host**: Zero to Agent.app hosts the local PeekabooBridge socket. The `peekaboo` CLI is the client and uses Zero to Agent.app's macOS permissions for screenshots, clicks, menus, dialogs, Dock actions, and window management.
...
- **Codex Computer Use**: the bundled `codex` plugin checks and can install Codex's `computer-use` MCP plugin (`extensions/codex/src/app-server/computer-use.ts`). Codex then owns native desktop-control tool calls during Codex-mode turns. Zero to Agent does not proxy those actions through PeekabooBridge.
- **Direct `cua-driver` MCP**: Zero to Agent can register TryCua's upstream `cua-driver mcp` server as a normal MCP server. This gives agents the CUA driver's own schemas and pid/window/element-index workflow. It does not route through the Codex marketplace or the PeekabooBridge socket.

Use Peekaboo for the broad macOS automation surface via Zero to Agent.app's permission-aware bridge host. Use agent-driven computer use when the gateway agent should see and control the desktop. It does this through a uniform `computer.act` node command that any vision model can drive. Use Codex Computer Use when a Codex-mode agent should rely on Codex's native plugin. Use direct `cua-driver mcp` to expose the CUA driver to any Zero to Agent-managed runtime as a normal MCP server.
...
export PREFIX="Zero to Agent-<full-openclaw-sha>-Peekaboo-<full-peekaboo-sha>-stable"
...
not need a Zero to Agent source checkout. The authorized operator handoff must independently provide the installer and
...
architecture. Each worker must contain a Mach-O Node runtime, the Zero to Agent package entrypoint, and build metadata matching
...
The elevation archive is Foundation-signed, notarized, stapled, named by the full Zero to Agent and Peekaboo source
commits, and contains exactly `Zero to Agent.app`. Its receipt binds the archive and portable-installer names and digests,
Zero to Agent and Peekaboo source revisions, signer, per-architecture CDHashes, architectures, entitlement digests, and Apple notarization
...
3. Zero to Agent.app (thin broker)
...
- The bridge checks **caller code signatures**. The production Zero to Agent host accepts only the exact Peekaboo CLI
...
- If no hosts are found, open one of the host apps (Peekaboo.app or Zero to Agent.app). Then check that permissions are granted.
