---
summary: "Setup guide for developers working on the OpenAgent macOS app"
read_when:
...
Build and run the OpenAgent macOS application from source.
...
Outputs `dist/OpenAgent.app`. Packaging requires a real signing identity by
...
version from the app: choose **About OpenAgent** in the menu bar, or run
...
OpenAgent lifecycle scripts for that install. Node remains the recommended
