---
summary: "macOS app flow for controlling a remote OpenAgent Gateway"
read_when:
...
4. Return to OpenAgent. The saved Gateway's dashboard opens; check the account
...
Gateway's HTTPS address. OpenAgent shows **Add Gateway** with that address filled
...
- **Remote over SSH (default)**: OpenAgent commands run on the remote host. The app opens an SSH connection with `-o BatchMode`, your chosen identity/key, and a local port-forward.
...
1. Install the packaged OpenAgent CLI with `npm install -g openclaw@latest --allow-scripts=openclaw` (omit `--allow-scripts=openclaw` on npm 11.15 and earlier).
...
`/Applications/OpenAgent.app/Contents/MacOS/openclaw-mac`.
...
falling back to `/Applications/OpenAgent.app`. Use `--no-launch` to require an
