...
read_when:
  - Packaging Zero to Agent.app
  - Debugging the macOS gateway launchd service
  - Installing the gateway CLI for macOS
title: "Gateway on macOS"
---

Zero to Agent.app bundles a private Node runtime and matching Zero to Agent package for
...
runtime and matching Zero to Agent package. The bundled installer owns that setup;
...
Read the version to install from the app: choose **About Zero to Agent** in the
...
- "Zero to Agent Active" enables/disables the LaunchAgent.
...
the list appears informationally under "Other Zero to Agent launchd jobs (macOS)".
...
direct arguments invoke an absolute Zero to Agent path with a Gateway lifecycle
...
Keep Zero to Agent state on a local, non-synced disk. Avoid iCloud Drive and other
...
`/Applications/Zero to Agent.app/Contents/MacOS/openclaw-mac` directly. See
