...
read_when:
  - Packaging OpenAgent.app
  - Debugging the macOS gateway launchd service
  - Installing the gateway CLI for macOS
title: "Gateway on macOS"
---

OpenAgent.app bundles a private Node runtime and matching OpenAgent package for
...
runtime and matching OpenAgent package. The bundled installer owns that setup;
...
Read the version to install from the app: choose **About OpenAgent** in the
...
- "OpenAgent Active" enables/disables the LaunchAgent.
...
the list appears informationally under "Other OpenAgent launchd jobs (macOS)".
...
direct arguments invoke an absolute OpenAgent path with a Gateway lifecycle
...
Keep OpenAgent state on a local, non-synced disk. Avoid iCloud Drive and other
...
`/Applications/OpenAgent.app/Contents/MacOS/openclaw-mac` directly. See
