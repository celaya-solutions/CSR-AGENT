...
- Same path: run a release app from `/Applications/OpenAgent.app`; keep development builds at one fixed path such as `dist/OpenAgent.app`.
...
1. Click **Grant** in OpenAgent.
2. If macOS opens System Settings, enable the running OpenAgent app under **Privacy & Security -> Screen & System Audio Recording** (called **Screen Recording** on older macOS versions).
3. Return to OpenAgent and retry the screenshot. **Dashboard → Settings → This Mac → Permissions** shows the refreshed access status.

After an explicit **Grant** request, OpenAgent checks ScreenCaptureKit as well as the macOS permission preflight. This lets it recognize access when the preflight still reports an old denial. Passive status checks do not initiate this probe before you request access.

If access still appears missing, quit and reopen OpenAgent from the same app path. Some macOS permission changes require an app restart before capture works. If both release and development builds are installed, grant access to the build you are actually running: approving `/Applications/OpenAgent.app` does not grant access to a development build with a different bundle identifier.

## Accessibility grants for Node and CLI runtimes

Prefer granting Accessibility to OpenAgent.app, Peekaboo.app, or another signed helper with its own bundle identifier instead of a generic `node` binary.
...
macOS keeps Accessibility, Event Posting, input listening, and Screen Recording in separate TCC buckets. One successful grant does not prove the others are usable. OpenAgent's Computer Control status checks Accessibility, Event Posting, and Screen Recording separately; this is why screenshots can succeed while clicks and typing fail.

An Accessibility row can also remain visibly enabled while its code requirement is pinned to an older build. When OpenAgent reports **Accessibility grant may be stale**, select OpenAgent under **System Settings -> Privacy & Security -> Accessibility**, remove it with **-**, then re-add `/Applications/OpenAgent.app`. Quit and reopen OpenAgent afterward because Accessibility trust can remain cached in the running process.
...
During a Computer execution, OpenAgent uses temporary keep-awake assertions for
...
Desktop viewer does not create an OpenAgent keep-awake execution.
...
OpenAgent honors that lock even when unattended desktop hosting is enabled.

Manual lock, logout, or an unknown desktop state releases keep-awake assertions
and retires active Computer executions. OpenAgent does not unlock the Mac or
...
Example resets (using OpenAgent's bundle ID, `ai.openclaw.mac`):
...
Workaround: move files into the OpenAgent workspace (`~/.openclaw/workspace`) if you want to avoid per-folder grants.
