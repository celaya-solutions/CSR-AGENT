...
- Same path: run a release app from `/Applications/Zero to Agent.app`; keep development builds at one fixed path such as `dist/Zero to Agent.app`.
...
1. Click **Grant** in Zero to Agent.
2. If macOS opens System Settings, enable the running Zero to Agent app under **Privacy & Security -> Screen & System Audio Recording** (called **Screen Recording** on older macOS versions).
3. Return to Zero to Agent and retry the screenshot. **Dashboard → Settings → This Mac → Permissions** shows the refreshed access status.

After an explicit **Grant** request, Zero to Agent checks ScreenCaptureKit as well as the macOS permission preflight. This lets it recognize access when the preflight still reports an old denial. Passive status checks do not initiate this probe before you request access.

If access still appears missing, quit and reopen Zero to Agent from the same app path. Some macOS permission changes require an app restart before capture works. If both release and development builds are installed, grant access to the build you are actually running: approving `/Applications/Zero to Agent.app` does not grant access to a development build with a different bundle identifier.

## Accessibility grants for Node and CLI runtimes

Prefer granting Accessibility to Zero to Agent.app, Peekaboo.app, or another signed helper with its own bundle identifier instead of a generic `node` binary.
...
macOS keeps Accessibility, Event Posting, input listening, and Screen Recording in separate TCC buckets. One successful grant does not prove the others are usable. Zero to Agent's Computer Control status checks Accessibility, Event Posting, and Screen Recording separately; this is why screenshots can succeed while clicks and typing fail.

An Accessibility row can also remain visibly enabled while its code requirement is pinned to an older build. When Zero to Agent reports **Accessibility grant may be stale**, select Zero to Agent under **System Settings -> Privacy & Security -> Accessibility**, remove it with **-**, then re-add `/Applications/Zero to Agent.app`. Quit and reopen Zero to Agent afterward because Accessibility trust can remain cached in the running process.
...
During a Computer execution, Zero to Agent uses temporary keep-awake assertions for
...
Desktop viewer does not create a Zero to Agent keep-awake execution.
...
Zero to Agent honors that lock even when unattended desktop hosting is enabled.

Manual lock, logout, or an unknown desktop state releases keep-awake assertions
and retires active Computer executions. Zero to Agent does not unlock the Mac or
...
Example resets (using Zero to Agent's bundle ID, `ai.openclaw.mac`):
...
Workaround: move files into the Zero to Agent workspace (`~/.openclaw/workspace`) if you want to avoid per-folder grants.
