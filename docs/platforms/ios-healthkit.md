...
OpenAgent can request a read-only summary of the current calendar day from a
...
- An iPhone or iPad running the OpenAgent iOS app where HealthKit reports health data as
  available.
...
- Readable Health data for any metrics you expect to see. An Apple Watch can
  contribute data to the Apple Health store, but the OpenAgent watchOS app is
...
3. Read the disclosure, then choose which Health categories OpenAgent may read
   in Apple's permission sheet.

The switch records your explicit OpenAgent sharing choice. It does not claim
...
- OpenAgent requests read access only. It cannot add or modify Health data.
- OpenAgent reads HealthKit only when `health.summary` is invoked. There is no
...
- HealthKit deliberately does not reveal whether read access was denied. A
  ...
  data type. OpenAgent cannot distinguish those cases.
...
OpenAgent's access in Apple's Health settings, but do not treat an empty result
