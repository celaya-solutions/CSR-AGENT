...
Zero to Agent can request a read-only summary of the current calendar day from a
...
- An iPhone or iPad running the Zero to Agent iOS app where HealthKit reports health data as
  available.
...
- Readable Health data for any metrics you expect to see. An Apple Watch can
  contribute data to the Apple Health store, but the Zero to Agent watchOS app is
...
3. Read the disclosure, then choose which Health categories Zero to Agent may read
   in Apple's permission sheet.

The switch records your explicit Zero to Agent sharing choice. It does not claim
...
- Zero to Agent requests read access only. It cannot add or modify Health data.
- Zero to Agent reads HealthKit only when `health.summary` is invoked. There is no
...
- HealthKit deliberately does not reveal whether read access was denied. A
  ...
  data type. Zero to Agent cannot distinguish those cases.
...
Zero to Agent's access in Apple's Health settings, but do not treat an empty result
