...
OpenAgent ships a native **Windows Hub** companion app plus Windows CLI support.
...
Windows Hub publishes independently from the OpenAgent CLI and Gateway. Download
...
and open the newest stable Windows Hub release. Regular OpenAgent stable releases
also mirror a pinned, release-validated Windows Hub build; that mirror can lag a
newer standalone Hub release.

After install, launch **OpenAgent Companion** from the Start menu or system
...
Windows Hub can register as an OpenAgent node so the agent can use declared
...
without a running OpenAgent Gateway.
...
For terminal-first use, install OpenAgent from PowerShell:
...
Then install OpenAgent inside WSL with the Linux quickstart:
...
Check Task Manager for `OpenAgent.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If not, launch **OpenAgent Companion** from
the Start menu.
