...
The OpenAgent Linux companion is a Tauri desktop app for local and remote
Gateways. It:
...
- installs the OpenAgent CLI and Node in a private managed runtime when local
...
matches the selected model, OpenAgent verifies that exact model before continuing
...
named `OpenAgent-<version>-amd64.deb` and `OpenAgent-<version>-amd64.AppImage`,
with a `SHA256SUMS.linux-app.txt` checksum file next to them. Download the
`.deb` and install it with `sudo apt install ./OpenAgent-<version>-amd64.deb`,
