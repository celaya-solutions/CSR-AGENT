...
OpenAgent has two separate Watch voice paths:
...
Pair the Watch with the iPhone in Apple's Watch app, install OpenAgent from
**Watch app -> My Watch -> Available Apps**, then open OpenAgent once on both
devices.
...
quick replies, respecting the iPhone's auto-connect setting. Update OpenAgent on
...
3. Open OpenAgent on the Watch before the setup code expires. Open **Talk on
   Watch** and wait for **Ready to talk**.
4. Tap **Start**, allow microphone access, and choose an agent if prompted.
   Keep OpenAgent on screen until it shows **Connected**. Opening the voice
...
and does not retry automatically. Bring OpenAgent to the foreground and tap
...
connecting stops with a message asking you to keep OpenAgent on screen. Navigating
...
OpenAgent is active, even when the paired iPhone is unavailable.
...
- OpenAgent is active on the watch. The non-voice direct node uses short HTTPS
...
3. Open OpenAgent on the watch before the short-lived setup code expires.
...
- Only genuine OpenAgent iOS builds distributed through Apple can use the hosted relay.
...
Why this design was created: to keep production APNs credentials out of user Gateways, avoid storing raw official-build APNs tokens on the Gateway, allow hosted relay usage only for official OpenAgent iOS builds, and prevent one Gateway from sending wake pushes to iOS devices owned by a different Gateway.
...
The iOS app is a mobile node surface, not a Codex Computer Use backend. Codex Computer Use and `cua-driver mcp` control a local macOS desktop through MCP tools; the iOS app exposes iPhone capabilities through OpenAgent node commands such as `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through OpenAgent by invoking node commands, but those calls go through the Gateway node protocol and follow iOS foreground/background limits. Use [Codex Computer Use](/plugins/codex-computer-use) for local desktop control and this page for iOS node capabilities.
...
The **Start Live Voice** App Shortcut opens OpenAgent to the current
chat and starts the same Talk path as the inline Talk control.

1. Open OpenAgent and [pair and connect to your Gateway](/platforms/ios#quick-start-pair-+-connect)
...
2. In **Shortcuts → Apps → OpenAgent**, choose **Start Live Voice**. You can also
   ask Siri: **"Start live voice with OpenAgent"**.
3. Allow microphone access when iOS prompts. Unlock your iPhone if asked, and
   keep OpenAgent in the foreground while Talk starts. The shortcut does not
...
- Watch shows no iPhone state: confirm the iPhone reports `watchPaired: true`
  ...
  from **My Watch -> Available Apps**. After either change, open OpenAgent on the
