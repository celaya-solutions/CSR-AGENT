...
Zero to Agent has two separate Watch voice paths:
...
Pair the Watch with the iPhone in Apple's Watch app, install Zero to Agent from
**Watch app -> My Watch -> Available Apps**, then open Zero to Agent once on both
devices.
...
quick replies, respecting the iPhone's auto-connect setting. Update Zero to Agent on
...
3. Open Zero to Agent on the Watch before the setup code expires. Open **Talk on
   Watch** and wait for **Ready to talk**.
4. Tap **Start**, allow microphone access, and choose an agent if prompted.
   Keep Zero to Agent on screen until it shows **Connected**. Opening the voice
...
and does not retry automatically. Bring Zero to Agent to the foreground and tap
...
connecting stops with a message asking you to keep Zero to Agent on screen. Navigating
...
Zero to Agent is active, even when the paired iPhone is unavailable.
...
- Zero to Agent is active on the watch. The non-voice direct node uses short HTTPS
...
3. Open Zero to Agent on the watch before the short-lived setup code expires.
...
- Only genuine Zero to Agent iOS builds distributed through Apple can use the hosted relay.
...
Why this design was created: to keep production APNs credentials out of user Gateways, avoid storing raw official-build APNs tokens on the Gateway, allow hosted relay usage only for official Zero to Agent iOS builds, and prevent one Gateway from sending wake pushes to iOS devices owned by a different Gateway.
...
The iOS app is a mobile node surface, not a Codex Computer Use backend. Codex Computer Use and `cua-driver mcp` control a local macOS desktop through MCP tools; the iOS app exposes iPhone capabilities through Zero to Agent node commands such as `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Zero to Agent by invoking node commands, but those calls go through the Gateway node protocol and follow iOS foreground/background limits. Use [Codex Computer Use](/plugins/codex-computer-use) for local desktop control and this page for iOS node capabilities.
...
The **Start Live Voice** App Shortcut opens Zero to Agent to the current
chat and starts the same Talk path as the inline Talk control.

1. Open Zero to Agent and [pair and connect to your Gateway](/platforms/ios#quick-start-pair-+-connect)
...
2. In **Shortcuts → Apps → Zero to Agent**, choose **Start Live Voice**. You can also
   ask Siri: **"Start live voice with Zero to Agent"**.
3. Allow microphone access when iOS prompts. Unlock your iPhone if asked, and
   keep Zero to Agent in the foreground while Talk starts. The shortcut does not
...
- Watch shows no iPhone state: confirm the iPhone reports `watchPaired: true`
  ...
  from **My Watch -> Available Apps**. After either change, open Zero to Agent on the
