---
summary: "Run the Zero to Agent Gateway on EasyRunner with Podman and Caddy"
read_when:
  - Deploying Zero to Agent on EasyRunner
  - Running the Gateway behind EasyRunner's Caddy proxy
  - Choosing persistent volumes and auth for a hosted Gateway
title: "EasyRunner"
---

EasyRunner hosts the Zero to Agent Gateway as a small containerized app behind its
...
- The official Zero to Agent image (`ghcr.io/openclaw/openclaw`) or your own build.
...
- Pull or build the new Zero to Agent image, then redeploy the EasyRunner app.
