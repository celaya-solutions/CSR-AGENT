---
summary: "Run the OpenAgent Gateway on EasyRunner with Podman and Caddy"
read_when:
  - Deploying OpenAgent on EasyRunner
  - Running the Gateway behind EasyRunner's Caddy proxy
  - Choosing persistent volumes and auth for a hosted Gateway
title: "EasyRunner"
---

EasyRunner hosts the OpenAgent Gateway as a small containerized app behind its
...
- The official OpenAgent image (`ghcr.io/openclaw/openclaw`) or your own build.
...
- Pull or build the new OpenAgent image, then redeploy the EasyRunner app.
