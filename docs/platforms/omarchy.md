---
summary: "Zero to Agent on Omarchy: bar plugin, desktop app handoff, and support"
read_when:
  - Installing the Zero to Agent bar plugin on Omarchy
  - Using agents, sessions, and quick prompts from the Omarchy bar
  - Troubleshooting duplicate Zero to Agent icons or desktop connection handoff
title: "Omarchy"
---

# Zero to Agent on Omarchy

The Zero to Agent Omarchy plugin puts agents, recent sessions, and a quick prompt in
...
This page covers the plugin in the Zero to Agent source tree. Install it separately;
...
- Either a connected Zero to Agent Linux desktop app with Omarchy integration, or
...
Use your existing Zero to Agent installation and authentication. In standalone
...
From a Zero to Agent source checkout containing `apps/linux/omarchy`, run:
...
Click the Zero to Agent mascot. Confirm that the panel shows the expected agents
...
and your existing Zero to Agent updater for the CLI and Gateway. After updating
...
| Two Zero to Agent icons                    | Update both the plugin and desktop app to versions with the integration. They must run in the same desktop user session.                                  |
| CLI not found                              | Check `OPENCLAW_DESKTOP_CLI`, `~/.openclaw/bin/openclaw`, or the graphical session's `PATH`. Install through your normal Zero to Agent installation path. |
...
For setup help, use [Zero to Agent support](/help). For a reproducible plugin or
handoff defect, file an [Zero to Agent bug report](https://github.com/openclaw/openclaw/issues/new?template=bug_report.yml)
