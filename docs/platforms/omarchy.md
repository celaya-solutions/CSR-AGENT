---
summary: "OpenAgent on Omarchy: bar plugin, desktop app handoff, and support"
read_when:
  - Installing the OpenAgent bar plugin on Omarchy
  - Using agents, sessions, and quick prompts from the Omarchy bar
  - Troubleshooting duplicate OpenAgent icons or desktop connection handoff
title: "Omarchy"
---

# OpenAgent on Omarchy

The OpenAgent Omarchy plugin puts agents, recent sessions, and a quick prompt in
...
This page covers the plugin in the OpenAgent source tree. Install it separately;
...
- Either a connected OpenAgent Linux desktop app with Omarchy integration, or
...
Use your existing OpenAgent installation and authentication. In standalone
...
From an OpenAgent source checkout containing `apps/linux/omarchy`, run:
...
Click the OpenAgent mascot. Confirm that the panel shows the expected agents
...
and your existing OpenAgent updater for the CLI and Gateway. After updating
...
| Two OpenAgent icons                    | Update both the plugin and desktop app to versions with the integration. They must run in the same desktop user session.                                  |
| CLI not found                              | Check `OPENCLAW_DESKTOP_CLI`, `~/.openclaw/bin/openclaw`, or the graphical session's `PATH`. Install through your normal OpenAgent installation path. |
...
For setup help, use [OpenAgent support](/help). For a reproducible plugin or
handoff defect, file an [OpenAgent bug report](https://github.com/openclaw/openclaw/issues/new?template=bug_report.yml)
