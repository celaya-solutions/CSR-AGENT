---
summary: "Install and use the Zero to Agent macOS menu bar app"
read_when:
...
The macOS app is the Zero to Agent **menu bar companion**: native tray UI, macOS
...
**Zero to Agent.app requires macOS 15.0 (Sequoia) or later.** This also applies to
...
Get macOS app builds from [Zero to Agent GitHub releases](https://github.com/openclaw/openclaw/releases).
When a release ships macOS app assets, look for:

- `Zero to Agent-<version>.dmg` (preferred)
...
1. Install and launch **Zero to Agent.app**.
...
5. Finish. The app opens the dashboard, where Zero to Agent guides the rest of the
...
is enabled. **About Zero to Agent** opens the standard macOS About panel with the app
...
Drag the empty header space or title in the docked Zero to Agent chat panel to move the app window. Its dock-position and close buttons remain clickable.
