---
summary: "Install and use the OpenAgent macOS menu bar app"
read_when:
...
The macOS app is the OpenAgent **menu bar companion**: native tray UI, macOS
...
**OpenAgent.app requires macOS 15.0 (Sequoia) or later.** This also applies to
...
Get macOS app builds from [OpenAgent GitHub releases](https://github.com/openclaw/openclaw/releases).
When a release ships macOS app assets, look for:

- `OpenAgent-<version>.dmg` (preferred)
- `OpenAgent-<version>.zip`
...
1. Install and launch **OpenAgent.app**.
...
5. Finish. The app opens the dashboard, where OpenAgent guides the rest of the
...
is enabled. **About OpenAgent** opens the standard macOS About panel with the app
...
Drag the empty header space or title in the docked OpenAgent chat panel to move the app window. Its dock-position and close buttons remain clickable.
