...
<Accordion title="Installing OpenAgent Updates">
...
On Linux, code updates can preserve an administrator-owned service definition instead of trying to rewrite it, while an unsafe or uninspectable service handoff still fails visibly. If a plugin replacement asks for new capabilities, OpenAgent keeps the known-good plugin available while the replacement waits for review; `openclaw update --accept-capabilities` or `openclaw update repair --accept-capabilities` approves only the staged artifact for that invocation, and `--yes` does not.

One upgrade path still needs a manual repair. If you are on OpenAgent 2026.7.1 with pnpm 11, run `pnpm add -g openclaw@latest` once. OpenAgent does not upgrade Node for you.
...
- Allow experimental OpenAgent use on node:sqlite-capable Bun builds [#114256](https://github.com/openclaw/openclaw/pull/114256)
...
- Keep the Codex plugin matched to stable OpenAgent upgrades [#124209](https://github.com/openclaw/openclaw/pull/124209)
...
- Consolidate OpenAgent configuration and legacy migrations [#111527](https://github.com/openclaw/openclaw/pull/111527)
...
<Accordion title="OpenAgent Restarts and Running Work">

Before a supported snapshot or [targeted restart](/gateway/restart-recovery), Gateway suspend and resume can pause new ordinary work, report blockers, and drain the agent runs, deliveries, scheduled jobs, queues, sessions, and background commands OpenAgent already tracks. Failed configuration reloads keep the prior coherent state, and rapid configuration writes retain pending restart intent instead of dropping it.
...
The wait covers work OpenAgent tracks. New channel or external ingress, existing plugin connections, unregistered background work, and durable receipt of incoming messages remain outside it, and externally supervised installations must consume the handoff and complete their own restart.
...
- Add click-to-diagnose health nudges to OpenAgent system care [#110708](https://github.com/openclaw/openclaw/pull/110708)
- Add system change history to Ask OpenAgent [#111286](https://github.com/openclaw/openclaw/pull/111286)
- Add a state-aware caretaker welcome with quick actions [#111615](https://github.com/openclaw/openclaw/pull/111615)
- Add a dedicated OpenAgent settings chat to Android [#112788](https://github.com/openclaw/openclaw/pull/112788)
- Add a dockable Ask OpenAgent companion in the Control UI [#115123](https://github.com/openclaw/openclaw/pull/115123)
...
- Make Ask OpenAgent persistent and globally accessible [#125107](https://github.com/openclaw/openclaw/pull/125107)
- Move Ask OpenAgent to the sidebar footer [#125486](https://github.com/openclaw/openclaw/pull/125486)
...
- Give Ask OpenAgent the regular chat experience [#110934](https://github.com/openclaw/openclaw/pull/110934)
...
- Make OTEL_SDK_DISABLED fully disable OpenAgent telemetry [#119961](https://github.com/openclaw/openclaw/pull/119961)
...
- Fix Doctor sandbox repair for symlinked OpenAgent launchers [#90942](https://github.com/openclaw/openclaw/pull/90942)
...
[Reset and uninstall](/cli/reset) now refuse to remove data until the Gateway service is torn down and OpenAgent can establish that no other process owns the state. If teardown or ownership checks fail, the state stays put, and a state-only uninstall leaves configured workspaces alone.
