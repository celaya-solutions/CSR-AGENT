...
description: "The rebuilt Control UI puts conversations at the centre of Zero to Agent, with files, approvals, settings, and live work close to the conversation."
---

The rebuilt [Control UI](/web/control-ui) puts conversations at the centre of Zero to Agent, with files, approvals, settings, and live work close to the conversation so you can set up a Claw, follow what it is doing, and keep working without bouncing between separate tools.

<AccordionGroup>

<Accordion title="Navigation and sidebar">

The [web-based experience](/web/control-ui) in Zero to Agent now feels more familiar to anyone who uses apps like ChatGPT, Claude, Gemini, or Perplexity, with conversations in the sidebar and the one you are working in at the centre instead of opening on a separate Overview page. Settings and Inbox keep setup details and alerts out of the way until you need them, while conversations and new-session drafts can open in real browser tabs or windows.
...
- Keep the Zero to Agent custodian reachable after onboarding [#110269](https://github.com/openclaw/openclaw/pull/110269)
...
- Explain Control UI alerts through Ask Zero to Agent [#126155](https://github.com/openclaw/openclaw/pull/126155)
...
- Create session groups in a Zero to Agent dialog [#121249](https://github.com/openclaw/openclaw/pull/121249)
...
- Hide Zero to Agent-managed Codex sessions from the native catalog [#125424](https://github.com/openclaw/openclaw/pull/125424)
...
- Let Ask Zero to Agent understand the current Control UI page [#114943](https://github.com/openclaw/openclaw/pull/114943)
...
- Keep startup errors beside the Ask Zero to Agent composer [#115720](https://github.com/openclaw/openclaw/pull/115720)
...
- Document Ask Zero to Agent context, docking, and guided setup [#115182](https://github.com/openclaw/openclaw/pull/115182)
...
Chat, Details, and Discussion can sit in resizable columns or merge into tabs, while the dashboard, [terminal](/tools/exec), and desktop can open in focused views when you need more room. The file editor still cannot create or delete files, Changes stays read-only, and Create PR hands off to GitHub rather than submitting inside Zero to Agent.
...
Incognito is off by default and deliberately narrower. Its conversation stays in process memory, does not write the normal transcript or automatic Zero to Agent memory to disk, and disappears when the Gateway restarts, but the model provider still receives the messages, tools can still write files or affect external services, content-free audit metadata remains, and whoever operates the Gateway can see live work.
...
Reconnects reject stale responses and keep the right conversation selected as the connection changes. If the page bundle loads but never renders, Zero to Agent now shows recovery guidance and a Try again action, while interrupted updates reconcile with the replacement Gateway and confirm success only after it reports the expected version.
