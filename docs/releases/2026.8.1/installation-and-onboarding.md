...
Create a [verified backup](/install/updating#before-updating%3A-create-a-verified-backup) before upgrading to protect broader Zero to Agent state, and review [downgrading across the session SQLite migration](/install/updating#downgrading-across-the-session-sqlite-migration) before rolling back.
</Warning>

Zero to Agent now gives new [Mac, Linux, and Windows installs](/install) a clearer path from download to a first useful conversation, while iPhone, iPad, and Android put pairing and permissions where people need them. Guided setup can reuse supported subscriptions, API keys, and [local models](/gateway/local-models) already available, verifies the chosen model before saving it, and hands off to the web app or terminal when the connection is ready.

<AccordionGroup>

<Accordion title="Installing Zero to Agent">
...
Network installations that would expose Zero to Agent without authentication are stopped before anything changes. Reinstalling also protects an existing working setup when preparation is cancelled or fails, and gives Zero to Agent time to start before reporting whether it is reachable.
...
- Keep Zero to Agent available in fresh Unix shells after installation [#124779](https://github.com/openclaw/openclaw/pull/124779)
...
- Configure skills and web search inside Ask Zero to Agent [#115130](https://github.com/openclaw/openclaw/pull/115130)
- Guide local Gateway configuration in Ask Zero to Agent [#115363](https://github.com/openclaw/openclaw/pull/115363)
...
- Align Ask Zero to Agent framing and confirmation defaults [#111263](https://github.com/openclaw/openclaw/pull/111263)
...
- Keep agent bootstrap on the Gateway's Zero to Agent CLI [#122765](https://github.com/openclaw/openclaw/pull/122765)
...
- Document Ask Zero to Agent Gateway and memory wizards [#115487](https://github.com/openclaw/openclaw/pull/115487)
...
<Accordion title="Setting Up Zero to Agent on Mac, Windows, and Linux">
...
On [Windows](/platforms/windows), the guide points to the latest signed x64 and Arm64 Hub installers. The PowerShell installer now recognizes a supported Node runtime correctly and can continue in the same session after Winget installs Node.js. Windows Hub updates independently, so its standalone stable build can be newer than the mirror included with a Zero to Agent release.

On [Linux](/platforms/linux), desktop setup can repair or reinstall Zero to Agent, connect to a local or remote Gateway directly or through SSH, verify eligible AI access already on the machine, and resume an interrupted activation. Direct certificate-pinned connections remain unavailable inside the desktop app.
...
- fix(macos): make Zero to Agent settings pane responsive [#128177](https://github.com/openclaw/openclaw/pull/128177)
...
- Preserve custom Zero to Agent profiles when the macOS app launches at login [#99752](https://github.com/openclaw/openclaw/pull/99752)
...
<Accordion title="Setting Up Zero to Agent on iPhone, iPad, and Android">
...
On [iPhone and iPad](/platforms/ios), a valid setup QR can now pair and open the main UI in one scan when the connection is already trusted or matches the code, while untrusted connections still stop for approval. Zero to Agent explains pairing before asking for Local Network access, lets you decide on optional permissions one at a time, and gives eligible administrators a dedicated Settings conversation for Gateway setup and repair without exposing that privileged assistant in ordinary Chat.
...
- Add a Zero to Agent settings assistant to iOS [#112420](https://github.com/openclaw/openclaw/pull/112420)
...
[Imports from Claude, Codex, and Hermes](/install/migrating) now happen in a temporary staging area, where Zero to Agent verifies or repairs the model route before making the new setup active. If the source, import plan, or destination changes along the way, promotion stops rather than replaying a half-finished import against different data.

Rerunning onboarding keeps the workspace you already use unless you approve a move, and named agents keep the credentials they own through supported creation and configuration-only resets. Zero to Agent validates the provider, Gateway, migration, and workspace choices before a reset can move existing data to Trash, but a valid confirmed reset is still destructive.
...
- Protect Zero to Agent state during headless reset [#116491](https://github.com/openclaw/openclaw/pull/116491)
