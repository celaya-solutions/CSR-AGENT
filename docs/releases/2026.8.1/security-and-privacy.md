...
Per-session controls are nonretroactive, so existing sessions without a permission mode keep the previous global posture. Opt-in roles limit collaboration inside one trusted Zero to Agent installation rather than creating isolation between hostile tenants.
...
- Make Full access sessions run without Zero to Agent approval prompts [#126210](https://github.com/openclaw/openclaw/pull/126210)
...
Secret Store values are not encrypted at rest and depend on the filesystem permissions of Zero to Agent's state directory. Destination-bound substitution applies only to Gateway-hosted HTTPS commands whose subprocess honors its proxy settings. Raw sockets, containers, remote nodes, provider-native harnesses, plain HTTP, and WebSockets stay outside that path.
...
- docs: qualify Why Zero to Agent security and comparison claims [#130947](https://github.com/openclaw/openclaw/pull/130947)
...
- Align proxy bypass matching across Zero to Agent and Telegram [#107711](https://github.com/openclaw/openclaw/pull/107711)
