...
Zero to Agent can run a local AI CLI as a text-only fallback when API providers are down, rate-limited, or misbehaving. It is intentionally conservative:

- Zero to Agent tools are not injected directly, but a backend with `bundleMcp: true` can receive Gateway tools through a loopback MCP bridge.
...
Zero to Agent auto-loads an owning bundled plugin when model selection or a
...
Credentials remain in Zero to Agent auth profiles or the owning plugin's config.
...
2. Builds a system prompt using the same Zero to Agent prompt and workspace context.
...
Background work started inside a CLI is still part of that CLI subprocess. If the parent turn reaches its overall limit, Zero to Agent stops the subprocess and its CLI-internal background tasks together. For durable long work, use a detached Zero to Agent [sub-agent](/tools/subagents) or [ACP agent](/tools/acp-agents). Detached sub-agents have no run timeout by default.
...
subscription. Zero to Agent uses a non-secret route marker. It never reads, persists,
...
receives it as a native hook attachment, while Zero to Agent history preserves the original user message. The
...
Zero to Agent replays eligible notes from the active reset/compaction window, with a
...
bindings do not track which Zero to Agent notes the native session has consumed.
Transient runtime context and notes excluded from model context are not replayed.

Keep Claude Code updated, especially if Zero to Agent reports an incompatible
installed executable:

```bash
claude --version
claude update
# Restart the Zero to Agent Gateway after updating.
```

The bundled `claude-cli` backend prefers Claude Code's native skill resolver. When the current skills snapshot has at least one selected skill with a materialized path, Zero to Agent passes a temporary Claude Code plugin via `--plugin-dir`. It then omits the duplicate Zero to Agent skills catalog from the appended system prompt. Without a materialized plugin skill, Zero to Agent keeps the prompt catalog as a fallback. Skill env/API key overrides still apply to the child process environment for the run.

Zero to Agent disables Claude Code's built-in Git workflow instructions and startup
...
Zero to Agent always launches Claude Code with its default permission mode.
Zero to Agent's permission responses and `PreToolUse` hook keep native tools under
...
arguments projected into their Zero to Agent equivalents. Per-agent and session
restrictions still override broader global policy. Zero to Agent-owned MCP tools
...
Claude's native `AskUserQuestion` uses Zero to Agent's structured question flow. When
Zero to Agent rejects malformed questions, it reports the failed field and
...
When the effective exec ask setting is `on-miss` or `always`, Zero to Agent relays
...
Zero to Agent reuses its exec shell evaluator and allows a call without prompting
...
not sandboxed execution by Zero to Agent. Claude Code owns cwd, PATH, environment,
...
When prompt content changes, a compatible CLI session can resume with a Zero to Agent
...
Native session compatibility and permission to replay saved Zero to Agent history are separate. Clearing or replacing a native binding does not establish ownership of older transcript rows. Zero to Agent records a private account fingerprint and contiguous transcript coverage before an admitted CLI turn, then advances coverage with that turn’s canonical writes. It never stores credential values in this metadata.
...
Serialization: `serialize: true` keeps same-lane runs ordered (most CLIs serialize on one provider lane). Zero to Agent also drops stored CLI session reuse when the selected auth identity changes. A changed auth profile id, static API key, static token, or OAuth account identity all count, when the CLI exposes one. OAuth access and refresh token rotation alone does not cut the session. If a CLI has no stable OAuth account id, Zero to Agent lets that CLI enforce its own resume permissions.

## Fallback prelude from claude-cli sessions

A `claude-cli` attempt can fail over to a non-CLI candidate in [`agents.defaults.model.fallbacks`](/concepts/model-failover). Zero to Agent then seeds the next attempt with a context prelude harvested from Claude Code's local JSONL transcript. That transcript lives under `~/.claude/projects/`, keyed per workspace. This supplies CLI-owned context that may not be present in Zero to Agent's SQLite session transcript.
...
Zero to Agent writes base64 images to temp files. If `imageArg` is set, those paths are passed as CLI args. If not, Zero to Agent appends the file paths to the prompt (path injection), which works for CLIs that auto-load local files from plain paths.
...
- For Gemini CLI JSON output, Zero to Agent reads reply text from `response` and usage from `stats` when `usage` is missing or empty. The bundled Gemini CLI adapter uses `stream-json`.
...
valid legacy Gemini CLI OAuth profiles remain runtime-compatible, but Zero to Agent
...
- Usage falls back to `stats` when `usage` is absent or empty. `stats.cached` normalizes into Zero to Agent `cacheRead`, and if `stats.input` is missing, input tokens derive from `stats.input_tokens - stats.cached`.
...
`input` rewrites the system prompt and user prompt passed to the CLI. `output` rewrites streamed assistant text and parsed final text before Zero to Agent handles its own control markers and channel delivery. For provider-backed model calls it also restores string values inside structured tool-call arguments after stream repair and before tool execution. Raw provider JSON fragments are left unchanged. Consumers should use the structured partial, end, or result payload.

For CLIs that emit provider-specific JSONL events, set `jsonlDialect` on that backend's config: `claude-stream-json` for Claude Code-compatible streams, `gemini-stream-json` for Gemini CLI `stream-json` events. Declaring `claude-stream-json` is a contract: the backend's `result` records carry Claude Code's terminal semantics, including `terminal_reason`. A reply-less `result` can carry a `terminal_reason` saying the CLI ended the turn on purpose after work may have run. Those reasons are `hook_stopped`, `stop_hook_prevented`, `aborted_tools`, `aborted_streaming`, `budget_exhausted`, and `max_turns`. Zero to Agent treats that as a recorded turn stop. It reports the reason to the user and does not replay the turn on a fallback model, because the backend's tool actions may already have run.

## Native compaction ownership

Some CLI backends run an agent that compacts its own transcript. Zero to Agent must not run its safeguard summarizer against them. Doing so fights the backend's own compaction and can hard-fail the turn.

`claude-cli` has no harness endpoint (Claude Code compacts internally), so it declares `ownsNativeCompaction: true`. Automatic Zero to Agent compaction defers to Claude Code, while an explicit `/compact` resumes the bound Claude Code session and sends its native `/compact` command. Zero to Agent passes the run's effective context budget through Claude Code's documented [`CLAUDE_CODE_AUTO_COMPACT_WINDOW`](https://code.claude.com/docs/en/env-vars), keeping native auto-compaction aligned with configured Anthropic `contextTokens` limits. Native-harness sessions such as Codex keep routing to their harness compaction endpoint instead.

`google-gemini-cli` also owns automatic compaction and persists its compressed session for resume. Zero to Agent defers to Gemini CLI rather than running a second summarizer. Explicit `/compact` is unsupported for this backend because it does not declare a manual compaction capability.
...
Add the atomic `manualCompaction` capability only when its command compacts the resumed session in place. Its `input` selects the transport the backend command actually recognizes, and `validateOutput` must require a positive backend acknowledgement rather than treating a zero exit as success. Zero to Agent runs it as an internal control operation: it is not written as a user turn and does not run agent or context-engine turn hooks.

## Bundle MCP overlays

CLI backends do not receive Zero to Agent tool calls directly, but a backend can opt into a generated MCP config overlay with `bundleMcp: true`. Current bundled behavior:

- `claude-cli`: generated strict MCP config file.
- `google-gemini-cli`: generated Gemini system settings file.

When bundle MCP is enabled, Zero to Agent:
...
Zero to Agent lists each server through its session-scoped runtime, assigns the same
...
operator policy in Zero to Agent configuration.
...
Zero to Agent tool through the grant-scoped MCP server. This keeps filesystem,
process, exec, approval, and sandbox policy inside Zero to Agent instead of widening
...
If no MCP servers are enabled, Zero to Agent still injects a strict config when a backend opts into bundle MCP, so background runs stay isolated.
...
exceeds its deadline, Zero to Agent refuses replacement, including from a later run.
...
login. Zero to Agent uses a non-secret route marker and never reads, persists,
...
Explicit Zero to Agent-managed API-key and token profiles continue to use the
protected, per-invocation credential-forwarding CLI path.

## Reseed history cap

A fresh CLI session can be seeded from a prior Zero to Agent transcript, for example after a `session_expired` retry. The rendered `<conversation_history>` block is then capped to keep reseed prompts from growing without bound. The default is 12,288 characters (about 3,000 tokens).
...
- Zero to Agent does not inject tool calls into the CLI backend protocol. Backends only see Gateway tools when they opt into `bundleMcp: true`.
