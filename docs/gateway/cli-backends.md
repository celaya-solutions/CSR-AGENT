...
OpenAgent can run a local AI CLI as a text-only fallback when API providers are down, rate-limited, or misbehaving. It is intentionally conservative:

- OpenAgent tools are not injected directly, but a backend with `bundleMcp: true` can receive Gateway tools through a loopback MCP bridge.
...
OpenAgent auto-loads an owning bundled plugin when model selection or a
...
Credentials remain in OpenAgent auth profiles or the owning plugin's config.
...
2. Builds a system prompt using the same OpenAgent prompt and workspace context.
...
subscription. OpenAgent uses a non-secret route marker. It never reads, persists,
...
receives it as a native hook attachment, while OpenAgent history preserves the original user message. The
...
OpenAgent replays eligible notes from the active reset/compaction window, with a
...
bindings do not track which OpenAgent notes the native session has consumed.
Transient runtime context and notes excluded from model context are not replayed.

Keep Claude Code updated, especially if OpenAgent reports an incompatible
installed executable:

```bash
claude --version
claude update
# Restart the OpenAgent Gateway after updating.
```

The bundled `claude-cli` backend prefers Claude Code's native skill resolver. When the current skills snapshot has at least one selected skill with a materialized path, OpenAgent passes a temporary Claude Code plugin via `--plugin-dir`. It then omits the duplicate OpenAgent skills catalog from the appended system prompt. Without a materialized plugin skill, OpenAgent keeps the prompt catalog as a fallback. Skill env/API key overrides still apply to the child process environment for the run.

OpenAgent disables Claude Code's built-in Git workflow instructions and startup
...
OpenAgent always launches Claude Code with its default permission mode.
OpenAgent's permission responses and `PreToolUse` hook keep native tools under
...
arguments projected into their OpenAgent equivalents. Per-agent and session
restrictions still override broader global policy. OpenAgent-owned MCP tools
...
Claude's native `AskUserQuestion` uses OpenAgent's structured question flow. When
OpenAgent rejects malformed questions, it reports the failed field and
...
When the effective exec ask setting is `on-miss` or `always`, OpenAgent relays
...
not sandboxed execution by OpenAgent. Claude Code owns cwd, PATH, environment,
...
When prompt content changes, a compatible CLI session can resume with an OpenAgent
...
Native session compatibility and permission to replay saved OpenAgent history are separate. Clearing or replacing a native binding does not establish ownership of older transcript rows. OpenAgent records a private account fingerprint and contiguous transcript coverage before an admitted CLI turn, then advances coverage with that turn’s canonical writes. It never stores credential values in this metadata.
...
Serialization: `serialize: true` keeps same-lane runs ordered (most CLIs serialize on one provider lane). OpenAgent also drops stored CLI session reuse when the selected auth identity changes. A changed auth profile id, static API key, static token, or OAuth account identity all count, when the CLI exposes one. OAuth access and refresh token rotation alone does not cut the session. If a CLI has no stable OAuth account id, OpenAgent lets that CLI enforce its own resume permissions.

## Fallback prelude from claude-cli sessions

A `claude-cli` attempt can fail over to a non-CLI candidate in [`agents.defaults.model.fallbacks`](/concepts/model-failover). OpenAgent then seeds the next attempt with a context prelude harvested from Claude Code's local JSONL transcript. That transcript lives under `~/.claude/projects/`, keyed per workspace. This supplies CLI-owned context that may not be present in OpenAgent's SQLite session transcript.
...
OpenAgent writes base64 images to temp files. If `imageArg` is set, those paths are passed as CLI args. If not, OpenAgent appends the file paths to the prompt (path injection), which works for CLIs that auto-load local files from plain paths.
...
Add the atomic `manualCompaction` capability only when its command compacts the resumed session in place. Its `input` selects the transport the backend command actually recognizes, and `validateOutput` must require a positive backend acknowledgement rather than treating a zero exit as success. OpenAgent runs it as an internal control operation: it is not written as a user turn and does not run agent or context-engine turn hooks.
...
OpenAgent lists each server through its session-scoped runtime, assigns the same
...
OpenAgent tool through the grant-scoped MCP server. This keeps filesystem,
process, exec, approval, and sandbox policy inside OpenAgent instead of widening
...
login. OpenAgent uses a non-secret route marker and never reads, persists,
...
Explicit OpenAgent-managed API-key and token profiles continue to use the
protected, per-invocation credential-forwarding CLI path.

## Reseed history cap

A fresh CLI session can be seeded from a prior OpenAgent transcript, for example after a `session_expired` retry. The rendered `<conversation_history>` block is then capped to keep reseed prompts from growing without bound. The default is 12,288 characters (about 3,000 tokens).
...
- OpenAgent does not inject tool calls into the CLI backend protocol. Backends only see Gateway tools when they opt into `bundleMcp: true`.
