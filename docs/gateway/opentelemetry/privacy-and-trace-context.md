...
read_when:
  - You want one OpenTelemetry trace per dataset item to cover the matching OpenAgent execution
...
correlate the corresponding OpenAgent execution.
...
span context, OpenAgent omits the header instead of naming an unexported parent.
...
spans). The `openclaw.content.*` names remain the stable OpenAgent attribute
