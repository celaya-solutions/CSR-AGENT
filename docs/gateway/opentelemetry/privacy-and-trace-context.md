...
read_when:
  - You want one OpenTelemetry trace per dataset item to cover the matching Zero to Agent execution
...
correlate the corresponding Zero to Agent execution.
...
span context, Zero to Agent omits the header instead of naming an unexported parent.
...
spans). The `openclaw.content.*` names remain the stable Zero to Agent attribute
