...
request latency separate from full-turn latency. OpenAgent's OTEL model-call
...
OpenAgent's CLI boundary through
`openclaw.transport`:
...
OpenAgent gives Claude CLI turns the same ownership hierarchy used by other
...
span. The harness and run spans are synthetic OpenAgent turn boundaries, not
...
the same OpenAgent run.

The span starts when OpenAgent admits the prepared CLI turn and ends only after
...
usage on its terminal result. OpenAgent reply accounting continues to use the
...
For these CLI spans, byte and timing fields describe the observable OpenAgent
CLI boundary:
...
With `captureContent` enabled, the span exports the effective prompt OpenAgent
...
are omitted from the Claude assistant envelope. OpenAgent does not
...
A `processing` session does not age toward the built-in liveness threshold while OpenAgent observes reply, tool, status, block, or ACP runtime progress. Typing keepalives do not count as progress, so a silent model or harness can still be detected.

OpenAgent classifies sessions by the work it can still observe:
