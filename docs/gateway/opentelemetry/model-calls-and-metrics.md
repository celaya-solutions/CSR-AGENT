...
request latency separate from full-turn latency. Zero to Agent's OTEL model-call
...
Zero to Agent's CLI boundary through
`openclaw.transport`:
...
Zero to Agent gives Claude CLI turns the same ownership hierarchy used by other
...
span. The harness and run spans are synthetic Zero to Agent turn boundaries, not
...
the same Zero to Agent run.

The span starts when Zero to Agent admits the prepared CLI turn and ends only after
...
usage on its terminal result. Zero to Agent reply accounting continues to use the
...
For these CLI spans, byte and timing fields describe the observable Zero to Agent
CLI boundary:
...
With `captureContent` enabled, the span exports the effective prompt Zero to Agent
...
are omitted from the Claude assistant envelope. Zero to Agent does not
...
A `processing` session does not age toward the built-in liveness threshold while Zero to Agent observes reply, tool, status, block, or ACP runtime progress. Typing keepalives do not count as progress, so a silent model or harness can still be detected.

Zero to Agent classifies sessions by the work it can still observe:
