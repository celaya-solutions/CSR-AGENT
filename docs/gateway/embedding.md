---
summary: "Supervise the Zero to Agent Gateway as a child process from Electron or another host app"
read_when:
  - Embedding Zero to Agent in a desktop or server application
...
and upgrades explicit without depending on Zero to Agent's private state layout.
...
Resolve Zero to Agent through the installed package as shown; do not assume that a
...
| `OPENCLAW_NO_RESPAWN=1`          | In an unmanaged embedding child, prevents Zero to Agent from handing an update restart to a detached child. Routine restarts remain in process, so the host keeps ownership of the tracked PID. |
...
Keep the Gateway as the only owner of Zero to Agent state. Common embedding operations
...
Install Zero to Agent through npm, pnpm, or another normal Node package installation so
