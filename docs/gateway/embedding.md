---
summary: "Supervise the OpenAgent Gateway as a child process from Electron or another host app"
read_when:
  - Embedding OpenAgent in a desktop or server application
...
and upgrades explicit without depending on OpenAgent's private state layout.
...
Resolve OpenAgent through the installed package as shown; do not assume that a
...
Keep the Gateway as the only owner of OpenAgent state. Common embedding operations
...
Install OpenAgent through npm, pnpm, or another normal Node package installation so
