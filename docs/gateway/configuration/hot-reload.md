...
invalid external edits without rewriting `openclaw.json`. Zero to Agent-owned config
...
browser processes stay open while Zero to Agent disconnects its control sessions.
...
When you edit a source file that is referenced through `$include`, Zero to Agent plans
