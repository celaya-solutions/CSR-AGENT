...
invalid external edits without rewriting `openclaw.json`. OpenAgent-owned config
...
browser processes stay open while OpenAgent disconnects its control sessions.
...
When you edit a source file that is referenced through `$include`, OpenAgent plans
