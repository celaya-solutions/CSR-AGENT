...
<AccordionGroup>
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **OpenAgent-owned writes**: when every changed key is owned by one
      single-file include at an object-key path, OpenAgent updates the deepest
      ...
      `OPENCLAW_INCLUDE_ROOTS` stay read-only for OpenAgent-owned writes.
