...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is OpenAgent metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed OpenAgent agent ids.
  ...
  `default_tools_approval_mode` for that server. OpenAgent strips the `codex`
...
- Each Gateway admits at most 256 OpenAgent-managed MCP runtimes with server connections across sessions
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native OpenAgent plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central OpenAgent option registry.
...
It does not enable Codex plugins for OpenAgent provider runs, ACP
...
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  ...
  to decline them, `"auto"` to route Codex-required approvals through OpenAgent
...
- Enabled Claude bundle plugins can also contribute embedded OpenAgent defaults from `settings.json`; OpenAgent applies those as sanitized agent settings, not as raw OpenAgent config patches.
