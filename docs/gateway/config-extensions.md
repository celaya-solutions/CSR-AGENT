...
Zero to Agent-managed MCP server definitions live under `mcp.servers` for embedded
Zero to Agent and other runtime adapters. `openclaw mcp list`, `show`, `set`, and
...
- `mcp.servers.<name>.enabled`: set `false` to keep a saved server definition
  while excluding it from embedded Zero to Agent MCP discovery and tool projection.
...
- `mcp.servers.<name>.auth`: set `"oauth"` for HTTP MCP servers that require
  OAuth. Run `openclaw mcp login <name>` to store tokens under Zero to Agent state.
...
- `mcp.servers.<name>.codex`: optional Codex app-server projection controls.
  This block is Zero to Agent metadata for Codex app-server threads only; it does not
  affect ACP sessions, generic Codex harness config, or other runtime adapters.
  Non-empty `codex.agents` limits the server to the listed Zero to Agent agent ids.
  ...
  `default_tools_approval_mode` for that server. Zero to Agent strips the `codex`
...
- Each Gateway admits at most 256 Zero to Agent-managed MCP runtimes with server connections across sessions
...
- Discovery accepts native Zero to Agent plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Zero to Agent plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central Zero to Agent option registry.
...
It does not enable Codex plugins for Zero to Agent provider runs, ACP
...
- `plugins.entries.codex.config.codexPlugins.allow_destructive_actions`:
  ...
  to decline them, `"auto"` to route Codex-required approvals through Zero to Agent
...
- Enabled Claude bundle plugins can also contribute embedded Zero to Agent defaults from `settings.json`; Zero to Agent applies those as sanitized agent settings, not as raw Zero to Agent config patches.
