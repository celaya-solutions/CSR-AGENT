...
OpenAgent can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the Gateway dashboard and WebSocket port. This keeps the gateway bound to loopback while Tailscale provides HTTPS, routing, and (for Serve) identity headers.
...
Status and audit output use **Tailscale exposure** for this OpenAgent Serve/Funnel mode. `off` means OpenAgent is not managing Serve or Funnel. It does not mean the local Tailscale daemon is stopped or logged out.
...
OpenAgent recognizes both paths on its dedicated listener and still requires the configured
Funnel password.
...
openagent gateway --tailscale funnel --auth password
```
...
When token mode has no credential, loopback startup generates a runtime-only token without writing it to config. A separate `openagent qr` command still needs usable configured or supplied token/password authentication. That startup-only token does not configure pairing. See [QR auth resolution](/cli/qr).

### Tailscale identity headers (Serve only)

When `tailscale.mode: "serve"` and `gateway.auth.allowTailscale` is `true`, Control UI/WebSocket auth can use Tailscale identity headers (`tailscale-user-login`) instead of a token/password. OpenAgent verifies the header by resolving the request's `x-forwarded-for` address via the local Tailscale daemon (`tailscale whois`) and matching it to the header login before accepting it. A request only qualifies when it reaches OpenAgent's dedicated managed-Tailscale listener with Tailscale's `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host` headers. Those headers never establish managed Serve provenance or tokenless auth on the ordinary Gateway listener.
...
- Named Tailscale Services are not supported by managed ingress because Tailscale requires them to run as persistent background routes. Existing `gateway.tailscale.serviceName` installs must run `openagent doctor --fix`. Doctor disables managed ingress and removes the key. Inspect the retained Service route, clear it with `tailscale serve clear <service-name>`, then enable device Serve with `gateway.tailscale.mode: "serve"` if desired.
- Older releases could advertise an externally configured default HTTPS Serve route that targeted a `gateway.bind: "lan"` listener. That route does not automatically gain trusted ingress provenance. Run `openagent doctor` to inspect it. Doctor leaves the configuration unchanged, because it cannot prove who owns the route. The route may belong to the current Tailscale hostname and be stale from an older OpenAgent release. If you confirm that, remove only its root handler with `tailscale serve --yes --https=443 --set-path=/ off` or `tailscale funnel --yes --https=443 --set-path=/ off`. Then configure `gateway.bind: "loopback"` plus `gateway.tailscale.mode: "serve"` manually, and restart the Gateway. If another service must retain ownership, leave managed Tailscale ingress off and use the explicit `trustedProxies` compatibility path above.
- `gateway.tailscale.preserveFunnel: true` is a deprecated migration guard. It detects an externally configured `tailscale funnel` route before reapplying Serve. If that route still targets the ordinary Gateway listener, OpenAgent leaves it unchanged and warns because the route is not managed ingress. Gateway-authenticated routes work only through the explicit `trustedProxies` compatibility path above and continue to require the configured auth. Plugin-authenticated webhook routes such as Google Chat and SMS keep using their own signature and auth checks. To migrate, first configure a durable `gateway.auth.password` (prefer a SecretRef) or `OPENCLAW_GATEWAY_PASSWORD`. Set `gateway.auth.mode` to `password`. Run `openagent config set gateway.tailscale.mode funnel`. Then run `openagent config unset gateway.tailscale.preserveFunnel`.
...
- OpenAgent-managed Serve/Funnel proxy to a dedicated `127.0.0.1:<ephemeral-port>` listener while ordinary local clients keep the configured Gateway port. Startup fails closed rather than sharing listener provenance, and the foreground claim releases the route when its Gateway owner disappears.
...
and keep OpenAgent managed ingress off until you resolve the conflict.
