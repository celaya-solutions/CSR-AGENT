...
- `commands.callbackUrl` must resolve to the Zero to Agent gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, Zero to Agent rejects callbacks with
