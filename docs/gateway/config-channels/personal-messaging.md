...
- For a self-hosted Bot API server in `--local` mode, `trustedLocalFileRoots` lists host paths Zero to Agent may read. Mount the server data volume on the Zero to Agent host and configure either its data root or per-token directory; container paths under `/var/lib/telegram-bot-api` are mapped into those roots. Other absolute paths remain rejected.
