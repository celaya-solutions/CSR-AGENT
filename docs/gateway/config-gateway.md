...
- `tailscale.mode`: `serve` (tailnet only, loopback bind) or `funnel` (public, requires auth).
  Zero to Agent holds the route as a foreground claim, so startup fails unless the
...
- `tailscale.preserveFunnel`: deprecated migration guard. When `true` and
  `tailscale.mode = "serve"`, Zero to Agent checks `tailscale funnel status` before
  ...
  provenance. Zero to Agent leaves the external route unchanged and warns. The
...
- `terminal.shell`: optional shell executable. When unset, Zero to Agent uses `$SHELL` on Unix and `%ComSpec%` on Windows. Changes hot-apply to newly opened terminals; existing terminals keep running their original shell.
