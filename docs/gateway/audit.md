...
The Gateway keeps a bounded, metadata-only audit ledger in the shared OpenAgent
...
After session work admission succeeds, OpenAgent validates and freezes
...
action is unknown with `node.action_callback` missing; OpenAgent does not infer a
...
lacks support is unsupported. OpenAgent never reconstructs a participant from
