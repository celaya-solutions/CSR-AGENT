...
The Gateway keeps a bounded, metadata-only audit ledger in the shared Zero to Agent
...
After session work admission succeeds, Zero to Agent validates and freezes
...
candidate execution ids and require exact selection; Zero to Agent never chooses the
...
action is unknown with `node.action_callback` missing; Zero to Agent does not infer a
...
lacks support is unsupported. Zero to Agent never reconstructs a participant from
