...
During startup or restart, the Gateway waits up to five minutes for another OpenAgent process to release lifecycle ownership, logging when it starts waiting and when it acquires ownership or times out.
...
- If the port is occupied by a different, non-gateway process, the error is the same; free the port or choose another with `openagent gateway --port <port>`.
