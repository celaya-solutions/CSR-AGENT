...
For a role-required sandbox, Zero to Agent caps configured `rw` workspace access at
...
<Note>
**Skills**: the `read` tool is sandbox-rooted. With `workspaceAccess: "none"`, Zero to Agent mirrors eligible skills into the sandbox workspace (`.../skills`) as read-only instruction roots; other private workspace files remain writable. With `"rw"`, workspace skills are readable from `/workspace/skills`, and eligible managed, bundled, or plugin skills are materialized into the generated read-only path `/workspace/.openclaw/sandbox-skills/skills`.
