...
team secrets, mutate shared configuration, or grant Zero to Agent write/admin scopes. System
...
`workspaceAccess: "rw"`, Zero to Agent reduces access to `"ro"` for role-required
...
1. For trusted-proxy Control UI connections, `x-openclaw-scopes` first caps
   ...
   scopes.
2. Zero to Agent unions a matching server-side identity grant with those scopes.
3. Zero to Agent applies `x-openclaw-scopes` to the final union as the session cap.
...
4. If the authenticated profile has an effective named operator role,
   Zero to Agent intersects the result with that role's configured scope ceiling.
