...
team secrets, mutate shared configuration, or grant OpenAgent write/admin scopes. System
...
`workspaceAccess: "rw"`, OpenAgent reduces access to `"ro"` for role-required
...
1. For trusted-proxy Control UI connections, `x-openclaw-scopes` first caps
   ...
   scopes.
2. OpenAgent unions a matching server-side identity grant with those scopes.
3. OpenAgent applies `x-openclaw-scopes` to the final union as the session cap.
...
4. If the authenticated profile has an effective named operator role,
   OpenAgent intersects the result with that role's configured scope ceiling.
