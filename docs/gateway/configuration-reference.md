---
summary: "Gateway config reference for core OpenAgent keys, defaults, and links to dedicated subsystem references"
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional; OpenAgent uses safe defaults when omitted.
...
Per-channel config keys live in [Configuration - channels](/gateway/config-channels): `channels.*` for Discord, Telegram, and other channel plugins (auth, access control, multi-account, mention gating).
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full OpenAgent agent run behind Control UI Talk realtime consults
