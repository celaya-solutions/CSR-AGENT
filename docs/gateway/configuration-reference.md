---
summary: "Gateway config reference for core Zero to Agent keys, defaults, and links to dedicated subsystem references"
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional; Zero to Agent uses safe defaults when omitted.
...
- `talk.*` (Talk mode)
  - `talk.consultThinkingLevel`: thinking level override for the full Zero to Agent agent run behind Control UI Talk realtime consults
