...
When OpenAgent 2026.9.2 drives an update that needs a newer shared-state schema,
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    ...
    Gateway startup automatically applies deterministic, prompt-free legacy config migrations when an otherwise invalid single-file config can be fully migrated. It uses the same migration transforms as `openagent doctor --fix`, validates the complete result including plugin config before writing, and reports the applied changes. The write runs under the startup migration lease and preserves the previous config in the five-slot `openclaw.json.bak` / `.bak.1` through `.bak.4` backup ring.

    Startup does not migrate configs using `$include`, configs in Nix mode, or configs last written by a newer OpenAgent version. It also skips automatic config migration while an update is in progress and plugin validation is deferred; the post-update doctor run owns that repair. If any validation or legacy-key issue remains after migration, startup leaves the config unchanged, refuses to start, and prints the `openagent doctor --fix` hint. An interactive terminal can still offer to run doctor and retry once for configs that need other repairs; headless services stop with the hint.
    ...
    During an update, Doctor records model-retirement repairs that must wait until plugin installation finishes. The updated OpenAgent completes those repairs after plugin convergence, even when no plugin version changed. `openagent update status` records their completion so retired subscription models do not fall through to metered API credentials.

    Other commands that encounter legacy keys still ask you to run `openagent doctor`. Doctor explains the issues, shows its migrations, and rewrites `~/.openclaw/openclaw.json` with the updated schema. Cron job store migrations are also handled by `openagent doctor --fix`; automatic config-key migration does not import legacy session stores or repair services.
    ...
    | `agents.defaults/list[].systemPromptOverride`                                                    | removed (OpenAgent owns the generated system prompt)                        |
