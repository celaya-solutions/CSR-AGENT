...
When Zero to Agent 2026.9.2 drives an update that needs a newer shared-state schema,
...
<AccordionGroup>
  ...
  <Accordion title="2. Legacy config key migrations">
    ...
    During an update, Doctor records model-retirement repairs that must wait until plugin installation finishes. The updated Zero to Agent completes those repairs after plugin convergence, even when no plugin version changed. `openclaw update status` records their completion so retired subscription models do not fall through to metered API credentials.
    ...
    | `agents.defaults/list[].systemPromptOverride`                                                    | removed (Zero to Agent owns the generated system prompt)                        |
