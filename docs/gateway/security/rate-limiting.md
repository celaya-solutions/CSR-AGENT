...
proxy is not configured in `gateway.trustedProxies`, OpenAgent cannot safely
...
proxy overwrite or safely rebuild forwarding headers. OpenAgent then restores
...
OpenAgent-managed Tailscale Serve and Funnel use a separate private loopback
...
non-loopback forwarded client address. OpenAgent then uses that client address
...
`gateway.tailscale.mode: "serve"` or `"funnel"` when OpenAgent should own the
