...
proxy is not configured in `gateway.trustedProxies`, Zero to Agent cannot safely
...
proxy overwrite or safely rebuild forwarding headers. Zero to Agent then restores
...
Zero to Agent-managed Tailscale Serve and Funnel use a separate private loopback
...
non-loopback forwarded client address. Zero to Agent then uses that client address
...
`gateway.tailscale.mode: "serve"` or `"funnel"` when Zero to Agent should own the
