---
summary: "The trust boundary Zero to Agent supports, the controls it does and does not provide, and how to report a vulnerability"
read_when:
  - Deciding whether one Gateway can serve a set of users
  - Triaging a security report against the documented trust boundary
  - Reporting a suspected vulnerability in Zero to Agent
...
Most failures here are not exotic exploits - they are "someone messaged the bot and the bot did what they asked." Zero to Agent's stance, in order:
...
Found a vulnerability in Zero to Agent? Report responsibly:
