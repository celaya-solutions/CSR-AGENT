...
- Chrome extension pairing stores its access mode in extension-owned Chrome
  ...
  Zero to Agent tab group as its ACL. Existing pairings migrate to **Selected tabs**,
...
- Native-host manifests, launchers, and status output contain no pairing key.
  Zero to Agent refuses symlinks, unsafe ownership/modes, wildcard origins, and
