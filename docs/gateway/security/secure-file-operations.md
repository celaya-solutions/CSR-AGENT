---
summary: "How OpenAgent handles local file access safely, and why optional fs-safe native acceleration is off by default"
read_when:
...
OpenAgent sets fs-safe's optional native helper to **off** by default:

- the guarded JavaScript paths support OpenAgent's normal filesystem operations.
- disabling native loading keeps runtime behavior deterministic across desktop, Docker, CI, and bundled-app environments.

fs-safe publishes prebuilt native helpers as optional platform packages for Linux x64/arm64 (glibc and musl), macOS x64/arm64, and Windows x64. A normal package install selects the matching package without a compiler. OpenAgent loads it through fs-safe's own dependency scope, including nested pnpm installs. Installs that omit optional dependencies retain the guarded JavaScript path. The `require` mode fails when the binding is unavailable.

OpenAgent only changes the _default_. An explicit setting always wins:

```bash
# Default OpenAgent behavior: guarded JavaScript fs-safe paths.
...
With the helper off, OpenAgent still gets fs-safe's Node-only guardrails:
...
This covers OpenAgent's normal threat model: trusted gateway code handling untrusted model/plugin/channel path input inside a single trusted operator boundary.
...
- Core code should use the fs-safe wrappers under `src/infra/*` so OpenAgent's process policy applies consistently.
- Archive extraction should use the fs-safe archive helpers with explicit size, entry-count, link, and destination limits.
- Secrets should use OpenAgent secret helpers or fs-safe secret/private-state helpers. Do not hand-roll mode checks around `fs.writeFile`.
