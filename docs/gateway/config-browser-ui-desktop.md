...
- Host-local opens with a stable native CDP target and browser identity are
  ...
  post-restart activity. OpenAgent verifies the profile and browser instance
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want OpenAgent to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise OpenAgent treats the loopback port as a
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  mode OpenAgent passes the endpoint to Chrome MCP instead of using auto-connect;
...
- Local managed `openclaw` profiles get a `cdpPort` allocated from the managed
  range when OpenAgent creates the profile. A profile you declare by hand must
...
OpenAgent connects only through loopback. An explicit `port` always selects
