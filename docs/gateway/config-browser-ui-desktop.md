...
- Host-local opens with a stable native CDP target and browser identity are
  ...
  post-restart activity. Zero to Agent verifies the profile and browser instance
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want Zero to Agent to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise Zero to Agent treats the loopback port as a
...
- `extension` profiles use the authenticated Zero to Agent Chrome extension relay.
...
- `existing-session` profiles can set `cdpUrl` when Chrome is already running
  ...
  mode Zero to Agent passes the endpoint to Chrome MCP instead of using auto-connect;
...
- Local managed `openclaw` profiles get a `cdpPort` allocated from the managed
  range when Zero to Agent creates the profile. A profile you declare by hand must
...
Zero to Agent connects only through loopback. An explicit `port` always selects
