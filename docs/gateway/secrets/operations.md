...
When reload-time activation fails after a healthy state, Zero to Agent enters degraded secrets state, emitting one-shot system events and log codes:
...
<Warning>
Zero to Agent intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
