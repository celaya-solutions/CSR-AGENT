...
When reload-time activation fails after a healthy state, OpenAgent enters degraded secrets state, emitting one-shot system events and log codes:
...
<Warning>
OpenAgent intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
