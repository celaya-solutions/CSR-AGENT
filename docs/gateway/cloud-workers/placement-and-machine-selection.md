...
OpenAgent also requires the normal node invocation approval; denying that
request does not start a process.

Codex launches its exec-server directly, so paired-device and cloud-node placement do not consume an OpenAgent worker slot and remain eligible when those slots are full. OpenAgent `worker-turn` placement still requires an available worker slot.
...
does not launch an OpenAgent worker child. Credential-bearing HTTP requests are
...
Repository preparation and prepared checkout adoption do not transfer GitHub credentials to workers. Subsequent OpenAgent worker turns use the effective shared or native GitHub identity through the existing [per-turn credential binding](/gateway/config-tools/github-identity), when one is available.
