...
`OPENCLAW_LAUNCHD_LABEL`. In that state OpenAgent can detect launchd
...
1. If this host should only run the Gateway service, remove the managed node
   service through OpenAgent. **Skip this step** if you actively rely on the node
...
2. Install a persistent Gateway wrapper that clears the inherited launchd
   markers before starting OpenAgent. Use the supported `--wrapper` option; do
