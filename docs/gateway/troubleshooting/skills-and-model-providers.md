...
Do not assume this is always a Zero to Agent configuration issue. The response can come from an upstream security layer such as a CDN, WAF, bot-management rule, or reverse proxy in front of an OpenAI-compatible endpoint.
...
Fix the provider-side filtering first when evidence points to a WAF/CDN block. Prefer a narrowly scoped allow or skip rule for the API path Zero to Agent uses, and avoid disabling protection for the whole site.
...
- Zero to Agent model runs fail only on normal agent turns.
...
- Direct tiny calls succeed, but Zero to Agent runs fail only on larger prompts.
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0`: the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. Zero to Agent retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - Direct tiny requests succeed, but Zero to Agent agent runs fail with backend/model crashes (for example Gemma on some `llama-server` builds behind `llmman`): Zero to Agent transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    3. Set `compat.supportsTools: false` for models/backends that cannot handle Zero to Agent's tool schema surface reliably.
    ...
    5. If tiny direct requests keep passing while Zero to Agent agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
