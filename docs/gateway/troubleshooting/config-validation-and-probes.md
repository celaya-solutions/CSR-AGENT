...
- OpenAgent keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or an OpenAgent-owned write.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an OpenAgent-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → an OpenAgent-owned write was rejected because it lost fields or size compared with the last-known-good backup.
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or OpenAgent could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
