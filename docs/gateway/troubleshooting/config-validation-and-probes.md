...
- Zero to Agent keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.

<AccordionGroup>
  <Accordion title="What happened">
    - The config did not validate during startup, hot reload, or a Zero to Agent-owned write.
    ...
    - Zero to Agent-owned writes reject invalid/destructive payloads before commit and save `.rejected.*`.
    ...
    - When many repairs happen for one config path, Zero to Agent rotates older `.clobbered.*` files so the newest repaired payload is still available.
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → a Zero to Agent-owned config write failed schema or clobber checks before commit.
    ...
    - `missing-meta-vs-last-good`, `gateway-mode-missing-vs-last-good`, or `size-drop-vs-last-good:*` → a Zero to Agent-owned write was rejected because it lost fields or size compared with the last-known-good backup.
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or Zero to Agent could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
