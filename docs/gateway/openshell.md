---
summary: "Use OpenShell as a managed sandbox backend for Zero to Agent agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend: Zero to Agent delegates sandbox lifecycle to
...
Zero to Agent Gateway, which continues to run the agent and host-side plugins.
...
- `openshell` CLI installed and on the Zero to Agent Gateway process's `PATH` (or a
...
- A supported compute runtime on the OpenShell gateway host when using local
  sandboxes
- Zero to Agent Gateway running on the host
...
Before configuring Zero to Agent, verify the OpenShell CLI as the same operating
system user that runs the Zero to Agent Gateway:
...
The Zero to Agent Gateway service must see the same OpenShell CLI, gateway
...
Validate the configuration and restart the Zero to Agent Gateway:
...
On the next agent turn Zero to Agent creates an OpenShell sandbox and routes tool
...
- Before `exec`, Zero to Agent syncs the local workspace into the sandbox.
- After `exec`, Zero to Agent syncs the remote workspace back to local.
- Within one Zero to Agent Gateway process, commands and file-tool operations sharing
...
Best for development workflows: local edits outside Zero to Agent show up on the
...
- On first use after sandbox creation, Zero to Agent seeds the remote workspace
...
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. Zero to Agent does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside Zero to Agent after the initial seed is invisible to the remote sandbox. Run `openclaw sandbox recreate` to re-seed.
</Warning>
...
| `policy`                  | `string`                 | unset         | Path to a sandbox policy YAML file on the Zero to Agent Gateway host                   |
...
rejected. Choose distinct, non-overlapping directories because Zero to Agent manages
...
It cannot select different OpenShell workspaces per Zero to Agent agent or session.
Changing it does not migrate existing sandboxes. Delete Zero to Agent's OpenShell
...
Zero to Agent keeps a registered sandbox's shipped legacy runtime name after an
...
upgrade. Zero to Agent preserves the registered runtime identity, refuses to create
...
If OpenShell cannot delete a sandbox, Zero to Agent reports the failure and keeps the
...
the Zero to Agent Gateway host instead of being copied into an untrusted sandbox.
...
Zero to Agent does not apply Docker image, root-filesystem, network, user, or package
settings to this backend.

Custom images used with the Zero to Agent filesystem bridge must provide:
...
First separate Zero to Agent Gateway health, plugin activation, and OpenShell
...
- **Plugin missing or backend unavailable:** Install
  ...
  validate the config, and restart the Zero to Agent Gateway. Run
...
- **Workspace missing or the wrong sandbox list:** Verify the selected
  ...
  enabling them in Zero to Agent. Remember that the Gateway service may not inherit
...
- **Mirror synchronization reports recovery paths:** Zero to Agent kept a host shadow
  ...
  Zero to Agent preserves remaining workspace entries instead of overwriting them
...
- **Recreate or prune cannot delete a sandbox:** Restore access to the original
  ...
  the scoped recreation. Zero to Agent retains the registry entry until deletion
  succeeds.

## How it works

1. Zero to Agent runs `sandbox get` for the sandbox name (with the selected
...
2. Zero to Agent runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
