---
summary: "Use OpenShell as a managed sandbox backend for OpenAgent agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend: OpenAgent delegates sandbox lifecycle to
...
OpenAgent Gateway, which continues to run the agent and host-side plugins.
...
- `openshell` CLI installed and on the OpenAgent Gateway process's `PATH` (or a
...
- A supported compute runtime on the OpenShell gateway host when using local
  sandboxes
- OpenAgent Gateway running on the host
...
Before configuring OpenAgent, verify the OpenShell CLI as the same operating
system user that runs the OpenAgent Gateway:
...
The OpenAgent Gateway service must see the same OpenShell CLI, gateway
...
Validate the configuration and restart the OpenAgent Gateway:
...
On the next agent turn OpenAgent creates an OpenShell sandbox and routes tool
...
- Before `exec`, OpenAgent syncs the local workspace into the sandbox.
- After `exec`, OpenAgent syncs the remote workspace back to local.
- Within one OpenAgent Gateway process, commands and file-tool operations sharing
...
Best for development workflows: local edits outside OpenAgent show up on the
...
- On first use after sandbox creation, OpenAgent seeds the remote workspace
...
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. OpenAgent does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside OpenAgent after the initial seed is invisible to the remote sandbox. Run `openclaw sandbox recreate` to re-seed.
</Warning>
...
| `policy`                  | `string`                 | unset         | Path to a sandbox policy YAML file on the OpenAgent Gateway host                   |
...
rejected. Choose distinct, non-overlapping directories because OpenAgent manages
...
It cannot select different OpenShell workspaces per OpenAgent agent or session.
Changing it does not migrate existing sandboxes. Delete OpenAgent's OpenShell
...
OpenAgent keeps a registered sandbox's shipped legacy runtime name after an
...
upgrade. OpenAgent preserves the registered runtime identity, refuses to create
...
If OpenShell cannot delete a sandbox, OpenAgent reports the failure and keeps the
...
the OpenAgent Gateway host instead of being copied into an untrusted sandbox.
...
OpenAgent does not apply Docker image, root-filesystem, network, user, or package
settings to this backend.

Custom images used with the OpenAgent filesystem bridge must provide:
...
First separate OpenAgent Gateway health, plugin activation, and OpenShell
...
- **Plugin missing or backend unavailable:** Install
  ...
  validate the config, and restart the OpenAgent Gateway. Run
...
- **Workspace missing or the wrong sandbox list:** Verify the selected
  ...
  enabling them in OpenAgent. Remember that the Gateway service may not inherit
...
- **Mirror synchronization reports recovery paths:** OpenAgent kept a host shadow
  ...
  OpenAgent preserves remaining workspace entries instead of overwriting them
...
- **Recreate or prune cannot delete a sandbox:** Restore access to the original
  ...
  the scoped recreation. OpenAgent retains the registry entry until deletion
  succeeds.

## How it works

1. OpenAgent runs `sandbox get` for the sandbox name (with the selected
...
2. OpenAgent runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
