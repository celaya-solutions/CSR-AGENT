...
read_when:
  - Building an operator, dashboard, or WebChat client outside the Zero to Agent repository
...
Package versions follow the Zero to Agent release train and are separate from the wire
...
[Zero to Agent changelog](https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md)
before each upgrade.
