---
summary: "Run OpenAgent with hardware-aware local model setup or an existing model server"
read_when:
  - You want OpenAgent to recommend and install a model for your Gateway hardware
...
For custom servers, leave room for the full OpenAgent prompt, tools, history, and model output. A model that loads or answers a short prompt may still fail an agent turn. Test actual tasks before making it your default, and review [local-model security](/gateway/security).
...
- Hidden OpenAgent attribution headers (`originator`, `version`, `User-Agent`) are not injected on custom proxy URLs.
...
- **Bracketed tool text**: some local models emit standalone bracketed tool requests as text, like `[tool_name]` followed by JSON and `[END_TOOL_REQUEST]`. OpenAgent promotes those to real tool calls only when the name exactly matches a registered tool for the turn. Otherwise it stays as hidden, unsupported text.
- **Unstructured tool-call-looking text**: a model can emit JSON, XML, or ReAct-style text that looks like a tool call but was not a structured invocation. OpenAgent then leaves it as text and logs a warning. The warning carries the run id, provider and model, detected pattern, and tool name when available. That is provider/model incompatibility, not a completed tool run.
