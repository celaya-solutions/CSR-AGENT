...
Zero to Agent can build a local diagnostics `.zip` for bug reports: sanitized Gateway
...
2. Zero to Agent sends a preamble and asks for one explicit exec approval, which runs
   `openclaw gateway diagnostics export --json`. Do not approve diagnostics via
   an allow-all rule.
3. After approval, Zero to Agent replies with the local bundle path, manifest
   summary, privacy notes, and relevant session ids.

In group chats, an owner can still run `/diagnostics`, but Zero to Agent sends the
...
approval also covers an OpenAI feedback upload for the Codex threads Zero to Agent
...
approval, the reply lists channels, Zero to Agent session ids, Codex thread ids, and
