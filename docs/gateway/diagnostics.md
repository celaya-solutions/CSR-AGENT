...
OpenAgent can build a local diagnostics `.zip` for bug reports: sanitized Gateway
...
In group chats, an owner can still run `/diagnostics`, but OpenAgent sends the
...
approval also covers an OpenAI feedback upload for the Codex threads OpenAgent
...
approval, the reply lists channels, OpenAgent session ids, Codex thread ids, and
