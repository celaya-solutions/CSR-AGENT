...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Zero to Agent
...
External apps talk to Zero to Agent through the Gateway protocol: WebSocket
...
<Note>
  ...
  [Embedding Zero to Agent](/gateway/embedding).
</Note>

<Note>
  This page is for code outside the Zero to Agent process. Plugin code that runs
  inside Zero to Agent should use documented `openclaw/plugin-sdk/*` subpaths instead.
</Note>
...
4. Pin the Zero to Agent version you test against.
5. Recheck the RPC reference when upgrading Zero to Agent.
...
fail-closed, and the host must not freeze or snapshot it. Zero to Agent retries the
...
Without `drain: true`, preparation remains refuse-only: Zero to Agent closes new
...
<Tip>
  For host wake scheduling, keep the Zero to Agent-facing part in an in-process
...
Use Gateway RPC when code lives outside Zero to Agent:
...
Use the Plugin SDK when code runs inside Zero to Agent:
...
plugins loaded by Zero to Agent.
