...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to OpenAgent
...
External apps talk to OpenAgent through the Gateway protocol: WebSocket
...
<Note>
  ...
  [Embedding OpenAgent](/gateway/embedding).
</Note>

<Note>
  This page is for code outside the OpenAgent process. Plugin code that runs
  inside OpenAgent should use documented `openclaw/plugin-sdk/*` subpaths instead.
</Note>
...
4. Pin the OpenAgent version you test against.
5. Recheck the RPC reference when upgrading OpenAgent.
...
fail-closed, and the host must not freeze or snapshot it. OpenAgent retries the
...
Without `drain: true`, preparation remains refuse-only: OpenAgent closes new
...
<Tip>
  For host wake scheduling, keep the OpenAgent-facing part in an in-process
...
Use Gateway RPC when code lives outside OpenAgent:
...
Use the Plugin SDK when code runs inside OpenAgent:
