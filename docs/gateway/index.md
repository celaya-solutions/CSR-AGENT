...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and a `Capability` line that matches what you expect. Use `openagent gateway status --require-rpc` for read-scope RPC proof, not just reachability.
...
OpenAgent's highest-leverage compatibility surface:
...
Installed gateway services record the resolved `--port` in supervisor metadata. After changing `gateway.port`, run `openagent doctor --fix` or `openagent gateway install --force` so launchd/systemd/schtasks starts the process on the new port.
...
The earlier `hot` and `restart` modes were retired in `v2026.7.2-beta.4`, stable from `v2026.8.1`. [`openagent doctor --fix`](/cli/doctor) maps both to `hybrid`.

## Operator command set

```bash
openagent gateway status
openagent gateway status --deep   # adds a system-level service scan
...
- `gateway probe` can warn about `multiple reachable gateway identities` when distinct gateways answer, or when OpenAgent cannot prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
...
```bash
OPENCLAW_CONFIG_PATH=~/.openclaw/a.json OPENCLAW_STATE_DIR=~/.openclaw-a openagent gateway --port 19001
OPENCLAW_CONFIG_PATH=~/.openclaw/b.json OPENCLAW_STATE_DIR=~/.openclaw-b openagent gateway --port 19002
```
...
Use `openagent gateway restart` for restarts. Do not chain `openagent gateway stop` and `openagent gateway start` as a restart substitute.

On macOS, `gateway stop` uses `launchctl bootout` by default. This removes the LaunchAgent from the current boot session without persisting a disable, so KeepAlive auto-recovery still works after unexpected crashes and `gateway start` re-enables cleanly. To persistently suppress auto-respawn across reboots, pass `--disable`: `openagent gateway stop --disable`.

LaunchAgent labels are `ai.openclaw.gateway` (default) or `ai.openclaw.<profile>` (named profile). `openagent doctor` audits and repairs service config drift.

### Existing system LaunchDaemons

OpenAgent installs and manages a per-user LaunchAgent. It does not install or manage system LaunchDaemons. If a custom LaunchDaemon already uses the same gateway label, OpenAgent refuses to write, start, restart, or repair a user LaunchAgent because two `KeepAlive` managers can repeatedly restart the same gateway.

The ownership check reads `launchctl print system/<label>` and also checks installed plists under `/Library/LaunchDaemons`. It fails closed when system ownership cannot be verified, and `--force` does not bypass it. `openagent gateway status` reports a loaded same-label system job; add `--deep` to scan installed system service files.
...
- To return to the supported user LaunchAgent, unload the system job with `sudo launchctl bootout system/<label>`, remove or relocate its actual plist, sign in to the macOS desktop as the target user, then run `openagent gateway install`.
...
Native Windows managed startup uses a Scheduled Task named `OpenAgent Gateway`
(or `OpenAgent Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, OpenAgent falls back to a per-user Startup-folder launcher
...
Replace `<user>` with the non-root account that owns the OpenAgent state and
...
By default, `User=` also supplies that account's `HOME`, which OpenAgent uses
...
Do not also let `openagent doctor --fix` install a user-level gateway service for the same profile/port. Doctor refuses that automatic install when it finds a system-level OpenAgent gateway service; use `OPENCLAW_SERVICE_REPAIR_POLICY=external` when the system unit owns the lifecycle.
...
```bash
openagent --dev setup
openagent --dev gateway --allow-unconfigured
openagent --dev status
```
