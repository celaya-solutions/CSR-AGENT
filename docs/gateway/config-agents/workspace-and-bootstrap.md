...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenAgent auto-detects by walking upward from the workspace.
...
When bootstrap context is truncated, OpenAgent always injects a concise
...
OpenAgent has multiple high-volume prompt/context budgets, and they are
...
- When `memory_get` omits `lines`, OpenAgent uses a built-in 120-line window and
...
OpenAgent adapts the resize ladder to the selected image model. For example, Claude Opus 4.8, OpenAI GPT-6 Astra, Qwen VL, and hosted Llama 4 vision models can use larger images than older/default high-detail vision paths, while multi-image turns are compressed more aggressively in `auto` mode to control token and latency cost.
