...
Optional repository root shown in the system prompt's Runtime line. If unset, Zero to Agent auto-detects by walking upward from the workspace.
...
When bootstrap context is truncated, Zero to Agent always injects a concise
...
Zero to Agent has multiple high-volume prompt/context budgets, and they are
...
- When `memory_get` omits `lines`, Zero to Agent uses a built-in 120-line window and
...
Zero to Agent adapts the resize ladder to the selected image model. For example, Claude Opus 4.8, OpenAI GPT-6 Astra, Qwen VL, and hosted Llama 4 vision models can use larger images than older/default high-detail vision paths, while multi-image turns are compressed more aggressively in `auto` mode to control token and latency cost.
