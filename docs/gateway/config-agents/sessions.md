...
- **`maintenance`**: session-store cleanup + retention controls.
  ...
  - Legacy `rotateBytes` is rejected by the current schema; `openagent doctor --fix` removes it from older configs.
...
Cold archives hold authoritative history. Supported OpenAgent backup commands
