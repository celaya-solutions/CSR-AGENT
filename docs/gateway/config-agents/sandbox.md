...
Defaults shown above (`off`/`docker`/`agent`/`none`/`bookworm-slim` image/`none` network/etc.) are the actual OpenAgent defaults, not just illustrative values.
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that OpenAgent materializes into temp files at runtime
...
Codex app-server turns in an active OpenAgent sandbox use this same egress setting for their native code-mode network access.
