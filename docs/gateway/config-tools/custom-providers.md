...
Configuring a custom/local provider `baseUrl` is also the narrow network trust decision for model HTTP requests: Zero to Agent allows that exact `scheme://host:port` origin through the guarded fetch path, without adding a separate config option or trusting other private origins.
...
<AccordionGroup>
  ...
  <Accordion title="Model catalog entries">
    ...
    Provider catalogs own `compat` for bundled and catalog-known model routes. Do not copy those flags into config: Zero to Agent uses the catalog row when the configured `api` and `baseUrl` still identify that route. `openclaw doctor --fix` removes matching legacy overrides and reports divergent values for review.
    ...
    | `reasoningEffortMap` | Maps Zero to Agent thinking labels to endpoint-specific labels. |
