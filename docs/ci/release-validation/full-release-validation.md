...
`OpenAgent Release Publish` is the manual mutating release workflow. Dispatch
...
release tag exists and after the OpenAgent npm preflight has succeeded (the preflight runs
...
Historical recovery may still supply a separate successful `OpenAgent NPM Release`
...
release SHA, then dispatches `OpenAgent NPM Release` after plugin npm succeeds.
...
`OpenAgent Release Checks` uses the trusted workflow ref to resolve the selected ref once into a `release-package-under-test` tarball, then passes that artifact to cross-OS checks and Package Acceptance, plus the live/E2E release-path Docker workflow when soak coverage runs. That keeps the package bytes consistent across release boxes and avoids repacking the same candidate in multiple child jobs. For the Codex npm-plugin live lane, release checks either pass a matching published plugin spec derived from `release_package_spec`, pass the operator-supplied `codex_plugin_spec`, or leave the input blank so the Docker script packs the selected checkout's Codex plugin.
