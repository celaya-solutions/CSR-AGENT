...
OpenAgent exposes four user-facing update channels:
...
`OpenAgent Release Publish`. Its Docker-only extended-stable path rechecks the
...
The Docker box lives in `OpenAgent Release Checks` through `openclaw-live-and-e2e-checks-reusable.yml`, plus the release-mode `install-smoke` workflow. It validates the release candidate through packaged Docker environments instead of only source-level tests.
...
The QA Lab box is also part of `OpenAgent Release Checks`. It is the agentic behavior and channel-level release gate, separate from Vitest and Docker package mechanics.
...
- `source=npm`: `openclaw@beta`, `openclaw@latest`, or an exact OpenAgent release version
...
For a complete regular beta or stable release, use `OpenAgent Release Prepare`
before publication and `OpenAgent Release Button` when ready to publish. Both run
...
**OpenAgent Release Button**, selecting the same protected tooling tag. This is
...
positive numeric child IDs. Start a new **OpenAgent Release Prepare** run on the
...
| Failure                                                                   | Recovery                                                                                                                                                                                      |
| ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Outer readiness seal/download fails after both child preparations succeed | On **OpenAgent Release Prepare**, rerun **Verify and seal prepared publication**. It reuses the child run IDs, resolves their current attempts once, and does not dispatch another build. |
...
| Publisher download fails before writes                                    | Start an explicit new **OpenAgent Release Button** run with the same `prepared_artifact`. No version bump or repack is needed.                                                            |
...
OpenAgent NPM Release child**. If npm contains the core version but that child
...
`OpenAgent Release Publish` remains the protected mutating owner. The monthly
...
6. After plugin npm succeeds, dispatch `OpenAgent NPM Release` with the release tag, npm dist-tag, and saved `preflight_run_id` after verifying the saved `full_release_validation_run_id` and exact run attempt. ClawHub proceeds in parallel.
...
To attach Windows assets later or retry a failed promotion, use the exact OpenAgent tag, exact published Windows source tag, and approved installer digests:
...
release tag plus the parent `OpenAgent Release Publish` run id, attempt, and
...
`OpenAgent NPM Release` accepts these operator-controlled inputs:
...
- `release_publish_run_id`: approved `OpenAgent Release Publish` run id; required when this workflow is dispatched by that parent (bot-actor real-publish calls)
...
`OpenAgent Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `OpenAgent NPM Release` preflight run id; required when `publish_openclaw_npm=true` or `plugin_publish_scope=all-publishable`
...
- `npm_dist_tag`: npm target tag for the OpenAgent package, one of `alpha`, `beta`, `latest`, or `extended-stable`
...
`OpenAgent Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks require the resolved commit to be reachable from an OpenAgent branch or release tag.
...
- For `OpenAgent NPM Release`, full commit SHA input is allowed only when `preflight_only=true`
- `OpenAgent Release Checks` and `Full Release Validation` are always validation-only
...
1. Run `OpenAgent NPM Release` with `preflight_only=true`. Before a tag exists, you may use the current full workflow-branch commit SHA for a validation-only dry run of the preflight workflow.
...
6. Run `OpenAgent Release Publish` from the protected `release-publish/<sha12>-<epoch>` tooling tag with the same `tag`, the same `npm_dist_tag`, the optional Windows input pair, the saved `preflight_run_id`, `full_release_validation_run_id`, and `full_release_validation_run_attempt`. It starts plugin npm and ClawHub in parallel, then promotes the prepared OpenAgent npm package once plugin npm succeeds. GitHub finalization waits for npm and Docker evidence; apps attach independently afterward.
