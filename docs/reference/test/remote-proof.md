...
read_when:
  ...
  - You are running OpenAgent tests on Crabbox or Testbox
---
...
Use the OpenAgent wrapper for proof: direct native Blacksmith commands target the
...
OpenAgent wrapper and workflow inputs. Routine task-needed Crabbox/Testbox use
...
Run trusted OpenAgent remote proof through the wrapper from the repository root:
...
when host caches cannot cross filesystems. The OpenAgent broker login endpoint
