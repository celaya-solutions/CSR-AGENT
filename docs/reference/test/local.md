...
OpenAgent sets `clearMocks: false`, so setup and `beforeAll` calls are preserved.
...
the always-bundled OpenAgent packages. fs-safe remains external so its native loader
