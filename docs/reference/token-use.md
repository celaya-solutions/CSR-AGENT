---
summary: "How OpenAgent builds prompt context and reports token usage + costs"
read_when:
...
OpenAgent tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OpenAgent assembles its own system prompt on every run. It includes:
...
OpenAgent derives the live tool-result cap from the effective model context
...
publish a `1050000` token total window, but OpenAgent defaults their active
...
For images, OpenAgent downscales transcript/tool image payloads before
...
- `/usage cost` -> local cost summary from OpenAgent session logs.
...
OpenAgent falls back to matching OAuth/API-key credentials from auth
...
OpenAgent keeps provider usage accounting separate from the current context
...
Provider prompt caching only applies within the cache TTL window. OpenAgent
...
OpenAgent sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus
...
Older configs can keep `context1m: true`, but OpenAgent no longer sends
...
(`sk-ant-oat-*`), OpenAgent preserves the OAuth-required Anthropic beta
