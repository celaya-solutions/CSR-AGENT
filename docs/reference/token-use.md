---
summary: "How Zero to Agent builds prompt context and reports token usage + costs"
read_when:
...
Zero to Agent tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Zero to Agent assembles its own system prompt on every run. It includes:
...
Zero to Agent derives the live tool-result cap from the effective model context
...
publish a `1050000` token total window, but Zero to Agent defaults their active
...
For images, Zero to Agent downscales transcript/tool image payloads before
...
- `/usage cost` -> local cost summary from Zero to Agent session logs.
...
Zero to Agent falls back to matching OAuth/API-key credentials from auth
...
Zero to Agent keeps provider usage accounting separate from the current context
...
Provider prompt caching only applies within the cache TTL window. Zero to Agent
...
Zero to Agent sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus
...
Older configs can keep `context1m: true`, but Zero to Agent no longer sends
...
(`sk-ant-oat-*`), Zero to Agent preserves the OAuth-required Anthropic beta
