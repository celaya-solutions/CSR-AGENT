...
- Adds an **estimated cost** for the last reply when OpenAgent has usage metadata and local pricing for the active model, including explicitly priced non-API-key providers such as Bedrock `aws-sdk` models.
...
- Gemini CLI note: both `stream-json` and legacy `json` output carry usage under `stats`. OpenAgent normalizes `stats.cached` into `cacheRead` and derives input tokens from `stats.input_tokens - stats.cached` when needed.
...
- MiniMax's raw `usage_percent` / `usagePercent` fields report remaining quota, so OpenAgent inverts them; count-based fields win when present. If the response includes a `model_remains` array, OpenAgent picks the chat-model entry, derives the window label from timestamps when needed, and includes the model name in the plan label.
...
Every reply or tool call runs on the current model provider. This is the primary source of usage and cost, including subscription-style hosted plans that bill outside OpenAgent's local UI: OpenAI Codex, Alibaba Cloud Model Studio Coding Plan, MiniMax Coding Plan, Z.AI/GLM Coding Plan, and Anthropic's Claude-login path with Extra Usage enabled.
