...
A single **Gateway process** owns session state end-to-end. UIs (web Control UI, TUI) query the Gateway for session lists and token counts. In remote mode, the per-agent SQLite database lives on the remote host, so checking your local machine's state will not reflect what the Gateway is using.
...
- **Store vs transcript mismatch?** Confirm the Gateway host and the store path from `openagent status`.
...
- **Every prompt seems to overflow on a small local model?** Confirm the provider reports the correct model context window. OpenAgent can cap the effective reserve only when that window is known.
