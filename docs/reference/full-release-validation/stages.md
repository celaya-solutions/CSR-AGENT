...
For directly dispatched `Zero to Agent NPM Release` preflight-only runs, if qualification
...
upstream registry. Plugin Prerelease and Zero to Agent Release Checks each dispatch an
...
| Product performance     | **Job:** `Run product performance evidence`<br />**Child workflow:** `Zero to Agent Performance`<br />**Proves:** release-profile performance (`profile=release`, `repeat=3`, `publish_reports=false`) against the target SHA. Selected for `all` except `npm-beta-v1`, or explicit `performance`; stable/full regressions block, beta results remain advisory. Selected children still finish and prove their report publisher was skipped.<br />**Rerun:** `rerun_group=performance`.                                                  |
...
`Zero to Agent Performance` permits report publication only for scheduled runs or a
