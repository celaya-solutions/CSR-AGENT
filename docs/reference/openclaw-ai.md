...
read_when:
  - You want to reuse OpenAgent's model transports in another application
...
`@openclaw/ai` is the publishable library form of OpenAgent's model execution
...
without any OpenAgent application code.
...
- **Host policy is injected, not bundled.** Request fetch guarding (for
  ...
  OpenAgent installs its real implementations in its stream facade.
- **One event-stream identity.** `@openclaw/ai/event-stream` is the canonical
  `EventStream` constructor shared by OpenAgent core, agent-core, and external
  consumers.
- **`internal/*` subpaths are not API.** They exist for the OpenAgent
  application itself and carry no semver guarantee.
- Provider ids, credentials, model catalogs, retries, and failover remain
  application concerns. OpenAgent layers those around this package; a library
...
| `./internal/*`   | OpenAgent-internal, no semver guarantee                                    |
