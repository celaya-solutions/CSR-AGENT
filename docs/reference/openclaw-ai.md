...
read_when:
  - You want to reuse Zero to Agent's model transports in another application
...
`@openclaw/ai` is the publishable library form of Zero to Agent's model execution
...
without any Zero to Agent application code.
...
- **Host policy is injected, not bundled.** Request fetch guarding (for
  ...
  Zero to Agent installs its real implementations in its stream facade.
- **One event-stream identity.** `@openclaw/ai/event-stream` is the canonical
  `EventStream` constructor shared by Zero to Agent core, agent-core, and external
  consumers.
- **`internal/*` subpaths are not API.** They exist for the Zero to Agent
  application itself and carry no semver guarantee.
- Provider ids, credentials, model catalogs, retries, and failover remain
  application concerns. Zero to Agent layers those around this package; a library
...
| `./internal/*`   | Zero to Agent-internal, no semver guarantee                                    |
