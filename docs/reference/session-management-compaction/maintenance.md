...
Zero to Agent no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The current schema rejects the legacy `session.maintenance.rotateBytes` key, and `openclaw doctor --fix` removes it from older configs.
...
or restoring that transcript fails explicitly. Zero to Agent does not substitute an
...
Stop the Gateway and back up its state. Using the current SQLite-capable Zero to Agent
...
inspection and validation sequence again so Zero to Agent can verify restored legacy
