...
On macOS, the owner's avatar defaults to the Gateway host account's user picture when no Zero to Agent avatar is saved. This uses the Mac running the Gateway, including when you connect from another device. A saved avatar always takes priority. Zero to Agent reads the picture locally and serves a resized copy through the authenticated avatar route; other people's profiles never inherit it. Restart the Gateway to pick up a changed macOS picture. If the picture cannot be read, initials remain visible.
...
The **Discover** tab is the store: featured plugins included with Zero to Agent,
...
Zero to Agent but disabled by default, so its action is **Enable**. Bundled plugins
...
Zero to Agent companion surface: the [iOS](/platforms/ios) and
...
Inside **Settings**, the dedicated sidebar includes **Ask Zero to Agent** and starts with a **Search settings** field for quickly finding settings sections.
...
tools, Read Aloud, and find previous, and Zero to Agent's existing debug-overlay shortcut.
...
the Zero to Agent app; ordinary browsers keep the Gateway settings. Talk trigger words
...
into a Zero to Agent agent. The Gateway discovers supported local memory on its own
...
For a narrower conversational path, open **Settings → Ask Zero to Agent** and say
...
Zero to Agent backup when state exists, writes a redacted migration report, and keeps
...
The dedicated MCP page is an operator view for Zero to Agent-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `openclaw mcp doctor --probe` when you need live server proof.
