...
OpenAgent Chat. Choose the machine and folder, optionally enter a first prompt,
...
machine's native account, model, and configuration; OpenAgent does not translate
model or authentication settings or automatically adopt the native session.
Native draft text is not sent to OpenAgent for automatic title preparation.
...
No matching OpenAgent model route is required. Each machine has one launch
...
and resuming existing sessions with their original profile. An OpenAgent agent's
...
OpenAgent worker placement, cloud/Auto selection, attachment submission, model
...
OpenAgent never starts a Chat or substitutes another host or home directory.
