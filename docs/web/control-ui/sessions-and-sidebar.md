...
it resembles an Android device label such as `Zero to Agent App · Phone · abc123`.
...
Zero to Agent Chat. Choose the machine and folder, optionally enter a first prompt,
...
machine's native account, model, and configuration; Zero to Agent does not translate
model or authentication settings or automatically adopt the native session.
Native draft text is not sent to Zero to Agent for automatic title preparation.
...
No matching Zero to Agent model route is required. Each machine has one launch
...
and resuming existing sessions with their original profile. A Zero to Agent agent's
...
Zero to Agent worker placement, cloud/Auto selection, attachment submission, model
...
Zero to Agent never starts a Chat or substitutes another host or home directory.
