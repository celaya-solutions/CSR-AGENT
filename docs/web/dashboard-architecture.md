...
The **widget is the Zero to Agent primitive**: the named, pinned, sized,
...
Zero to Agent's — so `show_widget` code stays as short as it is today and never
...
since browser cookies share a hostname boundary. Zero to Agent injects no Gateway token,
...
plugin registration, Zero to Agent verifies that every binding targets an RPC
...
**Accepted tradeoff:** Zero to Agent does not gate scriptable widgets on this
residual. Widget content gains access to sensitive Zero to Agent data only through
