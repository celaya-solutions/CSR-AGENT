...
The **widget is the OpenAgent primitive**: the named, pinned, sized,
...
OpenAgent's — so `show_widget` code stays as short as it is today and never
...
since browser cookies share a hostname boundary. OpenAgent injects no Gateway token,
...
**Accepted tradeoff:** OpenAgent does not gate scriptable widgets on this
residual. Widget content gains access to sensitive OpenAgent data only through
