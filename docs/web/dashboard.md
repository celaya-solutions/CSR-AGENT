...
- Re-open or repair a browser anytime: `openagent dashboard`. It copies/opens a single-use pairing link
...
- If clipboard and browser delivery both fail, `openagent dashboard` either gives a safe manual-token
  hint or tells you to run `openagent dashboard --json` and open its short-lived `browserUrl`; it never
...
- **Host-authorized browser handoff**: `openagent dashboard` issues a short-lived, single-use bootstrap
...
- **Missing-config runtime token**: if startup says it generated a runtime token, that token is ephemeral and cannot be recovered. Loopback still requires auth. Run `openagent doctor --generate-gateway-token`, restart the Gateway, then run `openagent gateway auth-token --show` in an interactive terminal and paste the output into Control UI settings.
...
This endpoint belongs to the deployment's authenticated proxy or handoff service. OpenAgent
does not expose an unauthenticated credential issuer. The service must independently verify
the browser's identity and authorization before using the host's `openagent dashboard --json`
...
- Confirm the gateway is reachable: local `openagent status`; remote, SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@gateway-host` then open `http://127.0.0.1:18789/`.
...
- For shared-secret authentication, retrieve or supply the configured secret from the gateway host:
  - Token: run `openagent gateway auth-token --show` in an interactive terminal on the Gateway host
  - Password: resolve the configured `gateway.auth.password` or `OPENCLAW_GATEWAY_PASSWORD`
  - SecretRef-managed token: run `openagent gateway auth-token --show`; if resolution fails, repair the external secret provider and rerun it
  - Runtime token generated because no shared secret was configured: run `openagent doctor --generate-gateway-token`, restart the Gateway, then use the configured token
...
- [WebChat](/web/webchat)
- [`openagent dashboard`](/cli/dashboard) — securely open the Control UI from the CLI
