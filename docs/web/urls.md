...
URL-encode each query value. The agent in the path owns the OpenAgent pane,
...
thread navigates to the adopted OpenAgent session link. The same catalog query
also works under `/dashboard/<agentId>`.

## Social previews

Use **Copy → Preview link** in a session's menu to share a link with an OpenAgent
...
The card shows generic OpenAgent branding, not the session's title, messages,
...
This is an operator deployment step; OpenAgent does not change proxy policies.
...
OpenAgent backup preserves both the installation identity and agent session
...
card must both be reachable without cookies. OpenAgent
