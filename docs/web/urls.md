...
URL-encode each query value. The agent in the path owns the Zero to Agent pane,
...
thread navigates to the adopted Zero to Agent session link. The same catalog query
also works under `/dashboard/<agentId>`.

## Social previews

Use **Copy → Preview link** in a session's menu to share a link with a Zero to Agent
...
The card shows generic Zero to Agent branding, not the session's title, messages,
...
This is an operator deployment step; Zero to Agent does not change proxy policies.
...
Zero to Agent backup preserves both the installation identity and agent session
...
card must both be reachable without cookies. Zero to Agent
...
| Ask Zero to Agent   | `/custodian`                                   | -                         | `?intent=new-agent`, `?onboarding=1`                                              |
