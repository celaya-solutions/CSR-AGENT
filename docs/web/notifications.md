...
Zero to Agent can ping you when something needs your attention, including an exec or plugin approval request. The ping arrives in the browser that runs the Control UI. It can also arrive through native macOS notifications when you use the Zero to Agent macOS app. Your first chat send may request permission automatically. **Settings → Notifications** remains the place to enable or repair the current device. Use the same page to check its status and send yourself a test.
...
| Zero to Agent macOS app                           | Native macOS notifications                         | Grant app permission, jump to System Settings when blocked, send a local test                           |
...
- **This browser or app** can mute one browser profile or installed Home Screen app. It can also add a source label or override individual categories. The account defaults stay unchanged. Native Zero to Agent app notifications are configured separately.
...
The Gateway sends Web Push directly to the browser vendor's push service. This works with a self-hosted Gateway and does not use the Zero to Agent-hosted iOS relay.
...
The browser Push API permits only one application-server key per service-worker registration. If a PWA subscription belongs to a different VAPID key, Zero to Agent removes the unusable row from the current Gateway. Zero to Agent also shows **Unavailable** and **Not subscribed**, with an error explaining the mismatch. To switch that PWA scope to the current Gateway, select **Unsubscribe**, then **Enable notifications** and **Send test**. Unsubscribing deactivates the shared browser subscription for every Gateway registered through that scope. After re-enabling, reconnect to each Gateway once.
...
1. Open **Settings → Notifications** in the Zero to Agent macOS app.
2. Select **Enable notifications** while the permission shows **Not requested**.
3. Approve the macOS permission prompt.
4. Select **Send test** to post a local Zero to Agent notification.

If the permission shows **Denied**, macOS will not re-prompt. Select **Open System Settings**. Allow notifications for Zero to Agent there. Then switch back. The page rechecks permission when the app regains focus. This permission belongs to macOS, not to Gateway config.
