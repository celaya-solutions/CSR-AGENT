...
Closed Terminal, Browser, Desktop, and Home/Ask Zero to Agent panels initialize when you open them rather than during initial navigation. Panels saved as open still restore after a reload.
...
those permissions and click the control again to retry. Zero to Agent does not replace
...
If you paste a setup code from **Devices → Pair device → Copy setup code** into **Gateway secret**, the UI shows an inline hint before you connect. Paste that code into **Settings → Gateway** in the Zero to Agent mobile app. For the Control UI, run `openclaw gateway auth-token --show` in an interactive terminal on the Gateway host and paste the shared token instead. If a connection with a setup code is rejected for a token or password mismatch, the login screen repeats this guidance.
...
name, or **Zero to Agent**, and the Zero to Agent mark. Its menu contains **Show one agent**,
...
- [Panels and docks](/web/control-ui/panels) — Ask Zero to Agent, the Home dock, the operator terminal, and the browser panel.
...
- <a id="openclaw-system-care" />[Zero to Agent system care](/web/control-ui/panels#openclaw-system-care)
...
- <a id="openclaw-chat-workspace-startup" />[Zero to Agent Chat workspace startup](/web/control-ui/sessions-and-sidebar#openclaw-chat-workspace-startup)
