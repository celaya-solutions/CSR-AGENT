...
- `/openclaw [request]` returns from the normal agent TUI to the [OpenAgent](#openclaw-setup-and-repair-helper) setup/repair chat, optionally forwarding one request.
...
- From inside OpenAgent, an `open-tui` request exits OpenAgent and opens the regular agent TUI. One example is asking to talk to a normal agent. Use `/openclaw` there to come back.
