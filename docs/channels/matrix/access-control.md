...
read_when:
  ...
  - Allowing traffic between two OpenAgent Matrix accounts
title: "Matrix access control"
sidebarTitle: "Access control"
---

Who may reach OpenAgent through Matrix, which supplemental context it keeps, and which senders may run commands.

## Bot-to-bot rooms

By default, Matrix messages from other configured OpenAgent Matrix accounts are ignored. Use `allowBots` to intentionally allow inter-agent traffic:
...
- OpenAgent still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix has no native bot flag; OpenAgent treats "bot-authored" as "sent by another configured Matrix account on this OpenAgent gateway".
