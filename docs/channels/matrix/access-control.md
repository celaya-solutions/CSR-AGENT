...
read_when:
  ...
  - Allowing traffic between two Zero to Agent Matrix accounts
title: "Matrix access control"
sidebarTitle: "Access control"
---

Who may reach Zero to Agent through Matrix, which supplemental context it keeps, and which senders may run commands.

## Bot-to-bot rooms

By default, Matrix messages from other configured Zero to Agent Matrix accounts are ignored. Use `allowBots` to intentionally allow inter-agent traffic:
...
- Zero to Agent still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix has no native bot flag; Zero to Agent treats "bot-authored" as "sent by another configured Matrix account on this Zero to Agent gateway".
...
If an unapproved Matrix user keeps messaging before approval, Zero to Agent reuses the same pending pairing code and may send a reminder reply after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Slash commands

Slash commands (`/new`, `/reset`, `/model`, `/agents`, `/session`, `/acp`, `/approve`, etc.) work directly in DMs. In rooms, Zero to Agent also recognizes commands prefixed with the bot's own Matrix mention, so `@bot:server /new` triggers the command path without a custom mention regex - this keeps the bot responsive to the room-style `@mention /command` posts that Element and similar clients emit when a user tab-completes the bot before typing the command.
