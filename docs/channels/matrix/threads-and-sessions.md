...
read_when:
  - Choosing how Matrix DM rooms map to Zero to Agent sessions
  - Binding a Matrix thread to an ACP session
title: "Matrix threads and sessions"
sidebarTitle: "Threads and sessions"
---

How Matrix rooms, DMs, and threads map onto Zero to Agent sessions, and how much room history each turn carries.
...
`dm.sessionScope` decides how Matrix DM rooms map to Zero to Agent sessions:
...
Selecting a reply target inside a thread preserves both the thread and the selected message. Ordinary threaded messages can carry reply metadata for older clients; Zero to Agent does not treat that compatibility fallback as a quoted message in the agent's context.
...
- DM user-target reuse only kicks in when current session metadata proves the same DM peer on the same Matrix account; otherwise Zero to Agent falls back to normal user-scoped routing.
...
When Zero to Agent detects a Matrix DM room colliding with another DM room on the same shared session, it posts a one-time `m.notice` suggesting `dm.sessionScope: "per-room"` to isolate the rooms. The notice only appears when thread bindings are enabled.
...
`--bind here` does not create a child Matrix thread. `threadBindings.spawnSessions` gates `/acp spawn --thread auto|here`, where Zero to Agent needs to create or bind a child thread.
...
- Room history is pending-only: Zero to Agent buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
