...
Structured content OpenAgent attaches to Matrix events, and the approval prompts built on it.
...
the text; OpenAgent-aware clients can render the structured controls. Replies that
...
Matrix native approval prompts are normal `m.room.message` events with OpenAgent-specific content under the `com.openclaw.approval` key. Stock clients still render the text body; OpenAgent-aware clients can read the structured approval id, kind, state, decisions, and exec/plugin details.

When a prompt is too long for one Matrix event, OpenAgent chunks the visible text and attaches `com.openclaw.approval` to the first chunk only. Allow/deny reactions bind to that first event, so long prompts keep the same approval target as single-event prompts.
