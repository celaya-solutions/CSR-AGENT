...
read_when:
  - You want to connect OpenAgent to WeChat or Weixin
  - You are installing or troubleshooting the openclaw-weixin channel plugin
  - You need to understand how external channel plugins run beside the Gateway
title: "WeChat"
---

OpenAgent connects to WeChat through Tencent's external
...
- `openclaw-weixin` is the OpenAgent channel id (`weixin` and `wechat` work as aliases).
...
The WeChat code does not live in the OpenAgent core repo. OpenAgent provides the
...
5. The plugin stores account credentials under the OpenAgent state directory
   (`~/.openclaw` by default).
...
7. Inbound WeChat messages are normalized through the channel contract, routed to
   the selected OpenAgent agent, and sent back through the plugin outbound path.

That separation matters: OpenAgent core stays channel-agnostic. WeChat login,
...
Version `2.4.8` does not register an OpenAgent pairing adapter or create pairing
...
This version reads a legacy account allowlist JSON file instead of OpenAgent's
...
On current OpenAgent, startup migration and `openclaw doctor --fix` import legacy
...
For integrations that implement OpenAgent's pairing API, see [Pairing](/channels/pairing).

## Compatibility

The package declares these OpenAgent requirements:

| Plugin version | Declared OpenAgent requirement | npm tag  |
| -------------- | ---------------------------------- | -------- |
...
If the plugin reports that your OpenAgent version is too old, either update
OpenAgent or install the legacy plugin line:
...
cannot load on OpenAgent 2026.8.1. If startup reports that this subpath is not
...
Tencent iLink API. In [issue #68451](https://github.com/openclaw/openclaw/issues/68451), that helper path exposed a bug in OpenAgent's
...
Current OpenAgent startup cleanup excludes the current process and its ancestors,
...
If the Gateway restarts repeatedly after enabling WeChat, update both OpenAgent and
the plugin:
...
JavaScript runtime files OpenAgent needs. Update/reinstall after the plugin
