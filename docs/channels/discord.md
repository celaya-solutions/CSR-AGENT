...
OpenAgent connects to Discord as a bot over the official Discord gateway. DMs and guild channels are supported.
...
- <a id="configure-openclaw-and-pair" />[Configure OpenAgent and pair](/channels/discord/setup#configure-openclaw-and-pair)
...
- `DISCORD_API_URL` overrides the process-wide, versioned Discord REST base (for example,
  `https://discord.example.com/api/v10`). OpenAgent also uses that origin for Gateway discovery,
