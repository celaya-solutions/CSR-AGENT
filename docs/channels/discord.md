...
Zero to Agent connects to Discord as a bot over the official Discord gateway. DMs and guild channels are supported.
...
- <a id="configure-openclaw-and-pair" />[Configure Zero to Agent and pair](/channels/discord/setup#configure-openclaw-and-pair)
...
- `DISCORD_API_URL` overrides the process-wide, versioned Discord REST base (for example,
  `https://discord.example.com/api/v10`). Zero to Agent also uses that origin for Gateway discovery,
