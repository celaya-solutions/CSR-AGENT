...
The Block Kit surfaces Zero to Agent renders natively in Slack.
...
renders line, bar, area, and pie charts in messages. Zero to Agent maps the portable
...
block. Standard presentation sends to other Zero to Agent channels receive that same
deterministic chart data as text unless they advertise native chart support. If
Slack rejects the chart with `invalid_blocks`, Zero to Agent
...
a presentation contains more than two valid charts, Zero to Agent keeps their order
...
renders structured rows and columns in messages. Zero to Agent maps an explicit
...
Zero to Agent maps header and string cells to Slack `raw_text` cells. Numeric cells
...
If Slack rejects native chart or table blocks with `invalid_blocks`, Zero to Agent
...
Markdown pipe tables remain authored text; Zero to Agent does not guess at table
structure or cell types. Existing trusted Slack-native producers can continue
to pass raw blocks through `channelData.slack.blocks`; Zero to Agent derives fallback
...
block in the message's top-level blocks or attachments. Zero to Agent renders those
...
`view_submission` and `view_closed` lifecycle events before Zero to Agent compacts
...
When those buttons are present, they are the primary approval UX; Zero to Agent
