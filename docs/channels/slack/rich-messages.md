...
The Block Kit surfaces OpenAgent renders natively in Slack.
...
renders line, bar, area, and pie charts in messages. OpenAgent maps the portable
...
block. Standard presentation sends to other OpenAgent channels receive that same
deterministic chart data as text unless they advertise native chart support. If
Slack rejects the chart with `invalid_blocks`, OpenAgent
...
a presentation contains more than two valid charts, OpenAgent keeps their order
...
renders structured rows and columns in messages. OpenAgent maps an explicit
...
OpenAgent maps header and string cells to Slack `raw_text` cells. Numeric cells
...
If Slack rejects native chart or table blocks with `invalid_blocks`, OpenAgent
...
Markdown pipe tables remain authored text; OpenAgent does not guess at table
structure or cell types. Existing trusted Slack-native producers can continue
to pass raw blocks through `channelData.slack.blocks`; OpenAgent derives fallback
...
block in the message's top-level blocks or attachments. OpenAgent renders those
...
`view_submission` and `view_closed` lifecycle events before OpenAgent compacts
...
When those buttons are present, they are the primary approval UX; OpenAgent
