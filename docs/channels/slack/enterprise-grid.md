...
read_when:
  - Installing OpenAgent at the Enterprise Grid organization level
...
OpenAgent. Generate an app-level token with `connections:write` for Socket Mode,
...
At startup, OpenAgent uses Slack `auth.test` to detect whether the token belongs
...
which workspaces have granted the installation; OpenAgent then applies the
...
loop-prevention rules used by workspace installs. OpenAgent retains the org
...
`features.shortcuts` list; OpenAgent accepts their callback IDs through the same
...
OpenAgent records Enterprise Grid destinations as
