...
read_when:
  - Installing Zero to Agent at the Enterprise Grid organization level
...
Zero to Agent. Generate an app-level token with `connections:write` for Socket Mode,
...
At startup, Zero to Agent uses Slack `auth.test` to detect whether the token belongs
...
which workspaces have granted the installation; Zero to Agent then applies the
...
loop-prevention rules used by workspace installs. Zero to Agent retains the org
...
`features.shortcuts` list; Zero to Agent accepts their callback IDs through the same
...
Zero to Agent records Enterprise Grid destinations as
