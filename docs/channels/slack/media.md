...
read_when:
  - Sending voice clips or files to OpenAgent in Slack
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, OpenAgent keeps processing the message and falls back to the file placeholder.
...
1. OpenAgent downloads the file from Slack's private URL using the bot token.
...
| Forwarded/shared attachments                  | Text and Slack-hosted image/file media are best-effort                             | Re-share directly in the OpenAgent thread                                 |
