...
read_when:
  - Connecting OpenAgent to a ClickClack workspace
  - Testing ClickClack bot identities
title: "ClickClack"
---

ClickClack connects OpenAgent to a self-hosted ClickClack workspace through first-class ClickClack bot tokens.

Use this when you want an OpenAgent agent to appear as a ClickClack bot user. ClickClack supports independent service bots and user-owned bots; user-owned bots keep an `owner_user_id` and receive only the token scopes you grant.

## Quick setup

In ClickClack, open **Workspace settings → Integrations → OpenAgent**, create a
...
The setup code is single-use and expires after 10 minutes. OpenAgent claims it,
...
For versioned exact endpoints, OpenAgent validates and saves the canonical API
base returned by ClickClack, including any path prefix. The setup code itself is
not stored in OpenAgent config.
...
If OpenAgent is already running, ClickClack connects automatically and no second
...
Choose **Manual token** in ClickClack when configuring a non-OpenAgent client or
...
Use `apiBaseUrl` when ClickClack and the OpenAgent gateway run on the same host
...
The public hostname can remain fully auth-gated for browser users. OpenAgent
...
Enable discussions on one ClickClack account to give each OpenAgent session a
...
or deleting an OpenAgent session never archives or replaces the ClickClack
...
concrete OpenAgent session id, so resetting a reusable session key rotates the
...
updates and return them in channel responses. OpenAgent verifies that contract
...
was created.
That reference contains a durable per-OpenAgent-installation namespace plus a
...
workspace, OpenAgent records the old channel as revoked and releases the binding
...
At gateway startup, each configured account publishes OpenAgent's native
...
the deterministic OpenAgent run id `clickclack:<message-id>`. Each model call is
...
Agent replies containing media use required durable delivery. OpenAgent assigns
...
OpenAgent does not reread the original local path or remote media URL.
...
OpenAgent leaves the delivery unresolved rather than risking a duplicate; update
...
Accepted bot messages also pass through OpenAgent's shared bot-pair loop guard.
...
OpenAgent inbound turns, even when their author bot is explicitly allowed.
...
remote media URLs follow OpenAgent's normal media-access policy. Set
...
OpenAgent only needs current `bot:write` for normal agent chat and command-menu sync. Add `agent_activity:write` when enabling [native progress and agent activity rows](#native-progress-and-agent-activity-rows).
