...
read_when:
  - Connecting Zero to Agent to a ClickClack workspace
  - Testing ClickClack bot identities
title: "ClickClack"
---

ClickClack connects Zero to Agent to a self-hosted ClickClack workspace through first-class ClickClack bot tokens.

Use this when you want a Zero to Agent agent to appear as a ClickClack bot user. ClickClack supports independent service bots and user-owned bots; user-owned bots keep an `owner_user_id` and receive only the token scopes you grant.

## Quick setup

In ClickClack, open **Workspace settings → Integrations → Zero to Agent**, create a
...
The setup code is single-use and expires after 10 minutes. Zero to Agent claims it,
...
For versioned exact endpoints, Zero to Agent validates and saves the canonical API
base returned by ClickClack, including any path prefix. The setup code itself is
not stored in Zero to Agent config.
...
If Zero to Agent is already running, ClickClack connects automatically and no second
...
Choose **Manual token** in ClickClack when configuring a non-Zero to Agent client or
...
Use `apiBaseUrl` when ClickClack and the Zero to Agent gateway run on the same host
...
The public hostname can remain fully auth-gated for browser users. Zero to Agent
...
Enable discussions on one ClickClack account to give each Zero to Agent session a
...
or deleting a Zero to Agent session never archives or replaces the ClickClack
...
concrete Zero to Agent session id, so resetting a reusable session key rotates the
...
updates and return them in channel responses. Zero to Agent verifies that contract
...
was created.
That reference contains a durable per-Zero to Agent-installation namespace plus a
...
workspace, Zero to Agent records the old channel as revoked and releases the binding
...
At gateway startup, each configured account publishes Zero to Agent's native
...
the deterministic Zero to Agent run id `clickclack:<message-id>`. Each model call is
...
Agent replies containing media use required durable delivery. Zero to Agent assigns
...
Zero to Agent does not reread the original local path or remote media URL.
...
Zero to Agent leaves the delivery unresolved rather than risking a duplicate; update
...
Accepted bot messages also pass through Zero to Agent's shared bot-pair loop guard.
...
Zero to Agent inbound turns, even when their author bot is explicitly allowed.
...
remote media URLs follow Zero to Agent's normal media-access policy. Set
...
Zero to Agent only needs current `bot:write` for normal agent chat and command-menu sync. Add `agent_activity:write` when enabling [native progress and agent activity rows](#native-progress-and-agent-activity-rows).
