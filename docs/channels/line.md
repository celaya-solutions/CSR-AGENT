...
read_when:
  - You want to connect Zero to Agent to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Zero to Agent via the LINE Messaging API. The plugin runs as a webhook
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Zero to Agent applies a strict pre-auth body limit (64 KB) and read timeout before verification.
- Zero to Agent processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
id, and Zero to Agent can only quote a message it kept that token for. Quoting
...
- A reply can only quote a message Zero to Agent received. LINE also returns a quote
...
- Only a message Zero to Agent handed to the agent as its own turn is remembered. In
...
- If LINE rejects a request carrying a quote token with HTTP 400, Zero to Agent
...
not one paragraph at a time — Zero to Agent's own default is 800 characters.
...
Card images and icons must use HTTPS. Zero to Agent removes images with malformed or
...
Outbound media URLs must be public HTTPS URLs of at most 2000 characters. Zero to Agent
...
- **No inbound events:** run `openclaw channels status --probe`. LINE only delivers
  ...
  Zero to Agent does not set either for you: the URL has an API but depends on a public
  address Zero to Agent does not know, and the **Use webhook** switch has no API at all.
