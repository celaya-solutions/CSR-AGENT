...
read_when:
  - You want to connect OpenAgent to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to OpenAgent via the LINE Messaging API. The plugin runs as a webhook
...
- LINE signature verification is body-dependent (HMAC over the raw body), so OpenAgent applies a strict pre-auth body limit (64 KB) and read timeout before verification.
- OpenAgent processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
id, and OpenAgent can only quote a message it kept that token for. Quoting
...
- A reply can only quote a message OpenAgent received. LINE also returns a quote
...
- Only a message OpenAgent handed to the agent as its own turn is remembered. In
...
- If LINE rejects a request carrying a quote token with HTTP 400, OpenAgent
...
not one paragraph at a time — OpenAgent's own default is 800 characters.
...
Card images and icons must use HTTPS. OpenAgent removes images with malformed or
...
Outbound media URLs must be public HTTPS URLs of at most 2000 characters. OpenAgent
...
- **No inbound events:** run `openclaw channels status --probe`. LINE only delivers
  ...
  OpenAgent does not set either for you: the URL has an API but depends on a public
  address OpenAgent does not know, and the **Use webhook** switch has no API at all.
