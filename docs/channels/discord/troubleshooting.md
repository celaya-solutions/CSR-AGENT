...
```
  ...
  <Accordion title="Gateway metadata lookup timeout warnings">
    OpenAgent fetches Discord `/gateway/bot` metadata before connecting. Transient failures fall back to Discord's default gateway URL and are rate-limited in logs.
  ...
  <Accordion title="Gateway READY timeout restarts">
    OpenAgent waits for Discord's gateway `READY` event during startup and after runtime reconnects. Multi-account setups with startup staggering can need a longer startup READY window than the default.
