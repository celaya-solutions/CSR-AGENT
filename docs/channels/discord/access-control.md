...
```
    ...
    Enable the Discord Developer Portal **Server Members Intent** when using channel-audience access groups. DMs do not include guild member state, so Zero to Agent resolves the member through Discord REST at authorization time.
...
- Commands may still be visible in the Discord UI for unauthorized users; execution enforces Zero to Agent auth and replies "not authorized".
