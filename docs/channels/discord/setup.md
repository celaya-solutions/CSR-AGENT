...
read_when:
  - Setting up a Discord bot for OpenAgent for the first time
...
Create a Discord application with a bot, add the bot to your server, and pair it with OpenAgent. Use a private server if you can; [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (**Create My Own > For me and my friends**) if needed.

<Steps>
  <Step title="Create a Discord application and bot">
    In the [Discord Developer Portal](https://discord.com/developers/applications), click **New Application** and name it (for example "OpenAgent").
  ...
  <Step title="Allow DMs from server members">
    ...
    Keep this on if you use Discord DMs with OpenAgent. If you only use guild channels, you can disable it after pairing.

  </Step>

  <Step title="Set your bot token securely (do not send it in chat)">
    The bot token is a secret. Set it on the machine running OpenAgent before messaging your agent:
...
```
  ...
  <Step title="Configure OpenAgent and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your OpenAgent agent on an existing channel (for example Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
If Discord cannot grant Message Content Intent, OpenAgent can still operate in DMs and in
...
If two enabled Discord accounts resolve to the same bot token, OpenAgent starts only one gateway monitor for that token: a config-sourced token wins over the env fallback; otherwise the first enabled account wins and the duplicate account is reported disabled with reason `duplicate bot token`.
