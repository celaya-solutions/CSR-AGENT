...
- `followUsers` accepts raw Discord user IDs and `discord:<id>` values. OpenAgent normalizes both forms before matching voice-state events.
...
- When a followed user joins an allowed voice channel, OpenAgent joins that channel. When the user moves, OpenAgent moves with them. When the active followed user disconnects, OpenAgent leaves.
- If multiple followed users are in the same guild and the active followed user leaves, OpenAgent moves to another tracked followed user's channel before leaving the guild. If several followed users move at once, the latest observed voice-state event wins.
...
- OpenAgent reconciles missed voice-state events on startup and at a bounded interval. Reconciliation samples configured guilds and caps REST lookups per run, so very large `followUsers` lists may take more than one interval to converge.
- If Discord or an admin moves the bot while it is following a user, OpenAgent rebuilds the voice session and preserves follow ownership when the destination is allowed. If the bot is moved outside `allowedChannels`, OpenAgent leaves and rejoins the configured target when one exists.
...
With no `voice.agentSession` block, each voice channel gets its own routed OpenAgent session. For example, `/vc join channel:234567890123456789` talks to the session for that Discord voice channel. The realtime model is only the voice front end; substantive requests are handed to the configured OpenAgent agent. If the realtime model produces a final transcript without calling the consult tool, OpenAgent forces the consult as a fallback so the default still behaves like talking to the agent.
...
Use this when the model hears its own Discord playback through an open mic, but you still want to interrupt it by speaking. OpenAgent keeps OpenAI from auto-interrupting on raw input audio, while `bargeIn: true` lets Discord speaker-start events and already-active speaker audio cancel active realtime responses before the next captured turn reaches OpenAI. Very early barge-in signals with `audioEndMs` below `minBargeInAudioEndMs` are treated as likely echo/noise and ignored so the model does not cut off at the first playback frame.
...
4. `barge-in detected source=active-speaker-audio` means OpenAgent saw live speaker audio while assistant playback was active. This is useful for distinguishing a real interruption from a Discord speaker-start event with no useful audio.
5. `barge-in requested reason=...` means OpenAgent asked the realtime provider to cancel or truncate the active response. `outputAudioMs` and `playbackChunks` describe generated audio; the subsequent `audioEndMs` truncation field records consumed audio for each native item. Discord measures local consumption from Opus packets prepared by AudioPlayer, at 20 ms granularity; it cannot measure remote listener playout.
...
- `audioMs`: assistant audio duration that OpenAgent counted before playback stopped.
...
- `source=speaker-start` followed by `speaker turn closed ... hasAudio=false` means Discord reported a speaker start but no audio reached OpenAgent. That can be a transient Discord voice event, noise gate behavior, or a client briefly keying the mic.
...
- `Discord voice receive backlog exceeded` means identity lookup, decoding, or local disk could not keep up with incoming audio. OpenAgent bounds each receive stream at 1,000 pending packets and 1 MiB of encoded audio; speak again after the bottleneck clears. Other speakers and any registered capture remain active, and incomplete speech cannot become a new conversation command.
...
- `capture ignored: ... reason=protected playback` means OpenAgent intentionally withheld conversational input while assistant audio was active. An explicitly started capture still records that speech. Enable `voice.realtime.bargeIn` if you want speech to interrupt playback.
- `barge-in ignored ... outputActive=false` means Discord or provider VAD reported speech, but OpenAgent had no active playback to interrupt. This should not cut off audio.
