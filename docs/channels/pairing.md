...
"Pairing" is OpenAgent's explicit access approval step.
...
DM pairing applies to channels that implement OpenAgent's pairing API. With DM
...
For channels that use OpenAgent's pairing API, state is stored in the shared SQLite
database at
