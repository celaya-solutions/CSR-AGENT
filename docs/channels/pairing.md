...
read_when:
  ...
  - Reviewing Zero to Agent security posture
title: "Pairing"
---

"Pairing" is Zero to Agent's explicit access approval step.
...
DM pairing applies to channels that implement Zero to Agent's pairing API. With DM
...
<Note>
WhatsApp's login QR links a WhatsApp account to Zero to Agent. DM access requests
...
Zero to Agent's pairing API. Check the plugin's documentation for version-specific
limitations.
...
For channels that use Zero to Agent's pairing API, state is stored in the shared SQLite
database at
...
5. On your phone, open the Zero to Agent app → **Settings** → **Gateway**.
6. Scan the QR code or paste the setup code, then connect.

Official Zero to Agent iOS and Android apps are approved automatically when their
...
3. On your phone, open the Zero to Agent iOS app → Settings → Gateway.
...
Zero to Agent advertises Tailscale setup URLs only when it owns the route through
...
ownership. If you confirm it is a stale route from an older Zero to Agent release,
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, Zero to Agent keeps the existing approval as-is and creates a fresh pending upgrade request. Use `openclaw devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
