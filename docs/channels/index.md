---
summary: "Messaging platforms Zero to Agent can connect to"
read_when:
  - You want to choose a chat channel for Zero to Agent
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

Zero to Agent can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
"External plugin" channels are maintained outside the Zero to Agent repo.
...
- [A2A](/channels/a2a) - Connect external agents to Zero to Agent through the A2A 1.0 JSON-RPC protocol (bundled plugin).
- [Buzz](/channels/buzz) - Connect Zero to Agent agents to Buzz rooms (official plugin).
...
- [iMessage](/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new Zero to Agent iMessage setups when host requirements fit (official plugin).
...
- [Mattermost](/channels/mattermost) - Mattermost bot setup and Zero to Agent config (official plugin).
...
- [Reef](/channels/reef) - Reef channel setup: guarded, end-to-end-encrypted messaging between Zero to Agent agents of different people (bundled plugin).
...
- [Synology Chat](/channels/synology-chat) - Synology Chat webhook setup and Zero to Agent config (official plugin).
...
- Channels can run simultaneously; configure multiple and Zero to Agent will route per chat.
