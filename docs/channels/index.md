...
OpenAgent can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
"External plugin" channels are maintained outside the OpenAgent repo.
...
- [A2A](/channels/a2a) - Connect external agents to OpenAgent through the A2A 1.0 JSON-RPC protocol (bundled plugin).
- [Buzz](/channels/buzz) - Connect OpenAgent agents to Buzz rooms (official plugin).
...
- [iMessage](/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new OpenAgent iMessage setups when host requirements fit (official plugin).
...
- [Mattermost](/channels/mattermost) - Mattermost bot setup and OpenAgent config (official plugin).
...
- [Reef](/channels/reef) - Reef channel setup: guarded, end-to-end-encrypted messaging between OpenAgent agents of different people (bundled plugin).
...
- [Synology Chat](/channels/synology-chat) - Synology Chat webhook setup and OpenAgent config (official plugin).
