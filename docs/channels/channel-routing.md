---
summary: "How OpenAgent picks an agent and session for each inbound message, and how replies route back"
read_when:
...
OpenAgent routes replies **back to the channel where a message came from**. The
...
OpenAgent infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, OpenAgent still records inbound session metadata, but it
...
when a guarded path must not create a new OpenAgent session. In that mode,
OpenAgent may update metadata and `lastRoute` for an existing session, but it
