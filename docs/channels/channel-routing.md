---
summary: "How Zero to Agent picks an agent and session for each inbound message, and how replies route back"
read_when:
...
Zero to Agent routes replies **back to the channel where a message came from**. The
...
Zero to Agent infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Zero to Agent still records inbound session metadata, but it
...
when a guarded path must not create a new Zero to Agent session. In that mode,
Zero to Agent may update metadata and `lastRoute` for an existing session, but it
