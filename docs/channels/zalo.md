...
Zalo ships as a bundled plugin in current OpenAgent releases, so packaged builds do not need a separate install.
...
- Reported real-world caveat: on some Marketplace-bot setups the bot could not be added to a group at all. If you hit that, verify with your bot's Zalo Bot Platform settings. It is a platform-side constraint, not an OpenAgent policy.
...
- Webhook mode: set `channels.zalo.webhookUrl` and `channels.zalo.webhookSecret`.
  ...
  - OpenAgent returns HTTP 200 only after it durably stores the raw event. Storage failures return HTTP 500. The durable `200` carries `x-openclaw-delivery-accepted: durable`. Reverse proxies can require that header to distinguish OpenAgent acceptance from a generic `200`. Authentication, validation, and storage-error responses omit it.
