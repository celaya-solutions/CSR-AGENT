---
summary: "Launch self-contained OpenAgent HTML widgets inside Discord Activities"
read_when:
...
- an existing [OpenAgent Discord bot](/channels/discord)
- a public HTTPS hostname that reaches the OpenAgent gateway
...
- Everyone who Discord permits into that channel can open its widgets. To narrow the audience, use Discord channel permissions. OpenAgent command and DM allowlists do not grant or remove access to already-posted channel content.
...
- confirm [hot reload](/gateway/configuration/hot-reload) applied the OpenAgent configuration; restart the Gateway if you changed its service environment
...
Launch the button from the channel where the agent posted it. OpenAgent tracks launches server-side when clicked, so a fresh launch record can resolve the exact widget even when Discord omits or mangles the button's custom ID. When neither the custom ID nor a launch record resolves, OpenAgent opens the most recently posted live widget in that channel. Older widgets remain addressable through buttons that preserve their custom ID.

### “You cannot launch Activities in this channel”

Discord does not launch Activities from forum-style channels. OpenAgent rejects the Activity component delivery there instead of posting a button that cannot work. Ask for the widget from a regular text channel instead.
