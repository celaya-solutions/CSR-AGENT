...
read_when:
  - You want people to reach an OpenAgent agent from Buzz
  - You are setting up a Buzz bot identity and room access
  - You are troubleshooting a Buzz connection
title: "Buzz"
---

Buzz is an official channel plugin that connects OpenAgent agents to team rooms
...
- Preserves Markdown in replies and sends text through OpenAgent's built-in
  `message` tool
...
- Resolves current Buzz profile names, avatars, room names, and room membership
  through OpenAgent's directory commands
...
- The **private key** lets OpenAgent authenticate and sign messages. It stays with
  the Gateway.
- The **public key** identifies the bot. Buzz owners use it for relay approval,
  room admins use it to grant the **Bot** role, and OpenAgent can use public keys
  in sender allowlists.

The relay URL points to one Buzz workspace. Each room has a UUID, and OpenAgent
...
<Warning>
Never give OpenAgent a human Buzz owner's private key. OpenAgent creates or uses a
...
3. OpenAgent reuses that account's bot identity or generates one automatically.
4. If the bot does not have room access yet, give the displayed public key to a
   Buzz room owner or admin.
5. OpenAgent waits for Buzz to confirm the **Bot** role and continues
...
6. If Buzz returns one room, OpenAgent selects it. If Buzz returns several,
   select the rooms to use and the default outbound room.
7. OpenAgent saves the configuration and silently verifies the authenticated
...
At startup, OpenAgent skips active configured rooms where the bot lacks the Bot
...
When the relay reports a membership change for a skipped room, OpenAgent checks
...
OpenAgent identity. Use the Buzz CLI as the existing human room owner or admin:
...
Run that command as the existing human owner or admin. Never give OpenAgent that
human private key.

After the Gateway connects, OpenAgent preserves an existing non-empty Buzz
...
configured Buzz rooms, and finally `OpenAgent`. This replaces the shortened
public key in Buzz after its profile cache refreshes.

OpenAgent also registers the same public identity in Buzz's agent directory. It
...
instead of treating it as a normal member. OpenAgent still receives messages
...
`channels.buzz.accounts.<id>.authTag` for a nested identity. OpenAgent includes
that attestation in the published profile so Buzz can show the verified human
owner.

While the Gateway is connected, OpenAgent publishes and refreshes the bot's
...
If the relay stops acknowledging presence, OpenAgent reconnects the affected
...
OpenAgent cannot grant room or relay access. It displays only the bot public key
...
as a destination for OpenAgent's built-in `message` tool and normal reply
delivery.
...
OpenAgent command or textual mention.
...
replies use OpenAgent's shared typing interval, which defaults to six seconds.
OpenAgent stops refreshing when the turn completes, is cancelled, fails, or the
...
openclaw message send \
  ...
  --message "Hello from OpenAgent"
```

### Native mentions

Write a unique current room member's profile name as `@Display Name`. OpenAgent
...
OpenAgent keeps a bounded snapshot of the configured rooms, their current
...
profile subscription does not reach EOSE within 10 seconds, OpenAgent treats the
...
If a configured room is archived or restored while OpenAgent is connected, the
...
Each configured room uses one room-scoped relay subscription. OpenAgent reserves
...
Unique current room names can resolve as outbound targets through OpenAgent's
...
Standard OpenAgent bindings can send each Buzz room to a different agent,
...
Without a room-specific binding, normal OpenAgent routing selects the default
...
rooms. OpenAgent loads Buzz's relay-signed room roster before accepting messages,
...
agent under the same mention and sender rules. Within each Gateway, OpenAgent
...
key and stores it in the selected account's `privateKey`, following OpenAgent's current
...
- Creating or administering rooms from OpenAgent
