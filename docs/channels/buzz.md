...
read_when:
  - You want people to reach a Zero to Agent agent from Buzz
  - You are setting up a Buzz bot identity and room access
  - You are troubleshooting a Buzz connection
title: "Buzz"
---

Buzz is an official channel plugin that connects Zero to Agent agents to team rooms
...
- Preserves Markdown in replies and sends text through Zero to Agent's built-in
  `message` tool
...
- Resolves current Buzz profile names, avatars, room names, and room membership
  through Zero to Agent's directory commands
...
- The **private key** lets Zero to Agent authenticate and sign messages. It stays with
  the Gateway.
- The **public key** identifies the bot. Buzz owners use it for relay approval,
  room admins use it to grant the **Bot** role, and Zero to Agent can use public keys
  in sender allowlists.

The relay URL points to one Buzz workspace. Each room has a UUID, and Zero to Agent
...
<Warning>
Never give Zero to Agent a human Buzz owner's private key. Zero to Agent creates or uses a
...
3. Zero to Agent reuses that account's bot identity or generates one automatically.
4. If the bot does not have room access yet, give the displayed public key to a
   Buzz room owner or admin.
5. Zero to Agent waits for Buzz to confirm the **Bot** role and continues
...
6. If Buzz returns one room, Zero to Agent selects it. If Buzz returns several,
   select the rooms to use and the default outbound room.
7. Zero to Agent saves the configuration and silently verifies the authenticated
...
At startup, Zero to Agent skips active configured rooms where the bot lacks the Bot
...
When the relay reports a membership change for a skipped room, Zero to Agent checks
...
Zero to Agent identity. Use the Buzz CLI as the existing human room owner or admin:
...
Run that command as the existing human owner or admin. Never give Zero to Agent that
human private key.

After the Gateway connects, Zero to Agent preserves an existing non-empty Buzz
...
configured Buzz rooms, and finally `Zero to Agent`. This replaces the shortened
public key in Buzz after its profile cache refreshes.

Zero to Agent also registers the same public identity in Buzz's agent directory. It
...
instead of treating it as a normal member. Zero to Agent still receives messages
...
`channels.buzz.accounts.<id>.authTag` for a nested identity. Zero to Agent includes
that attestation in the published profile so Buzz can show the verified human
owner.

While the Gateway is connected, Zero to Agent publishes and refreshes the bot's
...
If the relay stops acknowledging presence, Zero to Agent reconnects the affected
...
Zero to Agent cannot grant room or relay access. It displays only the bot public key
...
as a destination for Zero to Agent's built-in `message` tool and normal reply
delivery.
...
Zero to Agent command or textual mention.
...
replies use Zero to Agent's shared typing interval, which defaults to six seconds.
Zero to Agent stops refreshing when the turn completes, is cancelled, fails, or the
...
openclaw message send \
  ...
  --message "Hello from Zero to Agent"
```

### Native mentions

Write a unique current room member's profile name as `@Display Name`. Zero to Agent
...
Zero to Agent keeps a bounded snapshot of the configured rooms, their current
...
profile subscription does not reach EOSE within 10 seconds, Zero to Agent treats the
...
If a configured room is archived or restored while Zero to Agent is connected, the
...
Each configured room uses one room-scoped relay subscription. Zero to Agent reserves
...
Unique current room names can resolve as outbound targets through Zero to Agent's
...
Standard Zero to Agent bindings can send each Buzz room to a different agent,
...
Without a room-specific binding, normal Zero to Agent routing selects the default
...
rooms. Zero to Agent loads Buzz's relay-signed room roster before accepting messages,
...
agent under the same mention and sender rules. Within each Gateway, Zero to Agent
...
key and stores it in the selected account's `privateKey`, following Zero to Agent's current
...
- Creating or administering rooms from Zero to Agent
