...
Signal is a downloadable channel plugin (`@openclaw/signal`). The gateway talks to `signal-cli` over HTTP: either the native daemon (JSON-RPC + SSE) or the [bbernhard/signal-cli-rest-api](https://github.com/bbernhard/signal-cli-rest-api) container (REST + WebSocket). OpenAgent does not embed libsignal.
...
<Steps>
  ...
  <Step title="Link or register the account">
    - **QR link (fastest):** `signal-cli link -n "OpenAgent"`, then scan with Signal. See [Path A](#setup-path-a-link-existing-signal-account-qr).
...
2. Link a bot account: `signal-cli link -n "OpenAgent"`, then scan the QR in Signal.
...
4. Configure OpenAgent and verify the channel. Config changes follow [hot reload](/gateway/configuration/hot-reload); start the Gateway if it is offline. Restart it if you changed the service's `PATH` to find `signal-cli`.
...
To manage `signal-cli` yourself (slow JVM cold starts, container init, shared CPUs), run the daemon separately and point OpenAgent at it:
...
This skips auto-spawn and OpenAgent's startup wait. For a managed daemon with a slow start, set `channels.signal.transport.startupTimeoutMs`.
...
- Register or link your Signal account inside the container before connecting OpenAgent.
...
`transport.kind` controls which protocol and process lifecycle OpenAgent uses:
...
Container mode supports the same Signal operations as native mode where the container exposes matching APIs: sends, receives, attachments, typing indicators, read/viewed receipts, reactions, groups, and styled text. OpenAgent translates native Signal RPC calls into the container's REST payloads, including `group.{base64(internal_id)}` group IDs and `text_mode: "styled"` for formatted text.
...
- Replies to inbound messages include native Signal quote metadata when the backend accepts the inbound timestamp and author; if quote metadata is missing or rejected, OpenAgent sends the reply as a normal message.
...
- **Typing indicators**: OpenAgent sends typing signals via `signal-cli sendTyping` and refreshes them while a reply is running.
- **Read receipts**: when `channels.signal.sendReadReceipts` is true, OpenAgent forwards read receipts for allowed DMs.
...
Configure aliases for stable names on recurring Signal targets. Aliases are OpenAgent-side config only; they do not create or edit Signal contacts.
...
- `channels.signal.aliases`: OpenAgent-side aliases for DM or group delivery targets.
