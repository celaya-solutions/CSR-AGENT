...
The Adaptive Card surfaces Zero to Agent sends to Teams, the Graph-backed roster lookup, and the target formats that address them.

## Member info action

Zero to Agent exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve verified roster details for a configured conversation.
...
Microsoft Teams can deliver exec and plugin approval requests as Adaptive Cards in the originating conversation. Each card describes the requested command or plugin action and provides only the decisions allowed for that request, such as **Approve once**, **Always allow**, and **Deny**. After a decision or expiration, Zero to Agent updates the original card with its final status.
...
`approvals.exec` and `approvals.plugin` are independent; enabling one does not enable the other. Native card delivery also requires a configured Teams bot and at least one approver resolved from `channels.msteams.allowFrom` or `channels.msteams.defaultTo`. Approvers must be stable AAD object IDs; display names, email addresses, group entries, and conversation IDs do not grant approval access. Zero to Agent checks the clicking user's AAD object ID before resolving the request.
...
Zero to Agent sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `openclaw message poll --channel msteams --target conversation:<id> --poll-question "..." --poll-option "..." --poll-option "..."`.
- Votes are recorded by the gateway in Zero to Agent plugin-state SQLite under `state/openclaw.sqlite`.
...
Send semantic presentation payloads to Teams users or conversations using the `message` tool, CLI, or normal reply delivery. Zero to Agent renders them as Teams Adaptive Cards from the generic presentation contract.

The `presentation` parameter accepts semantic blocks. When `presentation` is provided, the message text is optional. Buttons render as Adaptive Card submit or URL actions. Select menus are not native in the Teams renderer, so Zero to Agent downgrades them to readable text before delivery.
...
- Proactive messages are only possible **after** a user has interacted, because Zero to Agent stores conversation references at that point.
