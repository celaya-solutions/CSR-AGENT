...
Microsoft Teams ships as a bundled plugin in current OpenAgent releases; no separate install is required in the normal packaged build.
...
**4. Configure OpenAgent** using the credentials from the output:
...
- Talk to OpenAgent via Teams DMs, group chats, or channels.
