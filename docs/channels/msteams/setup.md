...
Microsoft Teams ships as a bundled plugin in current Zero to Agent releases; no separate install is required in the normal packaged build.
...
**4. Configure Zero to Agent** using the credentials from the output:
...
- Talk to Zero to Agent via Teams DMs, group chats, or channels.
...
### Step 6: Configure Zero to Agent
