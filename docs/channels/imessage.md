---
summary: "Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new OpenAgent iMessage setups when host requirements fit."
read_when:
...
<Note>
For the usual OpenAgent iMessage deployment, run the Gateway and `imsg` on the same signed-in macOS Messages host. If your Gateway runs elsewhere, point `channels.imessage.cliPath` at a transparent SSH wrapper that runs `imsg` on the Mac.
...
For the common local setup, OpenAgent setup can offer a user-confirmed Homebrew install or update for `imsg` on the signed-in Messages Mac. Manual setup and SSH-wrapper topologies remain operator-managed: install or update `imsg` in the same user context that will run the Gateway or wrapper.
...
- <a id="configure-openclaw" />[Configure OpenAgent](/channels/imessage/setup#configure-openclaw)
...
<CardGroup cols={2}>
  ...
  <Card title="RPC adapters" icon="plug" href="/reference/rpc">
    The line-delimited JSON-RPC stdio protocol OpenAgent speaks to `imsg rpc`.
  </Card>
  <Card title="macOS VMs" icon="display" href="/install/macos-vm">
    Run OpenAgent in a sandboxed macOS VM, local or hosted, when you want iMessage isolated from your main Mac.
  </Card>
</CardGroup>
