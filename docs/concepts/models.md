---
summary: "How Zero to Agent resolves provider/model refs, config keys, and the `/model` chat command"
read_when:
...
endpoints, and authored request behavior stay on Zero to Agent. Plaintext official
...
<Steps>
  ...
  <Step title="Auth failover">
    Auth-profile rotation happens inside a provider before Zero to Agent moves to the next fallback model.
  </Step>
</Steps>
...
| Auto fallback                                                           | Temporary recovery state, stored as `modelOverrideSource: "auto"`. Zero to Agent periodically reprobes the original primary, clears the auto selection on recovery, and announces fallback/recovery transitions once per state change.                         |
...
If discovery fails, Zero to Agent reports the failure and keeps the last compatible
...
`openai/gpt-5.5` explicitly. Zero to Agent does not silently downgrade it.
...
- Model refs are parsed by splitting on the first `/`. Type `provider/model`. If the model ID itself contains `/` (OpenRouter-style), include the provider prefix, for example `/model openrouter/moonshotai/kimi-k2`. If you omit the provider, Zero to Agent tries an alias match first. It then tries a unique configured-provider match for that exact unprefixed model id. It then tries the configured default provider, which is a deprecated fallback. If that provider no longer exposes the configured default model, Zero to Agent uses the first configured provider and model instead. This avoids surfacing a stale removed-provider default.
...
Zero to Agent can refresh the model metadata shipped by installed provider plugins
without waiting for a new Zero to Agent release. The Gateway makes one background
...
Its scheduled workflow checks Zero to Agent's default-branch plugin manifests and
...
a new Zero to Agent release. When a valid native feed no longer supplies a model's
...
Marker persistence is source-authoritative. Zero to Agent writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values. It does this whenever it regenerates `models.json`, including command-driven paths like `openclaw agent`.

## Related

- [Agent runtimes](/concepts/agent-runtimes) — Zero to Agent, Codex, and other agent loop runtimes
