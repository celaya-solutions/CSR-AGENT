---
summary: "How OpenAgent resolves provider/model refs, config keys, and the `/model` chat command"
read_when:
...
<Steps>
  ...
  <Step title="Auth failover">
    Auth-profile rotation happens inside a provider before OpenAgent moves to the next fallback model.
  </Step>
</Steps>
...
If discovery fails, OpenAgent reports the failure and keeps the last compatible
...
`openai/gpt-5.5` explicitly. OpenAgent does not silently downgrade it.
...
- Model refs are parsed by splitting on the first `/`. Type `provider/model`. If the model ID itself contains `/` (OpenRouter-style), include the provider prefix, for example `/model openrouter/moonshotai/kimi-k2`. If you omit the provider, OpenAgent tries an alias match first. It then tries a unique configured-provider match for that exact unprefixed model id. It then tries the configured default provider, which is a deprecated fallback. If that provider no longer exposes the configured default model, OpenAgent uses the first configured provider and model instead. This avoids surfacing a stale removed-provider default.
...
- [Agent runtimes](/concepts/agent-runtimes) — OpenAgent, Codex, and other agent loop runtimes
