...
OpenAgent keeps shared session state on the Gateway. The Control UI, mobile
clients, ACP, `openagent tui <target>`, and `openagent attach <target>` project
...
Use `openagent tui` when you want to continue the conversation in a terminal.
Use `openagent attach` when you want a coding harness beside the session with a
temporary, session-scoped MCP grant.

Embedded local mode is separate: `openagent tui --local`, `openagent chat`, and
`openagent terminal` use the local agent runtime and cannot accept a session
...
terminal…**. The dialog copies a credential-free `openagent resume` command with
...
`cmd.exe`. Run it in an OpenAgent CLI profile that is already configured for that
...
```bash
openagent resume
openagent resume agent:main:deploy-monitor
```
...
```bash
openagent tui https://claw.example.com/dashboard/main/deploy-monitor-6db92d48
openagent tui deploy-monitor-6db92d48
```
...
Pass the same URL or reference to `openagent attach`:

```bash
openagent attach https://claw.example.com/dashboard/main/deploy-monitor-6db92d48
openagent attach deploy-monitor-6db92d48
```
...
origin. OpenAgent never reuses configured credentials or a stored device token
from another origin for that target. The credential-free command copied by
**Continue in terminal…** has a narrower rule: `openagent resume` may reuse the
...
2. Open **Settings > Devices** in that Gateway's Control UI and approve the
   ...
   request with `openagent devices approve --latest`, verify it, and run the
   printed `openagent devices approve <requestId>` command.
3. Retry the original command. OpenAgent stores the issued operator device token
...
| Session missing                    | The selected Gateway cannot find that key or short ID.                                   | For the configured Gateway, run `openagent sessions list`. For a URL target, choose the session in that Gateway's Control UI.                                    |
...
| `pairing-required`                 | The device is new or an existing device needs a role, scope, or metadata approval.       | Approve the pending request in **Settings > Devices**, or preview it with `openagent devices approve --latest` and run the printed exact-ID command, then retry. |
| `device-identity-required`         | The Gateway requires a signed device identity for this connection.                       | Use a current OpenAgent client, let it create its device identity, and complete pairing.                                                                         |
| `scope-mismatch`                   | The stored device token is valid but lacks the requested operator scope.                 | Review `openagent devices list`, approve the pending scope upgrade, and reconnect.                                                                               |
| `auth-rejected`                    | An explicit shared credential is wrong, or a paired-device token was revoked or rotated. | Verify explicit Gateway auth. For a stale device token, rotate it with `openagent devices rotate --device <deviceId> --role operator` or pair again.             |
