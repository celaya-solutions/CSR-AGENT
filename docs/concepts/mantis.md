...
title: "Mantis"
read_when:
  - Building or running live visual QA for Zero to Agent bugs
...
Mantis publishes visual CI evidence and a PR comment for Zero to Agent behavior.
...
- Zero to Agent (`extensions/qa-lab/src/mantis/*`): scenario runtime, `pnpm openclaw qa mantis <command>` CLI, evidence schema.
...
With `--gateway-setup`, the command creates a persistent disposable Zero to Agent
...
| `Mantis Web UI Chat Proof`        | manual dispatch | Runs the focused Zero to Agent Control UI chat Playwright proof against the candidate, verifies the browser sends through the mocked Gateway, captures screenshot/video artifacts, and posts PR evidence. This lane is web chat proof only, not WinUI/native-app or arbitrary visual proof. |
...
[Zero to Agent #138953](https://github.com/openclaw/openclaw/pull/138953) lets the
...
noVNC, Node 24.16+ or 26.1+ and pnpm, a Zero to Agent checkout, and
...
ref policy, candidate ref policy, Zero to Agent config patch, setup/stimulus steps,
