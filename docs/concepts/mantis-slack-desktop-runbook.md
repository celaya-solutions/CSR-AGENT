...
bugs that need a Linux desktop, VNC rescue, Slack Web, a real Zero to Agent gateway,
screenshots, videos, and a PR evidence comment. Use it when unit tests or the
headless Slack live lane cannot prove the bug.

## Terms

- **Mantis** - the Zero to Agent system that runs these scenarios and publishes visual
...
- **Mantis artifacts** - owned by the Zero to Agent run. Live under
...
- `crabbox.remote_run` - sync, browser launch, Zero to Agent install/build or
...
remote status but Mantis copied metadata proving either the Zero to Agent gateway
