...
Zero to Agent has two independent streaming layers, and there is **no true
...
streaming sends media early, Zero to Agent remembers that delivery for the turn. If
...
distinct text around media that was already streamed, Zero to Agent still sends the
...
When block streaming is enabled, Zero to Agent can **merge consecutive block
...
- If the final edit fails before the completed text is confirmed, Zero to Agent uses
...
- Telegram selected quote replies are an exception: when `replyToMode` is not
  `"off"` and selected quote text is present, Zero to Agent skips the answer preview
...
- Cards include **Open in Zero to Agent** only when the session is actually openable:
...
- `streaming.preview.toolProgress` (default `true`) controls tool-progress
  ...
  that use Zero to Agent's compact progress renderer, including Discord, Matrix,
