...
OpenAgent has two independent streaming layers, and there is **no true
...
streaming sends media early, OpenAgent remembers that delivery for the turn. If
...
distinct text around media that was already streamed, OpenAgent still sends the
...
When block streaming is enabled, OpenAgent can **merge consecutive block
...
- If the final edit fails before the completed text is confirmed, OpenAgent uses
...
- Telegram selected quote replies are an exception: when `replyToMode` is not
  `"off"` and selected quote text is present, OpenAgent skips the answer preview
