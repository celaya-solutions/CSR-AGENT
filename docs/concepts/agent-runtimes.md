---
summary: "How Zero to Agent separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between Zero to Agent, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------------ |
| Provider      | `anthropic`, `github-copilot`, `openai`      | How Zero to Agent authenticates, discovers models, and names model refs. |
...
| Channel       | Discord, Slack, Telegram, WhatsApp           | Where messages enter and leave Zero to Agent.                            |
...
- **Embedded harnesses** run inside Zero to Agent's prepared agent loop: the
...
| Surface                                          | Zero to Agent name/config            | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
That means Zero to Agent selects an OpenAI model ref, then asks the Codex
...
Zero to Agent session store becomes Codex.
...
3. **Zero to Agent explicitly chosen for an OpenAI model** -> keep the model ref as `openai/<model>` and set provider/model runtime policy to `agentRuntime.id: "openclaw"`. A selected `openai` OAuth profile is routed internally through Zero to Agent's Codex-auth transport.
...
| Surface                     | Zero to Agent embedded                                   | Codex app-server                                                                 |
| --------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------- |
| Model loop owner            | Zero to Agent, through the Zero to Agent embedded runner | Codex app-server                                                                 |
| Canonical thread state      | Zero to Agent transcript                                 | Codex thread, plus Zero to Agent transcript mirror                               |
| Zero to Agent dynamic tools | Native Zero to Agent tool loop                           | Bridged through the Codex adapter                                                |
| Native shell and file tools | Zero to Agent path                                       | Codex-native tools, bridged through native hooks where supported                 |
| Context engine              | Native Zero to Agent context assembly                    | Zero to Agent projects assembled context into the Codex turn                     |
| Compaction                  | Zero to Agent or selected context engine                 | Codex-native compaction, with Zero to Agent notifications and mirror maintenance |
| Channel delivery            | Zero to Agent                                            | Zero to Agent                                                                    |

Design rule: if Zero to Agent owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, Zero to Agent needs runtime
events or native hooks. If the native runtime owns canonical thread state,
Zero to Agent mirrors and projects context rather than rewriting unsupported
internals.
...
its native connection's authentication. Zero to Agent verifies that ownership against
...
Zero to Agent resolves an embedded runtime after provider and model resolution, in
this order:
...
4. If nothing claims the turn in `auto` mode, Zero to Agent falls back to
...
exception: a harness may declare that Zero to Agent can reproduce the exact request.
...
Completions adapters, and authored request overrides stay on Zero to Agent rather
...
profile, it uses Zero to Agent's Codex-auth transport while keeping the public model
...
When a runtime is not Zero to Agent, its docs should state which Zero to Agent surfaces
it supports:

| Question                               | Why it matters                                                                                         |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                        |
| Who owns canonical thread history?     | Determines whether Zero to Agent can edit history or only mirror it.                                   |
| Do Zero to Agent dynamic tools work?   | Messaging, sessions, cron, and Zero to Agent-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Zero to Agent-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume Zero to Agent equivalence where the native runtime owns more state.            |
