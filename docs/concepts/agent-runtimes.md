---
summary: "How OpenAgent separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between OpenAgent, Codex, ACP, or another native agent runtime
...
| ------------- | ----------------------------------- | -------------------------------------------------------------------- |
| Provider      | `anthropic`, `openai`, `openrouter` | How OpenAgent authenticates, discovers models, and names model refs. |
| Model         | `claude-opus-4-6`, `gpt-6-astra`    | The model selected for the agent turn.                               |
| Agent runtime | `claude-cli`, `codex`, `openclaw`   | The low-level loop or backend that executes the prepared turn.       |
| Channel       | Discord, Telegram                   | Where messages enter and leave OpenAgent.                            |
...
- **Embedded harnesses** run inside OpenAgent's prepared agent loop: the
...
| Surface                                          | OpenAgent name/config                | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
That means OpenAgent selects an OpenAI model ref, then asks the Codex
...
OpenAgent session store becomes Codex.
...
| Surface                     | OpenAgent embedded                               | Codex app-server                                                             |
| --------------------------- | ------------------------------------------------ | ---------------------------------------------------------------------------- |
| Model loop owner            | OpenAgent, through the OpenAgent embedded runner | Codex app-server                                                             |
| Canonical thread state      | OpenAgent transcript                             | Codex thread, plus OpenAgent transcript mirror                               |
| OpenAgent dynamic tools     | Native OpenAgent tool loop                       | Bridged through the Codex adapter                                            |
| Native shell and file tools | OpenAgent path                                   | Codex-native tools, bridged through native hooks where supported             |
| Context engine              | Native OpenAgent context assembly                | OpenAgent projects assembled context into the Codex turn                     |
| Compaction                  | OpenAgent or selected context engine             | Codex-native compaction, with OpenAgent notifications and mirror maintenance |
| Channel delivery            | OpenAgent                                        | OpenAgent                                                                    |

Design rule: if OpenAgent owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, OpenAgent needs runtime
events or native hooks. If the native runtime owns canonical thread state,
OpenAgent mirrors and projects context rather than rewriting unsupported
internals.
...
its native connection's authentication. OpenAgent verifies that ownership against
...
OpenAgent resolves an embedded runtime after provider and model resolution, in
this order:
...
4. If nothing claims the turn in `auto` mode, OpenAgent falls back to
...
exception: a harness may declare that OpenAgent can reproduce the exact request.
...
Completions adapters, and authored request overrides stay on OpenAgent rather
...
When a runtime is not OpenAgent, its docs should state which OpenAgent surfaces
it supports:

| Question                               | Why it matters                                                                                     |
| -------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                    |
| Who owns canonical thread history?     | Determines whether OpenAgent can edit history or only mirror it.                                   |
| Do OpenAgent dynamic tools work?       | Messaging, sessions, cron, and OpenAgent-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around OpenAgent-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume OpenAgent equivalence where the native runtime owns more state.            |
