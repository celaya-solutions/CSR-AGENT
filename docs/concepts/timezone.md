---
summary: "Where timezones show up in Zero to Agent — envelopes, tool payloads, system prompt"
read_when:
...
Zero to Agent standardizes timestamps so the model sees a **single reference time** instead of a mix of provider-local clocks. Three surfaces show timezones, each with its own purpose:
...
If `userTimezone` is unset, Zero to Agent resolves the host timezone at runtime via
