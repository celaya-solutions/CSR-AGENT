...
When a normal prompt arrives while a session run is already streaming and the queue mode is `steer` (the default, no config needed), Zero to Agent tries to send that prompt into the active runtime. Zero to Agent and the native Codex app-server harness implement the delivery details differently.
...
Steering does not interrupt a tool call that is already running. The Zero to Agent runtime checks at tool-launch boundaries as well as model boundaries:

1. The assistant asks for tool calls.
2. In sequential mode, Zero to Agent checks immediately before each call starts, including after asynchronous resolution, validation, and pre-execution hooks.
3. A running call finishes. If a steer is waiting afterward, the unstarted sequential tail is skipped.
4. In parallel mode, Zero to Agent prepares calls first, then checks once immediately before launching the prepared calls. Calls that have crossed that checkpoint continue together.
...
6. Zero to Agent appends the exact drained steering message before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of Zero to Agent runtime's internal steering queue. Zero to Agent batches queued prompts for the configured quiet window, then sends a single `turn/steer` request with all collected user input in arrival order. Codex's upstream turn scheduler owns its tool scheduling and consumes accepted steering at the next model boundary; Zero to Agent does not add per-tool preemption to that runtime.

Codex review and manual compaction turns reject same-turn steering. When a runtime cannot accept steering in `steer` mode, Zero to Agent waits for the active run to finish before starting the prompt.

## Tool launch boundaries

Zero to Agent distinguishes started work from requested work:

- A sequential call that is already running completes. Later calls have not started, so Zero to Agent returns synthetic skipped results for them and lets the model reconsider with the steer visible.
...
- Zero to Agent preserves the runtime's configured steering drain mode and FIFO order. One-at-a-time consumers keep later messages for later boundaries; `all` consumers inject the queued FIFO batch together. Codex receives messages collected during its quiet window as one batched `turn/steer`.
- With `/queue collect`, Zero to Agent does not steer. It waits until the active run ends, then creates a followup turn with compatible queued messages after the debounce window.
- With `/queue interrupt`, Zero to Agent aborts the active run and starts the newest message instead of steering.
...
An authorized Gateway client can withdraw a message still waiting in the Zero to Agent
