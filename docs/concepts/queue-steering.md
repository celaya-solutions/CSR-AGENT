...
When a normal prompt arrives while a session run is already streaming and the queue mode is `steer` (the default, no config needed), OpenAgent tries to send that prompt into the active runtime. OpenAgent and the native Codex app-server harness implement the delivery details differently.
...
Steering does not interrupt a tool call that is already running. The OpenAgent runtime checks at tool-launch boundaries as well as model boundaries:

1. The assistant asks for tool calls.
2. In sequential mode, OpenAgent checks immediately before each call starts, including after asynchronous resolution, validation, and pre-execution hooks.
3. A running call finishes. If a steer is waiting afterward, the unstarted sequential tail is skipped.
4. In parallel mode, OpenAgent prepares calls first, then checks once immediately before launching the prepared calls. Calls that have crossed that checkpoint continue together.
...
6. OpenAgent appends the exact drained steering message before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of OpenAgent runtime's internal steering queue. OpenAgent batches queued prompts for the configured quiet window, then sends a single `turn/steer` request with all collected user input in arrival order. Codex's upstream turn scheduler owns its tool scheduling and consumes accepted steering at the next model boundary; OpenAgent does not add per-tool preemption to that runtime.

Codex review and manual compaction turns reject same-turn steering. When a runtime cannot accept steering in `steer` mode, OpenAgent waits for the active run to finish before starting the prompt.

## Tool launch boundaries

OpenAgent distinguishes started work from requested work:

- A sequential call that is already running completes. Later calls have not started, so OpenAgent returns synthetic skipped results for them and lets the model reconsider with the steer visible.
...
- OpenAgent preserves the runtime's configured steering drain mode and FIFO order. One-at-a-time consumers keep later messages for later boundaries; `all` consumers inject the queued FIFO batch together. Codex receives messages collected during its quiet window as one batched `turn/steer`.
- With `/queue collect`, OpenAgent does not steer. It waits until the active run ends, then creates a followup turn with compatible queued messages after the debounce window.
- With `/queue interrupt`, OpenAgent aborts the active run and starts the newest message instead of steering.
...
An authorized Gateway client can withdraw a message still waiting in the OpenAgent
