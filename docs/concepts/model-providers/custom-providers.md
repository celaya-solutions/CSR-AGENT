...
In onboarding/configure model pickers, the Volcengine auth choice prefers both `volcengine/*` and `volcengine-plan/*` rows. If those models are not loaded yet, OpenAgent falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
...
In onboarding/configure model pickers, the BytePlus auth choice prefers both `byteplus/*` and `byteplus-plan/*` rows. If those models are not loaded yet, OpenAgent falls back to the unfiltered catalog instead of showing an empty provider-scoped picker.
