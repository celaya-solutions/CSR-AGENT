...
OpenAgent ships one **embedded agent runtime**: a built-in agent loop, tool
...
Inside the workspace, OpenAgent expects these user-editable files:
...
After a workspace has been observed, OpenAgent stores its setup state and
...
OpenAgent loads skills from these locations (highest precedence first):
...
The embedded agent runtime is OpenAgent-owned: model discovery, tool wiring,
...
OpenAgent. OpenAgent does not read session folders from other tools.

## Steering while streaming

Inbound prompts that arrive mid-run are steered into the current run by default.
The OpenAgent runtime checks for steering before unstarted tool launches and the
...
explicit `*.streaming.block.enabled: true` to enable block replies.
...
- If you omit the provider, OpenAgent tries an alias first, then a unique
  ...
  configured default model, OpenAgent falls back to the first configured
