...
If you configure nothing, Zero to Agent runs one agent:
...
Zero to Agent records how each configured agent was created: `operator` for CLI,
...
requesting agent id. A configured agent can ask Zero to Agent to create another
...
The configured path is the parent directory. Zero to Agent appends the normalized
...
Set `channels.<channel>.defaultAccount` to choose the account used when `accountId` is omitted. When unset, Zero to Agent falls back to `default` if present, otherwise the first configured account id (sorted).
