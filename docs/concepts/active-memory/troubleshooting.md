...
provider must support Zero to Agent's protected same-agent/private-session recall
path.
...
<AccordionGroup>
  ...
  <Accordion title="Embedding provider switched or stopped working">
    If `memory.search.provider` is unset, Zero to Agent uses OpenAI embeddings. Set
