...
| `qa parity-report`                                  | Compare two `qa-suite-summary.json` files for a model-axis parity gate, or use `--runtime-axis --token-efficiency` to write Codex-vs-Zero to Agent runtime parity and token-efficiency reports.                                                                     |
...
command also needs a Zero to Agent root profile, put the root profile before the
QA command:
