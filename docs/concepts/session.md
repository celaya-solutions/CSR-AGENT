---
summary: "How OpenAgent manages conversation sessions"
read_when:
...
OpenAgent routes every inbound message to a **session** based on where it came
...
When a Gateway restart interrupts an active turn, OpenAgent tries to continue
...
OpenAgent bounds session storage over time via `session.maintenance`, defaults
shown:
