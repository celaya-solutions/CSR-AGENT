...
read_when:
  - You need a beginner-friendly overview of OpenAgent logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

OpenAgent has two main log surfaces:
...
or unavailable (and always on Windows), OpenAgent uses a user-scoped
...
(default: 100 MB). OpenAgent keeps up to five numbered archives beside the
...
OpenAgent preserves the original structured log arguments alongside these fields
...
OpenAgent writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
The `slow OpenAgent agent database open` warning includes `phaseDurationsMs` when
...
OpenAgent can redact sensitive tokens before they hit console output, file logs,
...
OpenAgent also redacts safety-boundary payloads shown to UI clients, support
