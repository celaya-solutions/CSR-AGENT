...
summary: "Architecture and product boundary for supervising native Codex sessions from Zero to Agent."
read_when:
...
Codex supervision lets a Zero to Agent operator discover native Codex sessions,
create a Gateway-local branch, or continue an eligible paired-node thread
through the normal Zero to Agent Chat surface.
Codex App Server remains the thread and model-loop owner. Zero to Agent supplies the
...
- **Gateway-local branch**: the new supervised Chat that Zero to Agent creates from a
...
- **Canonical full Codex harness thread**: that canonical thread once it runs
  with Zero to Agent's full harness tool surface; every later model turn runs on it.
- **Supervised model-locked Chat**: a Zero to Agent Chat bound to a supervised
...
- native thread bindings to Zero to Agent sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
Zero to Agent Codex plugin -> supervision App Server connection
...
Ordinary Zero to Agent Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> Zero to Agent Chat and channel delivery
```
...
process-local; a thread unknown to Zero to Agent's supervision process is `notLoaded`
...
They do not import or replace credentials in either Zero to Agent's auth store or the
...
unbounded `thread/read`. Zero to Agent also rejects any serialized item page above
...
eventually discovered without importing them into Zero to Agent storage.

Catalog discovery is passive. Listing or reading metadata must not call
`thread/resume`, subscribe the Zero to Agent client to live thread requests, or
answer an approval.
...
The notice supersedes only earlier Zero to Agent generic policy. Independent managed,
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   Zero to Agent Chat and records a pending harness branch.
3. Stores the pending Codex-only model-lock policy, not a concrete model or
   ...
   returns the Zero to Agent `sessionKey`.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, Zero to Agent's cwd, policy, config, environment, the
   full Zero to Agent harness tool surface, and exactly the model and provider
...
omit Zero to Agent model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, Zero to Agent accepts
that native persisted selection. The outer Zero to Agent model and fallback chain
...
Deleting or resetting the Zero to Agent entry would otherwise discard the native
...
canonical harness thread on the first turn prevents Zero to Agent from becoming a
...
Atlas, or ChatGPT source remains eligible for both native and Zero to Agent catalogs.
...
known active Zero to Agent binding owner for the exact target or any non-archived
spawned descendant also rejects archive. Zero to Agent paginates Codex's experimental
...
the owner/admin check and are blocked while Zero to Agent sandboxing is active.

The node runner rejects overlapping Zero to Agent resume turns for the same thread
...
- audited handoff between Codex Desktop, CLI, and Zero to Agent
...
- Later resumes omit Zero to Agent model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer Zero to Agent model or fallback chain.
