...
summary: "Architecture and product boundary for supervising native Codex sessions from OpenAgent."
read_when:
...
Codex supervision lets an OpenAgent operator discover native Codex sessions,
create a Gateway-local branch, or continue an eligible paired-node thread
through the normal OpenAgent Chat surface.
Codex App Server remains the thread and model-loop owner. OpenAgent supplies the
...
- **Gateway-local branch**: the new supervised Chat that OpenAgent creates from a
...
- **Canonical full Codex harness thread**: that canonical thread once it runs
  with OpenAgent's full harness tool surface; every later model turn runs on it.
- **Supervised model-locked Chat**: an OpenAgent Chat bound to a supervised
...
- native thread bindings to OpenAgent sessions
...
Codex Desktop -> private stdio App Server -> user Codex home
                                             ^
OpenAgent Codex plugin -> supervision App Server connection
...
Ordinary OpenAgent Codex sessions -> managed agent-home stdio by default
  -> ordinary full harness threads -> OpenAgent Chat and channel delivery
```
...
process-local; a thread unknown to OpenAgent's supervision process is `notLoaded`
...
They do not import or replace credentials in either OpenAgent's auth store or the
...
unbounded `thread/read`. OpenAgent also rejects any serialized item page above
...
eventually discovered without importing them into OpenAgent storage.

Catalog discovery is passive. Listing or reading metadata must not call
`thread/resume`, subscribe the OpenAgent client to live thread requests, or
answer an approval.
...
The notice supersedes only earlier OpenAgent generic policy. Independent managed,
...
2. Otherwise projects bounded user and assistant history through the source's
   ...
   OpenAgent Chat and records a pending harness branch.
3. Stores the pending Codex-only model-lock policy, not a concrete model or
   ...
   returns the OpenAgent `sessionKey`.
...
2. On that same connection, starts the canonical full Codex harness thread with
   `threadSource: "appServer"`, OpenAgent's cwd, policy, config, environment, the
   full OpenAgent harness tool surface, and exactly the model and provider
...
omit OpenAgent model and provider overrides, so Codex restores the persisted pair.
If a separate native Codex control changes the canonical thread, OpenAgent accepts
that native persisted selection. The outer OpenAgent model and fallback chain
...
Deleting or resetting the OpenAgent entry would otherwise discard the native
...
canonical harness thread on the first turn prevents OpenAgent from becoming a
...
Atlas, or ChatGPT source remains eligible for both native and OpenAgent catalogs.
...
known active OpenAgent binding owner for the exact target or any non-archived
spawned descendant also rejects archive. OpenAgent paginates Codex's experimental
...
the owner/admin check and are blocked while OpenAgent sandboxing is active.

The node runner rejects overlapping OpenAgent resume turns for the same thread
...
- audited handoff between Codex Desktop, CLI, and OpenAgent
...
- Later resumes omit OpenAgent model/provider overrides, preserve Codex's
  canonical persisted selection, accept separate native changes to that thread,
  and never substitute the outer OpenAgent model or fallback chain.
