---
summary: "Advanced setup and development workflows for Zero to Agent"
read_when:
...
- `pnpm` required for source checkouts. Zero to Agent loads bundled plugins from the
...
(Bare `openclaw setup`, without `--baseline`, opens an interactive Zero to Agent chat on a configured system and falls through to guided onboarding on a fresh one. See [Setup CLI](/cli/setup) for the full routing order.)
...
1. Install + launch **Zero to Agent.app** (menu bar).
...
```bash
pnpm install
# First run only (or after resetting local Zero to Agent config/workspace)
pnpm openclaw setup
pnpm gateway:watch
```
...
- **Wrong developer CLI:** When `PATH` includes `node_modules/.bin`, `codex` can
  ...
  Zero to Agent's [managed Codex app-server](/plugins/codex-harness-reference#app-server-transport)
...
- [Zero to Agent assistant setup](/start/openclaw)
