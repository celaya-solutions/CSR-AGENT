---
summary: "Advanced setup and development workflows for OpenAgent"
read_when:
...
- `pnpm` required for source checkouts. OpenAgent loads bundled plugins from the
...
not imply support for its bundled npm as a source resolver. Source installs use pnpm.
...
- **Wrong port:** Gateway WS defaults to `ws://127.0.0.1:18789`; keep clients and CLI on the same port.
- **Wrong developer CLI:** When `PATH` includes `node_modules/.bin`, `codex` can
  ...
  OpenAgent's [managed Codex app-server](/plugins/codex-harness-reference#app-server-transport)
