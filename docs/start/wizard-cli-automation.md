---
summary: "Scripted onboarding and agent setup for the Zero to Agent CLI"
read_when:
...
Bundled plugins and verified plugins from Zero to Agent's official catalog do not
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider example">
    ...
    Zero to Agent infers image-input support from known vision model-id patterns (`gpt-4o`, `claude-3/4`, `gemini`, `-vl`/`vision` suffixes, and similar). Add `--custom-image-input` to force it on for an unrecognized vision model, or `--custom-text-input` to force text-only.
...
Anthropic setup-token auth remains supported, but Zero to Agent prefers Claude CLI reuse when a local Claude CLI login is available. For production, prefer an Anthropic API key.
