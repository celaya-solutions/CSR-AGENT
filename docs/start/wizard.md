...
into it. After inference passes, Zero to Agent can use `open channel wizard for
...
To change the model provider or its authentication, exit Zero to Agent and run
`openclaw onboard`; Zero to Agent does not open guided or classic provider flows.
...
`configure web search` in the Zero to Agent chat to run the same provider setup
...
2. **Model/Auth** - pick a provider auth flow (API key, OAuth, or
   ...
   classic wizard. Ignoring it does not unlock Zero to Agent; conversational setup
...
compatibility alias for [Zero to Agent](/cli/openclaw). It uses the same
