---
summary: "First-run setup flow for OpenAgent (macOS app)"
read_when:
...
The first time you open OpenAgent.app, macOS asks you to approve a downloaded
app. Click **Open** to continue.

<Frame>
<img src="/assets/macos-onboarding/01-macos-warning.jpeg" alt="macOS dialog that asks whether to open the downloaded OpenAgent app" />
</Frame>
</Step>
<Step title="Approve find local networks">
macOS then asks whether OpenAgent may find devices on your local network.
Click **Allow**. The app uses this to reach a Gateway on another machine.

<Frame>
<img src="/assets/macos-onboarding/02-local-networks.jpeg" alt="macOS dialog that asks whether OpenAgent may find devices on the local network" />
</Frame>
</Step>
...
<img src="/assets/macos-onboarding/03-security-notice.png" alt="OpenAgent welcome screen with the security notice" />
</Frame>

Security trust model:

- By default, OpenAgent is a personal agent: one trusted operator boundary.
...
Select the connection you want before OpenAgent saves any returned credential and runs one
...
The provider picker is built from installed manifests and OpenAgent's official
...
alias, OpenAgent tests and saves the provider's canonical model name while
...
onboarding in the dashboard. OpenAgent helps configure the remaining workspace,
...
during the agent's first real turn. OpenAgent remains available later under
**Settings → OpenAgent**.
</Step>
</Steps>
