...
summary: "How OpenAgent structures the built-in agent runtime: code layout, boundaries, resource manifests, and runtime selection."
---

OpenAgent owns the built-in agent runtime. Runtime code lives under `src/agents/`, model/provider transport lives under `src/llm/`, and `openclaw/plugin-sdk/*` barrels expose the plugin-facing contracts.
...
| `src/agents/runtime/`               | OpenAgent facade that wires `@openclaw/agent-core` to the plugin SDK LLM runtime and re-exports it plus local proxy utilities.                                                                                        |
| `src/agents/agent-tools*.ts`        | OpenAgent-owned tool definitions, parameter schemas, tool policy, before/after tool-call adapters, and host/sandbox edit tools.                                                                                       |
...
Core calls the built-in runtime through OpenAgent modules and SDK barrels. No external agent framework packages remain. Plugins use documented `openclaw/plugin-sdk/*` entrypoints and do not import `src/**` internals.
...
Resource packages declare OpenAgent resources in `package.json` metadata. Entries are file paths or globs relative to the package root:
...
- `auto` selects a registered plugin harness that supports the effective provider route, otherwise the built-in OpenAgent runtime. A provider or model prefix alone never selects a harness.
...
- [OpenAgent agent runtime workflow](/openclaw-agent-runtime)
