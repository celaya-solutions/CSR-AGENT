---
summary: Machine-checked security models for Zero to Agent's highest-risk paths.
...
Zero to Agent's formal security models (TLA+/TLC today) cover specific highest-risk paths: authorization, session isolation, tool gating, and misconfiguration safety. For each path, the models give a machine-checked argument that it enforces its intended policy, under explicit stated assumptions.
...
This is **not** a proof that Zero to Agent is secure in all respects, and it does not verify the full TypeScript implementation.
...
<Note>
That repository is currently unreachable (GitHub returns "Repository not found" as of this writing). If it is still broken for you, ask in the Zero to Agent maintainer channels for the current location before assuming the models were removed.
</Note>
...
Reproduction instructions are unavailable while the previously documented models repository is not publicly reachable. Ask in the Zero to Agent maintainer channels for a verified current location before attempting the targets below.
