...
read_when:
  - Letting an external service call Zero to Agent over HTTP
...
Gateway HTTP hooks: how an external service calls Zero to Agent to wake an agent or submit an agent turn. Part of the [Automations](/automation/cron-jobs) guide.
...
outbound automation webhook delivery: here, the external service calls Zero to Agent.
