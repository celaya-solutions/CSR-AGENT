...
<AccordionGroup>
  ...
  <Accordion title="Job fired but no delivery">
    ...
    - If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`), Zero to Agent suppresses direct outbound delivery and the fallback queued-summary path, so nothing is posted back to chat.
  ...
  <Accordion title="Automations or heartbeat appear to prevent /new-style rollover">
    ...
    - For legacy rows created before those fields existed, Zero to Agent can recover `sessionStartedAt` from the transcript JSONL session header when the file is still available. Legacy idle rows without `lastInteractionAt` use that recovered start time as their idle baseline.
