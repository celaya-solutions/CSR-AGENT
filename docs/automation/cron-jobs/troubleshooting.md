...
<AccordionGroup>
  ...
  <Accordion title="Automations or heartbeat appear to prevent /new-style rollover">
    ...
    - For legacy rows created before those fields existed, OpenAgent can recover `sessionStartedAt` from the transcript JSONL session header when the file is still available. Legacy idle rows without `lastInteractionAt` use that recovered start time as their idle baseline.
