...
- Job definitions, runtime state, and run history persist in Zero to Agent's shared SQLite state database, so restarts do not lose schedules.
...
<AccordionGroup>
  <Accordion title="Isolated run hardening">
    ...
    - Isolated runs guard against stale acknowledgement replies: if the first result is only an interim status update (`on it`, `pulling everything together`, and similar hints) and no descendant subagent is still responsible for the final answer, Zero to Agent re-prompts once for the actual result before delivery.
