...
read_when:
  - You need to check which Node.js versions OpenAgent supports
...
The **SQLite WAL-reset corruption bug** requires a safe loaded library: SQLite **3.51.3+**, **3.50.7+ within 3.50.x**, or **3.44.6+ within 3.44.x**. OpenAgent validates the library actually loaded because Node builds linked to shared system SQLite can use a different version from Node's own metadata.
