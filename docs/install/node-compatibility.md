...
read_when:
  - You need to check which Node.js versions Zero to Agent supports
  - You want to understand a Node.js minimum version or platform support change
---

This reference covers supported Node.js lines, why the minimum versions exist, and how they changed across Zero to Agent releases. For installation steps, see [Node.js](/install/node); for macOS companion app requirements, see [macOS](/platforms/macos).
...
The **SQLite WAL-reset corruption bug** requires a safe loaded library: SQLite **3.51.3+**, **3.50.7+ within 3.50.x**, or **3.44.6+ within 3.44.x**. Zero to Agent validates the library actually loaded because Node builds linked to shared system SQLite can use a different version from Node's own metadata.
...
On RPM-based distributions, the installer preserves a supported distro-owned Node package that links unsafe system SQLite and provisions a separate user-space runtime for Zero to Agent.
