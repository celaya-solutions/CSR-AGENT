---
summary: "Deploy Zero to Agent on Render with Infrastructure-as-Code"
read_when:
  - Deploying Zero to Agent to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Zero to Agent on [Render](https://render.com) using the repo's `render.yaml` Blueprint. It declares the service, disk, and environment variables in one file.
...
The Blueprint defaults to `starter`. To use the free tier, change `plan: free` **and delete the `disk:` block** in your fork's `render.yaml`; Render rejects a Blueprint that attaches a persistent disk to a free instance. Without that disk, Zero to Agent state resets on every deploy.
...
- **Vertical**: change the plan for more CPU/RAM. Usually sufficient for Zero to Agent.
- **Horizontal**: increase instance count (Standard plan and above). Requires sticky sessions or external state management since Zero to Agent keeps runtime state on the local disk.
