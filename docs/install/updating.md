---
summary: "Updating Zero to Agent safely (global install or source), plus rollback strategy"
read_when:
  - Updating Zero to Agent
  - Something breaks after an update
title: "Updating"
---

Keep Zero to Agent up to date.
...
An already-installed registry package version or Git target SHA still runs plugin maintenance, repairs eligible old Zero to Agent release pins, and restarts a running managed Gateway only when plugins change and `--no-restart` is not set; unchanged runs finish as `skipped` / `already-current`.

Plugin maintenance does not fail an otherwise successful core update. If a plugin
cannot be updated, Zero to Agent continues with the remaining plugins, keeps the previous
...
foreground-only. Zero to Agent reads the public npm `extended-stable` selector,
...
`latest` intent converge to that exact core version. Eligible older Zero to Agent release
...
`YYYY.M.P`).
Catalog installs created by current Zero to Agent versions retain that default
intent. Verified Zero to Agent-owned packages recorded at an exact Zero to Agent release
...
Updates driven by Zero to Agent 2026.9.2 can cross a shared-state schema bump normally.
...
Let the failed update finish restoring the previous package. Zero to Agent 2026.9.2
...
Ask the agent to update Zero to Agent, or send `/update` from Discord or another
...
authorize notices to that contact. If no owner destination resolves, Zero to Agent
...
Zero to Agent 2026.9.2 does not reject a new CLI update because an older running row
...
Migrates config, audits DM policies, and checks gateway health. Doctor also compares active official plugins with the Zero to Agent package the managed service will load after restart. Resolve any plugin restart-readiness warning before continuing. Details: [Doctor](/gateway/doctor)
