---
summary: "Run OpenAgent Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want OpenAgent running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on OpenAgent Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

Set up an Azure Linux VM with the Azure CLI, apply Network Security Group (NSG) hardening, configure Azure Bastion for SSH access, and install OpenAgent.
...
- Install OpenAgent with the installer script
- Verify the gateway
...
<Steps>
  ...
  <Step title="Install OpenAgent (in the VM shell)">
    ```bash
    ...
    The installer installs Node and dependencies if not already present, installs OpenAgent, and launches onboarding. See [Install](/install) for details.
...
- More detail on Azure deployment with the GitHub Copilot model provider: [OpenAgent on Azure with GitHub Copilot](https://github.com/johnsonshi/openclaw-azure-github-copilot)
