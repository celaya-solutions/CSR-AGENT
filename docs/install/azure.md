---
summary: "Run Zero to Agent Gateway 24/7 on an Azure Linux VM with durable state"
read_when:
  - You want Zero to Agent running 24/7 on Azure with Network Security Group hardening
  - You want a production-grade, always-on Zero to Agent Gateway on your own Azure Linux VM
  - You want secure administration with Azure Bastion SSH
title: "Azure"
---

Set up an Azure Linux VM with the Azure CLI, apply Network Security Group (NSG) hardening, configure Azure Bastion for SSH access, and install Zero to Agent.
...
- Install Zero to Agent with the installer script
- Verify the gateway
...
<Steps>
  ...
  <Step title="Install Zero to Agent (in the VM shell)">
    ```bash
    ...
    The installer installs Node and dependencies if not already present, installs Zero to Agent, and launches onboarding. See [Install](/install) for details.
...
- More detail on Azure deployment with the GitHub Copilot model provider: [Zero to Agent on Azure with GitHub Copilot](https://github.com/johnsonshi/openclaw-azure-github-copilot)
