---
summary: "Host OpenAgent on a DigitalOcean Droplet"
read_when:
  - Setting up OpenAgent on DigitalOcean
  - Looking for a simple paid VPS for OpenAgent
title: "DigitalOcean"
---

Run a persistent OpenAgent Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Install OpenAgent; run onboarding later as the non-root owner.
    curl -fsSL https://openclaw.ai/install.sh | bash -s -- --no-onboard

    # Create the non-root user that will own OpenAgent state and services.
    adduser openclaw
    ...
    Use the root shell only for system bootstrap. Run OpenAgent commands as the non-root `openclaw` user so state lives under `/home/openclaw/.openclaw/` and the Gateway installs as that user's systemd `--user` service.
...
DigitalOcean snapshots back up the whole Droplet. OpenAgent archives can be
