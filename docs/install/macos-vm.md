---
summary: "Run OpenAgent in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want OpenAgent isolated from your main macOS environment
...
Run OpenAgent in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume). This gives you:
...
Once you have SSH access to a macOS VM, continue at [Install OpenAgent](#6-install-openclaw) below.
...
5. SSH in, install OpenAgent, configure channels.
6. Done.
...
The VM runs in the background; OpenAgent's daemon keeps the gateway running. To check status:
...
This is the killer feature of running on macOS. Use [iMessage](/channels/imessage) with `imsg` to add Messages to OpenAgent.

Inside the VM:

1. Sign in to Messages.
2. Install `imsg`.
3. Grant Full Disk Access and Automation permission for the process running OpenAgent/`imsg`.
4. Verify RPC support with `imsg rpc --help`.

Add to your OpenAgent config:
