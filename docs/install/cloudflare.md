...
read_when:
  - You want to run Zero to Agent on Cloudflare Containers
...
Run one Zero to Agent installation behind a Cloudflare Worker and a named Durable Object, with the official Zero to Agent image and Litestream replication to R2.

<Warning>
  This deployment target is experimental. Litestream protects SQLite databases, not the complete Zero to Agent state directory. Read [Limits and recovery](#limits-and-recovery) before using production credentials.
</Warning>
...
- Node.js and npm
- Provider and channel credentials for your Zero to Agent setup
...
The Worker forwards every HTTP and WebSocket request to one stable Durable Object name. That Durable Object owns one Container instance and is the single-writer fence around the Litestream replica. The Container exposes Zero to Agent on port `8080`, and the Durable Object polls `/healthz` there before routing to it.
...
flowchart TD
    ...
    container[Container running the Zero to Agent Gateway on 8080]
...
<Steps>
  <Step title="Prepare the template">
    Clone Zero to Agent and enter the template directory:
  ...
  <Step title="Bootstrap Zero to Agent">
...
Verify current rates on [Cloudflare's Containers pricing page](https://developers.cloudflare.com/containers/pricing/) before committing; these figures are estimates from the published rate card and change independently of Zero to Agent.
...
- **Ephemeral disk:** every sleep, replacement, or host restart starts from the image plus the restored SQLite databases. Use [full Zero to Agent archives](/install/backups#full-archives) for config, credential files, plugin files, and workspaces.
...
- **Provider boundary:** this is a deployment template, not a Zero to Agent `cloudWorkers` provider. Its operator SSH access does not implement that provider's SSH execution contract.

## Update

Build a new derived image from a new immutable official Zero to Agent digest, push it, update the derived digest in `wrangler.jsonc`, and deploy:
