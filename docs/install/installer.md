...
read_when:
  ...
  - You want to install a private Node runtime without reinstalling Zero to Agent
title: "Installer internals"
---

Zero to Agent ships three installer scripts, served from `openclaw.ai`.

| Script                             | Platform             | What it does                                                                                        |
| ---------------------------------- | -------------------- | --------------------------------------------------------------------------------------------------- |
| [`install.sh`](#installsh)         | macOS / Linux / WSL  | Installs Node if needed, installs Zero to Agent via npm (default) or git, can run onboarding.       |
| [`install-cli.sh`](#install-clish) | macOS / Linux / WSL  | Installs Node + Zero to Agent into a local prefix (`~/.openclaw`) via npm or git. No root required. |
| [`install.ps1`](#installps1)       | Windows (PowerShell) | Installs Node if needed, installs Zero to Agent via npm (default) or git, can run onboarding.       |
...
Before changing packages, every installer probes the exact npm executable it will use. npm 11.15 and earlier installs normally; npm 11.16 and later, including npm 12, receives `--allow-scripts` for only the npm-resolved Zero to Agent candidate identity. An unreadable npm version stops before package mutation. A remaining `.openclaw-lifecycle-pending` marker or legacy `dist/openclaw-install-guard` makes the install fail instead of reporting a lifecycle-skipped package as successful.
...
When the active Node.js is unsupported, the CLI can offer `Update NodeJS: Y/N [N]:` before loading Zero to Agent. Enter **Y** to install a checksum-verified private runtime and retry the same command. Provisioning leaves system Node.js, shell settings, Zero to Agent packages, and Gateway services unchanged; the retried command keeps its normal behavior. Enter **N**, press Enter, or cancel to receive manual upgrade instructions.
...
<Steps>
  ...
  <Step title="Ensure a supported Node.js runtime">
    Checks the Node version and linked SQLite library, then installs Node if needed (Node 26 through Homebrew `node` on macOS; Node 24 LTS through NodeSource setup scripts on Linux apt/dnf/yum). On RPM-based Linux, a supported distro Node that links unsafe SQLite remains installed while Zero to Agent receives a user-space Node runtime. On macOS, Homebrew is installed only when the installer needs it for Node or Git. Node 24.16+ and Node 26.1+ are supported; Node 22, 23, and 25 are unsupported.
  ...
  <Step title="Install Zero to Agent">
...
If run inside a Zero to Agent checkout (`package.json` + `pnpm-workspace.yaml`), the script offers:
...
| `OPENCLAW_HOME=<path>`                            | Base directory for Zero to Agent state and default git/onboarding paths |
...
<Steps>
  ...
  <Step title="Install Zero to Agent under prefix">
...
With `--node-only`, `install-cli.sh` stops after provisioning Node into `<prefix>/tools/node-v<version>` and updating the `<prefix>/tools/node` alias. It skips Git, Zero to Agent installation, onboarding, and Gateway service work. This mode refuses musl Linux before any system package-manager changes.
...
| `--version <ver>`                       | Zero to Agent version or dist-tag (default: `latest`)                             |
...
| `OPENCLAW_INSTALL_METHOD=git\|npm`          | Install method                                                          |
| `OPENCLAW_VERSION=<ver>`                    | Zero to Agent version or dist-tag                                       |
...
<Steps>
  ...
  <Step title="Ensure a supported Node.js runtime">
    If missing, attempts install via winget, then Chocolatey, then Scoop. If no package manager is available, the script downloads the official Node.js 26 Windows zip into `%LOCALAPPDATA%\Zero to Agent\deps\portable-node` and adds it to the current process and user PATH. Node 24.16+ and Node 26.1+ are supported; Node 22, 23, and 25 are unsupported.
  </Step>
  <Step title="Install Zero to Agent">
    ...
    - `git` method: clone/update repo, install/build with pnpm, and install wrapper at `%USERPROFILE%\.local\bin\openclaw.cmd`. If Git is missing, the script bootstraps user-local MinGit under `%LOCALAPPDATA%\Zero to Agent\deps\portable-git` and adds it to the current process and user PATH.
