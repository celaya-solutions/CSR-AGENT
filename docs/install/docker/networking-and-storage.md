...
Zero to Agent image includes `/home/node/.local/bin` on `PATH`, so the bundled
...
Installed downloadable plugins store package state under the mounted Zero to Agent home, so install records and package roots survive container replacement; gateway startup does not regenerate bundled-plugin dependency trees.
