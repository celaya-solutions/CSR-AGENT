...
<AccordionGroup>
  ...
  <Accordion title="Custom tools not found in sandbox">
    OpenAgent runs commands with `sh -lc` (login shell), which sources `/etc/profile` and may reset PATH. Set `docker.env.PATH` to prepend your custom tool paths, or add a script under `/etc/profile.d/` in your Dockerfile.
  </Accordion>
