...
source line instead. Keep your Zero to Agent state, credentials, workspace, project
`.env`, and volumes.
...
[Update Zero to Agent](/install/docker-vm-runtime#update-openclaw) for image updates.
...
<AccordionGroup>
  ...
  <Accordion title="Permissions and EACCES">
    ...
    The same mismatch can show up as `blocked plugin candidate: suspicious ownership (... uid=1000, expected uid=0 or root)` followed by `plugin present but blocked` — the process uid and the mounted plugin directory owner disagree. Prefer running as the default uid 1000 and fixing the bind mount ownership. Only chown `/path/to/openclaw-config/npm` to `root:root` if you intentionally run Zero to Agent as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    5. **Persist browser downloads and caches**: use `OPENCLAW_HOME_VOLUME` or `OPENCLAW_EXTRA_MOUNTS`. Zero to Agent auto-detects the image's Playwright-managed Chromium on Linux.
...
The `latest*`, `main*`, and `extended-stable*` moving tags are rebuilt weekly from the same tagged release source so they pick up current OS security updates between Zero to Agent releases. Stable and extended-stable refreshes remain separate, and beta images are not rebuilt on this schedule.
