---
summary: "Optional Docker-based setup and onboarding for Zero to Agent"
read_when:
...
- Zero to Agent auto-detects the image's Playwright-managed Chromium on Linux. An
...
When you replace the Zero to Agent image but keep the same mounted state/config, the
...
already part of the root Zero to Agent runtime, so the ClickClack image selects only
...
separate x86_64 appliance archive containing a Zero to Agent npm tarball and pins
the Node, archive, and manifest digests. Build that appliance independently
from the same landed Zero to Agent source.
