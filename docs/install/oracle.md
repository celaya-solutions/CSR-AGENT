---
summary: "Host OpenAgent on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up OpenAgent on Oracle Cloud
  - Looking for free VPS hosting for OpenAgent
  - Want 24/7 OpenAgent on a small server
title: "Oracle Cloud"
---

Run a persistent OpenAgent Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
- `openclaw security audit` for an OpenAgent-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most OpenAgent features work fine; a small number of native binaries need ARM builds:
