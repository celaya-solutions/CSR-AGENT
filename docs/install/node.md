---
summary: "Install and configure Node.js for OpenAgent - version requirements, install options, and PATH troubleshooting"
title: "Node.js"
read_when:
...
Upgrade Node before updating OpenAgent to avoid SQLite TEXT truncation. See [Node.js compatibility](/install/node-compatibility) for the SQLite safety floors and macOS/ARMv7 support limits.
...
OpenAgent loads any `.env` file. Configure version-manager roots in your shell environment;
...
directory. OpenAgent's own private recovery directory is also allowed, so cached
...
OpenAgent prints the code page and continues searching other sources. Unsupported
...
Enter **Y** to download a compatible Node.js for OpenAgent and retry the same command. The download is checksum-verified and stored under `~/.openclaw/tools/cli-node` (or the home selected by `OPENCLAW_HOME`). The Node.js installation does not replace system Node.js, change shell settings, reinstall OpenAgent, or repair/restart Gateway services. The retried command keeps its normal behavior.
