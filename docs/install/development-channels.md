...
Zero to Agent ships four update channels:
...
| `extended-stable` | resolves the public npm `extended-stable` selector, verifies the exact selected package, and installs that exact version. Fails closed with no fallback to `latest`, `beta`, or `dev`. | unsupported: Zero to Agent leaves the checkout unchanged and asks you to use a package installation |
...
For these Git tag updates, Zero to Agent refreshes branches without adding force to
...
- Downgrade protection: if the target version is older than the current
  version, Zero to Agent prompts for confirmation (skip with `--yes`).
