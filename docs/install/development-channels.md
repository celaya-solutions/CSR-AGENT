...
OpenAgent ships four update channels:
...
| `extended-stable` | resolves the public npm `extended-stable` selector, verifies the exact selected package, and installs that exact version. Fails closed with no fallback to `latest`, `beta`, or `dev`. | unsupported: OpenAgent leaves the checkout unchanged and asks you to use a package installation |
...
For these Git tag updates, OpenAgent refreshes branches without adding force to
...
- Downgrade protection: if the target version is older than the current
  version, OpenAgent prompts for confirmation (skip with `--yes`).
