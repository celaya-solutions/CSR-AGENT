---
summary: "Recover from failed Zero to Agent updates in the Control UI or CLI"
read_when:
  - A Zero to Agent update failed
...
interactive terminal, Zero to Agent shows the selected agent, saved prompt path when
...
In the Control UI, a failed attempt opens **Ask Zero to Agent** with its recorded
...
diagnostic request. Ask Zero to Agent manually or run `openclaw triage` on the host.
...
and **Ask Zero to Agent**. It previews a bounded report containing the Zero to Agent
...
until an administrator confirms that preview. Zero to Agent then uses the existing
GitHub CLI issue flow. Fallback and pending outcomes retain the sanitized report
locally; a confirmed issue keeps only its durable issue URL. Zero to Agent first makes
...
- Zero to Agent version and install type;
