---
summary: "Recover from failed OpenAgent updates in the Control UI or CLI"
read_when:
  - An OpenAgent update failed
...
interactive terminal, OpenAgent shows the selected agent, saved prompt path when
...
In the Control UI, a failed attempt opens **Ask OpenAgent** with its recorded
...
and **Ask OpenAgent**. It previews a bounded report containing the OpenAgent
...
until an administrator confirms that preview. OpenAgent then uses the existing
GitHub CLI issue flow. Fallback and pending outcomes retain the sanitized report
locally; a confirmed issue keeps only its durable issue URL. OpenAgent first makes
