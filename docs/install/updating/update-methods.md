...
read_when:
  ...
  - You need to update or recover Zero to Agent with npm, pnpm, or bun directly
...
credentials, and workspace in `~/.openclaw`; it only changes which Zero to Agent
...
Node version during `preinstall`; Zero to Agent verifies the packaged `dist` inventory
...
npm 11.16+, it approves only the candidate Zero to Agent lifecycle; transitive
dependency scripts remain unapproved.
This avoids npm overlaying a new package onto stale files from the old one. If
the install command fails, Zero to Agent retries once with `--omit=optional`, which
...
Zero to Agent-managed npm update and plugin-update commands also clear npm's
...
explicit Zero to Agent update means "install the selected release now."

```bash
pnpm add -g --allow-build=openclaw openclaw@latest
```

If pnpm 11 installed Zero to Agent 2026.7.1, run that manual command once. That
...
or when the invoking package is orphaned or not the only active Zero to Agent
install there.

pnpm 12 retains the `global/v11` layout; the layout number does not need to match
the pnpm CLI major version.
Staged pnpm updates isolate both the global project and its launchers. Zero to Agent
...
If Zero to Agent shares a pnpm global install group with another package, the
...
`--trust` allows Zero to Agent's lifecycle scripts. The canonical `openclaw update`
path applies the same Zero to Agent-only Bun trust when it owns the install.
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    After package lifecycle completion, Zero to Agent treats packaged global installs as read-only at runtime, even when the global package directory is writable by the current user. Plugin package installs live in Zero to Agent-owned npm/git roots under the user config directory, and Gateway startup does not mutate the Zero to Agent package tree.

    Some Linux npm setups install global packages under root-owned directories such as `/usr/lib/node_modules/openclaw`. Zero to Agent supports that layout because plugin install/update commands write outside that global package directory.

  </Accordion>
  <Accordion title="Hardened systemd units">
    Give Zero to Agent write access to its config/state roots so explicit plugin installs, plugin updates, and doctor cleanup can persist their changes:
