...
read_when:
  ...
  - You need to update or recover OpenAgent with npm, pnpm, or bun directly
...
credentials, and workspace in `~/.openclaw`; it only changes which OpenAgent
...
Node version during `preinstall`; OpenAgent verifies the packaged `dist` inventory
...
npm 11.16+, it approves only the candidate OpenAgent lifecycle; transitive
dependency scripts remain unapproved.
This avoids npm overlaying a new package onto stale files from the old one. If
the install command fails, OpenAgent retries once with `--omit=optional`, which
...
explicit OpenAgent update means "install the selected release now."

```bash
pnpm add -g --allow-build=openclaw openclaw@latest
```

If pnpm 11 installed OpenAgent 2026.7.1, run that manual command once. That
...
or when the invoking package is orphaned or not the only active OpenAgent
install there.

pnpm 12 retains the `global/v11` layout; the layout number does not need to match
the pnpm CLI major version.
Staged pnpm updates isolate both the global project and its launchers. OpenAgent
...
If OpenAgent shares a pnpm global install group with another package, the
...
`--trust` allows OpenAgent's lifecycle scripts. The canonical `openclaw update`
path applies the same OpenAgent-only Bun trust when it owns the install.
...
<AccordionGroup>
  <Accordion title="Read-only package tree">
    After package lifecycle completion, OpenAgent treats packaged global installs as read-only at runtime, even when the global package directory is writable by the current user. Plugin package installs live in OpenAgent-owned npm/git roots under the user config directory, and Gateway startup does not mutate the OpenAgent package tree.

    Some Linux npm setups install global packages under root-owned directories such as `/usr/lib/node_modules/openclaw`. OpenAgent supports that layout because plugin install/update commands write outside that global package directory.

  </Accordion>
  <Accordion title="Hardened systemd units">
    Give OpenAgent write access to its config/state roots so explicit plugin installs, plugin updates, and doctor cleanup can persist their changes:
