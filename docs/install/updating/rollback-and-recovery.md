...
backup with its matching Zero to Agent release.
...
- The matching Zero to Agent package version or source revision and built runtime.
...
`↩️ Zero to Agent update rolled back to <previous>: <reason>`, retaining the original
...
The archive manifest records the Zero to Agent version and the source paths included
...
handoff. Before launch, Zero to Agent shows the agent, saved prompt path when available,
...
its Zero to Agent state, including running Doctor lint, `doctor --fix`, and health
