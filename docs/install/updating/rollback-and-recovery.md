...
- The matching OpenAgent package version or source revision and built runtime.
...
`↩️ OpenAgent update rolled back to <previous>: <reason>`, retaining the original
...
handoff. Before launch, OpenAgent shows the agent, saved prompt path when available,
...
its OpenAgent state, including running Doctor lint, `doctor --fix`, and health
