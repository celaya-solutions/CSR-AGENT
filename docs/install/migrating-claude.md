---
summary: "Move Claude Code and Claude Desktop local state into OpenAgent with a previewed import"
read_when:
  - You are coming from Claude Code or Claude Desktop and want to keep instructions, MCP servers, and skills
  - You need to understand what OpenAgent imports automatically and what stays archive-only
title: "Migrating from Claude"
---

OpenAgent imports local Claude state through the bundled Claude migration provider. The provider previews every item before changing state and redacts secrets in plans and reports. Standalone `openclaw migrate` creates a verified backup; the fresh onboarding path stages the import and publishes it only after verification succeeds.

<Note>
Onboarding imports require a fresh OpenAgent setup. If you already have local OpenAgent state, reset config, credentials, sessions, and the workspace first, or use `openclaw migrate` directly with `--overwrite` after reviewing the plan.
</Note>
...
<AccordionGroup>
  <Accordion title="Instructions and memory">
    - Project `CLAUDE.md` and `.claude/CLAUDE.md` content is copied or appended into the OpenAgent agent workspace `AGENTS.md`.
  ...
  <Accordion title="Skills and commands">
    - Claude skills with a `SKILL.md` file are copied into the OpenAgent workspace skills directory.
    - Claude command Markdown files under `.claude/commands/` or `~/.claude/commands/` are converted into OpenAgent skills with `disable-model-invocation: true`.
...
The provider copies these into the migration report for manual review, but does **not** load them into live OpenAgent config:
...
OpenAgent refuses to execute hooks, trust permission allowlists, or decode opaque OAuth and Desktop credential state automatically. Move what you need by hand after reviewing the archive.

## Source selection

Without `--from`, OpenAgent inspects the default Claude Code home at `~/.claude`, the sampled Claude Code `~/.claude.json` state file, and the Claude Desktop MCP config on macOS.

When `--from` points at a project root, OpenAgent imports only that project's Claude files, such as `CLAUDE.md`, `.claude/settings.json`, `.claude/commands/`, `.claude/skills/`, and `.mcp.json`. It does not read your global Claude home during a project-root import.
...
<Steps>
  ...
  <Step title="Apply with backup">
    ...
    OpenAgent creates and verifies a backup before applying.
...
`--yes` is required for `migrate apply` outside an interactive terminal; without it OpenAgent errors instead of applying, so scripts and CI must pass `--yes` explicitly. Preview first with `--dry-run --json`, then apply with `--json --yes` once the plan looks right.
...
<AccordionGroup>
  ...
  <Accordion title="MCP servers from Claude Desktop did not import">
    Claude Desktop reads `claude_desktop_config.json` from a platform-specific path. Point `--from` at that file's directory if OpenAgent did not detect it automatically.
  </Accordion>
  <Accordion title="Claude commands became skills with model invocation disabled">
    By design. Claude commands are user-triggered, so OpenAgent imports them as skills with `disable-model-invocation: true`. Edit each skill's frontmatter if you want the agent to invoke them automatically.
  </Accordion>
</AccordionGroup>
