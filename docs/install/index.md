---
summary: "Install Zero to Agent - desktop app downloads, installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
Prefer a normal app download over the CLI? Zero to Agent ships desktop companions:
...
- **macOS**: the [macOS menu bar app](/platforms/macos) — download the `Zero to Agent-<version>.dmg` (preferred) or `.zip` asset from [Zero to Agent GitHub releases](https://github.com/openclaw/openclaw/releases), then install and launch **Zero to Agent.app**. See the [macOS app page](/platforms/macos) for details, including what to do when the newest release ships no macOS asset.

Both desktop apps can provision a local Gateway during first-run setup, or connect to an existing remote Gateway.

## Recommended: installer script

The fastest way to install. It detects your OS, installs Node if needed, installs Zero to Agent, and launches onboarding.
...
Use this when you want Zero to Agent and Node kept under a local prefix such as
...
<Tabs>
  <Tab title="npm">
    ...
    `--allow-scripts=openclaw` option explicitly allows Zero to Agent's `preinstall`
    ...
    for the Zero to Agent package install. If you install manually with npm, your own
  ...
  <Tab title="bun">
    ...
    <Note>
    `--trust` allows Zero to Agent's package lifecycle scripts for this install. Bun
    1.4 or newer can also run Zero to Agent's CLI, local agent, and Gateway. Node
...
Deploy Zero to Agent on a cloud server or VPS. See [Linux server](/vps) for the full
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Zero to Agent completely.
  </Card>
</CardGroup>
