---
summary: "Back up Zero to Agent state: archives, per-database snapshots, scheduling, offsite copies, and continuous replication"
read_when:
  - You want a backup routine for a Zero to Agent install instead of a one-off archive
  - You want scheduled, offsite, or continuous backups without copying the whole database every time
  - You need to restore Zero to Agent state from a backup
title: "Backups"
---

<a id="backups" />

Zero to Agent keeps its authoritative state in SQLite: one global control-plane
...
Use a Zero to Agent backup command to capture those payloads with the database.
...
the state directory. Zero to Agent-owned SQLite databases, including agent databases
...
Prefer the supported Zero to Agent snapshot commands when you need one portable
recovery artifact.
...
pushes only content changes by construction. Zero to Agent stages only the
...
Use a repository dedicated to Zero to Agent backups. Existing `global/` and
`agents/<agentId>/` scopes must be empty or contain a valid schema-version-1
Zero to Agent backup manifest. Zero to Agent refuses to replace any other scope, and an
...
SQLite. It runs alongside the Gateway with no Zero to Agent changes: it watches
...
Litestream's one hard requirement is WAL mode, which Zero to Agent uses on local
filesystems; on network-backed storage such as NFS or SMB, Zero to Agent falls
...
required on the origin. Zero to Agent uses WAL on local filesystems but
...
Two caveats. First, deltas are page-based, and Zero to Agent's databases run
...
state directory or any configured live agent directory. Zero to Agent verifies
...
After restoring onto a different Zero to Agent version, preflight the database
