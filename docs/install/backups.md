---
summary: "Back up OpenAgent state: archives, per-database snapshots, scheduling, offsite copies, and continuous replication"
read_when:
  - You want a backup routine for an OpenAgent install instead of a one-off archive
  - You want scheduled, offsite, or continuous backups without copying the whole database every time
  - You need to restore OpenAgent state from a backup
title: "Backups"
---

<a id="backups" />

OpenAgent keeps its authoritative state in SQLite: one global control-plane
...
Use an OpenAgent backup command to capture those payloads with the database.
...
the state directory. OpenAgent-owned SQLite databases, including agent databases
...
Prefer the supported OpenAgent snapshot commands when you need one portable
recovery artifact.
...
pushes only content changes by construction. OpenAgent stages only the
...
OpenAgent backup manifest. OpenAgent refuses to replace any other scope, and an
...
SQLite. It runs alongside the Gateway with no OpenAgent changes: it watches
...
Litestream's one hard requirement is WAL mode, which OpenAgent uses on local
filesystems; on network-backed storage such as NFS or SMB, OpenAgent falls
...
required on the origin. OpenAgent uses WAL on local filesystems but
...
Two caveats. First, deltas are page-based, and OpenAgent's databases run
...
state directory or any configured live agent directory. OpenAgent verifies
