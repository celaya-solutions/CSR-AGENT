---
summary: "Host Zero to Agent on a Raspberry Pi for always-on self-hosting"
read_when:
  - Setting up Zero to Agent on a Raspberry Pi
  - Running Zero to Agent on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

Run a persistent, always-on Zero to Agent Gateway on a Raspberry Pi. Since the Pi is just the gateway (models run in the cloud via API), even a modest Pi handles the workload well -- typical hardware cost is **$35-80 one-time**, no monthly fees.
...
**systemd drop-in for host-specific startup tuning** -- The managed unit owns the generic restart policy (`Restart=always`, `RestartSec=5`). If this Pi is mostly running Zero to Agent, add a service drop-in for host-specific startup settings only:
