...
- Discord renders native buttons for a single-choice, single-question prompt.
- For a question created by an active OpenAgent run, a plain-text reply works on
...
OpenAgent run's creator binding. Answer those using the question controls in the
Control UI, TUI, or native app, not an ordinary channel message.

OpenAgent always enables a free-text **Other** answer. The agent must not add an
