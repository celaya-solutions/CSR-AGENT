...
- For a question created by an active Zero to Agent run, a plain-text reply works on
...
Zero to Agent run's creator binding. Answer those using the question controls in the
Control UI, TUI, or native app, not an ordinary channel message.

Zero to Agent always enables a free-text **Other** answer. The agent must not add an
