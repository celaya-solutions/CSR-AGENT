...
OpenAgent adds a short progress-card reminder only for non-main, non-sub-agent sessions when a web card renderer is connected to the Gateway and the run is not using the agent's utility model. Channel-only deployments such as a Telegram-only Gateway do not receive the reminder.
