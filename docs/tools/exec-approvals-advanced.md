...
- If OpenAgent cannot identify exactly one concrete local file for an interpreter/runtime command
...
session, OpenAgent also resumes that session with an internal followup so the agent observes that
...
After an approved async exec finishes, OpenAgent sends a followup `agent` turn to the same session.
...
channel, OpenAgent keeps the local deterministic `/approve` prompt visible. If the native runtime is
active and attempts delivery but no target receives the card, OpenAgent sends a same-chat fallback
...
- Telegram defaults to approver DMs (`target: "dm"`). Switch to `channel` or `both` to also show
  approval prompts in the originating Telegram chat/topic. For Telegram forum topics, OpenAgent
...
owner routing for approval prompts and final results. OpenAgent first tries a private route on the
