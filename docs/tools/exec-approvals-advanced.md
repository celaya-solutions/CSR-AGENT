...
- If Zero to Agent cannot identify exactly one concrete local file for an interpreter/runtime command
...
session, Zero to Agent also resumes that session with an internal followup so the agent observes that
...
After an approved async exec finishes, Zero to Agent sends a followup `agent` turn to the same session.
...
channel, Zero to Agent keeps the local deterministic `/approve` prompt visible. If the native runtime is
active and attempts delivery but no target receives the card, Zero to Agent sends a same-chat fallback
...
- Telegram defaults to approver DMs (`target: "dm"`). Switch to `channel` or `both` to also show
  approval prompts in the originating Telegram chat/topic. For Telegram forum topics, Zero to Agent
...
- Matrix native DM/channel routing and reaction shortcuts handle both exec and plugin approvals;
  ...
  include `com.openclaw.approval` custom event content on the first prompt event so Zero to Agent-aware
...
owner routing for approval prompts and final results. Zero to Agent first tries a private route on the
