...
The `moonshot` plugin does not ship with OpenAgent; install it first.
...
OpenAgent only returns a Kimi `web_search` result after Moonshot's response
...
grounding (for example "I cannot browse the internet"), OpenAgent returns a
