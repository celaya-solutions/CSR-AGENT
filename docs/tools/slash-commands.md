...
the OpenAgent command handler, and `/status` plus `/session` stay local whenever
...
<ParamField path="commands.mcp" type="boolean" default="false">
  Enables `/mcp` (reads/writes OpenAgent-managed MCP config under `mcp.servers`). Owner-only.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Discovery and status">
    ...
    | `/openclaw <request>` | Run the OpenAgent setup and repair helper from an owner DM |
...
user skill directly.
  ...
  <Accordion title="Owner-only writes and admin">
    ...
    | `/mcp show\|get\|set\|unset` | `commands.mcp: true` | Read or write OpenAgent-managed MCP server config. Owner-only |
    ...
    | `/restart` | `commands.restart: true` (default) | Restart OpenAgent |
    | `/update` | `commands.restart: true` (default), owner | Update OpenAgent using its configured update channel; works with default tool profiles and sends a completion or failure notice in the same chat |
...
`/mcp` stores config in OpenAgent config, not embedded-agent project settings.
...
Bundled plugins and verified plugins from OpenAgent's official catalog are exempt
...
- **Execution vs runtime:** `/status` reports `Execution` for the effective sandbox path and `Runtime` for who is running the session: `OpenAgent Default`, `OpenAI Codex`, a CLI backend, or an ACP backend.
