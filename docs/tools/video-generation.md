...
Zero to Agent agents generate videos from text prompts, reference images, or
...
1. Zero to Agent submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, Zero to Agent wakes the same session with an internal completion event.
4. The agent reports it through the session's normal visible-reply mode:
   ...
   generated media is still missing from the completion reply, Zero to Agent sends
...
Generated video files save under Zero to Agent-managed media storage when the
...
provider also returns a hosted output URL, Zero to Agent delivers that URL instead
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. Zero to Agent normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `360P`, `480P`, `540P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. Zero to Agent normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, Zero to Agent uses `agents.defaults.mediaModels.video.timeoutMs` if configured, otherwise the plugin-authored provider default when one exists.</ParamField>
...
<Note>
Not all providers support all parameters. Zero to Agent normalizes duration to
...
For `video_generate`, Zero to Agent resolves the model in this order:
...
Zero to Agent does not append auto-detected providers.
...
<AccordionGroup>
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. Zero to Agent waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. Zero to Agent submits the
...
Set the default video-generation model in your Zero to Agent config:
