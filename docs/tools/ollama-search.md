...
Zero to Agent supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  Zero to Agent never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, Zero to Agent defaults to
...
- If the resolved host is `https://ollama.com`, Zero to Agent calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise Zero to Agent calls the local proxy endpoint
  ...
  the local host.
- Zero to Agent warns during setup if a local Ollama host is unreachable or not
