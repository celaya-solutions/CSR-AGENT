...
OpenAgent supports **Ollama Web Search** as a bundled `web_search` provider,
...
<Note>
  OpenAgent never auto-selects Ollama Web Search over a higher-priority
...
config examples. If nothing is set, OpenAgent defaults to
...
- If the resolved host is `https://ollama.com`, OpenAgent calls
  `https://ollama.com/api/web_search` directly with the API key as bearer
  auth.
- Otherwise OpenAgent calls the local proxy endpoint
  ...
  the local host.
- OpenAgent warns during setup if a local Ollama host is unreachable or not
