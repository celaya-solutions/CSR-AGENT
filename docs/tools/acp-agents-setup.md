...
read_when:
  ...
  - Enabling the plugin-tools or OpenAgent-tools MCP bridge
...
Codex has two OpenAgent routes:
...
When OpenAgent uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal OpenAgent `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by OpenAgent before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, OpenAgent/acpx cannot force a model selection.
...
manually, and there is no separate postinstall step for OpenAgent itself. If an
adapter download or spawn fails, `/acp doctor` reports the failure.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose OpenAgent plugin-registered tools to
the ACP harness.
...
- Injects a built-in MCP server named `openclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled OpenAgent
  plugins.
...
### OpenAgent tools MCP bridge

By default, ACPX sessions also do **not** expose built-in OpenAgent tools through
...
- Injects a built-in MCP server named `openclaw-tools` into ACPX session
  bootstrap.
- Exposes selected built-in OpenAgent tools. The initial server exposes `cron`.
...
These ACPX harness permissions are separate from OpenAgent exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between OpenAgent `tools.exec.mode`, Codex Guardian
...
<Warning>
OpenAgent defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode`.
