---
summary: "Index of the OpenAgent Code Mode documentation, one page per reader job"
title: "Code Mode"
sidebarTitle: "Code Mode"
read_when:
  - You want to enable OpenAgent Code Mode for an agent run
...
Code mode is an experimental, opt-in OpenAgent agent-runtime feature. When
...
<Note>
OpenAgent Code Mode is off by default. To try it, open **Settings → Agents &
...
This page documents OpenAgent Code Mode, not Codex Code Mode. The two features
...
- OpenAgent Code Mode runs in the generic OpenAgent agent runtime and is
...
In OpenAgent Code Mode, `command` is a JavaScript or TypeScript alias for
...
- Every catalog-eligible enabled non-MCP tool (OpenAgent core, plugin, client) is
...
- Provider neutral: works for OpenAgent, plugin, MCP, and client tools without
