---
summary: "Index of the Zero to Agent Code Mode documentation, one page per reader job"
title: "Code Mode"
sidebarTitle: "Code Mode"
read_when:
  - You want to enable Zero to Agent Code Mode for an agent run
...
Code mode is an experimental, opt-in Zero to Agent agent-runtime feature. When
...
<Note>
Zero to Agent Code Mode is off by default. To try it, open **Settings → Agents &
...
This page documents Zero to Agent Code Mode, not Codex Code Mode. The two features
...
- Zero to Agent Code Mode runs in the generic Zero to Agent agent runtime and is
...
In Zero to Agent Code Mode, `command` is a JavaScript or TypeScript alias for
...
- Every catalog-eligible enabled non-MCP tool (Zero to Agent core, plugin, client) is
...
- Provider neutral: works for Zero to Agent, plugin, MCP, and client tools without
