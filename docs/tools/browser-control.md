...
that override because OpenAgent does not launch those browser processes.
...
- `navigation_check_failed`: OpenAgent could not validate the address, for example
...
OpenAgent, then restart the gateway. For Docker, also install the Chromium
...
- Download and trace paths are constrained to OpenAgent temp roots: `/tmp/openclaw{,/downloads}` (fallback: `${os.tmpdir()}/openclaw/...`).
- `upload` accepts files from the OpenAgent temp uploads root and
  OpenAgent-managed inbound media. Managed inbound media can be referenced as
...
Stable tab ids and labels survive Chromium raw-target replacement when OpenAgent
