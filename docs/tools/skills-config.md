...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. OpenAgent runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the OpenAgent process into the
...
OpenAgent retains at most 100 normalized findings for display. Only a `warn`
...
<Warning>
  Agent skill allowlists are a visibility and loading filter for OpenAgent
