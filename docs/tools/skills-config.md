...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  installs. Node remains the primary and recommended Zero to Agent runtime; Bun 1.4+
...
policy runs after Zero to Agent has staged source material and before the install
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. Zero to Agent runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the Zero to Agent process into the
...
Zero to Agent retains at most 100 normalized findings for display. Only a `warn`
...
Zero to Agent does not execute install policy during normal Gateway startup.
...
<Warning>
  Agent skill allowlists are a visibility and loading filter for Zero to Agent
