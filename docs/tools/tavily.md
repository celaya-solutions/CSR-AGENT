...
[Tavily](https://tavily.com) is a search API designed for AI applications. OpenAgent exposes it in two ways:
...
`published_date`, OpenAgent returns it as `published`; GMT news timestamps are
...
for OpenAgent's local result cache (default: 15 minutes). Set it to `0` to bypass
