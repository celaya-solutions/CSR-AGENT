...
[Tavily](https://tavily.com) is a search API designed for AI applications. Zero to Agent exposes it in two ways:
...
`published_date`, Zero to Agent returns it as `published`; GMT news timestamps are
...
for Zero to Agent's local result cache (default: 15 minutes). Set it to `0` to bypass
