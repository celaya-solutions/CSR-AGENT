...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the Zero to Agent session store, and may create a background task when the run is parent-owned.
    ...
    - Turn completion waits for queued output delivery. If delivery fails, Zero to Agent cancels the active turn and waits for backend cleanup before starting the next queued turn, within the configured turn timeout.
    - `close` ends the ACP session from Zero to Agent's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up Zero to Agent-owned wrapper and adapter process trees after `close`, and reaps stale Zero to Agent-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    Zero to Agent dynamic tools still execute through Zero to Agent, while Codex-native
    tools such as shell/apply-patch execute inside Codex. For Codex-native
    tool events, Zero to Agent injects a per-turn native hook relay so plugin hooks
    can block `before_tool_call`, observe `after_tool_call`, and route Codex
    `PermissionRequest` events through Zero to Agent approvals. Codex `Stop` hooks
    are relayed to Zero to Agent `before_agent_finalize`, where plugins can request
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    Zero to Agent picks `runtime: "acp"`, resolves the harness `agentId`, binds to
    ...
    `gemini`, or `opencode`. Do not pass a normal Zero to Agent config agent id
    ...
    runtime. When a Zero to Agent agent is configured with
    `runtime.type="acp"`, Zero to Agent uses `runtime.acp.agent` as the underlying
    harness id.
