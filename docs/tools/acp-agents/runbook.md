...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the OpenAgent session store, and may create a background task when the run is parent-owned.
    ...
    - Turn completion waits for queued output delivery. If delivery fails, OpenAgent cancels the active turn and waits for backend cleanup before starting the next queued turn, within the configured turn timeout.
    - `close` ends the ACP session from OpenAgent's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
    - The acpx plugin cleans up OpenAgent-owned wrapper and adapter process trees after `close`, and reaps stale OpenAgent-owned ACPX orphans during Gateway startup.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    OpenAgent dynamic tools still execute through OpenAgent, while Codex-native
    tools such as shell/apply-patch execute inside Codex. For Codex-native
    tool events, OpenAgent injects a per-turn native hook relay so plugin hooks
    can block `before_tool_call`, observe `after_tool_call`, and route Codex
    `PermissionRequest` events through OpenAgent approvals. Codex `Stop` hooks
    are relayed to OpenAgent `before_agent_finalize`, where plugins can request
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    OpenAgent picks `runtime: "acp"`, resolves the harness `agentId`, binds to
    ...
    `gemini`, or `opencode`. Do not pass a normal OpenAgent config agent id
    ...
    runtime. When an OpenAgent agent is configured with
    `runtime.type="acp"`, OpenAgent uses `runtime.acp.agent` as the underlying
    harness id.
