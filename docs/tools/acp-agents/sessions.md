...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    ACP sessions. If `agentId` is omitted, OpenAgent uses `acp.defaultAgent`
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, OpenAgent may default to persistent behaviour per
...
<ParamField path="streamTo" type='"parent"'>
  ...
  session as system events. OpenAgent records the full relay history in the
