...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    ACP sessions. If `agentId` is omitted, Zero to Agent uses `acp.defaultAgent`
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, Zero to Agent may default to persistent behaviour per
...
<ParamField path="streamTo" type='"parent"'>
  ...
  session as system events. Zero to Agent records the full relay history in the
