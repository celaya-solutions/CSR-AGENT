...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    Zero to Agent presents supported form fields as transient Gateway questions in
    ...
    and Decline choices; Zero to Agent does not fetch or open it. Explicitly secret
    ...
    What Zero to Agent sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. Zero to Agent agents get Zero to Agent's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in Zero to Agent where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. Zero to Agent converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see Zero to Agent-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    sessions, Zero to Agent uses an agent-to-agent (A2A) follow-up path after
    ...
    Zero to Agent skips the A2A follow-up only when the requester is the parent of
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not a Zero to Agent channel session key; Zero to Agent still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new Zero to Agent session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, **not** inside the Zero to Agent
sandbox.
...
- Zero to Agent's sandbox policy does **not** wrap ACP harness execution.
- Zero to Agent still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced Zero to Agent-native work.
