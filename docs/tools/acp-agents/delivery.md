...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    OpenAgent presents supported form fields as transient Gateway questions in
    ...
    and Decline choices; OpenAgent does not fetch or open it. Explicitly secret
    ...
    What OpenAgent sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. OpenAgent agents get OpenAgent's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_OPENCLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in OpenAgent where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. OpenAgent converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see OpenAgent-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    sessions, OpenAgent uses an agent-to-agent (A2A) follow-up path after
    ...
    OpenAgent skips the A2A follow-up only when the requester is the parent of
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an OpenAgent channel session key; OpenAgent still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new OpenAgent session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, **not** inside the OpenAgent
sandbox.
...
- OpenAgent's sandbox policy does **not** wrap ACP harness execution.
- OpenAgent still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced OpenAgent-native work.
