...
If no target resolves, Zero to Agent returns a clear error
(`Unable to resolve session target: ...`).

### Session owner and harness

The Zero to Agent agent that owns a session is separate from the external harness
...
name. Configured bindings use their Zero to Agent agent owner and their configured
...
| ---------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, Zero to Agent normalizes `openai/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai/gpt-5.4/high` to `reasoning_effort`.                                         |
| `/acp set thinking <level>`  | canonical option `thinking`          | Zero to Agent sends the backend-advertised equivalent when present, preferring `thinking`, then `effort`, `reasoning_effort`, or `thought_level`. For Codex ACP, the adapter maps values to `reasoning_effort`. |
| `/acp permissions <profile>` | canonical option `permissionProfile` | Zero to Agent sends the backend-advertised equivalent when present, such as `approval_policy`, `permission_profile`, `permissions`, or `permission_mode`.                                                       |
| `/acp timeout <seconds>`     | canonical option `timeoutSeconds`    | Zero to Agent sends the backend-advertised equivalent when present, such as `timeout` or `timeout_seconds`.                                                                                                     |
...
When a backend returns its accepted controls, Zero to Agent keeps an already-selected
...
session overrides, and the model reference keeps its Zero to Agent provider prefix.
