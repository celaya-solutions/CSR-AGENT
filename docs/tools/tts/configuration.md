...
the voice-design prompt. OpenAgent sends that prompt as the TTS `user` message
...
provider when Speech Swift and OpenAgent run on the same Mac. Use Local CLI for
direct executable integration on any supported host.

Install `ffmpeg` when a channel needs OpenAgent to convert WAV output to Opus or
raw PCM.
...
is required by OpenAgent's provider configuration but is not validated by the
loopback server.
</Tab>
<Tab title="macOS CLI">
The Homebrew `speech` executable can write directly to OpenAgent's
...
and verify synthesis before starting OpenAgent:

```bash
speech download-models
speech speak "Hello from OpenAgent" hello.wav
```
