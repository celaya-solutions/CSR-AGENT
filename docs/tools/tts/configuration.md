...
the voice-design prompt. Zero to Agent sends that prompt as the TTS `user` message
...
provider when Speech Swift and Zero to Agent run on the same Mac. Use Local CLI for
direct executable integration on any supported host.

Install `ffmpeg` when a channel needs Zero to Agent to convert WAV output to Opus or
raw PCM.
...
is required by Zero to Agent's provider configuration but is not validated by the
loopback server.
</Tab>
<Tab title="macOS CLI">
The Homebrew `speech` executable can write directly to Zero to Agent's
...
and verify synthesis before starting Zero to Agent:

```bash
speech download-models
speech speak "Hello from Zero to Agent" hello.wav
```
