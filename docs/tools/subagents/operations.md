...
OpenAgent warns when the delivery backlog reaches 25 and blocks new subagent
spawns at 50 until operators retry or dismiss enough retained deliveries. It
does not prune results to make room.

## Liveness and recovery

OpenAgent does not treat `endedAt` absence as permanent proof that a
...
For sub-agents that announce completion, OpenAgent also attempts a notice to the
