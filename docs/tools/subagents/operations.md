...
Zero to Agent warns when the delivery backlog reaches 25 and blocks new subagent
spawns at 50 until operators retry or dismiss enough retained deliveries. It
does not prune results to make room.

## Liveness and recovery

Zero to Agent does not treat `endedAt` absence as permanent proof that a
...
For sub-agents that announce completion, Zero to Agent also attempts a notice to the
...
rapid re-wedge window, Zero to Agent persists a recovery tombstone on that
