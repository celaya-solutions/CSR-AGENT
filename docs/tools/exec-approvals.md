...
Claude Code owns execution. It does not provide OpenAgent sandboxing. See
...
Otherwise it uses the default OpenAgent state directory:
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  Presentation only: when enabled, OpenAgent may attach parser-derived
...
requested exec policy in OpenAgent config (`tools.exec.*`) **and**
...
For OpenAgent-managed Claude sessions, OpenAgent launches Claude Code in its
`default` permission mode. OpenAgent's effective exec policy remains
...
If you want a more conservative setup, tighten OpenAgent exec policy back to
...
specific argument shape. OpenAgent uses ECMAScript (JavaScript) regular
...
instead of hand-editing the encoded value. If OpenAgent cannot parse argv
...
servers, and computer-use servers do not receive OpenAgent MCP tool grants.
OpenAgent only mints when durable persistence is offered and it can unambiguously
...
The durable grant is read when OpenAgent next prepares the Codex thread
...
blast radius. OpenAgent renders the sanitized summary on channel approval cards
...
session after the node reports completion. OpenAgent can also emit an
...
- For main-agent async approvals with an originating session, OpenAgent
  ...
  repair.
- If there is no session or the session cannot be resumed, OpenAgent can
