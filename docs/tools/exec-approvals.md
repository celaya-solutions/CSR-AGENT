...
Claude Code owns execution. It does not provide Zero to Agent sandboxing. See
...
- For shell scripts and direct interpreter/runtime file invocations, Zero to Agent also tries to bind one concrete local file operand. If that file changes after approval but before execution, the run is denied instead of executing drifted content.
- File binding is best-effort, not a complete model of every interpreter/runtime loader path. If exactly one concrete local file cannot be identified, Zero to Agent refuses to mint an approval-backed run rather than pretend full coverage.
...
Otherwise it uses the default Zero to Agent state directory:
...
points somewhere else, Zero to Agent never imports or archives approvals from the
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  Presentation only: when enabled, Zero to Agent may attach parser-derived
...
requested exec policy in Zero to Agent config (`tools.exec.*`) **and**
...
For Zero to Agent-managed Claude sessions, Zero to Agent launches Claude Code in its
`default` permission mode. Zero to Agent's effective exec policy remains
...
If you want a more conservative setup, tighten Zero to Agent exec policy back to
...
specific argument shape. Zero to Agent uses ECMAScript (JavaScript) regular
...
instead of hand-editing the encoded value. If Zero to Agent cannot parse argv
...
servers, and computer-use servers do not receive Zero to Agent MCP tool grants.
Zero to Agent only mints when durable persistence is offered and it can unambiguously
...
The durable grant is read when Zero to Agent next prepares the Codex thread
...
Zero to Agent does not reload grants for every tool call.
...
blast radius. Zero to Agent renders the sanitized summary on channel approval cards
...
session after the node reports completion. Zero to Agent can also emit an
...
- For main-agent async approvals with an originating session, Zero to Agent
  ...
  repair.
- If there is no session or the session cannot be resumed, Zero to Agent can
