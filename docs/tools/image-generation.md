...
providers. In chat sessions it runs asynchronously: Zero to Agent records a
...
session is inactive or its active wake fails, Zero to Agent sends an idempotent
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai` OAuth profile is configured, Zero to Agent routes image requests
...
nearby geometry option instead of the exact requested one, Zero to Agent remaps to
...
For `image_generate`, Zero to Agent tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Timeouts">
    ...
    by Zero to Agent's 600000 ms dynamic-tool bridge maximum.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="GPT Image 2.5 through OpenAI or fal">
    ...
    OpenAI accepts up to 5 reference images through Zero to Agent. fal accepts 16.
  ...
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai` OAuth profile is configured, Zero to Agent reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. Zero to Agent
    ...
    Zero to Agent forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly. When possible Zero to Agent maps
    ...
    valid. When only `aspectRatio` is specified, Zero to Agent still selects
    ...
    transparency-capable OpenAI image model. Zero to Agent routes default
  ...
  <Accordion title="Microsoft Foundry MAI image models">
    ...
    onboarding/model metadata so Zero to Agent can verify that the deployment is
  ...
  <Accordion title="OpenRouter image models">
    ...
    Zero to Agent forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="fal Krea 2">
    ...
    `image_size` schema used by Flux. Zero to Agent sends:
    ...
    Krea. Zero to Agent maps `size` to the closest supported Krea aspect ratio and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as Zero to Agent-managed image attachments

    Zero to Agent intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for Zero to Agent image generation" size=3840x2160 count=1
```
  </Tab>
