...
generated audio is still missing from the reply, Zero to Agent sends an
...
<Note>
Not all providers support all parameters. Zero to Agent still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Zero to Agent
...
Provider request timeouts are operator configuration only. Zero to Agent uses
...
- **Completion wake:** Zero to Agent injects an internal completion event back
...
For `music_generate`, Zero to Agent tries providers in this order:
