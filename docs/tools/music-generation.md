...
generated audio is still missing from the reply, OpenAgent sends an
...
<Note>
Not all providers support all parameters. OpenAgent still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, OpenAgent
...
Provider request timeouts are operator configuration only. OpenAgent uses
...
For `music_generate`, OpenAgent tries providers in this order:
