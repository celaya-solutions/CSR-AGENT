...
In `propose` and `auto` modes, `openagent doctor` runs the
...
or the `openagent skills workshop` CLI. For `non-main` mode, this restriction applies
...
`openagent plugins inspect skill-workshop`; that command now explains that Skill
