      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; Zero to Agent appends /v1/search
          },
        },
      },
    },
...
Cloudflare AI Gateway). Zero to Agent normalizes bare hosts by prepending
...
- Both providers return a `session_id`; Zero to Agent surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- Zero to Agent always forwards a resolved result count to Parallel as
  ...
  Zero to Agent's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
