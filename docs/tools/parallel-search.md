      ...
      parallel: {
        config: {
          webSearch: {
            ...
            baseUrl: "https://api.parallel.ai", // optional; OpenAgent appends /v1/search
          },
        },
      },
    },
...
Cloudflare AI Gateway). OpenAgent normalizes bare hosts by prepending
...
- Both providers return a `session_id`; OpenAgent surfaces it as `sessionId` in
...
- `searchId`, `warnings`, and `usage` from Parallel are passed through when
  present.
- OpenAgent always forwards a resolved result count to Parallel as
  ...
  OpenAgent's generic `web_search` default (5) -- Parallel's own API defaults
  to 10.
