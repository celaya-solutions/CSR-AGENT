...
read_when:
  ...
  - You are installing, pairing, disabling, or troubleshooting the Zero to Agent Chrome extension
...
The Zero to Agent Chrome extension lets the browser tool automate eligible tabs in
...
- Zero to Agent installed on the same machine as Chrome, or a Zero to Agent browser node
  on that machine
...
only when the registered host is a Windows executable. Zero to Agent does not install
...
enable **Zero to Agent** in Chrome. Zero to Agent never restarts Chrome or approves its
...
[Zero to Agent from the Chrome Web Store](https://chromewebstore.google.com/detail/openclaw/kcdjddhmeafeomebliikmbpblkmkfoig)
...
add it again from the Store. Zero to Agent does not clear Chrome's removal decision.

On macOS and Linux, the origin-locked native host permits the exact official
Store identity and Zero to Agent's deterministic development IDs. Once enabled, the
...
This still copies the bundled extension to a stable Zero to Agent-owned directory
...
Foundation Store ID. That identity never makes a recorded path Zero to Agent-owned.
...
- the recorded extension path resolves exactly to the installed or bundled
  Zero to Agent extension directory.
...
host name are not overwritten unless they are verifiably Zero to Agent-owned.
...
Zero to Agent and local clients such as mcporter then use that profile relay port.
...
- **Selected tabs** uses the **Zero to Agent** tab group as the access-control
...
in flight. Agent-created tabs stay in the Zero to Agent group in either mode.
...
in the Zero to Agent group, and applies the same pause and access-mode controls.
...
- **Use local Zero to Agent** clears the opt-out and retries the native host.
...
and disable automatic setup**, then **Use local Zero to Agent** to create the new
...
**Use local Zero to Agent** to reconnect. Until that explicit disconnect succeeds,
...
Zero to Agent-owned request, `missing` when no request exists, `foreign` for an
...
Remove only Zero to Agent's macOS Chrome Store installation request:
...
Remove only Zero to Agent-owned native-host manifests and launchers:
...
For a laptop that has Chrome but does not run Zero to Agent or a browser node, pair
...
Zero to Agent and an external client can stay connected together. When a client
...
an updated Zero to Agent installation. Update the Store extension when available.
...
Zero to Agent-owned mode-`0700` directory. Manifests are mode `0600`. The launcher is
...
- **Extension version mismatch:** reload the unpacked Zero to Agent extension from
  `chrome://extensions`, then rerun browser doctor. Fully restart Chrome if the
  running and bundled versions still differ.
- **Waiting for local Zero to Agent:** run `extension status`. Install or repair the
  owned native host.
