...
read_when:
  ...
  - You are installing, pairing, disabling, or troubleshooting the OpenAgent Chrome extension
...
The OpenAgent Chrome extension lets the browser tool automate eligible tabs in
...
- OpenAgent installed on the same machine as Chrome, or an OpenAgent browser node
  on that machine
...
only when the registered host is a Windows executable. OpenAgent does not install
...
enable **OpenAgent** in Chrome. OpenAgent never restarts Chrome or approves its
...
[OpenAgent from the Chrome Web Store](https://chromewebstore.google.com/detail/openclaw/kcdjddhmeafeomebliikmbpblkmkfoig)
...
add it again from the Store. OpenAgent does not clear Chrome's removal decision.

On macOS and Linux, the origin-locked native host permits the exact official
Store identity and OpenAgent's deterministic development IDs. Once enabled, the
...
This still copies the bundled extension to a stable OpenAgent-owned directory
...
Foundation Store ID. That identity never makes a recorded path OpenAgent-owned.
...
host name are not overwritten unless they are verifiably OpenAgent-owned.
...
OpenAgent and local clients such as mcporter then use that profile relay port.
...
- **Selected tabs** uses the **OpenAgent** tab group as the access-control
...
in flight. Agent-created tabs stay in the OpenAgent group in either mode.
...
in the OpenAgent group, and applies the same pause and access-mode controls.
...
- **Use local OpenAgent** clears the opt-out and retries the native host.
...
and disable automatic setup**, then **Use local OpenAgent** to create the new
...
**Use local OpenAgent** to reconnect. Until that explicit disconnect succeeds,
...
OpenAgent-owned request, `missing` when no request exists, `foreign` for an
...
Remove only OpenAgent's macOS Chrome Store installation request:
...
Remove only OpenAgent-owned native-host manifests and launchers:
...
For a laptop that has Chrome but does not run OpenAgent or a browser node, pair
...
OpenAgent and an external client can stay connected together. When a client
...
an updated OpenAgent installation. Update the Store extension when available.
...
OpenAgent-owned mode-`0700` directory. Manifests are mode `0600`. The launcher is
...
- **Extension version mismatch:** reload the unpacked OpenAgent extension from
  `chrome://extensions`, then rerun browser doctor. Fully restart Chrome if the
  running and bundled versions still differ.
- **Waiting for local OpenAgent:** run `extension status`. Install or repair the
  owned native host.
