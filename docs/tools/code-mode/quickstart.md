---
summary: "Enable Zero to Agent Code Mode, override one model, and recover from tool errors"
...
Await every tool call or handle its rejection explicitly. Zero to Agent drains
...
Zero to Agent does not automatically replay a failed program. Earlier calls may have
...
when Swarm is enabled and the native Zero to Agent `sessions_spawn` tool is present
