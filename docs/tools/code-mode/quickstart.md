---
summary: "Enable OpenAgent Code Mode, override one model, and recover from tool errors"
...
Await every tool call or handle its rejection explicitly. OpenAgent drains
...
OpenAgent does not automatically replay a failed program. Earlier calls may have
...
when Swarm is enabled and the native OpenAgent `sessions_spawn` tool is present
