...
OpenAgent tools can declare `outputSchema` for the structured value placed in
...
- OpenAgent compiles the schema before running the tool, then validates final
...
directory. For each code-mode `exec` call, OpenAgent builds the run-scoped tool
...
successful emitted or returned output that exceeds this budget, OpenAgent returns a bounded value
...
OpenAgent tools, not through the code-mode bridge.
...
context and persistence limits. OpenAgent reserves the complete result envelope,
