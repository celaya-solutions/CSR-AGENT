...
Zero to Agent tools can declare `outputSchema` for the structured value placed in
...
- Zero to Agent compiles the schema before running the tool, then validates final
...
directory. For each code-mode `exec` call, Zero to Agent builds the run-scoped tool
...
successful emitted or returned output that exceeds this budget, Zero to Agent returns a bounded value
...
Zero to Agent tools, not through the code-mode bridge.
...
context and persistence limits. Zero to Agent reserves the complete result envelope,
