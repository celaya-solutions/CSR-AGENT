...
`aborted` means the caller cancelled an active `exec` or `wait`; Zero to Agent
...
- `codeModeEngaged`: `true` only when code mode actually owned the model tool
  ...
  `"auto"` tier engages per model capability. Harnesses that bridge Zero to Agent's
  ...
  never engage Zero to Agent code mode, so they always read `false`; an attempt that
...
tool inputs beyond existing Zero to Agent trajectory policy.
