...
`aborted` means the caller cancelled an active `exec` or `wait`; OpenAgent
...
- `codeModeEngaged`: `true` only when code mode actually owned the model tool
  ...
  `"auto"` tier engages per model capability. Harnesses that bridge OpenAgent's
  ...
  never engage OpenAgent code mode, so they always read `false`; an attempt that
...
tool inputs beyond existing OpenAgent trajectory policy.
