...
If code mode is enabled but QuickJS-WASI cannot load, OpenAgent fails closed
...
For OpenAI models, the flag matters only when the run resolves to the OpenAgent
embedded agent runtime. Default OpenAI routing uses the Codex-style harness
surface, where OpenAgent code mode does not apply; the catalog flag never
...
1. Resolve the agent, model, provider, sandbox, channel, sender, and run
   policy.
2. Build the effective OpenAgent tool list, adding eligible plugin, MCP, and
   client tools.
...
when `tools.codeMode.enabled: true` is configured. Code mode and OpenAgent Tool
