...
If code mode is enabled but QuickJS-WASI cannot load, Zero to Agent fails closed
...
For OpenAI models, the flag matters only when the run resolves to the Zero to Agent
embedded agent runtime. Default OpenAI routing uses the Codex-style harness
surface, where Zero to Agent code mode does not apply; the catalog flag never
...
1. Resolve the agent, model, provider, sandbox, channel, sender, and run
   policy.
2. Build the effective Zero to Agent tool list, adding eligible plugin, MCP, and
   client tools.
...
when `tools.codeMode.enabled: true` is configured. Code mode and Zero to Agent Tool
