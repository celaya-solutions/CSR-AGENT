...
- `command` is accepted as an exec-compatible alias for hook policies and
  trusted rewrites (the normal Zero to Agent shell exec tool also uses a `command`
...
- If `language` is `"typescript"`, Zero to Agent transpiles before evaluation.
...
- Do not set `restartSafe` on a new `exec`. Set it to `true` only when Zero to Agent
  ...
  replay-safe. Zero to Agent rejects unmarked catalog tools and namespace
...
while the operator decision is pending, Zero to Agent suspends both the Code Mode
...
final value is JSON-compatible after Zero to Agent's output adapter runs.
...
In the built-in Zero to Agent runtime, the JSON Code Mode tool executes the original
...
`wait` exists because nested Zero to Agent tools can be slow, interactive, or stream
...
2. On suspension, Zero to Agent snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot and
   re-registers host callbacks by stable names.
4. Zero to Agent delivers nested tool results into the restored VM and drains
...
order: Zero to Agent core tools, bundled plugin tools, external plugin tools, MCP
...
Before the worker starts, Zero to Agent projects one effective winner per exact tool
...
Code mode supersedes the Zero to Agent Tool Search model surface for runs where it
is active.

When Code Mode engages through forced `true` or `"auto"` activation:

- Zero to Agent does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- Nested calls dispatch through the same Zero to Agent executor path that Tool
  Search uses.

See [Tool Search](/tools/tool-search) for the Zero to Agent compact catalog bridge
that code mode supersedes for active runs.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal Zero to Agent
