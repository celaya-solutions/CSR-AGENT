...
- `command` is accepted as an exec-compatible alias for hook policies and
  trusted rewrites (the normal OpenAgent shell exec tool also uses a `command`
...
- If `language` is `"typescript"`, OpenAgent transpiles before evaluation.
...
- Do not set `restartSafe` on a new `exec`. Set it to `true` only when OpenAgent
  ...
  replay-safe. OpenAgent rejects unmarked catalog tools and namespace
...
while the operator decision is pending, OpenAgent suspends both the Code Mode
...
final value is JSON-compatible after OpenAgent's output adapter runs.
...
In the built-in OpenAgent runtime, the JSON Code Mode tool executes the original
...
`wait` exists because nested OpenAgent tools can be slow, interactive, or stream
...
2. On suspension, OpenAgent snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot and
   re-registers host callbacks by stable names.
4. OpenAgent delivers nested tool results into the restored VM and drains
...
order: OpenAgent core tools, bundled plugin tools, external plugin tools, MCP
...
Before the worker starts, OpenAgent projects one effective winner per exact tool
...
Code mode supersedes the OpenAgent Tool Search model surface for runs where it
is active.

When Code Mode engages through forced `true` or `"auto"` activation:

- OpenAgent does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- Nested calls dispatch through the same OpenAgent executor path that Tool
  Search uses.

See [Tool Search](/tools/tool-search) for the OpenAgent compact catalog bridge
that code mode supersedes for active runs.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal OpenAgent
