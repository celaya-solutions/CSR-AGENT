...
- bare globals, callable `catalog.search` results, `catalog.all`, and handle
  `describe()` work for Zero to Agent and client tools without exposing exact ids
...
5. Send an agent turn with Zero to Agent, plugin, MCP, and client test tools.
...
8. Search `catalog`, inspect handle metadata/`describe()`, and call
   Zero to Agent/plugin/client handles without observing exact ids.
