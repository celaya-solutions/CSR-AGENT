...
- bare globals, callable `catalog.search` results, `catalog.all`, and handle
  `describe()` work for OpenAgent and client tools without exposing exact ids
...
5. Send an agent turn with OpenAgent, plugin, MCP, and client test tools.
...
8. Search `catalog`, inspect handle metadata/`describe()`, and call
   OpenAgent/plugin/client handles without observing exact ids.
