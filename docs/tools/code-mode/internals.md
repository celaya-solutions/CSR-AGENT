...
| Stability           | experimental Zero to Agent surface (Codex Code Mode is a separate, stable Codex harness surface) |
| Target surface      | generic Zero to Agent agent runs                                                                 |
...
- **Code mode**: the Zero to Agent runtime mode that hides catalog-compatible model
...
Every nested tool call crosses the host bridge and re-enters Zero to Agent,
...
- Zero to Agent caps the number of concurrently suspended runs per process (64) and
...
Zero to Agent loads `quickjs-wasi` as a direct dependency in the owning package; it
...
The runtime executes in a Node.js worker thread, outside Zero to Agent's main
