...
- **Code mode**: the OpenAgent runtime mode that hides catalog-compatible model
...
Every nested tool call crosses the host bridge and re-enters OpenAgent,
...
- OpenAgent caps the number of concurrently suspended runs per process (64) and
...
OpenAgent loads `quickjs-wasi` as a direct dependency in the owning package; it
...
The runtime executes in a Node.js worker thread, outside OpenAgent's main
