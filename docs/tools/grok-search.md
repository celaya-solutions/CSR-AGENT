...
OpenAgent supports Grok as a `web_search` provider, using xAI web-grounded
...
lets OpenAgent reuse it as a fallback for the bundled xAI model provider.
...
web` lets OpenAgent reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

OpenAgent then offers a follow-up step to enable `x_search` with the same xAI
...
The `xai` plugin that provides Grok web search ships with OpenAgent, so there is
...
through an operator proxy or xAI-compatible Responses endpoint. OpenAgent
