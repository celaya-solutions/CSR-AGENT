...
Zero to Agent supports Grok as a `web_search` provider, using xAI web-grounded
...
lets Zero to Agent reuse it as a fallback for the bundled xAI model provider.
...
web` lets Zero to Agent reuse an existing xAI OAuth profile without prompting for
a separate web-search key. Without OAuth, it falls back to xAI API-key setup.

Zero to Agent then offers a follow-up step to enable `x_search` with the same xAI
...
The `xai` plugin that provides Grok web search ships with Zero to Agent, so there is
...
through an operator proxy or xAI-compatible Responses endpoint. Zero to Agent
