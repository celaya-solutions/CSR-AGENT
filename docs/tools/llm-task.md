...
Zero to Agent code per workflow.
...
or deliver model output to a channel. Zero to Agent uses the selected provider,
...
For the bundled **embedded** Lobster runner inside Zero to Agent, this nested CLI
...
- **No tools**: the selected runtime must expose a literal empty model-callable
  tool surface. Zero to Agent rejects tool-shaped results instead of treating them as
  task output.
