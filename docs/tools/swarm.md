...
Numeric values must be positive integers. Zero to Agent bounds
...
To use the `agents.run`, `phase`, and `log` guest globals, enable Zero to Agent Code
...
hints only when its catalog contains the native Zero to Agent `sessions_spawn`
...
permitted by the requester's `subagents.allowAgents` policy. Zero to Agent rejects
...
Zero to Agent starts up to `maxConcurrent` children for the group and queues the rest
in submission order.
...
`agents_wait`, or await `agents.run()` in Zero to Agent Code Mode. Do not use
...
tool surface, cheaper model, or tighter sandbox policy. Zero to Agent does not ship
...
For structured output, Zero to Agent adds a synthetic `structured_output` tool to
...
You can use Swarm without Zero to Agent Code Mode. Its core tools are
...
Codex Code Mode automatically exposes eligible dynamic Zero to Agent tools under
`tools.*`. It does not use Zero to Agent's QuickJS guest API or require
...
upgrading Zero to Agent, so collector spawn fields remain unavailable on that thread.
...
With the currently supported Codex runtime, dynamic Zero to Agent tool results reach
...
Zero to Agent sub-agents. It does not support ACP runtime, thread binding, visible
