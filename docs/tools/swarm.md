...
Numeric values must be positive integers. OpenAgent bounds
...
To use the `agents.run`, `phase`, and `log` guest globals, enable OpenAgent Code
...
hints only when its catalog contains the native OpenAgent `sessions_spawn`
...
permitted by the requester's `subagents.allowAgents` policy. OpenAgent rejects
...
OpenAgent starts up to `maxConcurrent` children for the group and queues the rest
in submission order.
...
`agents_wait`, or await `agents.run()` in OpenAgent Code Mode. Do not use
...
tool surface, cheaper model, or tighter sandbox policy. OpenAgent does not ship
...
For structured output, OpenAgent adds a synthetic `structured_output` tool to
...
You can use Swarm without OpenAgent Code Mode. Its core tools are
...
Codex Code Mode automatically exposes eligible dynamic OpenAgent tools under
`tools.*`. It does not use OpenAgent's QuickJS guest API or require
...
upgrading OpenAgent, so collector spawn fields remain unavailable on that thread.
...
With the currently supported Codex runtime, dynamic OpenAgent tool results reach
...
OpenAgent sub-agents. It does not support ACP runtime, thread binding, visible
