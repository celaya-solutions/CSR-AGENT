---
summary: "Typed workflow runtime for Zero to Agent with resumable approval gates."
title: Lobster
read_when:
...
gateway URL/auth context for nested Zero to Agent CLI tool calls.
...
a bare envelope: Zero to Agent creates or resumes a durable flow record and applies
...
persists in Zero to Agent SQLite; Lobster's approval checkpoint is separate and must
...
- **No secrets** - Lobster doesn't manage OAuth; it calls Zero to Agent tools that
  do.
