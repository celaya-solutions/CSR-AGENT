---
summary: "Typed workflow runtime for OpenAgent with resumable approval gates."
title: Lobster
read_when:
...
gateway URL/auth context for nested OpenAgent CLI tool calls.
...
a bare envelope: OpenAgent creates or resumes a durable flow record and applies
...
persists in OpenAgent SQLite; Lobster's approval checkpoint is separate and must
...
- **No secrets** - Lobster doesn't manage OAuth; it calls OpenAgent tools that
  do.
