---
summary: "Export redacted trajectory bundles for debugging a Zero to Agent agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for a Zero to Agent session
...
Trajectory capture is Zero to Agent's per-session flight recorder. It records a
...
Zero to Agent writes the bundle under the workspace:
...
bundle; do not use allow-all. In group chats, Zero to Agent sends the approval
...
Trajectory capture is on by default for Zero to Agent agent runs.
...
| `metadata.json`       | Zero to Agent version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata |
...
This disables runtime trajectory capture before starting Zero to Agent.
...
Zero to Agent flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Zero to Agent:

```bash
export OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Zero to Agent logs an `openclaw-trajectory-flush` timeout and
...
Trajectory bundles are for support and debugging, not public posting. Zero to Agent
...
- confirm Zero to Agent was started without `OPENCLAW_TRAJECTORY=0`
