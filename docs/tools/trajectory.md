---
summary: "Export redacted trajectory bundles for debugging an OpenAgent agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an OpenAgent session
...
Trajectory capture is OpenAgent's per-session flight recorder. It records a
...
OpenAgent writes the bundle under the workspace:
...
bundle; do not use allow-all. In group chats, OpenAgent sends the approval
...
Trajectory capture is on by default for OpenAgent agent runs.
...
This disables runtime trajectory capture before starting OpenAgent.
...
OpenAgent flushes runtime trajectory rows during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting OpenAgent:

```bash
export OPENCLAW_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when OpenAgent logs an `openclaw-trajectory-flush` timeout and
...
Trajectory bundles are for support and debugging, not public posting. OpenAgent
...
- confirm OpenAgent was started without `OPENCLAW_TRAJECTORY=0`
