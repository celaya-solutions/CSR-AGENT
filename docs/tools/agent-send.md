...
`openagent agent` runs a single agent turn from the command line without an
...
output projection, and process status, use [`openagent agent exec`](/cli/agent#agent-exec).
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    openagent agent --agent main --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    openagent agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    openagent agent --session-id abc123 --message "Continue the task"
  ...
  <Step title="Deliver the reply to a channel">
    ...
    openagent agent --agent ops --message "Report ready" \
      --deliver --reply-channel telegram --reply-to 123456789

    # Deliver to a Discord channel
    openagent agent --agent ops --message "Generate report" \
...
openagent agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
...
<CardGroup cols={2}>
  <Card title="Agent CLI reference" href="/cli/agent" icon="terminal">
    Full `openagent agent` flag and option reference.
  </Card>
