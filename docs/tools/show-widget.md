...
`show_widget` is a core tool that shows a self-contained HTML widget on the user's current surface. Zero to Agent renders it inline in the Control UI and in iOS, Android, macOS, and Linux Quick Chat transcripts. The Linux dashboard uses the browser Control UI. In a Discord session with [Activities](/channels/discord-activities) enabled, the Discord plugin posts an **Open widget** button that launches it as an Activity.
...
When a widget presenter plugin is active, `presentation.target` also offers `node_panel`. Zero to Agent creates the same hosted widget document, selects a connected widget-panel-capable Mac, and opens its native panel at that document. The tool result names the selected Mac.

If no eligible Mac is connected or the node command fails, the widget still appears inline in chat and the result explains how to recover. Pair a Mac running Zero to Agent or open the macOS app, then retry. Widgets shown in a native panel are render-only. Widget actions are disabled there.
