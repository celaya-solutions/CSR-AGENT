...
read_when:
  - Running OpenAgent Gateway in WSL2 while Chrome lives on Windows
...
- OpenAgent points a browser profile at the address reachable from WSL2
...
```powershell
chrome.exe --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\OpenAgent\ChromeCDP"
```
...
If this fails, diagnose the Windows listeners below. OpenAgent is not the
problem yet.
...
that before touching OpenAgent config.

### Layer 3: configure the correct browser profile

Point OpenAgent at the address reachable from WSL2:
...
- use HTTP(S) when you want OpenAgent to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools
  socket URL
- test the same URL with `curl` before expecting OpenAgent to succeed
...
3. OpenAgent config: does `browser.profiles.<name>.cdpUrl` use that exact
...
from WSL2 second, and only then debug OpenAgent config or Control UI auth.
