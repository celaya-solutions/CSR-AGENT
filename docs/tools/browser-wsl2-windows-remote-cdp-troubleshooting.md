...
read_when:
  - Running Zero to Agent Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Zero to Agent Gateway runs inside WSL2, Chrome runs
...
- Zero to Agent points a browser profile at the address reachable from WSL2
...
```powershell
chrome.exe --remote-debugging-port=9222 --user-data-dir="$env:LOCALAPPDATA\Zero to Agent\ChromeCDP"
```
...
If this fails, diagnose the Windows listeners below. Zero to Agent is not the
problem yet.
...
that before touching Zero to Agent config.

### Layer 3: configure the correct browser profile

Point Zero to Agent at the address reachable from WSL2:
...
- use HTTP(S) when you want Zero to Agent to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools
  socket URL
- test the same URL with `curl` before expecting Zero to Agent to succeed
...
3. Zero to Agent config: does `browser.profiles.<name>.cdpUrl` use that exact
...
from WSL2 second, and only then debug Zero to Agent config or Control UI auth.
