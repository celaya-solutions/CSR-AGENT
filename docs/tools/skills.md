...
frontmatter and a markdown body. Zero to Agent loads bundled skills plus any local
...
When a session uses a different execution workspace, Zero to Agent also loads that
...
its parent repository. Installing Zero to Agent from a repository does not make that
repository's `.agents/skills/` a global bundled skill source.

Skill roots support grouped layouts. Zero to Agent discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** a Zero to Agent
  skill root. Use `openclaw migrate plan codex` to inventory those skills, then
  `openclaw migrate codex` to copy them into your Zero to Agent workspace.
</Note>

## Node-hosted skills

A connected headless node can publish skills installed in its active Zero to Agent
...
Zero to Agent resolves explicit references from authorized senders on every channel
...
distinct skills; Zero to Agent returns a visible error instead of ignoring extra or
...
<Note>
  Zero to Agent follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Zero to Agent keeps the skill's instructions out of the agent's normal
...
Zero to Agent filters skills at load time using `metadata.openclaw` (JSON5 object
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Zero to Agent lists each entry so you can
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** Zero to Agent does not auto-install Homebrew or translate brew
    ...
    - **Go:** Zero to Agent requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, Zero to Agent installs Go via
      ...
      dependency always targets a dedicated Zero to Agent-managed bin directory
...
When an agent run starts, Zero to Agent:

<Steps>
  <Step title="Reads skill metadata">
    Zero to Agent resolves the effective skill list for the agent, applying gating
...
`--plugin-dir`. Sessions with library selections use Zero to Agent's prompt catalog
and pinned revision paths instead. Zero to Agent omits `--plugin-dir` for those
...
Zero to Agent snapshots eligible skills **when a session starts** and reuses that
...
If the effective agent allowlist changes, Zero to Agent refreshes the snapshot to
keep visible skills aligned.

When native watch capacity is exhausted, Zero to Agent logs one warning and stops
...
<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, Zero to Agent watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, Zero to Agent can treat macOS-only skills as eligible when
    ...
    answering bin probes, Zero to Agent clears its cached bin matches.
...
When skills are eligible, Zero to Agent injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), Zero to Agent first preserves as many skill
...
For small context windows, the Zero to Agent embedded runtime further shortens the
