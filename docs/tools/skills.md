...
frontmatter and a markdown body. OpenAgent loads bundled skills plus any local
...
When a session uses a different execution workspace, OpenAgent also loads that
...
its parent repository. Installing OpenAgent from a repository does not make that
repository's `.agents/skills/` a global bundled skill source.

Skill roots support grouped layouts. OpenAgent discovers a skill whenever
...
A connected headless node can publish skills installed in its active OpenAgent
...
OpenAgent resolves explicit references from authorized senders on every channel
...
distinct skills; OpenAgent returns a visible error instead of ignoring extra or
...
<Note>
  OpenAgent follows the [AgentSkills](https://agentskills.io) spec. Frontmatter
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, OpenAgent keeps the skill's instructions out of the agent's normal
...
OpenAgent filters skills at load time using `metadata.openclaw` (JSON5 object
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, OpenAgent lists each entry so you can
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** OpenAgent does not auto-install Homebrew or translate brew
    ...
    - **Go:** OpenAgent requires Go 1.21 or newer for automatic skill installs.
      If `go` is missing and Homebrew is available, OpenAgent installs Go via
      ...
      dependency always targets a dedicated OpenAgent-managed bin directory
...
When an agent run starts, OpenAgent:

<Steps>
  <Step title="Reads skill metadata">
    OpenAgent resolves the effective skill list for the agent, applying gating
...
`--plugin-dir`. Sessions with library selections use OpenAgent's prompt catalog
and pinned revision paths instead. OpenAgent omits `--plugin-dir` for those
...
OpenAgent snapshots eligible skills **when a session starts** and reuses that
...
If the effective agent allowlist changes, OpenAgent refreshes the snapshot to
keep visible skills aligned.

When native watch capacity is exhausted, OpenAgent logs one warning and stops
...
<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, OpenAgent watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, OpenAgent can treat macOS-only skills as eligible when
    ...
    answering bin probes, OpenAgent clears its cached bin matches.
...
When skills are eligible, OpenAgent injects a compact XML block into the system
...
(`skills.limits.maxSkillsPromptChars`), OpenAgent first preserves as many skill
...
For small context windows, the OpenAgent embedded runtime further shortens the
