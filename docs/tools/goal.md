...
read_when:
  - You want Zero to Agent to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current Zero to Agent session.
...
Zero to Agent treats any text after `/goal` that is not a known action word as a
new objective.
...
stale or unknown token snapshot when the goal starts, Zero to Agent waits for the
...
Zero to Agent usage and model controls.

## Model tools

Zero to Agent exposes three goal tools to agent harnesses:
...
Zero to Agent keeps the line compact by truncating long objectives. Paused,
...
`/goal` works in command-capable Zero to Agent sessions, including the TUI and
...
fresh token snapshot yet. Usage refreshes as Zero to Agent records session usage
