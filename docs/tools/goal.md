...
read_when:
  - You want OpenAgent to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current OpenAgent session.
...
OpenAgent treats any text after `/goal` that is not a known action word as a
new objective.
...
stale or unknown token snapshot when the goal starts, OpenAgent waits for the
...
OpenAgent usage and model controls.

## Model tools

OpenAgent exposes three goal tools to agent harnesses:
...
OpenAgent keeps the line compact by truncating long objectives. Paused,
...
`/goal` works in command-capable OpenAgent sessions, including the TUI and
...
fresh token snapshot yet. Usage refreshes as OpenAgent records session usage
