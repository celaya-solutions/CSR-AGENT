...
<Steps>
  <Step title="Install the plugin">
    ...
    OpenAgent, so the install needs a scoped locator. The `clawhub:` prefix
...
- Rendered PNG/PDF files remain temporary materializations under `$TMPDIR/openclaw-diffs` because channel delivery requires a file path. SQLite owns their expiry metadata. OpenAgent writes no JSON sidecars.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in OpenAgent config.
  </Step>
