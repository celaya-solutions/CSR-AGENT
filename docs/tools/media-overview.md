...
read_when:
  - Looking for an overview of Zero to Agent's media capabilities
...
Zero to Agent generates images, videos, and music, understands inbound media
...
Zero to Agent until the shared realtime-voice contract can represent it.
</Note>
...
For async tools, Zero to Agent submits the request to the provider, returns a task
...
Zero to Agent wakes the agent with the generated media paths so it can tell the
...
Zero to Agent sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    other native model types; Zero to Agent has no provider contract for those
  ...
  <Accordion title="xAI">
    ...
    not registered in Zero to Agent until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
