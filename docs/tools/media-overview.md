...
read_when:
  - Looking for an overview of OpenAgent's media capabilities
...
OpenAgent generates images, videos, and music, understands inbound media
...
For async tools, OpenAgent submits the request to the provider, returns a task
...
OpenAgent wakes the agent with the generated media paths so it can tell the
