...
read_when:
  ...
  - Comparing Zero to Agent exec approvals with ACPX harness permissions
...
Set ACPX permissions separately from Zero to Agent exec approvals:
...
| Use Codex/Zero to Agent auto-review before humans | `tools.exec.mode: "auto"`                                                              |
...
Outside the full-permission Gateway session exception described above, host exec uses the stricter result of Zero to Agent config and the host-local approvals file. ACPX harness permissions do not loosen host exec approvals, and host exec approvals do not loosen ACPX harness prompts.
