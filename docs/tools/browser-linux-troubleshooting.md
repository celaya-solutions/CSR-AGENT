---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for OpenAgent browser control on Linux"
...
Snap's AppArmor confinement interferes with how OpenAgent spawns and monitors
the browser process.

Other common Linux launch failures:

- `The profile appears to be in use by another Chromium process`: stale
  `Singleton*` lock files in the managed profile directory. OpenAgent removes
...
If you must keep snap Chromium, configure OpenAgent to attach to a
...
[Unit]
Description=OpenAgent Browser (Chrome CDP)
...
These calls go to the OpenAgent browser control service, not to the Chrome CDP
...
- Local `openclaw`-driver profiles get a `cdpPort` allocated when OpenAgent
