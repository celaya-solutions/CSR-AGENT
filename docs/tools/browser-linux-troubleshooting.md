---
summary: "Fix Chrome/Brave/Edge/Chromium CDP startup issues for Zero to Agent browser control on Linux"
...
Snap's AppArmor confinement interferes with how Zero to Agent spawns and monitors
the browser process.

Other common Linux launch failures:

- `The profile appears to be in use by another Chromium process`: stale
  `Singleton*` lock files in the managed profile directory. Zero to Agent removes
...
- `Missing X server or $DISPLAY`: a visible browser was explicitly requested
  ...
  Zero to Agent in a real desktop session.
...
If you must keep snap Chromium, configure Zero to Agent to attach to a
...
[Unit]
Description=Zero to Agent Browser (Chrome CDP)
...
These calls go to the Zero to Agent browser control service, not to the Chrome CDP
...
- Local `openclaw`-driver profiles get a `cdpPort` allocated when Zero to Agent
