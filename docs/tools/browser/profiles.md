...
- `user`: built-in Chrome DevTools MCP attach profile for your **real
  ...
  prompt the first time Zero to Agent attaches, so someone must be at the computer.
- `chrome`: built-in [Chrome extension](/tools/chrome-extension) profile for
  ...
  desk because it drives tabs through the Zero to Agent browser extension instead of
...
Preview cards appear only for HTTP(S) page URLs when Zero to Agent can identify the
...
preserve the active Chrome tab when Zero to Agent can verify that the attached browser
