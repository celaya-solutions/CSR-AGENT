...
read_when:
  ...
  - You want OpenAgent to launch Brave, Edge, or another Chromium browser
---
...
control service. Pending browser operations are cancelled and OpenAgent-owned
...
OpenAgent disconnects its control sessions. Browser control starts again on the
...
Session tab cleanup applies only to tabs created by the OpenAgent browser tool
with `action: "open"`. OpenAgent does not adopt tabs that were already open,
...
OpenAgent-managed Chrome also applies a separate, best-effort cap of eight page
...
safely after restart. This durable path can cover OpenAgent-managed profiles,
regular remote CDP profiles, and existing-session profiles with an explicit
`cdpUrl`, provided OpenAgent can resolve both the native target and a stable
browser identity. Before closing a durable record, OpenAgent verifies that the
...
but OpenAgent does not automatically attach them to channel replies. To share a
...
understanding model is configured, OpenAgent keeps the normal image result so the
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, OpenAgent pauses new launch
...
- Guarded remote CDP connections now fail closed when the selected driver cannot
  ...
  default strict Browser policy because Chrome MCP cannot carry OpenAgent's
...
- `driver: "extension"` drives your signed-in Chrome through the [OpenAgent Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
OpenAgent uses it automatically. Set `browser.executablePath` to override
...
Per-profile `executablePath` only affects local managed profiles that OpenAgent
