...
read_when:
  ...
  - You want Zero to Agent to launch Brave, Edge, or another Chromium browser
---
...
control service. Pending browser operations are cancelled and Zero to Agent-owned
...
Zero to Agent disconnects its control sessions. Browser control starts again on the
...
Session tab cleanup applies only to tabs created by the Zero to Agent browser tool
with `action: "open"`. Zero to Agent does not adopt tabs that were already open,
...
Zero to Agent-managed Chrome also applies a separate, best-effort cap of eight page
...
safely after restart. This durable path can cover Zero to Agent-managed profiles,
regular remote CDP profiles, and existing-session profiles with an explicit
`cdpUrl`, provided Zero to Agent can resolve both the native target and a stable
browser identity. Before closing a durable record, Zero to Agent verifies that the
...
but Zero to Agent does not automatically attach them to channel replies. To share a
...
understanding model is configured, Zero to Agent keeps the normal image result so the
...
- Local `openclaw` profiles use a CDP port range starting 9 ports above the control port (default `18800`-`18899`). Zero to Agent allocates from that range for
...
- Repeated managed Chrome launch/readiness failures are circuit-broken per
  profile. After several consecutive failures, Zero to Agent pauses new launch
...
- Guarded remote CDP connections now fail closed when the selected driver cannot
  ...
  default strict Browser policy because Chrome MCP cannot carry Zero to Agent's
...
- Gateway/provider `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, and `NO_PROXY` environment variables do not automatically proxy the Zero to Agent-managed browser. Managed Chrome launches direct by default so provider proxy settings do not weaken browser SSRF checks.
- Zero to Agent-managed local CDP readiness probes and DevTools WebSocket connections bypass the managed network proxy for the exact launched loopback endpoint, so `openclaw browser start` still works when an operator proxy blocks loopback egress.
...
- `POST /start?headless=true` and `openclaw browser start --headless` request a
  ...
  remote CDP profiles reject the override because Zero to Agent does not launch those
  browser processes.
...
- `driver: "extension"` drives your signed-in Chrome through the [Zero to Agent Chrome extension](/tools/chrome-extension). The relay owns its loopback endpoint, so these profiles do not accept `cdpUrl`. This is the only signed-in-browser mode that works with nobody at the computer.
...
Zero to Agent uses it automatically. Set `browser.executablePath` to override
...
Per-profile `executablePath` only affects local managed profiles that Zero to Agent
