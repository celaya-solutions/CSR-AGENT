...
- **CDP startup or readiness failure** means OpenAgent cannot confirm that the browser control plane is healthy.
...
- For the local loopback `openclaw` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for OpenAgent's own local control plane.
