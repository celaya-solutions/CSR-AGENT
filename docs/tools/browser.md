---
summary: "Index of the OpenAgent browser documentation, one page per reader job"
read_when:
...
OpenAgent can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls. It runs through a small local control service inside the Gateway (loopback only) and is isolated from your personal browser.
