...
normalized results, cached by query for 15 minutes (configurable). Zero to Agent
...
<Steps>
  ...
  <Step title="Configure">
    ...
    [Zero to Agent](/cli/openclaw): say `configure web search` in `openclaw setup`
    or in the Control UI's **Settings → Ask Zero to Agent** chat. The hosted flow
  ...
  <Step title="Use it">
    ```javascript
    await web_search({ query: "Zero to Agent plugin SDK" });
    ```
...
no `provider` is set, Zero to Agent checks providers in this order and uses the
...
If an auto-detected provider fails, Zero to Agent tries the next eligible provider.
...
`openclaw configure --section web`. Zero to Agent does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Zero to Agent resolves only the
...
`web_search` tool automatically when Zero to Agent web search is enabled and no
...
search and Zero to Agent's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. Zero to Agent uses the
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  Zero to Agent's managed `web_search` instead
...
use the managed `web_search` fallback through Zero to Agent's dynamic tool namespace.
Use an explicit managed provider when you need Zero to Agent's provider-specific
...
Managed HTTP `web_search` provider calls use Zero to Agent's guarded fetch path,
scoped to the current provider's own hostname. For that hostname only,
Zero to Agent allows Surge, Clash, and sing-box fake-IP DNS answers in
...
`tools.web.search.cacheTtlMinutes` controls Zero to Agent's local search-result
...
Zero to Agent keeps startup resilient and reports a warning so you can reinstall the
...
- or omit that field and let Zero to Agent auto-detect the first ready web-fetch
...
`openclaw configure --section web`, Zero to Agent can also ask for:
...
Zero to Agent also offers optional `x_search` setup with the same credential right
...
provider, Zero to Agent does not show the `x_search` prompt.
...
optional structured filters. Zero to Agent constructs the built-in xAI `x_search`
...
`plugins.entries.xai.config.xSearch.cacheTtlMinutes` controls Zero to Agent's local
