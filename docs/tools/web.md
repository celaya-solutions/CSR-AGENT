...
<Steps>
  ...
  <Step title="Configure">
    ...
    or in the Control UI's **Settings → Ask OpenAgent** chat. The hosted flow
...
no `provider` is set, OpenAgent checks providers in this order and uses the
...
If an auto-detected provider fails, OpenAgent tries the next eligible provider.
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, OpenAgent resolves only the
...
`openclaw configure --section web`, OpenAgent can also ask for:
...
OpenAgent also offers optional `x_search` setup with the same credential right
...
provider, OpenAgent does not show the `x_search` prompt.
...
optional structured filters. OpenAgent constructs the built-in xAI `x_search`
...
`plugins.entries.xai.config.xSearch.cacheTtlMinutes` controls OpenAgent's local
