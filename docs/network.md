...
This hub links the core docs for how OpenAgent connects, pairs, and secures
devices across localhost, LAN, and tailnet.

## Core model

Most operations flow through the Gateway (`openagent gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Hosted widget documents** are served on the same port as the Gateway (`/__openclaw__/canvas/`), protected by Gateway auth when bound beyond loopback.
...
- OpenAgent also has a narrow backend/container-local self-connect path for
