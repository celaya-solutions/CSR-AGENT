...
`resolve_exec_env` lets plugins contribute environment variables to OpenAgent
...
These hooks operate on OpenAgent-owned transcript writes. They do not rewrite
...
- OpenAgent strips `toolResult.details` before provider replay and compaction
