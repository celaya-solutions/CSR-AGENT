...
`resolve_exec_env` lets plugins contribute environment variables to Zero to Agent
...
These hooks operate on Zero to Agent-owned transcript writes. They do not rewrite
...
- Zero to Agent strips `toolResult.details` before provider replay and compaction
