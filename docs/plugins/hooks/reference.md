...
| Option                  | Effect                                                                                                                                                                                                                                                      |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `matcher`               | Non-empty list of canonical Zero to Agent tool ids handled by `before_tool_call` or `after_tool_call`, such as `exec`, `apply_patch`, or `spawn_agent`. Omit to match all tools. Empty lists, wildcards, blanks, and provider-specific aliases are invalid. |
...
`inbound_claim` is not a global pre-routing broadcast. Zero to Agent invokes it only
...
If the budget expires, Zero to Agent logs `shutdown session-end drain timed out`
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when Zero to Agent has resolved the child session's native model before launch.
...
benchmarks, model-based graders, or other third-party evaluators. Zero to Agent
...
When evaluation input includes `correlationId`, Zero to Agent forwards it to the
...
revise with that hash and a correlation id, then repeat. Zero to Agent does not
