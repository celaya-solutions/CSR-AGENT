...
| Option                  | Effect                                                                                                                                                                                                                                                      |
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `matcher`               | Non-empty list of canonical OpenAgent tool ids handled by `before_tool_call` or `after_tool_call`, such as `exec`, `apply_patch`, or `spawn_agent`. Omit to match all tools. Empty lists, wildcards, blanks, and provider-specific aliases are invalid. |
...
`inbound_claim` is not a global pre-routing broadcast. OpenAgent invokes it only
...
If the budget expires, OpenAgent logs `shutdown session-end drain timed out`
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when OpenAgent has resolved the child session's native model before launch.
...
benchmarks, model-based graders, or other third-party evaluators. OpenAgent
...
When evaluation input includes `correlationId`, OpenAgent forwards it to the
...
revise with that hash and a correlation id, then repeat. OpenAgent does not
