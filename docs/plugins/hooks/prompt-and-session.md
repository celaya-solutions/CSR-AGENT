...
with the first OpenAgent version they build against for this contract. Older
...
When a run is blocked, OpenAgent stores only the replacement text in
...
`before_agent_finalize`, receive it when OpenAgent can identify the active run;
...
the conversation target identifier when OpenAgent can derive one from the
...
leave the hook promise pending forever. A timeout is logged and OpenAgent continues; it does not
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when OpenAgent can derive a
...
Handlers have a 15s default budget; on timeout, OpenAgent logs the failure and
...
Codex native `Stop` hooks are relayed into this hook as OpenAgent
...
behavior). On the embedded and CLI prompt-preparation paths, OpenAgent drains
