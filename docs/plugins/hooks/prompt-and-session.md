...
with the first Zero to Agent version they build against for this contract. Older
...
When a run is blocked, Zero to Agent stores only the replacement text in
...
`before_agent_finalize`, receive it when Zero to Agent can identify the active run;
...
the conversation target identifier when Zero to Agent can derive one from the
...
leave the hook promise pending forever. A timeout is logged and Zero to Agent continues; it does not
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Zero to Agent can derive a
...
Handlers have a 15s default budget; on timeout, Zero to Agent logs the failure and
...
Codex native `Stop` hooks are relayed into this hook as Zero to Agent
...
behavior). On the embedded and CLI prompt-preparation paths, Zero to Agent drains
