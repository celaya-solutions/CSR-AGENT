...
read_when:
  ...
  - You are projecting Zero to Agent cron wakes into an external host scheduler
...
policy runs from Zero to Agent config, covers CLI install and update paths, and
...
Zero to Agent no longer runs built-in install-time dangerous-code blocking, so it
...
Keep Zero to Agent as the source of truth for due checks and execution.
...
non-retryable configuration before registration. Zero to Agent does not provide an
