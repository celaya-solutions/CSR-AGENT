...
read_when:
  ...
  - You are projecting OpenAgent cron wakes into an external host scheduler
...
policy runs from OpenAgent config, covers CLI install and update paths, and
...
OpenAgent no longer runs built-in install-time dangerous-code blocking, so it
...
Keep OpenAgent as the source of truth for due checks and execution.
...
non-retryable configuration before registration. OpenAgent does not provide an
