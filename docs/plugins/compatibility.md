...
read_when:
  - You maintain a Zero to Agent plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Zero to Agent keeps older plugin contracts wired through named compatibility
...
Plugin authors should pin and test supported Zero to Agent host versions. This
...
Zero to Agent should not remove a documented plugin contract in the same release
...
The plugin inspector should live outside the core Zero to Agent repo as a
...
machine-readable output in CI annotations. Zero to Agent core should expose
...
lane when validating the external inspector against Zero to Agent plugin
packages. Run it from a clean Zero to Agent checkout after the package is built:
