...
read_when:
  - You maintain an OpenAgent plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

OpenAgent keeps older plugin contracts wired through named compatibility
...
Plugin authors should pin and test supported OpenAgent host versions. This
...
OpenAgent should not remove a documented plugin contract in the same release
...
The plugin inspector should live outside the core OpenAgent repo as a
...
machine-readable output in CI annotations. OpenAgent core should expose
...
lane when validating the external inspector against OpenAgent plugin
packages. Run it from a clean OpenAgent checkout after the package is built:
