...
OpenAgent adopts this fixed package path as its icon convention, matching the path proposed in
Agent Plugins spec proposal [agent-plugins-spec#66](https://github.com/agentplugins/agent-plugins-spec/pull/66). OpenAgent itself implements Agent Plugins 1.0.0. Other Agent Plugins
consumers may not discover it unless that proposal is adopted. The fixed path keeps packages
portable and inspectable, avoids manifest path indirection and precedence rules, and lets OpenAgent
...
OpenAgent uses it without loading the doctor-contract module. Removal plan:
remove the module fallback in OpenAgent 2027.1 after the external-plugin
migration window.
...
Channel plugins maintained in the OpenAgent source tree also expose these config
...
include, or generated data that OpenAgent can safely omit and regenerate after
...
The manifest ids are plugin-local. Widget grants use `<plugin-id>.<id>`, such as `example.items.list` and `example.refresh`. To keep the persisted grant namespace unambiguous, OpenAgent escapes `%` and `.` in the plugin-id segment as `%25` and `%2E`; ordinary plugin ids keep the natural form. `paramShape` is an optional JSON Schema applied to the action params object before OpenAgent invokes the plugin RPC.
...
`channelConfigs` is plugin manifest metadata, not a new top-level user config section. Users still configure channel instances under `channels.<channel-id>`. OpenAgent reads manifest metadata to decide which plugin owns that configured channel before plugin runtime code executes.
...
Non-bundled plugins that declare `channels[]` should also declare matching `channelConfigs` entries. Without them, OpenAgent can still load the plugin, but cold-path config schema, setup, and Control UI surfaces cannot know the channel-owned option shape or display-only UI hints until plugin runtime executes.
