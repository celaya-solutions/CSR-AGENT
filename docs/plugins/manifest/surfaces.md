...
Zero to Agent adopts this fixed package path as its icon convention, matching the path proposed in
Agent Plugins spec proposal [agent-plugins-spec#66](https://github.com/agentplugins/agent-plugins-spec/pull/66). Zero to Agent itself implements Agent Plugins 1.0.0. Other Agent Plugins
consumers may not discover it unless that proposal is adopted. The fixed path keeps packages
portable and inspectable, avoids manifest path indirection and precedence rules, and lets Zero to Agent
...
Zero to Agent uses it without loading the doctor-contract module. Removal plan:
remove the module fallback in Zero to Agent 2027.1 after the external-plugin
migration window.
...
Channel plugins maintained in the Zero to Agent source tree also expose these config
...
include, or generated data that Zero to Agent can safely omit and regenerate after
...
The manifest ids are plugin-local. Widget grants use `<plugin-id>.<id>`, such as `example.items.list` and `example.refresh`. To keep the persisted grant namespace unambiguous, Zero to Agent escapes `%` and `.` in the plugin-id segment as `%25` and `%2E`; ordinary plugin ids keep the natural form. `paramShape` is an optional JSON Schema applied to the action params object before Zero to Agent invokes the plugin RPC.
...
Use `commandAliases` when a plugin owns a runtime command name that users may mistakenly put in `plugins.allow` or try to run as a root CLI command. Zero to Agent uses this metadata for diagnostics without importing plugin runtime code.
...
`channelConfigs` is plugin manifest metadata, not a new top-level user config section. Users still configure channel instances under `channels.<channel-id>`. Zero to Agent reads manifest metadata to decide which plugin owns that configured channel before plugin runtime code executes.
...
Non-bundled plugins that declare `channels[]` should also declare matching `channelConfigs` entries. Without them, Zero to Agent can still load the plugin, but cold-path config schema, setup, and Control UI surfaces cannot know the channel-owned option shape or display-only UI hints until plugin runtime executes.
...
When `channels.chat` is configured, Zero to Agent considers both the channel id and the preferred plugin id. If the lower-priority plugin was only selected because it is bundled or enabled by default, Zero to Agent disables it in the effective runtime config so one plugin owns the channel and its tools. Explicit user selection still wins: if the user explicitly enables both plugins (via `plugins.allow` or a material `plugins.entries` config), Zero to Agent preserves that choice and reports duplicate channel/tool diagnostics instead of silently changing the requested plugin set.
