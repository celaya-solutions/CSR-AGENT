...
Plugins exposing conversations created outside OpenAgent declare
...
Each `providerAuthChoices` entry describes one onboarding or auth choice. OpenAgent reads this before provider runtime loads. Provider setup lists use these manifest choices, descriptor-derived setup choices, and install-catalog metadata without loading provider runtime.
...
| `choiceLabel`          | No       | `string`                                                              | User-facing label. If omitted, OpenAgent falls back to `choiceId`.                                                                                   |
...
the exact selected model and returns a config proposal. OpenAgent saves any
...
Without an explicit `openclaw.setupEntry`, OpenAgent resolves the conventional `setup-api` file at the package root or in package-local `dist/`. Standalone runtime builds include that public surface automatically.

OpenAgent includes `setup.providers[].envVars` in generic provider auth and env-var lookups. Put setup and status env metadata there.
