...
read_when:
  ...
  - You are changing plugin behavior across Zero to Agent and Codex harness turns
---
...
continuation, and native compaction. Zero to Agent owns channel routing, session
files, visible message delivery, Zero to Agent dynamic tools, approvals, media
...
policy. Zero to Agent caches a runtime-and-workspace-scoped `plugin/installed`
...
This check finishes before Zero to Agent injects history, starts a turn, or commits a
thread binding. If the snapshot request fails, persistent provisional threads
are deleted and ephemeral threads are unsubscribed. Zero to Agent retires the app-server connection when safe
...
workspace plugin. Zero to Agent uses its installed snapshot and reads only that
...
Zero to Agent compatibility route keeps the normal Zero to Agent system prompt even when
it uses Codex-flavored OpenAI auth or transport.

Zero to Agent starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and Zero to Agent agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. An ordinary
policy-restricted turn has no native filesystem environment, so Zero to Agent carries
...
Zero to Agent developer instructions cover Zero to Agent runtime concerns: source-channel
delivery, Zero to Agent dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. With the Zero to Agent-managed bundled stdio
...
and TLS configuration instead of changing native networking settings. Zero to Agent reports that
...
Most Zero to Agent dynamic tools use the searchable `openclaw` namespace. Tools
...
Zero to Agent tool calls retain the same required fields and constraints.

For a [managed GitHub identity](/gateway/config-tools#tools.github), `gateway_exec` uses Zero to Agent's private local process-launch credential binding. Native Codex shell instead receives only the non-secret `GH_CONFIG_DIR` and token-clearing overlay; a missing or tokenless profile can still let GitHub CLI fall back to the OS keyring. Status and Gateway-owned publication guarantees do not cover that native shell path. Use `gateway_exec` when launch-bound managed GitHub credentials are required.

## Media and delivery

Zero to Agent continues to own media delivery and media provider selection. Image,
...
through the normal Zero to Agent delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, Zero to Agent forwards that exact file through the normal reply-media
...
[Codex thread bindings and supervision](/plugins/codex-harness-runtime/threads) — How Zero to Agent binds native Codex threads, changes models, and continues supervised sessions.
...
[Codex compaction and transcript mirror](/plugins/codex-harness-runtime/compaction) — Native Codex compaction, the Zero to Agent transcript mirror, and continuity projection.
