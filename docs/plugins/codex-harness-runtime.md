...
read_when:
  ...
  - You are changing plugin behavior across OpenAgent and Codex harness turns
---
...
continuation, and native compaction. OpenAgent owns channel routing, session
files, visible message delivery, OpenAgent dynamic tools, approvals, media
...
policy. OpenAgent caches a runtime-and-workspace-scoped `plugin/installed`
...
This check finishes before OpenAgent injects history, starts a turn, or commits a
thread binding. If the snapshot request fails, persistent provisional threads
are deleted and ephemeral threads are unsubscribed. OpenAgent retires the app-server connection when safe
...
workspace plugin. OpenAgent uses its installed snapshot and reads only that
...
OpenAgent compatibility route keeps the normal OpenAgent system prompt even when
it uses Codex-flavored OpenAI auth or transport.

OpenAgent starts and resumes native Codex threads with Codex's built-in
personality disabled (`personality: "none"`) so workspace personality files
and OpenAgent agent identity stay authoritative. Native Codex keeps Codex-owned
base/model instructions and project-doc loading otherwise. An ordinary
policy-restricted turn has no native filesystem environment, so OpenAgent carries
...
OpenAgent developer instructions cover OpenAgent runtime concerns: source-channel
delivery, OpenAgent dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. With the OpenAgent-managed bundled stdio
...
and TLS configuration instead of changing native networking settings. OpenAgent reports that
...
Most OpenAgent dynamic tools use the searchable `openclaw` namespace. Tools
...
OpenAgent tool calls retain the same required fields and constraints.

For a [managed GitHub identity](/gateway/config-tools#tools.github), `gateway_exec` uses OpenAgent's private local process-launch credential binding. Native Codex shell instead receives only the non-secret `GH_CONFIG_DIR` and token-clearing overlay; a missing or tokenless profile can still let GitHub CLI fall back to the OS keyring. Status and Gateway-owned publication guarantees do not cover that native shell path. Use `gateway_exec` when launch-bound managed GitHub credentials are required.

## Media and delivery

OpenAgent continues to own media delivery and media provider selection. Image,
...
through the normal OpenAgent delivery path; media generation does not require
the legacy runtime. When Codex emits a native image-generation item with a
`savedPath`, OpenAgent forwards that exact file through the normal reply-media
...
[Codex thread bindings and supervision](/plugins/codex-harness-runtime/threads) — How OpenAgent binds native Codex threads, changes models, and continues supervised sessions.
...
[Codex compaction and transcript mirror](/plugins/codex-harness-runtime/compaction) — Native Codex compaction, the OpenAgent transcript mirror, and continuity projection.
