...
- External session catalogs use
  ...
  target before Zero to Agent advertises model-chat creation. Native terminal readiness
...
- `kind` is deprecated: declare an exclusive slot (`"memory"` or
  ...
  older plugins.
- `configSchema` can be a function for lazy evaluation. Zero to Agent resolves and
...
- A `nodeHostCommands` descriptor can define `isAvailable({ config, env })`.
  ...
  node's Gateway declaration. Zero to Agent evaluates it against the node-local
