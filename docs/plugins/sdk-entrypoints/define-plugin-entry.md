...
- External session catalogs use
  ...
  target before OpenAgent advertises model-chat creation. Native terminal readiness
...
- `kind` is deprecated: declare an exclusive slot (`"memory"` or
  ...
  older plugins.
- `configSchema` can be a function for lazy evaluation. OpenAgent resolves and
...
- A `nodeHostCommands` descriptor can define `isAvailable({ config, env })`.
  ...
  node's Gateway declaration. OpenAgent evaluates it against the node-local
