...
Zero to Agent namespaces this as `plugin.<plugin-id>.changed`. Event names are one
...
evaluate the plugin entry and the channel plugin object so Zero to Agent can
