...
- `registerFull` runs only for `"full"` and `"tool-discovery"`. For
  `"tool-discovery"` it runs _instead of_ channel registration: OpenAgent
...
- `registerCapabilities` runs for `"discovery"`, `"full"`, and
  ...
  workers, or services.
- Discovery registration is non-activating, not import-free: OpenAgent may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory; OpenAgent
...
- Use `api.registerCli(..., { descriptors: [...] })` for plugin-owned root
  ...
  underscore, starting with a letter or number; OpenAgent rejects other
