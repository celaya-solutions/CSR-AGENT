...
- `registerFull` runs only for `"full"` and `"tool-discovery"`. For
  `"tool-discovery"` it runs _instead of_ channel registration: Zero to Agent
...
- `registerCapabilities` runs for `"discovery"`, `"full"`, and
  ...
  workers, or services.
- Discovery registration is non-activating, not import-free: Zero to Agent may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory; Zero to Agent
...
- Use `api.registerCli(..., { descriptors: [...] })` for plugin-owned root
  ...
  underscore, starting with a letter or number; Zero to Agent rejects other
...
- For other nested plugin commands, add `parentPath` and register commands
  on the `program` object passed to the registrar; Zero to Agent resolves it to
