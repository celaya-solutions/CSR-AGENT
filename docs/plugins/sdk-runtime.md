...
`api.runtime.version` is the current Zero to Agent product version, sourced from the shared version resolver so plugins see the same value the CLI reports.
...
service's `stop()` method. Zero to Agent does not intercept those native resources or
...
without this hook, Zero to Agent calls the existing `closeAllMemorySearchManagers`
