...
`api.runtime.version` is the current OpenAgent product version, sourced from the shared version resolver so plugins see the same value the CLI reports.
...
service's `stop()` method. OpenAgent does not intercept those native resources or
...
without this hook, OpenAgent calls the existing `closeAllMemorySearchManagers`
