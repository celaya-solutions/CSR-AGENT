---
summary: "Configure native Codex plugins for Codex-mode OpenAgent agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode OpenAgent agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are discovering or installing a Codex marketplace plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode OpenAgent agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the OpenAgent turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. OpenAgent does not translate
Codex plugins into synthetic `codex_plugin_*` OpenAgent dynamic tools.
...
- Migration supports only `openai-curated` plugins that it observed as
  ...
  OpenAgent treats both names as the one curated catalog, so configured
...
`codexPlugins` has no effect on OpenAgent-provider runs, ACP conversation
...
owner does not need to add that marketplace to OpenAgent configuration before
...
allowlisted, those requirements still apply; OpenAgent does not bypass them.
...
OpenAgent install command afterward to authorize the already-installed plugin.
OpenAgent keeps apps hidden when the response omits the exact marketplace,
...
Installing the plugin bundle and configuring OpenAgent app access do not confirm
hosted app connections. When installation returns apps that still need sign-in,
OpenAgent provides **Open &lt;app&gt; in ChatGPT** links to the app pages returned by
...
When an authenticated owner creates an automation from a Codex turn, OpenAgent
...
Jobs created before app authority capture may keep their ordinary OpenAgent
...
This searches Codex catalogs, not OpenAgent's plugin registry or every ChatGPT
...
shared OpenAgent app access separately. Changes to OpenAgent app access take effect
...
declaration or setup URL alone does not establish that access. OpenAgent app
access can be disabled while an eligible ChatGPT page remains available;
opening that page does not enable OpenAgent app access. Plugins without hosted
...
OpenAgent app access. Status does not infer a separate connection state. Missing
...
configured plugin. It invalidates that runtime's OpenAgent app cache, calls
...
OpenAgent app access; local app-access changes take effect on the next message.
...
the `operator.admin` scope. OpenAgent's reserved `/codex` command is dispatched
...
again. `enable` and `disable` change OpenAgent's persisted policy; qualified
...
- During planning, OpenAgent reads source Codex `plugin/read` details and
...
For explicitly approved plugins from any discovered marketplace, OpenAgent uses
...
- OpenAgent does not add new Git or local marketplace sources in this flow.
  Additional sources must already be configured in Codex or be discoverable
  from the bound repository.

## App inventory and ownership

OpenAgent first reads and caches one `plugin/installed` snapshot scoped to the
...
OpenAgent reads installed app runtime state through `app/installed` and fetches
...
configured curated plugins are installed, OpenAgent combines their cache
invalidations into a single app-inventory refresh. Ordinary cached reads do
not force a connector refresh for every new thread. OpenAgent caches the
...
A plugin app is exposed only when OpenAgent can map it back to the configured
...
and callable for that thread. OpenAgent does not install, authenticate, or enable
...
names, OpenAgent uses its `plugin/installed` snapshot and reads only that exact
...
OpenAgent injects a restrictive `config.apps` patch for the Codex thread:
...
snapshot while `_default` is disabled. OpenAgent provisionally admits only
...
non-callable, OpenAgent logs one warning listing the unavailable apps and
...
or used. OpenAgent deletes a failed persistent provisional thread, unsubscribes
...
get `open_world_enabled: true`. OpenAgent does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: OpenAgent returns a deterministic decline.
- `true`: OpenAgent auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: OpenAgent exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into OpenAgent plugin
  approvals before returning the Codex approval response.
- `"ask"`: OpenAgent uses the same Codex write/destructive gating as
  ...
  For each admitted app using `"ask"`, OpenAgent selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  OpenAgent; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
account-wide default can become callable after OpenAgent starts and verifies
...
OpenAgent does not authenticate plugin apps on the owner's behalf.

For `plugin_detail_unavailable`, verify that the exact installed marketplace
and plugin identity select a matching `plugin/read` result. OpenAgent keeps
...
Codex thread bindings keep the app config they started with until OpenAgent
