---
summary: "Configure native Codex plugins for Codex-mode Zero to Agent agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Zero to Agent agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are discovering or installing a Codex marketplace plugin
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Zero to Agent agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Zero to Agent turn. Plugin calls stay in the native Codex transcript;
Codex app-server owns app-backed MCP execution. Zero to Agent does not translate
Codex plugins into synthetic `codex_plugin_*` Zero to Agent dynamic tools.
...
- Migration supports only `openai-curated` plugins that it observed as
  ...
  Zero to Agent treats both names as the one curated catalog, so configured
...
`codexPlugins` has no effect on Zero to Agent-provider runs, ACP conversation
...
owner does not need to add that marketplace to Zero to Agent configuration before
...
allowlisted, those requirements still apply; Zero to Agent does not bypass them.
...
Zero to Agent install command afterward to authorize the already-installed plugin.
Zero to Agent keeps apps hidden when the response omits the exact marketplace,
...
Installing the plugin bundle and configuring Zero to Agent app access do not confirm
hosted app connections. When installation returns apps that still need sign-in,
Zero to Agent provides **Open &lt;app&gt; in ChatGPT** links to the app pages returned by
...
When an authenticated owner creates an automation from a Codex turn, Zero to Agent
...
Jobs created before app authority capture may keep their ordinary Zero to Agent
...
This searches Codex catalogs, not Zero to Agent's plugin registry or every ChatGPT
...
shared Zero to Agent app access separately. Changes to Zero to Agent app access take effect
...
declaration or setup URL alone does not establish that access. Zero to Agent app
access can be disabled while an eligible ChatGPT page remains available;
opening that page does not enable Zero to Agent app access. Plugins without hosted
...
Zero to Agent app access. Status does not infer a separate connection state. Missing
...
configured plugin. It invalidates that runtime's Zero to Agent app cache, calls
...
Zero to Agent app access; local app-access changes take effect on the next message.
...
the `operator.admin` scope. Zero to Agent's reserved `/codex` command is dispatched
...
again. `enable` and `disable` change Zero to Agent's persisted policy; qualified
...
| Enabled    | Codex reports the plugin enabled, and Zero to Agent config allows it for Codex harness turns.                                      |
...
- During planning, Zero to Agent reads source Codex `plugin/read` details and
...
For explicitly approved plugins from any discovered marketplace, Zero to Agent uses
...
- Zero to Agent does not add new Git or local marketplace sources in this flow.
  Additional sources must already be configured in Codex or be discoverable
  from the bound repository.

## App inventory and ownership

Zero to Agent first reads and caches one `plugin/installed` snapshot scoped to the
...
Zero to Agent reads installed app runtime state through `app/installed` and fetches
...
configured curated plugins are installed, Zero to Agent combines their cache
invalidations into a single app-inventory refresh. Ordinary cached reads do
not force a connector refresh for every new thread. Zero to Agent caches the
...
A plugin app is exposed only when Zero to Agent can map it back to the configured
...
and callable for that thread. Zero to Agent does not install, authenticate, or enable
...
names, Zero to Agent uses its `plugin/installed` snapshot and reads only that exact
...
Zero to Agent injects a restrictive `config.apps` patch for the Codex thread:
...
snapshot while `_default` is disabled. Zero to Agent provisionally admits only
...
non-callable, Zero to Agent logs one warning listing the unavailable apps and
...
or used. Zero to Agent deletes a failed persistent provisional thread, unsubscribes
...
get `open_world_enabled: true`. Zero to Agent does not expose a separate
...
- Per-plugin `allow_destructive_actions` overrides the global policy for
  that plugin.
- `false`: Zero to Agent returns a deterministic decline.
- `true`: Zero to Agent auto-accepts only safe schemas it can map to an approval
  response, such as a boolean approve field.
- `"auto"`: Zero to Agent exposes destructive plugin actions to Codex, then
  turns ownership-proven MCP approval elicitations into Zero to Agent plugin
  approvals before returning the Codex approval response.
- `"ask"`: Zero to Agent uses the same Codex write/destructive gating as
  ...
  For each admitted app using `"ask"`, Zero to Agent selects Codex's human approvals
  reviewer for that app so Codex sends its approval elicitations to
  Zero to Agent; other apps and non-app thread approvals keep their configured
  reviewer and policy.
...
| Code                                              | Meaning                                                                                                                              | Fix                                                                                                                        |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `auth_required`                                   | Migration installed the plugin, but one of its apps still needs authentication. The entry is written disabled until you reauthorize. | Reauthorize the app in Codex, then enable the plugin in Zero to Agent.                                                     |
...
| `plugin_detail_unavailable`                       | Zero to Agent could not read the exact configured plugin's ownership details.                                                        | Inspect the target app-server's `plugin/installed` and `plugin/read` responses.                                            |
...
| `app_inventory_missing`, `app_inventory_stale`    | App readiness came from an empty or stale cache.                                                                                     | Zero to Agent schedules an async refresh automatically; plugin apps stay excluded until ownership and readiness are known. |
...
account-wide default can become callable after Zero to Agent starts and verifies
...
Zero to Agent does not authenticate plugin apps on the owner's behalf.

For `plugin_detail_unavailable`, verify that the exact installed marketplace
and plugin identity select a matching `plugin/read` result. Zero to Agent keeps
...
Codex thread bindings keep the app config they started with until Zero to Agent
