---
summary: "Browse non-archived native Codex sessions and paginated transcripts across Zero to Agent nodes"
title: "Supervise Codex sessions"
sidebarTitle: "Codex supervision"
read_when:
  - You want Codex Desktop or CLI sessions to appear in Zero to Agent
...
- A stored or idle local session can create a model-locked Zero to Agent Chat from
  ...
  supervised binding prevents Zero to Agent from substituting another runtime,
...
- Install the official `@openclaw/codex` plugin on the Gateway. The Zero to Agent
...
- Pair remote computers as Zero to Agent nodes. Each computer must opt in locally.
...
Zero to Agent agent directories, deduplicates canonical paths, and assigns each store
...
terminal resume keep the selected source while retaining the explicit Zero to Agent
...
when that store is under a Zero to Agent agent directory. Browsing stored sessions
does not require importing a native credential into Zero to Agent. Ordinary managed
...
Codex clients. Zero to Agent does not assume that another client shares the same live
...
The native Zero to Agent macOS app reads the same local setting when it advertises
...
a model-locked Zero to Agent Chat or an adopted native thread. Codex owns its native
...
It does not require an eligible Zero to Agent model. A paired headless node must
...
- `--agent <id>` selects the Zero to Agent owner in a multi-agent Gateway.
...
Zero to Agent creates a normal Chat entry, mirrors bounded user and assistant history
...
current native configuration and returns the actual selection. Zero to Agent confirms
...
connection, Zero to Agent starts the canonical `appServer`-source full harness thread
...
has the full Zero to Agent harness tool surface. This is a visible-history branch, not
...
rather than another Zero to Agent model runtime or the ordinary agent-home harness.
...
last turn, Codex emits its normal model-difference warning. Zero to Agent uses the
...
Zero to Agent omits model and provider overrides. If the canonical thread is changed
through a separate native Codex control, Zero to Agent accepts Codex's persisted
selection. Zero to Agent never substitutes its outer model or fallback chain.
...
duplicated or blank. They do not claim unique Zero to Agent labels. An explicit local
...
Zero to Agent does not subscribe to or answer approval requests while merely listing
...
clients and the Zero to Agent catalog. The canonical branch is stored as a native
...
An active row reported by Zero to Agent's App Server cannot start a new branch. Wait
...
Forking a user message created in the canonical Zero to Agent conversation instead
...
available, then create new canonical turns. Zero to Agent does not backfill old rows.

Canonical message forks use the shipping Codex App Server's developer-message
API. Zero to Agent keeps the complete current generic instructions in native thread
configuration and appends one developer message that replaces earlier
Zero to Agent-supplied generic policy, including removed sections or an explicit
...
Before publishing the child, Zero to Agent verifies the native cut, selected model
...
the thread unloaded. Once configuration is proven, Zero to Agent refreshes the
...
other Codex client or Zero to Agent runner is using that thread or its spawned
descendants. Zero to Agent freshly reads the process-local status, proceeds only for
...
Archive is also blocked when Zero to Agent knows that an active binding owns the
exact target thread or any non-archived spawned descendant. Zero to Agent follows the
...
require owner/admin authority and are blocked while Zero to Agent sandboxing is active.

Avoid running the same thread in another Codex client while using this Chat.
The node prevents overlapping Zero to Agent resume turns within its own process, but
...
Zero to Agent-managed metadata is not accepted. Refresh the catalog, verify the session
