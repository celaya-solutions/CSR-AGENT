---
summary: "Browse non-archived native Codex sessions and paginated transcripts across OpenAgent nodes"
title: "Supervise Codex sessions"
sidebarTitle: "Codex supervision"
read_when:
  - You want Codex Desktop or CLI sessions to appear in OpenAgent
...
- A stored or idle local session can create a model-locked OpenAgent Chat from
  ...
  supervised binding prevents OpenAgent from substituting another runtime,
...
OpenAgent agent directories, deduplicates canonical paths, and assigns each store
...
terminal resume keep the selected source while retaining the explicit OpenAgent
...
when that store is under an OpenAgent agent directory. Browsing stored sessions
does not require importing a native credential into OpenAgent. Ordinary managed
...
Codex clients. OpenAgent does not assume that another client shares the same live
...
a model-locked OpenAgent Chat or an adopted native thread. Codex owns its native
...
It does not require an eligible OpenAgent model. A paired headless node must
...
OpenAgent creates a normal Chat entry, mirrors bounded user and assistant history
...
current native configuration and returns the actual selection. OpenAgent confirms
...
connection, OpenAgent starts the canonical `appServer`-source full harness thread
...
has the full OpenAgent harness tool surface. This is a visible-history branch, not
...
rather than another OpenAgent model runtime or the ordinary agent-home harness.
...
last turn, Codex emits its normal model-difference warning. OpenAgent uses the
...
OpenAgent omits model and provider overrides. If the canonical thread is changed
through a separate native Codex control, OpenAgent accepts Codex's persisted
selection. OpenAgent never substitutes its outer model or fallback chain.
...
duplicated or blank. They do not claim unique OpenAgent labels. An explicit local
...
OpenAgent does not subscribe to or answer approval requests while merely listing
...
clients and the OpenAgent catalog. The canonical branch is stored as a native
...
An active row reported by OpenAgent's App Server cannot start a new branch. Wait
...
Forking a user message created in the canonical OpenAgent conversation instead
...
available, then create new canonical turns. OpenAgent does not backfill old rows.

Canonical message forks use the shipping Codex App Server's developer-message
API. OpenAgent keeps the complete current generic instructions in native thread
configuration and appends one developer message that replaces earlier
OpenAgent-supplied generic policy, including removed sections or an explicit
...
Before publishing the child, OpenAgent verifies the native cut, selected model
...
the thread unloaded. Once configuration is proven, OpenAgent refreshes the
...
other Codex client or OpenAgent runner is using that thread or its spawned
descendants. OpenAgent freshly reads the process-local status, proceeds only for
...
Archive is also blocked when OpenAgent knows that an active binding owns the
exact target thread or any non-archived spawned descendant. OpenAgent follows the
...
require owner/admin authority and are blocked while OpenAgent sandboxing is active.

Avoid running the same thread in another Codex client while using this Chat.
The node prevents overlapping OpenAgent resume turns within its own process, but
...
OpenAgent-managed metadata is not accepted. Refresh the catalog, verify the session
