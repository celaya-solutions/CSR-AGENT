...
read_when:
  - You want an OpenAgent agent to join a video meeting
...
OpenAgent has separate plugins for Google Meet, Microsoft Teams meetings, and Zoom. All three can join through Chrome, use the same participation modes, and run Chrome either on the Gateway host or on a paired node. Their platform URLs, installation model, and extra capabilities differ.
...
| ------------ | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| `agent`      | Realtime transcription goes to the configured OpenAgent agent; regular OpenAgent TTS speaks the reply. | Chrome talk-back requires a supported virtual-audio backend. |
| `bidi`       | A realtime voice model listens and replies directly, with OpenAgent agent delegation when supported.       | Chrome talk-back requires a supported virtual-audio backend. |
...
Use `transcribe` when the agent only needs meeting text. Use `agent` for normal OpenAgent reasoning and tools. Use `bidi` when low-latency direct voice is more important than routing each turn through the regular agent.
...
Stopping a meeting requests cancellation of any active agent consult. In `agent` mode, OpenAgent finishes active output and turn events before closing the session, then ignores late speech synthesis and audio delivery results.
  ...
  plugins: {
    entries: {
      "teams-meetings": {
        config: {
          defaultMode: "agent",
          chrome: { guestName: "OpenAgent Agent" },
...
| `chrome.browserProfile`          | OpenAgent browser profile used for the meeting                                |
...
`agent` mode continues to use regular OpenAgent TTS.
...
On a Linux desktop with PipeWire-Pulse, install the PulseAudio command-line tools. OpenAgent creates and reuses an `OpenAgent Meeting Audio` null sink and matching source in the desktop user's audio session:
...
The Gateway host still owns the OpenAgent agent and model credentials when Chrome runs on a paired node. Configure a realtime transcription provider and OpenAgent TTS for `agent` mode, or a realtime voice provider for `bidi` mode. The platform guides contain the provider and audio-command options.
...
When a join or status result includes `manualAction`, complete its reported step in the same OpenAgent Chrome profile before retrying. Repeatedly opening new tabs does not resolve an account, tenant, lobby, or CAPTCHA gate.
