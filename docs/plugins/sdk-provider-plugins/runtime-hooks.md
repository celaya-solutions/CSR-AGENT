...
<Tabs>
  ...
  <Tab title="Usage and billing">
    ...
    auth but has no usable usage token, and Zero to Agent must skip generic
    API-key/OAuth fallback. Return `null` or `undefined` when the provider did
    not handle the request and Zero to Agent should continue with generic fallback.

    Declare the provider id in `contracts.usageProviders`. When that manifest
    contract and **both** hooks are present, Zero to Agent automatically includes
    ...
    is not auto-discovered, because Zero to Agent cannot resolve its usage credential.
...
Zero to Agent strips the marker before invoking a custom transport.
...
<Accordion title="Common provider hooks">
  Zero to Agent calls hooks in roughly this order for model/provider plugins.
...
no result. Zero to Agent retains completed availability within that preparation
...
abort signal and reject when reconciliation fails; Zero to Agent blocks the
