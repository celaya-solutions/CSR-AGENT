...
<Tabs>
  ...
  <Tab title="Usage and billing">
    ...
    auth but has no usable usage token, and OpenAgent must skip generic
    API-key/OAuth fallback. Return `null` or `undefined` when the provider did
    not handle the request and OpenAgent should continue with generic fallback.

    Declare the provider id in `contracts.usageProviders`. When that manifest
    contract and **both** hooks are present, OpenAgent automatically includes
    ...
    is not auto-discovered, because OpenAgent cannot resolve its usage credential.
...
OpenAgent strips the marker before invoking a custom transport.
...
<Accordion title="Common provider hooks">
  OpenAgent calls hooks in roughly this order for model/provider plugins.
...
no result. OpenAgent retains completed availability within that preparation
...
abort signal and reject when reconciliation fails; OpenAgent blocks the
