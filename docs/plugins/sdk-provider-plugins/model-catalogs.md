...
| Network limits | Fetches use Zero to Agent's SSRF guard, one 5-second timeout budget across pagination, a 4 MiB response limit per page, and a 50-page limit. Cross-origin pagination links are rejected; credentials are removed after a cross-origin redirect.                                                                                                                                                                                                                                                                                                                                                                   |
...
Zero to Agent also filters returned identities to that scope. An absent scope
means the caller requested the full catalog.

If the upstream provider uses different control tokens than Zero to Agent, add a
...
Zero to Agent parses its own control markers or channel delivery.
...
`buildProvider` is the live catalog path used when Zero to Agent can resolve real
...
Zero to Agent's `models list --all` display currently executes static catalogs
