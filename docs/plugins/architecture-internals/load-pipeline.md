---
summary: "How OpenAgent discovers, gates, loads, and registers plugins, and what the plugin cache holds"
read_when:
  - Debugging plugin load order or registry state
  - You need to know when OpenAgent reads manifests instead of loading plugin runtime
...
At startup, OpenAgent does roughly this:
...
The manifest is the control-plane source of truth. OpenAgent uses it to:
...
derived set is empty, OpenAgent keeps the scope empty instead of widening to
