---
summary: "How Zero to Agent discovers, gates, loads, and registers plugins, and what the plugin cache holds"
read_when:
  - Debugging plugin load order or registry state
  - You need to know when Zero to Agent reads manifests instead of loading plugin runtime
...
At startup, Zero to Agent does roughly this:
...
The manifest is the control-plane source of truth. Zero to Agent uses it to:
...
derived set is empty, Zero to Agent keeps the scope empty instead of widening to
