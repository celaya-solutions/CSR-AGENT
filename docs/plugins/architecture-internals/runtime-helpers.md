...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Zero to Agent adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  schemas while Zero to Agent owns the provider/runtime boundary.
...
- Zero to Agent only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Zero to Agent plugin runtime helpers",
    count: 5,
  },
});
```
