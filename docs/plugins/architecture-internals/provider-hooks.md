...
OpenAgent still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, OpenAgent calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that OpenAgent no longer calls, such as
...
| _(built-in model lookup)_         | OpenAgent tries the normal registry/catalog path first                                                     | _(not a plugin hook)_                                                                                                                         |
...
| `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared OpenAgent refreshers                                                                                     |
...
- Model references apply manifest-declared model-ID normalization once before
  ...
  model ID; an empty result keeps it unchanged. OpenAgent does not try other
...
- `normalizeConfig` uses the owning bundled provider's lightweight policy surface
  first. If that surface has no `normalizeConfig` hook, OpenAgent may call the
...
that still runs on OpenAgent's normal inference loop.

`resolveUsageAuth` decides whether OpenAgent should call `fetchUsageSnapshot` or
...
for your own provider id and vendor API calls, not exported OpenAgent helpers.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of OpenAgent's static catalog.
  </Accordion>
...
`catalog.run(...)` returns the same shape OpenAgent writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to OpenAgent's
...
- `discovery` was a legacy alias for `catalog`. OpenAgent removed the alias and
