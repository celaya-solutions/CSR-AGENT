...
Zero to Agent still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Zero to Agent calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Zero to Agent no longer calls, such as
...
| _(built-in model lookup)_         | Zero to Agent tries the normal registry/catalog path first                                                     | _(not a plugin hook)_                                                                                                                         |
...
| `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared Zero to Agent refreshers                                                                                     |
...
- Model references apply manifest-declared model-ID normalization once before
  ...
  model ID; an empty result keeps it unchanged. Zero to Agent does not try other
...
- `normalizeConfig` uses the owning bundled provider's lightweight policy surface
  first. If that surface has no `normalizeConfig` hook, Zero to Agent may call the
...
that still runs on Zero to Agent's normal inference loop.

`resolveUsageAuth` decides whether Zero to Agent should call `fetchUsageSnapshot` or
...
for your own provider id and vendor API calls, not exported Zero to Agent helpers.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Zero to Agent's static catalog.
  </Accordion>
...
`catalog.run(...)` returns the same shape Zero to Agent writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Zero to Agent's
...
- `discovery` was a legacy alias for `catalog`. Zero to Agent removed the alias and
