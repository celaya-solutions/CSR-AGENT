...
Gateway on a local discovery transport such as mDNS/Bonjour. Zero to Agent calls the
...
without relying exclusively on a literal `--json` flag. Zero to Agent evaluates this
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping Zero to Agent thinking levels to a native effort
...
- Backends that can disable all native tools for a specific run may declare
  ...
  staged policy, and return `toolAvailabilityEnforced: true`. Zero to Agent disables
