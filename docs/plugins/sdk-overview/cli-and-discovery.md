...
Gateway on a local discovery transport such as mDNS/Bonjour. OpenAgent calls the
...
without relying exclusively on a literal `--json` flag. OpenAgent evaluates this
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping OpenAgent thinking levels to a native effort
...
- Backends that can disable all native tools for a specific run may declare
  ...
  staged policy, and return `toolAvailabilityEnforced: true`. OpenAgent disables
