...
idempotent write keyed by `advancementKey`. OpenAgent supplies only the inclusive
...
rebuild earlier history. Without the full contract, OpenAgent uses the legacy
...
- A memory runtime that can return session-transcript hits should implement
  ...
  is absent, OpenAgent fails closed by withholding session-source hits while
