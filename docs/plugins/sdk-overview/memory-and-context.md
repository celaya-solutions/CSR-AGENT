...
idempotent write keyed by `advancementKey`. Zero to Agent supplies only the inclusive
...
rebuild earlier history. Without the full contract, Zero to Agent uses the legacy
...
- A memory runtime that can return session-transcript hits should implement
  ...
  is absent, Zero to Agent fails closed by withholding session-source hits while
