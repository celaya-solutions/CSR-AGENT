...
subpath is for shared schema primitives and the generic builder. OpenAgent's
...
active runtime config snapshot when OpenAgent is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through OpenAgent's plugin
