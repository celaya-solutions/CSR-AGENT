...
subpath is for shared schema primitives and the generic builder. Zero to Agent's
...
active runtime config snapshot when Zero to Agent is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Zero to Agent's plugin
