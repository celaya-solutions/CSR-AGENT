...
URIs, `:memory:`, and OpenAgent's reserved incognito database basename are refused
...
- Unauthenticated requesters on a shared-thread harness still see the advertised
  ...
  requester. OpenAgent never falls back to another requester's credentials.
...
state, then return only lines for that run. OpenAgent freezes those lines and
...
Telegram's normal inbound agent path after the handler succeeds. OpenAgent keeps
