...
URIs, `:memory:`, and Zero to Agent's reserved incognito database basename are refused
...
- Unauthenticated requesters on a shared-thread harness still see the advertised
  ...
  requester. Zero to Agent never falls back to another requester's credentials.
...
state, then return only lines for that run. Zero to Agent freezes those lines and
...
Telegram's normal inbound agent path after the handler succeeds. Zero to Agent keeps
