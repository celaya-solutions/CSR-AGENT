...
For Chrome transports, `leave` keeps a reused user-owned tab open after clicking Meet's Leave call button. Tabs opened by Zero to Agent are closed after departure.
...
| `speechReady` / `speechBlockedReason` / `speechBlockedMessage` | Whether managed Chrome speech is allowed now; `speechReady: false` means Zero to Agent did not send the intro/test phrase |
...
| ------- | ---------------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Zero to Agent agent | Normal Zero to Agent TTS runtime       | You want "my agent is in the meeting" behavior        |
