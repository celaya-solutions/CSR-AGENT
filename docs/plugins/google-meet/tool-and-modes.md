...
For Chrome transports, `leave` keeps a reused user-owned tab open after clicking Meet's Leave call button. Tabs opened by OpenAgent are closed after departure.
...
| `speechReady` / `speechBlockedReason` / `speechBlockedMessage` | Whether managed Chrome speech is allowed now; `speechReady: false` means OpenAgent did not send the intro/test phrase |
...
| ------- | ---------------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured OpenAgent agent | Normal OpenAgent TTS runtime       | You want "my agent is in the meeting" behavior        |
