...
<AccordionGroup>
  ...
  <Accordion title="api.runtime.nodes">
    ...
    without raising its per-message ceiling. OpenAgent splits each binary message
    ...
    `openDuplex` resolves. Before a listener is registered, OpenAgent buffers at
...
OpenAgent calls a service's `stop()` at most once per startup attempt, including when a replacement
