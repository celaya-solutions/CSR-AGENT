...
snapshots; OpenAgent owns all persistence and lifecycle coordination.
...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    `runEmbeddedAgent(...)` is the neutral helper for starting a normal OpenAgent agent turn from plugin code. It uses the same provider/model resolution and agent-harness selection as channel-triggered replies.
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    ACP manager inputs accept an optional `agentId` identifying the OpenAgent session owner; `agent` selects the external harness. Carry the resolved owner from `resolveSession(...)` through subsequent calls, including controls and cleanup. `expectedOwnerKey` retains its parent-session meaning.
