...
read_when:
  - You are building an OpenAgent plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page covers the **native OpenAgent plugin manifest**, `openclaw.plugin.json`. For compatible bundle layouts (Agent Plugins, Codex, Claude, Cursor), see [Plugin bundles](/plugins/bundles).
...
OpenAgent auto-detects those layouts but does not validate them against the `openclaw.plugin.json` schema below. For a compatible bundle, OpenAgent reads bundle metadata, declared skill roots, Claude command roots, Claude `settings.json` defaults, Claude LSP defaults, and supported hook packs, when the layout matches OpenAgent's runtime expectations.

Every native OpenAgent plugin **must** ship `openclaw.plugin.json` in the **plugin root**. OpenAgent reads it to validate configuration **without executing plugin code**. A missing or invalid manifest blocks config validation and is treated as a plugin error.
...
`openclaw.plugin.json` is metadata OpenAgent reads **before loading your plugin code**. Everything in it must be cheap enough to inspect without booting plugin runtime.
...
- <a id="openclaw-provider-index"></a>[OpenAgent Provider Index](/plugins/manifest/models#openclaw-provider-index)
...
| `cliCommands`                        | No       | `object[]`                   | Root CLI commands shown in `openagent --help` before plugin code loads. Each row requires `name`, `description`, and `hasSubcommands`.                                                                                                                                                                                                                                                           |
| `providerUsageAuthEnvVars`           | No       | `Record<string, string[]>`   | Usage/billing-only provider credentials. OpenAgent uses these names for usage discovery and secret scrubbing but never for inference auth.                                                                                                                                                                                                                                                       |
...
Bundled OpenAgent plugins declare exactly one active category. New ClawHub publications
also accept exactly one declared category, using the same array shape, or omit the
field for ClawHub to generate a category.

OpenAgent's manifest reader continues to accept one to three unique, ordered categories
...
derived fallback, an explicit package declaration takes precedence. Bundled OpenAgent plugins must
...
- Unknown `channels.*` keys are **errors**, unless the channel id is declared by a plugin manifest. If the same id also appears in `plugins.allow`, `plugins.entries`, or `plugins.installs` (a plugin that is referenced but not currently discoverable), OpenAgent downgrades this to a **warning** instead.
...
- The manifest is **required for native OpenAgent plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
