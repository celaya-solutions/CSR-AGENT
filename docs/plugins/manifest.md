...
read_when:
  - You are building a Zero to Agent plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page covers the **native Zero to Agent plugin manifest**, `openclaw.plugin.json`. For compatible bundle layouts (Agent Plugins, Codex, Claude, Cursor), see [Plugin bundles](/plugins/bundles).
...
Zero to Agent auto-detects those layouts but does not validate them against the `openclaw.plugin.json` schema below. For a compatible bundle, Zero to Agent reads bundle metadata, declared skill roots, Claude command roots, Claude `settings.json` defaults, Claude LSP defaults, and supported hook packs, when the layout matches Zero to Agent's runtime expectations.

Every native Zero to Agent plugin **must** ship `openclaw.plugin.json` in the **plugin root**. Zero to Agent reads it to validate configuration **without executing plugin code**. A missing or invalid manifest blocks config validation and is treated as a plugin error.
...
`openclaw.plugin.json` is metadata Zero to Agent reads **before loading your plugin code**. Everything in it must be cheap enough to inspect without booting plugin runtime.
...
- <a id="openclaw-provider-index"></a>[Zero to Agent Provider Index](/plugins/manifest/models#openclaw-provider-index)
...
| `providerUsageAuthEnvVars`           | No       | `Record<string, string[]>`   | Usage/billing-only provider credentials. Zero to Agent uses these names for usage discovery and secret scrubbing but never for inference auth.                                                                                                                                                                                                                                                   |
...
Bundled Zero to Agent plugins declare exactly one active category. New ClawHub publications
also accept exactly one declared category, using the same array shape, or omit the
field for ClawHub to generate a category.

Zero to Agent's manifest reader continues to accept one to three unique, ordered categories
...
derived fallback, an explicit package declaration takes precedence. Bundled Zero to Agent plugins must
...
- Unknown `channels.*` keys are **errors**, unless the channel id is declared by a plugin manifest. If the same id also appears in `plugins.allow`, `plugins.entries`, or `plugins.installs` (a plugin that is referenced but not currently discoverable), Zero to Agent downgrades this to a **warning** instead.
...
- The manifest is **required for native Zero to Agent plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
