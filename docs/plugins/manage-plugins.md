---
summary: "Manage OpenAgent plugins from the Control UI or CLI"
read_when:
...
included with OpenAgent and disabled by default. Bundled plugins can be disabled
...
`plugins list` is a cold inventory check: what OpenAgent can discover from
...
OpenAgent asks you to review a third-party plugin's declared capabilities before
...
Bundled plugins and verified first-party plugins from OpenAgent's official
...
OpenAgent cannot carry acceptance forward from an artifact it cannot verify.
...
official plugin id, in which case OpenAgent uses
that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or
`npm-pack:` for deterministic source selection. OpenAgent's bundled and official
...
Managed Codex sessions continue in the same OpenAgent conversation after stopping
...
OpenAgent reports it and prints the explicit command that replaces the pin.
