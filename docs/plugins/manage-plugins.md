---
summary: "Manage Zero to Agent plugins from the Control UI or CLI"
read_when:
...
included with Zero to Agent and disabled by default. Bundled plugins can be disabled
...
`plugins list` is a cold inventory check: what Zero to Agent can discover from
...
Zero to Agent asks you to review a third-party plugin's declared capabilities before
...
Bundled plugins and verified first-party plugins from Zero to Agent's official
catalog do not require this capability review during setup, install, enable,
update, or Doctor repair. For separately installed first-party plugins, Zero to Agent checks
...
Zero to Agent cannot carry acceptance forward from an artifact it cannot verify.
...
official plugin id, in which case Zero to Agent uses
that local/official copy instead. Use `clawhub:`, `npm:`, `git:`, or
`npm-pack:` for deterministic source selection. Zero to Agent's bundled and official
...
Zero to Agent records the install but leaves the plugin disabled. Configure
...
Managed Codex sessions continue in the same Zero to Agent conversation after stopping
...
If Zero to Agent cannot prove exactly one package owner and a complete child list,
...
exact version pins and explicit tags, including trusted official Zero to Agent
...
Zero to Agent reports it and prints the explicit command that replaces the pin.
...
| ----------- | ---------------------------------------------------------------------------- | -------------------------------------------------------------- |
| ClawHub     | You want Zero to Agent-native discovery, scan summaries, versions, and hints | `openclaw plugins install clawhub:<package>`                   |
...
ClawHub is the primary public discovery surface for Zero to Agent plugins. Publish
