---
summary: "Set up Codex Computer Use for Codex-mode OpenAgent agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode OpenAgent agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. OpenAgent
...
Use this page when OpenAgent is already using the native Codex harness. For the
...
OpenAgent-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through OpenAgent's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the OpenAgent command:
...
available as a normal OpenAgent MCP server. Use the Codex Computer Use setup on
...
macOS. OpenAgent does not install `cua-driver`, grant those permissions, or
...
Computer Use in and lets OpenAgent install or re-enable it before the turn:
...
With this config, OpenAgent checks Codex app-server before each Codex-mode
turn. If Computer Use is missing but Codex app-server has already discovered
an installable marketplace, OpenAgent asks Codex app-server to install or
...
Codex home's `computer-use` directory. OpenAgent verifies the outer service and
...
OpenAgent-owned isolated agent homes. User-scoped homes and explicit
...
On macOS, OpenAgent exposes the selected desktop app's bundled marketplace
...
and plugin directory from the selected standard desktop app. OpenAgent then asks
...
installed, OpenAgent falls back to the managed Codex binary installed beside the
...
OpenAgent serializes native Codex config reads and Computer Use installation
...
isolated home, OpenAgent waits for the last old-generation turn to release its
...
during startup, OpenAgent fences the stale client before login or `thread/start`
...
OpenAgent uses the same app-server API that Codex itself exposes. The
...
marketplaces. During install, OpenAgent polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds (default 60 seconds).
When Codex reports `features.plugins` as disabled, OpenAgent skips this discovery wait.

If multiple known marketplaces contain Computer Use, OpenAgent prefers
...
`computer-use` is registered, OpenAgent selects the first valid standard desktop
...
from its managed path under `CODEX_HOME`; OpenAgent owns the wrapper lifecycle
for isolated homes.
...
reconciliation so OpenAgent does not override that selection.

## Remote marketplaces

Remote marketplace support was introduced in Codex 0.146.1 and remains
available in OpenAgent's pinned Codex 0.153.4. OpenAgent passes the opaque remote
...
## What OpenAgent checks

OpenAgent reports a stable setup reason internally and formats the
...
OpenAgent intentionally fails closed when `computerUse.enabled` is true. A
...
retry in a fresh OpenAgent session. If the host previously ran Computer Use
...
Codex-native tool hook could not reach an active OpenAgent relay through the
local bridge or Gateway fallback. Start a fresh OpenAgent session with `/new`
...
OpenAgent Gateway so old threads and hook registrations are dropped, then
