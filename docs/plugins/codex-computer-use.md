---
summary: "Set up Codex Computer Use for Codex-mode Zero to Agent agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Zero to Agent agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Zero to Agent
...
Use this page when Zero to Agent is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

This is distinct from Zero to Agent's built-in [node-backed computer tool](/nodes/computer-use). Use the built-in tool when the same agent contract should control a paired Mac whether the agent runs on the Gateway or another node. Use Codex Computer Use when Codex app-server should own local MCP installation, permissions, and native tool calls.

## Zero to Agent.app and Peekaboo

Zero to Agent.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Zero to Agent.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Zero to Agent agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as a Zero to Agent node and exposes mobile
...
Zero to Agent-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Zero to Agent's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Zero to Agent command:
...
available as a normal Zero to Agent MCP server. Use the Codex Computer Use setup on
...
macOS. Zero to Agent does not install `cua-driver`, grant those permissions, or
...
Computer Use in and lets Zero to Agent install or re-enable it before the turn:
...
With this config, Zero to Agent checks Codex app-server before each Codex-mode
turn. If Computer Use is missing but Codex app-server has already discovered
an installable marketplace, Zero to Agent asks Codex app-server to install or
...
Codex home's `computer-use` directory. Zero to Agent verifies the outer service and
...
Zero to Agent-owned isolated agent homes. User-scoped homes and explicit
...
On macOS, Zero to Agent exposes the selected desktop app's bundled marketplace
...
and plugin directory from the selected standard desktop app. Zero to Agent then asks
...
installed, Zero to Agent falls back to the managed Codex binary installed beside the
...
Zero to Agent serializes native Codex config reads and Computer Use installation
...
isolated home, Zero to Agent waits for the last old-generation turn to release its
...
during startup, Zero to Agent fences the stale client before login or `thread/start`
...
`codex` plugin command surface is available. These are Zero to Agent chat/runtime
...
Zero to Agent uses the same app-server API that Codex itself exposes. The
...
marketplaces. During install, Zero to Agent polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds (default 60 seconds).
When Codex reports `features.plugins` as disabled, Zero to Agent skips this discovery wait.

If multiple known marketplaces contain Computer Use, Zero to Agent prefers
...
`computer-use` is registered, Zero to Agent selects the first valid standard desktop
...
from its managed path under `CODEX_HOME`; Zero to Agent owns the wrapper lifecycle
for isolated homes.
...
reconciliation so Zero to Agent does not override that selection.

## Remote marketplaces

Remote marketplace support was introduced in Codex 0.146.1 and remains
available in Zero to Agent's pinned Codex 0.153.4. Zero to Agent passes the opaque remote
...
## What Zero to Agent checks

Zero to Agent reports a stable setup reason internally and formats the
...
If Zero to Agent says Computer Use is installed but the MCP server is unavailable,
...
Zero to Agent intentionally fails closed when `computerUse.enabled` is true. A
...
retry in a fresh Zero to Agent session. If the host previously ran Computer Use
...
Codex-native tool hook could not reach an active Zero to Agent relay through the
local bridge or Gateway fallback. Start a fresh Zero to Agent session with `/new`
...
Zero to Agent Gateway so old threads and hook registrations are dropped, then
