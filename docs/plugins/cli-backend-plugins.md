...
read_when:
  ...
  - You need to map a third-party CLI into OpenAgent's text fallback runner
---

CLI backend plugins let OpenAgent call a local AI CLI as a text inference
...
<Steps>
  ...
  <Step title="Declare backend ownership">
    ```json openclaw.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through OpenAgent",
    ...
    `cliBackends` is the runtime ownership list; it lets OpenAgent auto-load the
  ...
  <Step title="Register the backend">
    ...
    adapter is authoritative plugin code; OpenAgent config selects the backend
...
`CliBackendConfig` describes how OpenAgent should launch and parse the CLI. The
...
- Leave it undefined or set it to `"replace-binding"` to preserve the legacy
  clear-and-reseed behavior. OpenAgent clears the persisted binding and retries
...
operator-authored input. OpenAgent's policy and observation hooks still receive the
...
authority. OpenAgent retains cancellation, watchdogs, session policy, and MCP
...
command. OpenAgent hashes the bounded complete installed package tree, including
...
`sideQuestionToolMode: "disabled"`; otherwise OpenAgent fails closed when BTW
...
`ctx.toolAvailability.openClaw` is the exact list of OpenAgent tool names. The
host independently limits the generated MCP configuration and grant to that
OpenAgent list; plugins must not translate it in core or add transport prefixes.
...
- `toolAvailabilityEnforcement: "prepare-execution"` requires
  ...
  OpenAgent cleans up the staged resources before launch.

Runtime caps such as cron `toolsAllow` are normalized and group-expanded by
OpenAgent before this contract is built. Native tools are disabled, and a
...
The host-prepared instruction snapshot must remain authoritative. OpenAgent
...
disables its native tools and serves the selected OpenAgent tools through the
...
Codex native code mode projects `read` and `exec` after OpenAgent explicitly
...
Tool events describe work the backend already performed. OpenAgent renders and
...
### `ownsNativeCompaction`: opting out of OpenAgent compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so OpenAgent's safeguard summarizer never runs
...
`manualCompaction`, so an explicit OpenAgent `/compact` resumes the
...
over-budget session can stay over budget or go stale (OpenAgent no longer
rescues it):
...
CLI backends do not receive OpenAgent tools by default. If the CLI can consume
...
"always-on"` so OpenAgent can fail closed when a caller requires no native
...
Put credentials in OpenAgent auth profiles or plugin-owned config. Ensure the
