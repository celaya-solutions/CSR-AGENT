...
read_when:
  ...
  - You need to map a third-party CLI into Zero to Agent's text fallback runner
---

CLI backend plugins let Zero to Agent call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Zero to Agent at the plugin runtime module         |
...
<Steps>
  ...
  <Step title="Declare backend ownership">
    ```json openclaw.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Zero to Agent",
    ...
    `cliBackends` is the runtime ownership list; it lets Zero to Agent auto-load the
  ...
  <Step title="Register the backend">
    ...
    adapter is authoritative plugin code; Zero to Agent config selects the backend
...
`CliBackendConfig` describes how Zero to Agent should launch and parse the CLI. The
...
| `modelAliases`                                            | Map Zero to Agent model ids to CLI-native ids                                     |
| `sessionArgs`                                             | How to pass a session id using `{sessionId}`                                      |
| `sessionMode`                                             | `always`, `existing`, or `none`                                                   |
| `sessionIdFields`                                         | JSON fields Zero to Agent reads from CLI output                                   |
...
`claude-stream-json` is more than a parser choice: it declares that the backend's `result` records carry Claude Code's terminal semantics, including `terminal_reason`. A reply-less `result` whose `terminal_reason` is `hook_stopped`, `stop_hook_prevented`, `aborted_tools`, `aborted_streaming`, `budget_exhausted`, or `max_turns` is a recorded turn stop: Zero to Agent reports that reason to the user and does not replay the turn on a fallback model, because the backend's tool actions may already have run.
...
- Leave it undefined or set it to `"replace-binding"` to preserve the legacy
  clear-and-reseed behavior. Zero to Agent clears the persisted binding and retries
...
| `defaultAuthProfileId`             | Prefer a specific Zero to Agent auth profile                                |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into Zero to Agent's loopback MCP tool bridge                           |
| `ownsNativeCompaction`             | Backend owns its own automatic compaction - Zero to Agent defers            |
...
operator-authored input. Zero to Agent's policy and observation hooks still receive the
...
authority. Zero to Agent retains cancellation, watchdogs, session policy, and MCP
...
command. Zero to Agent hashes the bounded complete installed package tree, including
...
`sideQuestionToolMode: "disabled"`; otherwise Zero to Agent fails closed when BTW
...
`ctx.toolAvailability.openClaw` is the exact list of Zero to Agent tool names. The
host independently limits the generated MCP configuration and grant to that
Zero to Agent list; plugins must not translate it in core or add transport prefixes.
...
- `toolAvailabilityEnforcement: "prepare-execution"` requires
  ...
  Zero to Agent cleans up the staged resources before launch.

Runtime caps such as cron `toolsAllow` are normalized and group-expanded by
Zero to Agent before this contract is built. Native tools are disabled, and a
...
The host-prepared instruction snapshot must remain authoritative. Zero to Agent
...
disables its native tools and serves the selected Zero to Agent tools through the
...
Codex native code mode projects `read` and `exec` after Zero to Agent explicitly
...
Tool events describe work the backend already performed. Zero to Agent renders and
...
### `ownsNativeCompaction`: opting out of Zero to Agent compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so Zero to Agent's safeguard summarizer never runs
...
`manualCompaction`, so an explicit Zero to Agent `/compact` resumes the
...
over-budget session can stay over budget or go stale (Zero to Agent no longer
rescues it):
...
CLI backends do not receive Zero to Agent tools by default. If the CLI can consume
...
"always-on"` so Zero to Agent can fail closed when a caller requires no native
...
Put credentials in Zero to Agent auth profiles or plugin-owned config. Ensure the
