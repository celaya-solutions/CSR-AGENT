...
read_when:
  - You want OpenAgent to install and manage a local llama.cpp server
  - You want a local model recommendation for your Gateway hardware
  - You want OpenAgent to connect to an existing llama-server
...
The `llama-cpp` plugin provides one `llama-cpp` model provider. OpenAgent can
...
it exists, OpenAgent manages the process. Without it, `baseUrl` identifies an
...
Choose **Managed local server** when OpenAgent should install, start, and stop
...
an OpenAgent tool and return its contents. The tool check uses an isolated
...
declined, OpenAgent offers a separate embedding-only setup. It installs only the
...
must also be removed before OpenAgent can manage embeddings.
...
OpenAgent reads `/health`, `/models` (falling back to `/v1/models`), and
...
Both ownership choices use OpenAgent's normal chat, image, streaming, and tool
...
OpenAgent does not auto-select ROCm, SYCL, OpenVINO, or Vulkan archives.
