...
read_when:
  - You want Zero to Agent to install and manage a local llama.cpp server
  - You want a local model recommendation for your Gateway hardware
  - You want Zero to Agent to connect to an existing llama-server
...
The `llama-cpp` plugin provides one `llama-cpp` model provider. Zero to Agent can
...
| --------------------- | ----------------------------- | ---------------- |
| Managed local server  | Zero to Agent                 | Yes              |
...
it exists, Zero to Agent manages the process. Without it, `baseUrl` identifies an
...
Choose **Managed local server** when Zero to Agent should install, start, and stop
...
a Zero to Agent tool and return its contents. The tool check uses an isolated
...
declined, Zero to Agent offers a separate embedding-only setup. It installs only the
...
must also be removed before Zero to Agent can manage embeddings.
...
Zero to Agent reads `/health`, `/models` (falling back to `/v1/models`), and
...
Both ownership choices use Zero to Agent's normal chat, image, streaming, and tool
...
Zero to Agent does not auto-select ROCm, SYCL, OpenVINO, or Vulkan archives.
