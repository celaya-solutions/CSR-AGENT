---
summary: "Install and use Agent Plugins, Codex, Claude, and Cursor bundles as Zero to Agent plugins"
read_when:
  - You want to install an Agent Plugins, Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Zero to Agent maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Zero to Agent can install plugins from four external ecosystems: the vendor-neutral
[**Agent Plugins**](https://agent-plugins.org) standard, plus **Codex**,
**Claude**, and **Cursor**. These are called **bundles** - content and metadata
packs that Zero to Agent maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Zero to Agent plugins. Native plugins run
...
Cursor format. Instead of requiring authors to rewrite them as native Zero to Agent
plugins, Zero to Agent detects these formats and maps their supported content into
...
## What Zero to Agent maps from bundles

Not every bundle feature runs in Zero to Agent. Here is what works and what is
...
| ------------------------ | ------------------------------------------------------------------------------------------------------ | -------------- |
| Skill content            | Bundle skill roots load as normal Zero to Agent skills                                                 | All formats    |
| Commands                 | `commands/` and `.cursor/commands/` treated as skill roots                                             | Claude, Cursor |
| Agents and output styles | Claude `agents/` and `output-styles/` treated as skill roots                                           | Claude         |
| Hook packs               | Zero to Agent-style `HOOK.md` + `handler.ts` layouts                                                   | Claude, Codex  |
| MCP tools                | Bundle MCP config merged into embedded Zero to Agent settings; supported stdio and HTTP servers loaded | All formats    |
| Env contract             | `PLUGIN_ROOT` and `PLUGIN_DATA` env vars plus placeholder expansion for stdio MCP servers              | Agent Plugins  |
| LSP servers              | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded Zero to Agent LSP defaults  | Claude         |
| Settings                 | Claude `settings.json` imported as embedded Zero to Agent defaults                                     | Claude         |

#### Skill content

- Bundle skill roots load as normal Zero to Agent skill roots.
...
normal Zero to Agent skill loader.
...
as a supported hook. A bundle containing both layouts keeps its Zero to Agent hook packs.
Inspection does not execute handlers or prove that the running Gateway loaded them.

#### Embedded Zero to Agent settings

Claude `settings.json` is imported as default embedded Zero to Agent settings when
the bundle is enabled. Zero to Agent sanitizes shell override keys before applying
them:

- `shellPath`
- `shellCommandPrefix`

#### Embedded Zero to Agent LSP

- Enabled Claude bundles can contribute LSP server config.
- Zero to Agent loads `.lsp.json` plus any manifest-declared `lspServers` paths.
- Bundle LSP config is merged into the effective embedded Zero to Agent LSP
  defaults.
...
These are recognized and shown in diagnostics, but Zero to Agent does not run them:

- Claude `hooks/hooks.json` automation
- Cursor `.cursor/agents`, `.cursor/hooks.json`, `.cursor/rules`
- Codex `.app.json` metadata beyond capability reporting

## MCP for embedded Zero to Agent

- Enabled bundles can contribute MCP server config.
- Zero to Agent merges bundle MCP config into the effective embedded Zero to Agent
  settings as `mcpServers`.
- Zero to Agent exposes supported bundle MCP tools during embedded Zero to Agent agent
...
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Zero to Agent config. `openclaw mcp set` and `openclaw doctor --fix` normalize the common alias.
...
Zero to Agent registers bundle MCP tools with provider-safe names in the form
...
<AccordionGroup>
  <Accordion title="Agent Plugins bundles">
    ...
    - The manifest is strict JSON (not JSON5). Zero to Agent requires a non-empty
    ...
    - stdio servers launch with `PLUGIN_ROOT` (the plugin root) and
      `PLUGIN_DATA` (a persistent per-plugin data directory Zero to Agent creates
    ...
    - Zero to Agent reads `extensions["ai.openclaw"]`; it supports only
  ...
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Zero to Agent best when they use skill roots and Zero to Agent-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded Zero to Agent settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded Zero to Agent
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded Zero to Agent LSP defaults
...
Zero to Agent checks for native plugin format first:
...
bundle markers, Zero to Agent uses the native path. This prevents dual-format
...
- Zero to Agent-owned bundled plugins are either shipped lightweight in core or
...
- Zero to Agent does **not** load arbitrary bundle runtime modules in-process.
...
<AccordionGroup>
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Zero to Agent settings from `settings.json` are supported. Zero to Agent does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Zero to Agent hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
