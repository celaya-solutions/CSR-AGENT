---
summary: "Install and use Agent Plugins, Codex, Claude, and Cursor bundles as OpenAgent plugins"
read_when:
  - You want to install an Agent Plugins, Codex, Claude, or Cursor-compatible bundle
  - You need to understand how OpenAgent maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

OpenAgent can install plugins from four external ecosystems: the vendor-neutral
[**Agent Plugins**](https://agent-plugins.org) standard, plus **Codex**,
**Claude**, and **Cursor**. These are called **bundles** - content and metadata
packs that OpenAgent maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native OpenAgent plugins. Native plugins run
...
Cursor format. Instead of requiring authors to rewrite them as native OpenAgent
plugins, OpenAgent detects these formats and maps their supported content into
...
## What OpenAgent maps from bundles

Not every bundle feature runs in OpenAgent. Here is what works and what is
...
- Bundle skill roots load as normal OpenAgent skill roots.
...
as a supported hook. A bundle containing both layouts keeps its OpenAgent hook packs.
Inspection does not execute handlers or prove that the running Gateway loaded them.

#### Embedded OpenAgent settings

Claude `settings.json` is imported as default embedded OpenAgent settings when
the bundle is enabled. OpenAgent sanitizes shell override keys before applying
them:

- `shellPath`
- `shellCommandPrefix`

#### Embedded OpenAgent LSP

- Enabled Claude bundles can contribute LSP server config.
- OpenAgent loads `.lsp.json` plus any manifest-declared `lspServers` paths.
...
These are recognized and shown in diagnostics, but OpenAgent does not run them:

- Claude `hooks/hooks.json` automation
- Cursor `.cursor/agents`, `.cursor/hooks.json`, `.cursor/rules`
- Codex `.app.json` metadata beyond capability reporting

## MCP for embedded OpenAgent

- Enabled bundles can contribute MCP server config.
- OpenAgent merges bundle MCP config into the effective embedded OpenAgent
  settings as `mcpServers`.
- OpenAgent exposes supported bundle MCP tools during embedded OpenAgent agent
...
OpenAgent registers bundle MCP tools with provider-safe names in the form
...
<AccordionGroup>
  <Accordion title="Agent Plugins bundles">
    ...
    - The manifest is strict JSON (not JSON5). OpenAgent requires a non-empty
    ...
    - stdio servers launch with `PLUGIN_ROOT` (the plugin root) and
      `PLUGIN_DATA` (a persistent per-plugin data directory OpenAgent creates
    ...
    - OpenAgent reads `extensions["ai.openclaw"]`; it supports only
  ...
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit OpenAgent best when they use skill roots and OpenAgent-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded OpenAgent settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded OpenAgent
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded OpenAgent LSP defaults
...
OpenAgent checks for native plugin format first:
...
bundle markers, OpenAgent uses the native path. This prevents dual-format
...
- OpenAgent does **not** load arbitrary bundle runtime modules in-process.
...
<AccordionGroup>
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded OpenAgent settings from `settings.json` are supported. OpenAgent does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    OpenAgent hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
