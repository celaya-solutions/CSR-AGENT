---
summary: "Step-by-step guide to building a model provider plugin for OpenAgent"
title: "Building provider plugins"
sidebarTitle: "Provider plugins"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to OpenAgent
  - You need to understand provider auth, catalogs, and runtime hooks
---

Build a provider plugin to add a model provider (LLM) to OpenAgent: a model
...
API calls, not exported OpenAgent functions.

<Info>
  New to OpenAgent plugins? Read [Getting Started](/plugins/building-plugins)
  first for package structure and manifest setup.
</Info>

<Tip>
  Provider plugins add models to OpenAgent's normal inference loop. If the
...
<Steps>
  <Step title="Package and manifest">
    ...
    `setup.providers[].envVars` lets OpenAgent detect credentials without
    ...
    optional and lets OpenAgent auto-load your provider plugin from shorthand
    ...
    the OpenAgent release your plugin builds and tests against.
  ...
  <Step title="Register the provider">
    ...
    calls and response mapping in the plugin. OpenAgent owns the shared row
    ...
    OpenAgent keeps the inline value only while staged validation runs. At the
    ...
    an uppercase environment-style name. OpenAgent adds a stable suffix derived
  ...
  <Step title="Add dynamic model resolution">
    ...
    from `prepareDynamicModel`. OpenAgent applies the same configured overrides
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    web fetch, and web search alongside text inference. OpenAgent classifies this as a
