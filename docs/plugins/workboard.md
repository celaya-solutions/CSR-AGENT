...
OpenAgent Gateway. It is not a replacement for GitHub Issues, Linear, Jira, or
...
2. Open the **Workboard** plugin, select **Lifecycle**, and turn on the enabled
   switch. Because Workboard is included with OpenAgent, it does not need an
   **Install** action.
...
that Gateway's OpenAgent state (see [Storage](#storage)).
...
OpenAgent subagent sessions still own execution. One dispatch pass:
...
`defaultAssignee`, and `orchestratorProfile`. OpenAgent records this intent and
...
under the OpenAgent state directory: boards, cards, labels, lifecycle events,
