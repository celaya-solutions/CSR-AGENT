...
Zero to Agent Gateway. It is not a replacement for GitHub Issues, Linear, Jira, or
...
2. Open the **Workboard** plugin, select **Lifecycle**, and turn on the enabled
   switch. Because Workboard is included with Zero to Agent, it does not need an
   **Install** action.
...
that Gateway's Zero to Agent state (see [Storage](#storage)).
...
Zero to Agent subagent sessions still own execution. One dispatch pass:
...
`defaultAssignee`, and `orchestratorProfile`. Zero to Agent records this intent and
...
under the Zero to Agent state directory: boards, cards, labels, lifecycle events,
