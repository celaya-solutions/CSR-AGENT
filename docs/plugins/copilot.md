---
summary: "Run Zero to Agent embedded agent turns through the external GitHub Copilot SDK harness"
...
Zero to Agent's built-in harness. The Copilot CLI session owns the low-level
agent loop: native tool execution, native compaction (`infiniteSessions`), and
CLI-managed thread state under `copilotHome`. Zero to Agent still owns chat
...
- Zero to Agent with the `@openclaw/copilot` plugin installed.
...
- A writable `copilotHome` directory. Defaults to `<agentDir>/copilot` when
  Zero to Agent provides an agent directory, otherwise
...
[Quickstart](#quickstart). Without that opt-in, Zero to Agent uses its built-in
...
through Zero to Agent's guarded fetch path so DNS pinning and SSRF policy stay
owned by Zero to Agent. Use the native Zero to Agent runtime for local Ollama, LM
Studio, or LAN model servers.

## BYOK

Copilot BYOK uses the SDK's session-level custom provider contract. Zero to Agent
...
4. **Harness env-var fallback**, checked in this order (first non-empty value
   ...
   1. `OPENCLAW_GITHUB_TOKEN` — harness-specific override; lets you pin a
      token for the Zero to Agent harness without disturbing system-wide `gh` /
      Copilot CLI config.
...
Zero to Agent's `models.json` / `openclaw-agent.sqlite`), or
...
| `reasoningEffort`        | `"low" \| "medium" \| "high" \| "xhigh"`. Maps from Zero to Agent's `ThinkLevel` / `ReasoningLevel` resolution in `auto-reply/thinking.ts`.                                                                                                                                                     |
| `infiniteSessionConfig`  | Optional override for the SDK `infiniteSessions` block driven by `harness.compact`. Safe to leave as-is.                                                                                                                                                                                        |
| `hooksConfig`            | Optional native Copilot SDK `SessionHooks` config for tool/MCP, user-prompt, session, and error callbacks. Separate from Zero to Agent's portable lifecycle hooks.                                                                                                                              |
...
Zero to Agent plugin hooks need no Copilot-specific attempt configuration. The
...
`before_compaction` and `after_compaction`. Bridged Zero to Agent tools run
`before_tool_call` and report `after_tool_call`; `hooksConfig` remains for
native SDK-only callbacks with no portable equivalent.

Nothing else in Zero to Agent needs to know about these fields. Other plugins,
...
The Zero to Agent-side transcript journal (below) keeps receiving post-compaction
...
turn's messages into the Zero to Agent session transcript. Journal identity is
...
so Zero to Agent's `/btw` dispatcher (`src/agents/btw.ts`) falls through to the
...
- `ask_user` uses the provider-neutral gateway question runtime. The Control
  UI shows the same question card as other Zero to Agent questions, supported
...
Permission enforcement for bridged Zero to Agent tools happens **inside the tool
...
- `skipPermission: true` — tells the SDK not to fire
  ...
  wrapped `execute()` already performs the richer Zero to Agent policy check; an
  SDK-level prompt would either short-circuit Zero to Agent's enforcement
  (allow-all) or block every tool call (reject-all) — neither matches PI
  parity.

The in-tree Codex harness uses the same split: bridged Zero to Agent tools are
...
catalog executor instead of every Zero to Agent tool schema. When code mode is
...
when the SDK request allows them. Aborting the Zero to Agent attempt cancels the
