...
Native UI shipped with OpenAgent remains available for enabled bundled plugins,
including Workboard. OpenAgent determines bundled status from the loaded
...
approval, OpenAgent verifies and retains the exact archive and inspects its
...
Pending imports expire after one hour. OpenAgent keeps at most eight pending
...
Artifact activation also refuses to replace the plugin backing OpenAgent's active
inference route. Stop OpenAgent and install that artifact from a trusted shell.
