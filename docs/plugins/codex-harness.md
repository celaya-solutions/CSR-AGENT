---
summary: "Run Zero to Agent embedded agent turns through the official Codex app-server harness"
title: "Codex harness"
read_when:
...
app-server instead of the built-in Zero to Agent harness. Codex owns the
low-level agent session: native thread resume, native tool continuation,
native compaction, and app-server execution. Zero to Agent still owns chat
channels, session files, model selection, Zero to Agent dynamic tools, approvals,
media delivery, and the visible transcript mirror.

Pasted text saved as a `.txt` attachment is extracted by Zero to Agent and included in
...
`remoteWorkspaceRoot` to validate remote workspace attachment paths. Zero to Agent
...
If Codex owns auth before Platform versus ChatGPT routing is known, Zero to Agent
...
When no Zero to Agent sandbox is active, Zero to Agent starts Codex app-server threads
with Codex native code mode enabled (code-mode-only stays off by default), so
native workspace/code capabilities remain available alongside Zero to Agent
dynamic tools routed through the app-server `item/tool/call` bridge. An
ordinary Zero to Agent sandbox or restricted tool policy disables native code mode
...
an explicit finite tool allowlist still blocks native execution. Zero to Agent does
...
as a distinct Zero to Agent execution path. Use `gateway_exec` only when a command
needs Zero to Agent-managed Gateway environment access, including Secret Store
agent-readable environment values or protected egress sentinels. It is pinned
to the Gateway host and follows Zero to Agent exec policy. `gateway_process` uses the
existing per-session Zero to Agent process scope for background follow-up. Prefer
...
With the default `tools.exec.host: "auto"` and no active Zero to Agent sandbox,
...
are eligible. It keeps Zero to Agent's node approval policy in force and waits for the
...
an execution environment, Zero to Agent keeps its policy-filtered `exec` and
...
default, Zero to Agent hides the Codex-native shell and exposes `node_exec` only while
the node target is eligible. If it is unavailable, reconnect the configured node
or explicitly change the exec host. Zero to Agent does not silently fall back to the
app-server or Gateway machine.

`gateway_exec` is not exposed when an active Zero to Agent sandbox, a node-default
...
[Zero to Agent Code Mode](/tools/code-mode), an opt-in QuickJS-WASI runtime
for generic Zero to Agent runs with a different `exec` input shape. For the
...
After installing or updating Zero to Agent, explicitly verify the managed package
...
exact Codex version pinned by Zero to Agent. It does not execute custom, remote, or
macOS desktop-owned app-servers.

`/status` reports the resolved Zero to Agent Fast policy (`on`, `off`, or `auto`)
