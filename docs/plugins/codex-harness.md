---
summary: "Run OpenAgent embedded agent turns through the official Codex app-server harness"
title: "Codex harness"
read_when:
...
app-server instead of the built-in OpenAgent harness. Codex owns the
low-level agent session: native thread resume, native tool continuation,
native compaction, and app-server execution. OpenAgent still owns chat
channels, session files, model selection, OpenAgent dynamic tools, approvals,
media delivery, and the visible transcript mirror.

Pasted text saved as a `.txt` attachment is extracted by OpenAgent and included in
...
`remoteWorkspaceRoot` to validate remote workspace attachment paths. OpenAgent
...
If Codex owns auth before Platform versus ChatGPT routing is known, OpenAgent
...
When no OpenAgent sandbox is active, OpenAgent starts Codex app-server threads
with Codex native code mode enabled (code-mode-only stays off by default), so
native workspace/code capabilities remain available alongside OpenAgent
dynamic tools routed through the app-server `item/tool/call` bridge. An
ordinary OpenAgent sandbox or restricted tool policy disables native code mode
...
an explicit finite tool allowlist still blocks native execution. OpenAgent does
...
as a distinct OpenAgent execution path. Use `gateway_exec` only when a command
needs OpenAgent-managed Gateway environment access, including Secret Store
agent-readable environment values or protected egress sentinels. It is pinned
to the Gateway host and follows OpenAgent exec policy. `gateway_process` uses the
existing per-session OpenAgent process scope for background follow-up. Prefer
...
With the default `tools.exec.host: "auto"` and no active OpenAgent sandbox,
...
are eligible. It keeps OpenAgent's node approval policy in force and waits for the
...
an execution environment, OpenAgent keeps its policy-filtered `exec` and
...
default, OpenAgent hides the Codex-native shell and exposes `node_exec` only while
the node target is eligible. If it is unavailable, reconnect the configured node
or explicitly change the exec host. OpenAgent does not silently fall back to the
app-server or Gateway machine.

`gateway_exec` is not exposed when an active OpenAgent sandbox, a node-default
...
[OpenAgent Code Mode](/tools/code-mode), an opt-in QuickJS-WASI runtime
for generic OpenAgent runs with a different `exec` input shape. For the
...
After installing or updating OpenAgent, explicitly verify the managed package
...
exact Codex version pinned by OpenAgent. It does not execute custom, remote, or
macOS desktop-owned app-servers.

`/status` reports the resolved OpenAgent Fast policy (`on`, `off`, or `auto`)
