...
Codex launches its node exec-server directly rather than starting an OpenAgent
...
start an OpenAgent worker child, open a reverse tunnel, or copy provider, cloud,
...
The bundled Crabbox provider supports both OpenAgent `worker-turn` and Codex
...
outbound connection without starting an OpenAgent worker child or consuming a
