...
**OpenAgent uses the built-in harness instead of Codex:** confirm the effective
...
payload compatibility switches still do: Codex declares an OpenAgent fallback
...
OpenAgent ships. Update or remove custom, remote, or desktop
...
first. OpenAgent runs the local Codex app-server as a separate Rust child.
...
for up to 30 minutes. OpenAgent independently keeps up to 64 idle conversation
...
id that OpenAgent no longer has registered. This is a native Codex hook
...
OpenAgent Gateway so old threads are dropped and native hook registrations
are recreated.
...
used for OpenAgent loop detection and its no-policy marker. Required
