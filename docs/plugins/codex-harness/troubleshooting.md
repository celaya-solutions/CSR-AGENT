...
**Zero to Agent uses the built-in harness instead of Codex:** confirm the effective
...
payload compatibility switches still do: Codex declares a Zero to Agent fallback
...
Zero to Agent ships. Update or remove custom, remote, or desktop
...
first. Zero to Agent runs the local Codex app-server as a separate Rust child.
...
for up to 30 minutes. Zero to Agent independently keeps up to 64 idle conversation
...
check for bindings left behind by deleted or expired Zero to Agent sessions. Stop
...
id that Zero to Agent no longer has registered. This is a native Codex hook
...
Zero to Agent Gateway so old threads are dropped and native hook registrations
are recreated.
...
used for Zero to Agent loop detection and its no-policy marker. Required
