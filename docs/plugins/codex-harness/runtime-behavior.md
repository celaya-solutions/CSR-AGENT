...
read_when:
  - You want to know which OpenAgent tools reach a Codex turn
  - You are debugging turn liveness or parallel chats
  - You need the ownership split between OpenAgent and Codex
title: "Codex runtime behavior"
sidebarTitle: "Runtime behavior"
---

What the Codex harness owns during a turn, and what stays with OpenAgent. Part of the [Codex harness](/plugins/codex-harness) guide; [Where each section moved](/plugins/codex-harness#where-each-section-moved) lists every section.

## Dynamic tools and web search

Codex dynamic tools default to `searchable` loading. OpenAgent normally does
...
so OpenAgent does not project a second goal store into Codex turns. Most
remaining OpenAgent integration tools, such as messaging, media, cron,
...
OpenAgent instead publishes the distinct `gateway_exec` and `gateway_process`
names so native shell and the OpenAgent-managed environment path cannot be
confused.

Tools marked `catalogMode: "direct-only"`, including the OpenAgent `computer`
...
OpenAgent's managed `web_search` dynamic tool are mutually exclusive so
managed search cannot bypass native domain restrictions. OpenAgent uses the
...
while explicit OpenAgent or ACP delegation remains directly callable through
`sessions_spawn`. In Codex Code Mode, generic OpenAgent
...
`view_image` and OpenAgent suppresses its duplicate loader. The native Codex
schema accepts one local filesystem `path`. For text-only models, or when the
native tool surface is disabled, OpenAgent supplies `view_image` with its
...
Codex owns provider-stream liveness and native turn completion. OpenAgent waits
...
OpenAgent still bounds its own requests, dynamic tools, cancellation, and local
...
OpenAgent does not infer classifier activity from the assistant's wording.
...
cyber-policy refusal, OpenAgent retries it once on the configured Daybreak model
...
- The retry does not mirror the prompt into the transcript a second time. The
  ...
  turn still exists upstream in the native Codex thread, which OpenAgent does not
  rewrite.
...
full reconnect ladder. OpenAgent therefore treats the retry itself as the only
...
continuation, or interruption, OpenAgent keeps the original refusal even when
...
reads to finish. OpenAgent coordinates its own lifecycle operations for each
...
existing configuration. OpenAgent retains the configured native thread, including
for `/codex resume` of that chat's already-bound thread. Native provider policy refusals
end the current attempt without a native retry. OpenAgent's configured cyber
...
loaded failed thread still requires native unloading. OpenAgent preserves the
...
can operate outside OpenAgent's thread queue. Avoid concurrently reconfiguring the
...
- OpenAgent dynamic tools are supported. Codex asks OpenAgent to execute
  those tools, so OpenAgent remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  OpenAgent can observe or block selected native events through the
  supported relay, but it does not rewrite native tool arguments.
- `gateway_exec` and `gateway_process` are OpenAgent-owned dynamic tools. They
  ...
  native shell.
- Codex owns native compaction. OpenAgent keeps a transcript mirror for
  channel history, search, `/new`, `/reset`, and future model or harness
  switching, but does not replace Codex compaction with an OpenAgent or
...
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching OpenAgent provider/model settings.
- `tool_result_persist` applies to OpenAgent-owned transcript tool results,
