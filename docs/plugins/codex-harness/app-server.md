...
How Zero to Agent starts and authenticates the Codex app-server, and what it isolates from the operator environment. Part of the [Codex harness](/plugins/codex-harness) guide; [Where each section moved](/plugins/codex-harness#where-each-section-moved) lists every section.

## App-server policy

By default, the plugin starts Zero to Agent's managed Codex binary locally with
...
implicit YOLO posture, Zero to Agent selects allowed guardian permissions
instead. When a Zero to Agent sandbox is active for the session, Zero to Agent
...
Shell access instead goes through Zero to Agent sandbox-backed dynamic tools such
as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

Use normalized Zero to Agent exec mode for Codex native auto-review before
...
Zero to Agent does not preserve legacy unsafe Codex `approvalPolicy: "never"` or
...
works, but `tools.exec.mode: "auto"` is the normalized Zero to Agent surface.
...
file prompts use Zero to Agent's two-phase operator approval route. The prompt shows
...
Codex auto-review, full-access policy, and native hook or Zero to Agent policy
...
When Zero to Agent sees a ChatGPT subscription-style Codex auth profile, it
...
If a subscription profile hits a Codex usage limit, Zero to Agent records the
...
When native Codex plugins are configured, Zero to Agent reads and caches one
...
supplies authenticated app metadata in batches of at most 100 app IDs. Zero to Agent
...
Zero to Agent provisionally admits only explicitly allowed, ownership-proven apps,
...
The check runs before Zero to Agent starts a turn or commits a thread binding. If the
snapshot request fails, a persistent provisional thread is deleted and an
ephemeral thread is unsubscribed. If cleanup cannot be confirmed, Zero to Agent retires the app-server
connection instead of reusing an unsafe thread.

Account-wide app access never overrides an explicitly disabled configured
workspace plugin. When `app/read` omits that plugin's ownership, Zero to Agent uses
...
Zero to Agent does not install unknown apps or let the model authorize new plugin
...
For local stdio app-server launches, Zero to Agent sets `CODEX_HOME` to a
...
`~/.codex` by default. Zero to Agent preserves the normal process `HOME`;
...
`appServer.homeScope: "user"`, Zero to Agent instead uses the native user Codex
home and its existing account without injecting a Zero to Agent auth profile.
...
process. Zero to Agent removes `CODEX_HOME` and `HOME` from this list during
