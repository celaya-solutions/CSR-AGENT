...
How OpenAgent starts and authenticates the Codex app-server, and what it isolates from the operator environment. Part of the [Codex harness](/plugins/codex-harness) guide; [Where each section moved](/plugins/codex-harness#where-each-section-moved) lists every section.

## App-server policy

By default, the plugin starts OpenAgent's managed Codex binary locally with
...
implicit YOLO posture, OpenAgent selects allowed guardian permissions
instead. When an OpenAgent sandbox is active for the session, OpenAgent
...
Shell access instead goes through OpenAgent sandbox-backed dynamic tools such
as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

Use normalized OpenAgent exec mode for Codex native auto-review before
...
OpenAgent does not preserve legacy unsafe Codex `approvalPolicy: "never"` or
...
works, but `tools.exec.mode: "auto"` is the normalized OpenAgent surface.
...
file prompts use OpenAgent's two-phase operator approval route. The prompt shows
...
Codex auto-review, full-access policy, and native hook or OpenAgent policy
...
When OpenAgent sees a ChatGPT subscription-style Codex auth profile, it
...
If a subscription profile hits a Codex usage limit, OpenAgent records the
...
When native Codex plugins are configured, OpenAgent reads and caches one
...
supplies authenticated app metadata in batches of at most 100 app IDs. OpenAgent
...
OpenAgent provisionally admits only explicitly allowed, ownership-proven apps,
...
The check runs before OpenAgent starts a turn or commits a thread binding. If the
snapshot request fails, a persistent provisional thread is deleted and an
ephemeral thread is unsubscribed. If cleanup cannot be confirmed, OpenAgent retires the app-server
connection instead of reusing an unsafe thread.

Account-wide app access never overrides an explicitly disabled configured
workspace plugin. When `app/read` omits that plugin's ownership, OpenAgent uses
...
OpenAgent does not install unknown apps or let the model authorize new plugin
...
For local stdio app-server launches, OpenAgent sets `CODEX_HOME` to a
...
`~/.codex` by default. OpenAgent preserves the normal process `HOME`;
...
`appServer.homeScope: "user"`, OpenAgent instead uses the native user Codex
home and its existing account without injecting an OpenAgent auth profile.
...
process. OpenAgent removes `CODEX_HOME` and `HOME` from this list during
