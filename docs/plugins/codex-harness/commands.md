...
supports Zero to Agent text commands.
...
- `/codex goal` reads or updates the attached thread's native Codex goal. Codex automatic goal continuation stays disabled; Zero to Agent does not own autonomous follow-on turns yet.
- `/codex resume <thread-id>` attaches the current Zero to Agent session to an
...
The target native thread must be idle. Zero to Agent coordinates attachment, resume,
...
resume` or `/codex bind`. Zero to Agent reports that restriction and keeps the current
...
or Codex cannot confirm that it applied the configuration, Zero to Agent reports
...
If an ordinary Zero to Agent-managed native thread was deleted, the next turn starts
...
`/fast` controls the shared Zero to Agent policy. A directive-only `/fast off`
persists `off` in the Zero to Agent session and sends `null` on affected Codex
harness turns to clear the Zero to Agent-owned service-tier override. `/fast default`
...
is not a synonym for `/fast off`, and it does not change the shared Zero to Agent
