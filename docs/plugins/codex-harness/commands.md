...
supports OpenAgent text commands.
...
- `/codex goal` reads or updates the attached thread's native Codex goal. Codex automatic goal continuation stays disabled; OpenAgent does not own autonomous follow-on turns yet.
- `/codex resume <thread-id>` attaches the current OpenAgent session to an
...
The target native thread must be idle. OpenAgent coordinates attachment, resume,
...
resume` or `/codex bind`. OpenAgent reports that restriction and keeps the current
...
or Codex cannot confirm that it applied the configuration, OpenAgent reports
...
If an ordinary OpenAgent-managed native thread was deleted, the next turn starts
...
`/fast` controls the shared OpenAgent policy. A directive-only `/fast off`
persists `off` in the OpenAgent session and sends `null` on affected Codex
harness turns to clear the OpenAgent-owned service-tier override. `/fast default`
...
is not a synonym for `/fast off`, and it does not change the shared OpenAgent
