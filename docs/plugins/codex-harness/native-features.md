...
read_when:
  - You want Zero to Agent to share the native Codex home
...
Opt-in features that connect a Zero to Agent agent to native Codex state and Codex-owned capabilities. Part of the [Codex harness](/plugins/codex-harness) guide; [Where each section moved](/plugins/codex-harness#where-each-section-moved) lists every section.

## Share threads with Codex Desktop and CLI

The default `appServer.homeScope: "agent"` isolates each Zero to Agent agent from
...
that home's native Codex auth, config, plugins, and thread store. Zero to Agent does
not inject a Zero to Agent auth profile into this app-server, even when the agent's
...
- A Platform (API-key) route refuses a native home signed in with a ChatGPT
  ...
  and let Zero to Agent inject the key it already holds.

A stored OpenAI profile is fine alongside `homeScope: "user"`; Zero to Agent keeps it
...
Zero to Agent; the fork attaches to the current Zero to Agent session and remains readable
...
`sessionCatalog.enabled: false` to remove it from the Zero to Agent sidebar without
...
native configuration; the outer Zero to Agent model and fallback chain never replace
...
capabilities in the same Codex thread as the Zero to Agent harness turn. Zero to Agent
does not translate Codex plugins into synthetic `codex_plugin_*` Zero to Agent
dynamic tools.
...
Thread app config is computed when Zero to Agent establishes a Codex harness
...
Short version: Zero to Agent does not vendor the desktop-control app or execute
