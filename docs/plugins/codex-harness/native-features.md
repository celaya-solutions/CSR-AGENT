...
read_when:
  - You want OpenAgent to share the native Codex home
...
Opt-in features that connect an OpenAgent agent to native Codex state and Codex-owned capabilities. Part of the [Codex harness](/plugins/codex-harness) guide; [Where each section moved](/plugins/codex-harness#where-each-section-moved) lists every section.

## Share threads with Codex Desktop and CLI

The default `appServer.homeScope: "agent"` isolates each OpenAgent agent from
...
that home's native Codex auth, config, plugins, and thread store. OpenAgent does
not inject an OpenAgent auth profile into this app-server, even when the agent's
...
- A Platform (API-key) route refuses a native home signed in with a ChatGPT
  ...
  and let OpenAgent inject the key it already holds.
...
OpenAgent; the fork attaches to the current OpenAgent session and remains readable
...
`sessionCatalog.enabled: false` to remove it from the OpenAgent sidebar without
...
native configuration; the outer OpenAgent model and fallback chain never replace
...
capabilities in the same Codex thread as the OpenAgent harness turn. OpenAgent
does not translate Codex plugins into synthetic `codex_plugin_*` OpenAgent
dynamic tools.
...
Thread app config is computed when OpenAgent establishes a Codex harness
...
Short version: OpenAgent does not vendor the desktop-control app or execute
