...
OpenAI provider policy when the account supports it. For OpenAgent-managed turns,
...
- `agentRuntime.id: "openclaw"` opts a provider or model into the embedded
  OpenAgent runtime when that is intentional.
...
With Codex forced, OpenAgent fails early if the plugin is disabled, the app-server
...
[selection-time OpenAgent fallback](/concepts/agent-runtimes#runtime-selection)
