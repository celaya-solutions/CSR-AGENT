...
| Need                                                | Set                                                                                                       | Where                                   |
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| Enable the harness                                  | `plugins.entries.codex.enabled: true`                                                                     | Zero to Agent config                    |
...
| Keep an allowlisted plugin install                  | Include `codex` in `plugins.allow`                                                                        | Zero to Agent config                    |
...
| Add API-key backup for Codex runs                   | `openai:*` API-key profile listed after subscription auth in `auth.order.openai`                          | CLI auth profile + Zero to Agent config |
| Fail closed when Codex is unavailable               | Provider or model `agentRuntime.id: "codex"`                                                              | Zero to Agent model/provider config     |
| Use direct OpenAI API traffic                       | Provider or model `agentRuntime.id: "openclaw"` with normal OpenAI auth                                   | Zero to Agent model/provider config     |
...
Zero to Agent applies Codex restrictions per turn, not as a permanent session mode.
...
temporary, Zero to Agent preserves the normal thread binding and uses a temporary
restricted thread where necessary.

An ordinary **policy-restricted turn** occurs when an explicit Zero to Agent tool
...
containing only audited Zero to Agent-owned tools can also stay on the normal native
...
For an ordinary policy-restricted turn, Zero to Agent disables Codex native Code
...
MCP tools that pass the effective policy move to Zero to Agent's dynamic surface for
that turn. Other Zero to Agent dynamic tools use the same policy. The bounded workspace `AGENTS.md`
...
**Ring zero** is stronger and separate. It is the host-owned Zero to Agent system
...
normal app-server threads, Zero to Agent raises Codex's aggregate root-to-working-
...
so Zero to Agent supplies the bounded workspace `AGENTS.md` snapshot as thread-level
...
Zero to Agent ignores those local summarizer overrides at runtime, and
...
window. When the model entry has no authored cap, Zero to Agent sends no override.
...
Zero to Agent asks Codex to interrupt the native turn and keeps the per-thread fence
...
For the Zero to Agent Codex app-server harness, keep the default agent-scoped Codex
home and let Zero to Agent inject an `openai` API-key profile. Create the profile by
...
configuration unit. Apply them together. Zero to Agent can keep embedded and native
...
produced `5442` output tokens, and Zero to Agent sent the Codex app-server tier
...
output pricing. Zero to Agent currently sends the wire value
