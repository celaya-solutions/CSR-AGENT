...
OpenAgent applies Codex restrictions per turn, not as a permanent session mode.
...
temporary, OpenAgent preserves the normal thread binding and uses a temporary
restricted thread where necessary.

An ordinary **policy-restricted turn** occurs when an explicit OpenAgent tool
...
containing only audited OpenAgent-owned tools can also stay on the normal native
...
For an ordinary policy-restricted turn, OpenAgent disables Codex native Code
...
MCP tools that pass the effective policy move to OpenAgent's dynamic surface for
that turn. Other OpenAgent dynamic tools use the same policy. The bounded workspace `AGENTS.md`
...
**Ring zero** is stronger and separate. It is the host-owned OpenAgent system
...
normal app-server threads, OpenAgent raises Codex's aggregate root-to-working-
...
so OpenAgent supplies the bounded workspace `AGENTS.md` snapshot as thread-level
...
window. When the model entry has no authored cap, OpenAgent sends no override.
...
OpenAgent asks Codex to interrupt the native turn and keeps the per-thread fence
...
For the OpenAgent Codex app-server harness, keep the default agent-scoped Codex
home and let OpenAgent inject an `openai` API-key profile. Create the profile by
...
configuration unit. Apply them together. OpenAgent can keep embedded and native
...
produced `5442` output tokens, and OpenAgent sent the Codex app-server tier
...
output pricing. OpenAgent currently sends the wire value
