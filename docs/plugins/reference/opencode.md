---
summary: "Adds OpenCode model provider support to OpenAgent."
read_when:
...
Adds OpenCode model provider support to OpenAgent.
...
OpenAgent auto-detects the `opencode` CLI on the Gateway and paired nodes. Stored
...
creates an OpenAgent session whose first turn resumes the native OpenCode session
through ACP. OpenCode retains the full server-side model context, and the catalog
viewer continues to show that history. OpenAgent also imports the recent native
