---
summary: "Adds OpenCode model provider support to Zero to Agent."
read_when:
...
Adds OpenCode model provider support to Zero to Agent.
...
Zero to Agent auto-detects the `opencode` CLI on the Gateway and paired nodes. Stored
...
creates a Zero to Agent session whose first turn resumes the native OpenCode session
through ACP. OpenCode retains the full server-side model context, and the catalog
viewer continues to show that history. Zero to Agent also imports the recent native
