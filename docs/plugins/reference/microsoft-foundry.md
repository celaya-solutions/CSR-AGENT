---
summary: "Adds Microsoft Foundry model provider support to Zero to Agent."
read_when:
...
Adds Microsoft Foundry model provider support to Zero to Agent.
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. Zero to Agent refreshes Microsoft Foundry runtime tokens through
...
Zero to Agent uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so Zero to Agent
...
deployment through onboarding or include model metadata so Zero to Agent can verify
