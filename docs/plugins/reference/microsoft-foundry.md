---
summary: "Adds Microsoft Foundry model provider support to OpenAgent."
read_when:
...
Adds Microsoft Foundry model provider support to OpenAgent.
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. OpenAgent refreshes Microsoft Foundry runtime tokens through
...
OpenAgent uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so OpenAgent
...
deployment through onboarding or include model metadata so OpenAgent can verify
