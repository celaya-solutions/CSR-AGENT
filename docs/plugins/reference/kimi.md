---
summary: "Adds Kimi, Kimi Code, Kimi Coding model provider support to Zero to Agent."
read_when:
  - You are installing, configuring, or auditing the kimi plugin
title: "Kimi plugin reference"
---

<!-- Generated file. Do not edit by hand.
Run `pnpm plugins:inventory:gen` to rebuild it. Hand-written text survives only
between the openclaw-plugin-reference:manual-start and
openclaw-plugin-reference:manual-end comment markers. -->

Adds Kimi, Kimi Code, Kimi Coding model provider support to Zero to Agent.

## Distribution

- Package: `@openclaw/kimi-provider`
- Install route: npm or ClawHub: `clawhub:@openclaw/kimi-provider`

## Surface

- Providers: `kimi`, `kimi-code`, `kimi-coding`

## Related docs

- [kimi](/providers/moonshot)
