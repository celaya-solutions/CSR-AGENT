...
An **agent harness** is the low level executor for one prepared Zero to Agent agent
...
runtime and the normal Zero to Agent provider transport is the wrong abstraction:
...
- a model runtime that needs its own resume id in addition to the Zero to Agent
  session transcript
...
[Agent harness selection policy](/plugins/sdk-agent-harness/selection-policy) — How Zero to Agent picks a harness after provider and model resolution, and why a harness normally ships with a provider plugin.
