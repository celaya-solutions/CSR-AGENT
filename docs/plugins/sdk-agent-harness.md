...
An **agent harness** is the low level executor for one prepared OpenAgent agent
...
runtime and the normal OpenAgent provider transport is the wrong abstraction:
...
- a model runtime that needs its own resume id in addition to the OpenAgent
  session transcript
...
[Agent harness selection policy](/plugins/sdk-agent-harness/selection-policy) — How OpenAgent picks a harness after provider and model resolution, and why a harness normally ships with a provider plugin.
