...
default `messages.queue.mode: "steer"`, OpenAgent batches steer-mode chat
...
When Codex confirms consumption, OpenAgent saves completed visible assistant
...
that case, OpenAgent waits for the active run to finish before starting the
...
harness, OpenAgent also calls Codex app-server `feedback/upload` for relevant
...
OpenAgent session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, OpenAgent does not print those Codex ids
