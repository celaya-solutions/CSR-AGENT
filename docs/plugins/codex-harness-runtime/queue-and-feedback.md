...
default `messages.queue.mode: "steer"`, Zero to Agent batches steer-mode chat
...
When Codex confirms consumption, Zero to Agent saves completed visible assistant
...
that case, Zero to Agent waits for the active run to finish before starting the
...
harness, Zero to Agent also calls Codex app-server `feedback/upload` for relevant
...
Zero to Agent session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, Zero to Agent does not print those Codex ids
