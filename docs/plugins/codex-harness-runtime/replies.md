...
contract: the agent replies normally and Zero to Agent posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable Zero to Agent tool
...
visible answer, Zero to Agent can request a bounded final-answer turn in a private
...
tool-free summary operation that preserves that connection's account. Zero to Agent
