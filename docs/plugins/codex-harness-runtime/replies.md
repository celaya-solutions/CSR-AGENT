...
contract: the agent replies normally and OpenAgent posts the final text to the
...
Codex heartbeat turns get `heartbeat_respond` in the searchable OpenAgent tool
...
visible answer, OpenAgent can request a bounded final-answer turn in a private
...
tool-free summary operation that preserves that connection's account. OpenAgent
