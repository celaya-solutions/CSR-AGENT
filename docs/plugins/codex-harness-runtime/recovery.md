...
What Zero to Agent does about registered Codex app-server children when the Gateway stops without cleaning up. Part of the [Codex harness runtime](/plugins/codex-harness-runtime) guide; [Where each section moved](/plugins/codex-harness-runtime#where-each-section-moved) lists every section.

## Recovery after a hard Gateway stop

On POSIX systems, Zero to Agent checks for registered orphaned Codex app-server
...
Zero to Agent records the parent and child process identities in the current state
...
Cleanup only targets a registered child whose original Zero to Agent parent is no
...
Zero to Agent instance, processes registered under another state directory, and externally
...
unregistered children left by an older Zero to Agent version or scan command names
