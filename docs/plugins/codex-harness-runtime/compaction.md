---
summary: "Native Codex compaction, the Zero to Agent transcript mirror, and continuity projection"
read_when:
...
Who owns compaction on a Codex turn, and what the Zero to Agent transcript mirror keeps. Part of the [Codex harness runtime](/plugins/codex-harness-runtime) guide; [Where each section moved](/plugins/codex-harness-runtime#where-each-section-moved) lists every section.

## Compaction and transcript mirror

When the selected model uses the Codex harness, Codex app-server owns native
token-pressure and manual thread compaction. Zero to Agent separately owns its
transcript mirror. When `agents.defaults.compaction.maxActiveTranscriptBytes`
is set to a positive value, Zero to Agent checks that mirror before ordinary and
heartbeat turns. When the byte guard trips, Zero to Agent requires semantic
...
After host mirror compaction commits, Zero to Agent may request
`thread/compact/start` to synchronize an eligible native thread. This request
is secondary: Zero to Agent does not send it for host-isolated operations or
...
Zero to Agent keeps the request and shared-client lease open until Codex emits the
...
timeout, Zero to Agent requests a native turn interrupt. The lease and per-thread
...
period, Zero to Agent retires the connection before releasing the fence. Remote
...
synchronization described above, Zero to Agent starts native compaction only for
...
If context-engine compaction rotates the Zero to Agent session generation, the next
...
When Zero to Agent projects an existing session's continuity into a fresh Codex
...
When a context engine requests Codex thread-bootstrap projection, Zero to Agent
...
typed `thinking` content rather than ordinary final-answer text, so Zero to Agent's
existing reasoning visibility and history controls apply. Zero to Agent records
...
not rewrite Codex-native tool result records. It only applies when Zero to Agent
writes a Zero to Agent-owned session transcript tool result.
