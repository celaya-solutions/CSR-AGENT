---
summary: "Native Codex compaction, the OpenAgent transcript mirror, and continuity projection"
read_when:
...
Who owns compaction on a Codex turn, and what the OpenAgent transcript mirror keeps. Part of the [Codex harness runtime](/plugins/codex-harness-runtime) guide; [Where each section moved](/plugins/codex-harness-runtime#where-each-section-moved) lists every section.

## Compaction and transcript mirror

When the selected model uses the Codex harness, Codex app-server owns native
token-pressure and manual thread compaction. OpenAgent separately owns its
transcript mirror. When `agents.defaults.compaction.maxActiveTranscriptBytes`
is set to a positive value, OpenAgent checks that mirror before ordinary and
heartbeat turns. When the byte guard trips, OpenAgent requires semantic
...
After host mirror compaction commits, OpenAgent may request
`thread/compact/start` to synchronize an eligible native thread. This request
is secondary: OpenAgent does not send it for host-isolated operations or
...
OpenAgent keeps the request and shared-client lease open until Codex emits the
...
timeout, OpenAgent requests a native turn interrupt. The lease and per-thread
...
period, OpenAgent retires the connection before releasing the fence. Remote
...
synchronization described above, OpenAgent starts native compaction only for
...
If context-engine compaction rotates the OpenAgent session generation, the next
...
When OpenAgent projects an existing session's continuity into a fresh Codex
...
When a context engine requests Codex thread-bootstrap projection, OpenAgent
...
typed `thinking` content rather than ordinary final-answer text, so OpenAgent's
existing reasoning visibility and history controls apply. OpenAgent records
...
not rewrite Codex-native tool result records. It only applies when OpenAgent
writes an OpenAgent-owned session transcript tool result.
