...
Zero to Agent sandbox, sandboxed processes stream ordered stdout, stderr, or PTY
output notifications. Zero to Agent retains only a bounded recent-output buffer for
...
profile used for Zero to Agent worker turns, is separate from the experimental
...
without starting a Zero to Agent worker child or consuming a worker slot. Explicit
