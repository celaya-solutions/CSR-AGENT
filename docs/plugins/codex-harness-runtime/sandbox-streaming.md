...
OpenAgent sandbox, sandboxed processes stream ordered stdout, stderr, or PTY
output notifications. OpenAgent retains only a bounded recent-output buffer for
...
profile used for OpenAgent worker turns, is separate from the experimental
...
without starting an OpenAgent worker child or consuming a worker slot. Explicit
