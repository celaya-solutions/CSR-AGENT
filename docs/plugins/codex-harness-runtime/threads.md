---
summary: "How Zero to Agent binds native Codex threads, changes models, and continues supervised sessions"
read_when:
...
When a Zero to Agent session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The Zero to Agent model picker stays locked,
...
model-difference warning; the outer Zero to Agent model and fallback chain never
...
not resume a thread, subscribe Zero to Agent to its live events, or answer its
approvals.
...
Zero to Agent confirms the fork's subscription is released before starting the canonical
...
resumed. The canonical thread has the full Zero to Agent harness tool surface;
...
Zero to Agent allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. Zero to Agent blocks a known
