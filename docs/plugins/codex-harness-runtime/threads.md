---
summary: "How OpenAgent binds native Codex threads, changes models, and continues supervised sessions"
read_when:
...
When an OpenAgent session is attached to an existing Codex thread, the next
...
Supervised bindings are the exception. The OpenAgent model picker stays locked,
...
model-difference warning; the outer OpenAgent model and fallback chain never
...
not resume a thread, subscribe OpenAgent to its live events, or answer its
approvals.
...
OpenAgent confirms the fork's subscription is released before starting the canonical
...
resumed. The canonical thread has the full OpenAgent harness tool surface;
...
OpenAgent allows archive for a local `idle` or `notLoaded` row only after explicit
...
prove that another process is not using the thread. OpenAgent blocks a known
