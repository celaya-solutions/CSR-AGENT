...
read_when:
  - You are routing Codex native permission requests through OpenAgent
...
How native permission requests and MCP elicitations reach the OpenAgent approval flow. Part of the [Codex harness runtime](/plugins/codex-harness-runtime) guide; [Where each section moved](/plugins/codex-harness-runtime#where-each-section-moved) lists every section.

## Native permissions and MCP elicitations

For `PermissionRequest`, OpenAgent only returns explicit allow or deny
...
request, OpenAgent remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through OpenAgent's plugin approval
...
OpenAgent supports app-server MCP elicitation modes `form`, `openai/form`, and
`url`. Standard and extended forms can contain at most 12 fields. OpenAgent
...
to four bounded item IDs and titles. OpenAgent uses only those IDs and titles; it
...
choices. OpenAgent does not fetch or open the URL. URLs are limited to 2,048
...
Gateway question records. OpenAgent does not infer secrecy from field names.
...
OpenAgent preserves the questions on the async assistant message, and the Control
