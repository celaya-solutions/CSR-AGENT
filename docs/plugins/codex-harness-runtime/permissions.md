...
read_when:
  - You are routing Codex native permission requests through Zero to Agent
...
How native permission requests and MCP elicitations reach the Zero to Agent approval flow. Part of the [Codex harness runtime](/plugins/codex-harness-runtime) guide; [Where each section moved](/plugins/codex-harness-runtime#where-each-section-moved) lists every section.

## Native permissions and MCP elicitations

For `PermissionRequest`, Zero to Agent only returns explicit allow or deny
...
request, Zero to Agent remembers that exact provider/session/tool input/cwd
...
Codex MCP tool approval elicitations route through Zero to Agent's plugin approval
...
Zero to Agent supports app-server MCP elicitation modes `form`, `openai/form`, and
`url`. Standard and extended forms can contain at most 12 fields. Zero to Agent
...
to four bounded item IDs and titles. Zero to Agent uses only those IDs and titles; it
...
choices. Zero to Agent does not fetch or open the URL. URLs are limited to 2,048
...
Gateway question records. Zero to Agent does not infer secrecy from field names.
...
Zero to Agent preserves the questions on the async assistant message, and the Control
