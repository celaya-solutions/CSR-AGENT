...
`systemPrompt` replaces the complete Zero to Agent generic developer policy. An explicit
...
those transports. Use Zero to Agent-managed local stdio; for lease contention, stop
...
| Layer                                 | Owner                         | Purpose                                                                |
| ------------------------------------- | ----------------------------- | ---------------------------------------------------------------------- |
| Zero to Agent plugin hooks            | Zero to Agent                 | Product/plugin compatibility across Zero to Agent and Codex harnesses. |
| Codex app-server extension middleware | Zero to Agent bundled plugins | Per-turn adapter behavior around Zero to Agent dynamic tools.          |
| Codex native hooks                    | Codex                         | Low-level Codex lifecycle and native tool policy from Codex config.    |

Zero to Agent does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, Zero to Agent injects
per-thread Codex config for `PreToolUse`, `PostToolUse`, `PermissionRequest`,
and `Stop`.

Before starting or resuming a thread, Zero to Agent prepares the native relay's
...
so Codex's app-server reviewer and Zero to Agent's approval bridge handle real
...
controls; they are not exposed as Zero to Agent plugin hooks in the v1 contract.

For Zero to Agent dynamic tools, Zero to Agent executes the tool after Codex asks for
...
canonical tool record; Zero to Agent can mirror selected events but cannot rewrite
...
matching app-server approval. If a Zero to Agent `before_tool_call` hook returns
...
request for the same tool use, Zero to Agent opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through Zero to Agent approvals when
...
notifications and Zero to Agent adapter state, not native Codex hook commands.
...
debugging. They do not invoke Zero to Agent plugin hooks.
