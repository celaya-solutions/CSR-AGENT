...
`systemPrompt` replaces the complete OpenAgent generic developer policy. An explicit
...
those transports. Use OpenAgent-managed local stdio; for lease contention, stop
...
| Layer                                 | Owner                         | Purpose                                                                |
| ------------------------------------- | ----------------------------- | ---------------------------------------------------------------------- |
| OpenAgent plugin hooks            | OpenAgent                 | Product/plugin compatibility across OpenAgent and Codex harnesses. |
| Codex app-server extension middleware | OpenAgent bundled plugins | Per-turn adapter behavior around OpenAgent dynamic tools.          |
| Codex native hooks                    | Codex                         | Low-level Codex lifecycle and native tool policy from Codex config.    |

OpenAgent does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, OpenAgent injects
per-thread Codex config for `PreToolUse`, `PostToolUse`, `PermissionRequest`,
and `Stop`.

Before starting or resuming a thread, OpenAgent prepares the native relay's
...
so Codex's app-server reviewer and OpenAgent's approval bridge handle real
...
controls; they are not exposed as OpenAgent plugin hooks in the v1 contract.

For OpenAgent dynamic tools, OpenAgent executes the tool after Codex asks for
...
canonical tool record; OpenAgent can mirror selected events but cannot rewrite
...
matching app-server approval. If an OpenAgent `before_tool_call` hook returns
...
request for the same tool use, OpenAgent opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through OpenAgent approvals when
...
notifications and OpenAgent adapter state, not native Codex hook commands.
...
debugging. They do not invoke OpenAgent plugin hooks.
