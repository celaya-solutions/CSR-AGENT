---
summary: "How Zero to Agent dynamic tools are exposed to Codex app-server turns"
read_when:
  - You need to know which Zero to Agent tools Codex can call
...
Which Zero to Agent dynamic tools reach a Codex turn, and how they are loaded. Part of the [Codex harness reference](/plugins/codex-harness-reference); [Where each section moved](/plugins/codex-harness-reference#where-each-section-moved) lists every section.

## Dynamic tools

Codex dynamic tools default to `searchable` loading, exposed under the
`openclaw` namespace with `deferLoading: true`. Zero to Agent normally does not
...
available through the Zero to Agent dynamic-tool bridge as the durable session status
surface.

When a finite runtime allowlist disables native Code Mode, Zero to Agent sends an
empty execution-environment selection. In that direct, unsandboxed case,
Zero to Agent keeps its policy-filtered `exec` and `process` tools as the shell
fallback. Runtime allowlists and `codexDynamicToolsExclude` still apply.

Most remaining Zero to Agent integration tools, such as messaging, media, cron,
...
`sessions_spawn` remains available for explicit Zero to Agent or ACP delegation.
Message-tool-only source replies also stay direct, since that is a
turn-control contract.

Codex Code Mode projects generic Zero to Agent dynamic-tool results as text. Parse a
...
Zero to Agent adds that namespace to Codex's
