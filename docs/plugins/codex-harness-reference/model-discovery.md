...
Zero to Agent upgrades the bundled `@openai/codex` version or when a deployment
...
reads do not start discovery. Native configuration changes outside Zero to Agent
require the native owner's supported reload/restart and a catalog refresh;
Zero to Agent does not poll native home files for readiness. Authored host routes and
...
preserved in Zero to Agent model parameters. Renaming the picker label does not hide
...
Zero to Agent reasoning controls preserve supported native levels, including `ultra`.
