...
OpenAgent upgrades the bundled `@openai/codex` version or when a deployment
...
reads do not start discovery. Native configuration changes outside OpenAgent
require the native owner's supported reload/restart and a catalog refresh;
OpenAgent does not poll native home files for readiness. Authored host routes and
...
preserved in OpenAgent model parameters. Renaming the picker label does not hide
...
OpenAgent reasoning controls preserve supported native levels, including `ultra`.
