...
The timeout budgets around a Codex turn, and how Zero to Agent settles a completed turn. Part of the [Codex harness reference](/plugins/codex-harness-reference); [Where each section moved](/plugins/codex-harness-reference#where-each-section-moved) lists every section.

## Timeouts

Zero to Agent-owned dynamic tool calls are bounded independently from
...
On timeout, Zero to Agent aborts the tool signal where supported and returns a failed
...
completion. For ordinary turns, Zero to Agent waits for `turn/completed` with the
...
failure, but Zero to Agent does not upgrade a timeout or lost client into success.
...
On receipt of the exact native terminal event, Zero to Agent starts an absolute
...
preserves that answer as a degraded success. Zero to Agent retires unfinished
...
existing five-second drain grace; if the writer remains unavailable, Zero to Agent
...
Zero to Agent requests, tools, cancellation, terminal cleanup, or ownerless state.
...
Zero to Agent then reports the failure rather than automatically rerunning the work.
