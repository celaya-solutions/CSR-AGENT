...
The timeout budgets around a Codex turn, and how OpenAgent settles a completed turn. Part of the [Codex harness reference](/plugins/codex-harness-reference); [Where each section moved](/plugins/codex-harness-reference#where-each-section-moved) lists every section.

## Timeouts

OpenAgent-owned dynamic tool calls are bounded independently from
...
On timeout, OpenAgent aborts the tool signal where supported and returns a failed
...
completion. For ordinary turns, OpenAgent waits for `turn/completed` with the
...
failure, but OpenAgent does not upgrade a timeout or lost client into success.
...
On receipt of the exact native terminal event, OpenAgent starts an absolute
...
preserves that answer as a degraded success. OpenAgent retires unfinished
...
existing five-second drain grace; if the writer remains unavailable, OpenAgent
...
OpenAgent requests, tools, cancellation, terminal cleanup, or ownerless state.
...
OpenAgent then reports the failure rather than automatically rerunning the work.
