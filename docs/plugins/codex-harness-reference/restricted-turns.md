...
Codex-native capability outside the Zero to Agent policy boundary.
...
the harness applies the named Zero to Agent denial directly. Any other deny fails
...
Zero to Agent disables inherited and configured MCP servers, attests that they remain
...
agent path used by Zero to Agent setup and repair flows. The host must activate the
...
Zero to Agent's `AGENTS.md` developer-instruction carrier, so ambient workspace
