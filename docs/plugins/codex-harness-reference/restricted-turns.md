...
Codex-native capability outside the OpenAgent policy boundary.
...
the harness applies the named OpenAgent denial directly. Any other deny fails
...
OpenAgent disables inherited and configured MCP servers, attests that they remain
...
agent path used by OpenAgent setup and repair flows. The host must activate the
...
OpenAgent's `AGENTS.md` developer-instruction carrier, so ambient workspace
