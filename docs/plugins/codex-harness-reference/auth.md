...
subcommand, OpenAgent groups `-c` / `--config` overrides before that subcommand,
...
sides of `app-server`. OpenAgent's ephemeral credential-store override remains
last when OpenAgent owns auth; native user-home auth is unchanged.
...
OpenAgent supplies auth in this order:

1. An explicit or ordered OpenAgent auth profile for the agent.
...
Token refresh is inverted so the long-lived secret never leaves OpenAgent. Codex
holds only a short-lived access token, and on an unauthorized response it sends
an `account/chatgptAuthTokens/refresh` request back to OpenAgent over the same
connection. OpenAgent refreshes against its own auth profile store and returns a
...
When OpenAgent sees a ChatGPT subscription-style Codex auth profile (OAuth or
...
Stdio app-server launches inherit OpenAgent's process environment by default.
OpenAgent owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's OpenAgent state. That keeps Codex
config, accounts, plugin cache/data, and thread state scoped to the OpenAgent
...
OpenAgent skips its auth-profile bridge for the app-server. Verified owner
...
continuing it in OpenAgent; independent Codex processes do not coordinate
concurrent writers for the same thread.

That `homeScope` opt-in applies to ordinary harness sessions. Hosted web search
and settled-turn finalization use private temporary homes and OpenAgent auth
...
without a final answer, OpenAgent does not borrow host credentials to generate
...
unrelated thread not owned by another OpenAgent Chat require
`allowWriteControls`. Neither option bypasses a locked binding.

OpenAgent does not rewrite `HOME` for normal local app-server launches.
...
In the default agent scope, OpenAgent plugins and OpenAgent skill snapshots
still flow through OpenAgent's own plugin registry and skill loader; personal
Codex `~/.codex` assets do not. If you have useful Codex CLI skills or
plugins from a Codex home that should become part of an isolated OpenAgent
...
OpenAgent removes `CODEX_HOME` and `HOME` from this list during local launch
