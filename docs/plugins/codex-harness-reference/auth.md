...
subcommand, Zero to Agent groups `-c` / `--config` overrides before that subcommand,
...
sides of `app-server`. Zero to Agent's ephemeral credential-store override remains
last when Zero to Agent owns auth; native user-home auth is unchanged.
...
Zero to Agent supplies auth in this order:

1. An explicit or ordered Zero to Agent auth profile for the agent.
...
Token refresh is inverted so the long-lived secret never leaves Zero to Agent. Codex
holds only a short-lived access token, and on an unauthorized response it sends
an `account/chatgptAuthTokens/refresh` request back to Zero to Agent over the same
connection. Zero to Agent refreshes against its own auth profile store and returns a
...
When Zero to Agent sees a ChatGPT subscription-style Codex auth profile (OAuth or
...
Stdio app-server launches inherit Zero to Agent's process environment by default.
Zero to Agent owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's Zero to Agent state. That keeps Codex
config, accounts, plugin cache/data, and thread state scoped to the Zero to Agent
...
Zero to Agent skips its auth-profile bridge for the app-server. Verified owner
...
continuing it in Zero to Agent; independent Codex processes do not coordinate
concurrent writers for the same thread.

That `homeScope` opt-in applies to ordinary harness sessions. Hosted web search
and settled-turn finalization use private temporary homes and Zero to Agent auth
...
without a final answer, Zero to Agent does not borrow host credentials to generate
...
unrelated thread not owned by another Zero to Agent Chat require
`allowWriteControls`. Neither option bypasses a locked binding.

Zero to Agent does not rewrite `HOME` for normal local app-server launches.
...
In the default agent scope, Zero to Agent plugins and Zero to Agent skill snapshots
still flow through Zero to Agent's own plugin registry and skill loader; personal
Codex `~/.codex` assets do not. If you have useful Codex CLI skills or
plugins from a Codex home that should become part of an isolated Zero to Agent
...
Zero to Agent removes `CODEX_HOME` and `HOME` from this list during local launch
