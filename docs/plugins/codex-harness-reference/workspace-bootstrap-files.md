...
uninterrupted managed local stdio process owner and observed native unload before OpenAgent injects the full
...
OpenAgent does not write synthetic Codex project-doc files or depend on Codex
...
filesystem environment, so OpenAgent instead sends the bounded workspace
...
For OpenAgent workspace parity, local tool notes live in the `## Tools` section
...
- The compact loaded OpenAgent skills list uses the same parent-local layer.
...
workaround is not active. OpenAgent does not reroute or shut down those sessions.
