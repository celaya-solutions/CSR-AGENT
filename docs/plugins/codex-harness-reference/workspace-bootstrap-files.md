...
uninterrupted managed local stdio process owner and observed native unload before Zero to Agent injects the full
...
Zero to Agent does not write synthetic Codex project-doc files or depend on Codex
...
filesystem environment, so Zero to Agent instead sends the bounded workspace
...
For Zero to Agent workspace parity, local tool notes live in the `## Tools` section
...
- The compact loaded Zero to Agent skills list uses the same parent-local layer.
...
workaround is not active. Zero to Agent does not reroute or shut down those sessions.
