...
read_when:
  ...
  - You are running Codex native execution inside an OpenAgent sandbox
...
unattended OpenAgent turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, OpenAgent treats the implicit default as guardian
...
When an OpenAgent sandbox is active, the local Codex app-server process still
runs on the Gateway host. OpenAgent therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the OpenAgent sandbox
backend. Shell access is exposed through OpenAgent sandbox-backed dynamic tools
...
Ubuntu/AppArmor hosts. Fix the reported host namespace policy for the OpenAgent
...
The stable default is fail-closed: active OpenAgent sandboxing disables native
...
to try Codex's remote environment support with OpenAgent's sandbox backend.
...
When the flag is on and the current OpenAgent session is sandboxed, OpenAgent
...
OpenAgent-owned environment. If the app-server cannot register the environment,
...
notifications. OpenAgent retains only a bounded recent-output buffer for polling
...
the loopback exec-server unless it is running on the same host, so OpenAgent
...
OpenAgent worker slot. HTTP requests containing authentication, cookies, API
