...
read_when:
  ...
  - You are running Codex native execution inside a Zero to Agent sandbox
...
unattended Zero to Agent turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, Zero to Agent treats the implicit default as guardian
...
When a Zero to Agent sandbox is active, the local Codex app-server process still
runs on the Gateway host. Zero to Agent therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the Zero to Agent sandbox
backend. Shell access is exposed through Zero to Agent sandbox-backed dynamic tools
such as `sandbox_exec` and `sandbox_process` when the normal exec/process tools
are available.

<Note>
On Docker-backed Zero to Agent sandbox hosts (`agents.defaults.sandbox.mode` set to
...
Ubuntu/AppArmor hosts. Fix the reported host namespace policy for the Zero to Agent
...
The stable default is fail-closed: active Zero to Agent sandboxing disables native
...
to try Codex's remote environment support with Zero to Agent's sandbox backend.
...
When the flag is on and the current Zero to Agent session is sandboxed, Zero to Agent
...
Zero to Agent-owned environment. If the app-server cannot register the environment,
...
notifications. Zero to Agent retains only a bounded recent-output buffer for polling
...
the loopback exec-server unless it is running on the same host, so Zero to Agent
...
Zero to Agent worker slot. HTTP requests containing authentication, cookies, API
