...
`~/.codex`) and the Codex home of every configured OpenAgent agent. Register
additional local Codex stores only when sessions live in a home OpenAgent does
...
and the selected OpenAgent agent still owns the resulting connection; `homes`
...
resumes omit OpenAgent model and provider overrides so Codex restores the
...
OpenAgent binding owns the exact target or one of its non-archived spawned
descendants. OpenAgent follows Codex's descendant pagination and fails closed on
