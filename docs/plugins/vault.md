...
read_when:
  - You want Zero to Agent to read API keys from HashiCorp Vault
...
The bundled Vault plugin lets Zero to Agent resolve `exec` SecretRefs from
HashiCorp Vault at Gateway startup and reload time. Zero to Agent stores Vault
...
Zero to Agent config files. For the SecretRef runtime model, see
...
Zero to Agent defaults to KV v2 mounted at `secret`, matching Vault dev-server
examples. For production Vault, set `OPENCLAW_VAULT_KV_MOUNT` to your actual KV
mount path before creating SecretRef ids. With the Zero to Agent defaults, this
SecretRef id:
...
Use a scoped client token for Zero to Agent, not a root token. For the default KV v2
...
that starts Zero to Agent. The default auth method reads a Vault client token from
`VAULT_TOKEN`:
...
These variables must be present when Zero to Agent starts. The Vault plugin forwards
...
Use `--allow-exec` because the Vault plugin resolves through a Zero to Agent-managed
...
The target path must be a registered Zero to Agent SecretRef target. The setup
command does not create arbitrary named secrets in Zero to Agent; Vault remains the
secret store, and Zero to Agent stores SecretRefs only on supported config fields.
...
deployment config. Zero to Agent does not require a fixed mount, role, or path: each
