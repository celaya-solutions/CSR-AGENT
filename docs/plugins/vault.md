...
read_when:
  - You want OpenAgent to read API keys from HashiCorp Vault
...
The bundled Vault plugin lets OpenAgent resolve `exec` SecretRefs from
HashiCorp Vault at Gateway startup and reload time. OpenAgent stores Vault
...
OpenAgent config files. For the SecretRef runtime model, see
...
OpenAgent defaults to KV v2 mounted at `secret`, matching Vault dev-server
examples. For production Vault, set `OPENCLAW_VAULT_KV_MOUNT` to your actual KV
mount path before creating SecretRef ids. With the OpenAgent defaults, this
SecretRef id:
...
Use a scoped client token for OpenAgent, not a root token. For the default KV v2
...
that starts OpenAgent. The default auth method reads a Vault client token from
`VAULT_TOKEN`:
...
These variables must be present when OpenAgent starts. The Vault plugin forwards
...
Use `--allow-exec` because the Vault plugin resolves through an OpenAgent-managed
...
The target path must be a registered OpenAgent SecretRef target. The setup
command does not create arbitrary named secrets in OpenAgent; Vault remains the
secret store, and OpenAgent stores SecretRefs only on supported config fields.
...
deployment config. OpenAgent does not require a fixed mount, role, or path: each
