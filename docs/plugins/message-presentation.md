...
Message presentation is OpenAgent's shared contract for rich outbound chat UI.
...
- `action.type: "question"` identifies one choice for a live, runtime-authored
  `ask_user` question. Like `approval`, this is an OpenAgent runtime action;
  agents and plugins must not synthesize question IDs. Telegram and Discord
  map it to transport-private native callbacks and resolve the choice through
  the Gateway. When the question becomes answered, expired, or cancelled,
  Telegram and Discord edit the delivered message, remove its actions, and
  append the terminal status. Channels without native controls degrade to
  label text, and the user can answer with a plain-text reply.
- `intent: "custom-input"` switches a live question to its free-text answer
  ...
  Telegram maps it to **Other…** and Force Reply. Discord keeps the visible
  text route.
- `action.type: "url"` opens a normal link.
- `action.type: "web-app"` launches a channel-native web app. Set `url` for a
  URL-backed app or `widgetId` for an OpenAgent-hosted widget whose launch
...
Channel adapters that add manual-command guidance in their fallback UI must derive the command-present check
