...
Message presentation is Zero to Agent's shared contract for rich outbound chat UI.
...
- `action.type: "question"` identifies one choice for a live, runtime-authored
  `ask_user` question. Like `approval`, this is a Zero to Agent runtime action;
...
- `action.type: "web-app"` launches a channel-native web app. Set `url` for a
  URL-backed app or `widgetId` for a Zero to Agent-hosted widget whose launch
