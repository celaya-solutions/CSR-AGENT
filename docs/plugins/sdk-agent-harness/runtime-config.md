...
The bundled `codex` harness is the native Codex mode for embedded Zero to Agent
...
compaction, and app-server execution. Zero to Agent still owns the chat channel,
...
By default, Zero to Agent uses `auto` provider/model runtime policy: registered
...
declared fallback. An authored transport override may select Zero to Agent through
