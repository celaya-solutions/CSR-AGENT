...
The bundled `codex` harness is the native Codex mode for embedded OpenAgent
...
compaction, and app-server execution. OpenAgent still owns the chat channel,
...
By default, OpenAgent uses `auto` provider/model runtime policy: registered
...
declared fallback. An authored transport override may select OpenAgent through
