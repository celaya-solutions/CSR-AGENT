...
harness; Zero to Agent does not replay the request through another runtime.
...
Implement V2 for harness-owned or native authentication; Zero to Agent never invents
a host credential when only the legacy capability is present.

Native agent servers often have ambient built-in tools even when Zero to Agent sends
...
Audit evidence follows the same boundary. Zero to Agent can record registered plugin
...
`unsupported` unless an adapter invokes a Zero to Agent-owned callback before the
...
Delegates receive only work admission and embedded execution. Zero to Agent requires
