...
harness; OpenAgent does not replay the request through another runtime.
...
Implement V2 for harness-owned or native authentication; OpenAgent never invents
a host credential when only the legacy capability is present.

Native agent servers often have ambient built-in tools even when OpenAgent sends
...
Audit evidence follows the same boundary. OpenAgent can record registered plugin
...
`unsupported` unless an adapter invokes an OpenAgent-owned callback before the
...
Delegates receive only work admission and embedded execution. OpenAgent requires
