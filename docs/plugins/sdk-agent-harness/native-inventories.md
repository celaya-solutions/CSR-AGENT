...
read_when:
  - You are reporting native models to the Zero to Agent model picker
...
The read-only catalogs a harness exposes to Zero to Agent control surfaces: native model rows with their readiness, and MCP tool inventory owned outside the in-process MCP runtime. Part of the [Agent harness plugins](/plugins/sdk-agent-harness) reference.
...
A harness that owns MCP connections outside Zero to Agent's in-process MCP runtime
...
MCP overrides. `mcpServerNames` is the bounded set of Zero to Agent-configured
servers whose session policy the harness may represent. Return Zero to Agent's
...
final Zero to Agent tool policy and schema compatibility checks before exposing the
rows.
