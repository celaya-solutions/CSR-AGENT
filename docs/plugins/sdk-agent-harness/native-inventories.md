...
read_when:
  - You are reporting native models to the OpenAgent model picker
...
The read-only catalogs a harness exposes to OpenAgent control surfaces: native model rows with their readiness, and MCP tool inventory owned outside the in-process MCP runtime. Part of the [Agent harness plugins](/plugins/sdk-agent-harness) reference.
...
A harness that owns MCP connections outside OpenAgent's in-process MCP runtime
...
MCP overrides. `mcpServerNames` is the bounded set of OpenAgent-configured
servers whose session policy the harness may represent. Return OpenAgent's
...
final OpenAgent tool policy and schema compatibility checks before exposing the
rows.
