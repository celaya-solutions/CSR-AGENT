...
the prompt, deliver it through Zero to Agent's blocking reply path, and normalize
...
Zero to Agent's own blocking question tools — `ask_user`, and a `secrets` request —
...
object. Use `bindToolSurface(...)` before exposing plugin-built Zero to Agent tools,
...
Zero to Agent Code Mode's joined `agents.run()` path retains internal waiting; this
