...
the prompt, deliver it through OpenAgent's blocking reply path, and normalize
...
OpenAgent's own blocking question tools — `ask_user`, and a `secrets` request —
...
object. Use `bindToolSurface(...)` before exposing plugin-built OpenAgent tools,
...
OpenAgent Code Mode's joined `agents.run()` path retains internal waiting; this
