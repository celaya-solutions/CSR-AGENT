---
summary: "How Zero to Agent picks a harness after provider and model resolution, and why most harnesses also register a provider"
read_when:
...
How Zero to Agent chooses a harness after provider and model resolution, what a failure does to the model-fallback chain, and why a harness normally ships alongside a provider plugin. Part of the [Agent harness plugins](/plugins/sdk-agent-harness) reference.

## Selection policy

Zero to Agent chooses a harness after provider/model resolution:
...
4. If no registered harness matches, Zero to Agent uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, Zero to Agent does not
...
harness would repeat it. Zero to Agent records the actual selected harness at the
...
facts, declare the exact-request Zero to Agent fallback, or fail closed.
...
Zero to Agent. The harness then claims that provider in `supports(...)`.
...
- app-server request: Zero to Agent sends the bare model id to Codex and lets the
...
behavior stay on Zero to Agent. Plaintext official HTTP endpoints are rejected. Older `codex/gpt-*`
