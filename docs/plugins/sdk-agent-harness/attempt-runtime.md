...
seam is for async tool-result transforms that must run before OpenAgent or
...
canonical OpenAgent tool ids; omit `matcher` to match all tools.
...
`planning-only` so OpenAgent's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; OpenAgent does not infer it from assistant prose. The helper
...
dispatches the portable `agent_end` hook and OpenAgent's research capture
