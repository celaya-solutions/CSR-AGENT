...
seam is for async tool-result transforms that must run before Zero to Agent or
...
canonical Zero to Agent tool ids; omit `matcher` to match all tools.
...
`planning-only` so Zero to Agent's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; Zero to Agent does not infer it from assistant prose. The helper
...
dispatches the portable `agent_end` hook and Zero to Agent's research capture
