---
summary: "Native session bindings, the Zero to Agent transcript mirror, tool and media result delivery, terminal tool outcomes, and settled-turn finalization"
read_when:
...
How a native session binds to a Zero to Agent session and mirrors into its transcript, and how tool, media, terminal-outcome, and settled-turn results come back through the attempt result. Part of the [Agent harness plugins](/plugins/sdk-agent-harness) reference.

## Native sessions and transcript mirror

A harness may keep a native session id, thread id, or daemon-side resume
token. Keep that binding explicitly associated with the Zero to Agent session, and
keep mirroring user-visible assistant/tool output into the Zero to Agent
transcript.

The Zero to Agent transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in Zero to Agent harness on a later turn
...
routes history to its existing Zero to Agent child session or the owning harness.
...
Keep `childSessionKey` absent for native children: it describes a Zero to Agent
...
Core constructs the Zero to Agent tool list and passes it into the prepared
...
outputs on the same delivery path as Zero to Agent-backed runs.
...
Zero to Agent-tool media. On `message_tool_only` routes, this narrow provenance lets
...
outcome accumulator. A harness that executes Zero to Agent dynamic tools or native
...
- Report `outcome: "success"` or `outcome: "failure"`. Include the structured
  ...
  display text.
- Use `nativeMutation` only for native tools that do not use a Zero to Agent tool
  definition. Supply protocol-owned mutation and replay facts there; do not
  copy Zero to Agent's mutation classifier into the harness.
...
without terminal reports, Zero to Agent cannot preserve mutating-tool failure truth
across later tool calls, including quiet heartbeat completion.

## Settled tool finalization

Zero to Agent may need one final visible answer after a harness has completed every
...
capability boundary. Zero to Agent does not provide a generic fallback because it
...
compatibility. When the selected harness omits it, Zero to Agent preserves the
