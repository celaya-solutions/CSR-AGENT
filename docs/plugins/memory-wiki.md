...
turn it into OpenAgent-native concept pages and compiled digests.
...
snapshot in OpenAgent's shared SQLite plugin state. Runtime code uses the
...
In this scope, `vault.path` is a parent directory and OpenAgent appends the
normalized agent id:
