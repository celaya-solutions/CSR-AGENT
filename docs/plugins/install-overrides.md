...
When a setup-time flow installs a plugin whose id appears in the map, Zero to Agent
...
- Overrides do not inherit official trusted-source status. Even when the
  catalog entry normally represents a Zero to Agent-owned package, an override is
...
- Workspace `.env` files cannot enable install overrides; both env vars are on
  ...
  remote test command that launches Zero to Agent.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Zero to Agent state:
