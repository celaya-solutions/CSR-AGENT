...
read_when:
  - You want to build a simple Zero to Agent plugin that only adds agent tools
...
manifest metadata Zero to Agent needs to discover tools without loading plugin
runtime code.
...
`toolMetadata.<tool>.optional` manifest entry, so Zero to Agent can see that the
...
final tool result still flow normally. Zero to Agent preserves the current factory's
...
`defineToolPlugin` wraps plain return values into the Zero to Agent tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and Zero to Agent to keep the original value in `details`.
...
Zero to Agent compiles the schema before executing a catalog call, then validates the
...
Zero to Agent also grades the call outcome from `details`, so `status`, `ok`,
...
`configSchema` is optional. Omit it and Zero to Agent applies a strict empty object
...
Zero to Agent reads plugin config from the plugin's entry in the Gateway config. Do
...
Zero to Agent must read the plugin manifest before importing plugin runtime code.
...
`contracts.tools` is the important discovery contract: it tells Zero to Agent which
...
Zero to Agent SDK compatibility fields carry TypeScript `@deprecated` annotations,
...
From a separate Zero to Agent checkout or installed CLI, install the package path:
...
discovery and distribution surface for Zero to Agent plugins. See [ClawHub publishing](/clawhub/publishing) for owner scope and
release review.
