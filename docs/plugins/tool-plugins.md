...
read_when:
  - You want to build a simple OpenAgent plugin that only adds agent tools
...
manifest metadata OpenAgent needs to discover tools without loading plugin
runtime code.
...
final tool result still flow normally. OpenAgent preserves the current factory's
...
`defineToolPlugin` wraps plain return values into the OpenAgent tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and OpenAgent to keep the original value in `details`.
...
OpenAgent compiles the schema before executing a catalog call, then validates the
...
OpenAgent also grades the call outcome from `details`, so `status`, `ok`,
...
`configSchema` is optional. Omit it and OpenAgent applies a strict empty object
...
OpenAgent reads plugin config from the plugin's entry in the Gateway config. Do
...
`contracts.tools` is the important discovery contract: it tells OpenAgent which
