---
summary: "Create your first Zero to Agent plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new Zero to Agent plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend Zero to Agent without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the Zero to Agent repository. Publish
...
- All plugin APIs are [experimental](/plugins/sdk-overview#api-stability).
  Pin your Zero to Agent host version and test each version you declare compatible.
- Node 24.16+ or Node 26.1+, and `npm` or `pnpm`.
- TypeScript ESM modules.
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because Zero to Agent discovers
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Zero to Agent to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through Zero to Agent model fallback.
  </Card>
...
```json openclaw.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to Zero to Agent",
  "categories": ["other"],
  "contracts": {
    "tools": ["my_tool"]
  },
...
```
    ...
    appear in `contracts.tools` so Zero to Agent can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    For a bundled plugin in this repository, Zero to Agent discovers source-checkout
  ...
  <Step title="Test the package install">
    ...
    `npm-pack:` uses Zero to Agent's managed per-plugin npm project, so it catches
...
plugin is enabled. Optional tools need explicit user opt-in before Zero to Agent
...
turn or when the channel uses Gateway-owned delivery. Zero to Agent binds the route,
...
operator, installed plugin code, or a modified Zero to Agent runtime. Sensitive
...
aligned with `api.registerTool(..., { optional: true })` so Zero to Agent can avoid
