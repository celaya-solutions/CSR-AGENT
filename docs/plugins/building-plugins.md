---
summary: "Create your first OpenAgent plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new OpenAgent plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend OpenAgent without changing core. A plugin can add a messaging
...
- All plugin APIs are [experimental](/plugins/sdk-overview#api-stability).
  Pin your OpenAgent host version and test each version you declare compatible.
- Node 24.16+ or Node 26.1+, and `npm` or `pnpm`.
- TypeScript ESM modules.
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because OpenAgent discovers
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect OpenAgent to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through OpenAgent model fallback.
  </Card>
...
```json openclaw.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to OpenAgent",
  "categories": ["other"],
  "contracts": {
    "tools": ["my_tool"]
  },
...
```
    ...
    appear in `contracts.tools` so OpenAgent can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    For a bundled plugin in this repository, OpenAgent discovers source-checkout
...
plugin is enabled. Optional tools need explicit user opt-in before OpenAgent
...
turn or when the channel uses Gateway-owned delivery. OpenAgent binds the route,
...
operator, installed plugin code, or a modified OpenAgent runtime. Sensitive
...
aligned with `api.registerTool(..., { optional: true })` so OpenAgent can avoid
...
OpenAgent SDK compatibility fields carry TypeScript `@deprecated` annotations,
