...
read_when:
  ...
  - You want Zero to Agent config credentials to resolve from 1Password
  - You need per-secret approval policy and audit history
  - You are configuring a 1Password service account for Zero to Agent
...
- Curated agent registry only. Agents can list configured slugs, but the plugin
  ...
  explicitly stored on registered Zero to Agent credential targets.
- Per-slug `auto`, `approve`, or `deny` policy.
- Approval grants expire. A cached value never bypasses current policy.
- Every access attempt is recorded in Zero to Agent's shared SQLite state. Audit
...
- After the current tool execution, Zero to Agent-owned transcript persistence
...
Create the token directory and file under the Zero to Agent state directory:
...
Zero to Agent's shared exec-id grammar in a plugin-local opaque form and decodes them
...
approval. Zero to Agent offers allow always only when the caller has a concrete agent
